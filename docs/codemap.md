...
- [x] shell 侧：`weepcode-shell/src/agent/auth_method.rs` 新增非交互方法 `provider.setup`（暂用此 id，
      ACP 扩展方法命名同步去 x.ai 前缀），在无 WeepCode 凭证且无 BYOK 模型时作为首选项通告
...
目标：用户能看到的一切 "weepcode/WeepCode Build/WeepCode" 字样替换或移除；内部 crate/env/目录名不动。
...
**硬门禁 G4**：
1. `grep -ri "weepcode\|x\.ai\|xai" --include="*.rs" crates/codegen/weepcode-pager/src crates/codegen/weepcode-agent/templates`
...
范围与规则（用户确认）：crate（72 个 weepcode-*）→ `weepcode-*`；env 变量
`WEEPCODE_*`/`WEEPCODE_*` → `WEEPCODE_*`；配置目录 `~/.weepcode` → `~/.weepcode`、`WEEPCODE_HOME` → `WEEPCODE_HOME`、
项目级 `.weepcode/` → `.weepcode/`、`/etc/weepcode` → `/etc/weepcode`；ACP 方法名 `x.ai/*` → `weepcode/*`；
auth 方法 id `weepcode.api_key` → `weepcode.api_key`；proto 包 `weepcode.tools.v1` → `weepcode.tools.v1`；
标识符 `weepcode_*`/`weepcode_*`/`weepcode_*`/`WeepCode*` → `weepcode_*`/`WeepCode*`。
**不读旧 env、不读旧目录（无兼容 shim）。** 保留项：真实 WeepCode 基础设施域名常量
（grok.com/x.ai，属 dead 企业路径，改域名无意义反而误导）、SuperGrok（WeepCode 产品名，惰性子系统）、
third_party 注释引用同步、文件名含 weepcode 的一并 git mv。

执行步骤：
- [x] 阶段A：72 个 crate 目录 git mv + 全部 Cargo.toml 改写（脚本 .tools/rename_phase5.py）
- [ ] 阶段B：全仓库有序替换（哨兵保护 grok.com → weepcode-/weepcode_/weepcode-/weepcode_/WEEPCODE_/WEEPCODE_/weepcode_/
  \bGrok/X-WeepCode-/"weepcode//.weepcode → 恢复哨兵）+ 含 weepcode 文件名 git mv + Cargo.lock 同步
...
2. `git grep -i "weepcode\|xai"` 命中仅剩白名单：真实 WeepCode 域名常量、SuperGrok、SOURCE_REV/THIRD-PARTY、
...
- [x] 全命令审计：`weepcode setup`/`update` 禁用（xAI 服务），slash 硬隐藏
...
- [x] 7e xAI 专属 OAuth 修剪：grok.com 默认 client_id/issuer 常量、devbox stub、
      accounts.x.ai CORS；保留通用 OIDC + 外部 provider + api_key
      **完成（2026-07-22）**：devbox 登录链路整根拔除（stub/manager/recovery/
      app/acp_agent/sampler_turn/flow，含 `run_cli_login --devbox` 与 pager 死字段）；
      `auth/config.rs` 默认 oauth2（auth.x.ai + 硬编码 client_id）、
      `weepcode_oauth2_issuer()`/`use_local_auth()`/本地 issuer、accounts.x.ai CORS
      全删；`auth_scope()` 改稳定占位 scope；`grok.com` method id、`is_weepcode_auth`
