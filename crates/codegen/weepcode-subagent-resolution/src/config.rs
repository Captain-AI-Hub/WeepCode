...
//! `weepcode_shell::config::{SubagentRole, SubagentPersona, PersonaIOField}`.
//!
//! Methods that remain in `weepcode-shell` (on `SubagentsConfig`):
...
"#;
        ...
        assert_eq!(role.model.as_deref(), Some("weepcode-3"));
        assert_eq!(role.reasoning_effort.as_deref(), Some("high"));
        assert_eq!(
            role.prompt_file.as_deref(),
            Some(".weepcode/prompts/researcher.md")
        );
...
"#;
        ...
        assert_eq!(
            ...
            Some(".weepcode/personas/concise.md")
        );
        assert_eq!(persona.model.as_deref(), Some("weepcode-3-fast"));
