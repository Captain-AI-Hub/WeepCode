...
pub mod tips;
pub mod uname;
pub mod weepcode_home;
pub use weepcode_shared::clipboard;
pub use weepcode_shared::stderr::{stderr_lock, with_locked_stderr};
...
/// True for WeepCode-operated endpoints (`*.x.ai`, cli-chat-proxy, and optional
/// non-production WeepCode hosts when that feature is enabled).
...
/// *attach* a credential, use [`is_weepcode_api_bearer_url`].
pub fn is_weepcode_api_url(url: &str) -> bool {
    is_weepcode_api_url_impl(url, false)
}
/// Like [`is_weepcode_api_url`], but requires `https` on every arm, so a
...
pub fn is_weepcode_api_bearer_url(url: &str) -> bool {
    is_weepcode_api_url_impl(url, true)
}
fn is_weepcode_api_url_impl(url: &str, require_https: bool) -> bool {
    if require_https {
...
/// True if `pid` is a weepcode process; pairs with [`kill_process_by_pid`] to avoid killing a recycled PID.
/// Best-effort on macOS/BSD (liveness-only via `kill -0`), exact on Linux (/proc cmdline) and Windows (image path).
pub fn is_weepcode_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
        let cmdline_path = format!("/proc/{pid}/cmdline");
        match std::fs::read(&cmdline_path) {
            Ok(data) => String::from_utf8_lossy(&data).contains("weepcode"),
            Err(_) => false,
        }
    }
    #[cfg(windows)]
...
mod tests {
    ...
    #[test]
    fn test_is_weepcode_api_url() {
        assert!(is_weepcode_api_url("https://api.x.ai/v1"));
        assert!(is_weepcode_api_url("https://api.x.ai/v1/chat/completions"));
        assert!(is_weepcode_api_url("https://x.ai"));
        assert!(is_weepcode_api_url(
            "https://cli-chat-proxy.grok.com/v1/chat/completions"
        ));
        assert!(!is_weepcode_api_url("https://api.openai.com/v1"));
        assert!(!is_weepcode_api_url("https://api.anthropic.com/v1"));
        assert!(!is_weepcode_api_url(
            "https://generativelanguage.googleapis.com"
        ));
        assert!(!is_weepcode_api_url("https://api.x.ai.evil.example/v1"));
        assert!(!is_weepcode_api_url("https://evil-x.ai.attacker.com/v1"));
        assert!(!is_weepcode_api_url("https://prefixx.ai/v1"));
        assert!(!is_weepcode_api_url("not-a-url"));
        assert!(!is_weepcode_api_url(""));
        assert!(is_weepcode_api_url("http://api.x.ai/v1"));
        assert!(is_weepcode_api_url("http://localhost:11434/v1"));
    }
    #[test]
    fn test_is_weepcode_api_bearer_url() {
        assert!(is_weepcode_api_bearer_url("https://api.x.ai/v1"));
        assert!(!is_weepcode_api_bearer_url("http://api.x.ai/v1"));
        assert!(!is_weepcode_api_bearer_url("http://localhost:11434/v1"));
        {
            assert!(!is_weepcode_api_bearer_url("https://localhost:11434/v1"));
            assert!(!is_weepcode_api_bearer_url("https://127.0.0.2:11434/v1"));
            assert!(!is_weepcode_api_bearer_url("https://[::1]:11434/v1"));
        }
        assert!(is_weepcode_api_bearer_url("https://API.X.AI/v1"));
        assert!(!is_weepcode_api_bearer_url(
            "https://api.x.ai@attacker.example/v1"
        ));
        assert!(!is_weepcode_api_bearer_url("https://х.ai/v1"));
    }
    #[test]
    fn test_truncate() {
    ...
    #[test]
    fn is_weepcode_process_self_true_impossible_pid_false() {
        assert!(is_weepcode_process(std::process::id()));
        assert!(!is_weepcode_process(u32::MAX));
    }
}
