//! Scripted TUI scenario runner for weepcode-pager.
//!
...
pub struct EnvironmentConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional `config.toml` written into the run's isolated `$WEEPCODE_HOME`
...
impl ScriptedScenarioRunner {
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&weepcode_home).with_context(|| {
                format!("create scenario WEEPCODE_HOME {}", weepcode_home.display())
            })?;
            fs::write(weepcode_home.join("config.toml"), config_toml)
