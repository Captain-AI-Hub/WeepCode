...
//! [`MockInferenceServer`] from `weepcode-test-support` and provides the
...
pub use weepcode_test_support::mock_server::LogEntry;
pub use weepcode_test_support::mock_server::MockModelEntry as MockModel;
pub use weepcode_test_support::mock_server::StorageUpload;
// SSE event builders for `enqueue_response` scripts (reasoning turns etc.).
pub use weepcode_test_support::sse;
pub use weepcode_test_support::{
...
impl ContentController {
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.weepcode
    ...
    pub fn env_for_pager(&self) -> Vec<(String, String)> {
        ...
        vec![
            ("HOME".into(), home),
            // Explicit WEEPCODE_HOME prevents leaking the real user's
            // config.toml when $HOME alone isn't sufficient (e.g. if
            // WEEPCODE_HOME is set in the test runner's env).
            ("WEEPCODE_HOME".into(), weepcode_home),
            ("WEEPCODE_CLI_CHAT_PROXY_BASE_URL".into(), self.url()),
            ("WEEPCODE_WEEPCODE_API_BASE_URL".into(), self.url()),
            ("WEEPCODE_API_KEY".into(), "test-key-for-ci".into()),
            ...
            ("WEEPCODE_PROMPT_SUGGESTIONS".into(), "false".into()),
            // Compatibility set_turns remains request-FIFO, so retries stay off.
            ("WEEPCODE_MAX_RETRIES".into(), "0".into()),
        ]
    }
...
mod tests {
    ...
    /// on a blocked startup screen and breaks every PTY test.
    #[tokio::test]
    ...
    async fn env_for_pager_shape() {
        ...
        assert_eq!(
            get("WEEPCODE_HOME").as_deref(),
            content.home().join(".weepcode").to_str()
        );
        assert_eq!(get("WEEPCODE_CLI_CHAT_PROXY_BASE_URL"), Some(content.url()));
        assert_eq!(get("WEEPCODE_WEEPCODE_API_BASE_URL"), Some(content.url()));
        assert_eq!(get("WEEPCODE_API_KEY").as_deref(), Some("test-key-for-ci"));
        assert_eq!(get("WEEPCODE_TELEMETRY_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("WEEPCODE_FEEDBACK_ENABLED").as_deref(), Some("false"));
        assert_eq!(get("WEEPCODE_TRACE_UPLOAD").as_deref(), Some("false"));
        assert_eq!(get("WEEPCODE_PROMPT_SUGGESTIONS").as_deref(), Some("false"));
        assert_eq!(get("WEEPCODE_MAX_RETRIES").as_deref(), Some("0"));
