//! Unified PTY harness for weepcode-pager.
//!
...
//!    `weepcode-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `WEEPCODE_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// # use weepcode_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// through weepcode's stdout. Used to reproduce the doubled-line class of bugs
    /// where weepcode's diff renderer never re-asserts a region it didn't write
