...
//! elected leader) plus a fixed leader socket beneath its `WEEPCODE_HOME`; clients
...
impl LeaderCluster {
    ...
    /// one leader) and a fixed leader socket under its `WEEPCODE_HOME`.
    pub async fn start(rows: u16, cols: u16) -> Result<Self> {
        let content = ContentController::start()
            .await
            .context("start content controller")?;
        // One shared WEEPCODE_HOME => one leader; the socket lives beneath it so
        // every client (sharing the same env) elects/attaches to the same one.
        let weepcode_home = content.home().join(".weepcode");
        std::fs::create_dir_all(&weepcode_home).context("create weepcode home")?;
        let socket = weepcode_home.join("leader-e2e.sock");
    ...
    /// The cluster's sessions root: `WEEPCODE_HOME/sessions` (layout below is
    /// `sessions/<encoded-cwd>/<session-id>/updates.jsonl`).
    fn sessions_dir(&self) -> PathBuf {
        self.content.home().join(".weepcode").join("sessions")
    }
