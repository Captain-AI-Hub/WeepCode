...
/// Seed a fake WeepCode OAuth entry into the isolated home's `auth.json` so the
/// shell has session auth (the harness's `WEEPCODE_API_KEY` is ApiKey/BYOK mode
...
pub fn seed_fake_oauth(content: &ContentController, user: &str) {
    ...
    std::fs::create_dir_all(&weepcode_home).expect("create temp .weepcode");
    std::fs::write(
...
/// [`ContentController::env_for_pager`] minus `WEEPCODE_API_KEY`, so the entry
...
pub fn oauth_env_for_pager(content: &ContentController) -> Vec<(String, String)> {
    ...
    env.retain(|(k, _)| k != "WEEPCODE_API_KEY");
    env
}
