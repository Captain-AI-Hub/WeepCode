//! `paste-latency` — clipboard paste latency benchmark for `weepcode-pager`.
//!
...
//!
//! ```bash
//! cargo bench -p weepcode-pager-pty-harness --bench paste_latency -- --iterations 10
//!
//! # Compare an old release artifact, text mode only, JSON to a file:
//! cargo bench -p weepcode-pager-pty-harness --bench paste_latency -- \
//!   --binary ~/Downloads/weepcode-old --mode text --json /tmp/paste-old.json
//! ```
...
use weepcode_pager_pty_harness::{
...
#[command(
    name = "paste-latency",
    about = "Clipboard paste latency benchmark for weepcode-pager (macOS, real pasteboard)",
    long_about = None,
)]
struct Cli {
