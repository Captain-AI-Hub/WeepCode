//! `pty-bench` — PTY benchmark CLI for `weepcode-pager`.
//!
...
//! ```bash
//! cargo bench -p weepcode-pager-pty-harness \
...
//! ```bash
//! PAGER_BINARY=./artifacts/weepcode-${VERSION}-linux-x86_64 \
//!   cargo bench -p weepcode-pager-pty-harness \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for weepcode-pager",
    long_about = None,
)]
struct Cli {
