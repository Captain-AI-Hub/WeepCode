...
mod tests {
    ...
    async fn post_foreground(
        ...
        reqwest::Client::new()
            ...
            .header("x-weepcode-req-id", request_id)
            .header("x-weepcode-turn-idx", "1")
    ...
    fn explicit_request_headers_override_tool_count_heuristic() {
        ...
        headers.insert("x-weepcode-req-id", "title-request".parse().unwrap());
        assert!(
            !overrides
        ...
        headers.insert("x-weepcode-turn-idx", "1".parse().unwrap());
        assert!(
            overrides
                .classify(InferenceEndpoint::ChatCompletions, &headers, &body)
                .is_foreground()
        ...
        headers.insert("x-weepcode-req-id", "".parse().unwrap());
        headers.insert("x-weepcode-turn-idx", "".parse().unwrap());
        assert!(
            overrides
                .classify(InferenceEndpoint::ChatCompletions, &headers, &body)
                .is_foreground()
    ...
    async fn overlapping_duplicate_replays_but_sequential_identical_request_claims_next() {
        ...
        let replay = tokio::spawn({
            ...
            async move {
                let response = reqwest::Client::new()
                    .post(url)
                    .header("x-weepcode-req-id", "turn-id")
                    .header("x-weepcode-turn-idx", "1")
                    .json(&body)
                    .send()
                    .await
    ...
    async fn cancelling_replay_waits_for_primary_before_satisfaction() {
        ...
        let replay = tokio::spawn({
            ...
            async move {
                reqwest::Client::new()
                    .post(url)
                    .header("x-weepcode-req-id", "cancel-replay")
                    .header("x-weepcode-turn-idx", "1")
                    .json(&body)
                    .send()
                    .await
