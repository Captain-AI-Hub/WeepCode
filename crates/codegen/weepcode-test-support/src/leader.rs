//! Leader-mode (`weepcode agent --leader stdio`) test harness.
//!
...
use weepcode_acp_lib::LineBufferedRead;

use crate::env::weepcode_binary;
...
/// (version-skew) test. Falls back to [`weepcode_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "WEEPCODE_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`weepcode_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "WEEPCODE_BINARY_CLIENT";
...
/// (`WEEPCODE_BINARY_LEADER`, else the shared [`weepcode_binary`] resolution).
pub fn leader_binary() -> PathBuf {
    role_binary(LEADER_BINARY_ENV)
}

/// Binary for the client side of a version-skew test (`WEEPCODE_BINARY_CLIENT`,
/// else the shared [`weepcode_binary`] resolution).
...
impl acp::Client for LeaderAcpClient {
    ...
    async fn ext_notification(&self, args: acp::ExtNotification) -> acp::Result<()> {
        if &*args.method == "weepcode/leader_reconnected" {
            self.capture
                .reconnected_count
                .fetch_add(1, Ordering::SeqCst);
        }
...
/// A `weepcode agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderStdioClient {
    pub async fn spawn(server: &MockInferenceServer, cwd: &Path, home: &Path) -> Self {
        Self::spawn_with_binary(&weepcode_binary(), server, cwd, home).await
    }
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            .current_dir(cwd)
            // Hermetic env: the developer's shell may export WEEPCODE_* vars
            // (e.g. WEEPCODE_LEADER_SOCKET pointing at a REAL leader on this
            // machine). env_clear + explicit allowlist guarantees the test
            // can never touch a leader outside its sandbox home.
            .env_clear()
            .env("PATH", std::env::var("PATH").unwrap_or_default())
            .env("HOME", home)
            .env("WEEPCODE_HOME", home.join(".weepcode"))
            ...
            .env(
                "WEEPCODE_LEADER_SOCKET",
                home.join(".weepcode").join("leader.sock"),
            )
            .env("WEEPCODE_CLI_CHAT_PROXY_BASE_URL", server.url())
            .env("WEEPCODE_WEEPCODE_API_BASE_URL", server.url())
            .env("WEEPCODE_API_KEY", "test-key-for-ci")
            .env("WEEPCODE_TELEMETRY_ENABLED", "false")
            .env("WEEPCODE_FEEDBACK_ENABLED", "false")
            .env("WEEPCODE_TRACE_UPLOAD", "false")
            .env("WEEPCODE_INSTRUMENTATION", "disabled")
            // Inherited by the spawned leader, whose stderr goes to
            // ~/.weepcode/leader.log — keep it chatty for diagnosis.
            .env("RUST_LOG", "weepcode_shell=debug");
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init
            ...
            .expect("weepcode.api_key auth method");
        self.conn
            .authenticate(
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".weepcode").join("leader.lock")
}
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".weepcode").join("leader.log")).unwrap_or_default()
}
