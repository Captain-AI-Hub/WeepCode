...
//! Shared test utilities for weepcode-build crates: mock inference server, SSE
...
//! - [`WeepcodeStdioClient`] — ACP client that drives `weepcode agent stdio` as a subprocess
//! - [`RawStdioClient`] — raw-wire ACP driver for bytes the typed client can't
//!   produce (Foundation `\/` methods, string UUID ids)
//! - [`leader::LeaderStdioClient`] — ACP client that drives `weepcode agent --leader stdio` (unix)
//! - [`run_headless`] — Run `weepcode -p` against the mock server and capture output
//! - [`git_workdir`] — Create a temp directory with git repo (forces libgit2 init)
//! - [`weepcode_binary`] — Resolve the weepcode binary path (WEEPCODE_BINARY env or cargo_bin)
...
#[cfg(unix)]
pub mod uds_proxy;
pub use acp_client::{RawStdioClient, WeepcodeStdioClient};
pub use counting_server::spawn_counting_server;
pub use env::{EnvGuard, git_workdir, weepcode_binary};
pub use headless::{
