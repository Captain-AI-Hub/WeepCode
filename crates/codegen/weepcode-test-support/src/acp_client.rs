//! ACP stdio clients for testing weepcode sessions end-to-end: the typed
//! [`WeepcodeStdioClient`] (`agent-client-protocol::ClientSideConnection` —
...
use crate::env::{test_env_cmd_tokio, weepcode_binary};
use crate::headless::stderr_tail;
use crate::mock_server::MockInferenceServer;
use crate::process::spawn_piped_with_stderr_capture;

/// Spawn `weepcode agent stdio` with the canonical hermetic test env: the sandbox
/// from [`test_env_cmd_tokio`] plus the debug-logging kill-list, so the
/// hermeticity setup exists exactly once for the typed ([`WeepcodeStdioClient`])
...
/// kill-list so a test can still set e.g. `WEEPCODE_DEBUG_LOG=1` explicitly.
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init_resp
            ...
            .unwrap_or_else(|| {
                ...
                panic!(
                    "expected auth method 'weepcode.api_key' but got: {ids:?}\n\
...
/// Drives `weepcode agent stdio` with verbatim newline-delimited JSON-RPC lines.
///
/// Exists for wire shapes the typed [`WeepcodeStdioClient`] (`ClientSideConnection`,
