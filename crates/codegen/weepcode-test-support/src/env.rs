...
fn local_weepcode_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("weepcode-pager{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_weepcode_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
    ...
    let output = Command::new(&cargo)
        ...
        .args(["build", "-p", "weepcode-pager", "--bin", "weepcode-pager"])
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build weepcode-pager: {e}"));

    assert!(
        output.status.success(),
        "failed to build weepcode-pager for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "weepcode-pager build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve weepcode binary: `WEEPCODE_BINARY` env (CI) or a locally built `weepcode-pager` binary.
pub fn weepcode_binary() -> PathBuf {
    if let Ok(path) = std::env::var("WEEPCODE_BINARY") {
        let p = PathBuf::from(path);
        assert!(
            p.exists(),
            "WEEPCODE_BINARY does not exist: {}",
            p.display()
        );
        return p;
    }

    if let Ok(path) = std::env::var("CARGO_BIN_EXE_weepcode-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
    ...
    let binary = local_weepcode_binary_path();
    ensure_local_weepcode_binary(&binary);
    binary
}
...
/// Point weepcode at the mock server with a fake API key and telemetry disabled.
pub fn test_env_cmd_tokio(
    ...
    cmd.env("HOME", home)
        // HOME alone does not sandbox weepcode on Windows: the product resolves
        // `~` via `USERPROFILE`/Known Folders (`std::env::home_dir()`), so
        // without an explicit WEEPCODE_HOME every spawned child shares the real
        // `%USERPROFILE%\.weepcode` — test 1's models_cache.json (which embeds
        ...
        .env("WEEPCODE_CLI_CHAT_PROXY_BASE_URL", mock_url)
        .env("WEEPCODE_WEEPCODE_API_BASE_URL", mock_url)
        ...
        .env("WEEPCODE_DISABLE_AUTOUPDATER", "1");
}
