...
use weepcode_circuit_breaker::{Disposition, RetryPolicy};
...
impl ResolvedStorageConfig {
    ...
    /// mirrors `WeepcodeAuthCredentials::apply` precedence (deployment key wins).
...
pub struct UploadRetryPolicy {
    ...
    /// `WEEPCODE_UPLOAD_QUEUE_AUTH_PROBE_SECS`.
    pub auth_park_probe_interval: Duration,
}
pub const DEFAULT_AUTH_PARK_PROBE_INTERVAL: Duration = Duration::from_secs(300);
/// Smallest probe interval a `WEEPCODE_UPLOAD_QUEUE_AUTH_PROBE_SECS` override may
...
/// Resolve a `WEEPCODE_UPLOAD_QUEUE_AUTH_PROBE_SECS` override (seconds) into a probe
...
/// `session_id`, no GCS path). Read by `weepcode_workspace::recovery`.
...
struct UploadQueueItem {
    ...
    /// WeepCode client version string, stamped on the `gcs_queue_upload` tracing span.
...
pub struct UploadQueue {
    ...
    max_queue_bytes: u64,
    /// WeepCode client version string stamped on every `gcs_queue_upload` tracing span.
...
impl UploadQueue {
    ...
    pub fn spawn_with_concurrency(
        ...
        let queue_dir = weepcode_home.join("upload_queue");
        if let Err(e) = std::fs::create_dir_all(&queue_dir) {
            tracing::warn!(error = % e, "Failed to create upload queue dir");
        }
        if let Some(raw_secs) = std::env::var("WEEPCODE_UPLOAD_QUEUE_AUTH_PROBE_SECS")
            .ok()
            .and_then(|s| s.parse::<u64>().ok())
        {
            match auth_park_probe_override(raw_secs) {
                ...
                None => {
                    tracing::warn!(
                        "Ignoring WEEPCODE_UPLOAD_QUEUE_AUTH_PROBE_SECS={raw_secs}: a zero probe \
        ...
        let max_queue_bytes = std::env::var("WEEPCODE_UPLOAD_QUEUE_MAX_BYTES")
            .ok()
    ...
    /// Set the weepcode client version to stamp on every `gcs_queue_upload` span.
...
/// of entries removed.
pub fn cleanup_orphaned_uploads(weepcode_home: &Path, max_age: Duration) -> u64 {
    let cleaned = cleanup_queue_dir(&weepcode_home.join("upload_queue"), max_age, None);
