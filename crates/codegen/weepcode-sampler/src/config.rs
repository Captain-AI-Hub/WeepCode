...
//! `weepcode_sampling_types::SamplingConfig` so that the sampler crate
...
/// submit.
///
/// # Construction in `weepcode-shell`
///
...
/// composing chat-state's `weepcode_sampling_types::SamplingConfig`
/// with `Credentials` (api key, client version).
///
/// URL-derived request headers (e.g. `X-WeepCode-Token-Auth` for the
/// cli-chat-proxy) are
...
pub struct SamplerConfig {
    ...
    /// client itself sends the opt-in `x-weepcode-doom-loop-check` header on
