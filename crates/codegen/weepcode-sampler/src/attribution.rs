...
//! it" buckets.
//!
//! `weepcode-sampler` is intentionally decoupled from `weepcode-shell`
...
/// tools live in `weepcode-tools`
/// (`crates/codegen/weepcode-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `weepcode_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `weepcode_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `weepcode-shell/src/auth/manager.rs` to match, otherwise the
