//! HTTP client for the WeepCode sampling APIs.
//!
...
use weepcode_sampling_types::error::{parse_error_bytes, try_parse_stream_error};
...
pub use weepcode_sampling_types::ApiBackend;

/// Process-level fallback for the `x-weepcode-client-identifier` header.
const DEFAULT_CLIENT_IDENTIFIER: &str = "weepcode-shell";

/// Product identifier baked into User-Agent strings.
const AGENT_PRODUCT: &str = "weepcode-shell";
...
impl WeepcodeRequestHeaders<'_> {
    fn apply(&self, builder: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
        let mut b = builder
            .header("x-weepcode-conv-id", self.conv_id)
            .header("x-weepcode-req-id", self.req_id)
            .header("x-weepcode-model-override", self.model_id)
            .header("x-weepcode-session-id", self.session_id)
            .header("x-weepcode-agent-id", self.agent_id);
        if let Some(idx) = self.turn_idx {
            b = b.header("x-weepcode-turn-idx", idx);
        }
        if let Some(id) = self.deployment_id.filter(|s| !s.is_empty()) {
            b = b.header("x-weepcode-deployment-id", id);
        }
        if let Some(id) = self.user_id.filter(|s| !s.is_empty()) {
            b = b.header("x-weepcode-user-id", id);
        }
        b
    }
}

/// Whether `base_url` points at a first-party WeepCode/weepcode host.
///
/// `x-weepcode-*` tracking headers (conv/session/agent ids, deployment id, user
/// id, client version/identifier) exist for WeepCode's proxy observability and
...
/// Deserialize a Responses API SSE event, with a fallback for WeepCode-specific
...
/// when the API emits the WeepCode-specific `context_details` field.
    ...
    let mut event = match serde_json::from_str::<rs::ResponseStreamEvent>(data) {
        ...
        Err(first_err) => {
            ...
            if let Ok(mut value) = serde_json::from_str::<serde_json::Value>(data) {
                ...
                // (e.g., WeepCode-specific "x_search"). Instead of maintaining a
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = weepcode_sampling_types::reported_cost_ticks(
        value
...
pub(crate) const COST_USD_TICKS_METADATA_KEY: &str = "weepcode.cost_usd_ticks";
...
fn extract_model_metadata(headers: &reqwest::header::HeaderMap) -> Option<ResponseModelMetadata> {
    let context_window = headers
        .get("x-weepcode-context-window")
        .and_then(|v| v.to_str().ok())
        .and_then(|s| s.parse::<u64>().ok());

    let max_completion_tokens = headers
        .get("x-weepcode-max-completion-tokens")
        .and_then(|v| v.to_str().ok())
...
pub struct SamplingClient {
    ...
    /// Computed in [`SamplingClient::new`] from `base_url`: `x-weepcode-*`
    /// tracking headers are only attached when the endpoint is first-party.
    send_weepcode_tracking_headers: bool,
}
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<weepcode_sampling_types::DoomLoopRecoveryPolicy>,
}
...
fn agent_version() -> String {
    weepcode_version::VERSION.to_string()
}
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        // WeepCode tracking headers are only meaningful to (and only safe for)
        // first-party hosts; third-party provider endpoints never see them.
        let send_weepcode_tracking_headers = is_first_party_inference_base_url(&config.base_url);

        // Add x-weepcode-client-version header for version gating at the proxy.
        if send_weepcode_tracking_headers
            && let Some(client_version) = config.client_version.as_ref()
            && let Ok(header_value) = HeaderValue::from_str(client_version)
        {
            headers.insert(
                HeaderName::from_static("x-weepcode-client-version"),
                header_value,
            );
        }
        ...
        {
            headers.insert(
                HeaderName::from_static("x-weepcode-deployment-id"),
                header_value,
            );
        }
        ...
        {
            headers.insert(HeaderName::from_static("x-weepcode-user-id"), header_value);
        }
        ...
        if send_weepcode_tracking_headers {
            ...
            if let Ok(header_value) = HeaderValue::from_str(&client_id) {
                headers.insert(
                    HeaderName::from_static("x-weepcode-client-identifier"),
                    header_value,
                );
            }
        }
        ...
        Ok(Self {
            ...
            send_weepcode_tracking_headers,
        })
    }
    ...
    /// Attach per-request `x-weepcode-*` tracking headers for first-party
    /// endpoints only; third-party provider endpoints get the request
    /// builder back untouched so no WeepCode identifiers leak.
    fn with_tracking_headers(
        &self,
        tracking: WeepcodeRequestHeaders<'_>,
        builder: reqwest::RequestBuilder,
    ) -> reqwest::RequestBuilder {
        if self.send_weepcode_tracking_headers {
    ...
    fn format_request_headers(
        &self,
        x_weepcode_conv_id: &str,
        x_weepcode_req_id: &str,
        model_id: &str,
        include_accept: bool,
    ) -> Vec<String> {
        ...
        req_headers.push(Self::format_header(
            "x-weepcode-conv-id",
            x_weepcode_conv_id,
        ));
        req_headers.push(Self::format_header("x-weepcode-req-id", x_weepcode_req_id));
        req_headers.push(Self::format_header("x-weepcode-model-override", model_id));
        if include_accept {
    ...
    pub async fn chat_completion(
        ...
        let x_weepcode_conv_id = &payload.x_weepcode_conv_id.clone().unwrap_or_default();
        let x_weepcode_req_id = &payload.x_weepcode_req_id.clone().unwrap_or_default();
        ...
        let weepcode_headers = WeepcodeRequestHeaders {
            conv_id: x_weepcode_conv_id,
            req_id: x_weepcode_req_id,
            model_id: &model_id,
            session_id: payload.x_weepcode_session_id.as_deref().unwrap_or_default(),
            turn_idx: payload.x_weepcode_turn_idx.as_deref(),
            agent_id: payload.x_weepcode_agent_id.as_deref().unwrap_or_default(),
            deployment_id: payload.x_weepcode_deployment_id.as_deref(),
            user_id: payload.x_weepcode_user_id.as_deref(),
        };
        let http_request = self
            .with_tracking_headers(
                weepcode_headers,
    ...
    pub async fn chat_completion_stream(
        ...
        if !status.is_success() {
            ...
            let req_headers =
                self.format_request_headers(x_weepcode_conv_id, x_weepcode_req_id, &model_id, true);
    ...
    pub async fn create_response(
        ...
        let x_weepcode_conv_id = request.x_weepcode_conv_id.as_deref().unwrap_or_default();
        let x_weepcode_req_id = request.x_weepcode_req_id.as_deref().unwrap_or_default();
        ...
        let weepcode_headers = WeepcodeRequestHeaders {
            ...
            req_id: x_weepcode_req_id,
            model_id: &model_id,
            session_id: request.x_weepcode_session_id.as_deref().unwrap_or_default(),
            turn_idx: request.x_weepcode_turn_idx.as_deref(),
            agent_id: request.x_weepcode_agent_id.as_deref().unwrap_or_default(),
            deployment_id: request.x_weepcode_deployment_id.as_deref(),
            user_id: request.x_weepcode_user_id.as_deref(),
        };
        ...
        weepcode_sampling_types::patch_reasoning_text_types(&mut request_body);
        let http_request = self
            .with_tracking_headers(weepcode_headers, self.post(self.endpoint("responses")))
            .json(&request_body);
        ...
        if !status.is_success() {
            ...
            let req_headers = self.format_request_headers(
                x_weepcode_conv_id,
                x_weepcode_req_id,
                &model_id,
                false,
    ...
    /// gate that adds the opt-in `x-weepcode-doom-loop-check` request header, so
    ...
    pub async fn create_response_stream(
        ...
        // Inject WeepCode-specific fields not in async-openai's CreateResponse type.
        if self.defaults.stream_tool_calls {
            request_body["stream_tool_calls"] = serde_json::json!(true);
        }
        // Inject WeepCode-specific tools (e.g., x_search) that can't be expressed
    ...
    pub async fn create_message(
        ...
        let http_request = self
            .with_tracking_headers(weepcode_headers, self.post(self.endpoint("messages")))
            .json(&request.inner);
    ...
    pub async fn conversation_stream_responses(
        ...
        let trace = request.trace.take();
        let x_weepcode_conv_id = request.x_weepcode_conv_id.clone();
        let x_weepcode_req_id = request.x_weepcode_req_id.clone();
        let x_weepcode_session_id = request.x_weepcode_session_id.clone();
        let x_weepcode_turn_idx = request.x_weepcode_turn_idx.clone();
        let x_weepcode_agent_id = request.x_weepcode_agent_id.clone();

        // Collect WeepCode-specific tools that can't be expressed via rs::Tool
        // (e.g., x_search). These are injected as raw JSON after serialization.
        let extra_tools = weepcode_sampling_types::extra_raw_tools(&request.hosted_tools);

        let responses_request: rs::CreateResponse = (&request).into();

        let mut wrapper = CreateResponseWrapper::new(responses_request);
        wrapper.x_weepcode_conv_id = x_weepcode_conv_id;
        wrapper.x_weepcode_req_id = x_weepcode_req_id;
        wrapper.x_weepcode_session_id = x_weepcode_session_id;
        wrapper.x_weepcode_turn_idx = x_weepcode_turn_idx;
        wrapper.x_weepcode_agent_id = x_weepcode_agent_id;
...
mod tests {
    ...
    use weepcode_sampling_types::types::ChatRequestMessage;
    ...
    fn streaming_chat_request_serializes_correctly() {
        let request = ChatCompletionRequest {
            ...
            reasoning_effort: None,
            x_weepcode_conv_id: None,
            x_weepcode_req_id: None,
            x_weepcode_session_id: None,
            x_weepcode_turn_idx: None,
            x_weepcode_agent_id: None,
            x_weepcode_deployment_id: None,
            x_weepcode_user_id: None,
            trace: None,
        };
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the weepcode-shell agent product.
        assert!(ua.starts_with("my-client weepcode-shell/"));
    }

    #[test]
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = weepcode_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
        ...
        let make = |ticks: i64| {
            format!(
                ...
                "response": {{
                    ...
                    "model": "weepcode-build", "status": "completed", "output": [],
                    "usage": {{
                        "input_tokens": 10,
    ...
    fn first_party_host_detection() {
        ...
        assert!(is_first_party_inference_base_url(
    ...
    #[test]
    fn weepcode_tracking_headers_omitted_for_third_party_base_url() {
        ...
        let client = SamplingClient::new(config).unwrap();
        assert!(!client.send_weepcode_tracking_headers);
        for name in [
            "x-weepcode-client-version",
            "x-weepcode-deployment-id",
            "x-weepcode-user-id",
            "x-weepcode-client-identifier",
        ] {
            assert!(
    ...
    #[test]
    fn weepcode_tracking_headers_sent_to_first_party_base_url() {
        let config = SamplerConfig {
        ...
        assert!(client.send_weepcode_tracking_headers);
        assert_eq!(
            client.default_headers.get("x-weepcode-client-version"),
            Some(&HeaderValue::from_static("1.2.3")),
        );
        assert_eq!(
            client.default_headers.get("x-weepcode-user-id"),
