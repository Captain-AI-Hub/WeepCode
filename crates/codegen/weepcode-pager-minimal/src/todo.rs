...
use weepcode_shell::tools::TodoStatus;
    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (weepcode_pager::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    weepcode_pager::glyphs::ballot_x(),
...
mod tests {
    ...
    use weepcode_shell::tools::{TodoItem, TodoPriority};
