...
use weepcode_pager::app::PagerTerminal;
use weepcode_pager::app::app_view::{ActiveView, AppView};
use weepcode_pager::minimal_api;
use weepcode_pager::render::Renderable;
use weepcode_pager::scrollback::state::ScrollbackState;
use weepcode_pager::scrollback::wrappers::EntryRenderer;
use weepcode_pager::theme::Theme;
use weepcode_pager::views::prompt_widget::PromptStyle;
use weepcode_pager::views::turn_status;
...
/// welcome box.
pub(super) fn live_left_inset(_appearance: &weepcode_pager::appearance::AppearanceConfig) -> u16 {
    0
}
...
pub(super) fn prompt_style(
    appearance: &weepcode_pager::appearance::AppearanceConfig,
) -> PromptStyle {
    PromptStyle {
        focused: true,
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    weepcode_pager::render::draw::draw_frame(terminal, cursor, |frame, _link_spans| {
        let area = frame.area();
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                weepcode_pager::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(weepcode_pager::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a weepcode_pager::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
    appearance: &weepcode_pager::appearance::AppearanceConfig,
    cwd: &'a std::path::Path,
    tick: u64,
...
fn minimal_watchers(agent: &weepcode_pager::app::agent_view::AgentView) -> turn_status::Watchers {
    turn_status::Watchers {
        monitors: agent
            ...
            .values()
            .filter(|t| {
                t.is_monitor && t.status == weepcode_pager::app::agent::BgTaskStatus::Running
            })
            .count(),
...
fn minimal_advance_phase_timer(
    agent: &mut weepcode_pager::app::agent_view::AgentView,
) -> Option<weepcode_pager::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &weepcode_pager::app::agent_view::AgentView,
    activity: &Option<weepcode_pager::acp::tracker::TurnActivity>,
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let auto = weepcode_pager::app::minimal_auto_set_for_mouse_leak();
    let switch_back = weepcode_pager::app::minimal_show_switch_back_to_fullscreen();
...
fn render_prompt_info(
    ...
    use weepcode_pager::views::context_bar::fmt_tokens;
    ...
    if let (Some(used), Some(total)) = (used, total)
        && total > 0
    {
        let pct = weepcode_token_estimation::usage_percentage(used, total);
        segs.push((
...
fn minimal_pending_hint(
    pending: &Option<weepcode_pager::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    use super::*;
    fn agent() -> weepcode_pager::app::agent_view::AgentView {
        minimal_api::test_agent_view(Some("s1"), std::path::PathBuf::from("/tmp"))
    }
    #[test]
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        use weepcode_pager::app::agent::AgentState;
        use weepcode_pager::scrollback::RenderBlock;
        use weepcode_pager::scrollback::entry::ScrollbackEntry;
        use weepcode_pager::scrollback::types::DisplayMode;
        ...
        let appearance = super::super::commit::committed_appearance(
            &weepcode_pager::appearance::AppearanceConfig::default(),
        );
        let theme = Theme::current();
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        use weepcode_pager::acp::tracker::TurnActivity;
        use weepcode_pager::app::agent::AgentState;
        let theme = Theme::current();
        ...
        weepcode_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(false);
        let a = agent();
        ...
        weepcode_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(true);
        let mut buf = Buffer::empty(area);
        render_minimal_status(&mut buf, area, &a, &None, None, &theme);
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            weepcode_pager::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_info_renders_model_context_and_queued() {
        let mut a = agent();
        a.context_state = Some(weepcode_shell::session::ContextInfo {
            used: 276_000,
            total: 2_000_000,
            ..Default::default()
        });
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &weepcode_pager::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
    ...
    fn pending_hint_formats_press_again() {
        ...
        use weepcode_pager::app::app_view::PendingAction;
        use weepcode_pager::input::key::KeyShortcut;
