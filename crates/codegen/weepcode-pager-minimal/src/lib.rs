//! Minimal (scrollback-native) render mode — `weepcode --minimal`.
//!
//! In this mode finalized conversation blocks are printed once into the
//! terminal's *native* scrollback (via `weepcode_ratatui_inline::Terminal::insert_before`,
...
//! # Wiring
//!
//! `weepcode-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`weepcode_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`weepcode-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `weepcode-pager`.
///
...
/// subsequent calls are ignored (see [`weepcode_pager::minimal_hook`]).
pub fn install() {
    weepcode_pager::minimal_hook::install(weepcode_pager::minimal_hook::MinimalHooks { draw });
}
