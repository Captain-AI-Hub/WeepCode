...
use weepcode_pager::app::app_view::{AuthState, TrustState};
...
/// `Done`, when the user is not ZDR-blocked (that gate already blocks sessions,
/// and the input interceptor only answers trust under the same conditions).
    ...
    match hint {
        MinimalAuthHint::SigningIn { url, code } => {
            y = put_line(
                ...
                y,
                bottom,
                Line::from(Span::styled("Sign in to WeepCode", bold)),
            );
...
mod tests {
    ...
    fn auth_hint_maps_auth_state() {
        use weepcode_pager::app::app_view::AuthMode;
    ...
    fn auth_hint_maps_pending_trust_after_auth() {
        ...
        // The ZDR gate suppresses the trust question (matches welcome +
    ...
    fn render_auth_shows_url_and_code() {
        ...
        assert!(text.contains("Sign in to WeepCode"), "header: {text:?}");
