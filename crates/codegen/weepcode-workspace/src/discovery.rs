...
//! `weepcode-agent` and `weepcode-tools` rather than duplicating them.
...
//!
//! Re-exports [`AgentsMdTracker`] from `weepcode-tools` for external
...
// workspace crate without a direct weepcode-tools dependency.
pub use weepcode_tools::types::agents_md_tracker::AgentsMdTracker;

// Re-export the config types that callers pass into WorkspaceConfig.
pub use weepcode_agent::plugins::discovery::DiscoveryConfig as PluginDiscoveryConfig;
pub use weepcode_agent::plugins::trust::TrustStore as PluginTrustStore;
pub use weepcode_agent::prompt::skills::SkillsConfig;
...
///
/// Delegates to [`weepcode_agent::prompt::skills::list_skills`] with
...
pub async fn discover_agents_md(root_cwd: &Path) -> Vec<Value> {
    let cwd_str = root_cwd.to_string_lossy();
    let files = weepcode_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd_str,
    ...
    files
        .into_iter()
        .map(|mut file| {
            // Strip rules-file YAML frontmatter so it does not leak as raw YAML (matches weepcode-build render).
            if file.file_path.contains("/.weepcode/rules/")
                || file.file_path.contains("/.claude/rules/")
            {
                file.content = weepcode_tools::implementations::skills::skill::extract_skill_body(
                    &file.content,
                );
            }
            file
...
///
/// Delegates to [`weepcode_agent::plugins::discover_plugins`] with
...
pub fn discover_plugins(
    ...
    let discovered = weepcode_agent::plugins::discover_plugins(
        Some(root_cwd),
        config,
        trust_store,
        project_trusted,
...
/// Load the project config from `<root_cwd>/.weepcode/config.toml`.
///
...
pub fn load_project_config(root_cwd: &Path) -> Value {
    let config_path = root_cwd.join(".weepcode").join("config.toml");
    match weepcode_config::load_config_file(&config_path) {
...
mod tests {
    ...
    // `~/.weepcode/skills/`, so on a developer machine the result may be
    ...
    async fn discover_skills_finds_skill_md() {
        ...
        let skills_dir = tmp.path().join(".weepcode").join("skills").join("my-skill");
    ...
    async fn discover_skills_respects_ignore_config() {
        let tmp = tempfile::tempdir().unwrap();
        let skills_dir = tmp.path().join(".weepcode").join("skills").join("ignored");
        fs::create_dir_all(&skills_dir).unwrap();
        fs::write(
            skills_dir.join("SKILL.md"),
            "---\nname: ignored\ndescription: Ignored\n---\n",
    ...
    fn agent_config_file_wire_matches_workspace_types_mirror() {
        // The RPC serializes weepcode-build's AgentConfigFile and the remote
        ...
        // break discovery.
        let src = weepcode_agent::prompt::agents_md::AgentConfigFile {
            file_name: "AGENTS.md".to_string(),
            file_path: "/repo/AGENTS.md".to_string(),
            content: "# Instructions\n".to_string(),
        };
        let json = serde_json::to_value(&src).unwrap();
        let mirror: weepcode_workspace_types::rpc::agents_md::AgentConfigFile =
    ...
    // Discovery also scans the real `~/.weepcode`, so fixtures use test-unique names.
    #[tokio::test]
    async fn discover_agents_md_strips_rules_frontmatter() {
        let tmp = tempfile::tempdir().unwrap();
        let rules_dir = tmp.path().join(".weepcode").join("rules");
        ...
        let rule = files
            ...
            .find(|f| {
                f["file_path"].as_str().is_some_and(|p| {
                    p.ends_with("/.weepcode/rules/xyzzy-discover-agents-md-test.md")
                })
            })
    ...
    // from `~/.weepcode/plugins/`, so tests check for specific plugins.
    ...
    // developer machine with WeepCode installed it may return non-Null even
