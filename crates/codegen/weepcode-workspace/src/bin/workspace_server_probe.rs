...
//! access token from `~/.weepcode/auth.json` does (same identity).
...
use url::Url;
use uuid::Uuid;
use weepcode_computer_hub_sdk::pool::HubConnectionPool;
use weepcode_computer_hub_sdk::{AuthCredential, ToolHarnessBuilder};
...
async fn call_tool(
    harness: &weepcode_computer_hub_sdk::ToolHarness,
    name: &str,
    args: Value,
) -> anyhow::Result<Value> {
...
async fn connect_and_bind(
...
) -> anyhow::Result<weepcode_computer_hub_sdk::ToolHarness> {
    ...
    let metadata = json!({
        "tools": [
            {"id": "WeepcodeBuild:run_terminal_cmd", "name_override": "run_terminal_command"},
            {"id": "WeepcodeBuild:read_file"},
        ],
    });
    let tools = harness
