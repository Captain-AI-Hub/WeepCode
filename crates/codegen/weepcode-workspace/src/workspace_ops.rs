...
use weepcode_computer_hub_sdk::ToolHarness;
use weepcode_tools::types::output::ToolRunResult;
use weepcode_workspace_client::{WorkspaceClient, is_transport_fatal};
pub use weepcode_workspace_types::rpc::WorkspaceRpc;
pub use weepcode_workspace_types::rpc::agents_md::DiscoverAgentsMdReq;
pub use weepcode_workspace_types::rpc::code_nav::{
...
pub use weepcode_workspace_types::rpc::fs::{
...
pub use weepcode_workspace_types::rpc::git::{
...
pub use weepcode_workspace_types::rpc::hooks::{
    HookEventNameWire, HookRegistryReq, HookRegistryWire, HookSpecWire,
};
pub use weepcode_workspace_types::rpc::hunks::{
...
pub use weepcode_workspace_types::rpc::search::{FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq};
pub use weepcode_workspace_types::rpc::session::{BeginPromptReq, EndPromptReq, RewindToReq};
pub use weepcode_workspace_types::rpc::skills::DiscoverSkillsReq;
pub use weepcode_workspace_types::rpc::workspace::WorkspaceInfoReq;
pub use weepcode_workspace_types::rpc::worktree::{
...
fn hunk_line_info_to_wire(info: &weepcode_hunk_tracker::types::HunkLineInfo) -> HunkLineInfoWire {
    HunkLineInfoWire {
...
fn hunk_source_to_wire(source: weepcode_hunk_tracker::types::HunkSource) -> HunkSourceWire {
    use weepcode_hunk_tracker::types::HunkSource as S;
    match source {
...
fn hunk_to_wire(hunk: &weepcode_hunk_tracker::types::Hunk) -> HunkWire {
    HunkWire {
...
fn file_content_status_to_wire(
    status: weepcode_hunk_tracker::types::FileContentStatus,
) -> FileContentStatusWire {
    use weepcode_hunk_tracker::types::FileContentStatus as S;
    match status {
...
fn file_content_view_to_wire(
    view: weepcode_hunk_tracker::types::FileContentView,
) -> FileContentViewWire {
    FileContentViewWire {
...
fn file_content_entry_to_wire(
    entry: weepcode_hunk_tracker::FileContentEntry,
...
fn session_stats_to_wire(stats: &weepcode_hunk_tracker::types::SessionStats) -> SessionStatsWire {
    SessionStatsWire {
...
fn turn_summary_to_wire(turn: weepcode_hunk_tracker::types::TurnSummary) -> TurnSummaryWire {
    TurnSummaryWire {
...
fn session_summary_to_wire(summary: weepcode_hunk_tracker::SessionSummary) -> SessionSummaryWire {
    SessionSummaryWire {
...
fn tracker_action(kind: HunkActionKind) -> weepcode_hunk_tracker::types::HunkAction {
    match kind {
        HunkActionKind::Accept => weepcode_hunk_tracker::types::HunkAction::Accept,
        HunkActionKind::Reject => weepcode_hunk_tracker::types::HunkAction::Reject,
    }
}
...
fn session_tracker(
    ws: &WorkspaceHandle,
    session_id: Option<&str>,
) -> WorkspaceResult<weepcode_hunk_tracker::HunkTrackerHandle> {
    let sid = session_id
...
impl WorkspaceOp for HunkSingleActionReq {
    async fn execute(
        ...
        let hunk_id =
            weepcode_hunk_tracker::types::HunkId::from_string(self.action.hunk_id.clone());
    ...
    async fn execute(
        ...
        for h in &all_hunks {
            ...
            let is_agent = matches!(
                h.source,
                weepcode_hunk_tracker::types::HunkSource::AgentEdit { .. }
            );
...
fn hook_registry_to_wire(
    registry: &weepcode_hooks::discovery::HookRegistry,
...
) -> WorkspaceResult<weepcode_hooks::discovery::HookRegistry> {
    let value = serde_json::to_value(wire).map_err(|e| WorkspaceError::HubError(e.to_string()))?;
    let mut registry: weepcode_hooks::discovery::HookRegistry =
        serde_json::from_value(value).map_err(|e| WorkspaceError::HubError(e.to_string()))?;
...
fn resolve_index_for_workspace(
    ...
    std::sync::Arc<weepcode_codebase_graph::IndexManagerHandle>,
    std::path::PathBuf,
)> {
...
fn query_result_to_response(
    result: Result<weepcode_codebase_graph::QueryResult, weepcode_codebase_graph::QueryError>,
) -> CodeNavResponse {
    match result {
...
fn symbol_locations_to_response(
    locations: Vec<weepcode_codebase_graph::SymbolLocation>,
) -> CodeNavResponse {
    CodeNavResponse {
        locations: locations
            .into_iter()
...
///   session's [`FinalizedToolset`](weepcode_tools::registry::types::FinalizedToolset).
...
impl WorkspaceOps {
    ...
    pub fn bind_local_session(
        ...
        hunk_tracker: weepcode_hunk_tracker::HunkTrackerHandle,
    ...
    pub async fn hook_registry(&self) -> WorkspaceResult<weepcode_hooks::discovery::HookRegistry> {
    ...
    ///   [`FinalizedToolset`](weepcode_tools::registry::types::FinalizedToolset)
        ...
        match self {
            ...
            Self::Proxy { client } => {
                if !client.is_connected() {
                    return Err(weepcode_tool_runtime::ToolError::network_error(
                        "The workspace server connection was lost. \
                         Please restart your session to reconnect.",
                    ));
                }
                let tool_id = weepcode_tool_protocol::ToolId::new(name).map_err(|e| {
                    weepcode_tool_runtime::ToolError::custom(
                        "hub_proxy_error",
                        format!("invalid tool name: {e}"),
                    )
                })?;
                let mut ctx = weepcode_tool_runtime::ToolCallContext::default();
                ctx.call_id = weepcode_tool_protocol::ToolCallId::new(call_id.to_owned())
...
mod tests {
    ...
    /// request types are defined in `weepcode-workspace-types` (and re-exported
    ...
    fn git_op_cwd_uses_explicit_git_root_per_window() {
        ...
        let window_a = std::path::PathBuf::from("/repos/weepcode-main");
        let window_b = std::path::PathBuf::from("/repos/weepcode-main-2");
        assert_eq!(
    ...
    async fn end_local_session_drops_bound_toolset() {
        ...
        let toolset = std::sync::Arc::new(
            weepcode_tools::registry::types::FinalizedToolset::empty_for_test(),
        );
    ...
    fn hunk_to_wire_serializes_identically() {
        use weepcode_hunk_tracker::types::{Hunk, HunkSource};
    ...
    fn file_content_entry_to_wire_serializes_identically() {
        use weepcode_hunk_tracker::FileContentEntry;
        use weepcode_hunk_tracker::types::FileContentView;
    ...
    fn session_summary_to_wire_serializes_identically() {
        use std::sync::Arc;
        use weepcode_hunk_tracker::SessionSummary;
        use weepcode_hunk_tracker::types::{Hunk, HunkSource, TurnSummary};
    ...
    fn hook_registry_wire_round_trip_both_directions() {
        let spec = weepcode_hooks::config::HookSpec {
            ...
            event: weepcode_hooks::event::HookEventName::PreToolUse,
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/home/u/.weepcode/hooks"),
            extra_env: std::collections::HashMap::from([("FOO".to_string(), "bar".to_string())]),
        };
        let mut registry = weepcode_hooks::discovery::HookRegistry::default();
    ...
    fn hook_event_name_wire_covers_all_upstream_variants() {
        use weepcode_hooks::event::HookEventName as E;
    ...
    /// can't depend on `weepcode-hooks`, and `hook_registry_to_wire` only couples
    ...
    fn hook_spec_wire_covers_all_upstream_fields() {
        use weepcode_hooks::config::HookSpec;
