...
pub enum ClientType {
    ...
    #[default]
    #[serde(rename = "generic", alias = "weepcode-shell", alias = "weepcode_shell")]
    Generic,
    /// WeepCode TUI client - show fancy options with interactive bash term selection
    #[serde(rename = "weepcode-tui", alias = "weepcode_tui")]
    WeepcodeTUI,
    /// WeepCode Web client - identified by clientIdentifier "weepcode-web"
    #[serde(rename = "weepcode_web")]
    WeepcodeWeb,
    /// Named client (`"nebula"`) — uses the generic permission UI
    #[serde(rename = "nebula")]
    Nebula,
    /// IDE extension client (VS Code and similar) - identified by clientIdentifier "weepcode-code-extension"
    #[serde(rename = "extension")]
    Extension,
    /// WeepCode Pager client - TUI-like terminal pager with interactive permission UI.
    /// Treated identically to WeepcodeTUI for permission options (gets bash highlights +
    /// interactive selection). Reports as "pager" for telemetry attribution.
    ///
    /// Accepts both the hyphenated `"weepcode-pager"` (what the pager actually
    /// sends over the wire, matching `PAGER_CLIENT_TYPE`) and the underscored
    /// `"weepcode_pager"` form for symmetry with the rest of this enum.
    #[serde(rename = "weepcode-pager", alias = "weepcode_pager")]
    WeepcodePager,
    /// WeepCode Desktop (Electron) client - identified by clientIdentifier "weepcode-desktop".
    /// Uses TUI-style bash permission options (primary command extraction + prefix matching)
    /// but without interactive `<`/`>` word selection.
    #[serde(rename = "weepcode_desktop")]
    Desktop,
}
impl ClientType {
    /// Product token for the `User-Agent` header (e.g. `weepcode-pager`).
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "weepcode-shell",
            Self::WeepcodeTUI => "weepcode-tui",
            Self::WeepcodeWeb => "weepcode-web",
            Self::Nebula => "nebula",
            Self::Extension => "weepcode-code-extension",
            Self::WeepcodePager => "weepcode-pager",
            Self::Desktop => "weepcode-desktop",
        }
    }
    /// Resolve from ACP `clientIdentifier` string (e.g. `"weepcode-web"`, `"weepcode-desktop"`).
    pub fn from_client_identifier(id: Option<&str>) -> Self {
        match id {
            Some("weepcode-web") => Self::WeepcodeWeb,
            Some("nebula") => Self::Nebula,
            Some("weepcode-code-extension") => Self::Extension,
            Some("weepcode-desktop") => Self::Desktop,
            Some("weepcode-pager") => Self::WeepcodePager,
            _ => Self::Generic,
        }
    }
    ...
    pub fn feedback_label(&self) -> &'static str {
        match self {
            Self::WeepcodeTUI | Self::WeepcodePager => "tui",
            Self::WeepcodeWeb => "web",
            Self::Nebula => "nebula",
...
impl From<&weepcode_tools::types::ToolInput> for AccessKind {
    fn from(input: &weepcode_tools::types::ToolInput) -> Self {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.weepcode/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use weepcode_tools::implementations::weepcode_build_hashline::edit::types::HashlineEditInput;
        use weepcode_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use weepcode_tools::implementations::weepcode_build::bash::BashToolInput;
        use weepcode_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use weepcode_tools::implementations::use_tool::UseToolInput;
        use weepcode_tools::types::ToolInput;
    ...
    fn monitor_maps_to_bash_access() {
        use weepcode_tools::implementations::weepcode_build::monitor::types::MonitorInput;
        use weepcode_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use weepcode_tools::implementations::weepcode_build::web_fetch::WebFetchInput;
        use weepcode_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use weepcode_tools::implementations::weepcode_build::web_search::WebSearchInput;
        use weepcode_tools::types::ToolInput;
    ...
    #[test]
    fn client_type_deserializes_weepcode_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("weepcode-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("weepcode_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
