...
///
/// ```bash
/// WEEPCODE_API_KEY='weepcode-some-key' cargo run --bin weepcode-pager
/// ```
///
...
///   prefix: ["WEEPCODE_API_KEY=weepcode-some-key"]
///   highlighted_words: ["cargo", "run", "--bin", "weepcode-pager"]
///   suffix: []
...
mod tests {
    ...
    fn test_parse_plain_commands_from_script() {
        ...
        assert_eq!(
            ...
            Some(BashCommandHighlights {
                prefix: vec!["WEEPCODE_API_KEY=weepcode-some-key".to_owned()],
                highlighted_words: vec![
                    "cargo".to_owned(),
                    "run".to_owned(),
                    "--bin".to_owned(),
                    "weepcode-pager".to_owned()
                ],
                suffix: vec![],
            })
        );
        ...
        let redirection_command = "cargo build --bin weepcode-pager 2>&1";
        assert_eq!(
        ...
        let another_long_command = "cargo test --package weepcode-shell --lib -- permission::bash_command_splitting::tests::test_parse_plain_commands_from_script --exact --nocapture 2>&1";
        assert_eq!(
