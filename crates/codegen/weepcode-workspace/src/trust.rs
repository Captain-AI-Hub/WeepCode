...
//!
//! Persists per-folder trust decisions to `~/.weepcode/trusted_folders.toml`.
...
//!
//! The store is rooted at [`weepcode_config::user_weepcode_home`] — the **Option**
//! home that resolves to `None` (rather than a cwd-relative `./.weepcode`) when
//! neither `$WEEPCODE_HOME` nor a home directory is set (e.g. a minimal container /
...
//! a `./.weepcode/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.weepcode/`.
...
impl TrustStore {
    /// Load the trust store from `<user_weepcode_home>/trusted_folders.toml`.
    ///
    ...
    /// Default on-disk path: `<user_weepcode_home>/trusted_folders.toml`, or `None`
    /// when no user home resolves.
    ///
    /// Resolves via [`weepcode_config::user_weepcode_home`], never
    /// [`weepcode_config::weepcode_home`], so it never falls back to a cwd-relative
    /// `./.weepcode` — that fallback would let an untrusted cloned repo's `.weepcode`
    /// masquerade as the user-global store and self-trust the checkout.
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(weepcode_config::user_weepcode_home())
    }

    /// Map a resolved user-weepcode-home to the store path, preserving "no home" as
    ...
    fn default_path_in(user_weepcode_home: Option<PathBuf>) -> Option<PathBuf> {
        Some(user_weepcode_home?.join(TRUST_FILE_NAME))
    }
    ...
    fn record_decision(&mut self, workspace_key: &Path, trusted: bool) -> io::Result<()> {
        ...
        let Some(path) = self.path.as_deref() else {
            tracing::warn!(
                path = %canonical.display(),
                trusted,
                "folder trust: no user weepcode home resolved; trust decision not recorded"
            );
            return Ok(());
        };
...
///
/// A weepcode-managed worktree first collapses onto its recorded source repo's git
/// ROOT (via the `~/.weepcode/worktrees.db` registry), so every `weepcode -w` worktree
...
/// `weepcode -w` was launched from (the recorded source repo may be a repo subdir).
...
/// `weepcode -w` worktree of a repo shares one trust key. The collapse fires ONLY
...
fn git_derived_workspace_key(cwd: &Path) -> PathBuf {
    // A weepcode-managed worktree (any creation mode, incl. standalone clones git
    ...
    if let Some(source_repo) = crate::worktree::source_repo_for_cwd(&cwd.to_string_lossy()) {
        ...
        // ONE key regardless of the subdir weepcode -w was launched from (parity with
...
/// sharing `~/.weepcode/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.weepcode/trusted-hook-projects` file listed one canonical project
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = weepcode_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
/// is testable without the process-global weepcode-home cache. Returns the number
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match weepcode_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        let home = PathBuf::from("/home/alice/.weepcode");
        assert_eq!(
        ...
        // cwd-relative `./.weepcode` that weepcode_home() would invent, which is exactly
        // how a cloned repo's own `<repo>/.weepcode/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
        assert_eq!(TrustStore::default_path_in(None), None);
    }

    #[test]
    fn default_path_sources_from_user_weepcode_home() {
        // Thin source-pin: the production accessor reads user_weepcode_home()
        // (Option, no cwd fallback), not weepcode_home(). The real regression guard
        // is the seam test above (default_path_in(None) == None).
        assert_eq!(
            TrustStore::default_path(),
            weepcode_config::user_weepcode_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
    ...
    fn workspace_key_collapses_linked_worktrees_onto_main_checkout() {
        // Every linked `weepcode -w` worktree of a repo must share ONE trust key:
    ...
    // ── workspace_key registry collapse (weepcode-managed worktrees) ─────────
    ...
    /// Point `WEEPCODE_HOME` at an isolated tempdir and register one weepcode-managed
    ...
    /// dir)`; the [`LockedTestEnv`] holds the lock and restores `WEEPCODE_HOME` on
    /// drop (before releasing the lock), so the caller may bind it any way.
    fn register_weepcode_worktree(
        temp: &tempfile::TempDir,
        name: &str,
        source_repo: &Path,
        creation_mode: &str,
    ) -> (LockedTestEnv, PathBuf) {
        use weepcode_fast_worktree::{WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
    ...
    #[test]
    fn workspace_key_collapses_standalone_weepcode_worktree_onto_source_repo() {
        ...
        let (_env, wt) = register_weepcode_worktree(&temp, "wt", &source_repo, "standalone");

        let expected = canonicalize_or_owned(&source_repo);
        assert_eq!(
            workspace_key(&wt),
            expected,
            "a standalone weepcode worktree must collapse onto its recorded source repo"
        );
    ...
    fn workspace_key_collapses_worktree_onto_source_repo_git_root() {
        ...
        let (_env, wt) = register_weepcode_worktree(&temp, "wt", &subdir, "standalone");
    ...
    fn workspace_key_ignores_registry_for_cwd_outside_worktrees_dir() {
        ...
        // `<weepcode_home>/worktrees`: `worktree_record_for_cwd` skips the registry
        ...
        // worktree cwd, and `outside` is its OWN git repo (under weepcode HOME but not
        ...
        let (_env, _wt) = register_weepcode_worktree(&temp, "wt", &source_repo, "standalone");

        // Under weepcode HOME but NOT under `<home>/worktrees`, and its own git repo.
        let outside = root
            .join("weepcode-home")
            .join("not-worktrees")
            .join("proj");
        ...
        assert_eq!(
            ...
            "a cwd outside <weepcode_home>/worktrees keys on its own repo root"
        );
        assert_ne!(
            key,
