...
//! spawns the unchanged `/usr/local/bin/weepcode-preview-proxy` binary as a
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/weepcode-preview-proxy";
...
/// `weepcode_workspace_preview_proxy_restart_total{reason}` — (re)start events the
...
static PREVIEW_PROXY_RESTART_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "weepcode_workspace_preview_proxy_restart_total",
...
impl PreviewArgs {
    ...
    /// `weepcode-preview-proxy/src/cli.rs`). Absent options and a false
...
/// Build the unspawned proxy command. Secrets (`WEEPCODE_SERVER_KEY` /
/// `WEEPCODE_SESSION_ID`) reach the proxy by env inheritance — never argv.
fn build_preview_command(cfg: &PreviewArgs) -> io::Result<tokio::process::Command> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Raw pre_exec, NOT weepcode_tty_utils::detach_command: the proxy must stay in
...
/// `weepcode-preview-proxy`'s `/__control/activity` route).
...
/// the default in `weepcode-preview-proxy/src/cli.rs`).
...
mod tests {
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match weepcode-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "WEEPCODE_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
