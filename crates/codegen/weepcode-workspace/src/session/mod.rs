...
use std::sync::Arc;
use weepcode_computer_hub_mcp_adapter::McpBridgeHandle;
use weepcode_hunk_tracker::HunkTrackerHandle;
use weepcode_mcp::servers::McpState;
use weepcode_tool_protocol::ToolId;
use weepcode_tool_runtime::WorkspaceViewerContext;
use weepcode_tools::notification::types::{ToolNotification, ToolNotificationHandle};
use weepcode_tools::registry::types::{FinalizedToolset, ToolConfig, ToolServerConfig};
...
pub struct WorkspaceSession {
    ...
    ///
    /// [`HunkTrackerActor`]: weepcode_hunk_tracker::HunkTrackerActor
    ...
    pub(crate) hunk_checkpoints:
        Arc<tokio::sync::Mutex<HashMap<usize, weepcode_hunk_tracker::HunkTurnDelta>>>,
...
impl WorkspaceSession {
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<weepcode_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<weepcode_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<weepcode_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<weepcode_hooks::error::HookError>,
    ...
    /// Auth provider for WeepCode service calls.
    pub(crate) auth_provider: Option<weepcode_computer_hub_sdk::SharedAuthProvider>,
    ...
    pub(crate) activity_notify_handle:
        arc_swap::ArcSwap<Option<weepcode_tools::notification::types::ToolNotificationHandle>>,
    ...
    pub(crate) local_registry: weepcode_computer_hub_sdk::LocalRegistry,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn weepcode_tools::implementations::lsp::LspBackend>>,
    ...
    /// (from `WEEPCODE_WORKSPACE_REWIND_ALL_OUTCOMES`, default off).
    pub(crate) workspace_rewind_all_outcomes: bool,
    /// Resolved `$WEEPCODE_WORKSPACE_HOME` — the workspace-owned on-disk state root
    /// (`<weepcode_home>/workspace` by default). The upload queue spills here.
    pub(crate) workspace_home: std::path::PathBuf,
    pub(crate) upload_queue: Option<std::sync::Arc<weepcode_file_utils::queue::UploadQueue>>,
    ...
    /// (`WEEPCODE_WORKSPACE_EVENTS_ENABLED=true`). When `false`, every
    /// [`session_event_writer`](Self::session_event_writer) hands back an
    /// [`EventWriter::noop()`](weepcode_file_utils::events::EventWriter::noop) and
    ...
    /// enabled (`WEEPCODE_WORKSPACE_TOOL_DEFS_ENABLED=true`).
    ...
    pub(crate) session_event_writers:
        Arc<dashmap::DashMap<String, weepcode_file_utils::events::EventWriter>>,
    ...
    pub(crate) inflight_enqueues: dashmap::DashMap<
        (String, u64),
        tokio::task::JoinHandle<weepcode_file_utils::queue::EnqueueOutcome>,
    >,
...
impl WorkspaceShared {
    ...
    pub fn upload_queue(&self) -> Option<&std::sync::Arc<weepcode_file_utils::queue::UploadQueue>> {
    ...
    /// [`EventWriter::noop()`](weepcode_file_utils::events::EventWriter::noop)
    ...
    pub(crate) fn session_event_writer(
        &self,
        session_id: &str,
    ) -> weepcode_file_utils::events::EventWriter {
    ...
    pub(crate) fn session_event_writer_cached(
        &self,
        session_id: &str,
    ) -> Option<weepcode_file_utils::events::EventWriter> {
    ...
    /// Auth provider used for WeepCode service calls.
    pub fn auth_provider(&self) -> Option<&weepcode_computer_hub_sdk::SharedAuthProvider> {
    ...
    ///
    /// Returns a clone of the [`ToolServer`](weepcode_computer_hub_sdk::ToolServer)
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<weepcode_workspace_types::WorkspaceEvent> {
...
pub(crate) fn get_or_open_session_writer(
    enabled: bool,
    writers: &dashmap::DashMap<String, weepcode_file_utils::events::EventWriter>,
    workspace_home: &Path,
    session_id: &str,
) -> weepcode_file_utils::events::EventWriter {
...
mod tests {
    ...
    use dashmap::DashMap;
    use weepcode_file_utils::events::{Event, EventWriter};
