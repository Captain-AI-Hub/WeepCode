...
use std::sync::Arc;
use weepcode_tools::registry::types::{
...
pub(crate) fn resolve_session_toolset(
    ...
    local_registry: Option<weepcode_computer_hub_sdk::LocalRegistry>,
    ...
    notification_handle: Option<weepcode_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<weepcode_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<weepcode_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn weepcode_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            weepcode_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// enabled (`WEEPCODE_WORKSPACE_TOOL_STATE_ENABLED=true`; any other value keeps
/// legacy behavior).
pub fn tool_state_enabled() -> bool {
    std::env::var("WEEPCODE_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
fn sanitize_session_id(session_id: &str) -> String {
    ...
    if modified {
        let digest = weepcode_file_utils::sha256_hex(session_id.as_bytes());
        safe.push('-');
...
/// `WEEPCODE_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
/// [`LocalTerminalBackend`]: weepcode_tools::computer::local::LocalTerminalBackend
pub struct WorkspaceSessionContextFactory {
    auth: Option<weepcode_computer_hub_sdk::SharedAuthProvider>,
    api_base_url: Option<String>,
    /// Resolved `$WEEPCODE_WORKSPACE_HOME` when tool-state persistence is enabled;
...
impl WorkspaceSessionContextFactory {
    ...
    /// (`$WEEPCODE_WORKSPACE_HOME`). Callers should only invoke this when
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(weepcode_tools::computer::local::LocalFs)
            as Arc<dyn weepcode_tools::computer::types::AsyncFileSystem>;
        let notification_handle = weepcode_tools::notification::ToolNotificationHandle::noop();
        let (image_gen_config, video_gen_config, web_search_config, app_builder_deployer_config) =
            if let (Some(auth), Some(url)) = (&self.auth, &self.api_base_url) {
                ...
                match cred {
                    weepcode_computer_hub_sdk::AuthCredential::Bearer { token, .. } => {
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    headers.insert(
        ...
        format!("weepcode-workspace/{version}"),
    );
    headers.insert("x-weepcode-client-version".to_string(), version.to_string());
...
/// `WEEPCODE_DISABLE_WEB_FETCH=1` is set.
fn build_web_fetch_config()
-> weepcode_tools::implementations::weepcode_build::web_fetch::WebFetchConfig {
    use weepcode_tools::implementations::weepcode_build::web_fetch::{
        WebFetchConfig, WebFetchParams,
    };
    if std::env::var("WEEPCODE_DISABLE_WEB_FETCH").is_ok_and(|v| v == "1" || v == "true") {
        return WebFetchConfig::Disabled;
    }
    let mut params = WebFetchParams::default();
    if let Ok(proxy) = std::env::var("WEEPCODE_WEB_FETCH_PROXY") {
        params.proxy_endpoint = Some(proxy);
    }
    WebFetchConfig::Enabled { params }
}
fn default_web_search_model() -> String {
    std::env::var("WEEPCODE_WEB_SEARCH_MODEL")
        .unwrap_or_else(|_| "weepcode-4.20-multi-agent".to_string())
}
    ...
    pub fn baseline_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("WeepcodeBuild:search_replace", Some(ToolKind::Edit)),
                tc("WeepcodeBuild:grep", Some(ToolKind::Search)),
...
mod tests {
    ...
    async fn resolve_session_toolset_mcp_merge_dedup_by_id_baseline_wins() {
        ...
        let mut mcp_dup = test_support::tc("WeepcodeBuild:read_file", Some(ToolKind::Read));
    ...
    fn backfill_tool_kinds_fills_known_kindless_ids_only() {
        let kinds = HashMap::from([
            ("WeepcodeBuild:search_replace".to_owned(), ToolKind::Edit),
            ("WeepcodeBuild:read_file".to_owned(), ToolKind::Read),
        ]);
        let config = ToolServerConfig {
            tools: vec![
                test_support::tc("WeepcodeBuild:search_replace", None),
                test_support::tc("adhoc.opaque", None),
                test_support::tc("WeepcodeBuild:read_file", Some(ToolKind::Search)),
            ],
        ...
        assert_eq!(
            kind_of("WeepcodeBuild:search_replace"),
            Some(ToolKind::Edit)
        );
        assert_eq!(
        ...
        assert_eq!(
            kind_of("WeepcodeBuild:read_file"),
    ...
    async fn resolve_session_toolset_readonly_filters_kindless_pinned_tools() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("WeepcodeBuild:read_file", None),
                test_support::tc("WeepcodeBuild:grep", None),
                test_support::tc("WeepcodeBuild:list_dir", None),
                test_support::tc("WeepcodeBuild:search_replace", None),
                test_support::tc("WeepcodeBuild:run_terminal_cmd", None),
            ],
            behavior_preset: None,
        };
        let (eff, ts, _backend) = resolve_session_toolset(
    ...
    async fn resolve_session_toolset_mcp_kind_none_dropped_under_readonly() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("WeepcodeBuild:read_file", Some(ToolKind::Read)),
        ...
        assert!(
            kept_ids.contains(&"WeepcodeBuild:read_file"),
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "WEEPCODE_WORKSPACE_TOOL_STATE_ENABLED";
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use weepcode_tools::types::resources::{State, WebCitationCounter};
