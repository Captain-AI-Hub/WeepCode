...
use weepcode_computer_hub_sdk::ToolServerHandler;
use weepcode_tool_protocol::{HookEvent, HookFrame, SessionId, ToolId, ToolServerEvictParams};
...
use weepcode_tools::implementations::weepcode_build::scheduler::interval::interval_to_human;
use weepcode_tools::implementations::weepcode_build::scheduler::types::{
    SchedulerCommand, SchedulerHandle,
};
use weepcode_tools::registry::types::FinalizedToolset;
use weepcode_tools::types::resources::Terminal;
use weepcode_workspace_types::rpc::workspace::{
...
static WORKSPACE_RPC_CALLER_MISMATCH_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "weepcode_workspace_rpc_caller_mismatch_total",
...
static WORKSPACE_RPC_MUTATION_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "weepcode_workspace_rpc_mutation_total",
...
static WORKSPACE_RPC_REQUESTS_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "weepcode_workspace_rpc_requests_total",
...
static WORKSPACE_RPC_DURATION_SECONDS: std::sync::LazyLock<HistogramVec> =
    std::sync::LazyLock::new(|| {
        register_histogram_vec!(
            "weepcode_workspace_rpc_duration_seconds",
...
) -> Vec<weepcode_workspace_types::rpc::workspace::BackgroundTaskSummaryWire> {
    use weepcode_tools::computer::types::TaskKind;
    use weepcode_tools::types::resources::Terminal;
    use weepcode_tools::types::tool::ToolKind;
    use weepcode_workspace_types::rpc::workspace::BackgroundTaskSummaryWire;
    let terminal = {
...
async fn list_session_todos(
    toolset: &weepcode_tools::registry::types::FinalizedToolset,
) -> Vec<weepcode_workspace_types::rpc::workspace::TodoSummaryWire> {
    use weepcode_tools::implementations::weepcode_build::todo::{TodoState, TodoStatus};
    use weepcode_tools::types::resources::State;
    use weepcode_workspace_types::rpc::workspace::TodoSummaryWire;
    let res = toolset.resources.lock().await;
...
impl WorkspaceRpcHandler {
    ...
    async fn dispatch(
        ...
        use weepcode_workspace_types::rpc::git::{GitBranchInfoReq, GitMetadataReq};
        use weepcode_workspace_types::rpc::search::FuzzyStatusReq;
        use weepcode_workspace_types::rpc::skills::DiscoverPluginsReq;
        use weepcode_workspace_types::rpc::workspace::{
        ...
        use weepcode_workspace_types::rpc::worktree::WorktreeCreateSyncReq;
...
impl ToolServerHandler for WorkspaceRpcHandler {
    ...
    async fn handle_hook_request(&self, session_id: SessionId, frame: HookFrame) -> Option<Value> {
        use weepcode_tool_protocol::turn_hook::{self, TurnHookRequest};
...
mod tests {
    ...
    use weepcode_tool_protocol::turn_hook;
    use weepcode_tools::implementations::weepcode_build::scheduler::types::ScheduledTask;
    /// Helper: consume the first item from a ToolStream.
    async fn next_item(
        stream: &mut ToolStream<TypedToolOutput>,
    ) -> Option<weepcode_tool_runtime::ToolStreamItem<TypedToolOutput>> {
    ...
    async fn handle_evict_triggers_two_phase_drain() {
        use weepcode_tool_protocol::ToolServerLifecycleStatus;
        let handle = make_handle();
    ...
    async fn list_background_tasks_rpc_stays_truthful_across_rebinds() {
        ...
        use weepcode_workspace_types::rpc::workspace::ListBackgroundTasksResponse;
        let handle = make_handle();
        ...
        let read_only = ToolServerConfig {
            tools: vec![tc(
                ...
                Some(weepcode_tools::types::tool::ToolKind::Read),
            )],
    ...
    async fn handle_call_wraps_in_envelope_with_value() {
        ...
        ctx.extensions
            .insert(weepcode_tool_runtime::SessionContext("main".to_owned()));
        ...
        match item {
            weepcode_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => {
                let ok_val = typed
                    .value
                    .get("ok")
    ...
    async fn handle_call_records_rpc_metrics_and_collapses_unknown_method() {
        ...
        let has_bogus_series = prometheus::gather()
            .iter()
            .filter(|mf| mf.name() == "weepcode_workspace_rpc_requests_total")
    ...
    async fn handle_hook_cancel_marks_call_completed() {
        use weepcode_tool_protocol::ToolCallId;
        let handle = make_handle();
        let handler = WorkspaceRpcHandler::new(handle.clone());
        let tracker = handle.activity_tracker();
