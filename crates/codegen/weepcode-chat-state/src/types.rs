...
/// emitter in `weepcode-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (weepcode login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
    /// From user config ([model.*] api_key, env_key, WEEPCODE_API_KEY). Not refreshable.
    ApiKey,
}
...
/// `weepcode_sampling_types::SamplingConfig` (which is secret-free).
