//! weepcode-chat-state — Actor-based chat state management for WeepCode agents.
//!
//! This crate extracts conversation state management from `weepcode-shell`'s
//! `acp_session.rs` into a standalone actor. It follows the same actor pattern
//! as `weepcode-hunk-tracker`:
//!
//! ```text
//! ┌────────────────┐                  ┌──────────────────────────────────────┐
