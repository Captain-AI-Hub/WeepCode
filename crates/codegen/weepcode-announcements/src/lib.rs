//! Shared announcement types, persistence, and formatting for WeepCode CLI apps.
//!
//! This crate provides the common logic used by `weepcode-shell` and
//! `weepcode-pager` for handling announcements (banner notifications).
...
/// Read hidden announcement ids from `~/.weepcode/announcements.json`.
...
/// Write hidden announcement ids to `~/.weepcode/announcements.json`.
...
fn announcements_state_path() -> PathBuf {
    weepcode_tools::util::weepcode_home::weepcode_home().join("announcements.json")
}
...
///
/// Precedence: `WEEPCODE_ANNOUNCEMENTS_OVERRIDE` env var (JSON override) → remote announcements.
...
pub fn resolve_startup(
    ...
    if let Ok(raw) = std::env::var("WEEPCODE_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => return Some(list),
            Err(e) => {
                tracing::warn!(error = %e, "invalid WEEPCODE_ANNOUNCEMENTS_OVERRIDE JSON; ignoring override");
            }
        }
    }
...
mod tests {
    ...
    fn resolve_startup_env_override() {
        ...
        unsafe {
            std::env::set_var("WEEPCODE_ANNOUNCEMENTS_OVERRIDE", r#"[{"id":"test"}]"#);
        }
        ...
        // SAFETY: test-only
        unsafe {
            std::env::remove_var("WEEPCODE_ANNOUNCEMENTS_OVERRIDE");
        }
    }
    ...
    fn cta_parses_nested_partial_and_absent() {
        let full: RemoteAnnouncement = serde_json::from_str(
            r#"{"id":"p","severity":"promo","cta":{"label":"Open docs","url":"https://example.com/weepcode","caption":"or use Ctrl+O"}}"#,
        )
        .unwrap();
        let cta = full.cta.as_ref().expect("cta present");
        assert_eq!(cta.label.as_deref(), Some("Open docs"));
        assert_eq!(cta.url.as_deref(), Some("https://example.com/weepcode"));
