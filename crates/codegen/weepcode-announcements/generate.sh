...
# are the single source of truth for the `weepcode/announcements/update` payload.
# Pipeline:
...
DESKTOP_DIR="$REPO_ROOT/frontend/apps/weepcode-desktop"
...
for f in "${bindings[@]}"; do
  {
    ...
    echo "// Regenerate via generate.sh in the weepcode-announcements package."
    cat "$f"
  } >"$DST/$(basename "$f")"
done
