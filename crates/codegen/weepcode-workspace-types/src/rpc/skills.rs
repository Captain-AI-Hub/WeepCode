...
//! `weepcode-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    /// cwd/.weepcode/skills
    Local,
    /// repo_root/.weepcode/skills
    Repo,
    /// ~/.weepcode/skills
    User,
    /// ~/.weepcode/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.weepcode/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    // Fixture mirrored field-for-field from the weepcode-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.weepcode/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.weepcode/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.weepcode/plugins/infra-plugin",
            "plugin_data": "/root/.weepcode/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "weepcode-4",
            "effort": "high",
            "user_invocable": true,
