...
//!
//! `weepcode_hooks` pulls in `git2`/`reqwest`/`weepcode-tools`, too heavy for
...
/// Wire mirror of `weepcode_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `weepcode_hooks::config::HookSpec`.
///
...
/// `weepcode-hooks`, so a server-side test (`weepcode-workspace`'s
...
/// Wire mirror of `weepcode_hooks::event::HookEventName`.
///
        ...
        let json = serde_json::json!({
            "hooks": {
                "pre_tool_use": [{
                    ...
                    "url_raw": null,
                    "timeout_ms": 5000,
                    "source_dir": "/home/u/.weepcode/hooks",
                    "extra_env": { "FOO": "bar" }
                }]
            }
        });
