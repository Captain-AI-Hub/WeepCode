//! Wire types for the `weepcode-workspace` API.
//!
...
//! `weepcode-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `weepcode-workspace-types` (instead of `weepcode-workspace` or
/// `weepcode-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `weepcode_workspace::permission`
