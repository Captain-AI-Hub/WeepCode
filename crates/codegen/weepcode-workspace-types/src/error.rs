...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] weepcode_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `weepcode-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `weepcode_tools::ToolError`) happens
