...
//! agent-session config types in `weepcode-config` and friends.
...
/// `weepcode-tools` once the wire surface is firm.
...
///
/// TODO(workspace): align with `weepcode_config::ProjectConfig`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
