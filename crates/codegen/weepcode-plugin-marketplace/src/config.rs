//! Parse marketplace sources from `~/.weepcode/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
//! name = "WeepCode Official"
//! git = "https://github.com/weepcode-org/weepcode-plugin-marketplace.git"
//!
//! [[marketplace.sources]]
//! name = "Local Dev"
//! path = "~/dev/my-plugins"
...
/// `WEEPCODE_MARKETPLACE_REQUIRE_SHA=1`. Tighten-only: either source can enable,
...
pub fn env_require_sha() -> bool {
    weepcode_config::env_bool("WEEPCODE_MARKETPLACE_REQUIRE_SHA").unwrap_or(false)
}
...
/// and `known_marketplaces.json` files under `~/.weepcode/` and `~/.claude/`.
pub fn load_extra_sources_from_settings(existing: &[MarketplaceSource]) -> Vec<MarketplaceSource> {
    let roots: Vec<PathBuf> = [
        weepcode_config::user_weepcode_home(),
...
/// instead of `~/.weepcode`/`~/.claude`. Each root is checked for
...
mod tests {
    ...
    fn parse_git_source() {
        ...
        let sources = load_sources(&config);
        assert_eq!(sources.len(), 1);
        assert_eq!(sources[0].name, "WeepCode Official");
        assert!(
            matches!(&sources[0].kind, SourceKind::Git { url, branch } if url.contains("weepcode-org") && branch.as_deref() == Some("main"))
        );
    }
    ...
    fn require_sha_policy_composition() {
        ...
        unsafe { std::env::remove_var("WEEPCODE_MARKETPLACE_REQUIRE_SHA") };
        assert!(!load_require_sha(&empty), "absent everywhere → off");
        assert!(load_require_sha(&enabled), "config alone can enable");

        unsafe { std::env::set_var("WEEPCODE_MARKETPLACE_REQUIRE_SHA", "1") };
        assert!(load_require_sha(&empty), "env alone can enable");

        unsafe { std::env::set_var("WEEPCODE_MARKETPLACE_REQUIRE_SHA", "0") };
        assert!(
