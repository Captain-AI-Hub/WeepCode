...
//!
//! Catalog file lookup, in order: `.weepcode-plugin/marketplace.json` (preferred),
//! `.weepcode-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.weepcode-plugin/marketplace.json` (preferred WeepCode convention)
/// 2. `.weepcode-plugin/plugin.json`
...
mod tests {
    ...
    #[test]
    fn load_index_valid_weepcode_dir() {
        let dir = tempfile::tempdir().unwrap();
        let weepcode_dir = dir.path().join(".weepcode-plugin");
        std::fs::create_dir_all(&weepcode_dir).unwrap();
        std::fs::write(
            weepcode_dir.join("marketplace.json"),
            r#"{"name": "weepcode", "plugins": []}"#,
        )
        .unwrap();
        let result = load_index(dir.path()).unwrap();
        assert!(result.is_some());
        assert_eq!(result.unwrap().name, "weepcode");
    }

    #[test]
    fn load_index_weepcode_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [
            (".weepcode-plugin", "weepcode"),
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "weepcode");
    }
