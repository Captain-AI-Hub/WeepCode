...
/// Display name of the official WeepCode marketplace source.
pub const OFFICIAL_SOURCE_NAME: &str = "WeepCode Official";

/// Git URL of the official WeepCode marketplace source. Auto-registered on first run.
pub const OFFICIAL_SOURCE_GIT_URL: &str = "https://github.com/weepcode-org/plugin-marketplace.git";

/// Whether `url` is the official WeepCode marketplace source, normalizing case, a
/// `www.` prefix, a trailing `/` or `.git`, and HTTPS/SSH forms before comparing.
pub fn is_official_source_url(url: &str) -> bool {
    canonical_github_owner_repo(url).as_deref() == Some("weepcode-org/plugin-marketplace")
}
...
mod tests {
    ...
    fn is_official_matches_canonical_https() {
        ...
        assert!(is_official_source_url(
            "https://github.com/weepcode-org/plugin-marketplace"
        ));
    }
    ...
    fn is_official_matches_ssh_form() {
        ...
        assert!(is_official_source_url(
            "git@github.com:weepcode-org/plugin-marketplace"
        ));
        assert!(is_official_source_url(
            "ssh://git@github.com/weepcode-org/plugin-marketplace.git"
        ));
        assert!(is_official_source_url(
            "ssh://git@github.com/weepcode-org/plugin-marketplace"
        ));
    }
    ...
    fn is_official_rejects_unrelated_urls() {
        ...
        assert!(!is_official_source_url(
            "https://github.com/weepcode-org/some-other-repo.git"
        ));
        assert!(!is_official_source_url(""));
    }
    ...
    fn is_official_matches_noncanonical_forms() {
        ...
        assert!(is_official_source_url(
            "http://github.com/weepcode-org/plugin-marketplace"
        ));
        assert!(is_official_source_url(
            "https://www.github.com/weepcode-org/plugin-marketplace.git"
        ));
        assert!(is_official_source_url(
            "git@github.com:WeepCode-org/plugin-marketplace.git"
        ));
