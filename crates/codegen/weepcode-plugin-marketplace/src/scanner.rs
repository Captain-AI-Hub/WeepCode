...
//!    the lookup order — `.weepcode-plugin/marketplace.json` is preferred), use it.
...
fn scan_single_plugin(plugin_dir: &Path, relative_path: &str) -> MarketplaceEntry {
    ...
    let manifest_result = weepcode_agent::plugins::manifest::load_manifest(plugin_dir);
    let manifest = match &manifest_result {
        Ok(weepcode_agent::plugins::manifest::ManifestLoadResult::Found(m)) => Some(m.as_ref()),
        _ => None,
    };
...
mod tests {
    ...
    fn url_sourced_entry_carries_keywords() {
        let dir = tempfile::tempdir().unwrap();
        let weepcode_dir = dir.path().join(".weepcode-plugin");
        std::fs::create_dir_all(&weepcode_dir).unwrap();
        std::fs::write(
            weepcode_dir.join("marketplace.json"),
            r#"{
                "name": "kw-marketplace",
                "plugins": [{
                    "name": "remote-plugin",
    ...
    #[test]
    fn weepcode_plugin_dir_index_drives_scan_end_to_end() {
        let dir = tempfile::tempdir().unwrap();
        make_plugin(dir.path(), "weepcode-plugin", "1.0.0");
        ...
        std::fs::write(
            weepcode_dir.join("marketplace.json"),
            r#"{
                "name": "weepcode-marketplace",
                "plugins": [{
                    "name": "weepcode-plugin",
                    "description": "From the .weepcode-plugin index",
                    "category": "design",
                    "source": { "type": "local", "path": "./plugins/weepcode-plugin" },
                    "tags": ["weepcode"]
                }]
            }"#,
        )
        ...
        let plugins = scan_marketplace(dir.path()).entries;
        assert_eq!(plugins.len(), 1);
        assert_eq!(plugins[0].name, "weepcode-plugin");
        assert_eq!(plugins[0].category.as_deref(), Some("design"));
        assert_eq!(plugins[0].tags, vec!["weepcode"]);
        assert!(plugins[0].keywords.is_empty());
    }
    ...
    fn write_weepcode_file(dir: &Path, file: &str, content: &str) {
        let weepcode_dir = dir.join(".weepcode-plugin");
        std::fs::create_dir_all(&weepcode_dir).unwrap();
        std::fs::write(weepcode_dir.join(file), content).unwrap();
    }
    ...
    fn url_entry_gets_components_when_sha_matches() {
        ...
        write_weepcode_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn url_entry_without_pinned_sha_gets_no_components() {
        let dir = tempfile::tempdir().unwrap();
        write_weepcode_file(dir.path(), "marketplace.json", &url_marketplace_index(""));
        write_weepcode_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn malformed_catalog_degrades_to_no_components() {
        ...
        write_weepcode_file(dir.path(), "plugin-index.json", "not json");
