...
//! `.weepcode-plugin/plugin-index.json` (preferred), then
...
use weepcode_hooks_plugins_types::PluginComponents;
...
mod tests {
    ...
    #[test]
    fn load_catalog_parses_weepcode_plugin_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".weepcode-plugin", BASIC);
    ...
    #[test]
    fn load_catalog_prefers_weepcode_dir_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".weepcode-plugin", BASIC);
        write_catalog(
            dir.path(),
    ...
    fn load_catalog_malformed_returns_none() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".weepcode-plugin", "not json");
        assert!(load_catalog(dir.path()).is_none());
    }
    ...
    fn load_catalog_ignores_unknown_fields() {
        ...
        write_catalog(
            ...
            ".weepcode-plugin",
            r#"{
                "$schema": "https://x.ai/weepcode/plugin-index.schema.json",
                "version": 1,
