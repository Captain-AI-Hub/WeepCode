...
This crate extracts a first-class `Agent` type from `weepcode-shell`.
...
`weepcode-shell`, another in-process host, or a headless batch runner.
...
in `.weepcode/agents/` (project-level) or `~/.weepcode/agents/` (user-level).

```rust
use weepcode_agent::{AgentDefinition, AgentBuilder};
use weepcode_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".weepcode/agents/code-reviewer.md")?;
...
```rust
use weepcode_agent::discovery;

// Find all .md files in .weepcode/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  weepcode-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`weepcode-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `weepcode-agent` depends on it for tool setup.
- **`weepcode-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `weepcode-agent` (AGENTS.md
...
|---|---|---|
| `weepcode-build` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p weepcode-agent

# Test
cargo test -p weepcode-agent

# Clippy
cargo clippy -p weepcode-agent --fix --allow-dirty
