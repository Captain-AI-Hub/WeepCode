...
//!
//! Plugins from project directories (`.weepcode/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.weepcode/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.weepcode/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.weepcode/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_weepcode_home() so a project's `.weepcode/trusted-plugins` is never
        // read as the user trust store when neither WEEPCODE_HOME nor a home dir resolves.
        let Some(weepcode) = weepcode_config::user_weepcode_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = weepcode.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.weepcode/trusted-plugins`.
    ...
    /// rewrites `~/.weepcode/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.weepcode/trusted-plugins`.
