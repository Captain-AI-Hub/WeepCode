...
///
/// Replaces `${WEEPCODE_PLUGIN_ROOT}`, `${CLAUDE_PLUGIN_ROOT}`,
/// `${WEEPCODE_PLUGIN_DATA}`, and `${CLAUDE_PLUGIN_DATA}` with the provided values.
///
/// Delegates to [`weepcode_tools::util::substitute_plugin_tokens`], the single
...
pub fn substitute_env_vars(s: &str, plugin_root: &str, plugin_data: &str) -> String {
    weepcode_tools::util::substitute_plugin_tokens(s, Some(plugin_root), Some(plugin_data))
}
...
mod tests {
    ...
    fn load_manifest_fallback_paths() {
        ...
        std::fs::create_dir_all(plugin_root.join(".weepcode-plugin")).unwrap();

        // Write manifest in .weepcode-plugin/ fallback location
        std::fs::write(
            plugin_root.join(".weepcode-plugin/plugin.json"),
    ...
    fn substitute_env_vars_replaces_all() {
        let input =
            "${WEEPCODE_PLUGIN_ROOT}/bin:${CLAUDE_PLUGIN_ROOT}/lib:${WEEPCODE_PLUGIN_DATA}/cache";
