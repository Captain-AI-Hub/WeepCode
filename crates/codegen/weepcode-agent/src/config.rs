//! Agent definition types — parsed from `.weepcode/agents/*.md` files.
...
use weepcode_tools::implementations::codex;
use weepcode_tools::implementations::memory;
use weepcode_tools::implementations::opencode;
use weepcode_tools::implementations::search_tool;
use weepcode_tools::implementations::use_tool;
use weepcode_tools::implementations::weepcode_build;
use weepcode_tools::implementations::weepcode_build_concise;
use weepcode_tools::registry::types::{ToolConfig, ToolServerConfig};
...
/// Orchestrator-specific prompt body appended to the standard WeepcodeBuild
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&weepcode_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&weepcode_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&weepcode_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&weepcode_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&weepcode_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_weepcode_build_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_weepcode_build_toolset() -> ToolServerConfig {
    let mut tools = default_weepcode_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&weepcode_build::EnterPlanModeTool).into());
    tools.push((&weepcode_build::ExitPlanModeTool).into());
    tools.push((&weepcode_build::AskUserQuestionTool).into());
    tools.push((&weepcode_build::WebSearchTool).into());
    tools.push((&weepcode_build::ImageGenTool).into());
    tools.push((&weepcode_build::ImageToVideoTool).into());
    tools.push((&weepcode_build::ReferenceToVideoTool).into());
    tools.push((&weepcode_build::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&weepcode_build::LspTool).into());
    ToolServerConfig {
        tools,
        behavior_preset: None,
    }
}
/// Toolset for the `weepcode-computer` (workspace/sandbox) preset.
fn weepcode_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&weepcode_build::ReadFileTool).into(),
        (&weepcode_build::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&weepcode_build::ListDirTool).into(),
        (&weepcode_build::GrepTool).into(),
        (&weepcode_build::KillTerminalCommandTool).into(),
        (&weepcode_build::GetTerminalCommandOutputTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("weepcode-build", workspace_weepcode_build_toolset()),
        ("weepcode-build-concise", weepcode_build_concise_toolset()),
        ("weepcode-build-plan", weepcode_build_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("weepcode-computer", weepcode_computer_toolset()),
    ]
}
...
fn default_weepcode_build_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&weepcode_build::ReadFileTool).into(),
            ...
            (&weepcode_build::TodoWriteTool).into(),
...
fn weepcode_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&weepcode_build_concise::BashConciseTool).into(),
            (&weepcode_build_concise::ReadFileConciseTool).into(),
            (&weepcode_build_concise::SearchReplaceConciseTool).into(),
            (&weepcode_build::ListDirTool).into(),
            (&weepcode_build::GrepTool).into(),
            kill_task_tool_config(),
            (&weepcode_build::TodoWriteTool).into(),
...
pub fn weepcode_build_hashline_toolset(
    hashline_tools: Vec<weepcode_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<weepcode_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
    ...
    tools.extend([
        ...
        wait_tasks_tool_config(),
        task_tool_config(),
        (&weepcode_build::WebSearchTool).into(),
        (&weepcode_build::SchedulerCreateTool).into(),
        (&weepcode_build::SchedulerDeleteTool).into(),
        (&weepcode_build::SchedulerListTool).into(),
        (&weepcode_build::MonitorTool).into(),
...
fn weepcode_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&weepcode_build::ReadFileTool).into(),
...
fn orchestrator_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            (&weepcode_build::MonitorTool).into(),
            (&weepcode_build::WebSearchTool).into(),
            (&weepcode_build::WebFetchTool).into(),
            (&weepcode_build::ImageGenTool).into(),
            (&weepcode_build::ImageToVideoTool).into(),
            (&weepcode_build::ReferenceToVideoTool).into(),
...
fn weepcode_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&weepcode_build::ReadFileTool).into(),
...
fn weepcode_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&weepcode_build::ReadFileTool).into(),
...
/// - `model: weepcode-3-fast` → `Override("weepcode-3-fast")`
...
pub enum BuiltinAgentName {
    ...
    WeepcodeBuildPlan,
    WeepcodeBuildPlanNoSubagents,
    WeepcodeBuildAskUser,
    Codex,
    Opencode,
    GeneralPurpose,
    Explore,
    Plan,
    BrowserUse,
    #[strum(serialize = "weepcode-build-orchestrator")]
    WeepcodeBuildOrchestrator,
}
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::WeepcodeBuild => AgentDefinition::default_weepcode_build(),
            Self::WeepcodeBuildConcise => AgentDefinition::weepcode_build_concise(),
            Self::WeepcodeBuildPlan => AgentDefinition::weepcode_build_plan(),
            Self::WeepcodeBuildPlanNoSubagents => {
                AgentDefinition::weepcode_build_plan_no_subagents()
            }
            Self::WeepcodeBuildAskUser => AgentDefinition::weepcode_build_ask_user(),
            ...
            Self::WeepcodeBuildOrchestrator => AgentDefinition::weepcode_build_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .weepcode/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_weepcode_build_toolset")]
    ...
    pub capability_mode: Option<weepcode_tool_types::SubagentCapabilityMode>,
    #[serde(default)]
...
pub enum AgentScope {
    /// .weepcode/agents/ (project-level, highest priority)
    Project,
    /// ~/.weepcode/agents/ (user-level)
    User,
    /// ~/.weepcode/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_weepcode_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.weepcode/agent-memory/<name>/`
    User,
    /// `<project>/.weepcode/agent-memory/<name>/`
    Project,
    /// `<project>/.weepcode/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: weepcode_config::weepcode_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".weepcode/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".weepcode/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `weepcode_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let weepcode = weepcode_config::user_weepcode_home();
        let home = dirs::home_dir();
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), weepcode.as_deref())
        {
            if path.starts_with(&dir) {
                return scope;
            }
        }
        if path_str.contains(".weepcode/agents/") || path_str.contains(".weepcode\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".weepcode/bundled/agents/")
            || path_str.contains(".weepcode\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    /// (`!inject_default_tools`). Stock `weepcode-build*` agents leave all
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<weepcode_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ...
            ["WeepcodeBuild:grep", "WeepcodeBuildHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_weepcode_build_toolset(),
    ...
    pub fn general_purpose() -> Self {
        ...
        Self {
            description: weepcode_tool_types::GENERAL_PURPOSE_SUBAGENT
                .description
                .to_string(),
    ...
    pub fn explore() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: weepcode_tool_types::EXPLORE_SUBAGENT
                .description
                .to_string(),
    ...
    pub fn plan() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: weepcode_tool_types::PLAN_SUBAGENT.description.to_string(),
    ...
    pub fn weepcode_build_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::WeepcodeBuildOrchestrator,
                "WeepcodeBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_weepcode_build_toolset();
        }
...
mod tests {
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("weepcode-build").unwrap();
    ...
    fn weepcode_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&weepcode_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&weepcode_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn weepcode_computer_preset_is_curated_weepcode_build_subset() {
        let gc = toolset_for_preset("weepcode-computer").unwrap();
        let gb = toolset_for_preset("weepcode-build").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = weepcode_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "weepcode-computer tool `{}` must also ship in the weepcode-build preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "weepcode-computer should be a curated subset of weepcode-build"
        );
    }
    #[test]
    fn weepcode_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("weepcode-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&weepcode_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&weepcode_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            !ids.contains(
                ToolConfig::from(&weepcode_build::TaskOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&weepcode_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&weepcode_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&weepcode_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&weepcode_build::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The weepcode-computer preset must ship a full-file write tool (legacy
    /// `write_file` parity) — the same OpenCode `write` tool the weepcode-build
    ...
    #[test]
    fn weepcode_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("weepcode-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "weepcode-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn weepcode_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("weepcode-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&weepcode_build::LspTool).id,
            ToolConfig::from(&weepcode_build::EnterPlanModeTool).id,
            ToolConfig::from(&weepcode_build::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "weepcode-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_weepcode_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_weepcode_build_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::WeepcodeBuildOrchestrator => true,
            BuiltinAgentName::WeepcodeBuild
            | BuiltinAgentName::WeepcodeBuildConcise
            | BuiltinAgentName::WeepcodeBuildPlan
            | BuiltinAgentName::WeepcodeBuildPlanNoSubagents
    ...
    fn is_strict_harness_agent_type_classifies_by_name() {
        for strict in ["codex", "weepcode-build-orchestrator"] {
            assert!(
        ...
        for non_strict in [
            ...
            "weepcode-build-plan",
            "weepcode-build-concise",
            "weepcode-build-ask-user",
            "opencode",
            "browser-use",
            "custom-user-agent",
            "",
            "weepcode-build-totally-made-up",
        ] {
            assert!(
...
"#;
    ...
    fn test_model_override_display_shows_id() {
        ...
        assert_eq!(
            ModelOverride::Override("weepcode-3-fast".to_string()).to_string(),
            "weepcode-3-fast"
        );
    }
    #[test]
...
"#;
        ...
        assert_eq!(def.model, ModelOverride::Override("weepcode-3".to_string()));
    }
    #[test]
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.weepcode/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.weepcode/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_weepcode_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
    ...
    fn test_from_json_has_default_toolset_with_task_tool() {
        let json = serde_json::json!(
            { "name" : "weepcode-build", "description" : "Multi-surface coding agent.",
            "promptMode" : "extend", "permissionMode" : "dontAsk", "agentsMd" : true,
            "promptBody" : "You are a coding assistant." }
        );
        let def = AgentDefinition::from_json(&json).unwrap();
        let task_tool_id = "WeepcodeBuild:task";
        assert!(
    ...
    fn test_model_override_serde_explicit_model_id() {
        let yaml = "\"weepcode-3-fast\"";
        let m: ModelOverride = serde_yaml::from_str(yaml).unwrap();
        assert_eq!(m, ModelOverride::Override("weepcode-3-fast".to_string()));
    }
    #[test]
    ...
    fn test_model_override_serialize_override() {
        let m = ModelOverride::Override("weepcode-3-fast".to_string());
        let s = serde_json::to_string(&m).unwrap();
        assert_eq!(s, "\"weepcode-3-fast\"");
    }
    #[test]
    fn test_model_override_in_frontmatter() {
        let content = "---\nname: test\ndescription: Test\nmodel: weepcode-3-fast\n---\n";
        let def = AgentDefinition::parse(content).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("weepcode-3-fast".to_string())
        );
    }
    #[test]
    ...
    fn test_model_override_in_json() {
        let json = serde_json::json!(
            { "name" : "test", "description" : "Test", "model" : "weepcode-code-fast-1" }
        );
        let def = AgentDefinition::from_json(&json).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("weepcode-code-fast-1".to_string())
        );
    }
    #[test]
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("weepcode-build", BuiltinAgentName::WeepcodeBuild),
            (
                "weepcode-build-concise",
                BuiltinAgentName::WeepcodeBuildConcise,
            ),
