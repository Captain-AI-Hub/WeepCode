...
//!
//! Searches from cwd to repo root, plus `~/.weepcode/`. Also discovers
//! `*.md` files in `.weepcode/rules/` and `.claude/rules/` directories.
...
/// Find `*.md` files in `.weepcode/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.weepcode/, git repo root, and session cwd.
...
async fn read_agents_config_with_options(
    ...
    let global_dir = weepcode_tools::util::weepcode_home::weepcode_home();
    ...
    // Always include weepcode_home (~/.weepcode/) first, then ~/.claude/ and ~/.cursor/
...
/// Used by `weepcode-shell` to structurally detect legacy untagged AGENTS.md
...
fn render_agents_md(configs: &[AgentConfigFile]) -> Option<String> {
    ...
    for config in configs {
        ...
        // .weepcode/rules/*.md) so globs/paths metadata doesn't leak into the
        // system prompt as raw YAML.
        let is_rules_file = config.file_path.contains("/.weepcode/rules/")
            || config.file_path.contains("/.claude/rules/");
        let content = if is_rules_file {
            weepcode_tools::implementations::skills::skill::extract_skill_body(&config.content)
        } else {
