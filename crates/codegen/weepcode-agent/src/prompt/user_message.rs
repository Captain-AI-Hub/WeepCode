...
//! - `Default` -- the legacy WeepCode Build prefix (built by the shell layer).
...
use weepcode_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy WeepCode Build prefix: `<user_info>` + optional `<git_status>`.
...
/// One discovered rule file (AGENTS.md / Claude.md / .weepcode/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `weepcode_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.weepcode/, ~/.claude/).
    ...
    /// (`~/.weepcode/projects/<encoded-cwd>/mcps`). Surfaced in
