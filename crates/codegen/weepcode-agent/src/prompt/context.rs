...
//! `TemplateRenderer` in `weepcode-tools`. This struct does NOT own a
...
pub struct PromptContext {
    ...
    /// Identity in the primary weepcode-build system prompt (`You are <label>…`).
    /// Not the UI picker name. Defaults to [`DEFAULT_SYSTEM_PROMPT_LABEL`].
    #[serde(default = "default_system_prompt_label")]
    pub system_prompt_label: String,
}
/// Default identity in the system prompt (`You are <label>.`).
/// WeepCode: neutral default; the WeepCode "WeepCode" identity is gone.
pub const DEFAULT_SYSTEM_PROMPT_LABEL: &str = "WeepCode";
...
mod tests {
    ...
    fn test_placeholders_system_prompt_label_override() {
        let mut ctx = test_context();
        ctx.system_prompt_label = "WeepCode Internal".into();
        let p = ctx.placeholders();
        assert_eq!(p["system_prompt_label"], "WeepCode Internal");
    }
    #[test]
    ...
    fn test_agents_md_paths_use_display_cwd_in_rendered_section() {
        ...
        let overlay_path = "/root/.weepcode/worktrees/my-project/ab-123-a-overlay";
