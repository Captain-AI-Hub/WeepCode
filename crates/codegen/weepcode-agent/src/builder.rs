...
use std::sync::Arc;
use weepcode_tools::bridge::ToolBridge;
use weepcode_tools::computer::types::{AsyncFileSystem, TerminalBackend};
...
/// The WeepCode [`ToolKind`] a vendor-compat `tools:` allowlist entry resolves to, so
/// a plugin's upstream allowlist still binds. Backed by the shared vendor-to-WeepCode
/// tool registry in `weepcode-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    weepcode_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.weepcode/worktrees/project/fork-...-overlay`) that must stay
    ...
    parent_scheduler_handle:
        Option<weepcode_tools::implementations::weepcode_build::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn weepcode_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: weepcode_tools::implementations::web_search::WebSearchConfig,
    ...
    backend_search: bool,
    web_fetch_config: weepcode_tools::implementations::weepcode_build::web_fetch::WebFetchConfig,
    lsp: Option<std::sync::Arc<dyn weepcode_tools::implementations::lsp::LspBackend>>,
    image_gen_config: weepcode_tools::implementations::weepcode_build::image_gen::ImageGenConfig,
    video_gen_config: weepcode_tools::implementations::weepcode_build::video_gen::VideoGenConfig,
    ...
    compat: weepcode_tools::types::compat::CompatConfig,
    ...
    api_key_provider: Option<weepcode_tools::types::SharedApiKeyProvider>,
    attribution_callback: Option<weepcode_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in weepcode-shell).
    ...
    preloaded_skills: Option<Vec<weepcode_tools::implementations::skills::types::SkillInfo>>,
}
/// Ensure plan mode tools (`enter_plan_mode`, `exit_plan_mode`,
/// `ask_user_question`) are present in the tool config.
fn ensure_plan_mode_tools(tool_config: &mut weepcode_tools::registry::types::ToolServerConfig) {
    use weepcode_tools::implementations::weepcode_build;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("WeepcodeBuild:enter_plan_mode");
    let missing_exit = !existing.contains("WeepcodeBuild:exit_plan_mode");
    let missing_ask = !existing.contains("WeepcodeBuild:ask_user_question");
    drop(existing);
    if missing_enter {
        tool_config
            .tools
            .push((&weepcode_build::EnterPlanModeTool).into());
    }
    if missing_exit {
        tool_config
            .tools
            .push((&weepcode_build::ExitPlanModeTool).into());
    }
    if missing_ask {
        tool_config
            .tools
            .push((&weepcode_build::AskUserQuestionTool).into());
    }
}
...
fn merge_tool_params(
    tool_config: &mut weepcode_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(weepcode_tools::computer::local::LocalFs),
            notification_handle,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<weepcode_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn weepcode_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: weepcode_tools::implementations::weepcode_build::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: weepcode_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `WEEPCODE_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: weepcode_tools::implementations::weepcode_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn weepcode_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    /// allowing image generation via the WeepCode Imagine API with session
    /// credentials. When `Disabled` (default), the tool is not registered.
    pub fn with_image_gen_config(
        mut self,
        config: weepcode_tools::implementations::weepcode_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    /// allowing video generation via the WeepCode Video Generation API with
    /// session credentials. When `Disabled` (default), the tool is not
    /// registered.
    pub fn with_video_gen_config(
        mut self,
        config: weepcode_tools::implementations::weepcode_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: weepcode_tools::implementations::weepcode_build::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: weepcode_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the WeepcodeBuild Task description.
    ...
    ///
    /// When disabled, `WeepcodeBuild:ask_user_question` is stripped from the
    ...
    /// Without this, only auto-discovered skills (cwd/.weepcode/skills, ~/.weepcode/skills)
    ...
    fn resolve_definition(&self) -> AgentDefinition {
        ...
        let mut def = AgentDefinition::default_weepcode_build();
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_search_config.is_enabled() {
                ...
                tool_config
                    .tools
                    .push((&weepcode_build::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use weepcode_tools::implementations::weepcode_build;
                tool_config
                    .tools
                    .push((&weepcode_build::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&weepcode_tools::implementations::weepcode_build::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&weepcode_tools::implementations::weepcode_build::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&weepcode_tools::implementations::weepcode_build::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config.tools.push(
                    (&weepcode_tools::implementations::weepcode_build::ImageToVideoTool).into(),
                );
                tool_config.tools.push(
                    (&weepcode_tools::implementations::weepcode_build::ReferenceToVideoTool).into(),
                );
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&weepcode_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let weepcode_build_ns =
                weepcode_tools::types::tool::ToolNamespace::WeepcodeBuild.to_string();
            let mem_search_id = format!(
                "{weepcode_build_ns}:{}",
                weepcode_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{weepcode_build_ns}:{}",
                weepcode_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                weepcode_tools::types::tool::ToolNamespace::WeepcodeBuild,
            );
        ...
        if task_stripped {
            use weepcode_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::WeepcodeBuild, "run_terminal_cmd", true)
                && !has_satisfier(
                    ToolNamespace::WeepcodeBuildConcise,
                    "run_terminal_cmd",
                    true,
                )
        ...
        if let weepcode_tools::implementations::weepcode_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["WeepcodeBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                ...
                &[
                    ...
                    "WeepcodeBuildConcise:run_terminal_cmd",
                ],
                bash_params,
            );
        }
        if let Some(ref ask_params) = self.ask_user_question_params_json {
            merge_tool_params(
                &mut tool_config,
                &["WeepcodeBuild:ask_user_question"],
                ask_params,
            );
        }
        ...
        if !definition.tools.is_empty() {
            ...
            if unresolved.is_empty() {
                ...
                tracing::warn!(
                    ...
                    definition.tools,
                    "tools allowlist had unmappable entries; keeping full weepcode toolset"
                );
            }
        }
        tool_config
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                weepcode_tools::types::resources::TruncationCfg(
                    weepcode_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(weepcode_sampling_types::HostedTool::WebSearch {
                    allowed_domains: None,
                });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(weepcode_sampling_types::HostedTool::XSearch);
            }
        }
...
/// CLI naming for the shared [`weepcode_tool_types::build_task_description`] builder.
const TASK_TOOL_NAMING: weepcode_tool_types::TaskToolNaming<'static> =
    weepcode_tool_types::TaskToolNaming {
...
/// CLI [`weepcode_tool_types::SubagentToolNaming`]: each kind maps to its
...
const SUBAGENT_TOOL_NAMING: weepcode_tool_types::SubagentToolNaming<'static> =
    weepcode_tool_types::SubagentToolNaming {
...
/// shared [`weepcode_tool_types`] catalog and rendered with [`SUBAGENT_TOOL_NAMING`]
...
fn builtin_tools_fragment(name: BuiltinAgentName) -> String {
    let subagent = match name {
        BuiltinAgentName::GeneralPurpose => weepcode_tool_types::GENERAL_PURPOSE_SUBAGENT,
        BuiltinAgentName::Explore => weepcode_tool_types::EXPLORE_SUBAGENT,
        BuiltinAgentName::Plan => weepcode_tool_types::PLAN_SUBAGENT,
        _ => return String::new(),
    };
...
/// [`weepcode_tool_types::SubagentDescriptor`] and defers to
/// [`weepcode_tool_types::build_task_description`] so the CLI and the prod chat
...
pub(crate) fn build_task_description(
    ...
    let descriptors: Vec<weepcode_tool_types::SubagentDescriptor> = subagents
        .iter()
        .map(|entry| {
            ...
            weepcode_tool_types::SubagentDescriptor {
        ...
        .collect();
    let mut description =
        weepcode_tool_types::build_task_description(&descriptors, &TASK_TOOL_NAMING);
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        weepcode_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: weepcode_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    fn build_task_description_builtin_includes_tools() {
        ...
        assert!(
            desc.contains(weepcode_tool_types::GENERAL_PURPOSE_SUBAGENT.tools_template),
            "should include general-purpose tool names"
        );
        assert!(
            desc.contains(weepcode_tool_types::EXPLORE_SUBAGENT.tools_template),
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: weepcode_tools::types::config_source::ConfigSource::Project {
    ...
    fn build_task_description_lists_public_model_slugs() {
        ...
        assert!(!desc.contains(concat!("weepcode", " models")));
    }
    #[test]
    ...
    async fn build_pager_agent(
        ...
        use weepcode_tools::computer::local::LocalTerminalBackend;
        use weepcode_tools::notification::ToolNotificationHandle;
        AgentBuilder::new(
            std::env::temp_dir(),
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                label: "weepcode-build / subagents+ask_user",
                profile: AgentDefinition::default_weepcode_build,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "weepcode-build / subagents / no-ask-user",
                profile: AgentDefinition::default_weepcode_build,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "weepcode-build / no-subagents / ask_user",
                profile: AgentDefinition::default_weepcode_build,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "weepcode-build / no-subagents / no-ask-user",
                profile: AgentDefinition::default_weepcode_build,
                subagents: false,
                ask_user: false,
            },
            PagerFlagCase {
                label: "weepcode-build-ask-user / subagents",
                profile: AgentDefinition::weepcode_build_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "weepcode-build-ask-user / no-subagents",
                profile: AgentDefinition::weepcode_build_ask_user,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "weepcode-build-plan",
                profile: AgentDefinition::weepcode_build_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "weepcode-build-plan / no-ask-user",
                profile: AgentDefinition::weepcode_build_plan,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "weepcode-build-plan-no-subagents",
                profile: AgentDefinition::weepcode_build_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "weepcode-build-plan-no-subagents / no-ask-user",
                profile: AgentDefinition::weepcode_build_plan_no_subagents,
                subagents: false,
                ask_user: false,
            },
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use weepcode_tools::computer::local::LocalTerminalBackend;
        use weepcode_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_weepcode_build();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use weepcode_tools::computer::local::LocalTerminalBackend;
        use weepcode_tools::implementations::weepcode_build::ask_user_question::AskUserQuestionParams;
        use weepcode_tools::notification::ToolNotificationHandle;
        use weepcode_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_weepcode_build();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "WeepcodeBuild:ask_user_question"),
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use weepcode_tools::computer::local::LocalTerminalBackend;
        use weepcode_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_weepcode_build();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("WeepcodeBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use weepcode_tools::computer::local::LocalTerminalBackend;
        use weepcode_tools::implementations::weepcode_build::bash::BashParams;
        use weepcode_tools::notification::ToolNotificationHandle;
        use weepcode_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_weepcode_build();
    ...
    /// Compat allowlist names (`Read`, `Bash`, `Grep`) map to their WeepCode
    /// equivalents by `ToolKind` — a real restricted toolset, not zero tools.
    #[tokio::test]
    async fn claude_tool_names_map_to_weepcode_equivalents() {
    ...
    /// Shell, LSP, ask, and task-lifecycle tool names resolve to their weepcode
    ...
    /// treat `mcp__*` as always-on, so weepcode keeps the MCP meta-tools
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        use weepcode_tools::computer::local::LocalTerminalBackend;
        use weepcode_tools::implementations::web_search::WebSearchConfig;
    ...
    /// weepcode-build toolsets have no Skill tool — skills are read from
    ...
    /// Compat `ToolSearch` meta-tool maps to weepcode's `search_tool` (MCP
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, weepcode_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, weepcode_sampling_types::HostedTool::XSearch)),
