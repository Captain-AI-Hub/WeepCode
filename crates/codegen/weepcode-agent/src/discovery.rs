...
//!
//! Searches `.weepcode/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.weepcode/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.weepcode/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".weepcode/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.weepcode/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.weepcode/agents` / `.claude/agents`) under each
...
fn all_subagents_with_home(
    ...
    let discovered = discover_with_home(cwd, home, weepcode_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.weepcode/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.weepcode/agents/` walking from `cwd` up to repo root
/// 2. `~/.weepcode/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.weepcode/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user weepcode agents, `.claude`
/// compat agents, then bundled. `.weepcode` dirs resolve from `weepcode_home`
/// (WEEPCODE_HOME-aware) plus the legacy literal `~/.weepcode` when WEEPCODE_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    weepcode_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.weepcode, included only when it differs from weepcode_home
    // (i.e. WEEPCODE_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_home = home
        .map(|h| h.join(".weepcode"))
        .filter(|legacy| weepcode_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = weepcode_home {
        dirs.push((g.join("agents"), AgentScope::User));
    }
    if let Some(l) = &legacy_home {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let weepcode = weepcode_config::user_weepcode_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), weepcode.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, weepcode_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let weepcode = weepcode_config::user_weepcode_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), weepcode.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, weepcode_home);
...
///
/// Project-level `.weepcode/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let weepcode = weepcode_config::user_weepcode_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), weepcode.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, weepcode_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, weepcode_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the WeepCode
...
/// Load project agent definitions from every `.weepcode/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_origin_for_scope(scope: PluginScope) -> crate::plugins::PluginOrigin {
        ...
        match scope {
            ...
            PluginScope::Project => PluginOrigin::ProjectWeepcode,
            PluginScope::User => PluginOrigin::UserWeepcode,
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_weepcode_when_weepcode_home_differs() {
        let home = Path::new("/home/u");
        let weepcode = Path::new("/custom/weepcodehome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(weepcode))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&weepcode.join("agents")));
        assert!(paths.contains(&home.join(".weepcode").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&weepcode.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".weepcode").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_weepcode_home_is_dot_weepcode() {
        let home = Path::new("/home/u");
        let weepcode = home.join(".weepcode");
        let count = user_agent_dirs(Some(home), Some(&weepcode))
            .into_iter()
            .filter(|(p, _)| *p == weepcode.join("agents"))
            .count();
        assert_eq!(
            count, 1,
            "no duplicate ~/.weepcode/agents when weepcode_home == ~/.weepcode"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_weepcode_build() {
        let def = by_name("weepcode-build");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "weepcode-build");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".weepcode").join("agents");
        let agents_dir_2 = inner_dir.join(".weepcode").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".weepcode").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".weepcode")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".weepcode").join("agents");
        let bundled_dir = home.join(".weepcode").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".weepcode")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def =
            by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".weepcode")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".weepcode").join("agents");
        let bundled_dir = home.join(".weepcode").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "weepcode-build" that shadows the built-in
        write_agent_file(
            &agents_dir,
            "weepcode-build.md",
            "weepcode-build",
            "Custom weepcode-build",
        );

        let def = by_name_in_cwd("weepcode-build", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "weepcode-build");
        assert_eq!(def.description, "Custom weepcode-build");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .weepcode/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("weepcode-build-orchestrator")
            .expect("from_str must resolve weepcode-build-orchestrator");
        assert_eq!(variant, BuiltinAgentName::WeepcodeBuildOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "weepcode-build-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("weepcode-build-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find weepcode-build-orchestrator");
        assert_eq!(def.name, "weepcode-build-orchestrator");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.weepcode/agents/) agent named "explore" should NOT shadow
