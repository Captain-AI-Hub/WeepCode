...
//! primitive consumed cross-crate by `weepcode-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.weepcode/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.weepcode`/`.mcp.json`/plugins would look repo-local. Drop
...
/// from `weepcode-workspace`, which depends on THIS crate) to keep the dep edge
