...
use weepcode_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use weepcode_pager::client_identity::PAGER_CLIENT_VERSION;
use weepcode_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use weepcode_shell::agent::config::Config as AgentConfig;
use weepcode_shell::leader::{
...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    if let Some(v) = &args.weepcode_ws_origin {
        config.weepcode_com_config.weepcode_ws_origin = v.clone();
    }
    if let Some(v) = &args.weepcode_ws_url {
        config.weepcode_com_config.weepcode_ws_url = v.clone();
    }
}
/// Apply global endpoint CLI args to an existing config.
fn apply_agent_endpoint_args(
    agent_args: &weepcode_pager::app::AgentArgs,
    config: &mut AgentConfig,
) {
    if let Some(v) = &agent_args.cli_chat_proxy_base_url {
        config.endpoints.cli_chat_proxy_base_url = Some(v.clone());
    }
    if let Some(v) = &agent_args.weepcode_api_base_url {
        config.endpoints.weepcode_api_base_url = v.clone();
    }
}
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   WeepCode agent server starting...");
    eprintln!();
...
/// Entrypoint tag for `weepcode -p`; keys the quiet stderr default in `init_tracing_simple`.
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use weepcode_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(weepcode_telemetry::sampling_log::layer())
        .with(weepcode_telemetry::instrumentation::layer())
        .with(weepcode_telemetry::hooks_log::layer())
        .with(weepcode_telemetry::otel_layer::build_otel_layer(
            weepcode_telemetry::otel_layer::OtelClientInfo {
                client_name: "weepcode-pager",
                client_version: weepcode_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            weepcode_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    weepcode_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    weepcode_telemetry::external::init(
        weepcode_shell::agent::config::resolve_external_otel_config(
            weepcode_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: weepcode_version::VERSION.to_owned(),
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = weepcode_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !weepcode_shell::util::is_weepcode_process(pid) {
            if let Some(ref lock) = d.lock_path {
                eprintln!("  PID {pid} is not a weepcode process, removing stale lock");
        ...
        if let Err(e) = weepcode_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => {
            LeaderTarget::Environment(weepcode_shell::env::WeepcodeBuildEnvironment::Production)
        }
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, weepcode_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = weepcode_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = weepcode_shell::leader::LeaderClient::connect(
        socket_path.to_path_buf(),
        "weepcode-pager-leader-cli",
        ClientMode::Stdio,
...
fn leader_info_json(
    ...
    info: Option<&weepcode_shell::leader::ControlPayload>,
...
/// Env override for the `weepcode workspace` gate: any truthy value enables the
/// command locally, a falsy one disables it — bypassing the remote settings flag.
const WORKSPACE_COMMAND_ENV: &str = "WEEPCODE_WORKSPACE_COMMAND";
/// Resolution of the `weepcode workspace` gate. `Unknown` is kept separate from
...
/// The `WEEPCODE_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`);
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&weepcode_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
/// Truthy parse for weepcode on/off env vars: everything enables except the common
...
fn fetch_remote_settings() -> Option<weepcode_shell::util::config::RemoteSettings> {
    join_early_prefetch(weepcode_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`weepcode workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `weepcode workspace`. Check your \
             network connection (run `weepcode login` if you are signed out), then \
             try again."
            )
        }
    }
...
async fn connect_workspace_control(
    ...
    let ws_url = &agent_config.weepcode_com_config.weepcode_ws_url;
    let socket = socket_path_for_ws_url(ws_url);
    LeaderClient::connect(
        socket,
        "weepcode-workspace-cli",
        ClientMode::Stdio,
        ClientCapabilities::default(),
    )
    .await
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a weepcode session, or run `weepcode workspace start`."
        )
    })
}
async fn workspace_control(
    ...
    let raw_config = weepcode_shell::config::load_effective_config_disk_only()
...
async fn workspace_start(
    ...
    use weepcode_shell::auth::ensure_authenticated;
    let raw_config = weepcode_shell::config::load_effective_config()
        .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
    let agent_config = AgentConfig::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    if !use_leader {
        anyhow::bail!(
            "`weepcode workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.weepcode/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.weepcode_com_config,
        false,
        Some("No cached credentials found. Run `weepcode login` first."),
    )
    .await?;
    let env_urls = LeaderEnvUrls::from(&agent_config.weepcode_com_config);
...
fn cache_outgoing_acp_state(msg: &str, state: &std::sync::Mutex<StdioReplayState>) {
    ...
    match method {
        ...
        "weepcode/session/close" | "_x.ai/session/close" => {
...
const REPLAY_LOAD_REQUEST_ID: &str = "weepcode/leader-replay/session-load";
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    weepcode_telemetry::sentry::flush_on_shutdown();
    weepcode_telemetry::otel_layer::shutdown_otel();
    weepcode_telemetry::debug_log::flush();
    std::process::exit(exit_code);
}
/// Emitted by both leader guards (server mode and leader-connect) so the two sites
/// can't drift.
const PLUGIN_DIR_LEADER_WARNING: &str = "weepcode: --plugin-dir is ignored in leader mode; run with --no-leader to \
     load per-process plugins";
/// Run the `agent` subcommand, dispatching to the appropriate mode.
async fn run_agent_command(
    agent_args: Box<weepcode_pager::app::AgentArgs>,
    ...
    let _otel_guard = weepcode_telemetry::otel_layer::otel_guard();
    weepcode_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => weepcode_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(
    ...
    let early_prefetch = weepcode_shell::agent::models::start_early_prefetch(None);
    weepcode_shell::agent::mvp_agent::warm_async_http_client();
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(weepcode_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = weepcode_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    if let Some(warning) = launch_yolo.blocked_warning {
        eprintln!("weepcode: {warning}");
    }
    agent_config.default_yolo_mode = launch_yolo.yolo;
    agent_config.default_auto_mode = weepcode_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
    );
    ...
    agent_config.resolve_runtime_fields(&weepcode_shell::agent::config::RuntimeResolutionContext {
    ...
    if use_leader {
        ...
        let env_urls =
            weepcode_shell::leader::LeaderEnvUrls::from(&agent_config.weepcode_com_config);
        ...
        match mode {
            ClientMode::Stdio => {
                ...
                let stdin_task = tokio::spawn(async move {
                    let mut stdin_lines = weepcode_acp_lib::spawn_stdin_line_reader();
                    loop {
                        tokio::select! {
                                    ...
                                    .await
                                {
                                    Ok((new_tx, mut new_rx, _disconnect_rx)) => {
                                        ...
                                        let notification = format!(
                                            r#"{{"jsonrpc":"2.0","method":"weepcode/leader_reconnected","params":{params}}}"#
                                        );
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = weepcode_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            weepcode_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            let mut agent_config = agent_config.clone();
            apply_headless_args_to_config(&a.headless, &mut agent_config);
            // WeepCode: leader auto-update removed (no release channel); the
            // shell-side LeaderAutoUpdateConfig type stays compiled/tested.
            let leader_auto_update: Option<weepcode_shell::agent::app::LeaderAutoUpdateConfig> =
                None;
            run_leader(
                &agent_config,
...
/// Sets `WEEPCODE_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for
/// `weepcode dashboard`, and clears `args.command` so the regular
...
/// the leader roster — so `weepcode dashboard` does NOT force leader mode and
/// is compatible with `--no-leader`.
///
/// The only gate is the feature flag: a disabled dashboard
/// (`[dashboard].enabled = false` / `WEEPCODE_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !weepcode_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.weepcode/config.toml and \
             unsetting WEEPCODE_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("WEEPCODE_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<weepcode_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(weepcode_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<weepcode_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(weepcode_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    weepcode_shell::heap_profile::install(weepcode_shell::heap_profile::HeapProfileHooks {
...
fn main() {
    weepcode_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    weepcode_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        weepcode_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        weepcode_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    if let Some(code) = weepcode_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    weepcode_pager::memory_trace::start(
        weepcode_shell::util::weepcode_home::weepcode_home().join("memtrace"),
    );
    raise_fd_limit();
    if let Err(e) = weepcode_config::validate_requirements() {
        eprintln!("Couldn't start WeepCode: {e}");
        eprintln!();
        eprintln!(
            "Update WeepCode to a version the policy allows, or ask your administrator \
             to fix the managed requirements."
        );
        std::process::exit(2);
    }
    let _sentry_guard = weepcode_telemetry::sentry::init(weepcode_telemetry::sentry::Config {
        client: "weepcode-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: weepcode_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    weepcode_pager::docs::extract_user_guide_docs(
        &weepcode_shell::util::weepcode_home::weepcode_home(),
    );
    weepcode_crash_handler::install_terminal_restore_only();
    if weepcode_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = weepcode_shell::util::weepcode_home::weepcode_home().join("crash");
        if let Some(report) = weepcode_crash_handler::check_previous_crash(&crash_dir) {
            eprintln!("WeepCode crashed during your last session.");
            eprintln!("  Signal:  {}", report.signal_name);
            eprintln!("  Version: {}", report.app_version);
            eprintln!("  Report:  {}", report.report_path.display());
            eprintln!();
        }
        if !weepcode_crash_handler::install(weepcode_crash_handler::CrashHandlerConfig {
    ...
    let crashed = weepcode_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
    ...
    weepcode_telemetry::debug_log::flush();
    if let Err(e) = result {
        weepcode_tty_utils::restore_native_stderr();
        eprintln!("Error: {e:#}");
        drop(_sentry_guard);
        std::process::exit(1);
    }
...
async fn async_main() -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("WEEPCODE_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("WEEPCODE_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(
                weepcode_shell::agent::chat_modes::WEEPCODE_CHAT_MODE_ENV,
                "1",
            );
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(weepcode_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("WEEPCODE_DEBUG_LOG", path);
            std::env::remove_var("WEEPCODE_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("WEEPCODE_DEBUG_LOG", "1");
        set_if_unset("WEEPCODE_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        weepcode_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return weepcode_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        weepcode_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        weepcode_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    weepcode_shell::config::apply_sandbox(
        None,
    ...
    weepcode_shell::http::set_client_name(if is_interactive {
        weepcode_workspace::permission::ClientType::WeepcodePager
    } else {
        weepcode_workspace::permission::ClientType::Generic
    });
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!(
                        ...
                        weepcode_version::CHANNEL_NAME.unwrap_or("unknown"), }
                    );
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    println!(
                ...
                if args.leader || args.no_leader {
                    ...
                    anyhow::bail!(
                        "top-level {flag} applies to the pager TUI, not the agent subcommand. \
                         Use `weepcode-pager agent {flag}` instead."
                    );
                }
            ...
            Command::Inspect { json } => {
                ...
                weepcode_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return weepcode_pager::mcp_cmd::run(mcp_args).await;
            }
            Command::Plugin(plugin_args) => {
                init_tracing_simple("cli");
                let _otel_guard = weepcode_telemetry::otel_layer::otel_guard();
                return weepcode_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = weepcode_telemetry::otel_layer::otel_guard();
                let config = weepcode_shell::config::load_effective_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to load config: {e}"))?;
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return weepcode_pager::models::list_available_models(&agent_config).await;
            }
            ...
            Command::Worktree(worktree_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return weepcode_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
            ...
            Command::Sessions(sessions_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return weepcode_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return weepcode_pager::export_cmd::run(export_args);
            }
            Command::Trace(trace_args) => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return weepcode_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return weepcode_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check: _,
                json: _,
                ...
                init_tracing_simple("cli");
                // WeepCode: the update channels serve xAI `grok` binaries —
                // letting this run could overwrite a WeepCode install with
                // upstream artifacts. Disabled until a WeepCode channel exists.
                eprintln!(
                    "`weepcode update` is not available: the release channel serves \
                     xAI binaries. Build from source instead (cargo build -p weepcode-pager-bin)."
                );
                std::process::exit(1);
            }
            Command::AddModel {
                ...
                init_tracing_simple("cli");
                // WeepCode: WeepCode OAuth login is gone. `login` prints the
                ...
                println!(
                    "Configure an API provider for WeepCode.\n\
                     ...
                     \n\
                     Option 2 — edit ~/.weepcode/config.toml directly:\n\
                     \n\
                     ...
                     \n\
                     Or export WEEPCODE_API_KEY for a quick single-provider setup."
                );
                weepcode_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = weepcode_shell::config::load_effective_config_disk_only()
                ...
                let config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                weepcode_shell::auth::run_cli_logout(&config)?;
                weepcode_shell::instrumentation::finalize_and_exit(0);
            }
    ...
    let headless_prompt = weepcode_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(weepcode_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some() {
            if args.output_format == weepcode_pager::headless::OutputFormat::Plain {
                args.output_format = weepcode_pager::headless::OutputFormat::Json;
            }
            if args.self_verify {
                anyhow::bail!(
        ...
        return weepcode_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            weepcode_pager::headless::HeadlessOptions {
    ...
    let _otel_guard = weepcode_telemetry::otel_layer::otel_guard();
    let result = weepcode_pager::app::run(args).await;
    weepcode_sandbox::flush();
    result.map(|_| ())
}
#[cfg(test)]
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            weepcode_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: weepcode_shell::heap_profile::JemallocStats) {
    ...
    fn jemalloc_dump_rejects_interior_nul_path() {
        ...
        let path = Path::new(OsStr::from_bytes(b"/tmp/weepcode-jemalloc-\0.heap"));
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            weepcode_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!weepcode_shell::heap_profile::prof_available());
            return;
        }
        assert!(weepcode_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(weepcode_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(weepcode_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        weepcode_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    use clap::Parser as _;
    /// `weepcode dashboard` flags the startup hook without forcing leader mode —
    /// the dashboard is independent of leader mode, so the launch keeps
    /// whatever leader setting the user (or config) chose.
    #[serial_test::serial(WEEPCODE_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        let mut args = PagerArgs::try_parse_from(["weepcode", "dashboard"]).unwrap();
        ...
        assert_eq!(
            std::env::var("WEEPCODE_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("WEEPCODE_OPEN_DASHBOARD_AT_STARTUP") };
    }
    /// `weepcode dashboard --no-leader` is allowed — the dashboard does not
    /// require a leader, so the combination launches into the dashboard in
    /// non-leader mode.
    #[serial_test::serial(WEEPCODE_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["weepcode", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    #[serial_test::serial(WEEPCODE_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("WEEPCODE_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("WEEPCODE_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("WEEPCODE_WORKSPACE_COMMAND", "off") };
    ...
    async fn replay_waits_for_load_response_through_notifications() {
        ...
        let responder = tokio::spawn(async move {
            ...
            response_tx
                .send(
                    r#"{"jsonrpc":"2.0","method":"weepcode/leader/version_mismatch","params":{}}"#
                        .to_string(),
                )
                .unwrap();
            response_tx
