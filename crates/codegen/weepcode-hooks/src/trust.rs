...
// (`~/.weepcode/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_weepcode_home>/trusted-hook-projects`), or `None` when no user weepcode home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(weepcode_config::user_weepcode_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user weepcode home (set $WEEPCODE_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$WEEPCODE_HOME/disabled-hooks`, or `None` when no user weepcode
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(weepcode_config::user_weepcode_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        let weepcode_dir = dir.join(".weepcode");
        std::fs::create_dir_all(&weepcode_dir).unwrap();
        weepcode_dir.join("trusted-hook-projects")
    }
