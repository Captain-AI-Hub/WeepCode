//! Integration tests for weepcode-hooks.
//!
...
use weepcode_hooks::discovery::load_hooks;
use weepcode_hooks::dispatcher;
use weepcode_hooks::event::*;
use weepcode_hooks::result::HookDecision;
use weepcode_hooks::runner::RunContext;
...
async fn hook_receives_env_vars() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$WEEPCODE_HOOK_EVENT" > {f}; echo "NAME=$WEEPCODE_HOOK_NAME" >> {f}; echo "SESSION=$WEEPCODE_SESSION_ID" >> {f}; echo '{{"decision":"allow"}}'"#,
...
/// runner-reserved keys (`WEEPCODE_HOOK_EVENT`, `WEEPCODE_HOOK_NAME`,
/// `WEEPCODE_SESSION_ID`, `WEEPCODE_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$WEEPCODE_HOOK_EVENT" > {f}; echo "NAME=$WEEPCODE_HOOK_NAME" >> {f}; echo "SESSION=$WEEPCODE_SESSION_ID" >> {f}; echo "ROOT=$WEEPCODE_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
        f = output_file.display(),
    );
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "WEEPCODE_HOOK_EVENT": "spoofed_event",
                                "WEEPCODE_HOOK_NAME": "spoofed_name",
                                "WEEPCODE_SESSION_ID": "spoofed_session",
                                "WEEPCODE_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "WEEPCODE_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed WEEPCODE_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "WEEPCODE_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed WEEPCODE_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "WEEPCODE_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed WEEPCODE_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
