...
Sample hooks for WeepCode. Copy to `~/.weepcode/hooks/` to enable globally, or to `<project>/.weepcode/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.weepcode/hooks/bin
cp examples/hooks/safe-shell.json ~/.weepcode/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.weepcode/hooks/bin/
chmod +x ~/.weepcode/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.weepcode/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.weepcode/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.weepcode/hooks/bin/
chmod +x ~/.weepcode/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.weepcode/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.weepcode/hooks/bin
cp examples/hooks/session-log.json ~/.weepcode/hooks/
cp examples/hooks/bin/session-log.sh ~/.weepcode/hooks/bin/
chmod +x ~/.weepcode/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.weepcode/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.weepcode/hooks/bin
cp examples/hooks/tool-logger.json ~/.weepcode/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.weepcode/hooks/bin/
chmod +x ~/.weepcode/hooks/bin/tool-logger.sh
```
...
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match WeepCode names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.weepcode/hooks/`. The hook stops running on the next session.