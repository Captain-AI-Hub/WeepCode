...
//! `off`. Custom profiles via `~/.weepcode/sandbox.toml` or `.weepcode/sandbox.toml`.
...
/// Load sandbox config from `~/.weepcode/sandbox.toml` and `.weepcode/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.weepcode/sandbox.toml
    let global_path = weepcode_home().join("sandbox.toml");
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: <workspace>/.weepcode/sandbox.toml (additive only)
    let project_path = workspace.join(".weepcode").join("sandbox.toml");
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    let global = load_config_file(&weepcode_home().join("sandbox.toml")).unwrap_or_default();
    let project =
        load_config_file(&workspace.join(".weepcode").join("sandbox.toml")).unwrap_or_default();
...
impl ProfileName {
    ...
    pub(crate) fn capability_set_from_profile(
        ...
        // directories like ~/.weepcode/ that may not exist on first run.
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.weepcode/sandbox.toml or .weepcode/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
