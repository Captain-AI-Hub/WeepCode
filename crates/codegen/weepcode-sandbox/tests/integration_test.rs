//! Integration tests for weepcode-sandbox.
//!
...
fn test_profile_capability_set_construction() {
    use weepcode_sandbox::ProfileName;
...
fn test_sandbox_logger() {
    use weepcode_sandbox::{SandboxEvent, SandboxLogger};
...
fn test_should_restrict_child_network_default() {
    ...
    assert!(!weepcode_sandbox::should_restrict_child_network());
}
