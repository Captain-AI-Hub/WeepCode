...
//!
//! ```
//! use weepcode_mermaid::{default_engine, render_checked, RenderLimits, RenderParams};
//!
...
pub enum MermaidTheme {
    /// Light surfaces with dark text (e.g. `WeepcodeDay`).
    #[default]
    Light,
    /// Dark surfaces with light text (e.g. `WeepcodeNight`, `TokyoNight`).
    Dark,
}
