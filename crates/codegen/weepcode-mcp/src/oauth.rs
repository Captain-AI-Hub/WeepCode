...
const MCP_OAUTH_CLIENT_NAME: &str = "WeepCode";
...
// mode disabled, multiple `weepcode` invocations).
//
// Layer 1 (cross-process): filesystem lock at $WEEPCODE_HOME/mcp_auth_{safe_name}.lock
...
fn auth_lock_path(server_name: &str) -> std::path::PathBuf {
    let safe: String = server_name
        ...
        .collect();
    weepcode_config::weepcode_home().join(format!("mcp_auth_{safe}.lock"))
}
