...
//!
//! Credentials are stored in `$WEEPCODE_HOME/mcp_credentials.json`, keyed by a
//! composite key derived from the server name and URL. This keeps MCP OAuth
//! tokens isolated from the user's WeepCode auth (`auth.json`).
//!
...
///
/// Local helper (not shell-base): `weepcode-mcp` sits below `config-types` in the
...
/// File name for the credential store inside `$WEEPCODE_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$WEEPCODE_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$WEEPCODE_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user weepcode home (set $WEEPCODE_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$WEEPCODE_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(weepcode_config::user_weepcode_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$WEEPCODE_HOME/mcp_credentials.json`. Existing credential files must keep
    ...
    fn save_and_load_from_file() {
        let dir = std::env::temp_dir().join("weepcode-mcp-credentials-test");
