...
//! lookup must be cheaper than re-loading from `~/.weepcode/config.toml`.
...
//! `weepcode_shell::util::config::set_theme()` (and friends), invoked
...
static CURRENT: AtomicU8 = AtomicU8::new(ThemeKind::WeepcodeNight as u8);
...
/// back to `WeepcodeNight` if the byte is somehow out of range (which
...
fn theme_kind_from_u8(byte: u8) -> ThemeKind {
    match byte {
        x if x == ThemeKind::WeepcodeNight as u8 => ThemeKind::WeepcodeNight,
        x if x == ThemeKind::WeepcodeDay as u8 => ThemeKind::WeepcodeDay,
...
/// When `None`, `to_theme_kind()` defaults to `WeepcodeNight` / `WeepcodeDay`.
...
///
/// On the first call, reads from `~/.weepcode/config.toml` (via the shell's
...
pub fn current_kind() -> ThemeKind {
    ...
    if terminal_native_locked() {
        return ThemeKind::WeepcodeNight;
    }
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    weepcode_markdown::set_color_level_cap(if locked {
        weepcode_markdown::ColorLevel::Basic
    } else {
        weepcode_markdown::ColorLevel::TrueColor
    });
}
...
///
/// Precedence:
/// 1. Environment variable (`WEEPCODE_THEME`)
/// 2. Config file (`[ui].theme`)
/// 3. Default: `WeepcodeNight`
#[must_use]
...
fn resolve_from_appearance(appearance: Option<system_appearance::SystemAppearance>) -> ThemeKind {
    ...
    appearance
        ...
        .unwrap_or(ThemeKind::WeepcodeNight)
}
...
/// and the user's dark/light theme mapping. Falls back to `WeepcodeNight`
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `weepcode_shell::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = weepcode_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = weepcode_config::load_effective_config_disk_only() else {
...
pub fn reset_for_test() {
    ...
    CURRENT.store(ThemeKind::WeepcodeNight as u8, Ordering::Relaxed);
...
mod tests {
    ...
    fn terminal_native_lock_pins_kind_and_blocks_apply_kind() {
        with_test_env(|| {
            set(ThemeKind::WeepcodeDay);
            set_terminal_native_lock(true);
            assert!(terminal_native_locked());
            assert_eq!(current_kind(), ThemeKind::WeepcodeNight, "nominal kind");

            let applied = super::super::Theme::apply_kind(ThemeKind::WeepcodeDay);
            assert_eq!(applied, ThemeKind::WeepcodeNight, "apply_kind must no-op");
            assert_eq!(current_kind(), ThemeKind::WeepcodeNight);
    ...
    fn terminal_native_lock_serves_terminal_default_palette() {
        with_test_env(|| {
            ...
            assert_ne!(
                theme.text_primary,
                super::super::Theme::weepcodeday().text_primary,
                "must not serve the cached (WeepcodeDay) theme"
            );
        });
    }
    ...
    fn terminal_native_lock_caps_quantize_at_ansi16() {
        ...
        with_test_env(|| {
            ...
            for input in [
                Color::Rgb(0x26, 0x26, 0x26), // weepcodeday text_primary
    ...
    fn resolve_no_osc11_explicit_auto_and_default() {
        with_test_env(|| {
            assert_eq!(
                resolve_from_config(Some(ThemeKind::WeepcodeDay), false),
                ThemeKind::WeepcodeDay
            );
            assert!(!is_auto_mode());
            ...
            assert_eq!(resolve_from_config(None, false), ThemeKind::WeepcodeNight);
        });
    }
    ...
    #[test]
    fn resolve_auto_dark_system_returns_weepcodenight() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Dark));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::WeepcodeNight);
        });
    }

    #[test]
    fn resolve_auto_light_system_returns_weepcodeday() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Light));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::WeepcodeDay);
        });
    }

    #[test]
    fn resolve_auto_detection_failure_returns_weepcodenight() {
        with_test_env(|| {
    ...
    #[test]
    fn resolve_from_config_no_config_returns_weepcodenight() {
        with_test_env(|| {
    ...
    fn resolve_from_config_explicit_theme_returns_it() {
        with_test_env(|| {
            let result = resolve_from_config(Some(ThemeKind::WeepcodeDay), true);
            assert_eq!(result, ThemeKind::WeepcodeDay);
            assert!(
                !is_auto_mode(),
    ...
    fn set_then_current_kind_round_trips() {
        with_test_env(|| {
            ...
            set(ThemeKind::WeepcodeDay);
            assert_eq!(current_kind(), ThemeKind::WeepcodeDay);
        });
    }
