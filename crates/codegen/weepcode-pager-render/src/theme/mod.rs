...
//! The default theme is WeepcodeNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! WeepcodeNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::WeepcodeNight, ThemeKind::WeepcodeDay];
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::WeepcodeNight => "weepcodenight",
            Self::TokyoNight => "tokyonight",
            Self::WeepcodeDay => "weepcodeday",
    ...
    /// when quantized to 256 or 16 colors. WeepcodeNight uses neutral grays
    /// that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::WeepcodeNight => false,
            Self::TokyoNight => true,
            Self::WeepcodeDay => false,
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "weepcodenight" | "weepcode-night" | "dark" => Some(Self::WeepcodeNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "weepcodeday" | "weepcode-day" | "light" | "day" => Some(Self::WeepcodeDay),
...
/// `"weepcodenight"` → `"WeepCode Night"`). Falls back to `value` verbatim.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "weepcodenight" => "WeepCode Night",
        "weepcodeday" => "WeepCode Day",
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.weepcode/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::WeepcodeNight => Self::weepcodenight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::WeepcodeDay => Self::weepcodeday(),
            ...
            // if reached, fall back to WeepcodeNight.
            ThemeKind::Auto => Self::weepcodenight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `WEEPCODE_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on weepcodenight, erasing every state signal). We pin each
    ...
    /// theme (e.g., WeepcodeNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // WeepcodeNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    weepcode_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::WeepcodeNight.is_auto());
        assert!(!ThemeKind::WeepcodeDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::weepcodeday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::weepcodenight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::weepcodeday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. WeepcodeNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::WeepcodeNight => Theme::weepcodenight(),
                ThemeKind::WeepcodeDay => Theme::weepcodeday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_weepcodenight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark WeepcodeNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::weepcodenight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("weepcodenight"),
            Some(ThemeKind::WeepcodeNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::WeepcodeNight));
        assert_eq!(
            ThemeKind::from_name("weepcodeday"),
            Some(ThemeKind::WeepcodeDay)
        );
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::WeepcodeDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("weepcodenight", ThemeKind::WeepcodeNight),
            ("weepcode-night", ThemeKind::WeepcodeNight),
            ("dark", ThemeKind::WeepcodeNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("weepcodeday", ThemeKind::WeepcodeDay),
            ("weepcode-day", ThemeKind::WeepcodeDay),
            ("light", ThemeKind::WeepcodeDay),
            ("day", ThemeKind::WeepcodeDay),
