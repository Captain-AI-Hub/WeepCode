...
/// in `config.toml`. When `None`, defaults to `WeepcodeNight` / `WeepcodeDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::WeepcodeDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::WeepcodeNight),
    }
}
