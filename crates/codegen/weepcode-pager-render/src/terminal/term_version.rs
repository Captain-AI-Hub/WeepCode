...
/// `refine_unknown_brand_for_host` may rewrite `TerminalContext::brand`: the
/// native-Windows `Unknown -> WindowsTerminal` guess must not license a
/// version attribution.
...
mod tests {
    ...
    fn snapshot(pairs: &[(&str, &str)]) -> weepcode_telemetry::events::TerminalTelemetry {
