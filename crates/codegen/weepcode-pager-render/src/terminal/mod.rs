...
/// Run `tmux show-option -gqv <option>` with a bounded subprocess and return
/// the trimmed value, or `None` if the query fails or returns empty /
/// whitespace.
pub fn tmux_show_option(option: &str) -> Option<String> {
    tmux_probe::query_option(option).into_option()
}
...
pub enum TerminalName {
    ...
    JetBrains,
    /// WeepCode Desktop (Electron app).
    #[strum(to_string = "WeepCode Desktop")]
    WeepcodeDesktop,
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` weepcode is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches weepcode, which
    ...
    pub fn telemetry_snapshot(&self) -> weepcode_telemetry::events::TerminalTelemetry {
        ...
        weepcode_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: weepcode_shared::clipboard::native_tool_name().to_owned(),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `weepcode ssh` can
    // forward env vars.
    let mut ctx = build_terminal_context_from_env(&env);
...
/// `weepcode-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
///    explicit Byobu backend) won; a real mux nested inside either still wins.
///    Between the two, `HERDR_ENV` beats the `CMUX_*` markers: the shape they
///    appear in together is herdr running in a cmux panel, where the `CMUX_*`
///    values are inherited rather than fresh.
///
...
fn terminal_name_from_term_program(value: &str) -> Option<TerminalName> {
    ...
    match normalized.as_str() {
        ...
        "grokdesktop" => Some(TerminalName::WeepcodeDesktop),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.weepcode/pager.toml` and
