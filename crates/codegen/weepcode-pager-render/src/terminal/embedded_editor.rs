//! Detects whether weepcode is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` weepcode is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `weepcode-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).
