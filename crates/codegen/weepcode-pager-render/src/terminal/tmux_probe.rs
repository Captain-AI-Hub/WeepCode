...
fn run_tmux_bounded(
    ...
    let group = weepcode_tty_utils::ProcessGroup::new()
        .and_then(|mut group| {
...
fn terminate_tmux_tree(group: &weepcode_tty_utils::ProcessGroup, child: &mut std::process::Child) {
    terminate_owned_group(group);
    let _ = child.wait();
}

fn terminate_owned_group(group: &weepcode_tty_utils::ProcessGroup) {
...
mod tests {
    ...
    fn command_protocol_uses_exact_argv_and_pager_env() {
        ...
        for (request, args) in cases {
            ...
            let actual: HashMap<OsString, Option<OsString>> = cmd
                ...
                .collect();
            let expected: HashMap<OsString, Option<OsString>> = weepcode_tty_utils::pager_env()
                .into_iter()
    ...
    /// A leader that exits successfully while a descendant still holds the
    /// captured pipes must still return the leader's output after process-group
    /// teardown closes those inherited handles.
    #[cfg(unix)]
    #[test]
    #[serial_test::serial(tmux_probe_path)]
    fn successful_exit_with_pipe_holding_descendant_returns_captured_output() {
        ...
        let output = result
            ...
            .expect("successful probe must drain output after group teardown");
