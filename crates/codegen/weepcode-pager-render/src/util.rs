...
pub use weepcode_config::weepcode_home;

/// Path to `$WEEPCODE_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    weepcode_home().join("pager.toml")
}

/// User-facing label for the user weepcode directory (``~/.weepcode`` or ``$WEEPCODE_HOME``).
///
/// Derived from resolved [`weepcode_home()`] vs `weepcode_config::default_weepcode_home()`,
/// not from whether `WEEPCODE_HOME` is set in the environment.
pub fn display_weepcode_home_prefix() -> String {
    if weepcode_home() == weepcode_config::default_weepcode_home() {
        "~/.weepcode".to_string()
    } else {
        "$WEEPCODE_HOME".to_string()
    }
}

/// User-facing path under [`weepcode_home()`], e.g. ``~/.weepcode/config.toml``.
pub fn display_user_weepcode_path(relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_weepcode_home_prefix();
...
/// Abbreviate an absolute path for display: prefer [`weepcode_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let weepcode = weepcode_home();
    if let Ok(rest) = path_buf.strip_prefix(&weepcode) {
        let prefix = display_weepcode_home_prefix();
...
/// True when `path` is under user [`weepcode_home()`] (not project `{cwd}/.weepcode`).
pub fn is_under_user_weepcode_home(path: &Path) -> bool {
    path.starts_with(weepcode_home())
}
...
mod tests {
    ...
    #[test]
    fn display_weepcode_home_prefix_default_install() {
        if std::env::var("WEEPCODE_HOME").is_ok() {
            return;
        }
        assert_eq!(display_weepcode_home_prefix(), "~/.weepcode");
    }

    #[test]
    fn display_user_weepcode_path_joins_relative() {
        let path = display_user_weepcode_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".weepcode") || path.contains("$WEEPCODE_HOME"));
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_weepcode() {
        if let Ok(home) = std::env::var("HOME") {
            if home.is_empty() {
                return;
            }
            let full = format!("{home}/.weepcode/memory/MEMORY.md");
