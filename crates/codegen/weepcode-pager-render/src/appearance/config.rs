...
pub struct AnimationConfig {
    ...
    /// Also enabled by the `WEEPCODE_FPS=1` env var. Default: false.
    pub show_fps: bool,
}
...
pub struct RawAnimationConfig {
    ...
    /// Requires a debug build. Also enabled by WEEPCODE_FPS=1 env var. Default: false.
    pub show_fps: bool,
}
...
fn lookup_named_color(name: &str) -> Result<Color, String> {
    // Named colors use the WeepcodeNight RGB palette. They are quantized via
...
impl RawAppearanceConfig {
    pub fn to_toml_with_comments() -> String {
        ...
        let pager_path = crate::util::display_user_weepcode_path("pager.toml");
        let header = format!(
            "\
# WeepCode Pager Appearance Configuration ({pager_path})
...
pub fn persist_respect_manual_folds(enabled: bool) -> std::io::Result<()> {
    ...
    if weepcode_config::user_weepcode_home().is_none() {
        return Err(Error::new(
            ErrorKind::NotFound,
            "no user weepcode home resolved; refusing to write a cwd-relative pager.toml \
