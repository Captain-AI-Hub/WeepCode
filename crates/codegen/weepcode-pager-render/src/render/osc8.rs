...
//! frame diff (see `weepcode_ratatui_inline::Terminal::flush_with_links`).
...
/// One path segment without spaces (`main.rs`, `.weepcode`, `@scope`). Leading `.`
/// matches dot-directories and `%` matches percent-encoded segments — weepcode
/// session media lives under `~/.weepcode/sessions/%2F…/images/1.jpg`.
...
/// Relative file path (`images/1.png`, `.weepcode/x.txt`) — one or more `/`-joined
...
/// media lives at `~/.weepcode/sessions/%2F…/images/1.jpg`, which wraps in
...
mod tests {
    ...
    #[test]
    fn scan_detects_weepcode_session_media_path() {
        // Dot-directory (`.weepcode`), percent-encoded session segment, and a
        // trailing sentence period — the shape of `image_gen` output prose.
        let line =
            make_line("Saved to /Users/alice/.weepcode/sessions/%2Fabc/00000000/images/1.jpg.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
        ...
        assert_eq!(
            ...
            "file:///Users/alice/.weepcode/sessions/%252Fabc/00000000/images/1.jpg",
        );
    }
    ...
    fn scan_detects_media_path_soft_wrapped_across_rows() {
        ...
        let row0 = make_line(
            "Image generated and saved to /Users/alice/.weepcode/sessions/%2FUsers%2Fali",
        );
        ...
        let expected_url = "file:///Users/alice/.weepcode/sessions/%252FUsers%252Fali\
    ...
    fn scan_wrapped_path_trailing_sentence_period_excluded() {
        ...
        let row0 = make_line("Saved to /Users/me/.weepcode/sessions/%2Fabc/019f3a86/ima");
        ...
        for link in overlay.links() {
            assert_eq!(
                &*resolve_link_target(&link.target)
                    .and_then(|resolved| resolved.osc8_url)
                    .expect("url"),
                "file:///Users/me/.weepcode/sessions/%252Fabc/019f3a86/images/1.jpg"
            );
        }
    ...
    fn scan_file_path_with_dots_and_hyphens() {
        let line = make_line("Reading /tmp/weepcode-impl-summary.md now.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
        ...
        assert_eq!(
            &*resolve_link_target(&overlay.links()[0].target)
                .and_then(|resolved| resolved.osc8_url)
                .expect("url"),
            "file:///tmp/weepcode-impl-summary.md"
        );
    }
    ...
    fn scan_detects_tilde_file_path() {
        ...
        let raw = "~/Desktop/weepcode-pager-retention-findings-2026-06-06.md";
    ...
    fn scan_relative_path_not_partially_linkified() {
        // A relative path like `crates/codegen/weepcode-pager/src/render` should
        // NOT produce a link for the `/weepcode-pager/src/render` substring.
        let line = make_line("find crates/codegen/weepcode-pager/src/render -name '*.rs'");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
