...
//! after every tool call to check for SKILL.md files in `.weepcode/skills/`,
...
/// Directories that contain skill definitions (`.weepcode/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".weepcode", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.weepcode/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.weepcode/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.weepcode/) are outside the git root so the walk breaks early.
