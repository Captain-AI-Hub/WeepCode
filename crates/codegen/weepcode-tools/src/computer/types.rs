...
/// (`weepcode-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (weepcode-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in weepcode-tools, spawns processes)
/// - `AcpTerminalBackend` (in weepcode-shell, calls ACP protocol)
#[async_trait::async_trait]
