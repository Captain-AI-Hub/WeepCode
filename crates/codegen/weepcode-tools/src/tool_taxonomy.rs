...
//! `ToolInput`, proto, or runtime). A future `weepcode-tool-taxonomy` leaf crate
...
/// [`CanonicalToolMeta`].
pub const TOOL_META_KEY: &str = "weepcode/tool";
...
///   share it (weepcode `read_file` → `"Read"`).
...
mod tests {
    ...
    fn identity(kind: ToolKind) -> ToolIdentity {
        ToolIdentity {
            tool_kind: kind,
            namespace: ToolNamespace::WeepcodeBuild,
    ...
    fn namespace_round_trips_snake_case_with_pascal_aliases() {
        ...
        fn wire_and_pascal(ns: ToolNamespace) -> (&'static str, &'static str) {
            match ns {
                ToolNamespace::WeepcodeBuild => ("weepcode_build", "WeepcodeBuild"),
                ToolNamespace::WeepcodeBuildConcise => {
                    ("weepcode_build_concise", "WeepcodeBuildConcise")
                }
                ToolNamespace::WeepcodeBuildHashline => {
                    ("weepcode_build_hashline", "WeepcodeBuildHashline")
                }
