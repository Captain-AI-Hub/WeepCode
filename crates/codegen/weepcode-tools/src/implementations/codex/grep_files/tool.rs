...
use crate::implementations::weepcode_build::grep::ripgrep::rg_path;
...
///
/// Shares `ToolKind::Search` with the weepcode-build `GrepTool`. These tools are
/// namespace-exclusive — consumers enable either `WeepcodeBuild` or `Codex` search,
...
impl weepcode_tool_runtime::Tool for CodexGrepFilesTool {
    type Args = CodexGrepFilesInput;
    type Output = CodexGrepFilesOutput;

    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new("grep_files").expect("valid tool id")
    }
    ...
    ) -> Result<CodexGrepFilesOutput, weepcode_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn test_ctx(cwd: &Path) -> weepcode_tool_runtime::ToolCallContext {
    ...
    async fn tool_reports_empty_pattern_error() {
        ...
        let result = weepcode_tool_runtime::Tool::run(&tool, test_ctx(tmp.path()), input)
            .await
            .unwrap();
        match result {
