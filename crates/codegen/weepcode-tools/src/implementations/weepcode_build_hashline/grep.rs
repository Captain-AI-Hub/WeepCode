...
use crate::implementations::weepcode_build::grep::{GrepSearchInput, GrepTool, OutputMode};
...
impl crate::types::tool_metadata::ToolMetadata for HashlineGrepTool {
    ...
    fn tool_namespace(&self) -> ToolNamespace {
        ToolNamespace::WeepcodeBuildHashline
    }
...
impl weepcode_tool_runtime::Tool for HashlineGrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new("hashline_grep").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let mut rt_ctx = weepcode_tool_runtime::ToolCallContext::new(call_id);
        rt_ctx.extensions.insert(resources.clone());
        rt_ctx.extensions.insert(weepcode_tool_runtime::Cwd(cwd));
        let mut result = weepcode_tool_runtime::Tool::run(&grep, rt_ctx, input)
            .await
            .map_err(|e| {
                weepcode_tool_runtime::ToolError::execution(
                    weepcode_tool_protocol::ToolId::new("grep").expect("valid"),
                    e.to_string(),
                )
            })?;
        ...
        if matches!(output_mode, OutputMode::Content) && result.exit_code == 0 {
            ...
            let (fs, scheme) = {
                ...
                let scheme = params
                    .0
                    .build_scheme()
                    .map_err(weepcode_tool_runtime::ToolError::invalid_arguments)?;
                (fs, scheme)
            };
