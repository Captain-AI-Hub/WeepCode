//! `image_edit` tool — edits or transforms images via the WeepCode Imagine
...
use crate::implementations::weepcode_build::image_gen::{ImageGenClient, ImageGenResponse};
...
fn compress_reference(
    raw_bytes: Vec<u8>,
) -> Result<(Vec<u8>, &'static str), weepcode_tool_runtime::ToolError> {
    ...
    let img = ImageReader::new(Cursor::new(&raw_bytes))
        ...
        .ok_or_else(|| {
            weepcode_tool_runtime::ToolError::invalid_arguments("failed to decode image reference")
        })?;
...
async fn resolve_to_data_url(value: &str) -> Result<String, weepcode_tool_runtime::ToolError> {
...
impl weepcode_tool_runtime::Tool for ImageEditTool {
    type Args = ImageEditInput;
    type Output = ToolOutput;

    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new("image_edit").expect("valid tool id")
    }
