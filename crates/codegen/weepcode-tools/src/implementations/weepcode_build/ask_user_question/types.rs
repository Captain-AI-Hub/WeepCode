...
//!
//! - **`weepcode-tools`** — tool blocks on a oneshot, formats the result.
//! - **`weepcode-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`weepcode-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `weepcode-tools`.
...
register_resource!("weepcode_build", "UserQuestionSender", UserQuestionSender);
