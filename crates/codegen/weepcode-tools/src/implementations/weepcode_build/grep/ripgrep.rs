...
const RG_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-rg/rg-",
    env!("WEEPCODE_TOOLS_RG_VER"),
    "-",
    env!("WEEPCODE_TOOLS_RG_TARGET"),
    ".bin"
));
...
fn resolve_bundled_rg() -> std::io::Result<PathBuf> {
    ...
    let p = crate::util::weepcode_home().join("vendor").join(concat!(
        "rg-",
        env!("WEEPCODE_TOOLS_RG_VER"),
        "-",
        env!("WEEPCODE_TOOLS_RG_TARGET")
...
/// binary to ~/.weepcode/vendor/ and returns that path.
