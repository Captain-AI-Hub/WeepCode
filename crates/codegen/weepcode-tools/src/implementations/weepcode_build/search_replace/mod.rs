...
register_resource!("weepcode_build", "SearchReplace", SearchReplaceParams);
...
) -> Result<SearchReplaceOutput, weepcode_tool_runtime::ToolError> {
            ...
            let output = match e.io_error_kind() {
                ...
                _ => {
                    return Err(weepcode_tool_runtime::ToolError::execution(
                        weepcode_tool_protocol::ToolId::new("search_replace").expect("valid"),
                        e.to_string(),
                    ));
                }
            };
...
impl weepcode_tool_runtime::Tool for SearchReplaceTool {
    type Args = SearchReplaceInput;
    type Output = SearchReplaceOutput;
    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new("search_replace").expect("valid tool id")
    }
    fn description(
        &self,
...
mod tests {
    ...
    async fn consecutive_edits_succeed_without_prior_read() {
        ...
        let result1 = weepcode_tool_runtime::Tool::run(
            &tool,
        ...
        let result2 = weepcode_tool_runtime::Tool::run(
            &tool,
            test_ctx(shared),
