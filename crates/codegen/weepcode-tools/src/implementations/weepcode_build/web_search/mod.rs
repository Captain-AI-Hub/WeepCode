...
impl weepcode_tool_runtime::Tool for WebSearchTool {
    type Args = WebSearchInput;
    type Output = WebSearchOutput;

    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new("web_search").expect("valid tool id")
    }
    ...
    ) -> Result<WebSearchOutput, weepcode_tool_runtime::ToolError> {
        ...
        let (content, citations) = client
            ...
            .map_err(|e| {
                weepcode_tool_runtime::ToolError::execution(
                    weepcode_tool_protocol::ToolId::new("web_search").expect("valid"),
                    e.to_string(),
                )
            })?;
