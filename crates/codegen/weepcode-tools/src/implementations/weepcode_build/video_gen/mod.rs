...
//! the WeepCode Video Generation API and save them to the local filesystem so the
...
const WEEPCODE_VIDEO_BASE_MODEL: &str = "weepcode-imagine-video";
const WEEPCODE_VIDEO_QUALITY_MODEL: &str = "weepcode-imagine-video-1.5-preview";
...
const DEFAULT_ZDR_VIDEO_KEY_PREFIX: &str = "weepcode-videos/";
...
pub use weepcode_tools_api::slash_commands::{
...
impl S3AccessCredentials {
    ...
    fn to_static(&self) -> weepcode_file_utils::s3::S3StaticCredentials {
        weepcode_file_utils::s3::S3StaticCredentials {
...
/// HTTP client for WeepCode Video Generation API. Cloned per-request; shares `Arc` state.
#[derive(Clone)]
...
impl VideoGenClient {
    pub fn new(
        ...
        api_key_provider: Option<SharedApiKeyProvider>,
    ) -> Result<Self, weepcode_tool_runtime::ToolError> {
        ...
        extra_headers.into_iter().try_for_each(|(key, value)| {
            ...
            Ok::<(), weepcode_tool_runtime::ToolError>(())
        })?;
    ...
    pub async fn generate_with_images(
    ...
    ) -> Result<VideoOutcome, weepcode_tool_runtime::ToolError> {
        ...
        if !status.is_success() {
            ...
            return Err(weepcode_tool_runtime::ToolError::new(
                weepcode_tool_runtime::ToolErrorKind::Custom,
                ...
                "done" => {
                    ...
                    return match presigned {
                        ...
                        None if video_url.is_empty() => {
                            Err(weepcode_tool_runtime::ToolError::invalid_arguments(
    ...
    async fn download_video(&self, url: &str) -> Result<Vec<u8>, weepcode_tool_runtime::ToolError> {
    ...
    async fn finish_zdr_video(
        ...
        let config = self.zdr_video_output_s3.as_ref().ok_or_else(|| {
            weepcode_tool_runtime::ToolError::invalid_arguments(
    ...
    async fn presign_zdr_output_urls(
    ...
    ) -> Result<ZdrPresignedUrls, weepcode_tool_runtime::ToolError> {
        ...
        let upload_url = weepcode_file_utils::s3::presign_put_url(
            &config.region,
            endpoint,
    ...
    async fn presign_and_download(
        ...
        urls: &ZdrPresignedUrls,
        request_id: &str,
    ) -> Result<Vec<u8>, weepcode_tool_runtime::ToolError> {
        let get_url = self
    ...
    async fn presign_zdr_get_url(
        ...
        let url = weepcode_file_utils::s3::presign_get_url(
            &config.region,
            endpoint,
            &creds.to_static(),
            &config.bucket,
...
async fn resolve_image_reference(value: &str) -> Result<String, weepcode_tool_runtime::ToolError> {
...
) -> Result<(VideoGenClient, std::path::PathBuf), weepcode_tool_runtime::ToolError> {
...
async fn save_video_bytes(
    ...
    video_bytes: &[u8],
) -> Result<std::path::PathBuf, weepcode_tool_runtime::ToolError> {
...
) -> Result<MediaGenOutput, weepcode_tool_runtime::ToolError> {
    match outcome {
...
impl weepcode_tool_runtime::Tool for ImageToVideoTool {
    type Args = ImageToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new(IMAGE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
...
impl weepcode_tool_runtime::Tool for ReferenceToVideoTool {
    type Args = ReferenceToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new(REFERENCE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
...
mod tests {
    ...
    fn image_and_reference_payload_fields_are_serialized() {
        let payload = GenerateVideoPayload {
            model: WEEPCODE_VIDEO_QUALITY_MODEL,
            prompt: "animate",
        ...
        let payload = GenerateVideoPayload {
            model: WEEPCODE_VIDEO_BASE_MODEL,
            prompt: "blend",
            image: None,
            duration: Some(6),
    ...
    async fn image_to_video_rejects_bad_duration() {
        ...
        let err = weepcode_tool_runtime::Tool::run(
            &tool,
