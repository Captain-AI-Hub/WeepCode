...
crate::register_resource!("weepcode_build", "Todo", TodoState);
...
impl weepcode_tool_runtime::Tool for TodoWriteTool {
    ...
    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new("todo_write").expect("valid tool id")
    }
...
mod tests {
    ...
    async fn state_serialization_roundtrip() {
        ...
        assert!(
            state_map.get("weepcode_build.Todo").is_some(),
            "TodoState should serialize under 'weepcode_build.Todo'"
        );
