...
//! (in `weepcode-tools`) and the subagent coordinator (in `weepcode-shell`).
//!
//! ## Resource types
//!
...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"weepcode-build-plan"`) whose `AgentDefinition` decides the child's harness
...
pub use weepcode_tool_types::is_not_sentinel;
...
/// defense-in-depth check in `weepcode-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "WeepcodeBuild:run_terminal_cmd" | "WeepcodeBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
/// the cross-crate drain loop in `weepcode-shell` relies on
...
register_resource!("weepcode_build", "SubagentEventSender", SubagentEventSender);
...
pub struct MonitorEventNotification {
    ...
    /// events inside another session's turn. `None` for legacy / non-weepcode-build
...
pub type MonitorEventBuffer = weepcode_interjection_core::EventQueue<MonitorEventNotification>;

register_resource!("weepcode_build", "MonitorEventBuffer", MonitorEventBuffer);
...
/// weepcode-shell produces it, the channel protocol carries it, and the
/// compaction pipeline in weepcode-chat-state (via `RunningSubagentSummary`)
...
register_resource!("weepcode_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("weepcode_build", "SessionIdResource", SessionIdResource);

/// Carries the current parent prompt/turn ID for TaskTool subagent scoping.
///
/// Set by weepcode-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by weepcode-shell at turn start.
...
register_resource!("weepcode_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("WeepcodeBuild:run_terminal_cmd", ToolKind::Execute),
                tc("WeepcodeBuild:read_file", ToolKind::Read),
                tc("WeepcodeBuild:list_dir", ToolKind::List),
                tc("WeepcodeBuild:grep", ToolKind::Search),
                tc("WeepcodeBuild:kill_task", ToolKind::KillTaskAction),
                tc(
    ...
    fn read_only_filter_keeps_background_task_tools_when_task_tool_remains() {
        let mut config = ToolServerConfig {
            tools: vec![
                ...
                tc("WeepcodeBuild:task", ToolKind::Task),
            ],
            behavior_preset: None,
        };
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("WeepcodeBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
