...
use crate::implementations::weepcode_build::task_output::{
...
use weepcode_tool_types::{MultiTaskOutputResult, TaskOutputOutput, WaitMode, WaitTasksToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for WaitTasksTool {
    ...
    fn description_template(&self) -> &str {
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            weepcode_tool_types::build_wait_tasks_description(
                &weepcode_tool_types::WaitTasksToolNaming {
...
/// [`weepcode_tool_types::build_wait_tasks_description`] builder so the CLI and
...
fn wait_tasks_description(
    ...
    weepcode_tool_types::build_wait_tasks_description(&weepcode_tool_types::WaitTasksToolNaming {
...
impl weepcode_tool_runtime::Tool for WaitTasksTool {
    type Args = WaitTasksToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new("wait_tasks").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let timeout = crate::implementations::weepcode_build::task_output::capped_wait_timeout(
            input.timeout_ms,
        );
