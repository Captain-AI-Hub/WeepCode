...
use crate::implementations::weepcode_build::task::TaskTool;
use crate::implementations::weepcode_build::task::backend::SubagentBackendResource;
use crate::implementations::weepcode_build::task::types::{
    SubagentSnapshot, SubagentSnapshotStatus,
};
use crate::implementations::weepcode_build_concise::BashConciseTool;
...
/// `TaskOutput` cap. Env override: `WEEPCODE_MAX_WAIT_BLOCK_MS`.
const MAX_WAIT_BLOCK: Duration = Duration::from_secs(600);

fn max_wait_block() -> Duration {
    std::env::var("WEEPCODE_MAX_WAIT_BLOCK_MS")
        .ok()
...
pub(crate) fn background_bash_requires_exprs() -> Vec<Expr<ToolRequirement>> {
    ...
    let weepcode_build_bash = Expr::Value(ToolRequirement::Tool {
        namespace: ToolMetadata::tool_namespace(&BashTool).to_string(),
        id: weepcode_tool_runtime::Tool::id(&BashTool)
            .as_str()
            .to_string(),
    ...
    let weepcode_build_concise_bash = Expr::Value(ToolRequirement::Tool {
        namespace: ToolMetadata::tool_namespace(&BashConciseTool).to_string(),
        id: weepcode_tool_runtime::Tool::id(&BashConciseTool)
            .as_str()
            .to_string(),
        if_params: Some(Expr::Value(ToolParamsRequirement {
            key: "enabled_background".to_string(),
    ...
    let opencode_bash = Expr::Value(ToolRequirement::Tool {
        ...
        id: weepcode_tool_runtime::Tool::id(&OpenCodeBashTool)
            .as_str()
            .to_string(),
        if_params: None,
    });
...
pub(crate) fn task_output_requires_expr() -> Expr<ToolRequirement> {
    ...
    let task_tool = Expr::Value(ToolRequirement::Tool {
        ...
        id: weepcode_tool_runtime::Tool::id(&TaskTool)
            .as_str()
            .to_string(),
        if_params: None,
    });
...
impl TaskOutputTool {
    async fn run_single_task(
    ...
    ) -> Result<TaskOutputOutput, weepcode_tool_runtime::ToolError> {
        let contract_version = ctx
            .extensions
            .get::<weepcode_tool_runtime::BehaviorVersion>()
            .map(|v| v.0.clone());
        ...
        let waits = weepcode_tool_types::task_output_waits(timeout_ms);
        ...
        if let Some(snapshot) = snapshot {
            let read_file_name;
            {
                ...
                read_file_name = renderer.render("${{ tools.by_kind.read }}").map_err(|e| {
                    weepcode_tool_runtime::ToolError::invalid_arguments(e.to_string())
                })?;
            }
    ...
    pub(crate) async fn run_multi_tasks(
        ...
        let (terminal, backend, read_file_name, max_output_bytes) = {
            ...
            let rfn = renderer
                ...
                .map_err(|e| weepcode_tool_runtime::ToolError::invalid_arguments(e.to_string()))?;
            let mob = res
                .get::<TruncationCfg>()
                .map(|cfg| {
                    cfg.0
...
pub(crate) use weepcode_tool_types::MAX_MULTI_WAIT_IDS;
...
fn format_subagent_snapshot(snap: &SubagentSnapshot) -> TaskOutputOutput {
    ...
    match &snap.status {
        ...
        SubagentSnapshotStatus::Completed {
            ...
            output.push_str("\n\n");
            output.push_str(&weepcode_tool_types::format_resume_footer(
                &snap.subagent_id,
                &snap.subagent_type,
...
impl crate::types::tool_metadata::ToolMetadata for TaskOutputTool {
    ...
    fn description_template(&self) -> &str {
        ...
        // fallback mirrors the default weepcode-build toolset.
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            weepcode_tool_types::build_task_output_description(
                &weepcode_tool_types::TaskOutputToolNaming {
...
/// [`weepcode_tool_types::build_task_output_description`] builder so the CLI and
...
fn task_output_description(
    ...
    weepcode_tool_types::build_task_output_description(&weepcode_tool_types::TaskOutputToolNaming {
...
impl weepcode_tool_runtime::Tool for TaskOutputTool {
    type Args = TaskOutputToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new("get_task_output").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        // The static fallback is the shared builder's default weepcode-build
    ...
    fn resources_with_backend_query() -> (
        ...
        tokio::sync::mpsc::UnboundedReceiver<
            crate::implementations::weepcode_build::task::types::SubagentEvent,
        >,
    ) {
        use crate::implementations::weepcode_build::task::backend::{
            ChannelBackend, SubagentBackendResource,
        };
        use crate::implementations::weepcode_build::task::types::SubagentEvent;
    ...
    fn unwrap_query(
        event: crate::implementations::weepcode_build::task::types::SubagentEvent,
    ) -> crate::implementations::weepcode_build::task::types::SubagentQueryRequest {
        match event {
            crate::implementations::weepcode_build::task::types::SubagentEvent::Query(q) => q,
