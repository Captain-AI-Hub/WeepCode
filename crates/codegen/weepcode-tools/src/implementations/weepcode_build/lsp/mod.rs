...
//! `LspTool` (Tool trait impl) under the `WeepcodeBuild` namespace.
...
impl weepcode_tool_runtime::ToolOutput for LspToolOutput {}
...
impl weepcode_tool_runtime::Tool for LspTool {
    type Args = LspToolInput;
    type Output = LspToolOutput;

    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new("lsp").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: LspToolInput,
    ) -> Result<LspToolOutput, weepcode_tool_runtime::ToolError> {
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    weepcode_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.weepcode/lsp.json or <cwd>/.weepcode/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
