...
use regex::Regex;
use weepcode_config::shell::AmpersandSemantics;
...
static BASH_CAPABILITIES: LazyLock<weepcode_tool_protocol::ToolCapabilities> =
    LazyLock::new(|| weepcode_tool_protocol::ToolCapabilities {
        is_read_only: false,
        tool_scope: Some(weepcode_tool_protocol::ToolScope::Write),
        streaming: Some(weepcode_tool_protocol::StreamingSpec {
...
fn bash_output_chunk_progress(
    spec: &weepcode_tool_protocol::StreamingSpec,
    chunk: &BashOutputChunk,
    last_total: &mut usize,
) -> Option<weepcode_tool_runtime::ToolProgress> {
    // `stream_chunk` counts in `u64`; convert at the boundary.
    let mut cursor = *last_total as u64;
    let progress = weepcode_tool_runtime::stream_chunk(
        spec,
        &chunk.base.output,
...
pub struct BashParams {
    ...
    /// weepcode-build opts up to 10h via weepcode-shell's `BashToolConfig`.
    ///
    /// When set:
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "weepcode_build.Bash";
    ...
    fn chat_completion_output(&self) -> Option<weepcode_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Foreground(bash) => {
                weepcode_tool_runtime::ToolOutput::chat_completion_output(bash)
            }
...
// particular production weepcode-build sets it to 10h in weepcode-shell's
...
/// [`weepcode_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
/// should use `background: true`. Env override: `WEEPCODE_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("WEEPCODE_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in weepcode_build bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the weepcode_build
...
impl BashTool {
    ...
    ///   the terminal backend default applies (`WEEPCODE_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `WEEPCODE_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = weepcode_config::shell::chain_separator();
...
impl weepcode_tool_runtime::Tool for BashTool {
    type Args = BashToolInput;
    type Output = BashToolOutput;

    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new("run_terminal_cmd").expect("valid tool id")
    }
    ...
    fn description(
        ...
        weepcode_tool_types::ToolDescription::new(
            "run_terminal_cmd",
    ...
    async fn execute(
        &self,
        mut ctx: weepcode_tool_runtime::ToolCallContext,
        input: BashToolInput,
    ) -> weepcode_tool_runtime::ToolStream<BashToolOutput> {
        ...
        if input.is_background {
            ...
            return Box::pin(async_stream::stream! {
                yield weepcode_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input).await);
            });
        }
        ...
        let Some(spec) = BASH_CAPABILITIES.streaming.as_ref() else {
            return Box::pin(async_stream::stream! {
                yield weepcode_tool_runtime::ToolStreamItem::Terminal(Err(
            ...
            loop {
                tokio::select! {
                    ...
                    maybe = rx.recv() => {
                        match maybe {
                            Some(ToolNotification::BashOutputChunk(chunk)) => {
                                ...
                                {
                                    yield weepcode_tool_runtime::ToolStreamItem::Progress(p);
                                }
                            }
                            Some(notif) => {
                    ...
                    result = &mut run_fut => {
                        ...
                        yield weepcode_tool_runtime::ToolStreamItem::Terminal(result);
                        break;
                    }
                }
            }
    ...
    async fn run(
        ...
        input: BashToolInput,
    ) -> Result<BashToolOutput, weepcode_tool_runtime::ToolError> {
        ...
        let ampersand = weepcode_config::shell::ampersand_semantics();
        if let Some(violation) = should_reject_background_op(
            ...
            return Err(weepcode_tool_runtime::ToolError::invalid_arguments(message));
        }
        ...
        if input.is_background {
            ...
            let request = TerminalRunRequest {
                ...
                // existing weepcode_build callers that never opted in are
                // unaffected.
            ...
            // We tag it for statistics (weepcode_build backend) and can surface an educational
...
mod tests {
    ...
    fn read_chunk_progress(p: &weepcode_tool_runtime::ToolProgress) -> (String, usize, bool, bool) {
        match p {
            weepcode_tool_runtime::ToolProgress::Custom { subkind, payload } => {
    ...
    fn bash_output_chunk_progress_caps_oversized_delta() {
        ...
        while last < total {
            let p = bash_output_chunk_progress(spec, &chunk, &mut last).unwrap();
            let weepcode_tool_runtime::ToolProgress::Custom { payload, .. } = p else {
    ...
    async fn bash_streaming_progress_suppressed_when_gate_off() {
        ...
        let tool = BashTool;
        let mut stream = weepcode_tool_runtime::Tool::execute(
            &tool,
            ctx,
        ...
        let mut terminal: Option<Result<BashToolOutput, weepcode_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                weepcode_tool_runtime::ToolStreamItem::Progress(_) => {
                    progress += 1;
                }
    ...
    async fn foreground_command_success() {
        ...
        let result = weepcode_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
    ...
    async fn background_injects_python_unbuffered() {
        ...
        let _ = weepcode_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
    ...
    fn background_chat_completion_is_none() {
        ...
        assert!(weepcode_tool_runtime::ToolOutput::chat_completion_output(&out).is_none());
    }
    ...
    mod resolve_effective_timeout_tests {
        ...
        fn foreground_positive_clamped_to_default_max() {
            ...
            // weepcode-shell opts up to 10h via max_timeout_secs. Background unaffected.
    ...
    mod versioned_background_check {
        ...
        ) -> weepcode_tool_runtime::ToolCallContext {
            let mut ctx = weepcode_tool_runtime::ToolCallContext::new(
                weepcode_tool_protocol::ToolCallId::new_v7(),
            );
            ctx.extensions.insert(resources);
            ctx.extensions
                .insert(weepcode_tool_runtime::BehaviorVersion(
