//! `image_gen` tool — generates images via the WeepCode Imagine API and saves
...
const WEEPCODE_IMAGINE_MODEL: &str = "weepcode-imagine-image-quality";
// Some Imagine models (e.g. `weepcode-imagine-image`, selectable via `model_override`)
...
/// HTTP client for WeepCode Imagine API. Cloned per-request; shares `Arc` state.
#[derive(Clone)]
pub struct ImageGenClient {
    ...
    /// [`WEEPCODE_IMAGINE_MODEL`]). `image_edit` uses its own model and is
    /// unaffected.
    model: String,
...
impl ImageGenClient {
    pub fn new(
        ...
        let model = model_override
            ...
            .unwrap_or_else(|| WEEPCODE_IMAGINE_MODEL.to_owned());
...
pub enum ImageGenConfig {
    ...
    Enabled {
        ...
        /// ([`WEEPCODE_IMAGINE_MODEL`]). Driven by the remote
...
impl ImageGenConfig {
    ...
    /// default quality model ([`WEEPCODE_IMAGINE_MODEL`]) is used.
...
impl weepcode_tool_runtime::Tool for ImageGenTool {
    type Args = ImageGenInput;
    type Output = ToolOutput;

    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new("image_gen").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(weepcode_tool_runtime::Tool::id(&tool).as_str(), "image_gen");
        assert!(
    ...
    fn per_tool_gates_are_independent() {
        let cfg = ImageGenConfig::Enabled {
            ...
            image_edit_enabled: true,
            model_override: Some("weepcode-imagine-image".into()),
        };
        assert!(cfg.has_credentials());
        assert!(!cfg.image_gen_enabled());
        assert!(cfg.image_edit_enabled());
        assert_eq!(cfg.model_override(), Some("weepcode-imagine-image"));
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("weepcode-imagine-image")), None)
                .unwrap()
                .model,
            "weepcode-imagine-image"
        );
    }
