...
use crate::implementations::weepcode_build::task::types::SubagentCancelOutcome;
use crate::types::requirements::{Expr, ToolRequirement};
use crate::types::resources::Terminal;
use crate::types::template_renderer::TemplateRenderer;
use crate::types::tool::ToolKind;
use crate::types::tool::ToolNamespace;
use weepcode_tool_types::{KillTaskOutput, KillTaskResult, KillTaskToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for KillTaskTool {
    ...
    fn description_template(&self) -> &str {
        ...
        // fallback mirrors the default weepcode-build toolset on the current OS.
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            weepcode_tool_types::build_kill_task_description(
                &weepcode_tool_types::KillTaskToolNaming {
    ...
    fn requires_expr(&self) -> Expr<ToolRequirement> {
        ...
        let task_tool = Expr::Value(ToolRequirement::Tool {
            ...
            id: weepcode_tool_runtime::Tool::id(&TaskTool).to_string(),
            if_params: None,
        });
        let mut arms =
            crate::implementations::weepcode_build::task_output::background_bash_requires_exprs();
        arms.push(task_tool);
        Expr::Or(arms)
    }
}
...
/// [`weepcode_tool_types::build_kill_task_description`] builder so the CLI and
...
fn kill_task_description(
    ...
    weepcode_tool_types::build_kill_task_description(&weepcode_tool_types::KillTaskToolNaming {
...
impl weepcode_tool_runtime::Tool for KillTaskTool {
    type Args = KillTaskToolInput;
    type Output = KillTaskOutput;

    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new("kill_task").expect("valid tool id")
    }
    ...
    ) -> Result<KillTaskOutput, weepcode_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn make_ctx_with_version(
        ...
        ctx.extensions
            .insert(weepcode_tool_runtime::BehaviorVersion(version.to_owned()));
        ctx
    }
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(weepcode_tool_runtime::Tool::id(&tool).as_str(), "kill_task");
