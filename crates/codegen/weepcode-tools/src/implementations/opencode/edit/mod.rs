...
use crate::implementations::weepcode_build::search_replace::CONTEXT_LINES;
use crate::implementations::weepcode_build::search_replace::helpers::{
...
impl weepcode_tool_runtime::Tool for EditTool {
    type Args = EditInput;
    type Output = SearchReplaceOutput;

    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new("edit").expect("valid tool id")
    }
        ...
        && !parent.as_os_str().is_empty()
    {
        tokio::fs::create_dir_all(parent).await.map_err(|e| {
            weepcode_tool_runtime::ToolError::execution(
                weepcode_tool_protocol::ToolId::new("edit").expect("valid"),
                e.to_string(),
            )
        })?;
    }
...
// are imported from `weepcode_build::search_replace::helpers` — shared across
// both the weepcode_build and opencode edit tools.
...
mod tests {
    ...
    fn tool_id_and_kind() {
        ...
        let tool = EditTool;
        assert_eq!(weepcode_tool_runtime::Tool::id(&tool).as_str(), "edit");
