...
${%- endfor %}
${%- else %}
(No skills available. Skills can be added in ~/.weepcode/skills/ or .weepcode/skills/)
${%- endif %}
</available_skills>"#;
...
impl weepcode_tool_runtime::Tool for SkillTool {
    type Args = SkillInput;
    type Output = SkillOutput;

    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new("skill").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: SkillInput,
    ) -> Result<SkillOutput, weepcode_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = SkillTool;
        assert_eq!(weepcode_tool_runtime::Tool::id(&tool).as_str(), "skill");
