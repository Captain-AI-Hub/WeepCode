//! LSP server configuration from `.weepcode/lsp.json`.
...
/// This is the canonical merge function — both session startup and `weepcode inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::weepcode_home::weepcode_home().join("lsp.json");
    let project_path = cwd.join(".weepcode").join("lsp.json");
...
/// Load LSP server configs from `~/.weepcode/lsp.json` and `<cwd>/.weepcode/lsp.json`.
