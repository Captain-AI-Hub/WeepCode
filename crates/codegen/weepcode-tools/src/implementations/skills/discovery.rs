...
/// name (e.g. `~/.weepcode/skills/shell`) is NOT dropped — only skills physically
...
/// `cwd` (exclusive). At each directory, checks for `.weepcode/skills/`,
...
/// visits known config roots (`.weepcode`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.weepcode` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".weepcode", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.weepcode is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.weepcode/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.weepcode/ → kept (user content).
        let weepcode_shell = tmp.path().join(".weepcode").join("skills").join("shell");
        std::fs::create_dir_all(&weepcode_shell).unwrap();
        std::fs::write(
            weepcode_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (weepcode_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.weepcode/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .weepcode skill in an intermediate dir.
        ...
        .unwrap();
        let weepcode_skill = sub.join(".weepcode").join("skills").join("weepcode-dyn");
        std::fs::create_dir_all(&weepcode_skill).unwrap();
        std::fs::write(
            weepcode_skill.join("SKILL.md"),
            "---\nname: weepcode-dyn\n---\n",
        )
        .unwrap();
        ...
        assert!(
            names_on.contains(&"weepcode-dyn"),
            "weepcode-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only weepcode-dyn discovered.
        ...
        assert!(
            names_off.contains(&"weepcode-dyn"),
            "weepcode-dyn missing: {names_off:?}"
        );
        assert!(
