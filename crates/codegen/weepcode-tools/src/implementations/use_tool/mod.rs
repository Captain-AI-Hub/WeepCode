...
crate::register_resource!("weepcode_build", "UseTool", UseToolParams);
...
async fn dispatch_local_mcp(
    ...
    let tool_id = weepcode_tool_protocol::ToolId::new(tool_name).map_err(|_| {
        weepcode_tool_runtime::ToolError::invalid_arguments(format!(
...
fn is_local_tool_id_rejection(err: &weepcode_tool_runtime::ToolError, tool_name: &str) -> bool {
    err.kind == weepcode_tool_runtime::ToolErrorKind::InvalidArguments
    ...
    if let Some(source) = gateway_source {
            ...
            && let Some(dispatch) = dispatch.clone()
        {
            match dispatch_local_mcp(dispatch, tool_name, tool_input.clone(), ctx.clone()).await {
                ...
                Err(err)
                    if err.kind != weepcode_tool_runtime::ToolErrorKind::NotFound
...
impl weepcode_tool_runtime::Tool for UseTool {
    type Args = UseToolInput;
    type Output = ToolOutput;

    fn id(&self) -> weepcode_tool_protocol::ToolId {
        weepcode_tool_protocol::ToolId::new("use_tool").expect("valid tool id")
    }
    ...
    fn capabilities(&self) -> weepcode_tool_protocol::ToolCapabilities {
        weepcode_tool_protocol::ToolCapabilities {
            is_read_only: false,
...
mod tests {
    ...
    impl weepcode_tool_runtime::ToolDispatch for MockToolDispatch {
        async fn call(
            &self,
            ...
            weepcode_tool_runtime::terminal_only(Ok(
    ...
    #[async_trait::async_trait]
    impl weepcode_tool_runtime::ToolDispatch for CapturingDispatch {
        async fn call(
            &self,
            tool_id: weepcode_tool_protocol::ToolId,
            ...
            if !matches!(
                ...
                return weepcode_tool_runtime::terminal_only(Err(
                    weepcode_tool_runtime::ToolError::not_found(tool_id, "Tool not found"),
                ));
            }
    ...
    fn ctx_capturing() -> (weepcode_tool_runtime::ToolCallContext, SharedArgs) {
    ...
    #[async_trait::async_trait]
    impl weepcode_tool_runtime::ToolDispatch for NotFoundDispatch {
        async fn call(
            &self,
            tool_id: weepcode_tool_protocol::ToolId,
            _args: serde_json::Value,
            ...
            weepcode_tool_runtime::terminal_only(Err(weepcode_tool_runtime::ToolError::not_found(
                tool_id,
                "Tool not found",
            )))
        }
    ...
    #[async_trait::async_trait]
    impl weepcode_tool_runtime::ToolDispatch for InvalidArgumentsDispatch {
        async fn call(
            &self,
            _tool_id: weepcode_tool_protocol::ToolId,
            _args: serde_json::Value,
            _ctx: weepcode_tool_runtime::ToolCallContext,
        ) -> weepcode_tool_runtime::ToolStream<weepcode_tool_runtime::TypedToolOutput> {
            weepcode_tool_runtime::terminal_only(Err(
                weepcode_tool_runtime::ToolError::invalid_arguments("local validation failed"),
            ))
        }
    }
    ...
    #[async_trait::async_trait]
    impl weepcode_tool_runtime::ToolDispatch for ErrorToolDispatch {
        async fn call(
            &self,
            _tool_id: weepcode_tool_protocol::ToolId,
            _args: serde_json::Value,
            ...
            let tid = weepcode_tool_protocol::ToolId::new(&self.error)
    ...
    fn new_ctx() -> weepcode_tool_runtime::ToolCallContext {
        let call_id = weepcode_tool_protocol::ToolCallId::new_v7();
        weepcode_tool_runtime::ToolCallContext::new(call_id)
    }

    fn ctx_with_dispatch(
        dispatch: impl weepcode_tool_runtime::ToolDispatch + 'static,
    ...
    async fn rejects_builtin_tool_names() {
        ...
        assert_eq!(
            err.kind,
            weepcode_tool_runtime::ToolErrorKind::InvalidArguments
        );
    ...
    fn ctx_capturing_with_resources(
        resources: crate::types::resources::SharedResources,
    ) -> (weepcode_tool_runtime::ToolCallContext, SharedArgs) {
        let args: SharedArgs = Arc::new(std::sync::Mutex::new(None));
        let mut ctx = new_ctx();
        ctx.extensions
