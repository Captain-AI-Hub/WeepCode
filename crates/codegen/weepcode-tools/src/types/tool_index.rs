...
//!
//! `ToolSearchIndex` is defined in `weepcode-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `weepcode-shell`
...
/// in `weepcode-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `weepcode-shell` after MCP initialization.
#[derive(Clone)]
