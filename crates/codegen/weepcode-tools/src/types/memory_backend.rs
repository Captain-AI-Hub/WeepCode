...
//!
//! `MemoryBackend` is defined in `weepcode-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `weepcode-tools`.
/// Mirrors `weepcode_shell::session::memory_log::TARGET`.
pub const MEMORY_LOG_TARGET: &str = "weepcode_memory";
