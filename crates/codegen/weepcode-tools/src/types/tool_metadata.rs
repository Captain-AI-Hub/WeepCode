//! `ToolMetadata` — weepcode-tools-specific metadata for tools.
//!
//! Each tool implements two traits:
//! 1. `weepcode_tool_runtime::Tool` — typed Args/Output, `run()` with actual logic
...
//! ## Context helpers
//!
//! Tools access session state through `weepcode_tool_runtime::ToolCallContext`
...
/// WeepCode-tools-specific metadata trait.
///
/// Each tool struct implements this alongside `weepcode_tool_runtime::Tool`.
...
pub trait ToolMetadata: Send + Sync {
    ...
    /// Namespace grouping (WeepcodeBuild, Cursor, OpenCode, ...).
    /// Used to build the fully-qualified tool ID at registration time
    /// (e.g., `"WeepcodeBuild:grep"`).
...
) -> Result<SharedResources, weepcode_tool_runtime::ToolError> {
    ctx.extensions
        .get::<SharedResources>()
        .map(|arc| (*arc).clone())
        .ok_or_else(|| {
...
) -> Result<PathBuf, weepcode_tool_runtime::ToolError> {
    if let Some(cwd) = ctx.extensions.get::<weepcode_tool_runtime::Cwd>() {
...
pub fn test_ctx(resources: SharedResources) -> weepcode_tool_runtime::ToolCallContext {
    ...
    ctx.extensions
        .insert(weepcode_tool_runtime::WorkspaceViewerContext {
...
pub fn test_ctx_with_call_id(
    ...
    let id = weepcode_tool_protocol::ToolCallId::new(call_id)
        .unwrap_or_else(|_| weepcode_tool_protocol::ToolCallId::new_v7());
    let mut ctx = weepcode_tool_runtime::ToolCallContext::new(id);
    ctx.extensions.insert(resources);
    ctx.extensions
        .insert(weepcode_tool_runtime::WorkspaceViewerContext {
            stream_tool_progress: true,
...
pub fn behavior_version(ctx: &weepcode_tool_runtime::ToolCallContext) -> Option<String> {
    ctx.extensions
