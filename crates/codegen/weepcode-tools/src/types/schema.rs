...
///
/// Use with `#[schemars(with = "WeepcodeIntegerSchema")]` on `Option<usize>` fields.
pub struct WeepcodeIntegerSchema;
impl schemars::JsonSchema for WeepcodeIntegerSchema {
    fn schema_name() -> std::borrow::Cow<'static, str> {
        "weepcode_integer_schema".into()
    }
...
/// Lenient boolean deserializers (shared via `weepcode-tool-types`), re-exported so
/// fields reference them under the same `crate::types::schema::` path as above.
pub use weepcode_tool_types::{deserialize_lenient_bool, deserialize_lenient_option_bool};
#[cfg(test)]
mod tests {
    use super::*;
