...
//!
//! The tool runtime contract (`Tool` trait) lives in `weepcode_tool_runtime`.
...
///
/// Serializes to snake_case (`weepcode_build`, `mcp`, …) for the
...
/// (e.g. `"WeepcodeBuild:read_file"`); only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum ToolNamespace {
    #[serde(alias = "WeepcodeBuild")]
    WeepcodeBuild,
    #[serde(alias = "WeepcodeBuildConcise")]
    WeepcodeBuildConcise,
    #[serde(alias = "WeepcodeBuildHashline")]
    WeepcodeBuildHashline,
    #[serde(alias = "Codex")]
    Codex,
