...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.weepcode/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.weepcode/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.weepcode/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.weepcode/ or ~/.claude/.
    User { path: PathBuf },
