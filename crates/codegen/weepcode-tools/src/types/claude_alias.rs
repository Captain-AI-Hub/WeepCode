//! Canonical external-settings tool name ↔ WeepCode tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`weepcode-hooks`) needs the
//! WeepCode tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`weepcode-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `weepcode` names are test-checked against the live registry.

use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to WeepCode, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// WeepCode [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// WeepCode tool names this Claude tool maps to (empty when there is no direct
    /// WeepCode tool — the entry then only contributes a `kind`).
    weepcode: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, weepcode: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        weepcode,
...
const fn match_only(claude: &'static str, weepcode: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        weepcode,
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (weepcode-build reads SKILL.md)
...
/// The WeepCode [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The WeepCode tool names a Claude matcher term fires on.
pub fn weepcode_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.weepcode)
...
/// The Claude names that map to `weepcode_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(weepcode_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.weepcode.contains(&weepcode_name))
        .map(|t| t.claude)
}

/// Every distinct WeepCode name the table references, for the `weepcode-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn weepcode_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.weepcode.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a WeepCode name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.weepcode.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
