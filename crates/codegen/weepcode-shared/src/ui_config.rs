...
use weepcode_config_types::DisplayRefreshSettings;
...
pub struct UiConfig {
    ...
    /// connect time (CLI `--hunk-tracker-mode` / `WEEPCODE_HUNK_TRACKER` override
...
pub struct ContextualHints {
    ...
    /// SSH wrap session-load tip (recommend `weepcode wrap ssh` when the session
...
impl Default for UiConfig {
    fn default() -> Self {
        Self {
            ...
            theme: None,
            fork_secondary_model: weepcode_models::default_model().to_string(),
            yolo: false,
            ui_theme: None,
            compact_mode: false,
            simple_mode: None,
