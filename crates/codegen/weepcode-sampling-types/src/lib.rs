//! Pure data types for the WeepCode sampling / chat-completion API layer.
//!
...
//! WeepCode agent stack.  It intentionally contains **no I/O** (no HTTP clients,
//! no file system access) so it can be depended on by downstream crates
//! (e.g., `weepcode-chat-state`) without pulling in the full `weepcode-shell`.
