...
// Re-export ToolDefinition and FunctionTool from weepcode-tools.
...
pub use weepcode_tools::types::definition::{FunctionTool, ToolDefinition};
...
pub struct Usage {
    ...
    /// WeepCode extension: request price in USD ticks (1 USD = 1e10 ticks).
...
/// custom header fields for WeepCode request tracking, similar to
...
pub struct CreateResponseWrapper {
    ...
    /// WeepCode-specific tool definitions that can't be expressed via
...
/// Wrapper around `MessagesRequest` that adds custom header fields for WeepCode
    ...
    fn parse_reasoning_efforts_meta_absent_is_none() {
        ...
        assert!(
            parse_reasoning_efforts_meta(Some(
                json!({ "agentType": "weepcode" }).as_object().unwrap()
            ))
            .is_none()
        );
    }
