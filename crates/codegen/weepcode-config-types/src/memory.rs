//! Memory-system configuration value types, extracted from weepcode-shell
...
//! `weepcode-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.weepcode/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.weepcode/memory/` are scanned:
