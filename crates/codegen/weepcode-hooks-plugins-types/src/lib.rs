...
/// Plugin scope.
///
/// Maps from `PluginScope` in `weepcode-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `weepcode-agent`. Optional on [`PluginInfo`]
...
pub enum PluginOrigin {
    ...
    /// WeepCode's install registry (marketplace or direct git/local install).
    MarketplaceInstall {
...
/// Hook event type.
///
/// Maps from `HookEventName` in `weepcode-hooks`. The source type's
...
pub struct HookInfo {
    ...
    /// Whether this hook is disabled via ~/.weepcode/disabled-hooks.
    #[serde(default)]
    pub disabled: bool,
}
...
mod tests {
    ...
    fn hooks_action_serde_roundtrip() {
        let action = HooksAction::Add {
            path: "/home/user/.weepcode/hooks".into(),
        };
    ...
    fn hook_info_camel_case_fields() {
        let hook = HookInfo {
            ...
            url: None,
            timeout_ms: 5000,
            source_dir: "/home/user/.weepcode/hooks".into(),
            disabled: false,
        };
    ...
    fn plugin_info_camel_case_fields() {
        let plugin = PluginInfo {
            ...
            root: "/home/user/.weepcode/plugins/test-plugin".into(),
            ...
            origin: Some(PluginOrigin::UserWeepcode),
            conflict: None,
        };
