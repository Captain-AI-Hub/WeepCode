//! Installed weepcode CLI version, lockstepped with shipping binaries.

use semver::Version;

pub const TEST_VERSION_ENV: &str = "WEEPCODE_TEST_VERSION";

pub const VERSION: &str = match option_env!("WEEPCODE_VERSION") {
    Some(v) => v,
    None => env!("CARGO_PKG_VERSION"),
};

/// Release-channel label suffix for user-facing version display.
///
/// WeepCode ships no release channel (no xAI update service), so this is the
/// empty string — `display_version`/`display_version_with_commit` render the
/// bare version, e.g. `"0.2.102"` / `"0.2.102 (abc1234)"`.
pub const CHANNEL_LABEL: &str = "";

/// Release-channel name for machine-readable version payloads (`None` ⇒
/// `"unknown"` at call sites). WeepCode has no channel.
pub const CHANNEL_NAME: Option<&str> = None;
...
/// or `""` (empty when no channel applies). In WeepCode pass [`CHANNEL_LABEL`]
/// (always `""`).
///
