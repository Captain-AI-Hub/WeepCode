...
use weepcode_sqlite_journal::JournalMode;
...
impl WorktreeDb {
    /// Open (or create) the DB at `weepcode_home/worktrees.db`.
    pub fn open(weepcode_home: &Path) -> Result<Self> {
        Self::open_at(&weepcode_home.join("worktrees.db"))
    }
    ...
    /// Open the default DB at `~/.weepcode/worktrees.db`.
    ///
    /// Discovers weepcode home via `$WEEPCODE_HOME`, falling back to the canonicalized
    /// `$HOME/.weepcode` (matching `weepcode_config::weepcode_home`).
    ...
    pub fn open_default() -> Result<Self> {
        Self::open(&resolve_weepcode_home()?)
    }
...
pub fn resolve_weepcode_home() -> Result<PathBuf> {
    if let Ok(v) = std::env::var("WEEPCODE_HOME") {
        return Ok(PathBuf::from(v));
    }
    let home =
        PathBuf::from(std::env::var("HOME").context("neither $WEEPCODE_HOME nor $HOME is set")?);
    // Canonicalize the home dir so worktree paths share the same physical .weepcode
    // tree as trust/hooks even when it is symlinked. The dunce canonicalization
    // must stay in sync with weepcode_config::default_weepcode_home();
    // home resolution deliberately differs ($HOME here vs std::env::home_dir()).
    Ok(dunce::canonicalize(&home).unwrap_or(home).join(".weepcode"))
}

/// Serializes tests that mutate the process-global `WEEPCODE_HOME` env var so they
...
#[cfg(test)]
static WEEPCODE_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

/// Test-only isolation for code that resolves the DB via `open_default()`.
///
/// Holds [`WEEPCODE_HOME_ENV_LOCK`] (serializing concurrent setters), points
/// `WEEPCODE_HOME` at a fresh private tmp dir, and restores the prior value on drop.
/// Use instead of hand-rolling the lock + restore guard + tmp dir per test.
///
/// `Drop` restores `WEEPCODE_HOME` before `_lock` releases, so the env is correct
/// before another waiting setter proceeds.
#[cfg(test)]
pub(crate) struct WeepcodeHomeFixture {
    _lock: std::sync::MutexGuard<'static, ()>,
    prev: Option<std::ffi::OsString>,
    /// The isolated weepcode home; pass to `WorktreeDb::open` to read the same DB
...
#[cfg(test)]
impl WeepcodeHomeFixture {
    pub(crate) fn new() -> Self {
        let lock = WEEPCODE_HOME_ENV_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner());
        let tmp = tempfile::TempDir::new().unwrap();
        let home = tmp.path().join("weepcode-home");
        std::fs::create_dir_all(&home).unwrap();
        // Warm up the DB (journal-mode conversion + schema) before exposing it
        // via WEEPCODE_HOME, sparing the test hot loop set_journal_mode's retry
        // sleeps. This open has exclusive access (nothing reaches the path
        // until WEEPCODE_HOME points here); set_journal_mode's retry is the actual
        // race fix.
        let _ = WorktreeDb::open(&home);
        let prev = std::env::var_os("WEEPCODE_HOME");
        unsafe { std::env::set_var("WEEPCODE_HOME", &home) };
        Self {
            _lock: lock,
            prev,
            home,
...
#[cfg(test)]
impl Drop for WeepcodeHomeFixture {
    fn drop(&mut self) {
        unsafe {
            match self.prev.take() {
                Some(p) => std::env::set_var("WEEPCODE_HOME", p),
                None => std::env::remove_var("WEEPCODE_HOME"),
            }
        }
    }
}
