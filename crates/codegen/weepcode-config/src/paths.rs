//! Filesystem locations for weepcode config files and binaries.

use std::path::PathBuf;
use std::sync::OnceLock;

static WEEPCODE_HOME: OnceLock<PathBuf> = OnceLock::new();
...
/// The default user weepcode directory (`~/.weepcode`, canonicalized) used when
/// `WEEPCODE_HOME` is unset. Exposed so callers (e.g. display helpers) can detect
/// whether [`weepcode_home()`] is the default without duplicating the computation.
///
...
/// `~/.weepcode/marketplace-cache`. `dunce` strips the prefix whenever the path
...
/// `weepcode_fast_worktree::db::resolve_weepcode_home` (deliberately standalone crate).
pub fn default_weepcode_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".weepcode")
}

/// Per-user config directory: `$WEEPCODE_HOME` or `~/.weepcode`. Created if needed.
pub fn weepcode_home() -> PathBuf {
    WEEPCODE_HOME
        .get_or_init(|| {
            let weepcode_home = if let Ok(v) = std::env::var("WEEPCODE_HOME") {
                PathBuf::from(v)
            } else {
                default_weepcode_home()
            };
...
/// The user-global weepcode home, but only when one genuinely resolves: `Some` when
/// `$WEEPCODE_HOME` is set or a home directory is found, `None` otherwise. Unlike
/// [`weepcode_home()`], this never falls back to a cwd-relative `.weepcode`, so callers
/// that *scan* user-global weepcode resources (hooks, marketplace sources, ...) don't
/// mistake a project's `.weepcode` tree for the user-global one when no home resolves.
pub fn user_weepcode_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("WEEPCODE_HOME").is_some() || std::env::home_dir().is_some();
    resolvable.then(weepcode_home)
}

/// Canonical weepcode application path: `$WEEPCODE_HOME/bin/weepcode` (Unix) or `weepcode.exe` (Windows).
pub fn weepcode_application() -> PathBuf {
    weepcode_application_in(&weepcode_home())
}

/// [`weepcode_application`] under an explicit home instead of `$WEEPCODE_HOME`.
pub fn weepcode_application_in(home: &std::path::Path) -> PathBuf {
...
/// System-wide config directory: `/etc/weepcode/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/weepcode"))
    } else {
        None
    }
}
...
/// `weepcode_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = weepcode_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_weepcode_home_has_no_verbatim_prefix() {
        ...
        let home = default_weepcode_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".weepcode"));
    }
