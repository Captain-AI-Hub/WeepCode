//! Persist API-provider profiles (`[model.<slug>]` tables) in `~/.grok/config.toml`.
//!
...
impl ProviderProfileInput {
    ...
    pub fn validated(self) -> Result<Self, String> {
        ...
        match url::Url::parse(&trimmed.base_url) {
            Ok(u) if matches!(u.scheme(), "http" | "https") && !u.host_str().unwrap_or("").is_empty() => {}
...
fn provider_profile_table(profile: &ProviderProfileInput) -> TomlValue {
    ...
    table.insert("model".to_string(), TomlValue::String(profile.model_id.clone()));
    table.insert("name".to_string(), TomlValue::String(profile.display_name.clone()));
    table.insert("base_url".to_string(), TomlValue::String(profile.base_url.clone()));
    table.insert("api_key".to_string(), TomlValue::String(profile.api_key.clone()));
    table.insert(
...
/// Upsert `profile` as `[model.<slug>]` in `~/.grok/config.toml` and point
...
/// kept separate so tests never touch the real `~/.grok`.
pub fn upsert_provider_profile_at(
    ...
    let mut root: TomlValue = match std::fs::read_to_string(path) {
        Ok(contents) => toml::from_str(&contents)
            .with_context(|| format!("refusing to overwrite unparseable config {}", path.display()))?,
...
mod tests {
    ...
    fn wire_str_round_trip() {
        ...
        assert_eq!(ProviderApiFormat::from_wire_str("grok"), None);
    }
