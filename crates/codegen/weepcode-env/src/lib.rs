...
//! Backend environment presets for the WeepCode CLI crate family: endpoint URL
//! defaults, environment selection, and env-var test support.
//!
//! Public builds expose production endpoints. Values resolve as a `WEEPCODE_*`
//! env-var override when set, else the compiled production default.
/// The endpoint set for one backend environment.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct WeepcodeBuildEndpoints {
...
const PRODUCTION_ENDPOINTS: WeepcodeBuildEndpoints = WeepcodeBuildEndpoints {
...
pub enum WeepcodeBuildEnvironment {
    #[default]
    Production,
}
impl WeepcodeBuildEnvironment {
    pub fn from_flags(_dev: bool, _staging: bool) -> Self {
        WeepcodeBuildEnvironment::Production
    }
    /// Indicator string for display; `None` for Production.
    pub fn indicator(&self) -> Option<&'static str> {
        match self {
            WeepcodeBuildEnvironment::Production => None,
        }
    }
    pub fn is_production(&self) -> bool {
        matches!(self, WeepcodeBuildEnvironment::Production)
    }
    fn env_prefix(&self) -> &'static str {
        match self {
            WeepcodeBuildEnvironment::Production => "WEEPCODE_PRODUCTION",
        }
    }
    /// Compiled endpoint set for this environment (production by default).
    pub fn endpoints(&self) -> WeepcodeBuildEndpoints {
        match self {
            WeepcodeBuildEnvironment::Production => PRODUCTION_ENDPOINTS,
        }
    }
    ...
    /// `WEEPCODE_GATEWAY_URL` opt-in takes precedence.
    pub fn gateway_ws_url(&self) -> String {
        self.resolve("_GATEWAY_WS_URL", self.endpoints().gateway_ws_url)
    }
}
impl std::fmt::Display for WeepcodeBuildEnvironment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            WeepcodeBuildEnvironment::Production => write!(f, "production"),
        }
    }
}
...
mod tests {
    ...
    fn test_env_prefix() {
        assert_eq!(
            WeepcodeBuildEnvironment::Production.env_prefix(),
            "WEEPCODE_PRODUCTION"
        );
    }
    #[test]
    fn env_var_guard_set_value_updates_then_restores_on_drop() {
        const KEY: &str = "WEEPCODE_WEEPCODE_ENV_VAR_GUARD_SET_VALUE_PROBE";
    ...
    fn relay_and_gateway_urls_are_distinct() {
        assert_ne!(
            WeepcodeBuildEnvironment::Production.relay_ws_url(),
            WeepcodeBuildEnvironment::Production.gateway_ws_url(),
        );
    }
    #[test]
    fn test_from_flags() {
        assert_eq!(
            WeepcodeBuildEnvironment::from_flags(false, false),
            WeepcodeBuildEnvironment::Production
        );
    }
}
