...
fn external_stream_end_to_end() {
    ...
    let mut cfg = weepcode_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
    ...
    cfg.client = weepcode_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    weepcode_telemetry::external::init(Some(cfg));
    assert!(weepcode_telemetry::external::is_active());
    ...
    weepcode_telemetry::log_event(weepcode_telemetry::events::SessionNew {
    ...
    assert!(
        resource_service_name
            .as_deref()
            .is_some_and(|s| s.contains("weepcode-cli")),
        "service.name=weepcode-cli is a wire commitment: {resource_service_name:?}"
    );
    for expected in [
        "weepcode_code.session_start",
        "weepcode_code.user_prompt",
        "weepcode_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "weepcode_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "weepcode_code.session.count" {
    ...
    assert!(
        metric_names
            .iter()
            .any(|n| n == "weepcode_code.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(
        metric_names
            .iter()
            .any(|n| n == "weepcode_code.token.usage")
    );
    assert_eq!(
    ...
    weepcode_telemetry::external::shutdown();
    assert!(
