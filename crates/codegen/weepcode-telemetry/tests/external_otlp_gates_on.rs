...
use weepcode_telemetry::external::{self, ExternalOtelRemotePolicy, IdentityAttrs};
...
const SECRET_MODEL: &str = "weepcode-4-sk-LEAKmodel1234567890abcd";
...
fn external_stream_gates_on_end_to_end() {
    ...
    let mut cfg = external::ExternalOtelConfig::resolve_with(
        |name| match name {
            "WEEPCODE_EXTERNAL_OTEL" => Some("1".into()),
    ...
    assert!(!weepcode_telemetry::is_enabled());

    weepcode_telemetry::log_event(weepcode_telemetry::events::SessionHarness {
        ...
        permission_mode: weepcode_telemetry::enums::PermissionMode::Ask,
    ...
    weepcode_telemetry::log_event(weepcode_telemetry::events::PromptSubmitted {
        prompt_length: 100,
        model_id: "weepcode-4".into(),
    ...
    weepcode_telemetry::log_event(weepcode_telemetry::events::ModelResponseReceived {
    ...
    weepcode_telemetry::log_event(weepcode_telemetry::events::ToolCallCompleted {
    ...
    let harness = col::find_event(&collected, "weepcode_code.session_start")
        .expect("session_start must be present");
    assert_eq!(harness.scope_name, "weepcode.code");
    assert_eq!(
        harness
            .resource
            .get("service.name")
            .and_then(|v| v.as_str()),
        Some("weepcode-cli"),
        "service.name=weepcode-cli is a wire commitment"
    );
    assert_eq!(
        harness
            .resource
            .get("weepcode_code.schema.version")
            .and_then(|v| v.as_str()),
        Some("v1")
    );
    ...
    let prompt =
        col::find_event(&collected, "weepcode_code.user_prompt").expect("user_prompt present");
    ...
    let tool =
        col::find_event(&collected, "weepcode_code.tool_result").expect("tool_result present");
    assert_eq!(
    ...
    let tokens = col::find_metric(&collected, "weepcode_code.token.usage");
    ...
    let sessions = col::find_metric(&collected, "weepcode_code.session.count");
