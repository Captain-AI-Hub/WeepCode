...
//!
//! Extracted from `weepcode-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `weepcode-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `weepcode-workspace` (remote sampler / workspace server).
    Workspace,
}
...
impl EmitterOrigin {
    ...
    pub fn event_prefix(self) -> &'static str {
        match self {
            EmitterOrigin::Shell => "weepcode-shell-",
            EmitterOrigin::Workspace => "weepcode-workspace-",
        }
    }
}
...
/// event-name prefix (`weepcode-shell-*` vs `weepcode-workspace-*`).
///
...
mod tests {
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "weepcode-shell-");
        assert_eq!(
    ...
    /// `format!("weepcode-shell-{suffix}")` event name byte-for-byte, since every
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("weepcode-shell-{suffix}");
    ...
    fn workspace_origin_event_name_uses_workspace_prefix() {
        ...
        assert_eq!(name, "weepcode-workspace-turn");
    }
