...
//!
//! `weepcode-pager` uses this module to set up OTLP
...
const ENV_OTEL_FILTER: &str = "WEEPCODE_OTEL_FILTER";
...
/// (`AuthManager`, `EndpointsConfig`, `WeepcodeComConfig`).
///
/// Built by the binaries (`weepcode-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `weepcode_shell::auth::credential_provider::build_otel_credential_provider`.
pub struct OtelLayerConfig {
    ...
    /// Value for the `X-WeepCode-Token-Auth` header (typically `"weepcode-cli"`).
...
pub struct OtelClientInfo {
    /// Binary name (`weepcode-pager`) -> `client.name`.
...
///
/// - `client_name`: binary name (e.g. `"weepcode-tui"`, `"weepcode-pager"`) -- stored as
///   `client.name` resource attribute for dashboards to distinguish client types.
/// - `client_version`: `CARGO_PKG_VERSION` -- sent in the `x-weepcode-client-version` header.
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let tracer = provider.tracer("weepcode-cli");
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid WEEPCODE_OTEL_FILTER '{}': {}. Using default '{}'.",
...
struct RefreshableSpanExporter {
    ...
    /// Value for `X-WeepCode-Token-Auth`. Only sent when `credentials.needs_token_auth_header()`.
...
fn build_export_headers(
    ...
    snapshot: &weepcode_auth::CredentialSnapshot,
    ...
    if let Some(value) = token_auth_header {
        headers.insert("X-WeepCode-Token-Auth".to_string(), value.to_string());
    }
...
fn build_server_provider(client: OtelClientInfo, config: OtelLayerConfig) -> SdkTracerProvider {
    ...
    let mut provider = SdkTracerProvider::builder().with_resource(
        opentelemetry_sdk::Resource::builder_empty()
            .with_service_name("weepcode-cli")
    ...
    if config.exporter.enabled {
        ...
        static_headers.insert(
            "x-weepcode-client-version".to_string(),
...
mod tests {
    ...
    use std::sync::Mutex;
    use weepcode_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
    ...
    fn make_exporter(
        ...
        RefreshableSpanExporter {
            ...
            token_header_value: Arc::from("weepcode-cli"),
