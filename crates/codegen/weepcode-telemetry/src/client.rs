...
//!
//! Extracted from `weepcode-shell::agent::telemetry::track`. The HTTP client is
...
/// whichever [`EmitterOrigin`] prefix it carries (`weepcode-shell-` /
/// `weepcode-workspace-`). Unprefixed names pass through unchanged. Kept in
...
mod tests {
    ...
    /// identical to the previous `strip_prefix("weepcode-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("weepcode-shell-turn"), "turn");
        assert_eq!(
            event_value("weepcode-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_workspace_prefix() {
        assert_eq!(event_value("weepcode-workspace-turn"), "turn");
    }
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("weepcode-shell-workspace-x"), "workspace-x");
    }
    ...
    /// (e.g. a future `"weepcode-shell-ext-"` origin), an earlier `ALL` entry could
