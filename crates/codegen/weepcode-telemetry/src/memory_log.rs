...
//!
//! Provides a dedicated tracing target (`weepcode_memory`) with an optional
//! file logger that writes to `~/.weepcode/logs/memory.log`.
//!
//! ## When to use
//!
...
//! WEEPCODE_MEMORY_LOG=0 weepcode                # disable even when enabled
//! tail -f ~/.weepcode/logs/memory.log      # watch in another terminal
//! ```

/// Tracing target for all memory system operations.
pub const TARGET: &str = "weepcode_memory";
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "WEEPCODE_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.weepcode/logs/memory.log`. Filters to `weepcode_memory=trace`.
    /// Set `WEEPCODE_MEMORY_LOG=0` to disable, `WEEPCODE_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || weepcode_home().join("logs").join("memory.log");
