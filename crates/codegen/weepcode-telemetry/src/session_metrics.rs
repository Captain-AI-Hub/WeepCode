...
impl TraceUploadReason {
    ...
    pub fn from_upload_method(method: &Option<weepcode_file_utils::UploadMethod>) -> Self {
        match method {
            Some(weepcode_file_utils::UploadMethod::Proxy { .. }) => Self::Proxy,
            Some(weepcode_file_utils::UploadMethod::S3 { .. }) => Self::DirectS3,
            Some(weepcode_file_utils::UploadMethod::Direct { .. }) => Self::DirectGcs,
...
#[cfg(test)]
mod tests {
    use weepcode_file_utils::UploadMethod;

    use super::TraceUploadReason;

    /// The `weepcode-shell-doom_loop_recovery` Mixpanel event's name and
