...
//! `Authorization`/`X-WeepCode-Token-Auth`/`x-userid`;
...
fn build_resource(cfg: &ExternalOtelConfig) -> opentelemetry_sdk::Resource {
    ...
    opentelemetry_sdk::Resource::builder_empty()
        // RQ6 (final): `weepcode-cli`, a wire commitment.
...
mod tests {
    ...
    /// `X-WeepCode-Token-Auth`, `x-userid`, or `x-teamid` unless customer-supplied
    ...
    fn exporter_metadata_is_customer_headers_only() {
        ...
        for forbidden in ["x-weepcode-token-auth", "x-userid", "x-teamid"] {
