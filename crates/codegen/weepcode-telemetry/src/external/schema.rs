...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "weepcode_code.session_start",
            Self::SessionEnd => "weepcode_code.session_end",
            Self::UserPrompt => "weepcode_code.user_prompt",
            Self::TurnCompleted => "weepcode_code.turn_completed",
            Self::ApiRequest => "weepcode_code.api_request",
            Self::ApiError => "weepcode_code.api_error",
            Self::ToolResult => "weepcode_code.tool_result",
            Self::ToolDecision => "weepcode_code.tool_decision",
            Self::McpServerConnection => "weepcode_code.mcp_server_connection",
            Self::PermissionModeChanged => "weepcode_code.permission_mode_changed",
            Self::SkillActivated => "weepcode_code.skill_activated",
            Self::PluginLoaded => "weepcode_code.plugin_loaded",
            Self::Compaction => "weepcode_code.compaction",
            Self::Subagent => "weepcode_code.subagent",
            Self::Auth => "weepcode_code.auth",
            Self::InternalError => "weepcode_code.internal_error",
            Self::ModelSwitched => "weepcode_code.model_switched",
            Self::ContextualTip => "weepcode_code.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    /// `weepcode_code.session.count`.
    SessionCount,
    /// `weepcode_code.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
        token_type: &'static str,
        model: String,
        count: u64,
    },
    /// `weepcode_code.turn.count`.
    TurnCount {
        outcome: &'static str,
        model: String,
    },
    /// `weepcode_code.tool.decision`.
    ToolDecision {
        tool_name: String,
        decision: &'static str,
    ...
    /// `weepcode_code.tool.usage`.
    ToolUsage {
        tool_name: String,
        outcome: &'static str,
    },
    /// `weepcode_code.error.count`.
    ErrorCount {
...
pub(crate) const METRIC_SESSION_COUNT: &str = "weepcode_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "weepcode_code.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "weepcode_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "weepcode_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "weepcode_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "weepcode_code.error.count";
...
/// Built-in tool names (closed enum surface in `weepcode-tools`) that pass
...
fn tool_outcome_label(o: &weepcode_file_utils::events::types::ToolOutcome) -> &'static str {
    use weepcode_file_utils::events::types::ToolOutcome;
    match o {
...
/// `SessionHarness` → `weepcode_code.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `weepcode_code.session.count` (metric only; the flagship
...
/// `SessionEnded` → `weepcode_code.session_end`.
...
/// `PromptSubmitted` → `weepcode_code.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `weepcode_code.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `weepcode_code.api_request` + `token.usage`.
...
/// `RateLimitHit` → `weepcode_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `weepcode_code.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `weepcode_code.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `weepcode_code.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `weepcode_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `weepcode_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `weepcode_code.permission_mode_changed`.
...
/// `ContextualTip` → `weepcode_code.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `weepcode_code.permission_mode_changed`.
...
/// `SkillDispatched` → `weepcode_code.skill_activated`. Skill names are
...
/// `PluginInstalled` → `weepcode_code.plugin_loaded`.
...
/// `PluginUsed` → `weepcode_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `weepcode_code.compaction`.
...
/// `CompactionTriggered` → `weepcode_code.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `weepcode_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `weepcode_code.subagent` (`phase=completed`).
...
/// `InternalError` → `weepcode_code.internal_error`. Error class only — no
...
/// `ModelSwitched` → `weepcode_code.model_switched`.
