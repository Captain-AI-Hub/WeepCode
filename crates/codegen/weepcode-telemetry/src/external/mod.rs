...
//!
//! Enterprise customers point the WeepCode CLI at *their own* OpenTelemetry
//! collector (standard `OTEL_*` env vars + the `WEEPCODE_EXTERNAL_OTEL` master
...
//!   headers.
//! - Default **off**: with `WEEPCODE_EXTERNAL_OTEL` unset (or no exporter
...
//!   data-retention opt-outs (user-confirmed): those govern WeepCode-side
...
impl IdentityAttrs {
    pub fn from_snapshot(snapshot: &weepcode_auth::CredentialSnapshot) -> Self {
        Self {
    ...
    if cfg.internal_pipeline_consumed_otel_vars {
        tracing::warn!(
            "external otel: refusing to activate — the internal trace pipeline consumed \
             ...
             WEEPCODE_INTERNAL_OTLP_* to use the external stream."
        );
        return None;
    }
