...
//!
//! Extracted from `weepcode-shell::agent::unique_identifier` so the
//! telemetry engine can stamp events without depending on shell internals.
//! `$WEEPCODE_HOME` is resolved through `weepcode-config::weepcode_home`.
...
/// This function caches the result in `$WEEPCODE_HOME/agent_id` so subsequent calls
...
fn load_or_compute_agent_id() -> String {
    let cache_path = weepcode_config::weepcode_home().join("agent_id");
...
/// Write `$WEEPCODE_HOME/agent_id` as owner-read/write only (Unix 0o600) — it is a
...
fn write_agent_id_cache(path: &std::path::Path, id: &str) -> std::io::Result<()> {
    ...
    weepcode_config::fs_atomic::write_atomically(path, id, Some(0o600))
}
...
/// Returns true when workspace marker env vars (`WEEPCODE_ROOT` and `WEEPCODE_USER`) are set.
///
...
pub fn has_workspace_env_markers() -> bool {
    std::env::var("WEEPCODE_ROOT").is_ok() && std::env::var("WEEPCODE_USER").is_ok()
}

/// Opt-in special-user gate for telemetry.
///
/// Enabled only when `WEEPCODE_TELEMETRY_SPECIAL_USER=1` (or `true`). There is no
/// hardcoded username allowlist.
pub fn is_special_user() -> bool {
    matches!(
        std::env::var("WEEPCODE_TELEMETRY_SPECIAL_USER").as_deref(),
