...
//!
//! These structs were extracted from `weepcode-shell` so they can be
...
pub enum ContextualTipKind {
    ...
    WordSelect,
    /// SSH session without `weepcode wrap` → tip to wrap the ssh command locally.
    SshWrap,
}
...
impl CompactionScope {
    pub fn begin(
        ...
        let percentage =
            weepcode_token_estimation::usage_percentage_u8(tokens_used, context_window);
...
pub struct ToolCallCompleted {
    pub tool_name: String,
    pub outcome: weepcode_file_utils::events::types::ToolOutcome,
    pub duration_ms: u64,
...
// ---------------------------------------------------------------------------
// Pager events (called from weepcode-pager via log_event)
// ---------------------------------------------------------------------------
...
/// first prompt when WeepCode Build is launched from a non-project directory).
...
/// Lets the funnel attribute the click to the welcome hero vs the in-session
/// header vs the banner vs the dashboard, and distinguish keyboard (`Ctrl+O`)
/// activations from pointer/OSC 8 ones.
...
pub struct ClipboardCopy {
    ...
    /// An explicit `weepcode wrap` OSC 52 sink was active.
    pub osc52_sink: bool,
...
/// `weepcode-shell-manual_auth` (the `manual_auth` binding gets the `weepcode-shell-`
