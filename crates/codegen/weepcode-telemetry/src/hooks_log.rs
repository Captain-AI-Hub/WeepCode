...
//! file logger that writes to `~/.weepcode/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `weepcode_hooks` or `weepcode_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! WEEPCODE_HOOKS_LOG=1 weepcode              # enable, write to ~/.weepcode/logs/hooks.log
//! WEEPCODE_HOOKS_LOG=/tmp/h.log weepcode     # write to custom path
//! WEEPCODE_HOOKS_LOG=0 weepcode              # explicitly disable
//! tail -f ~/.weepcode/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "WEEPCODE_HOOKS_LOG";
...
///
/// Writes to `~/.weepcode/logs/hooks.log` (or custom path via `WEEPCODE_HOOKS_LOG`).
/// Filters to hooks (`weepcode_hooks`) and plugins (`weepcode_agent::plugins`) targets.
/// Set `WEEPCODE_HOOKS_LOG=0` to disable, `WEEPCODE_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "weepcode_hooks=debug,weepcode_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || weepcode_home().join("logs").join("hooks.log");
