#!/bin/bash
#
# WeepCode CLI installer (enterprise channel).
#
...
#
# Usage:
#   WEEPCODE_CLI_DOWNLOAD_BASE_URL=https://downloads.example.com/weepcode bash install-enterprise.sh
#   WEEPCODE_CLI_DOWNLOAD_BASE_URL=https://downloads.example.com/weepcode bash install-enterprise.sh 0.1.42
#
...
# Read a token from ~/.weepcode/auth.json for the given scope key.
# Format: {"scope_url": {"key": "token"}, ...}
read_weepcode_token() {
    local auth_file="$HOME/.weepcode/auth.json"
    local scope="$1"
...
if [ -n "$WEEPCODE_DEPLOYMENT_KEY" ]; then
    ...
    if [ -n "$OIDC_TOKEN" ]; then
        ...
        echo "Auth: using legacy token from ~/.weepcode/auth.json." >&2
    fi
fi
...
if [ -n "$AUTH_SOURCE" ]; then
    echo "Installing WeepCode $version ($platform, $AUTH_SOURCE)..." >&2
else
    echo "Installing WeepCode $version ($platform)..." >&2
fi

binary_path="$DOWNLOAD_DIR/weepcode-$platform"
artifact_base="${BASE_URL}/weepcode-${version}-${platform}"

if [ "$os" = "windows" ]; then
    binary_path="${binary_path}.exe"
fi

echo "  Downloading weepcode ${version}..." >&2
if [ "$os" = "windows" ]; then
    if ! download_file_parallel "${artifact_base}.exe" "$binary_path"; then
        if ! download_file_parallel "$artifact_base" "$binary_path"; then
            ...
            if is_not_found "${artifact_base}.exe"; then
                echo "Error: WeepCode is not yet available for your system ($platform)." >&2
            else
...
if [ "$os" = "windows" ]; then
    ...
    for bin_name in weepcode.exe agent.exe; do
    ...
    echo "  Binary installed to $BIN_DIR/weepcode.exe and $BIN_DIR/agent.exe." >&2
else
    chmod +x "$binary_path"
    ln -sf "$binary_path" "$BIN_DIR/weepcode"
    ln -sf "$binary_path" "$BIN_DIR/agent"
    echo "  Binary linked to $BIN_DIR/weepcode and $BIN_DIR/agent." >&2
fi
...
CONFIG_FILE="$HOME/.weepcode/config.toml"
...
if [ "$os" = "windows" ]; then
    echo "WeepCode $version installed to $BIN_DIR/weepcode.exe" >&2
else
    echo "WeepCode $version installed to $BIN_DIR/weepcode" >&2
fi

# --- Ensure weepcode is on PATH ---

path_has_dir() {
    case ":$PATH:" in *":$1:"*) return 0 ;; *) return 1 ;; esac
}

# Try to symlink into a directory already on PATH so weepcode works immediately
...
if [ "$os" != "windows" ] && ! path_has_dir "$BIN_DIR"; then
    for candidate in "$HOME/.local/bin" "/usr/local/bin"; do
        if path_has_dir "$candidate" && [ -d "$candidate" ] && [ -w "$candidate" ]; then
            ln -sf "$BIN_DIR/weepcode" "$candidate/weepcode"
            ln -sf "$BIN_DIR/agent" "$candidate/agent"
            SYMLINK_CREATED="$candidate"
            echo "  Symlinked $candidate/weepcode -> $BIN_DIR/weepcode" >&2
...
# Also update shell config so ~/.weepcode/bin is on PATH for future sessions
...
if [ -n "$config_file" ]; then
    ...
    if [ "$user_shell" = "fish" ]; then
        new_block='# >>> weepcode installer >>>
fish_add_path $HOME/.weepcode/bin
# <<< weepcode installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> weepcode installer >>>
export PATH="$HOME/.weepcode/bin:$PATH"
fpath=(~/.weepcode/completions/zsh $fpath)
...
# <<< weepcode installer <<<'
    ...
    if grep -qs "weepcode installer" "$config_file" 2>/dev/null; then
        # Replace existing block in-place (strip old >>> to <<< lines, insert new)
        tmp="$config_file.tmp.$$"
        awk '
            /# >>> weepcode installer >>>/ { skip=1; next }
            /# <<< weepcode installer <<</ { skip=0; next }
            !skip { print }
        ' "$config_file" > "$tmp" && mv "$tmp" "$config_file"
    else
...
if path_has_dir "$BIN_DIR" || [ -n "$SYMLINK_CREATED" ]; then
    echo "Run 'weepcode' or 'agent' to get started!" >&2
elif [ -n "$config_file" ]; then
    echo "Restart your terminal, then run 'weepcode' or 'agent' to get started!" >&2
else
    echo "Add $BIN_DIR to your PATH, then run 'weepcode' or 'agent' to get started:" >&2
    echo '  export PATH="$HOME/.weepcode/bin:$PATH"' >&2
fi

if [ "$os" = "windows" ]; then
    echo "To use weepcode from cmd.exe or PowerShell, add %USERPROFILE%\\.weepcode\\bin to your PATH." >&2
fi
