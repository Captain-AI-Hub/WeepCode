...
if [ "$os" = "windows" ]; then
    ...
    if ! "$binary_tmp" --version </dev/null >/dev/null 2>&1; then
        echo "Error: downloaded weepcode failed to run; keeping the existing install." >&2
        rm -f "$binary_tmp"
        exit 1
    fi
    mv -f "$binary_tmp" "$binary_path"
    # Use relative symlinks when BIN_DIR and DOWNLOAD_DIR share a parent
    # (default layout: ~/.weepcode/bin and ~/.weepcode/downloads are siblings).
    ...
    ln -sf "$link_target" "$BIN_DIR/weepcode"
