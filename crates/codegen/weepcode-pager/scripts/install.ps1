#
# WeepCode CLI installer for PowerShell — https://x.ai/cli/install.ps1
#
...
#   $env:WEEPCODE_VERSION="0.1.42"; irm https://x.ai/cli/install.ps1 | iex           # specific version (alt)
#   $env:WEEPCODE_DEPLOYMENT_KEY="<key>"; irm https://x.ai/cli/install.ps1 | iex
#
...
if (-not $Version -and $env:WEEPCODE_VERSION) {
    $Version = $env:WEEPCODE_VERSION
}
...
$WeepCodeDir = Join-Path $env:USERPROFILE '.weepcode'
...
function Read-WeepCodeToken([string]$Scope) {
    $authFile = Join-Path $WeepCodeDir 'auth.json'
...
if ($env:WEEPCODE_DEPLOYMENT_KEY) {
    ...
    if ($oidcToken) {
        ...
        Write-Host 'Auth: using legacy token from ~/.weepcode/auth.json.' -ForegroundColor DarkGray
    }
}
...
$Channel = if ($env:WEEPCODE_CHANNEL) { $env:WEEPCODE_CHANNEL } else { 'stable' }
...
if ($AuthSource) {
    Write-Host "Installing WeepCode $resolvedVersion ($platform, $AuthSource)..." -ForegroundColor Cyan
} else {
    Write-Host "Installing WeepCode $resolvedVersion ($platform)..." -ForegroundColor Cyan
}

# --- Download binary ---

$binaryPath = Join-Path $DownloadDir "weepcode-$platform.exe"
$artifactBase = "$BaseUrl/weepcode-$resolvedVersion-$platform"
...
foreach ($binName in @('weepcode.exe', 'agent.exe')) {
...
Write-Host "  Installed to $BinDir\weepcode.exe and $BinDir\agent.exe." -ForegroundColor DarkGray

# --- Generate completions (best-effort) ---

$completionsDir = Join-Path (Join-Path $WeepCodeDir 'completions') 'powershell'
try {
    New-Item -ItemType Directory -Path $completionsDir -Force | Out-Null
    & (Join-Path $BinDir 'weepcode.exe') completions powershell 2>$null |
        Set-Content (Join-Path $completionsDir 'weepcode.ps1') -ErrorAction SilentlyContinue
} catch {}

# --- Persist installer config ---

$ConfigFile = Join-Path $WeepCodeDir 'config.toml'
...
Write-Host "WeepCode $resolvedVersion installed to $BinDir\weepcode.exe" -ForegroundColor Green
...
if ($pathEntries -notcontains $BinDir) {
    ...
    # Update current session so weepcode works immediately.
...
Write-Host ''
Write-Host "Run 'weepcode' or 'agent' to get started!" -ForegroundColor Cyan
