//! Headless single-turn mode (`weepcode -p "prompt"`).
//!
//! Runs the agent in-process via
//! `spawn_weepcode_shell`, sends the ACP lifecycle (init → auth → session → prompt),
...
use weepcode_acp_lib::{AcpAgentTx, AcpClientMessageBox, AcpClientRx, acp_send};
use weepcode_shell::agent::auth_method::AuthMethodKind;
use weepcode_shell::agent::config::Config as AgentConfig;
use weepcode_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
...
use weepcode_shell::util::config as cli_config;

use crate::acp::model_state::{EffortTokenError, ModelState};
use crate::acp::spawn::spawn_weepcode_shell;
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<weepcode_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use weepcode_workspace::permission::types::RuleAction;
...
fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<weepcode_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = weepcode_shell::agent::config::AgentDefinition::from_json(&value)
...
fn apply_agent_flag(agent: &Option<String>, config: &mut weepcode_shell::agent::config::Config) {
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    weepcode_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
///
/// WeepCode: there is no browser login anymore — headless runs need either a
/// provider profile in `config.toml` (written by the TUI setup form) or an
/// API key in the environment.
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "No API provider configured. Run `weepcode` interactively and complete \
         \"Configure API Provider\" on the welcome screen (it writes a \
         [model.<name>] entry to ~/.weepcode/config.toml), or set WEEPCODE_API_KEY."
            .to_string()
    } else {
        "No API provider configured. Add a [model.<name>] table with \
         base_url/api_key to ~/.weepcode/config.toml, or set the WEEPCODE_API_KEY \
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !weepcode_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !weepcode_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &weepcode_tools::types::compat::CompatConfig::default());
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'weepcode models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    weepcode_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = weepcode_shell::agent::config::AgentMode::Headless;
    ...
    // hub_url lives on `weepcode workspace` / WorkspaceStartArgs only).
    ...
    agent_config.cli_agent_overrides = weepcode_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        weepcode_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }

    let cancel = CancellationToken::new();
    let memory_config = agent_config.memory_config.clone();
    let spawned = match spawn_weepcode_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let track_active = std::env::var("WEEPCODE_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = weepcode_shell::active_sessions::register(
            weepcode_shell::active_sessions::ActiveSession {
    ...
    if options.self_verify {
        prompt_blocks.push(acp::ContentBlock::Text(acp::TextContent::new(
            skill_body(weepcode_shell::builtin::CHECK_SKILL_MD).to_string(),
        )));
    }
        ...
        let n = n.clamp(2, 10);
        {
            prompt_blocks.insert(
                ...
                acp::ContentBlock::Text(acp::TextContent::new(format!(
                    ...
                    skill_body(weepcode_shell::builtin::BEST_OF_N_SKILL_MD)
                ))),
            );
        }
    }
    ...
    // Handle result
    if track_active {
        // Non-blocking flock so a slow/network ~/.weepcode can't hang exit.
        let _ = weepcode_shell::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = weepcode_shell::sampling::error::prompt_usage_from_error(&err)
...
fn handle_ext_notification(
    ...
    if method == "weepcode/task_completed" {
        #[derive(serde::Deserialize)]
    ...
    if method == "weepcode/monitor_event" {
        return ExtEvent::MonitorEvent;
    }

    match method {
        "weepcode/session_notification" | "weepcode/session/update" => {}
    ...
    let Ok(weepcode_notif) = serde_json::from_str::<XaiNotif>(notif.request.params.get()) else {
        return ExtEvent::None;
    };

    match weepcode_notif.update {
...
mod tests {
    ...
    fn reap_request_for_task_kills_with_session_scope() {
        ...
        assert_eq!(request.method.as_ref(), "weepcode/task/kill");
    ...
    fn reap_request_for_subagent_cancels_with_stripped_id() {
        ...
        assert_eq!(request.method.as_ref(), "weepcode/subagent/cancel");
        let params: serde_json::Value = serde_json::from_str(request.params.get()).unwrap();
        assert_eq!(params["subagentId"], "sub-7");
    }

    use super::*;
    use weepcode_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            weepcode_workspace::permission::types::PatternMode::Domain
        );
    ...
    ) -> weepcode_acp_lib::AcpArgsBox<acp::ExtNotification> {
    ...
    fn headless_session_update_unknown_method_is_none() {
        ...
        let raw = serde_json::value::to_raw_value(&payload).unwrap();
        let (tx, _rx) = tokio::sync::oneshot::channel();
        let notif = weepcode_acp_lib::AcpArgs {
            request: acp::ExtNotification::new("weepcode/other", raw.into()),
            response_tx: tx,
        }
        .boxed();
        assert!(matches!(
