//! Streaming output filter for `weepcode wrap`: OSC 52 clipboard interception,
...
///
/// Delegates to [`weepcode_shell::util::clipboard::set_text`] which uses
...
fn set_local_clipboard(data: &[u8]) {
    ...
    if let Err(e) = weepcode_shell::util::clipboard::set_text(text) {
...
pub(crate) fn host_clipboard_image_frame() -> Vec<u8> {
    let image = weepcode_pager_render::clipboard::system_clipboard_get_image();
