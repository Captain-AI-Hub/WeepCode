//! `/docs` -- open How-to Guides (in-TUI).
//!
...
mod tests {
    ...
    #[test]
    fn web_arg_is_unknown_target_now() {
        // WeepCode: there is no online docs site; `/docs web` is not special.
        let models = ModelState::default();
        let mut ctx = make_ctx(&models);
        assert!(matches!(
            DocsCommand.run(&mut ctx, "web"),
    ...
    #[test]
    fn suggest_args_includes_howto_and_titles() {
        let models = ModelState::default();
