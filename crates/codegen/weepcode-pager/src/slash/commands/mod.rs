...
pub fn builtin_commands() -> Vec<Arc<dyn SlashCommand>> {
    vec![
        ...
        Arc::new(add_model::AddModelCommand),
...
mod tests {
    ...
    fn sample_models() -> ModelState {
        ...
        let id_fast = acp::ModelId::new(Arc::from("weepcode-4.5"));
        models.available.insert(
            id_fast.clone(),
            acp::ModelInfo::new(id_fast.clone(), "WeepCode 4.5".to_string()),
        );
        let id_pro = acp::ModelId::new(Arc::from("weepcode-4.3"));
        models.available.insert(
            id_pro.clone(),
            acp::ModelInfo::new(id_pro.clone(), "WeepCode 4.3".to_string()),
        );
    ...
    fn model_resolves_by_display_name() {
        ...
        let result = cmd.run(&mut ctx, "WeepCode 4.5");
        match result {
            CommandResult::Action(Action::SetDefaultModel(id)) => {
                assert_eq!(id.0.as_ref(), "weepcode-4.5");
            }
    ...
    fn model_resolves_by_model_id() {
        ...
        let mut ctx = make_ctx(&models);
        let cmd = model::ModelCommand;
        let result = cmd.run(&mut ctx, "weepcode-4.3");
        match result {
            CommandResult::Action(Action::SetDefaultModel(id)) => {
                assert_eq!(id.0.as_ref(), "weepcode-4.3");
            }
            other => panic!("expected Action(SetDefaultModel), got {other:?}"),
        }
    }
    ...
    fn model_suggest_args_returns_available_models() {
        ...
        assert!(
            items
                .iter()
                .any(|i| i.display.starts_with("WeepCode 4.5") && i.insert_text == "WeepCode 4.5")
        );
        assert!(
            items
                .iter()
                .any(|i| i.display == "WeepCode 4.3" && i.insert_text == "WeepCode 4.3")
        );
    }
    #[test]
