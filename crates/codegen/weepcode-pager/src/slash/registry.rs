...
impl CommandRegistry {
    ...
    /// Returns `None` for hidden commands, menu-hidden commands, or
    /// commands whose `required_tools()` are not all in the advertised
    /// toolset.
    ///
    ...
    /// `/dashboard`, or `/auto` when the auto permission-mode feature is
    /// unavailable — those must stay fail-closed) and commands whose
    /// `required_tools()` are not all in the advertised toolset.
    ///
...
mod tests {
    ...
    /// (feature-gated) and tool-gated commands stay unresolvable for
    /// dispatch, exactly like `get()`.
    #[test]
    fn get_for_dispatch_respects_hard_gates() {
        // Hard-hidden by name (/auto is fail-closed by default).
        let auto: Arc<dyn SlashCommand> = Arc::new(DummyCommand {
            name: "auto",
            aliases: &[],
        });
        ...
        let reg = CommandRegistry::new(vec![auto, gated]);

        assert!(reg.get_for_dispatch("auto").is_none(), "hidden stays hard");
        assert!(
