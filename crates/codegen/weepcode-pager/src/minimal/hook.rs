...
//! render mode.
//!
//! `weepcode-pager` cannot depend on `weepcode-pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`weepcode-pager-bin`) wires it up once at
//! startup by calling `weepcode_pager_minimal::install()`. When nothing has
