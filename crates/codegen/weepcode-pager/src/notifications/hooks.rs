...
fn execute_hook(
    ...
    cmd.arg("-c")
        .arg(command)
        .env("WEEPCODE_EVENT", event_str)
        .env("WEEPCODE_MESSAGE", message)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    if let Some(sid) = session_id {
        cmd.env("WEEPCODE_SESSION_ID", sid);
    }
...
mod tests {
    ...
    fn sets_environment_variables() {
        ...
        let command = format!(
            "printf 'WEEPCODE_EVENT=%s\\nWEEPCODE_MESSAGE=%s\\nWEEPCODE_SESSION_ID=%s\\n' \
             \"$WEEPCODE_EVENT\" \"$WEEPCODE_MESSAGE\" \"$WEEPCODE_SESSION_ID\" > {}",
            out.display()
        );
        ...
        assert!(
            content.contains("WEEPCODE_EVENT=Turn complete"),
            "missing WEEPCODE_EVENT: {content}"
        );
        assert!(
            content.contains("WEEPCODE_MESSAGE=hello world"),
            "missing WEEPCODE_MESSAGE: {content}"
        );
        assert!(
            content.contains("WEEPCODE_SESSION_ID=sess-42"),
            "missing WEEPCODE_SESSION_ID: {content}"
        );
    }
    ...
    fn omits_session_id_when_none() {
        ...
        let content = std::fs::read_to_string(&out).unwrap();
        assert!(
            !content.contains("WEEPCODE_SESSION_ID"),
            "WEEPCODE_SESSION_ID should not be set: {content}"
        );
    }
    ...
    fn run_hook_passes_correct_env_via_thread() {
        ...
        assert!(content.contains("WEEPCODE_EVENT=Turn complete"));
        assert!(content.contains("WEEPCODE_MESSAGE=test body payload"));
        assert!(content.contains("WEEPCODE_SESSION_ID=test-session-123"));
    }
}
