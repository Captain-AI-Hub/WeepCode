...
pub struct EnumChoice {
    /// Canonical persisted value (e.g. `"weepcodenight"`).
    pub canonical: &'static str,
    /// Display label shown in the chooser (e.g. `"WeepCode Night"`).
...
pub fn current_value_for(
    ...
    match key {
        ...
        // Live cache; `WEEPCODE_PROMPT_SUGGESTIONS` env overrides at the gate.
        ...
        "fork_secondary_model" => Some(SettingValue::String({
            let baseline = weepcode_shell::models::default_model();
...
mod tests {
                    ...
                    assert_eq!(
                        *default,
                        ...
                        "toolset.ask_user_question.timeout_enabled default drifts from the \
                         shared resolver const in weepcode-tools"
                    );
                }
    ...
    fn current_value_for_auto_dark_theme_filters_auto_value() {
        ...
        assert_eq!(
            value,
            SettingValue::Enum("weepcodenight"),
    ...
    fn contextual_hints_group_and_children_registered() {
        ...
        let ui = UiConfig {
            contextual_hints: weepcode_shell::agent::config::ContextualHints {
                undo: Some(false),
                ..Default::default()
            },
            ..UiConfig::default()
