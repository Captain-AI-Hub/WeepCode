...
use weepcode_acp_lib::{AcpAgentTx, AcpClientRx, acp_send};
...
pub struct AcpConnection {
    ...
    /// Whether the agent is a weepcode-shell instance.
    pub is_weepcode_shell: bool,
    ...
    // When available, add field: startup_announcements: Option<Vec<weepcode_announcements::RemoteAnnouncement>>
    ...
    /// `AuthManager` for pager-side authenticated requests.
    ///
    ...
    pub auth_manager: std::sync::Arc<weepcode_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<weepcode_shell::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_weepcode_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    let env_urls = weepcode_shell::leader::LeaderEnvUrls::from(&agent_config.weepcode_com_config);
    ...
    let auth_manager = std::sync::Arc::new(weepcode_shell::auth::AuthManager::new(
        ...
        agent_config.weepcode_com_config.clone(),
    ));

    // Leader has no in-process agent; init this process's product telemetry client.
    weepcode_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = weepcode_shell::util::weepcode_home::weepcode_home().join("config.toml");
...
fn client_capabilities_meta(flags: &ConnectFlags) -> serde_json::Value {
    ...
    serde_json::json!({
        "weepcode/incrementalBashOutput": true,
        "weepcode/hunkTracker": { "mode": hunk_mode },
        "weepcode/bashOutputNoColor": true,
        "weepcode/gitHeadChanged": true,
    })
}
...
async fn initialize(
    ...
    // Check if this is a weepcode-shell agent
    let is_weepcode_shell = resp
        .meta
        .as_ref()
...
/// When `weepcode.api_key` was advertised, non-interactive credentials were
...
mod tests {
    ...
    #[test]
    fn startup_auth_weepcode_com_no_provider_needs_login_pending() {
    ...
    #[test]
    fn startup_auth_weepcode_com_with_external_provider_command() {
    ...
    #[test]
    fn startup_auth_non_weepcode_com_no_login() {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.weepcode/config.toml` with a
    ...
    /// change re-orders `build_auth_methods()` to put `weepcode.api_key` anywhere
    ...
    /// (`agent::auth_method::tests::enterprise_byok_first_method_is_weepcode_api_key`
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use weepcode_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};
        ...
        assert!(
            ...
            "shell built auth_methods for a BYOK user, but the pager still \
             ...
             weepcode.api_key first or the pager stopped treating weepcode.api_key as \
             a no-login method.",
        );
        assert!(label.is_none());
        assert!(method_id.is_none());
    ...
    /// Inverse direction: when `weepcode.api_key` is NOT in the list, the pager
    /// MUST show the login screen. We assert this with `weepcode.api_key` present
    ...
    /// `startup_auth_weepcode_com_no_provider_needs_login_pending` above and
    /// either passes or fails on a meaningful new code path.
    #[test]
    fn startup_auth_weepcode_api_key_not_first_still_requires_login() {
        use weepcode_shell::agent::auth_method::{
            WEEPCODE_API_KEY_METHOD_ID, WEEPCODE_COM_METHOD_ID,
        };

        let methods = vec![
            make_auth_method(WEEPCODE_COM_METHOD_ID, "WeepCode", None),
            make_auth_method(WEEPCODE_API_KEY_METHOD_ID, "weepcode.api_key", None),
        ];
        let (needs, _, _, _) = startup_auth_metadata(&methods);
        assert!(
            needs,
            "with grok.com first, the pager must require login -- pinning \
             the BAD-ordering failure mode (weepcode.api_key not first)",
        );
    }
    ...
    fn client_capabilities_meta_defaults_absent_or_blank_mode_to_agent_only() {
        ...
        assert_eq!(absent["weepcode/hunkTracker"]["mode"], "agent_only");
        let blank = client_capabilities_meta(&ConnectFlags {
            hunk_tracker_mode: Some("   ".into()),
            ..Default::default()
        });
        assert_eq!(blank["weepcode/hunkTracker"]["mode"], "agent_only");
    }
    ...
    fn client_capabilities_meta_canonicalizes_off_and_mixed_case() {
        ...
        for raw in ["off", "OFF", "Disabled"] {
            ...
            assert_eq!(meta["weepcode/hunkTracker"]["mode"], "off", "raw={raw}");
        }
    }
}
