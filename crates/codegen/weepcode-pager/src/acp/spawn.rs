...
//!
//! Simplified to only support WeepcodeShell (in-process) mode.
...
use weepcode_shell::{
    ...
    auth::AuthManager,
    util::weepcode_home::weepcode_home,
};
...
/// Spawn a WeepcodeShell agent in a background thread.
///
/// Returns the ACP client channel for communication and a cancellation token.
pub async fn spawn_weepcode_shell(
    agent_config: AgentConfig,
    cancel: &CancellationToken,
    memory_config: Option<weepcode_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    // skills to ~/.weepcode/skills/), and model catalog construction.
    let (agent_config, models_manager) =
        weepcode_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)
