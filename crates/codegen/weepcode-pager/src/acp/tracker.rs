...
use weepcode_tools::types::output::{ReadFileOutput, SearchToolOutput, WebFetchOutput};
use weepcode_tools::util::strip_redundant_session_cd;
...
impl AcpUpdateTracker {
    ...
    fn handle_user_message(
        ...
        let mut block = if let Some(dt) = display_override {
            ...
            let skill_display =
                weepcode_tools::implementations::skills::skill::extract_skill_display_text(&text);
...
fn user_message_hidden_from_scrollback(
    ...
    if let Some(pid) = meta.prompt_id.as_deref()
        && weepcode_shell::session::PromptOrigin::from_prompt_id(pid)
...
fn extract_edit_error(tc: &acp::ToolCall) -> String {
    use weepcode_tools::types::output::SearchReplaceOutput;
    if let Some(ref raw) = tc.raw_output
...
fn extract_listdir_content(raw: &Option<serde_json::Value>) -> Option<String> {
    let val = raw.as_ref()?;
    match serde_json::from_value::<ToolOutput>(val.clone()) {
        Ok(ToolOutput::ListDir(weepcode_tools::types::output::ListDirOutput::Content(c))) => {
            Some(c.content)
        }
        _ => None,
    }
...
fn extract_use_tool_output(raw: &Option<serde_json::Value>) -> Option<String> {
    ...
    if let Ok(output) = serde_json::from_value::<ToolOutput>(val.clone()) {
        let text = match output {
            ToolOutput::MCP(mcp) => {
                use weepcode_tools::types::output::MCPOutputDetails;
                match mcp.output() {
...
mod tests {
    use super::*;
    use std::sync::Arc;
    /// Default meta with no timestamps (simulates old weepcode-shell or tests that
    ...
    fn test_search_tool_call_flow() {
        use weepcode_tools::types::output::{GrepFileMatch, GrepLineMatch, GrepSearchOutput};
        let mut tracker = AcpUpdateTracker::new();
    ...
    /// No stream_start_ms (old weepcode-shell) should not break anything.
    #[test]
    ...
    #[test]
    fn is_task_tool_recognizes_weepcode_build_variant() {
...
[CI] IN_PROGRESS=5 SUCCESS=38\n\
    ...
    fn video_tool_variants_use_typed_path_not_generic_scrape() {
        ...
        let cases: &[(&str, ToolOutput)] = &[
            (
                "ImageToVideo",
                ToolOutput::ImageToVideo(weepcode_tools::types::output::MediaGenOutput::new(
                    video_path.clone(),
                )),
            ),
            (
                "ReferenceToVideo",
                ToolOutput::ReferenceToVideo(weepcode_tools::types::output::MediaGenOutput::new(
                    video_path.clone(),
                )),
            ),
        ];
    ...
    fn media_gen_ref_skips_uploaded_only_video() {
        let output =
            ToolOutput::ImageToVideo(weepcode_tools::types::output::MediaGenOutput::uploaded(
    ...
    /// A media tool can complete with plain text instead of a generated file.
    /// The media renderer has no file to open, so it must surface the text in
    /// the card body (not a bare title) and must NOT mark the card as an error.
    #[test]
    fn media_text_output_shows_body_text_not_error() {
        let message = "Image generation is unavailable for this request.";
        let output = ToolOutput::Text(weepcode_tools::types::output::TextOutput::from(message));
        let tc = acp::ToolCall::new(
            acp::ToolCallId::new(Arc::from("text-only-img")),
            "image_gen",
        )
        .kind(acp::ToolKind::Other)
        .status(acp::ToolCallStatus::Completed)
        ...
        assert!(
            block.is_success(),
            "text-only media output is a successful result, not an error"
        );
        assert!(
            block
                .output
                .as_deref()
                .unwrap_or_default()
                .contains("unavailable for this request"),
            "text output must be shown in the card body, got: {:?}",
            block.output
        );
    }
}
