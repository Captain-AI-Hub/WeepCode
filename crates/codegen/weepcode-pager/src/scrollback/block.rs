...
impl RenderBlock {
    ...
    pub fn context_info(
        snapshot: weepcode_shell::session::ContextInfo,
    ...
    pub fn with_hyperlinks<R>(
        &self,
        f: impl FnOnce(&[weepcode_markdown::HyperlinkTarget]) -> R,
    ) -> R {
        match self {
...
mod searchable_text_tests {
    ...
    use weepcode_shell::session::ContextInfo;
    ...
    fn subagent_failed_indexes_metadata_and_error() {
        ...
        if let RenderBlock::Subagent(b) = &mut block {
            ...
            b.model = Some("weepcode-test".into());
        ...
        assert!(text.contains("weepcode-test"), "got: {text:?}");
    ...
    fn context_info_indexes_model_only() {
        ...
        let block = RenderBlock::context_info(snapshot, "weepcode-4.5");
        // Only the model name is source text; the rest is a numeric breakdown.
        assert_eq!(block.searchable_text().as_deref(), Some("weepcode-4.5"));
    }
