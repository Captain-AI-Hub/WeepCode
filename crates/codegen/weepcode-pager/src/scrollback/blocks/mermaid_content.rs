...
//! the generic [`CodeBlockSpan`](weepcode_markdown::CodeBlockSpan) API) and
...
use weepcode_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](weepcode_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](weepcode_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a weepcode_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `WeepcodeDay` is the only light theme; every other concrete theme (and the
/// `WeepcodeNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `weepcode_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::WeepcodeDay)
}
...
mod tests {
    ...
    #[test]
    fn theme_is_dark_maps_weepcodeday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::WeepcodeDay),
            "WeepcodeDay is the light theme"
        );
        for dark in [
            ThemeKind::WeepcodeNight,
