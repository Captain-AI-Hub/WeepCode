...
pub enum SessionEvent {
    ...
    /// Message comes from agent via WeepcodeSessionUpdate::HookAnnotation.
    HookAnnotation {
        /// The hook message
        message: String,
    },
...
mod tests {
    ...
    fn model_unavailable_with_switch() {
        let event = SessionEvent::ModelUnavailable {
            previous_model_id: "weepcode-4.5".into(),
            new_model_id: "weepcode-build".into(),
            reason: "Model \"weepcode-4.5\" is no longer available.".into(),
        };
        assert_eq!(
            event.message(),
            "Model \"weepcode-4.5\" is no longer available. Switched to \"weepcode-build\"."
        );
    }
