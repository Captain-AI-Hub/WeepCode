...
/// Extract user-guide docs to `<weepcode_home>/docs/user-guide/`.
///
/// Called from the pager binary startup so the model can read them from disk.
pub fn extract_user_guide_docs(weepcode_home: &std::path::Path) {
    let docs_dir = weepcode_home.join("docs").join("user-guide");
