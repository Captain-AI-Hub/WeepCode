...
use weepcode_shell::auth::{AuthManager, try_ensure_fresh_auth};
use weepcode_shell::session::merge::MergedSession;
...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    // deployments that only configure a deployment_key + custom weepcode_api_base_url.
    // If the user has previously run the interactive `weepcode` TUI (which succeeds
    ...
    let auth = try_ensure_fresh_auth(&agent_config.weepcode_com_config).await;
    ...
    let client = weepcode_shell::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions = weepcode_shell::session::merge::fetch_merged(
                Some(&client),
                cwd.to_str(),
                None,
                limit,
        ...
        SessionsCommand::Search { query, limit } => {
            ...
            use weepcode_shell::session::merge::REMOTE_TIMEOUT;
            use weepcode_shell::session::storage::search::{SessionSearchRequest, execute_search};
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = weepcode_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
