...
/// Start (if needed) and submit the initial prompt from `weepcode "<prompt>"`.
///
...
pub(super) fn dispatch_show_undo_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(weepcode_telemetry::events::ContextualTip {
            tip: weepcode_telemetry::events::ContextualTipKind::Undo,
...
pub(in crate::app) fn show_small_screen_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(weepcode_telemetry::events::ContextualTip {
            tip: weepcode_telemetry::events::ContextualTipKind::SmallScreen,
            action: weepcode_telemetry::events::ContextualTipAction::Shown,
        });
    }
}

/// Show the one-shot "Over SSH? Run `weepcode wrap ssh <host>` locally…" hint at
...
pub(in crate::app) fn show_ssh_wrap_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(weepcode_telemetry::events::ContextualTip {
            tip: weepcode_telemetry::events::ContextualTipKind::SshWrap,
            action: weepcode_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_show_plan_nudge(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(weepcode_telemetry::events::ContextualTip {
            tip: weepcode_telemetry::events::ContextualTipKind::PlanMode,
            action: weepcode_telemetry::events::ContextualTipAction::Shown,
        });
    }
    vec![]
...
pub(super) fn dispatch_show_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(weepcode_telemetry::events::ContextualTip {
            tip: weepcode_telemetry::events::ContextualTipKind::WordSelect,
            action: weepcode_telemetry::events::ContextualTipAction::Shown,
        });
    }
...
pub(super) fn dispatch_accept_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    log_event(weepcode_telemetry::events::ContextualTip {
        tip: weepcode_telemetry::events::ContextualTipKind::WordSelect,
        action: weepcode_telemetry::events::ContextualTipAction::Accepted,
    });
...
fn maybe_show_send_now_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(weepcode_telemetry::events::ContextualTip {
            tip: weepcode_telemetry::events::ContextualTipKind::SendNow,
            action: weepcode_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
    ...
    if !literal && trimmed.starts_with('/') {
        ...
        let exec_result = {
            ...
            if let Some(invocation) = parse_invocation(trimmed) {
                ...
                {
                    use weepcode_telemetry::events::{PagerCommandSource, PagerSlashCommand};
                    use weepcode_telemetry::session_ctx::log_event;
...
pub(super) fn handle_prompt_response(
    ...
    if let Some(agent) = app.agents.get_mut(&agent_id) {
        ...
        // at every turn boundary. This must run before the reconnect early
        // return below, which skips the fetch gate entirely — a prior ghost
        // would otherwise survive that path.
