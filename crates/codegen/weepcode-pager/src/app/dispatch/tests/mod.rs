...
fn cta_entry(name: &str, status: &str) -> weepcode_hooks_plugins_types::MarketplacePluginEntry {
...
fn cta_outcome(
    status: weepcode_hooks_plugins_types::OutcomeStatus,
    message: &str,
...
/// Plant a Build session under the process `weepcode_home()` (OnceLock-cached;
/// do not rely on setting `WEEPCODE_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = weepcode_shell::util::weepcode_home::weepcode_home();
    let encoded = weepcode_shell::util::weepcode_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: weepcode_tools::implementations::weepcode_build::ask_user_question::AskUserQuestionMode::Default,
        questions: vec![
...
fn enqueue_permission_with_enable_always_approve(
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    weepcode_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
    ...
    agent.permission_queue.push_back(PermissionViewState {
        request: weepcode_acp_lib::AcpArgs {
            request,
            response_tx,
        },
        id: 1,
...
const POLICY_WARNING: &str =
    weepcode_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
...
fn push_synthetic_permission(
    ...
    let request = weepcode_acp_lib::AcpArgs {
