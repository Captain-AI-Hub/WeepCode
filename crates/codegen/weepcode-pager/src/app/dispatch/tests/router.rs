...
fn critical_announcement(id: &str) -> weepcode_announcements::RemoteAnnouncement {
...
fn promo_announcement(id: &str) -> weepcode_announcements::RemoteAnnouncement {
    weepcode_announcements::RemoteAnnouncement {
        id: Some(id.into()),
        message: Some(format!("{id} message")),
        severity: Some("promo".into()),
        cta: Some(weepcode_announcements::AnnouncementCta {
            label: Some("Go".into()),
...
/// the promo url (observed via the `WEEPCODE_TEST_OPEN_URL_FILE` seam, from every
...
#[serial_test::serial(WEEPCODE_TEST_OPEN_URL_FILE)]
#[test]
fn announcements_open_cta_opens_promo_and_noops_under_critical() {
    use weepcode_telemetry::events::AnnouncementCtaSurface;
    let url_file =
        std::env::temp_dir().join(format!("weepcode-cta-open-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&url_file);
    unsafe { std::env::set_var("WEEPCODE_TEST_OPEN_URL_FILE", &url_file) };
    ...
    unsafe { std::env::remove_var("WEEPCODE_TEST_OPEN_URL_FILE") };
    let _ = std::fs::remove_file(&url_file);
}
...
fn tick_propagates_available_commands_to_bootstrap() {
    ...
    let skill_meta = serde_json::json!(
        { "scope" : "user", "path" : "/home/user/.weepcode/skills/pick-best/SKILL.md", }
    );
