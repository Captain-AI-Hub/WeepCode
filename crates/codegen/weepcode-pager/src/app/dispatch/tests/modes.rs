...
fn enable_always_approve_sends_response_and_flips_yolo_and_persists() {
    ...
    //     ~/.weepcode/config.toml AND fires x.ai/yolo_mode_changed.
    let persist = effects
        .iter()
        .find_map(|e| match e {
...
/// guard MUST precede the `weepcode_telemetry::log_event` call
...
//
// Mirrors the `yolo` test pattern. These exercise the dispatch path directly (not the
