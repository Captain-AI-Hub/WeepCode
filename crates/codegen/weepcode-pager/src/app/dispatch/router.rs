...
use super::auth::{
    dispatch_cancel_login, dispatch_login, dispatch_logout, dispatch_provider_setup_cancel,
    dispatch_provider_setup_submit, dispatch_submit_auth_code, dispatch_switch_account,
};
use super::ctx::{
    ...
    let effects = match action {
        ...
        Action::AnnouncementsOpenCta(surface) => {
            if let Some((promo, url)) = crate::views::announcements::promo_cta_target(
                ...
                log_event(weepcode_telemetry::events::AnnouncementCtaClicked {
                    id: promo_id,
                    source: surface,
                });
        ...
        Action::ProviderSetupSubmit => dispatch_provider_setup_submit(app),
        Action::ProviderSetupCancel => dispatch_provider_setup_cancel(app),
        Action::CopyAuthUrl => {
            ...
            OutcomeStatus::Success => {
                    ...
                    && modal.result_notice.is_none()
                {
                    let entry_index = match modal.last_plugins_action {
                        Some(weepcode_hooks_plugins_types::PluginsAction::Uninstall { .. }) => None,
            ...
            OutcomeStatus::ConfirmationRequired => {
                if let Some(ref mut modal) = agent.extensions_modal {
                    let confirmed_action = modal.last_plugins_action.as_ref().map(|a| {
                        ...
                        if let weepcode_hooks_plugins_types::PluginsAction::Uninstall {
                            ref mut confirmed,
                            ..
                        } = action
                        {
