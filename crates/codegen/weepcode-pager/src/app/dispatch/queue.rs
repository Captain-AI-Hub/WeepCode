...
///
/// Delegates to the shared implementation in `weepcode_tools::reminders`.
...
fn format_cron_prompt(prompt: &str, task_id: &str, human_schedule: &str) -> String {
    weepcode_tools::reminders::format_scheduled_task_prompt(prompt, task_id, human_schedule)
}
...
mod tests {
    ...
    fn drain_after_editing_sends_correct_prompt() {
        ...
        assert!(
            effects.is_empty(),
            "drain should be blocked while p3 is being edited"
        );
