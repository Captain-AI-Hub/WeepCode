...
pub(super) fn plugin_cta_candidates(
    response: weepcode_hooks_plugins_types::MarketplaceListResponse,
) -> (
    Vec<weepcode_hooks_plugins_types::MarketplacePluginEntry>,
    bool,
) {
    let mut candidates = Vec::new();
    let mut official_source_present = false;
    for source in response.sources {
        let is_official = source.source_name == weepcode_plugin_marketplace::OFFICIAL_SOURCE_NAME
            || weepcode_plugin_marketplace::is_official_source_url(&source.source_url_or_path);
        if !is_official {
            continue;
        }
...
pub(super) fn cta_install_relative_path(
    candidates: &[weepcode_hooks_plugins_types::MarketplacePluginEntry],
    name: &str,
) -> String {
    candidates
        .iter()
...
pub(super) fn plugin_cta_phase_for(
    ...
    use weepcode_plugin_marketplace::matcher::{KeywordCandidate, match_plugin_keyword};
...
pub(super) fn handle_cta_plugin_install_done(
    ...
    log_event(weepcode_telemetry::events::PluginCtaInstalled {
    ...
    let install_result = match result {
        Ok(outcome) if outcome.status == weepcode_hooks_plugins_types::OutcomeStatus::Success => {
    ...
    match result {
        Ok(servers) => {
            ...
            if needs_auth {
                ...
                log_event(weepcode_telemetry::events::ExtensionsModalOpened {
                    trigger: weepcode_telemetry::events::ExtensionsModalTrigger::AuthHandoff,
    ...
    match result {
        Ok(mut response) => {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                ...
                if enabled {
                    agent.plugin_cta.dismissed = weepcode_shell::config::dismissed_plugin_ctas();
                }
                ...
                if matches!(
                    ...
                    {
                        log_event(weepcode_telemetry::events::PluginCtaImpression {
