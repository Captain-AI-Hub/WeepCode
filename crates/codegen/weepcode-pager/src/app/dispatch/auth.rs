...
pub(super) fn dispatch_login(app: &mut AppView) -> Vec<Effect> {
    ...
    // WeepCode: in-TUI provider setup replaces the WeepCode OAuth flow. Opening
    ...
    // post-auth menu.
    if method_id.0.as_ref() == weepcode_shell::agent::auth_method::PROVIDER_SETUP_METHOD_ID {
...
/// (`weepcode.api_key`); on failure redisplay the error on the form.
        ...
        Ok(_slug) => {
            ...
            vec![Effect::Authenticate {
                ...
                method_id: agent_client_protocol::AuthMethodId::new(
                    weepcode_shell::agent::auth_method::WEEPCODE_API_KEY_METHOD_ID,
                ),
                use_oauth: false,
...
pub(super) fn handle_auth_complete(
        ...
        && *current_seq == request_seq
    {
        if let Some(meta_val) = meta.as_ref()
            && let Ok(auth_meta) =
                serde_json::from_value::<weepcode_shell::auth::AuthMeta>(meta_val.clone())
        {
