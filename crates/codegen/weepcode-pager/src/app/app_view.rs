...
use weepcode_acp_lib::AcpAgentTx;
...
/// [`weepcode_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<weepcode_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: weepcode_shell::util::config::WorktreeHintMode) -> Self {
        use weepcode_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        weepcode_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            weepcode_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`weepcode_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `WEEPCODE_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `WEEPCODE_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("WEEPCODE_ESC_DOUBLE_PRESS_MS").ok()))
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: weepcode_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`weepcode_shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.weepcode/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`WEEPCODE_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `WEEPCODE_FPS` env on release
    /// builds, where the dev overlay is compiled out) — see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<weepcode_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<weepcode_announcements::RemoteAnnouncement>,
    ...
    /// Whether the plugin marketplace CTA is enabled. Env `WEEPCODE_PLUGIN_CTA`
    ...
    /// Dashboard entry points (`/dashboard`, `Ctrl+\`, `weepcode dashboard`, the
    ...
    /// non-selectable headers. Gated by `WEEPCODE_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `WEEPCODE_CANCEL_REWIND` env /
    ...
    /// empty and inert outside `--minimal`; the `weepcode-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<weepcode_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat:
        weepcode_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `weepcode_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// Stamps `_meta["weepcode/session"].kind = "chat"` and omits Build agent
    ...
    /// at startup and on settings toggles from `WEEPCODE_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: weepcode_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<weepcode_shell::util::config::ContextualHintsRemote>,
    ...
    /// One-shot gate for the SSH `weepcode wrap` tip: set after the first
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, weepcode_telemetry::events::AnnouncementCtaSurface)>,
    ...
    /// `~/.weepcode/config.toml`. `None` when the file/section is absent
...
impl AppView {
    ...
    /// True when the user should not see the prompt (ZDR-blocked).
    ...
    pub fn apply_auth_meta(&mut self, meta: &weepcode_shell::auth::AuthMeta) {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: weepcode_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: weepcode_shell::util::config::auto_permission_mode_enabled_from_disk(),
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<weepcode_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    /// `/always-approve` and `/auto` stay offered as true toggles. Call after gate
    /// flips, startup, reconnect, and session create/switch (so new agents inherit
    /// the gate).
    ...
    /// Rows the dev `WEEPCODE_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<weepcode_shell::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    weepcode_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
    ...
    if let Event::Mouse(mouse) = ev {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        weepcode_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
impl AppView {
    ...
    /// Log [`weepcode_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use weepcode_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = weepcode_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
                ...
                .insert((key.clone(), surface))
            {
                weepcode_telemetry::session_ctx::log_event(
                    weepcode_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: weepcode_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    /// One-shot SSH `weepcode wrap` tip trigger, run at the top of every `draw`
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            weepcode_telemetry::session_ctx::log_event(weepcode_telemetry::events::ContextualTip {
                tip: weepcode_telemetry::events::ContextualTipKind::ImageInput,
                action: weepcode_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    fn app_draw_drains_deferred_release_after_flush() {
        ...
        let mut terminal = weepcode_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![weepcode_announcements::RemoteAnnouncement {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![weepcode_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: weepcode_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn apply_auth_meta_clears_api_key_flag_on_personal_login() {
        ...
        app.apply_auth_meta(&weepcode_shell::auth::AuthMeta::default());
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(weepcode_workspace::foreign_sessions::RecentForeignSession {
                tool: weepcode_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![weepcode_workspace::file_system::FuzzyMatchResult {
