...
//! inside [`weepcode_mermaid::render_checked`] is a no-op there: a panic in the
...
//!
//! 1. The pager re-execs itself as `weepcode-pager __mermaid-render` (see
...
//!    via [`weepcode_mermaid::run_with_timeout`], which **kills and reaps** the
...
//!    ([`weepcode_mermaid::RenderLimits`]) and the raster megapixel/height caps
//!    (see `weepcode-mermaid`). The parent also rejects obviously-oversized
...
pub fn render_via_subprocess(
    ...
    cmd.arg(MERMAID_RENDER_SUBCOMMAND)
        ...
        .stdout(Stdio::null())
        .stderr(Stdio::null())
        .envs(weepcode_tty_utils::pager_env());
...
fn render_source_to_png(
    ...
    target_width_px: u32,
    quality: MermaidRenderQuality,
) -> Result<RenderedDiagram, weepcode_mermaid::MermaidError> {
...
thread_local! {
    ...
    /// `WEEPCODE_HOME` (whose `weepcode_home()` value is cached first-write-wins, an
    /// isolation hazard under the full parallel suite — PNGs could land in the
    /// real `~/.weepcode`). Thread-local, so each parallel test is independent; the
    /// `TempDir` guard lives here so the dir outlives the view. Mirrors the
    /// `subagent::REPLAY_WEEPCODE_HOME` test seam. Production never sets this.
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test seam: a hermetic per-test cache dir (no `WEEPCODE_HOME` mutation).
        #[cfg(test)]
...
mod tests {
    ...
    fn is_render_subcommand_matches_only_argv1() {
        ...
        assert!(!is_render_subcommand(&argv(&["weepcode"])));
        assert!(!is_render_subcommand(&argv(&["weepcode", "chat"])));
        assert!(!is_render_subcommand(&argv(&[])));
    ...
    /// hermetic, with no process-global `WEEPCODE_HOME` mutation. The `TempDir` lives
    ...
    /// so there is no cross-test contamination and no `weepcode_home()` cache race.
    ...
    fn agent_with_session(name: &str) -> AgentView {
        ...
        let cwd = PathBuf::from("/weepcode-mermaid-test").join(name);
    ...
    fn mermaid_is_rendering_matches_by_source_across_theme_width_change() {
        ...
        let live_key =
            MermaidCacheKey::derive(src, ThemeKind::WeepcodeDay, 240, MermaidRenderQuality::Open);
        assert_ne!(
            click_key, live_key,
