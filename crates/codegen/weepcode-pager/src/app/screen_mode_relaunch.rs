...
pub(crate) const WEEPCODE_SCREEN_MODE_ENV: &str = "WEEPCODE_SCREEN_MODE";
...
/// `weepcode "do the thing"` does not re-submit on resume. Keeps everything else
...
pub(crate) fn build_screen_mode_relaunch_args(
    ...
    while let Some(arg) = iter.next() {
        ...
        // Bare positional prompt (e.g. `weepcode "fix the bug"`). Must not re-fire
    ...
    // Keep a CLI mode flag for hand-pasted resume hints that omit WEEPCODE_SCREEN_MODE.
    if want_minimal {
...
pub(crate) fn screen_mode_relaunch_resume_hint(session_id: &str, want_minimal: bool) -> String {
    ...
    format!("{WEEPCODE_SCREEN_MODE_ENV}={mode} weepcode {flag} --resume {session_id}")
}
...
/// Parse a [`WEEPCODE_SCREEN_MODE_ENV`] or config `[ui] screen_mode` value
...
/// Consume the one-shot screen-mode override env (see [`WEEPCODE_SCREEN_MODE_ENV`]).
///
...
/// (tool shells, workers, nested `weepcode` invocations) would inherit a forced
...
pub(crate) fn take_screen_mode_env_override() -> Option<super::ScreenMode> {
    let raw = std::env::var_os(WEEPCODE_SCREEN_MODE_ENV);
    if raw.is_some() {
        ...
        unsafe { std::env::remove_var(WEEPCODE_SCREEN_MODE_ENV) };
    }
...
mod tests {
    ...
    fn adds_minimal_and_resume() {
        let out = build_screen_mode_relaunch_args(args(&["weepcode", "--no-leader"]), "abc", true);
        assert_eq!(
            as_strs(&out),
    ...
    fn adds_fullscreen_and_resume() {
        let out = build_screen_mode_relaunch_args(args(&["weepcode", "--no-leader"]), "abc", false);
        assert_eq!(
            as_strs(&out),
    ...
    fn strips_session_id_flag() {
        ...
        let out = build_screen_mode_relaunch_args(
            args(&["weepcode", "-s", "11111111-1111-1111-1111-111111111111"]),
            "new",
            false,
        );
    ...
    fn strips_bare_worktree_without_eating_next_flag() {
        let out = build_screen_mode_relaunch_args(
            args(&["weepcode", "--worktree", "--no-leader"]),
            "new",
            false,
        );
        assert_eq!(
    ...
    fn strips_prior_minimal_and_resume() {
        let out = build_screen_mode_relaunch_args(
            args(&["weepcode", "--minimal", "--resume", "old", "--no-leader"]),
            "new",
            false,
        );
        assert_eq!(
    ...
    fn strips_prior_fullscreen_flag() {
        let out = build_screen_mode_relaunch_args(
            args(&["weepcode", "--fullscreen", "--resume", "old", "--no-leader"]),
            "new",
            true,
        );
        assert_eq!(
    ...
    fn strips_short_resume_and_continue() {
        let out = build_screen_mode_relaunch_args(
            args(&["weepcode", "-r", "old", "-c", "--no-leader"]),
            "sid",
            true,
        );
    ...
    fn strips_resume_equals_form() {
        let out = build_screen_mode_relaunch_args(
            args(&["weepcode", "--resume=old-id", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn strips_positional_prompt() {
        let out = build_screen_mode_relaunch_args(
            args(&["weepcode", "--no-leader", "fix the bug"]),
            "sid",
            true,
        );
        assert_eq!(
    ...
    fn double_dash_and_following_positionals_dropped() {
        // `weepcode --no-leader -- "fix the bug"`: everything after `--` is the
    ...
    fn resume_without_value_then_flag_is_not_eaten() {
        // `weepcode --resume --no-leader` (resume most-recent; next token is a flag).
        let out = build_screen_mode_relaunch_args(
            args(&["weepcode", "--resume", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn take_env_override_consumes_the_variable() {
        ...
        unsafe { std::env::set_var(WEEPCODE_SCREEN_MODE_ENV, "minimal") };
        assert_eq!(
            take_screen_mode_env_override(),
            Some(super::super::ScreenMode::Minimal)
        );
        assert!(
            std::env::var_os(WEEPCODE_SCREEN_MODE_ENV).is_none(),
            "env var must be removed after being read"
        );
        // Unparseable values are still removed (never leak to children).
        unsafe { std::env::set_var(WEEPCODE_SCREEN_MODE_ENV, "bogus") };
        assert_eq!(take_screen_mode_env_override(), None);
        assert!(std::env::var_os(WEEPCODE_SCREEN_MODE_ENV).is_none());
        // Absent stays absent.
        assert_eq!(take_screen_mode_env_override(), None);
    }
    ...
    fn failed_relaunch_hint_includes_screen_mode_env() {
        // Recovery command must carry WEEPCODE_SCREEN_MODE so following the
        ...
        assert_eq!(
            ...
            "WEEPCODE_SCREEN_MODE=fullscreen weepcode --fullscreen --resume abc-sid"
        );
        assert_eq!(
            screen_mode_relaunch_resume_hint("abc-sid", true),
            "WEEPCODE_SCREEN_MODE=minimal weepcode --minimal --resume abc-sid"
        );
    }
