...
//!
//! Env sandboxing follows this crate's `serial(WEEPCODE_HOME)` idiom; note
//! `weepcode_home()` is process-cached (OnceLock), so disk assertions always go
//! through [`effective_weepcode_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary: the harness
//! mutates process-global env (proxy URLs, `WEEPCODE_API_KEY`,
//! `WEEPCODE_LEADER_SOCKET`, `WEEPCODE_HOME`) for a real agent's whole lifetime, and
//! in a several-thousand-test process that poisons concurrently-running tests
//! (and `weepcode_home()`'s OnceLock is usually already pinned). Run on demand:
//!
//! ```bash
//! cargo test -p weepcode-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
use weepcode_shell::agent::mvp_agent::MvpAgent;
...
/// The weepcode home the agent actually persisted under: `weepcode_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_weepcode_home() -> PathBuf {
    weepcode_config::weepcode_home()
}
...
struct PagerLeaderCluster {
    ...
    /// agent on the same WEEPCODE_HOME (two agents on one updates.jsonl is the
    ...
    _weepcode_home: TempDir,
}

impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(WEEPCODE_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let sock_path = weepcode_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("WEEPCODE_HOME", weepcode_home.path()),
            crate::test_util::EnvVarGuard::set("WEEPCODE_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("WEEPCODE_WEEPCODE_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("WEEPCODE_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("WEEPCODE_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("WEEPCODE_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("WEEPCODE_TRACE_UPLOAD", "false"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (weepcode_version), not
        ...
        // weepcode-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: weepcode-shell/tests/test_leader_soak.rs ("Real agent
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same WEEPCODE_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `WEEPCODE_LEADER_SOCKET`, flock held
    ...
    async fn client(&mut self, name: &str, reconnect: bool) -> ClusterClient {
        ...
        let (reconnector, status_rx) = if reconnect {
            ...
            let reconnector = LeaderReconnector::new(
                ...
                ClientMode::Stdio,
                LeaderEnvUrls {
                    weepcode_ws_url: String::new(),
                    weepcode_ws_origin: String::new(),
                },
                LeaderClientCapabilities {
                    client_version: Some("0.0.0-test".to_string()),
                    ..Default::default()
        ...
        if !self.authenticated {
            let _: acp::AuthenticateResponse = bounded(
                "authenticate",
                acp_send(
                    acp::AuthenticateRequest::new(acp::AuthMethodId::new("weepcode.api_key"))
