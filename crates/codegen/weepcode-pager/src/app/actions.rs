...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: weepcode_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    OpenTranscriptPager,
    /// Minimal mode (`weepcode --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: weepcode_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    ExecuteHooksAction(weepcode_hooks_plugins_types::HooksAction),
    /// Execute a plugins management action from the modal.
    ExecutePluginsAction(weepcode_hooks_plugins_types::PluginsAction),
    /// Execute a marketplace management action from the modal.
    ExecuteMarketplaceAction(weepcode_hooks_plugins_types::MarketplaceAction),
    ...
    AnnouncementsOpenCta(weepcode_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"weepcodenight"`, `"auto"`).
    SetTheme(String),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `weepcode dashboard`).
    OpenDashboard,
...
///
/// Both variants write to `~/.weepcode/config.toml` and route ACP
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: weepcode_workspace::foreign_sessions::EnabledForeignSessionSources,
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.weepcode/config.toml`.
    ...
    /// Persist a typed setting to `~/.weepcode/config.toml`. On failure,
    ...
    /// registry (`~/.weepcode/active_sessions.json`).
    ...
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`weepcode-build-0.1` when the
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<weepcode_workspace::session::git::RestoreDegree>,
    },
    ...
    SessionLoaded {
        ...
        /// `_meta["weepcode/runningPromptId"]`), present only when the session was
    ...
    SessionListLoaded {
        ...
        /// Degraded conversations lane (`_meta["weepcode/partial"]`), surfaced
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<weepcode_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<weepcode_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<weepcode_shell::util::changelog::ChangelogEntry>,
    },
    ...
    HooksListLoaded {
        agent_id: AgentId,
        result: Result<weepcode_hooks_plugins_types::HooksListResponse, String>,
    },
    /// Plugins list fetched from shell.
    PluginsListLoaded {
        agent_id: AgentId,
        result: Result<weepcode_hooks_plugins_types::PluginsListResponse, String>,
    },
    /// Hooks action completed.
    HooksActionResult {
        agent_id: AgentId,
        result: Result<weepcode_hooks_plugins_types::ActionOutcome, String>,
    },
    ...
    MarketplaceListLoaded {
        agent_id: AgentId,
        result: Result<weepcode_hooks_plugins_types::MarketplaceListResponse, String>,
    },
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        info: Box<weepcode_shell::session::SessionInfoResponse>,
        text: String,
    },
    ...
    AuthCopiedTimeout,
    DeepSearchResults {
        results: Vec<weepcode_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
