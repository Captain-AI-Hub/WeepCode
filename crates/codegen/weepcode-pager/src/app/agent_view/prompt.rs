...
impl AgentView {
    ...
    pub(in crate::app) fn handle_prompt_key(
            ...
            && self.prompt.prompt_suggestion_visible()
        {
            if key!(Tab).matches(key) || key!(Right).matches(key) {
                if self.prompt.accept_prompt_suggestion() {
                    ...
                    weepcode_telemetry::session_ctx::log_event(
                        weepcode_telemetry::events::PromptSuggestion {
                            action: weepcode_telemetry::events::PromptSuggestionAction::Accepted,
                            chars,
                            words,
                        },
                    );
                ...
                && self.prompt.text().is_empty()
            {
                ...
                weepcode_telemetry::session_ctx::log_event(
                    weepcode_telemetry::events::PromptSuggestion {
                        action: weepcode_telemetry::events::PromptSuggestionAction::Dismissed,
                        chars,
                        words,
                    },
                );
        ...
        //     (Bash/Remember: Backspace/Esc/Ctrl+W/U/C).
    ...
    pub(super) fn try_handle_esc_policy(&mut self, key: &KeyEvent) -> Option<InputOutcome> {
        ...
        // Bash/Remember mode on an empty-composer Esc before the
