                ...
                if let Err(msg) = self.prompt.insert_image(pasted) {
                    ...
                    if ctx.source.tip_showing() {
                        weepcode_telemetry::session_ctx::log_event(
                            weepcode_telemetry::events::ContextualTip {
...
pub(super) mod paste_key_tests {
    ...
    -> crate::views::question_view::QuestionViewState {
        let question =
            weepcode_tools::implementations::weepcode_build::ask_user_question::Question {
                question: "Pick one?".to_string(),
                options: vec![
                weepcode_tools::implementations::weepcode_build::ask_user_question::QuestionOption
    ...
    fn agent_completion_inserts_unreadable_file_url_as_path_text() {
        ...
        assert_eq!(
            agent.prompt.text(),
            "/definitely/missing/weepcode-primary-paste.png "
        );
        assert!(agent.prompt.images.is_empty());
    }
    #[test]
