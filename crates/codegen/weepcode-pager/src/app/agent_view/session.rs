...
impl AgentView {
    ...
    /// reconnect cursor and both dedup highwaters (ACP + WeepCode) when the id
    ...
    pub(crate) fn bind_session_id(&mut self, session_id: agent_client_protocol::SessionId) {
        if self.session.session_id.as_ref() != Some(&session_id) {
            ...
            self.last_applied_weepcode_event_seq = None;
        }
    ...
    pub fn new(session: AgentSession, scrollback: ScrollbackState) -> Self {
        ...
        let mut view = Self {
            ...
            last_applied_weepcode_event_seq: None,
    ...
    pub fn apply_full_context_info(&mut self, next: weepcode_shell::session::ContextInfo) {
        if self.chat_kind {
    ...
    pub fn apply_context_used(&mut self, used: u64, total: u64) {
        ...
        match self.context_state.as_mut() {
            Some(snap) => {
                ...
                snap.usage_pct = weepcode_token_estimation::usage_percentage_u8(used, snap.total);
                snap.free_tokens = weepcode_token_estimation::free_tokens(snap.total, used);
            }
            None => {
                self.context_state = Some(weepcode_shell::session::ContextInfo::from_notification(
                    used, total,
                ));
            }
        }
    ...
    /// Set the sharing-enabled flag on this view (gates the `/share`
    /// command-palette entry). Use this instead of mutating
    /// `sharing_enabled` directly when a new agent is created or a
    /// session is loaded.
