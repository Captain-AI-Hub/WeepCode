...
pub(crate) struct SessionReload {
    ...
    /// Live dedup highwaters (ACP + WeepCode) as of window open (same restore
    /// rationale).
    last_applied_event_seq: Option<u64>,
    last_applied_weepcode_event_seq: Option<u64>,
...
pub struct PluginCtaState {
    ...
    pub candidates: Vec<weepcode_hooks_plugins_types::MarketplacePluginEntry>,
...
pub struct AgentView {
    ...
    ///
    /// ACP stream only — the WeepCode stream keeps its own highwater
    /// ([`Self::last_applied_weepcode_event_seq`]) because the two streams are not
    /// delivered in one id order: ACP lines ride the agent's FIFO event
    /// pipeline while WeepCode lines are emitted direct-to-gateway, so a fresh WeepCode
    ...
    /// WeepCode-stream sibling of [`Self::last_applied_event_seq`] (see there for
    /// why the highwaters are split). Same drop rule, replay-exempt.
    pub last_applied_weepcode_event_seq: Option<u64>,
    /// Raw `eventId` of the most recent update APPLIED to this root session —
    /// replay or live, on both the ACP and WeepCode paths; dropped updates (dedup,
    ...
    pub context_state: Option<weepcode_shell::session::ContextInfo>,
    ...
    pub(crate) deferred_session_mode: Option<weepcode_tools::types::SessionMode>,
...
fn translate_project_select(
    ...
    use weepcode_telemetry::events::{ProjectPickerOutcome, ProjectPickerSelected};
    let project_dir_options = resolved_paths.len().saturating_sub(1);
    let emit = |outcome: ProjectPickerOutcome| {
        weepcode_telemetry::session_ctx::log_event(ProjectPickerSelected {
            outcome,
...
fn remember_mode_enabled() -> bool {
    let path = weepcode_tools::util::weepcode_home::weepcode_home().join("config.toml");
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &weepcode_tools::implementations::weepcode_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
...
pub(super) mod test_fixtures {
    ...
    /// upstream by the `_meta["weepcode/replayed"]` gate); it is pinned here so the
