...
/// `weepcode-shell/src/agent/handlers/session.rs::handle_roster_list`), which
...
mod tests {
    ...
    fn agent_entry() -> weepcode_shell::agent::roster::RosterEntry {
        use weepcode_shell::agent::roster as agent;
        agent::RosterEntry {
    ...
    fn roster_list_response_survives_result_envelope() {
        ...
        assert_eq!(e.model_id.as_deref(), Some("weepcode-4"));
        assert!(e.yolo);
