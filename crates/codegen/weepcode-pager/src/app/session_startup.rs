...
pub enum DeferredSessionStartup {
    ...
    /// Fresh plain WeepCode session whose first prompt resumes a foreign tool session.
    ForeignResume {
        tool: weepcode_workspace::foreign_sessions::ForeignSessionTool,
        native_id: String,
    },
}
...
pub fn fork_session_params(
    ...
    let source_cwd = weepcode_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = weepcode_shell::util::weepcode_home::weepcode_home().join("sessions");
    let encoded = weepcode_shell::util::weepcode_home::encode_cwd_dirname(&cwd_str);
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    weepcode_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = weepcode_shell::session::persistence::list_summaries(Some(cwd)).await?;
    let first = summaries.first().ok_or_else(|| {
        anyhow::anyhow!(
            "No session found for current directory. \
             Use 'weepcode' to start a new session."
        )
    })?;
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &weepcode_shell::agent::config::Config,
) -> std::sync::Arc<weepcode_shell::auth::AuthManager> {
    let auth = std::sync::Arc::new(weepcode_shell::auth::AuthManager::new(
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if weepcode_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = weepcode_shell::session::resolve_local_session(session_id, cwd) {
        tracing::info!(
    ...
    if let Some(original_cwd) = weepcode_shell::session::resolve_local_session_any_cwd(session_id) {
        tracing::info!(
    ...
    let agent_config = weepcode_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?;
    use weepcode_shell::agent::session_registry_client::SessionRegistryClient;
    use weepcode_shell::auth::{AuthManager, ensure_authenticated_or_noninteractive};
    use weepcode_shell::session::restore::restore_session_with_storage;
    use weepcode_shell::util::weepcode_home::weepcode_home;
    ...
    let storage_client = weepcode_shell::auth::credential_provider::build_storage_client_for_proxy(
    ...
    let progress: weepcode_shell::session::restore::ProgressCallback =
...
mod tests {
    ...
    fn intent_default_is_new_auto() {
        assert_eq!(
            parse(&["weepcode"]).session_startup_intent().unwrap(),
    ...
    fn intent_continue() {
        assert_eq!(
            parse(&["weepcode", "-c"]).session_startup_intent().unwrap(),
            SessionStartupIntent::Resume {
                session_id: None,
                most_recent_for_cwd: true,
            }
    ...
    fn intent_session_id_alone_is_new_with_id() {
        assert_eq!(
            parse(&["weepcode", "--session-id", "my-id"])
                .session_startup_intent()
                .unwrap(),
    ...
    fn intent_session_id_with_resume_without_fork_errors() {
        let err = parse(&["weepcode", "-r", "a", "-s", "b"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_fork_with_resume() {
        assert_eq!(
            parse(&["weepcode", "-r", "old", "--fork-session"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
    ...
    fn intent_fork_with_resume_and_new_id() {
        assert_eq!(
            parse(&["weepcode", "-r", "old", "--fork-session", "-s", "new"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
                source_session_id: Some("old".into()),
    ...
    fn intent_fork_alone_errors() {
        let err = parse(&["weepcode", "--fork-session"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_fork_with_worktree_errors() {
        let err = parse(&["weepcode", "-r", "a", "--fork-session", "-w"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_from_flags_matches_pager_args() {
        let args = parse(&["weepcode", "-r", "old", "--fork-session", "-s", "new"]);
    ...
    fn materialize_ctx_chat_mode_from_args() {
        assert!(!MaterializeCtx::from_pager_args(&parse(&["weepcode"])).chat_mode);
    }
