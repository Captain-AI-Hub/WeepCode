...
pub enum Command {
    /// Run WeepCode without the interactive UI
    Agent(Box<AgentArgs>),
    /// Show the configuration WeepCode discovers for this directory
    Inspect {
    ...
    Logout,
    /// Add an API provider / model (WeepCode's provider setup; no sign-in exists)
    #[command(alias = "login", alias = "config")]
    AddModel {
...
Examples:
  weepcode wrap docker exec -it my-container bash
  weepcode wrap kubectl exec -it my-pod -- bash

See ~/.weepcode/README.md for more information.
")]
    Wrap(WrapArgs),
    ...
    /// Not available in WeepCode (the release channel serves xAI binaries)
    Update {
    ...
    /// `WEEPCODE_WORKSPACE_COMMAND=1` to enable it locally for testing.
    #[command(hide = true)]
    ...
    /// `~/.weepcode/config.toml` or when the `WEEPCODE_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
...
/// Targets a running leader process by PID (used by `weepcode leader` / `weepcode workspace`).
#[derive(Debug, clap::Args, Clone, Default)]
pub struct LeaderTargetArgs {
    /// Leader process ID from `weepcode leader list`.
    #[arg(long)]
    pub pid: Option<u32>,
}
#[derive(Debug, clap::Args, Clone)]
...
pub struct AgentArgs {
    ...
    /// Override the public WeepCode API base URL.
    #[arg(long = "weepcode-api-base-url")]
    pub weepcode_api_base_url: Option<String>,
    /// Agent runtime mode
    #[command(subcommand)]
    ...
    pub fn canonical_plugin_dirs(&self) -> Vec<PathBuf> {
        self.plugin_dirs
            ...
            .filter_map(|p| match dunce::canonicalize(p) {
                ...
                Ok(_) => {
                    eprintln!(
                        "weepcode: --plugin-dir {}: not a directory; skipping",
                        p.display()
                    );
                    None
                }
                Err(e) => {
                    eprintln!("weepcode: --plugin-dir {}: {e}; skipping", p.display());
                    None
                }
            })
            .collect()
...
pub enum AgentCmd {
    ...
    Stdio,
    /// Run the agent headlessly over the WeepCode WebSocket relay
...
pub struct HeadlessArgs {
    #[arg(long = "weepcode-ws-origin")]
    pub weepcode_ws_origin: Option<String>,
    #[arg(long = "weepcode-ws-url")]
    pub weepcode_ws_url: Option<String>,
}
...
pub struct ServeArgs {
    ...
    #[arg(long, env = "WEEPCODE_AGENT_SECRET")]
...
fn version_with_channel() -> &'static str {
    ...
    V.get_or_init(|| {
        let label = weepcode_version::CHANNEL_LABEL;
        weepcode_version::display_version_with_commit(env!("VERSION_WITH_COMMIT"), label)
    })
}
...
pub struct PagerArgs {
    ...
    #[arg(long)]
    pub cwd: Option<PathBuf>,
    /// Use a custom leader socket path instead of the default `~/.weepcode/leader.sock`.
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    /// persists per-segment markdown to grep. Sets `WEEPCODE_COMPACTION_MODE`.
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            weepcode_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    /// Default for all `weepcode -p` runs is to wait (up to `--background-wait-timeout`)
    ...
    #[arg(long, env = "WEEPCODE_SANDBOX", value_name = "PROFILE")]
    ...
    /// Session-scoped only — does not write config. To default plain `weepcode` to
    /// minimal, set `[ui] screen_mode = "minimal"` in ~/.weepcode/config.toml.
    #[arg(long = "minimal")]
    pub minimal: bool,
    ...
    pub fullscreen: bool,
    /// Write sampling events to ~/.weepcode/logs/sampling.jsonl.
    #[arg(long = "log-sampling", env = "WEEPCODE_LOG_SAMPLING", hide = true)]
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `weepcode "fix the bug"` or `weepcode --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    /// explicit `--sandbox`/`WEEPCODE_SANDBOX` that differs from the saved profile is
    ...
    pub fn saved_resume_profile(&self) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                weepcode_shell::session::persistence::resumed_session_sandbox_profile(
                    Some(&id),
                    cwd,
                )
            }
            ResumeTarget::MostRecentForCwd => {
                weepcode_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<weepcode_sandbox::ProfileName>().ok()
                    != s.parse::<weepcode_sandbox::ProfileName>().ok() =>
            {
    ...
    /// whitespace. This is the `weepcode "<prompt>"` launch form; the headless
...
mod tests {
    ...
    fn version_flag_exits_zero() {
        let err = PagerArgs::try_parse_from(["weepcode", "--version"]).unwrap_err();
    ...
    fn version_short_flag_exits_zero() {
        let err = PagerArgs::try_parse_from(["weepcode", "-v"]).unwrap_err();
        assert_eq!(err.kind(), clap::error::ErrorKind::DisplayVersion);
        assert!(
            err.exit_code() == 0,
            "-v must exit 0; got {}",
    ...
    fn resume_target_classifies_flags() {
        assert_eq!(
            PagerArgs::try_parse_from(["weepcode"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["weepcode", "-c"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["weepcode", "--resume"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["weepcode", "--resume", "sess-1"])
                .unwrap()
                .resume_target(),
            ResumeTarget::SessionId("sess-1".to_string())
        );
        assert_eq!(
            PagerArgs::try_parse_from(["weepcode", "-s", "sess-2"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["weepcode", "-r", "old", "--fork-session"])
                .unwrap()
                .resume_target(),
    ...
    fn minimal_and_fullscreen_flags_conflict() {
        let args = PagerArgs::try_parse_from(["weepcode", "--minimal"]).unwrap();
        assert!(args.minimal && !args.fullscreen);
        let args = PagerArgs::try_parse_from(["weepcode", "--fullscreen"]).unwrap();
        assert!(args.fullscreen && !args.minimal);
        let err = PagerArgs::try_parse_from(["weepcode", "--minimal", "--fullscreen"]).unwrap_err();
    ...
    fn startup_sandbox_profile_no_resume() {
        assert_eq!(
            PagerArgs::try_parse_from(["weepcode", "--sandbox", "strict"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(Some("strict".to_string()))
        );
        assert_eq!(
            PagerArgs::try_parse_from(["weepcode", "--sandbox", ""])
                .unwrap()
                .startup_sandbox_profile(None),
    ...
    fn leader_socket_flag_parses_at_root() {
        let args = PagerArgs::try_parse_from(["weepcode", "--leader-socket", "/tmp/leader-x.sock"])
    ...
    fn leader_socket_flag_defaults_to_none() {
        let args = PagerArgs::try_parse_from(["weepcode"]).expect("bare weepcode parses");
        assert!(args.leader_socket.is_none());
    }
    #[test]
    fn leader_mgmt_list_info_kill_parse() {
        let list = PagerArgs::try_parse_from(["weepcode", "leader", "list", "--json"])
            .expect("weepcode leader list --json");
        assert!(matches!(
            list.command,
        ...
        let info = PagerArgs::try_parse_from(["weepcode", "leader", "info", "--pid", "42"])
            .expect("weepcode leader info --pid");
        assert!(matches!(
            info.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        let kill = PagerArgs::try_parse_from(["weepcode", "leader", "kill"])
            .expect("weepcode leader kill");
        assert!(matches!(
            kill.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        assert!(PagerArgs::try_parse_from(["weepcode", "leader", "profile"]).is_err());
    }
    #[test]
    fn debug_file_flag_parses_and_is_global() {
        let root = PagerArgs::try_parse_from(["weepcode", "--debug-file", "/tmp/fire.txt"])
        ...
        let sub =
            PagerArgs::try_parse_from(["weepcode", "agent", "stdio", "--debug-file", "/tmp/f.txt"])
    ...
    fn positional_prompt_seeds_interactive_session() {
        let args = PagerArgs::try_parse_from(["weepcode", "fix the bug"])
    ...
    #[test]
    fn bare_weepcode_has_no_initial_prompt() {
        let args = PagerArgs::try_parse_from(["weepcode"]).expect("bare weepcode parses");
    ...
    fn worktree_flag_and_initial_prompt_combine() {
        ...
        let c = PagerArgs::try_parse_from(["weepcode", "-w", "x"]).expect("-w x parses");
        assert_eq!(c.worktree.as_deref(), Some("x"));
        assert_eq!(c.initial_prompt(), None);
    }
    #[test]
    fn trust_flag_parses_on_pager_and_alias() {
        let bare = PagerArgs::try_parse_from(["weepcode"]).expect("bare weepcode parses");
        assert!(!bare.trust);
        let long = PagerArgs::try_parse_from(["weepcode", "--trust"]).expect("--trust parses");
        assert!(long.trust);
        let alias = PagerArgs::try_parse_from(["weepcode", "--trust-folder"])
            .expect("--trust-folder parses");
        assert!(alias.trust);
    }
    #[test]
    fn reasoning_effort_and_effort_alias_parse_same_field() {
        let long = PagerArgs::try_parse_from(["weepcode", "--reasoning-effort", "high"])
            .expect("--reasoning-effort parses");
        assert_eq!(long.reasoning_effort.as_deref(), Some("high"));
        let alias = PagerArgs::try_parse_from(["weepcode", "--effort", "high"])
    ...
    fn reasoning_effort_accepts_max_and_remapped_ids() {
        let max = PagerArgs::try_parse_from(["weepcode", "--effort", "max"]).expect("max parses");
        assert_eq!(max.reasoning_effort.as_deref(), Some("max"));
        let deep = PagerArgs::try_parse_from(["weepcode", "--reasoning-effort", "deep"])
            .expect("deep parses");
    ...
    fn agent_args_effort_alias_parses() {
        let args = PagerArgs::try_parse_from(["weepcode", "agent", "--effort", "max", "stdio"])
