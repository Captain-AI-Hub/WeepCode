...
/// Rate-limit errors: legacy free-usage codes use a generic fallback, otherwise
/// server detail is surfaced when present; empty detail uses an auth-aware
/// fallback (see [`format_rate_limited_user_message`]).
...
) -> (bool, Option<String>, Option<weepcode_workspace::session::git::RestoreDegree>) {
...
/// CANONICAL wire parser for `LoadSessionResponse._meta["weepcode/runningPromptId"]`.
///
...
pub(crate) fn parse_session_load_running_prompt_id(
    ...
    resp_meta
        .and_then(|m| m.get("weepcode/runningPromptId"))
        .and_then(|v| v.as_str())
        .map(String::from)
}
...
pub(crate) fn sanitize_user_error(raw: &str) -> String {
    if raw.contains(weepcode_fast_worktree::OUT_OF_DISK_CONTEXT)
        || raw.contains(weepcode_fast_worktree::ENOSPC_OS_MESSAGE)
    {
...
/// |-------|-----------|----------|--------------------------------|--------------------|
/// | false | false     | false    | `weepcode-build` (default)         | `false`            |
/// | false | true      | false    | `weepcode-build` (default)         | `false`            |
/// | false | false     | true     | `weepcode-build-ask-user`          | omitted (shell gate) |
/// | false | true      | true     | `weepcode-build-ask-user`          | omitted (shell gate) |
/// | true  | false     | false    | `weepcode-build-plan-no-subagents` | `false`            |
/// | true  | true      | false    | `weepcode-build-plan`              | `false`            |
/// | true  | false     | true     | `weepcode-build-plan-no-subagents` | omitted (shell gate) |
/// | true  | true      | true     | `weepcode-build-plan`              | omitted (shell gate) |
///
/// When [`Self::chat_mode`] is set (gateway light-frontend / `--chat`), Build
/// `agentProfile` injection is omitted (K12) and `_meta["weepcode/session"].kind`
...
impl SessionFlags {
    ...
    ///
    /// Returns `None` for the default `weepcode-build` profile (no `_meta`
    ...
    pub(super) fn agent_profile(&self) -> Option<&'static str> {
        ...
        match (self.plan_mode, self.subagents, self.ask_user) {
            (true, true, _) => Some("weepcode-build-plan"),
            (true, false, _) => Some("weepcode-build-plan-no-subagents"),
            (false, _, true) => Some("weepcode-build-ask-user"),
            (false, _, false) => None,
        }
    }
    ...
    /// `WEEPCODE_AGENT` — the env var chooses the *agent*, but the tool-strip is
    ...
    pub(super) fn to_meta(&self) -> Option<acp::Meta> {
        ...
        if self.chat_mode {
            if self.plan_mode || self.agent_override.is_some()
                || std::env::var("WEEPCODE_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
            {
                tracing::warn!(
        ...
        } else if std::env::var("WEEPCODE_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
        {} else if let Some(profile) = self.agent_profile() {
            meta.insert("agentProfile".into(), serde_json::json!(profile));
        }
        if self.chat_mode {
            meta.insert("weepcode/session".into(), serde_json::json!({ "kind" : "chat" }));
        }
        if !self.ask_user {
...
pub(super) const CHAT_FORBIDDEN_WORKSPACE_BIND_KEYS: &[&str] = &[
    "envId",
    "weepcode/cloud_server_id",
    "weepcode/cloud_existing_workspace",
];
/// Stamp `_meta["weepcode/session"].kind = "chat"` and strip Build `agentProfile` (K12).
pub(super) fn apply_chat_kind_meta(meta: &mut Option<acp::Meta>) {
    let obj = meta.get_or_insert_with(acp::Meta::new);
    obj.insert("weepcode/session".into(), serde_json::json!({ "kind" : "chat" }));
...
pub(super) fn extract_first_user_prompt(
    info: &weepcode_shell::session::info::Info,
) -> Option<String> {
    use std::io::BufRead;
    let history_path = weepcode_shell::session::persistence::session_dir(info)
...
pub(super) fn count_chat_history_stats(history_path: &Path) -> (usize, usize) {
    use std::io::BufRead;
    use weepcode_shell::sampling::{AssistantItem, ConversationItem, UserItem};
...
/// response's `_meta["weepcode/partial"]` envelope.
...
impl ConversationsPartial {
    ...
    pub(crate) fn picker_notice(self) -> &'static str {
        match self {
            Self::NoOauth => "Couldn't load your chats \u{2014} add a provider with /add-model",
            Self::Timeout | Self::Error => "Couldn't load conversations \u{2014} retry",
        }
    }
}
/// Read `_meta["weepcode/partial"]` from a session-list payload. `None` when the
...
pub(super) fn parse_session_list_partial(
    ...
    let partial = payload.get("_meta")?.get("weepcode/partial")?;
    ...
    entries
        ...
        .filter_map(|v| {
            ...
            let is_conversation = v
                .get("_meta")
                .and_then(|m| m.get("weepcode/session"))
            ...
            use weepcode_tools::implementations::skills::skill::extract_skill_display_text;
            let display = if let Some(ref fp) = first_prompt {
                ...
                let info_cwd = v
                    ...
                    .unwrap_or_default()
                    .to_string();
                let info = weepcode_shell::session::info::Info {
        ...
        .filter_map(|mut e| {
            ...
            if e.source == "remote"
                && weepcode_shell::session::resolve_local_session_any_cwd(&e.id)
                    .is_some()
            {
...
pub(crate) async fn persist_setting(
    ...
    match key {
        "compact_mode" => {
            ...
            weepcode_shell::util::config::set_compact_mode(b)
                .await
        ...
        "show_timestamps" => {
            ...
            weepcode_shell::util::config::set_show_timestamps(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_timeline" => {
            ...
            weepcode_shell::util::config::set_show_timeline(b)
                .await
                .map_err(|e| e.to_string())
        }
        "simple_mode" => {
            ...
            weepcode_shell::util::config::set_simple_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.undo" => {
            ...
            weepcode_shell::util::config::set_contextual_hint_undo(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.plan_mode" => {
            ...
            weepcode_shell::util::config::set_contextual_hint_plan_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.image_input" => {
            ...
            weepcode_shell::util::config::set_contextual_hint_image_input(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.send_now" => {
            ...
            weepcode_shell::util::config::set_contextual_hint_send_now(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.small_screen" => {
            ...
            weepcode_shell::util::config::set_contextual_hint_small_screen(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.word_select" => {
            ...
            weepcode_shell::util::config::set_contextual_hint_word_select(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.ssh_wrap" => {
            ...
            weepcode_shell::util::config::set_contextual_hint_ssh_wrap(b)
                .await
                .map_err(|e| e.to_string())
        }
        "theme" => {
            ...
            weepcode_shell::util::config::set_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_dark_theme" => {
            ...
            weepcode_shell::util::config::set_auto_dark_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_light_theme" => {
            ...
            weepcode_shell::util::config::set_auto_light_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "default_model" => {
            ...
            weepcode_shell::util::config::set_default_model(s)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_speed" => {
            ...
            weepcode_shell::util::config::set_scroll_speed(i)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_mode" => {
            ...
            weepcode_shell::util::config::set_scroll_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "invert_scroll" => {
            ...
            weepcode_shell::util::config::set_invert_scroll(b)
                .await
                .map_err(|e| e.to_string())
        }
        "display_refresh_auto_cadence" => {
            ...
            weepcode_shell::util::config::set_display_refresh_auto_cadence(b)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_lines" => {
            ...
            weepcode_shell::util::config::set_scroll_lines(i)
                .await
                .map_err(|e| e.to_string())
        }
        "default_selected_permission" => {
            ...
            weepcode_shell::util::config::set_default_selected_permission(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "cancel_subagents_on_turn_cancel" => {
            ...
            weepcode_shell::util::config::set_cancel_subagents_on_turn_cancel(
                    s.to_string(),
                )
                .await
                .map_err(|e| e.to_string())
        ...
        "vim_mode" => {
            ...
            weepcode_shell::util::config::set_vim_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "remember_tool_approvals" => {
            ...
            weepcode_shell::util::config::set_remember_tool_approvals(b)
                .await
                .map_err(|e| e.to_string())
        }
        "toolset.ask_user_question.timeout_enabled" => {
            ...
            weepcode_shell::util::config::set_ask_user_question_timeout_enabled(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_thinking_blocks" => {
            ...
            weepcode_shell::util::config::set_show_thinking_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "group_tool_verbs" => {
            ...
            weepcode_shell::util::config::set_group_tool_verbs(b)
                .await
                .map_err(|e| e.to_string())
        }
        "collapsed_edit_blocks" => {
            ...
            weepcode_shell::util::config::set_collapsed_edit_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "prompt_suggestions" => {
            ...
            weepcode_shell::util::config::set_prompt_suggestions(b)
                .await
                .map_err(|e| e.to_string())
        }
        "keep_text_selection" => {
            ...
            weepcode_shell::util::config::set_keep_text_selection(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        ...
        "render_mermaid" => {
            ...
            weepcode_shell::util::config::set_render_mermaid(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "hunk_tracker_mode" => {
            ...
            weepcode_shell::util::config::set_hunk_tracker_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "screen_mode" => {
            ...
            weepcode_shell::util::config::set_screen_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "max_thoughts_width" => {
            ...
            weepcode_shell::util::config::set_max_thoughts_width(i)
                .await
                .map_err(|e| e.to_string())
        }
        "show_tips" => {
            ...
            weepcode_shell::util::config::set_show_tips(b)
                .await
                .map_err(|e| e.to_string())
        }
        "auto_update" => {
            ...
            weepcode_shell::util::config::set_auto_update(b)
                .await
                .map_err(|e| e.to_string())
        }
        "fork_secondary_model" => {
            ...
            weepcode_shell::util::config::set_fork_secondary_model(s)
                .await
                .map_err(|e| e.to_string())
        }
...
pub(crate) async fn persist_permission_mode_and_notify(
    ...
    let disk_result = weepcode_shell::util::config::update_config(|cfg| {
...
pub(super) fn marketplace_outcome_succeeded(
    outcome: &weepcode_hooks_plugins_types::ActionOutcome,
) -> bool {
    outcome.status == weepcode_hooks_plugins_types::OutcomeStatus::Success
}
...
/// serializes (`weepcode_shell::extensions::task::KillTaskResponse` +
/// `weepcode_shell::session::result::ExtMethodResult`) so the contract stays
...
pub(super) fn parse_kill_outcome(
    resp: &str,
) -> Option<weepcode_tools::types::KillOutcome> {
    use weepcode_shell::extensions::task::KillTaskResponse;
    use weepcode_shell::session::result::ExtMethodResult;
...
pub(super) fn parse_subagent_kill_outcome(resp: &str) -> SubagentKillOutcome {
    use weepcode_shell::extensions::task::{
...
/// A blocking flock on the shared, possibly-network `~/.weepcode` lock must never
...
pub(super) fn unregister_active_session_best_effort_in(
    ...
    match weepcode_shell::active_sessions::try_unregister_in(root, session_id) {
        Ok(true) => {}
        Ok(false) => {
            tracing::debug!(
