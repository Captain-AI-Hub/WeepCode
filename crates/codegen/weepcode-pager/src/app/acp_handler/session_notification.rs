use super::*;
use weepcode_shell::sampling::error::format_rate_limited_user_message;
...
/// Replay gate shared by the ACP and WeepCode session-update paths. Returns `true`
...
pub(super) fn handle_session_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    match &session_notif.update {
        WeepcodeSessionUpdate::TaskBackgrounded { .. } => {
            return handle_task_backgrounded(notif, app);
        }
        WeepcodeSessionUpdate::TaskCompleted { .. } => {
            return handle_task_completed(notif, app);
        }
        WeepcodeSessionUpdate::ScheduledTaskCreated { .. } => {
            return handle_scheduled_task_created(notif, app);
        }
        WeepcodeSessionUpdate::ScheduledTaskDeleted { .. } => {
        ...
        })
    {
        tracing::debug!(
            ...
            last_applied = agent.last_applied_weepcode_event_seq,
            "weepcode/session update DROPPED by dedup highwater (event_seq <= last_applied)"
        );
        return false;
    }
    ...
    let changed = match session_notif.update {
        ref update @ (WeepcodeSessionUpdate::AutoCompactStarted { .. }
        | WeepcodeSessionUpdate::AutoCompactCompleted { .. }
        | WeepcodeSessionUpdate::AutoCompactFailed { .. }
        | WeepcodeSessionUpdate::AutoCompactCancelled { .. }
        | WeepcodeSessionUpdate::RetryState(_)
        | WeepcodeSessionUpdate::ImageDropped { .. }
        | WeepcodeSessionUpdate::MemoryFlushCompleted { .. }
        | WeepcodeSessionUpdate::MemoryDreamCompleted { .. }
        | WeepcodeSessionUpdate::MemorySessionSaved { .. }) => {
            ...
            if let WeepcodeSessionUpdate::AutoCompactCompleted { tokens_after, .. } = update {
                refresh_context_used(agent, *tokens_after);
                agent.todo.update_todos(Vec::new());
            }
            changed
        }
        WeepcodeSessionUpdate::ImageCompressed {
            ref images,
            ref message,
        } => apply_image_compressed(agent, images, message),
        WeepcodeSessionUpdate::TurnCompleted {
            prompt_id,
            stop_reason,
            agent_result,
            ..
        ...
        WeepcodeSessionUpdate::HookAnnotation { message } => {
        ...
        WeepcodeSessionUpdate::HookExecution {
            event_name,
            ...
            let hook_entries: Vec<HookRunEntry> = runs
                ...
                .map(|r| {
                    let status = match r.status {
                        weepcode_shell::extensions::notification::HookRunStatusDto::Success {
                            elapsed_ms,
                        } => HookRunStatus::Success {
                            elapsed: std::time::Duration::from_millis(elapsed_ms),
                        },
                        weepcode_shell::extensions::notification::HookRunStatusDto::Skipped => {
                            HookRunStatus::Skipped
                        }
                        weepcode_shell::extensions::notification::HookRunStatusDto::Failed {
                            error,
                            elapsed_ms,
        ...
        WeepcodeSessionUpdate::HooksChanged {
            hooks,
            project_trusted,
            load_errors,
        } => {
            if let Some(ref mut modal) = agent.extensions_modal {
                ...
                modal.hooks_data =
                    TabDataState::Loaded(weepcode_hooks_plugins_types::HooksListResponse {
                        hooks,
                        project_trusted,
                        load_errors,
                    });
        ...
        WeepcodeSessionUpdate::PluginsChanged { plugins } => {
            if let Some(ref mut modal) = agent.extensions_modal {
                use crate::views::extensions_modal::TabDataState;
        ...
        WeepcodeSessionUpdate::SessionSummaryGenerated { session_summary } => {
            agent.generated_session_title =
                Some(crate::util::decode_html_entities(&session_summary).into_owned());
            true
        }
        WeepcodeSessionUpdate::SessionRecap { summary, auto } => {
        ...
        WeepcodeSessionUpdate::SessionRecapUnavailable => {
            if meta.is_replay {
                false
        ...
        WeepcodeSessionUpdate::ModelAutoSwitched {
            previous_model_id,
            new_model_id,
            reason,
        } => {
        ...
        WeepcodeSessionUpdate::ModelChanged {
            model_id,
            reasoning_effort,
        } => {
            ...
            use weepcode_shell::sampling::types::ReasoningEffort;
            let new_model_id = acp::ModelId::new(model_id.clone());
            if !agent.session.models.available.contains_key(&new_model_id) {
                if weepcode_shell::agent::chat_modes::process_chat_mode_enabled() {
        ...
        WeepcodeSessionUpdate::MemoryFiles { files } => {
        ...
        WeepcodeSessionUpdate::InteractionResolved { tool_call_id } => {
...
/// Handle an WeepCode session notification that targets a child (subagent) session.
///
...
pub(super) fn handle_child_session_notification(
    ...
    match update {
        WeepcodeSessionUpdate::AutoCompactStarted { .. }
        | WeepcodeSessionUpdate::AutoCompactCompleted { .. }
        | WeepcodeSessionUpdate::AutoCompactFailed { .. }
        | WeepcodeSessionUpdate::AutoCompactCancelled { .. }
        | WeepcodeSessionUpdate::RetryState(_) => {
            let compact_tokens = match &update {
                WeepcodeSessionUpdate::AutoCompactCompleted { tokens_after, .. } => {
                    Some(*tokens_after)
                }
                _ => None,
            };
                ...
                && let Some(info) = agent.subagent_sessions.get_mut(child_sid)
            {
                ...
                if let Some(cw) = info.context_window_tokens.filter(|&cw| cw > 0) {
                    info.context_usage_pct = Some(weepcode_token_estimation::usage_percentage_u8(
                        tokens_after,
                        cw,
                    ));
                }
        ...
        ref update @ (WeepcodeSessionUpdate::MemoryFlushCompleted { .. }
        | WeepcodeSessionUpdate::MemoryDreamCompleted { .. }
        | WeepcodeSessionUpdate::MemorySessionSaved { .. }) => {
            if let Some(child_view) = agent.subagent_views.get_mut(child_sid) {
                apply_session_event(
...
pub(super) fn apply_session_event(
    ...
    match update {
        WeepcodeSessionUpdate::AutoCompactStarted { percentage, .. } => {
        ...
        WeepcodeSessionUpdate::AutoCompactFailed { error } => {
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[weepcode_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
...
pub(super) fn apply_retry_state(
    retry: &weepcode_shell::extensions::notification::RetryState,
    session: &mut AgentSession,
    scrollback: &mut crate::scrollback::state::ScrollbackState,
    is_api_key_auth: bool,
) {
        ...
        RetryState::Exhausted {
            ...
            if *rate_limited {
                weepcode_telemetry::session_ctx::log_event(
                    weepcode_telemetry::events::RateLimitHit {
                        model_id: session
                            .models
                            .current
                            .as_ref()
...
pub(super) fn detect_plan_mode_change(update: &acp::SessionUpdate, agent: &mut AgentView) -> bool {
    use weepcode_tools::types::SessionMode;
