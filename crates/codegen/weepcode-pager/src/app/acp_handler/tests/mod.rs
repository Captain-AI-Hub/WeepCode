...
use weepcode_shell::extensions::notification::RetryState;
use weepcode_shell::extensions::notification::SessionUpdate as WeepcodeSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> weepcode_shell::extensions::notification::ImageCompressedEntry {
    weepcode_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn make_exit_plan_ext(
    ...
    weepcode_acp_lib::AcpArgs<acp::ExtRequest>,
    tokio::sync::oneshot::Receiver<weepcode_acp_lib::AcpResult<acp::ExtResponse>>,
) {
...
pub(super) fn make_exit_plan_ext_with_tool_call_id(
    ...
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    let request = acp::ExtRequest::new("weepcode/exit_plan_mode", raw.into());
    let (tx, rx) = tokio::sync::oneshot::channel();
    (
        weepcode_acp_lib::AcpArgs {
            request,
            response_tx: tx,
        },
        rx,
...
pub(super) fn send_tool_call_update(
    ...
    handle(
        AcpClientMessage::SessionNotification(weepcode_acp_lib::AcpArgs {
...
pub(super) fn make_inject_notif(payload: &serde_json::Value) -> acp::ExtNotification {
    ...
    acp::ExtNotification::new(
        "weepcode/scheduled_task_inject_prompt",
...
pub(super) fn make_fired_notif(
    ...
    let notif = SessionNotification {
        ...
        update: WeepcodeSessionUpdate::ScheduledTaskFired {
            task_id: task_id.into(),
            prompt: prompt.into(),
    ...
    acp::ExtNotification::new("weepcode/scheduled_task_fired", std::sync::Arc::from(raw))
}
...
pub(super) fn critical_announcement(
    id: &str,
) -> weepcode_announcements::RemoteAnnouncement {
    weepcode_announcements::RemoteAnnouncement {
        id: Some(id.into()),
...
pub(super) fn announcements_update_notif(
    r#gen: u64,
    announcements: &[weepcode_announcements::RemoteAnnouncement],
) -> acp::ExtNotification {
    acp::ExtNotification::new(
        "weepcode/announcements/update",
        std::sync::Arc::from(
            serde_json::value::to_raw_value(
                    &serde_json::json!(
...
pub(super) fn make_created_ext_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: WeepcodeSessionUpdate::ScheduledTaskCreated {
            task_id: task_id.into(),
            prompt: prompt.into(),
            human_schedule: human_schedule.into(),
            next_fire_at: next_fire_at.map(str::to_string),
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("weepcode/scheduled_task_created", std::sync::Arc::from(raw))
}
pub(super) fn make_deleted_ext_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: WeepcodeSessionUpdate::ScheduledTaskDeleted {
            task_id: task_id.into(),
        },
        meta: None,
    };
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("weepcode/scheduled_task_deleted", std::sync::Arc::from(raw))
}
...
pub(super) fn weepcode_model_switch_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: WeepcodeSessionUpdate::ModelAutoSwitched {
...
pub(super) fn weepcode_unhandled_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: WeepcodeSessionUpdate::MemoryFlushStarted,
        meta: Some(serde_json::json!({ "eventId" : event_id })),
    };
    acp::ExtNotification::new(
        "weepcode/session/update",
...
pub(super) fn prompt_complete_ext(session_id: &str) -> acp::ExtNotification {
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    acp::ExtNotification::new("weepcode/session/prompt_complete", std::sync::Arc::from(raw))
}
...
pub(super) fn weepcode_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
    stop_reason: &str,
    is_replay: bool,
    ...
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: WeepcodeSessionUpdate::TurnCompleted {
...
pub(super) fn weepcode_wake_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
...
pub(super) fn weepcode_hook_execution_notif_for_prompt(
    session_id: &str,
    event_name: &str,
    prompt_id: Option<&str>,
    is_replay: bool,
) -> acp::ExtNotification {
    use weepcode_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: WeepcodeSessionUpdate::HookExecution {
...
pub(super) fn weepcode_hook_execution_notif(
    session_id: &str,
    event_name: &str,
    is_replay: bool,
) -> acp::ExtNotification {
    weepcode_hook_execution_notif_for_prompt(session_id, event_name, None, is_replay)
}
...
pub(super) fn interjection_ext_with_id(
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("weepcode/session/interjection", std::sync::Arc::from(raw))
}
...
/// Build an `ExtNotification` envelope with an explicit WeepCode session method.
pub(super) fn make_ext_session_notification_with_method(
    ...
    AcpClientMessage::ExtNotification(weepcode_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    })
}
...
pub(super) fn test_subagent_spawned(
    parent_sid: &str,
    child_sid: &str,
) -> WeepcodeSessionUpdate {
    WeepcodeSessionUpdate::SubagentSpawned {
...
pub(super) fn test_subagent_finished(child_sid: &str) -> WeepcodeSessionUpdate {
...
pub(super) fn test_subagent_finished_with_wake(
    ...
    WeepcodeSessionUpdate::SubagentFinished {
        subagent_id: child_sid.into(),
        child_session_id: child_sid.into(),
...
pub(super) fn test_subagent_progress(
    ...
    WeepcodeSessionUpdate::SubagentProgress {
        subagent_id: child_sid.into(),
        parent_session_id: parent_sid.into(),
        child_session_id: child_sid.into(),
        duration_ms: 100,
...
/// Shared temp `WEEPCODE_HOME` for disk-replay tests. `weepcode_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `WEEPCODE_HOME`
...
pub(super) fn replay_disk_test_home() -> &'static std::path::Path {
    ...
    HOME.get_or_init(|| {
            ...
            unsafe {
                std::env::set_var("WEEPCODE_HOME", tmp.path());
            }
            tmp
        })
        .path()
}
/// Runs `f` with a thread-local weepcode home override so disk replay tests do not
/// depend on process-wide `weepcode_home()` cache order when the full suite runs.
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    let home = replay_disk_test_home();
    crate::app::subagent::set_replay_weepcode_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_weepcode_home_for_tests(None);
    out
}
pub(super) fn write_child_updates_jsonl(
    ...
    let sessions_dir = weepcode_home
        .join("sessions")
...
pub(super) fn dispatch_goal_update(
    ...
    handle(
        AcpClientMessage::ExtNotification(weepcode_acp_lib::AcpArgs {
            request: acp::ExtNotification::new("weepcode/session_notification", raw.into()),
            response_tx: tx,
        }),
        app,
    )
...
pub(super) fn make_permission_message(
    ...
    let msg = AcpClientMessage::RequestPermission(weepcode_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    });
    (msg, rx)
...
pub(super) fn interaction_resolved_ext(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: WeepcodeSessionUpdate::InteractionResolved {
            tool_call_id: tool_call_id.into(),
        },
        meta: None,
    };
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("weepcode/session_notification", std::sync::Arc::from(raw))
}
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = weepcode_workspace::session::git::GitHeadChanged {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("weepcode/git_head_changed", std::sync::Arc::from(raw))
}
pub(super) fn make_task_backgrounded_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: WeepcodeSessionUpdate::TaskBackgrounded {
            tool_call_id: tool_call_id.into(),
            task_id: task_id.into(),
            command: command.into(),
            cwd: "/tmp".into(),
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("weepcode/task_backgrounded", std::sync::Arc::from(raw))
}
...
pub(super) fn make_replayed_task_backgrounded_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("weepcode/session/update", std::sync::Arc::from(raw))
}
...
pub(super) fn send_late_bg_detection(app: &mut AppView, tc_id: &str) {
    use serde_json::json;
    use weepcode_tools::types::output::{BashOutput, ToolOutput};
    let agent = app.agents.get_mut(&AgentId(0)).unwrap();
    let meta = crate::acp::meta::NotificationMeta::default();
    let bash = BashOutput {
...
pub(super) fn task_completed_notif(
    ...
    use weepcode_tools::types::TaskSnapshot;
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: WeepcodeSessionUpdate::TaskCompleted {
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("weepcode/task_completed", std::sync::Arc::from(raw))
}
pub(super) fn make_monitor_event_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: WeepcodeSessionUpdate::MonitorEvent {
            task_id: task_id.into(),
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("weepcode/monitor_event", std::sync::Arc::from(raw))
}
...
pub(super) fn make_models_update_notif(
    ...
    acp::ExtNotification::new("weepcode/models/update", std::sync::Arc::from(raw))
}
...
pub(super) fn model_changed_ext(
    ...
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: WeepcodeSessionUpdate::ModelChanged {
...
pub(super) fn make_mcp_init_progress_notif(
    ...
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    acp::ExtNotification::new("weepcode/mcp/init_progress", std::sync::Arc::from(raw))
}
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: weepcode_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use weepcode_shell::extensions::mcp::{
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("weepcode/mcp/server_status", std::sync::Arc::from(raw))
}
...
pub(super) fn make_servers_updated_notif() -> acp::ExtNotification {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("weepcode/mcp/servers_updated", std::sync::Arc::from(raw))
}
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = weepcode_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
        server_name: "weepcode_com_linear".to_string(),
        tools: Vec::new(),
    };
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("weepcode/mcp/tools_changed", std::sync::Arc::from(raw))
}
...
pub(super) fn make_tools_changed_notif_pre_h2() -> acp::ExtNotification {
    let payload = serde_json::json!({ "serverName" : "weepcode_com_linear", "tools" : [] });
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("weepcode/mcp/tools_changed", std::sync::Arc::from(raw))
}
...
pub(super) fn make_mcp_initialized_notif(session_id: &str) -> acp::ExtNotification {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("weepcode/mcp_initialized", std::sync::Arc::from(raw))
}
