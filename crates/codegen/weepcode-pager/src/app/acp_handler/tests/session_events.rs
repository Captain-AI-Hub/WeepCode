#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn apply_compaction_started_sets_activity() {
        ...
        let update = WeepcodeSessionUpdate::AutoCompactStarted {
            tokens_used: 90000,
    ...
    fn apply_image_dropped_pushes_scrollback_block() {
        ...
        let update = WeepcodeSessionUpdate::ImageDropped {
            notes: notes.clone(),
        };
    ...
    fn retry_exhausted_rate_limited_empty_reason_uses_oauth_fallback() {
        use weepcode_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_OAUTH;
    ...
    #[test]
    fn retry_exhausted_rate_limit_surfaces_server_message() {
        let rpm = RetryState::Exhausted {
            attempts: 2,
            reason: "The service is temporarily at capacity. Please retry shortly.".into(),
            is_rate_limited: true,
        };
    ...
    fn apply_retry_state_failed_clears_in_flight_prompt() {
        ...
        assert!(
            session.in_flight_prompt.is_none(),
            "in_flight_prompt must be cleared for ordinary failures"
        );
    }
    ...
    /// Legacy WebLogin auth keeps its verbose message (with `weepcode logout` /
    /// `weepcode login` guidance), not the generic re-auth prompt.
    #[test]
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) ... deprecated authentication method (WebLogin) ... \
                          run `weepcode logout` then `weepcode login`"
                    .into(),
            },
            &mut session,
            &mut scrollback, false);
    ...
    fn apply_compaction_completed_defers_message_until_turn_end() {
        ...
        let update = WeepcodeSessionUpdate::AutoCompactCompleted {
    ...
    fn apply_unhandled_event_returns_false() {
        let mut session = make_session(Some("s1"));
        let mut scrollback = ScrollbackState::new();
        let update = WeepcodeSessionUpdate::MemoryFlushStarted;
        assert!(!apply_session_event(&update, &mut session, &mut scrollback, false));
    }
    ...
    fn child_compact_started_does_not_reset_context_used() {
        ...
        child_view.context_state = Some(weepcode_shell::session::ContextInfo::from_notification(
            90_000, 131_072,
        ));
        agent
            .subagent_views
