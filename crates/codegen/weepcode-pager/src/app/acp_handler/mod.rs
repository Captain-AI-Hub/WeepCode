...
//! and WeepCode session extension notifications (`x.ai/session_notification` and
...
use weepcode_acp_lib::AcpClientMessage;

use super::actions::Effect;
use weepcode_shell::extensions::notification::{
    SessionNotification, SessionUpdate as WeepcodeSessionUpdate, is_reauthable_failure,
};
use weepcode_shell::tools::todo::todo_item_from_plan_entry;
use weepcode_workspace::permission::bash_command_splitting::BashCommandHighlights;
        ...
        AcpClientMessage::SessionNotification(notif) => {
            ...
            let affected = match find_session_match(app, &notif.request.session_id) {
                Some(SessionMatch::Root(id)) => {
                    ...
                    // time and drained FIFO. The WeepCode stream is direct-emitted
                    // and keeps a SEPARATE highwater (see the WeepCode dedup in
...
/// Handle an WeepCode extension notification.
///
...
fn handle_ext_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    match notif.method.as_ref() {
        ...
        "weepcode/follow_ups" => handle_follow_ups(notif, app),
        "weepcode/task_backgrounded" => handle_task_backgrounded(notif, app),
        "weepcode/task_completed" => handle_task_completed(notif, app),
        "weepcode/models/update" => handle_models_update(notif, app),
        "weepcode/settings/update" => handle_settings_update(notif, app),
        "weepcode/sessions/changed" => handle_sessions_changed(notif, app),
        "weepcode/queue/changed" => handle_queue_changed(notif, app),
        ...
        "weepcode/session/prompt_complete" => handle_prompt_complete(notif, app),
        "weepcode/session/interjection" => handle_interjection(notif, app),
        "weepcode/monitor_event" => handle_monitor_event(notif, app),
        "weepcode/scheduled_task_created" => handle_scheduled_task_created(notif, app),
        "weepcode/scheduled_task_fired" => handle_scheduled_task_fired(notif, app),
        "weepcode/scheduled_task_deleted" => handle_scheduled_task_deleted(notif, app),
        "weepcode/scheduled_task_inject_prompt" => handle_scheduled_task_inject_prompt(notif, app),
        "weepcode/announcements/update" => handle_announcements_update(notif, app),
        "weepcode/git_head_changed" => handle_git_head_changed(notif, app),
        "weepcode/mcp/init_progress" => handle_mcp_init_progress(notif, app),
        "weepcode/mcp/tools_changed" | "weepcode/mcp_initialized" => {
            handle_mcp_tools_changed(notif, app)
        }
        "weepcode/mcp/server_status" if push_server_status_enabled() => {
            handle_mcp_server_status(notif, app)
        }
        "weepcode/mcp/servers_updated" => handle_mcp_servers_updated(notif, app),
        _ => false,
    }
}
...
fn handle_ext_method(ext: weepcode_acp_lib::AcpArgs<acp::ExtRequest>, app: &mut AppView) -> bool {
    match ext.request.method.as_ref() {
        "weepcode/ask_user_question" => handle_ask_user_question(ext, app),
        "weepcode/exit_plan_mode" => handle_exit_plan_mode(ext, app),
        unknown => {
