...
/// `WEEPCODE_MCP_PUSH_SERVER_STATUS=0` to disable.
pub(super) fn push_server_status_enabled() -> bool {
    use std::sync::OnceLock;
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        weepcode_shell::util::config::resolve_mcp_push_server_status(
...
/// `weepcode-shell/src/session/acp_session.rs` — toggle-tool ~L6661,
...
///    [`weepcode_shell::extensions::mcp::McpToolsChanged`] struct), and
...
pub(super) fn handle_mcp_tools_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if method == "weepcode/mcp_initialized"
...
/// ([`weepcode_shell::extensions::mcp::McpServerStatusPayload`] +
/// [`weepcode_shell::extensions::mcp::McpServerStatus`]) instead of
...
pub(super) fn handle_mcp_server_status(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    use weepcode_shell::extensions::mcp::{McpServerStatus, McpServerStatusPayload, McpToolEntry};
        ...
        match serde_json::from_value::<Vec<McpToolEntry>>(raw) {
            ...
            Err(e) => {
                tracing::warn!(
                    server = %payload.name,
                    error = %e,
                    "weepcode/mcp/server_status: tools field present but not Vec<McpToolEntry>; status still applied"
                );
                None
            }
        }
...
/// on config reload (`crates/codegen/weepcode-shell/src/agent/mvp_agent.rs`
