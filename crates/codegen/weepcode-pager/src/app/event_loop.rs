...
/// [`decide`](weepcode_workspace::folder_trust::decide) precedence.
///
...
pub(crate) async fn run(
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("WEEPCODE_LOG_SAMPLING", "1") };
    }
    ...
    let launch_effective_ui = weepcode_shell::config::load_effective_config()
        .ok()
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(weepcode_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = weepcode_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match weepcode_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.sharing_enabled = remote_settings
        ...
        .unwrap_or(false);
    app.plugin_cta_enabled = weepcode_config::env_bool("WEEPCODE_PLUGIN_CTA")
        .or_else(|| remote_settings.as_ref().and_then(|s| s.plugin_cta))
        .unwrap_or(false);
    // Voice is applied after auth_meta so API-key detection is accurate.
    app.session_picker_grouped = std::env::var("WEEPCODE_SESSION_PICKER_GROUPED")
        .ok()
        ...
        .or_else(|| {
            weepcode_shell::config::load_effective_config()
                .ok()
    ...
    if needs_interactive_login {
        ...
        if connection.needs_login {
            ...
            let weepcode_com = connection
                .auth_methods
                .iter()
                .find(|m| m.id().0.as_ref() == "grok.com");
            if let Some(method) = weepcode_com {
    ...
    let post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    weepcode_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<weepcode_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == weepcode_shell::agent::auth_method::WEEPCODE_API_KEY_METHOD_ID
        });
    }

    // Load persisted per-ID hidden state
    app.hidden_announcement_ids = weepcode_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = weepcode_shell::config::load_merged_requirements();
    let user_config = weepcode_shell::config::load_from_disk().ok();
    let managed_config = weepcode_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match weepcode_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = weepcode_shell::agent::config::resolve_compat_sessions_from_raw(
    ...
    app.zdr_access_enabled = weepcode_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        weepcode_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        weepcode_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        weepcode_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    {
        use weepcode_shell::util::config::{resolve_announcements, resolve_tips};
        ...
        app.active_announcements = weepcode_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            ...
            app.tip = weepcode_shell::util::tips::pick_and_advance(&app.tips, &weepcode_home);
        }
    }

    let hints = weepcode_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.weepcode/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        weepcode_shell::util::config::clamped_display_permission_mode(
            weepcode_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        weepcode_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = weepcode_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = weepcode_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // WEEPCODE_SCROLL_SPEED/_MODE/_LINES + WEEPCODE_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    // Initial prompt from the CLI positional (`weepcode "fix the bug"`). When
    ...
    // `weepcode dashboard` startup: open the dashboard view immediately. The
    // CLI subcommand wrote a `WEEPCODE_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("WEEPCODE_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.weepcode/config.toml [ui]`
                // rather than `~/.weepcode/pager.toml`. The watcher only knows
...
pub(crate) fn load_initial_ui_config() -> weepcode_shell::agent::config::UiConfig {
    use weepcode_shell::agent::config::UiConfig;
    let Ok(root) = weepcode_shell::config::load_effective_config() else {
...
fn coalesce_rapid_keys(events: Vec<Event>) -> Vec<Event> {
    ...
    let events: Vec<Event> = events
        .into_iter()
        .filter(|ev| !matches!(ev, Event::Key(ke) if ke.kind == KeyEventKind::Release))
        .collect();
