...
use weepcode_acp_lib::AcpResult;
use weepcode_markdown::StreamingMarkdownRenderer;
pub use weepcode_tools::implementations::weepcode_build::ask_user_question::{
...
pub enum LocalQuestionKind {
    ...
    AgentTypeMismatch {
        ...
        effort: Option<weepcode_shell::sampling::types::ReasoningEffort>,
    },
}
...
pub struct QuestionViewState {
    ...
    /// locally-driven questions that only offer fixed options with no
    /// free-text fallback.
...
impl QuestionViewState {
    ...
    /// questions have no freeform row, so `InputMode` must be unreachable.
    /// Callers gate on `no_freeform` / [`Self::is_on_freeform_row`] too;
...
impl QuestionViewState {
    ...
    pub fn build_accepted_response(
        &self,
    ) -> weepcode_tools::implementations::weepcode_build::ask_user_question::AskUserQuestionExtResponse
    {
    ...
    pub fn send_ext_response(
        &mut self,
        response: weepcode_tools::implementations::weepcode_build::ask_user_question::AskUserQuestionExtResponse,
    ) -> bool {
...
mod tests {
    ...
    /// `no_freeform` questions have no "Other" row, so activating freeform
    /// input must be impossible: focus stays in Navigation and nothing gets
    /// marked selected. Regression test for a no-freeform modal letting the
    /// user type after clicking under the last option.
    #[test]
    ...
    /// wrongly activated freeform input.
    #[test]
