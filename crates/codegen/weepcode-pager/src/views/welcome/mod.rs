...
/// `base`. Used by the changelog renderers.
...
struct WelcomeLayoutInput<'a> {
    ...
    prompt_compact: bool,
    announcement: Option<&'a weepcode_announcements::RemoteAnnouncement>,
...
impl WelcomeLayout {
    ...
    fn compute_inner(input: WelcomeLayoutInput<'_>, allow_hero_box: bool) -> Self {
        ...
        let gate_info = changelog_height;
        ...
        // Stacked info slot: the changelog, measured at the centered menu width
        // inside the inset.
...
pub(super) enum VersionBadgeMode {
    /// Full badge: team | api_key | **WeepCode** VERSION+channel **Beta** (right-aligned).
    Full,
...
pub(super) fn render_version_badge(
    ...
    let channel = weepcode_version::CHANNEL_LABEL;
    match &mode {
        VersionBadgeMode::Full => {
            ...
            spans.push(Span::styled(
                format!("{}{}", weepcode_version::VERSION, channel),
                Style::default().fg(theme.gray),
            ));
            spans.push(Span::styled(
                " Beta",
...
pub struct WelcomeRenderParams<'a> {
    ...
    pub announcement: Option<&'a weepcode_announcements::RemoteAnnouncement>,
    ...
    pub foreign_resume_hint: Option<&'a weepcode_workspace::foreign_sessions::RecentForeignSession>,
    pub is_api_key_auth: bool,
    pub session_picker_content_results:
        Option<&'a [weepcode_shell::extensions::session_search::SearchSessionHit]>,
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        ...
        // Pending login screen. Skipped under the ZDR gate (the arm above)
        // since it already blocks sessions. The `if let` destructure makes the
...
/// inset. `logo_height` selects the min menu width.
#[cfg(test)]
...
mod tests {
    ...
    fn foreign_resume_tip_names_each_tool_and_age() {
        use weepcode_workspace::foreign_sessions::ForeignSessionTool;
        ...
        for (tool, label) in [
            ...
            let hint = weepcode_workspace::foreign_sessions::RecentForeignSession {
                tool,
    ...
    fn command_auth_arm_shows_url_and_waiting() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.x.ai/oauth2/authorize?client_id=weepcode";
    ...
    fn long_ann() -> weepcode_announcements::RemoteAnnouncement {
...
the usual channels. "
    ...
    fn announcement_equal_for_short_message() {
        let area = Rect::new(0, 0, 120, 60);
        let a = weepcode_announcements::RemoteAnnouncement {
            title: Some("FYI".into()),
