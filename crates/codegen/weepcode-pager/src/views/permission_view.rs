...
use weepcode_workspace::permission::bash_command_splitting::{
...
pub struct PermissionViewState {
    ...
    pub request: weepcode_acp_lib::AcpArgs<acp::RequestPermissionRequest>,
    ...
    /// permissions). Imported from `weepcode-shell`, NOT duplicated locally.
...
/// ```text
///  3 [ ] Tell WeepCode what to do differently
...
mod tests {
    ...
    fn empty_view_state(mcp_scope: Option<McpScopeState>) -> PermissionViewState {
        ...
        let perm = weepcode_acp_lib::AcpArgs {
            request,
            response_tx,
        };
        PermissionViewState {
    ...
    fn render_shows_planned_mcp_args() {
        ...
        state.description = vec![
            ...
            "  \"body\": \"hello from weepcode\"".to_string(),
            "}".to_string(),
        ];
        ...
        assert!(
            text.contains("\"body\": \"hello from weepcode\""),
            "args JSON not rendered:\n{text}"
        );
    ...
    ///
    /// ```text
    /// PERMISSION_UI_RENDER_REVIEW=1 cargo test -p weepcode-pager --lib \
    ...
    fn historic_bash_fixture_path() -> std::path::PathBuf {
        // CARGO_MANIFEST_DIR = crates/codegen/weepcode-pager
