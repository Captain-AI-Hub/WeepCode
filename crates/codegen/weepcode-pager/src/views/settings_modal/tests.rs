...
/// `permission_mode`'s "auto" choice is gated off when the auto gate is off and
/// offered when it's on. Pure — no process-global mutation.
#[test]
fn enum_choice_gated_off_covers_permission_auto() {
...
fn editor_render_fixture(buffer: &str, cursor_byte: usize) -> SettingsModalState {
    ...
    let snapshot = PagerLocalSnapshot {
        available_models: vec![(
            "WeepCode Test".to_string(),
            acp::ModelId::new(Arc::from("weepcode-test")),
        )],
        ..PagerLocalSnapshot::default()
    };
...
fn render_editing_value_cursor_at_logical_position_when_buffer_fits() {
    let mut s = editor_render_fixture("WeepCode Test", 4); // cursor between "WeepCode" and " Test"
    let area = Rect {
        x: 0,
        y: 0,
        width: 60,
...
fn picking_enum_esc_returns_to_browse() {
    ...
    match outcome {
        ...
        other => panic!("expected Action::PreviewTheme(\"weepcodenight\") on Esc, got {other:?}"),
    }
    assert!(matches!(s.mode, SettingsModalMode::Browse));
}
...
///
/// Uses an "Opt out"-style choice with a long description that
/// wraps at width=60. The fixture mirrors the bug report — the
/// screenshot in the user-feedback PR showed exactly this choice
/// clipped with `…`.
...
fn picker_visual_smoke_debug() {
    let entries = vec![SettingMeta {
        ...
        description: "Controls whether coding data may be retained and used for training.",
        keywords: &["test"],
        kind: SettingKind::Enum {
            default: "opt-out",
            choices: &[
                EnumChoice {
                    canonical: "opt-in",
                    display: "Opt in",
                    description: "Allow coding session data to be retained for training and product improvement.",
                },
                EnumChoice {
                    canonical: "opt-out",
                    display: "Opt out",
...
fn picker_long_description_wraps_to_multiple_lines() {
    ...
    assert!(
        r3.contains("Allow coding session data"),
...
fn picker_multi_line_choice_hit_rect_spans_all_lines() {
    ...
    let entries = vec![SettingMeta {
        ...
        owner: SettingOwner::Shared,
        label: "Coding data sharing",
        description: "Controls whether coding data may be retained.",
        keywords: &["test"],
        kind: SettingKind::Enum {
            default: "opt-in",
            choices: &[
...
fn editing_value_chars_mutate_buffer_and_invalid_enter_is_noop() {
        ...
        SettingsModalMode::EditingValue {
            ...
            assert!(
                ...
                "validation_error must be Some for unknown model 'a' \
                 (catalog has 'WeepCode 4 Fast' only)",
            );
        }
    ...
    // (catalog has 'WeepCode 4 Fast'; "a" doesn't match) is
...
fn editing_value_esc_returns_to_browse() {
    ...
    s.mode = SettingsModalMode::EditingValue {
        key: "default_model",
        buffer: "weepcode-4".to_string(),
        cursor_byte: "weepcode-4".len(),
        validation_error: None,
    };
    let outcome = handle_settings_key(&mut s, &KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE));
    assert!(matches!(outcome, SettingsKeyOutcome::Changed));
...
/// State seeded with the real registry for the two-line layout tests.
/// Uses the production `render_mermaid` row (label 23 + any value +
/// chevron/chrome exceeds 28 cells), so rendering at width=28 forces
/// the two-line layout. (Replaces the retired `coding_data_sharing`
/// row these tests originally exercised.)
fn two_line_row_state() -> SettingsModalState {
    SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
...
fn two_line_row_hit_rect_spans_both_lines() {
    ...
    // should fire the default action (open the enum picker).
    s.list_area = area;
...
fn two_line_row_with_expansion_renders_three_segments() {
    let mut s = two_line_row_state();
    // The `render_mermaid` row's label + value (with chevron) won't
    ...
    let row_idx = s
        ...
        .expect("render_mermaid must be registered");
    s.selected = row_idx;
    s.expanded_keys.insert("render_mermaid");
    ...
    assert!(
        label_line.contains("Render Mermaid diagrams"),
        "line 1 must contain the row label: {label_line:?}"
    );
    // The value (display: "Auto") is on line 2.
    let value_line = buf_row_text(&buf, rect.y + 1, area.x, area.width);
    assert!(
        value_line.contains("Auto"),
...
//    the "Tip · Ask WeepCode…" docs footer and the keybindings hints.
//
...
fn docs_footer_tip_is_centered() {
    ...
    // SHORT = "Tip · Ask WeepCode to change a setting" (34 cells);
    ...
    assert!(
        !row_short.contains("weepcodeday"),
        "width=40 must NOT render LONG path (no `weepcodeday`): {row_short:?}",
    );
    assert!(
...
fn tip_line_has_blank_row_above() {
    ...
    for y in 0..area.height {
        let txt = buf_row_text(&buf, y, area.x, area.width);
        if txt.contains("Tip") && txt.contains("Ask WeepCode") {
            tip_y = Some(y);
            break;
        }
    }
...
fn click_settings_breadcrumb_collapses_picker_to_browse() {
    ...
    // `"weepcodenight"`. Tightened from the previous `Action(_) |
    ...
    match outcome {
        ...
        other => panic!(
            "expected Action(PreviewTheme(\"weepcodenight\")) — the keyboard \
...
fn click_settings_breadcrumb_after_nav_reverts_to_original() {
    ...
    // Pick a different index. The default theme is `weepcodenight`
...
fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
    ...
    match outcome {
        SettingsKeyOutcome::ActionPair(
            ...
            // Default theme is `weepcodenight`; entering the picker
            // captures `original_value = current value = weepcodenight`,
...
/// misleading: on dark themes (WeepcodeNight, TokyoNight, RosePine
/// Moon) `bg_visual` is actually *lighter* than `bg_highlight`;
/// only on the WeepcodeDay light theme is title darker. The
...
fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
    ...
    let raw_theme = match crate::theme::Theme::current_kind() {
        crate::theme::ThemeKind::WeepcodeNight => crate::theme::Theme::weepcodenight(),
        crate::theme::ThemeKind::TokyoNight => crate::theme::Theme::tokyonight(),
        crate::theme::ThemeKind::WeepcodeDay => crate::theme::Theme::weepcodeday(),
        ...
        crate::theme::ThemeKind::Auto => crate::theme::Theme::weepcodenight(),
    };
    assert_ne!(
