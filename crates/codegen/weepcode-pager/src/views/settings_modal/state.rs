...
/// Row visibility: `hidden_in_minimal` rows are dropped in minimal mode. Pure
/// for unit tests.
...
/// WeepCode STT language list (25 codes + client-only `auto` = 26) with headroom.
...
/// `permission_mode`'s "auto" when the auto gate is off. Pure (gate passed as
/// arg) so it's unit-testable without touching process globals.
