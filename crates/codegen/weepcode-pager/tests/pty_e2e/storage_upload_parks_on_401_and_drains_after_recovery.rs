...
async fn storage_upload_parks_on_401_and_drains_after_recovery() {
    ...
    // Trace uploads are gated on first-party WeepCode OAuth (`is_weepcode_auth()`:
    // AuthMode::Oidc + WeepCode issuer); the harness's WEEPCODE_API_KEY is ApiKey mode
    ...
    env_refs.retain(|(k, _)| *k != "WEEPCODE_TRACE_UPLOAD");
    env_refs.push(("WEEPCODE_TRACE_UPLOAD", "true"));
    env_refs.push(("WEEPCODE_TELEMETRY_TRACE_UPLOAD", "true"));
    env_refs.push(("WEEPCODE_UPLOAD_QUEUE_AUTH_PROBE_SECS", "2"));
    ...
    //
    // Accounting (weepcode-file-utils upload queue):
    ...
    //   not fire inside this 3s window; with `WEEPCODE_UPLOAD_QUEUE_AUTH_PROBE_SECS=2`
