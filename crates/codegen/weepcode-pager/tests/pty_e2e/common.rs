...
pub(crate) fn contextual_hints_env(content: &ContentController) -> Vec<(String, String)> {
    ...
    env.push(("WEEPCODE_CONTEXTUAL_HINTS".into(), "1".into()));
    env
}
...
// `weepcode_pager_pty_harness::flows` (re-exported above).
...
/// the harness's default `WEEPCODE_API_KEY` (ApiKey/BYOK mode, no auth.json entry)
/// would never fetch `/v1/settings`. Spawns WITHOUT `WEEPCODE_ANNOUNCEMENTS_OVERRIDE`
...
pub(crate) fn spawn_polling_session_with_env(
    ...
    env_refs.push(("WEEPCODE_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS", "1"));
...
/// (`WEEPCODE_TEST_VERSION`) and an explicit `WEEPCODE_FOLDER_TRUST` — `1` when `feature_on`,
/// else `0` (an explicit opt-out that overrides the now-on default). HOME/WEEPCODE_HOME
...
pub(crate) fn trust_env(content: &ContentController, feature_on: bool) -> Vec<(String, String)> {
    let mut env = content.env_for_pager();
    // A self-built (unstamped) weepcode auto-trusts and never prompts; simulate a
    ...
    // auto-trust.
    env.push(("WEEPCODE_TEST_VERSION".into(), "0.0.0-sim".into()));
    // Set WEEPCODE_FOLDER_TRUST explicitly: the default is on, so `0` is the opt-out
    // that exercises the feature-off path rather than relying on an absent var.
    let folder_trust = if feature_on { "1" } else { "0" };
    env.push(("WEEPCODE_FOLDER_TRUST".into(), folder_trust.into()));
    env
}
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".weepcode")
        .join(weepcode_workspace::trust::TRUST_FILE_NAME);
    let store = weepcode_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&weepcode_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `weepcode_pager_pty_harness::flows`
// (re-exported above).
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    let weepcode_home = content.home().join(".weepcode");
    std::fs::create_dir_all(&weepcode_home).expect("create fake WEEPCODE_HOME");
    let config = format!(
        "[mcp_servers.{MCP_TEST_SERVER}]\ncommand = \"{command}\"\nargs = []\nstartup_timeout_sec = 2\n"
    );
    std::fs::write(weepcode_home.join("config.toml"), config).expect("write config.toml");
}
...
// unit tests in weepcode-hooks::discovery::tests. The PTY E2E test requires
...
/// Seed `~/.weepcode/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{WEEPCODE_HOME|HOME}/.weepcode/config.toml` location
...
pub(crate) fn seed_ui_config(content: &ContentController, ui_body: &str) {
    let weepcode_home = content.home().join(".weepcode");
    std::fs::create_dir_all(&weepcode_home).expect("create .weepcode");
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let weepcode_home = content.home().join(".weepcode");
    std::fs::create_dir_all(&weepcode_home).expect("create .weepcode");
    // Minimal opt-in only — matches load_config's `{WEEPCODE_HOME|HOME}/.weepcode/config.toml`.
...
pub(crate) fn seed_keep_text_selection_config(content: &ContentController) {
    ...
    std::fs::create_dir_all(&weepcode_home).expect("create .weepcode");
    std::fs::write(
        weepcode_home.join("config.toml"),
...
pub(crate) fn mouse_toggle_env(content: &ContentController) -> Vec<(String, String)> {
    let mut env = content.env_for_pager();
    env.push(("WEEPCODE_MOUSE_REPORTING_TOGGLE".into(), "true".into()));
    env
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "WEEPCODE_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    ...
    env.push((
        ...
        weepcode_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string(),
    ));
    let env_refs: Vec<(&str, &str)> = env.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
    PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &[], &env_refs).expect("spawn pager")
}
...
/// `weepcode wrap` run budget. Same contention math as the requirements-version
...
/// Run `weepcode wrap <wrap_args...>` to completion inside a PTY with an isolated
/// `WEEPCODE_HOME`, returning the exit code (`None` if it never exited within
...
pub(crate) fn run_wrap_driving(
    ...
    let mut env: Vec<(&str, &str)> = vec![("WEEPCODE_HOME", &home_str), ("NO_COLOR", "1")];
    env.extend_from_slice(extra_env);

    let mut harness = PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &args, &env)
        .expect("spawn weepcode wrap");
...
/// `weepcode wrap` hands to the user's shell without depending on any real
...
/// Dump an asciinema cast of `harness` into `$WEEPCODE_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("WEEPCODE_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
pub(crate) use weepcode_pager_pty_harness::host_clipboard::{
...
pub(crate) use weepcode_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
