...
//!
//! The stack is `tmux -> nvim :terminal -> weepcode`: nvim/tmux repaint weepcode's
//! pane out-of-band (no weepcode PTY resize), and weepcode's diff renderer only
...
//! into the virtual terminal, bypassing weepcode) and then check whether weepcode
//! heals it. `NVIM` is set in weepcode's env so it sees the embedded-editor
//! context the fix keys off (mirroring a real nvim `:terminal`).
//!
//! The fix: weepcode forces a full clear+repaint on `FocusGained` in
//! editor/multiplexer contexts, so the injected row is gone after refocus.
//! The final assertion guards that heal.

use super::common::*;

/// Unique sentinel that weepcode would never render on its own.
...
async fn out_of_band_stale_row_heals_on_focus_gained() {
    ...
    env.push((
        "NVIM".into(),
        "/tmp/weepcode-pty-harness-fake-nvim.sock".into(),
    ));
    ...
    // virtual screen at col 1 — the static left margin weepcode's diff renderer
    ...
    // weepcode must not self-heal out-of-band content via ordinary diff redraws:
    // it only rewrites cells whose own model changed, so this row is stranded.
    h.update(Duration::from_millis(300));
    assert!(
        h.contains_text(STALE_MARKER),
        "stale row should survive a normal redraw (weepcode's diff renderer doesn't own it)\nscreen:\n{}",
        h.screen_contents()
    );

    // A FocusGained (CSI I) forces a full clear+repaint that re-asserts weepcode's
