...
use weepcode_pager_pty_harness::PtyHarness;
pub(crate) use weepcode_pager_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
/// (`weepcode-pager-pty-harness/src/scroll_matrix/runner.rs`) ports this
...
/// `weepcode-pager-pty-harness/src/scroll_matrix/session.rs` is a port of
...
/// (e.g. `WEEPCODE_SCROLL_SPEED`) appended to the controller's env.
