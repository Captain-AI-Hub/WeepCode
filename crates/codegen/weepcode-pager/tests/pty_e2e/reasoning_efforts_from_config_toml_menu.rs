...
async fn reasoning_efforts_from_config_toml_menu() {
    let content = ContentController::start_with_models(vec![
        MockModel::new("weepcode-4.5").with_supports_reasoning_effort(true),
    ])
    .await
    .expect("start content");
    content.set_response(format!("{MOCK_RESPONSE_SENTINEL} turn."));

    // Seed `~/.weepcode/config.toml` with a per-model reasoning-effort menu.
    let weepcode_home = content.home().join(".weepcode");
    std::fs::create_dir_all(&weepcode_home).expect("create .weepcode");
    // Quote the dotted model id: bare `[model.weepcode-4.5]` is TOML key-path syntax (model.weepcode-4.5), not the id "weepcode-4.5".
    std::fs::write(
