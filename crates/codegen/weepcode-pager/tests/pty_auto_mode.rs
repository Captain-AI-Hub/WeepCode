...
//!
//! Uses `weepcode-pager-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.weepcode/auth.json` from `WEEPCODE_AUTH_JSON` (path) or the
//! developer's `~/.weepcode/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p weepcode-pager --test pty_auto_mode -- --ignored --nocapture`

use std::path::{Path, PathBuf};
use std::time::Duration;

use weepcode_pager_pty_harness::{PtyHarness, pager_binary};
...
/// Prefer explicit path, else the user's real `~/.weepcode/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("WEEPCODE_AUTH_JSON") {
    ...
    dirs_next_home()
        .map(|h| h.join(".weepcode/auth.json"))
        .filter(|p| p.is_file())
}
...
fn prepare_sandbox(home: &Path, gate_on: bool) -> Vec<(String, String)> {
    ...
    let _ = std::fs::create_dir_all(&weepcode);
    if let Some(src) = auth_json_source() {
        let dest = weepcode.join("auth.json");
        ...
        eprintln!("pty_auto_mode: no ~/.weepcode/auth.json — may hit device login");
    }

    let home_s = home.display().to_string();
    let mut env = vec![
        ("HOME".into(), home_s.clone()),
        ("WEEPCODE_HOME".into(), weepcode.display().to_string()),
        ...
        ("TMUX".into(), "".into()),
        // Do not set WEEPCODE_API_KEY — prefer OIDC entry in auth.json (pty_e2e pattern).
    ];
    // Pin the feature gate explicitly so the cycle is deterministic regardless
    // of the developer's shell. `WEEPCODE_AUTO_PERMISSION_MODE` is the highest gate
    // layer below requirements; "1"/"0" parse to on/off (weepcode_config::
    ...
    env.push((
        "WEEPCODE_AUTO_PERMISSION_MODE".into(),
...
fn require_auth() -> bool {
    std::env::var("WEEPCODE_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness = PtyHarness::new(&binary, ROWS, COLS, &[], &env_refs)
        .expect("spawn pager in PTY (weepcode-pager-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "WEEPCODE_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
