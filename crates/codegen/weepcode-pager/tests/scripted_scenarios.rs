...
//!
//! These tests exercise the real `weepcode-pager` binary through a PTY using
...
/// enterprise deploy report: with `WEEPCODE_GOAL=1`, `/goal` must
...
/// explicitly off (`WEEPCODE_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `WEEPCODE_FOLDER_TRUST=1` and a git repo that
...
/// also pins `WEEPCODE_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
