...
fn render_with_filter_active_and_small_viewport_clamps_scroll() {
    ...
    weepcode_pager::views::settings_modal::render_settings_modal(
...
fn defaults_round_trip_through_registry() {
    use weepcode_pager::settings::{SettingValue, current_value_for};
    let reg = SettingsRegistry::defaults();
    ...
    weepcode_pager::appearance::cache::set_keep_text_selection(
        weepcode_pager::appearance::TextSelection::Flash,
    );
    weepcode_pager::appearance::cache::set_show_thinking_blocks(true);
    weepcode_pager::appearance::cache::set_prompt_suggestions(true);
    weepcode_pager::appearance::cache::set_group_tool_verbs(true);
    weepcode_pager::appearance::cache::set_scroll_mode(
        weepcode_pager::appearance::ScrollMode::Auto,
    );
    weepcode_pager::appearance::cache::set_invert_scroll(false);
    // 3 = the registry default shown while the profile is in charge.
    weepcode_pager::appearance::cache::set_scroll_lines(3);
    ...
    let expected = |key: &str| -> SettingValue {
        match key {
            ...
            "theme" => SettingValue::Enum("weepcodenight"),
            "auto_dark_theme" => SettingValue::Enum("weepcodenight"),
            "auto_light_theme" => SettingValue::Enum("weepcodeday"),
    ...
    for meta in reg.all() {
        ...
        let default_value = weepcode_pager::settings::default_value_for(meta);
...
fn d_key_emits_open_reset_confirm_for_every_setting() {
    ...
    for meta in reg.all() {
        ...
        let mut s = make_state();
        // Some rows are hidden in this context (e.g. group-child keys never
        // appear as top-level rows). Skip settings with no visible row; their
        // reset path is covered by the dispatch
        // round-trip tests.
        let has_row = s
            .rows
            .iter()
...
// `crates/codegen/weepcode-pager/src/app/dispatch.rs::tests` (next to
...
fn pr4_mouse_click_in_theme_picker_is_no_op() {
    ...
    s.mode = SettingsModalMode::PickingEnum {
        key: "theme",
        choices_idx: 0,
        original_value: SettingValue::Enum("weepcodenight"),
        supports_preview: true,
    };
    synth_rects(&mut s);
    let outcome = handle_settings_mouse(
...
fn pr6_current_value_for_reads_pager_snapshot() {
    use weepcode_pager::settings::current_value_for;
...
fn pr6_permission_mode_picker_enter_dispatches_set_permission_mode_commit() {
    use weepcode_pager::app::actions::PermissionModeKind;
    let reg = SettingsRegistry::defaults();
...
fn reset_overlay_dims_all_rows_except_target() {
    ...
    use weepcode_pager::views::settings_modal::ResetConfirmOverlay;
...
/// "Ask WeepCode" tip footer at the bottom of the content area in
...
fn docs_footer_renders_for_browse_and_picker() {
    ...
    for fixture_label in ["browse", "picker"] {
        ...
        assert!(
            all_text.contains("Ask WeepCode"),
            "[{fixture_label}] docs footer (`Ask WeepCode`) must appear in the rendered modal:\n\
             {all_text}"
        );
        assert!(
            all_text.contains("change theme to weepcodeday"),
...
fn render_modal_to_string(s: &mut SettingsModalState, width: u16, height: u16) -> String {
    ...
    let mut buf = Buffer::empty(area);
    weepcode_pager::views::settings_modal::render_settings_modal(&mut buf, area, s, false, None);
...
fn restart_pill_hidden_when_edited_but_collapsed() {
    use weepcode_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use weepcode_shell::agent::config::UiConfig;
...
fn reset_confirm_prompt_helper_builds_well_formed_string_for_every_setting() {
    use weepcode_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use weepcode_pager::views::modal::{ActiveModal, ModalConfirmation, reset_confirm_prompt};
    use weepcode_shell::agent::config::UiConfig;
    let reg = SettingsRegistry::defaults();
    for meta in reg.all() {
...
fn pr14_default_model_picker_commits_resolved_model_id() {
    let snapshot = PagerLocalSnapshot {
        available_models: vec![
            (
                "WeepCode 4.5".to_string(),
                agent_client_protocol::ModelId::new(std::sync::Arc::from("weepcode-4.5")),
            ),
            (
                "WeepCode 3".to_string(),
                agent_client_protocol::ModelId::new(std::sync::Arc::from("weepcode-3")),
            ),
        ],
        ..PagerLocalSnapshot::default()
    };
    ...
    // Walk down past row 0 ("(no override)") to row 1 ("WeepCode 4.5").
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Down));
    assert!(
        matches!(outcome, SettingsKeyOutcome::Changed),
...
fn pr8_default_model_and_max_thoughts_width_defaults_roundtrip() {
    ...
    assert_eq!(
        weepcode_pager::settings::default_value_for(dm_meta),
    ...
    assert_eq!(
        weepcode_pager::settings::default_value_for(mt_meta),
...
/// `coding_data_sharing` picker (retired in Phase 7a).
#[test]
...
fn pr10_plan_mode_picker_enter_dispatches_set_commit() {
    use weepcode_pager::app::actions::PlanModeKind;
...
fn render_mermaid_picker_enter_dispatches_set_commit() {
    use weepcode_pager::appearance::RenderMermaid;
...
fn pr14_string_settings_use_known_model_validator() {
    use weepcode_pager::settings::DynamicEnumSource;
    let reg = SettingsRegistry::defaults();
...
fn keep_text_selection_picker_enter_dispatches_set_commit() {
    use weepcode_pager::appearance::TextSelection;
    ...
    weepcode_pager::appearance::cache::set_keep_text_selection(TextSelection::Flash);
    let mut s = make_state();
    navigate_to(&mut s, "keep_text_selection");
    let _ = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn keep_text_selection_hold_snapshot_seeds_picker_at_hold() {
    ...
    weepcode_pager::appearance::cache::set_keep_text_selection(TextSelection::Hold);
    let ui = UiConfig {
...
fn invert_scroll_enter_dispatches_typed_setter() {
    weepcode_pager::appearance::cache::set_invert_scroll(true);
    let mut s = make_state();
    navigate_to(&mut s, "invert_scroll");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn show_thinking_blocks_space_dispatches_typed_setter() {
    ...
    weepcode_pager::appearance::cache::set_show_thinking_blocks(false);
    let mut s = make_state();
...
fn prompt_suggestions_space_dispatches_typed_setter() {
    // Pin off so space toggles to true.
    weepcode_pager::appearance::cache::set_prompt_suggestions(false);
    let mut s = make_state();
...
fn group_tool_verbs_cache_off_dispatches_on() {
    ...
    weepcode_pager::appearance::cache::set_group_tool_verbs(false);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
...
fn collapsed_edit_blocks_space_dispatches_typed_setter() {
    ...
    weepcode_pager::appearance::cache::set_collapsed_edit_blocks(false);
    let mut s = make_state();
...
fn collapsed_edit_blocks_cache_on_dispatches_off() {
    ...
    weepcode_pager::appearance::cache::set_collapsed_edit_blocks(true);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
