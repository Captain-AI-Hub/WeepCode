...
- **Global hooks** — from `~/.weepcode/hooks/`
- **Project hooks** — from `.weepcode/hooks/` in your repo
...
Plugin hooks automatically receive `WEEPCODE_PLUGIN_ROOT` and `WEEPCODE_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.weepcode/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `weepcode plugin marketplace add <source>`)
...
Sources land in `~/.weepcode/config.toml`:
...
Or in `~/.weepcode/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../weepcode-hooks/examples/README.md) — ready-to-use sample hooks
