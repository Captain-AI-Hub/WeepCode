...
- **Hooks** -- a `hooks/hooks.json` file of lifecycle hooks. Plugin hooks also receive `WEEPCODE_PLUGIN_ROOT` and `WEEPCODE_PLUGIN_DATA` (see the [Hooks guide](10-hooks.md) for every environment variable passed to hooks).
- **MCP servers** -- a `.mcp.json` file of server configurations
- **LSP servers** -- a `.lsp.json` file of language server configurations

If a plugin includes a `plugin.json` manifest, the manifest can override paths or add metadata; otherwise components load from the convention directories. The manifest is optional: without one, WeepCode discovers the components above from their standard directories.
...
| `WEEPCODE_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory, for plugin state, caches, and logs. |

WeepCode sets these values and overrides any value you declare for the same key in the hook JSON's `env` map. (WeepCode also sets the `CLAUDE_PLUGIN_ROOT` and `CLAUDE_PLUGIN_DATA` aliases for compatibility.) See the [Hooks guide](10-hooks.md) for every environment variable passed to hooks.

---

## Plugin locations

WeepCode discovers plugins from these locations, in priority order:
...
| `.weepcode/plugins/` | Project -- shared with the team through version control | Requires trust |
| `~/.weepcode/plugins/` | User -- personal plugins for every project | Trusted automatically |
| `[plugins].paths` (config) | Custom directories you add in `config.toml` | Depends on location |

WeepCode also reads the `.claude/plugins/` equivalents for compatibility. When two plugins share a name, the higher-priority location wins.

The Agent SDKs load per-session plugins through `WeepCodeOptions.plugins`, which arrives as `_meta.pluginDirs` on `session/new` and `session/load`; because the caller controls the directory, these plugins are always trusted -- their hooks and MCP servers activate without a prompt, and they never persist beyond the session. The `--plugin-dir` flag is the process-wide equivalent for direct CLI use (repeatable: `weepcode agent --no-leader --plugin-dir A --plugin-dir B stdio`); it applies to dedicated agent processes only and is ignored in leader mode (the shared leader discovers its own plugins).
...
```bash
weepcode plugin list [--json] [--available]   # List installed plugins (--available requires --json)
weepcode plugin install <source> --trust      # Git URL, GitHub shorthand (user/repo), or local path
weepcode plugin uninstall <name> [--confirm] [--keep-data]   # Aliases: rm, remove
weepcode plugin update [<name>]               # Omit the name to update all plugins
weepcode plugin enable <name>
weepcode plugin disable <name>
weepcode plugin details <name>                # Show the plugin's component inventory
weepcode plugin validate [<path>]             # Validate plugin.json (default: current directory)
weepcode plugin tag [<path>] [--push] [--force] [--dry-run]   # Tag a release from the manifest version
```

Run `weepcode plugin install <source>` without `--trust` and WeepCode prints the source and warns that installing will activate the plugin's hooks, MCP servers, and skills, then stops without installing. Add `--trust` to install it.
...
or `WEEPCODE_MARKETPLACE_REQUIRE_SHA=1`. Both are tighten-only: either one enables
...
```bash
weepcode plugin marketplace list [--json]
weepcode plugin marketplace add <url>         # Git URL, GitHub shorthand (user/repo), or local path
weepcode plugin marketplace remove <url>      # Git URL or local path of a configured source
weepcode plugin marketplace update [<name>]   # Omit the name to refresh all sources
```

### Example: set up a team marketplace

```bash
weepcode plugin marketplace add my-org/team-plugins
weepcode plugin marketplace list
weepcode plugin install my-org/team-plugins --trust
weepcode plugin list
weepcode plugin update
```
...
Configure plugin directories and per-plugin state in `~/.weepcode/config.toml`:
...
To hide the hooks and plugins UI — the `/hooks` and `/plugins` commands and the scrollback annotations — set this in `~/.weepcode/pager.toml`:
...
Place this file at `~/.weepcode/settings.json` or `~/.claude/settings.json`.
...
WeepCode trusts plugins from `~/.weepcode/plugins/` automatically. Project plugins in `.weepcode/plugins/` require explicit trust. To trust a plugin, install it with `--trust`:

```bash
weepcode plugin install <source> --trust
```
...
```bash
weepcode inspect          # Show plugins with their skills, agents, hooks, and MCP servers
weepcode inspect --json   # Emit machine-readable JSON
```
