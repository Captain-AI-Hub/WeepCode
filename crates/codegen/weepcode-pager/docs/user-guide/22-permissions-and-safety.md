...
WeepCode can read files, search code, edit files, and run shell commands. The permission system controls what the agent is allowed to do. You can combine several independent layers: permission rules, permission modes, hooks, and the OS-level sandbox.
...
The user-level `~/.weepcode/requirements.toml` is under the user's control, so a developer can remove the lock by editing that file. For enforcement that users cannot override, deploy the setting in the root-owned system file `/etc/weepcode/requirements.toml`.

> **Note:** WeepCode honors the permission rules in Claude Code's `managed-settings.json`, but not its `disableBypassPermissionsMode` lock. To disable always-approve in WeepCode, use `requirements.toml` as shown above.

---

## Configuring Permissions

WeepCode reads permission rules from three compatible sources. Rules from all sources are merged into one set; a rule's effect depends on its action (`deny` > `ask` > `allow`), not on which file it came from.
...
|-------|------|-----------------------|
| Global (all projects) | `~/.weepcode/config.toml` | No |
| Project (committed) | `<project>/.weepcode/config.toml` | Yes (commit it) |
| Project (personal) | `<project>/.claude/settings.local.json` | No (gitignore it) |
| Interactive grants | Stored internally by WeepCode, per project | No |

Notes on scoping:

- WeepCode discovers a `.weepcode/config.toml` at every directory level from the repository root down to your working directory, so a subdirectory can add rules on top of the repo root's.
...
- WeepCode has no native `config.local.toml`. For personal, uncommitted rules in a project, use `.claude/settings.local.json`; WeepCode reads it directly (see [Claude Code Compatibility](#3-claude-code-compatibility-claudesettingsjson)).
...
To stop prompts for a specific command in one project, add a narrow allow rule to that project's `.weepcode/config.toml` (or `.claude/settings.json`):
...
```bash
weepcode -p "Review the API changes" \
  --allow 'Bash(git *)' \
  --allow 'Bash(gh *)' \
  --allow 'Read' \
  --allow 'Grep' \
...
### 2. Native Configuration (`~/.weepcode/config.toml` and `.weepcode/config.toml`)
...
Rules from the global `~/.weepcode/config.toml` and every project `.weepcode/config.toml` (from the repo root down to your working directory) are merged into one rule set, alongside any `.claude/settings.json` rules.

Managed configuration deployed by your organization also contributes `[permission]` rules: the system `/etc/weepcode/managed_config.toml`, and a user-level copy that WeepCode maintains automatically at `~/.weepcode/managed_config.toml`. Managed rules merge like rules from any other source, with two properties specific to managed `allow` rules: your own `deny` and `ask` rules win over a managed `allow` (severity ordering), and a catch-all managed `allow` is ignored when always-approve is locked off. For rules that users cannot edit away, use the root-owned system `/etc/weepcode/requirements.toml`.
...
WeepCode reads `~/.claude/settings.json` and `~/.claude/settings.local.json`, plus the project-level `<project>/.claude/settings.json` and `settings.local.json` (walking up to the repo root). The native `.weepcode` source for permission rules is `config.toml`, described in the section above.
...
Supported `defaultMode` values are `default`, `acceptEdits`, `bypassPermissions`, `dontAsk`, and `plan`. WeepCode reads `defaultMode` from its canonical location under `permissions`; a top-level `defaultMode` is also accepted when the nested key is absent.
...
**Chained commands.** WeepCode parses each command like a shell and splits it on `&&`, `||`, `;`, `|`, and newlines. The rule actions treat segments differently:
...
`MCPTool(...)` patterns match the full WeepCode tool name in `server__tool` form, with glob support: `MCPTool(linear__*)` matches every tool from the `linear` server. WeepCode tool names carry no `mcp__` prefix, so a rule written as `mcp__server__tool` never matches an MCP call; write `MCPTool(server__tool)` instead.
...
Interactive grants are stored in WeepCode's own state directory under your home directory, scoped to the directory you launched WeepCode from. A grant made in one project never applies in another, grants are not written into the repository, and they are not meant to be hand-edited.

Interactive grants are personal, per-machine state. For an allowlist you can review in code review and share with teammates, use declarative rules in the project's `.weepcode/config.toml` instead.
...
**`~/.weepcode/hooks/git-gh-only.json`**
...
```bash
chmod +x ~/.weepcode/hooks/git-gh-only.sh
```
...
```bash
weepcode -p "Implement the feature using only git and GitHub CLI" \
  --allow 'Read' \
  --allow 'Grep' \
  --allow 'Bash(git *)' \
  --allow 'Bash(gh *)'
