...
> **Status: alpha.** The schema below is versioned (`weepcode_code.schema.version = v1`);
...
```bash
export WEEPCODE_EXTERNAL_OTEL=1                  # master switch
...
`WEEPCODE_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
| `OTEL_LOG_USER_PROMPTS` | `0` | Content gate: prompt text on `weepcode_code.user_prompt` (60 KB cap, secret-scrubbed). |
...
> `WEEPCODE_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`WEEPCODE_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
...
the `[telemetry]` `otel_*` keys through `weepcode setup` managed config /
...
| `weepcode_code.schema.version` | `v1` |
...
## Metrics (meter scope `weepcode.code`)

| Metric | Unit | Attributes |
|---|---|---|
| `weepcode_code.session.count` | `{session}` | base attrs only |
| `weepcode_code.token.usage` | `{token}` | `type` = `input` \| `output` \| `reasoning` \| `cache_read`; `model` |
| `weepcode_code.turn.count` | `{turn}` | `outcome` = `completed` \| `cancelled` \| `error`; `model` |
| `weepcode_code.tool.decision` | `{decision}` | `tool_name`, `decision` = `allow` \| `deny` \| `cancelled` \| `followup`, `access_kind`, `permission_mode` |
| `weepcode_code.tool.usage` | `{call}` | `tool_name`, `outcome` |
| `weepcode_code.error.count` | `{error}` | `error_category`, `model` |

There is no `cost.usage` metric: join `weepcode_code.token.usage` with your own
...
|---|---|
| `weepcode_code.session_start` | `model`, `permission_mode`, `mcp_server_count`, `plugin_count`, `skill_count`, `hook_count`, `memory_enabled`, `is_git_repo`, `client_identifier` |
| `weepcode_code.session_end` | `duration_secs`, `turn_count`, `tool_call_count`, `compaction_count`, `model` |
| `weepcode_code.user_prompt` | `prompt_length`, `model`, `screen_mode?` (`fullscreen` \| `inline` \| `minimal` \| `headless` \| `other`); `prompt` (**prompts**) |
| `weepcode_code.turn_completed` | `outcome`, `duration_ms`, `tool_call_count`, `model`, `error_category?`, `cancellation_category?` |
| `weepcode_code.api_request` | `model`, `duration_ms`, `stop_reason?`, `input_tokens`, `output_tokens`, `reasoning_tokens`, `cache_read_tokens` |
| `weepcode_code.api_error` | `error_category`, `model`, `status_code?`, `duration_ms?` |
| `weepcode_code.tool_result` | `tool_name`, `outcome`, `success`, `duration_ms`, `file_extension`; `tool_parameters`, `file_path` (**details**) |
| `weepcode_code.tool_decision` | `tool_name`, `decision`, `access_kind`, `permission_mode`, `source` |
| `weepcode_code.mcp_server_connection` | `status`, `transport_type`, `duration_ms`, `tool_count?`, `error_type?`; `mcp_server.name` (**details**; collapsed to `mcp_server` otherwise) |
| `weepcode_code.permission_mode_changed` | `to_mode`, `trigger` |
| `weepcode_code.skill_activated` | `skill_source`; `skill.name` (**details**) |
| `weepcode_code.plugin_loaded` | `install_kind?`, `success`, `error_category?`; `plugin_name` (**details**) |
| `weepcode_code.compaction` | `duration_ms`, `tokens_before`, `tokens_after`, `model?` |
| `weepcode_code.subagent` | `phase` = `launched` \| `completed`, `subagent_type?`, `outcome?`, `duration_ms?` |
| `weepcode_code.auth` | `auth_method` |
| `weepcode_code.internal_error` | `error_type` (class only — no message, no location) |
| `weepcode_code.model_switched` | `from_model`, `to_model`, `success`, `error_code?` |
...
sum by (model, type) (rate(weepcode_code_token_usage_total[1h]))

# Sessions per team per day
sum by (team_id) (increase(weepcode_code_session_count_total[1d]))

# Tool-permission denial ratio
sum(rate(weepcode_code_tool_decision_total{decision="deny"}[1h]))
  / sum(rate(weepcode_code_tool_decision_total[1h]))
```
