...
- **`weepcode dashboard`** — launches the TUI directly into the dashboard.
...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.weepcode/config.toml` under `[keybindings]` like every other shortcut.
...
```
 WeepCode · Dashboard — 4 agents · 2 awaiting
...
The state icon matches WeepCode's sibling views (
`tasks_pane`):
...
rebound via `~/.weepcode/config.toml`.
...
Set `WEEPCODE_AGENT_DASHBOARD=0` to force-disable the feature for a single
...
process. The plan's Phase 4 ("supervisor / `weepcode --bg`") would list
