...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.weepcode/agents/` or `~/.weepcode/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.weepcode/personas/`; applied during subagent resolution |
...
| **Examples** | `weepcode-build`, `explore`, `plan` | `researcher`, `concise` |
...
WeepCode discovers file-based personas from these locations, in priority order:

- `.weepcode/personas/*.toml` (project)
- `~/.weepcode/personas/*.toml` (user)
...
> **Note:** WeepCode applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, WeepCode resolves the effective model and reasoning effort in this order, highest priority first:
...
WeepCode manages worktrees through the `weepcode/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "weepcode-build"               # route explore to a specific model
```
...
# instructions_file = ".weepcode/personas/concise.md"  # or load from a file
```

WeepCode also discovers roles from `.weepcode/roles/*.toml` and personas from `.weepcode/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

WeepCode shows running and finished work in side panes on the agent screen:
...
- `Subagent running: "do the thing" (Implementer · weepcode-3) — Thinking`
