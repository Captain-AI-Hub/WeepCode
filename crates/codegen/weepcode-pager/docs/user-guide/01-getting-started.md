...
Once authenticated, WeepCode presents a full-screen TUI with two main areas:

- **Scrollback** -- the conversation history showing your prompts, WeepCode's responses, tool calls, file edits, and more.
- **Prompt** -- the input area at the bottom where you type messages.

Type a message and press `Enter` to send it. WeepCode reads files, runs commands, and edits code as needed. Each tool run streams into the scrollback in real time.
...
By default, WeepCode asks for permission before executing shell commands or editing files. You can approve individually or toggle always-approve mode:
...
Every conversation is a **session**. Sessions are automatically saved to `~/.weepcode/sessions/` and can be resumed later. Each session tracks the full conversation history, tool calls, file edits, and task state.
...
- **Agent messages** -- WeepCode's responses with full markdown rendering and syntax highlighting
- **Thinking blocks** -- WeepCode's reasoning process (collapsible)
...
WeepCode has built-in tools for:
...
```
/model weepcode-build                 # Switch model
...
weepcode "fix the failing auth test and run it"
...
weepcode --worktree=feat "refactor module X"

# Base the worktree on a specific branch (e.g. main) instead of the current HEAD:
weepcode -w --ref main "implement feature from main"
...
Run WeepCode non-interactively for scripting, CI/CD, and automation:
...
```bash
weepcode -p "Review changes for bugs" --output-format json --yolo | jq -r '.text'
```
...
Add per-project instructions by creating an `AGENTS.md` file in your repository. WeepCode reads these files and injects their contents as a project-instructions message at the start of the conversation:

```
~/.weepcode/AGENTS.md           # Global rules (apply to all projects)
<repo-root>/AGENTS.md       # Repository-level rules
<cwd>/AGENTS.md             # Directory-level rules (highest priority)
```

Deeper files take precedence. WeepCode also reads `CLAUDE.md` files for compatibility.
