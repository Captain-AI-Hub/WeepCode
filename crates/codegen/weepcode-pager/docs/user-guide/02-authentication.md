...
Submitting writes a `[model.<name>]` entry to `~/.weepcode/config.toml`, makes it
...
Running `weepcode add-model` (or its alias `weepcode login`) prints this same configuration guidance (there is no
browser sign-in). `weepcode logout` clears `~/.weepcode/auth.json` if one exists.
...
2. **Active session token** — from `~/.weepcode/auth.json` (enterprise flows below).
3. **`WEEPCODE_API_KEY`** — fallback when nothing else applies.
...
`WEEPCODE_AUTH_PROVIDER_COMMAND`. Tokens refresh by re-running the binary with
`WEEPCODE_AUTH_EXPIRED=1`.

---

## Hot Reload

Changes to `~/.weepcode/config.toml` are picked up automatically — new
...
In the TUI, file logging defaults to `DEBUG`; set `WEEPCODE_LOG_FILE` to choose the
...
- **Nothing happens on submit** — Open `~/.weepcode/config.toml` and confirm the
