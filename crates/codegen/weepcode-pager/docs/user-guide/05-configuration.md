...
WeepCode reads configuration from local config files, environment variables, and
...
2. **Environment variables** (e.g., `WEEPCODE_API_KEY`, `WEEPCODE_MEMORY`)
3. **config.toml** (`~/.weepcode/config.toml`)
...
Location: `~/.weepcode/config.toml`

If the file does not exist, WeepCode uses built-in defaults. Specify only the values you want to override.
...
**Disable vim input mode**); WeepCode writes your choice to `[ui] simple_mode` in
`config.toml`.
...
The setting can also be overridden with the `WEEPCODE_DEFAULT_SELECTED_PERMISSION`
...
(`/settings` → **Vim scrollback navigation**). WeepCode writes the change to
`[ui] vim_mode` in `~/.weepcode/config.toml` immediately and applies it to every
...
`weepcode` launches. Configure it from `/settings` → **Default screen mode**
...
`WEEPCODE_SCROLL_SPEED`, `WEEPCODE_SCROLL_MODE`, `WEEPCODE_INVERT_SCROLL`
(`1`/`true`/`0`/`false`), and `WEEPCODE_SCROLL_LINES`. Precedence: env var →
...
(`WEEPCODE_ASK_USER_QUESTION_TIMEOUT_ENABLED` /
`WEEPCODE_ASK_USER_QUESTION_TIMEOUT_SECS`) → user config → managed →
...
env_key = "WEEPCODE_API_KEY"               # env var(s) holding the API key; string or array (first set, non-empty wins)
...
Credential resolution: `api_key` > `env_key` > signed-in session token > `WEEPCODE_API_KEY`.
...
MCP servers can also be configured per-project in `.weepcode/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; other sections load only from `~/.weepcode/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.weepcode/config.toml` (current dir) > `<repo-root>/.weepcode/config.toml` > `~/.weepcode/config.toml`. `[permission]` rules are not overridden by priority; they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions (requires `--experimental-memory` or `WEEPCODE_MEMORY=1`).
...
[subagents.models]
explore = "weepcode-build"               # route to different models
```
...
`weepcode inspect` reports cells that still need session-start resolution as
...
The `[hints]` table holds small persisted UI preferences — mostly "stop asking me" opt-outs. WeepCode writes these for you when you pick a "don't ask again" / "reset in config.toml" option in the TUI, but you can edit or remove them by hand. Deleting a key restores the default behavior.

`[hints]` is read from the **effective config merge** (same precedence as other settings): system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`. Higher-priority layers override lower ones. The TUI only **writes** opt-outs to user `~/.weepcode/config.toml`.
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "weepcode"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `weepcode`. |
...
| WeepCode Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |

When `method = "auto"`, WeepCode detects the terminal brand and selects the best
...
`$WEEPCODE_EVENT`, `$WEEPCODE_MESSAGE`, and `$WEEPCODE_SESSION_ID`.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'WeepCode' -message '$WEEPCODE_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$WEEPCODE_MESSAGE' ntfy.sh/my-weepcode-alerts"
...
fires, try `condition = "always"` as a fallback. WeepCode supports focus tracking
...
Set these only to point telemetry at your own infrastructure or to turn parts of it off. The built-in endpoint and credentials are managed by WeepCode; leave them unset to use the defaults.
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= WEEPCODE_EXTERNAL_OTEL)
...
|----------|-------------|
| `WEEPCODE_API_KEY` | API key from console.x.ai |
| `WEEPCODE_AUTH_PROVIDER_COMMAND` | External auth binary path |
| `WEEPCODE_AUTH_PROVIDER_LABEL` | Display name on TUI login screen |
| `WEEPCODE_AUTH_TOKEN_TTL` | Token lifetime in seconds |
| `WEEPCODE_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `WEEPCODE_OIDC_ISSUER` | OIDC issuer URL |
| `WEEPCODE_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `WEEPCODE_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |
...
| `WEEPCODE_SUBAGENTS` | Enable (`1`) or disable (`0`) subagents |
| `WEEPCODE_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool |
| `WEEPCODE_AGENT` | Custom agent definition path or name |
| `WEEPCODE_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |

### Logging

| Variable | Description |
|----------|-------------|
| `WEEPCODE_LOG_FILE` | Write logs to this file path (the value is used verbatim as the path) |
| `RUST_LOG` | Log level filter (for example `debug`); controls the `WEEPCODE_LOG_FILE` log and headless stderr output |
...
| `WEEPCODE_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore` |

### Telemetry

| Variable | Description |
|----------|-------------|
| `WEEPCODE_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `WEEPCODE_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.weepcode/config.toml` | Main configuration file |
| `~/.weepcode/pager.toml` | TUI appearance configuration |
...
| `~/.weepcode/lsp.json` | LSP server configuration (user-scoped) |
| `~/.weepcode/logs/` | Internal log files (for example `unified.jsonl`, MCP server logs) |
| `.weepcode/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some configuration can be set per-project by placing files in `.weepcode/` within your repository:

| File | What it configures |
|------|--------------------|
| `.weepcode/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.weepcode/config.toml`) |
| `.weepcode/skills/` | Project-specific skills |
| `.weepcode/hooks/` | Project-specific lifecycle hooks |
| `.weepcode/agents/` | Project-specific agent definitions |
| `.weepcode/lsp.json` | LSP server configuration |
| `.weepcode/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.weepcode/lsp.json` | All projects |
| Project | `.weepcode/lsp.json` | Current repository |
...
1. **Project** -- `.weepcode/lsp.json`
2. **User** -- `~/.weepcode/lsp.json`
