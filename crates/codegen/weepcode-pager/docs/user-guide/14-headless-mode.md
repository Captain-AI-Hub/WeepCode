...
Headless mode runs WeepCode non-interactively from the command line. It accepts a single prompt, executes it with full tool access, and returns the result. Use it to automate tasks, script workflows, build integrations, and parse output programmatically.
...
WeepCode processes the prompt, runs any necessary tools, and prints the result to stdout. The process exits when the response is complete.
...
On failure, WeepCode emits an error object (process exit non-zero). Prompt-level
...
WeepCode may also emit `max_turns_reached` and `auto_compact_*` events; treat the list as non-exhaustive and switch on `type`.

---

## Session Management in Headless Mode

By default, each `weepcode -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
weepcode -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
weepcode -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
weepcode -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
weepcode -p "What's the secret number?" --resume abc123
```
...
weepcode -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
weepcode --prompt-file ./prompt.txt
```
...
for file in src/*.js; do
  weepcode -p "Migrate $file from CommonJS to ES modules." --yolo
done
```
...
WeepCode's headless mode can be wrapped as an OpenAI-compatible chat completion API:
...
RESULT=$(weepcode -p "Review this PR for bugs. Output JSON with 'issues' array." \
...
weepcode -p "Format all files" --yolo

# Run tests and fix failures
weepcode -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
| ------------------------------- | ------------------------------------------------------------- |
| `WEEPCODE_API_KEY`        | API key for authentication (required when no browser login)   |
...
- **`WEEPCODE_API_KEY`** — simplest for CI. See [Environment Variables](#environment-variables-for-headless) above.
- **`weepcode login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`weepcode login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `weepcode -p "..." --yolo --rules "Never delete files"`.
- For debugging, raise the log level and capture stderr: `RUST_LOG=debug weepcode -p "..." 2> debug.log`.

---

## Project Root Discovery

When WeepCode starts, it discovers the project root by walking upward from `--cwd`
...
WeepCode discovers that repository as the project root and scopes its discovery (AGENTS.md, skills, git history) to it, which can make
...
WeepCode stores data in `~/.weepcode` (override with `WEEPCODE_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):
...
For containers or CI, mount `~/.weepcode` read-only:

- Pre-populate `auth.json` or use `WEEPCODE_API_KEY`
- Session persistence fails silently (ephemeral)
- Update checks log a warning and skip

```bash
export WEEPCODE_API_KEY="weepcode-..."
export WEEPCODE_DISABLE_AUTOUPDATER=1
weepcode -p "..." --no-auto-update
```
...
| `WEEPCODE_DISABLE_AUTOUPDATER=1`    | Process   |
| Non-TTY stderr (auto-detected)  | Automatic |
| `[cli] auto_update = false`     | Persistent|

`WEEPCODE_DISABLE_AUTOUPDATER` set to a falsy value (`0`, `false`, `off`, `no`, or empty, any
case) counts as not set. The agent SDKs
inject `WEEPCODE_DISABLE_AUTOUPDATER=1` for the non-leader agents they spawn (a falsy value in
the SDK's isolation env keeps updates on), and the stdio agent skips its background update
unless it runs from the managed install (`$WEEPCODE_HOME/bin/weepcode`).
...
- Resume: `weepcode -p "continue" --resume "<id>"` or `weepcode -p "continue" --continue`
