#!/usr/bin/env node
// Runs once after npm install/update. Reads the weepcode binary from the
// matching per-platform optional dependency (@weepcode-official/weepcode-<platform>)
// and installs it to ~/.weepcode/bin/ using versioned filenames:
//
//   Unix:    weepcode-<version>  +  weepcode  (symlink)
//   Windows: weepcode-<version>.exe  +  weepcode.exe  (copy)
//
...
const CANONICAL_DIR = path.join(os.homedir(), '.weepcode', 'bin');
...
if (!SUPPORTED.has(key)) {
    console.error(`@weepcode-official/weepcode: unsupported platform ${key}`);
    process.exit(0);
}
...
function resolvePlatformPackageDir() {
    const platformPkg = `@weepcode-official/weepcode-${key}`;
    try {
...
if (!version) {
    console.error('@weepcode-official/weepcode: unable to determine version');
    process.exit(0);
}
...
function installBinary(binName, sourceDir, vendorSubpath) {
    ...
    if (fs.existsSync(brPath)) {
        ...
        console.error(`@weepcode-official/weepcode: missing binary at ${brPath}`);
        return false;
    }
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error(`@weepcode-official/weepcode: failed to update ${canonicalPath}: ${e2.message}`);
                console.error('Close all running weepcode processes and try again.');
                return false;
            }
        }
    } else {
...
// Uses an exact prefix match + hyphen + digit to avoid weepcode-* matching weepcode-pager-*.
...
if (!platformDir) {
    console.error(`@weepcode-official/weepcode: platform package @weepcode-official/weepcode-${key} not installed.`);
    console.error('  This usually means npm was invoked with --no-optional, or the install failed.');
    console.error('  Try: npm install -g @weepcode-official/weepcode');
    process.exit(0);
}

installBinary('weepcode', platformDir, `weepcode${EXE}`);
cleanupOldVersions('weepcode');
cleanupOldVersions('weepcode-pager');

// Write installer config
const configDir = path.join(os.homedir(), '.weepcode');
...
// Persist the npm registry so `weepcode update` and the trampoline use the same one.
const npmRegistry = process.env.WEEPCODE_NPM_REGISTRY
    || (() => {
        try {
            const resolved = execSync(
                'npm config get @weepcode-official:registry',
...
// Set WEEPCODE_INSTALL_COMPLETIONS=1 to auto-generate to ~/.weepcode/completions.
const WEEPCODE_PATH = path.join(CANONICAL_DIR, `weepcode${EXE}`);
if (process.env.WEEPCODE_INSTALL_COMPLETIONS === '1' && !IS_WINDOWS) {
    try {
        ...
        const zshRes = spawnSync(WEEPCODE_PATH, ['completions', 'zsh'], { encoding: 'utf8' });
        if (zshRes.status === 0) fs.writeFileSync(zshPath, zshRes.stdout);
        console.log('Completions generated to ~/.weepcode/completions (bash/zsh)');
    } catch {}
} else if (!IS_WINDOWS) {
