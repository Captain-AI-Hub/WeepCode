...
//   1. Brotli-compresses the built binary into `../weepcode-<platform>/bin/<bin>.br`
...
// (`@weepcode-official/weepcode`) lists all six as `optionalDependencies` pinned to
// the same version; npm installs only the one matching the host's
// `os` + `cpu` filters.
//
// Why brotli? npm's tarball ceiling is ~200 MB and the raw weepcode binary is
...
const NOTICES_SOURCE = path.resolve(
    npmRoot, '..', '..', 'weepcode-tools', 'THIRD_PARTY_NOTICES.md');
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `weepcode-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] weepcode-${platform}-${arch}@${VERSION}: ` +
