...
- **weepcode login** now requests WeepCode Projects scopes so workspace listing works after consent.
...
- **`weepcode plugin install`** now accepts a marketplace's registered name as a qualifier.
...
- **Fixed background task tracking** when using weepcode -p --no-wait-for-background so tasks are properly reaped on exit.
...
- **weepcode inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct weepcode --minimal --resume command.
...
- **WeepCode no longer crashes** when printing resume hints after the terminal pane has closed.
...
- **weepcode setup --json** prints your team's managed configuration without installing it.
...
- **How-to Guides** modal now shows a tip linking to Ask WeepCode above the footer shortcuts.
...
- **`weepcode worktree ls`** now works as a short alias for `weepcode worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.weepcode/config.toml`.
...
- **File links and media** now work for worktree sessions under ~/.weepcode/worktrees/.
...
- **weepcode inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain weepcode launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive weepcode agent stdio without silent parse drops on session/* calls.
...
- **weepcode --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- **weepcode --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **weepcode update** on Windows no longer fails when a previous .old executable is still running.
...
- **weepcode models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `weepcode plugin marketplace add`.
...
- **Plan files** now default to .weepcode/plan.md to match WeepCode conventions.
...
- **--minimal** flag now shows in `weepcode --help`.
...
- **Tool result previews** no longer paint opaque panels in `weepcode --minimal`.
- **weepcode wrap** now correctly handles quoted strings and shell aliases.
...
- **WeepCode.com chat sessions** now integrate fully with the gateway bridge for model catalog and resume.
...
- **`weepcode update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `weepcode wrap`.
...
- **weepcode agent stdio** now checks for updates in the background like other modes.
...
- **weepcode wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Fixed `weepcode agent stdio` hangs** on Windows when used with persistent clients such as VS Code.
...
- **Added `weepcode wrap`** to run any command with local clipboard support.
...
- **Fixed `weepcode agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or weepcode-desktop.
...
- You can now pass --json-schema to weepcode -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `WEEPCODE_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **OIDC sessions** with WEEPCODE_API_KEY present no longer lose refresh on idle.
...
- **weepcode -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in weepcode -p and weepcode agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `weepcode` after running `weepcode update`.
- Headless (`weepcode -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`weepcode plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **weepcode sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **weepcode quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, weepcode sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, WEEPCODE_HUNK_TRACKER, or config.
...
- **WeepCode** can now export usage metrics and events to your own OpenTelemetry collector when enabled.
...
- **`weepcode dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`weepcode mcp add`** now accepts positional arguments (e.g. `weepcode mcp add filesystem -- npx ...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`weepcode update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.weepcode.
...
- **MCP servers menu** and slash commands now work when starting weepcode outside a project directory.
...
- **weepcode --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `weepcode --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `weepcode update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`weepcode --debug`** now produces per-session log files under ~/.weepcode/debug/ even with a leader process.
...
- **`weepcode update`** now restarts any older running leader so all clients get the new binary.
...
- **`weepcode login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `weepcode agent --leader` without forced interactive login or timeouts.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the WEEPCODE_IMAGE_GEN_HARNESS env var.
...
- **weepcode inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via WEEPCODE_CURSOR_MCPS_ENABLED / WEEPCODE_CURSOR_HOOKS_ENABLED.
...
- **Cursor skills and rules** are now discovered alongside WeepCode and Claude directories.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the weepcode-build model instead of falling back to the active session model.
...
- **image_gen** now uses the higher-quality weepcode-imagine-image-quality model.
