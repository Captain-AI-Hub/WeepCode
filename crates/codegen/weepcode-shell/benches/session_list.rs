...
//!
//! Run: `cargo bench -p weepcode-shell --bench session_list`
...
use weepcode_fast_worktree::{
    ListFilter, WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus,
};
use weepcode_shell::session::info::Info;
use weepcode_shell::session::persistence::Summary;
use weepcode_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
use weepcode_shell::session::unified_list::{ListReq, UnifiedListResult, build_unified_list};
...
impl Fixture {
    fn new(home: TempDir) -> Self {
        ...
        for workspace_index in 0..WORKSPACE_COUNT {
            ...
            let encoded = weepcode_shell::util::weepcode_home::encode_cwd_dirname(&cwd);
    ...
    fn assert_correct(&self, runtime: &tokio::runtime::Runtime) {
        let discovered = weepcode_shell::session::worktree::candidate_worktree_cwds_for_same_repo(
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    repo.remote(
        "origin",
        "https://github.com/weepcode-org/session-list-benchmark.git",
    )
    ...
    let worktree_base = weepcode_shell::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(weepcode_shell::session::worktree::build_label_metadata(
                label, false,
            )),
        })
...
fn write_summary(
    ...
    let summary = Summary {
        ...
        git_remotes: vec!["git@github.com:weepcode-org/benchmark.git".to_owned()],
...
fn bench_session_list(c: &mut Criterion) {
    ...
    unsafe {
        std::env::set_var("WEEPCODE_HOME", home.path());
    }
    assert_eq!(
