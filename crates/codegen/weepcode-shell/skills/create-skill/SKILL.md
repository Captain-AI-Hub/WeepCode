...
name: create-skill
description: >
  Interactively create a new WeepCode skill (SKILL.md + optional scripts/references).
  Use when the user wants to create a skill, scaffold a skill, or runs /create-skill.
metadata:
  short-description: "Create a new WeepCode skill"
---

# Create Skill

Interactively gather requirements from the user and create a fully working WeepCode skill on disk.
...
2. **Scope** - present the user with two options:
   - **Project** (Recommended): `<repo-root>/.weepcode/skills/<name>/SKILL.md` - available only in this repo, shareable with teammates
   - **User**: `~/.weepcode/skills/<name>/SKILL.md` - available in all projects
...
- Trigger phrases and keywords so WeepCode knows when to auto-invoke it
...
Where `<SKILL_DIR>` is:
- User scope: `~/.weepcode/skills/<name>`
- Project scope: `<repo-root>/.weepcode/skills/<name>`
...
2. Tell the user the skill is ready and how to use it:
   ...
   - Automatic: WeepCode will invoke it when the description matches user intent
