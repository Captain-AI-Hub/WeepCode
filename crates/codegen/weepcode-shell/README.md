...
- **Using WeepCode**
  ...
  - [SSH Passthrough](#ssh-passthrough-weepcode-ssh) — Apple Terminal clipboard support
- **Configuration**
...
- **Reference**
  - [Introspection (`weepcode inspect`)](#introspection)
...
- [Building with WeepCode](#building-with-weepcode) — headless API, ACP SDK integration
...
export WEEPCODE_OIDC_ISSUER="https://acme.okta.com"
export WEEPCODE_OIDC_CLIENT_ID="0oa1b2c3d4e5f6g7h8i9"
```

Customers typically also override the API endpoint to point at their own proxy:
```bash
export WEEPCODE_CLI_CHAT_PROXY_BASE_URL="https://weepcode-proxy.acme.com/v1"
```

**3. Run `weepcode`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.weepcode/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
WeepCode is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.

#### How It Works

```
┌──────────────┐     sh -c     ┌────────────────────────┐
│     WeepCode     │──────────────▶│  your auth binary      │
│              │               │                        │
...
1. WeepCode runs your command via `sh -c "<command>"`
...
4. **stdout** → captured by WeepCode and saved to `~/.weepcode/auth.json` as the access token
5. exit 0 → success; exit non-zero → WeepCode falls through to interactive login
...
|--------|---------------|-------------|
| **stdout** | The token — nothing else | WeepCode (parsed and stored in `auth.json`) |
| **stderr** | Login URLs, status messages, errors, progress | The user (displayed in their terminal) |

**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. WeepCode reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
Use JSON if your tokens expire and you want WeepCode to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells WeepCode when to proactively refresh. Without it, WeepCode assumes tokens last 30 days.
...
# Print ONLY the token to stdout (WeepCode captures this)
...
```bash
# Or via environment variables
export WEEPCODE_AUTH_PROVIDER_COMMAND="/usr/local/bin/my-auth-provider"
export WEEPCODE_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export WEEPCODE_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, WeepCode cannot detect expiry proactively and will only refresh after a 401.
...
RESP=$(curl -s -X POST https://auth.acme.com/device/code -d "client_id=weepcode-cli")
...
When WeepCode needs to refresh an expired token, it re-runs your binary with `WEEPCODE_AUTH_EXPIRED=1` set in the environment. Your binary can use this to take a faster silent-refresh path:

```bash
#!/bin/sh
if [ "$WEEPCODE_AUTH_EXPIRED" = "1" ]; then
...
`WEEPCODE_AUTH_EXPIRED` is optional — if your binary ignores it, WeepCode still works. It just runs the same flow for both login and refresh.

### Automatic Credential Refresh

WeepCode supports automatic credential refresh for external auth providers and OIDC. When WeepCode detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. WeepCode handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, WeepCode re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), WeepCode re-runs the binary and retries the request once.
- **OIDC:** If you're using OIDC and have a `refresh_token`, WeepCode silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# WeepCode refreshes tokens 5 minutes before expiry by default.
# Set to 0 to only refresh on 401. Set higher for very short-lived tokens.
export WEEPCODE_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `weepcode login` before starting — WeepCode runs your binary automatically on first launch. You _can_ run `weepcode login` to explicitly hydrate `auth.json` ahead of time if you prefer.
- If both OIDC and `auth_provider_command` are configured: at **login** time, WeepCode tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
weepcode --debug-file /tmp/weepcode-auth.log -p "hello"
tail -f /tmp/weepcode-auth.log
```
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.weepcode/auth.json` (set by `weepcode login`)                                                                                                                              |
| `X-WeepCode-Token-Auth: weepcode-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-weepcode-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `weepcode-build` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `weepcode-build`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `weepcode login` to refresh.
...
weepcode --cwd ~/projects/my-app

# Start with an initial task
weepcode --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
weepcode --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
weepcode --always-approve --prompt "Format all files"
```
...
Run WeepCode non-interactively from the command line. Use headless mode when you need to:
...
| `-m, --model <MODEL>`   | Model to use (e.g., `weepcode-build`)               |
...
weepcode -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
weepcode -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
weepcode -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
weepcode -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
weepcode -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
weepcode -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
weepcode -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
weepcode -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
weepcode -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
weepcode --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
weepcode -p "What does this project do?"

# Use a specific model
weepcode -p "Optimize this function" -m weepcode-build

# Get JSON output for parsing
weepcode -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
weepcode -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
weepcode -p "Remember: the secret number is 42" --output-format json
weepcode -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
weepcode -p "Continue where we left off" -c

# Run in a different directory
weepcode -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
weepcode -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
weepcode -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
weepcode -p "Generate a README" > README.md

# Parse JSON output with jq
weepcode -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
weepcode -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | weepcode -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  weepcode -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
weepcode -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
Run WeepCode as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `weepcode-build`)                           |
...
```bash
weepcode agent headless --weepcode-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`weepcode ssh`)

Use `weepcode ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.

```bash
# Basic usage (same args as ssh)
weepcode ssh user@host

# With SSH flags
weepcode ssh -t user@host
weepcode ssh -L 8080:localhost:8080 user@host

# With remote command
weepcode ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `weepcode ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.

This runs entirely locally.

---

## Building with WeepCode

WeepCode can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `weepcode -p` and parses JSON output.
...
class WeepCodeChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["weepcode", "-p", prompt, "-m", model, "--cwd", self.cwd,
                "--output-format", "streaming-json" if stream else "json", "--always-approve"]

    async def create(self, messages, model="weepcode-build", stream=False):
...
class WeepCodeChat {
  ...
  async create(
    ...
    { model = "weepcode-build", stream = false } = {},
  ) {
    const prompt =
      messages.length === 1
        ? messages[0].content
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("weepcode", this.buildArgs(prompt, model, true));
...
# Usage
async def main():
    client = await WeepCodeACPChat(cwd=".").init()
...
class WeepCodeACPChat {
  ...
  async init() {
    this.proc = spawn("weepcode", ["agent", "stdio"]);
...
// Usage
const client = await new WeepCodeACPChat(".").init();
...
WeepCode implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
WeepCode reads configuration from `~/.weepcode/config.toml`. If the file doesn't exist, WeepCode uses sensible defaults. You only need to specify values you want to override.
...
[models]
default = "weepcode-build"           # model used for new sessions
web_search = "weepcode-4.20-multi-agent"   # model used by the web_search tool
...
telemetry = false                      # anonymous usage telemetry (env: WEEPCODE_TELEMETRY_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: WEEPCODE_RESPECT_GITIGNORE)
...
Configure telemetry destinations and credentials. Empty values disable the corresponding sink. Env vars take precedence over config values. Builds from the public source tree carry no telemetry defaults: `events_url` and `events_api_key` are unset, so nothing is sent unless you supply values here or via env.

```toml
[telemetry]
events_url = "https://example.com/events"  # env: WEEPCODE_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: WEEPCODE_TELEMETRY_EVENTS_API_KEY
trace_upload = true                         # env: WEEPCODE_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `WEEPCODE_TELEMETRY_BUILD_EVENTS_URL` and `WEEPCODE_TELEMETRY_BUILD_EVENTS_API_KEY` in the build environment. Config-file and runtime env values override build-time defaults.

### LSP Servers

WeepCode can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives WeepCode language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, WeepCode can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — WeepCode can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

WeepCode looks for server definitions in:

- project config: `<repo>/.weepcode/lsp.json`
- user config: `~/.weepcode/lsp.json`
...
- LSP tools are enabled (`WEEPCODE_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
WEEPCODE_LSP_TOOLS=1 weepcode
```
...
If LSP tools are enabled but no usable server config is found, WeepCode emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
WeepCode does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `WEEPCODE_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
With this config, `weepcode` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). WeepCode reads these files and appends their contents to the system prompt.

WeepCode scans for agent rules in this order:

1. `~/.weepcode/` (global rules)
...
Within each directory, WeepCode checks for these filenames:
...
Skills are reusable prompt packages that extend WeepCode with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.

### Skill Locations

WeepCode discovers skills from these directories (in priority order):

| Location                    | Scope | Priority |
| --------------------------- | ----- | -------- |
| `./.weepcode/skills/`           | Local | Highest  |
| `<repo_root>/.weepcode/skills/` | Repo  | Medium   |
| `~/.weepcode/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.weepcode/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how WeepCode decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, WeepCode invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when WeepCode automatically invokes the skill. Be specific about trigger phrases and use cases.
...
WeepCode discovers agent definitions from `.weepcode/agents/` (project), `~/.weepcode/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `WEEPCODE_AGENT` env var
4. Default `weepcode-build` agent
...
```bash
weepcode --agent-profile ./my-agent.md
# or
export WEEPCODE_AGENT="my-custom-agent"
```
...
```bash
export WEEPCODE_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "weepcode-build"              # route explore to a lighter model
```
...
model = "weepcode-build"
prompt_file = ".weepcode/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".weepcode/personas/concise.md"  # or load from file
```

Both are also discovered from `.weepcode/roles/*.toml` and `.weepcode/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend WeepCode with additional tools, skills, and MCP servers from external packages.

### Plugin Locations

| Location                    | Scope   |
| --------------------------- | ------- |
| `.weepcode/plugins/`            | Project |
| `~/.weepcode/plugins/`          | User    |
...
WeepCode discovers hooks from `.weepcode/hooks/` in the project directory. Manage them with:
...
**Credential resolution order:** `api_key` → `env_key` → `WEEPCODE_API_KEY`. If neither `api_key` nor `env_key` is set, WeepCode falls back to the global `WEEPCODE_API_KEY` environment variable.

The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, WeepCode falls back to built-in defaults for known models.
...
[model.weepcode-4.20-0309-reasoning]
temperature = 0.5
api_key = "sk-custom"
```

**How it works:** When you override a built-in model, WeepCode starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
**Web search model:** Set `[models] web_search`, `WEEPCODE_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `weepcode models` output). You also need a `[model.*]` entry so WeepCode knows
...
>
> [model.my-custom-model]              # 2. tell WeepCode how to reach it
...
Point WeepCode at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `WEEPCODE_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `WEEPCODE_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `WEEPCODE_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export WEEPCODE_MODELS_BASE_URL="https://api.acme.com/v1"
export WEEPCODE_API_KEY="weepcode-..."
weepcode
```

WeepCode fetches the model list from `{WEEPCODE_MODELS_BASE_URL}/models` on startup and sends inference requests to `WEEPCODE_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `WEEPCODE_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.weepcode/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, WeepCode uses API key auth (`Authorization: Bearer`) instead of session auth. `weepcode login` is not required — only the API key.

---

## MCP Servers

Extend WeepCode's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.weepcode/config.toml`:
...
MCP servers can also be configured per-project in `.weepcode/config.toml`. WeepCode walks from the current directory up to the git repo root, loading `.weepcode/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.weepcode/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.weepcode/config.toml` | This repository   | ↑        |
| `<cwd>/.weepcode/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.weepcode/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.weepcode/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.weepcode/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.weepcode/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `WEEPCODE_MEMORY=1` / `[memory] enabled = true` in config).

Cross-session memory lets WeepCode remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.weepcode/memory/`:
- **Global** (`~/.weepcode/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.weepcode/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.weepcode/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries

Workspace directories are suffixed with a short hash for uniqueness (e.g. `weepcode-a3f7b2c9/`). The hash is derived from the git remote URL so all clones and worktrees of the same repository share the same memory directory.
...
```bash
# Per-session flag
weepcode --experimental-memory

# Environment variable (persists for the shell session)
export WEEPCODE_MEMORY=1
weepcode
...
At the end of each session, WeepCode saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.weepcode/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
WeepCode searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
weepcode memory edit

# Open global MEMORY.md
weepcode memory edit --global

# Show memory statistics: file count, chunk count, and index size
weepcode memory stats
```

### Configuration reference

Key options under `[memory]` in `~/.weepcode/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.weepcode/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, WeepCode emits the `weepcode-shell-memory_injection`
...
WeepCode can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.weepcode/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.weepcode/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.weepcode/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.weepcode/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.weepcode/sandbox.toml` (global) or `.weepcode/sandbox.toml` (per-project):
...
```bash
weepcode --sandbox devbox
```

### How It Works

The sandbox is applied to the **entire weepcode process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), WeepCode logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.weepcode/sandbox-events.jsonl`
...
Use `weepcode inspect` to see everything WeepCode discovers in the current directory:

```bash
weepcode inspect          # human-readable output
weepcode inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.weepcode/skills/`, `~/.weepcode/skills/`, plugins, and config paths
...
WeepCode automatically discovers configuration from Claude Code directories alongside native `.weepcode/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How WeepCode uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.weepcode/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by WeepCode at runtime.

---

## Built-in Tools

WeepCode includes these tools by default:
...
WeepCode automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.weepcode/sessions/`, organized by URL-encoded working directory:

```
~/.weepcode/sessions/<encoded-cwd>/<session-id>/
...
weepcode -p "Remember: X=42" -s my-session
weepcode -p "What is X?" -s my-session

# Resume existing session (errors if not found)
weepcode -p "Continue" -r my-session

# Continue most recent session in current directory
weepcode -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
weepcode -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.weepcode/config.toml` | Configuration file                                  |
| `~/.weepcode/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.weepcode/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.weepcode/memory/`     | Cross-session memory files and index                |
| `~/.weepcode/skills/`     | User-scoped skill definitions                       |
| `~/.weepcode/plugins/`    | User-scoped plugins                                 |
| `~/.weepcode/agents/`     | User-scoped agent definitions                       |
| `.weepcode/config.toml`   | Project-scoped config (MCP servers)                 |
| `.weepcode/skills/`       | Project-scoped skill definitions                    |
| `.weepcode/plugins/`      | Project-scoped plugins                              |
| `.weepcode/agents/`       | Project-scoped agent definitions                    |
| `.weepcode/hooks/`        | Project-scoped hooks                                |
| `.weepcode/lsp.json`      | LSP server configuration                            |
...
| `WEEPCODE_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `WEEPCODE_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `WEEPCODE_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `WEEPCODE_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `WEEPCODE_HOME`                     | Override config directory (default: `~/.weepcode`)                                                           |
| `WEEPCODE_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `WEEPCODE_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `WEEPCODE_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `WEEPCODE_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `WEEPCODE_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `WEEPCODE_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `WEEPCODE_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `WEEPCODE_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `WEEPCODE_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.weepcode/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `WEEPCODE_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `weepcode` commands and flags.
...
weepcode completions bash > ~/.local/share/bash-completion/completions/weepcode
```

Reload your shell or run `source ~/.bashrc`.

Alternative (WeepCode-managed location):

```bash
mkdir -p ~/.weepcode/completions/bash
weepcode completions bash > ~/.weepcode/completions/bash/weepcode.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.weepcode/completions/bash/weepcode.bash" ]] && source "$HOME/.weepcode/completions/bash/weepcode.bash"
```
...
```zsh
fpath=("$HOME/.weepcode/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `weepcode` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `weepcode --debug` or `WEEPCODE_LOG_FILE` instead:

```bash
# Per-session debug log (~/.weepcode/debug/<sessionId>.txt)
weepcode --debug

# Log to a custom path
WEEPCODE_LOG_FILE=/tmp/weepcode-debug.log weepcode

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.weepcode/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `WEEPCODE_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
WEEPCODE_LOG_FILE=/tmp/weepcode-debug.log RUST_LOG="info,weepcode_shell::auth=debug" weepcode
```
...
weepcode --debug-file /tmp/weepcode-auth.log -p "hello"
grep "auth:" /tmp/weepcode-auth.log
```
...
ls ~/.weepcode/sessions/

# Read session metadata
cat ~/.weepcode/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.weepcode/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.weepcode/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
