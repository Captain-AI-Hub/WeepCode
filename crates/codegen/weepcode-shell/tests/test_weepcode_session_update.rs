...
//! 1. WeepCode session notifications (e.g., diff_review) can be sent via ext_notification
...
use weepcode_shell::extensions::notification::{
    DiffContent, SessionNotification, SessionUpdate as WeepcodeSessionUpdate,
};
use weepcode_shell::session::info::Info as SessionInfo;
use weepcode_shell::session::persistence::default_model_id;
use weepcode_shell::session::storage::{JsonlStorageAdapter, SessionUpdate, StorageAdapter};

/// Test that WeepCode session notifications round-trip through storage correctly.
#[tokio::test]
async fn test_weepcode_session_notification_storage_roundtrip() {
    ...
    let weepcode_notification = SessionNotification {
        session_id: session_id.clone(),
        update: WeepcodeSessionUpdate::DiffReview {
    ...
    adapter
        .append_update(
            &info,
            &SessionUpdate::Xai(Box::new(weepcode_notification.clone())),
        )
        .await
        .unwrap();
    ...
    assert_eq!(
        loaded.updates.len(),
        2,
        "Should have 2 updates (1 WeepCode + 1 ACP)"
    );

    // Verify WeepCode notification
    match &loaded.updates[0] {
        SessionUpdate::Xai(notification) => {
            assert_eq!(notification.session_id, session_id);
            match &notification.update {
                WeepcodeSessionUpdate::DiffReview { content } => {
...
async fn test_turn_completed_round_trips_through_storage() {
    ...
    adapter
        .append_update(&info, &SessionUpdate::Xai(Box::new(weepcode_notification)))
        .await
        .unwrap();
...
/// Test that totalTokens can be extracted from both ACP and WeepCode notifications.
#[tokio::test]
async fn test_extract_total_tokens_from_mixed_updates() {
    ...
    // Add WeepCode notification with totalTokens
    let weepcode_notification = SessionNotification {
        session_id: session_id.clone(),
        update: WeepcodeSessionUpdate::DiffReview { content: vec![] },
...
#[test]
fn test_weepcode_session_notification_serialization() {
