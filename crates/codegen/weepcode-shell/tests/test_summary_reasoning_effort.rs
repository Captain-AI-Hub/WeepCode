...
//!
//! Each test spawns a real `weepcode agent stdio` process against a mock
//! inference server and asserts on the persisted `summary.json`.
//!
//! Run locally:
//! ```bash
//! cargo test -p weepcode-shell --test test_summary_reasoning_effort -- --ignored
//! ```
...
/// Find `summary.json` for `session_id` under `<home>/.weepcode/sessions/` and
...
fn read_summary(home: &std::path::Path, session_id: &str) -> serde_json::Value {
    let sessions_root = home.join(".weepcode").join("sessions");
...
async fn test_fresh_session_persists_reasoning_effort() {
    with_local_set(|| async {
        ...
        let weepcode_dir = home.path().join(".weepcode");
        std::fs::create_dir_all(&weepcode_dir).expect("create .weepcode dir");
        std::fs::write(
...
"#,
        ...
        let client = WeepcodeStdioClient::spawn_with_home(&server, workdir.path(), home).await;
