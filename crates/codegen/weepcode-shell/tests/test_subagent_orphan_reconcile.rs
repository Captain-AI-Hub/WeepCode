...
//!
//! This test spawns a real `weepcode agent stdio` process, seeds an orphaned
...
//! ```bash
//! cargo test -p weepcode-shell --test test_subagent_orphan_reconcile -- --ignored
//! ```
...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        // WeepcodeStdioClient sets HOME=<temp>; the binary uses <HOME>/.weepcode as WEEPCODE_HOME.
        let weepcode_home = shared_home.path().join(".weepcode");
        let session_dir = locate_session_dir(&weepcode_home, session_id.0.as_ref());
