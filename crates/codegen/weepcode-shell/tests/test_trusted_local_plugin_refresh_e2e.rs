...
//! 4. Smoke-validate session JSON under `$WEEPCODE_HOME/sessions/` after exit.
//!
//! Requires a built `weepcode` binary (`WEEPCODE_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p weepcode-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p weepcode-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use weepcode_agent::plugins::SharedPluginRegistryHandle;
use weepcode_agent::plugins::discovery::DiscoveryConfig;
use weepcode_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _weepcode_guard = EnvVarGuard::set("WEEPCODE_HOME", &weepcode_home);
    ...
    let mut registry = InstallRegistry::empty(weepcode_home.join("installed-plugins"));
    ...
    // weepcode_home() caches the first WEEPCODE_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        weepcode_agent::plugins::PluginScope::CliOverride
    );
...
#[tokio::test]
#[ignore = "requires pre-built weepcode binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/WEEPCODE_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via weepcode_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _weepcode_guard = EnvVarGuard::set("WEEPCODE_HOME", &weepcode_home);
    ...
    cmd.env("HOME", &home);
    cmd.env("WEEPCODE_HOME", &weepcode_home);
    ...
    // Smoke: session storage under WEEPCODE_HOME/sessions has JSON artifacts.
