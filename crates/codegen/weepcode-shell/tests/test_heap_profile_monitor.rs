...
use weepcode_shell::auth::{AuthManager, AuthMode, WeepcodeAuth, WeepcodeComConfig};
use weepcode_shell::heap_profile::{
...
use weepcode_test_support::{EnvGuard, MockInferenceServer};
use weepcode_version::TEST_VERSION_ENV;
...
fn seed_auth_json(home: &Path, token: &str) {
    let scope = WeepcodeComConfig::default().auth_scope();
...
fn assert_storage_auth(uploads: &[weepcode_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
...
impl Harness {
    async fn start(remote: RemoteSettings) -> Self {
        ...
        let auth = Arc::new(AuthManager::new(home.path(), WeepcodeComConfig::default()));
