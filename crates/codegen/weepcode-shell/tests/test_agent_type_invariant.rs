...
//! - Session resume
//!
//! Each test spawns a real `weepcode agent stdio` process, speaks the full ACP
...
//! Run locally:
//! ```bash
//! cargo test -p weepcode-shell --test test_agent_type_invariant -- --ignored
//! ```
...
fn invalidate_models_cache(home: &std::path::Path) {
    let cache = home.join(".weepcode").join("models_cache.json");
    if cache.exists() {
        std::fs::remove_file(&cache).expect("failed to delete models_cache.json");
    }
}
/// Start a mock server with two models:
/// - `default-model`: no agent_type (→ defaults to "weepcode-build")
...
/// - `model-a`: no agent_type (→ "weepcode-build")
/// - `model-b`: no agent_type (→ "weepcode-build")
...
/// `weepcode-build` harness. The system prompt sent to the LLM should contain
/// the weepcode-build identity string.
#[tokio::test]
#[ignore]
async fn test_default_model_uses_weepcode_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("WeepCode") || sys_prompt.contains("weepcode"),
            "default model should use weepcode-build harness\nsystem prompt preview: {}",
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        drop(writer);
        let reader =
            WeepcodeStdioClient::spawn_with_home(&server, workdir.path(), shared_home).await;
        ...
        let original_has_weepcode =
            original_sys_prompt.contains("WeepCode") || original_sys_prompt.contains("weepcode");
        let resumed_has_weepcode =
            resumed_sys_prompt.contains("WeepCode") || resumed_sys_prompt.contains("weepcode");
        assert_eq!(
            original_has_weepcode,
            resumed_has_weepcode,
            "resumed session should use the same harness as the original\n\
             original identity markers: weepcode={original_has_weepcode}\n\
             resumed identity markers: weepcode={resumed_has_weepcode}\n\
...
/// default to `"weepcode-build"`. This exercises the serde default.
#[tokio::test]
#[ignore]
async fn test_model_without_agent_type_defaults_to_weepcode_build() {
    with_local_set(|| async {
            ...
            assert!(
                sys_prompt.contains("WeepCode") || sys_prompt.contains("weepcode"),
                "model without agent_type should default to weepcode-build harness\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
}
/// The `WEEPCODE_AGENT` escape hatch should override the model's agent_type.
/// Setting `WEEPCODE_AGENT=weepcode-build` with an alternate-agent model should use
/// weepcode-build harness.
#[tokio::test]
#[ignore]
async fn test_weepcode_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.args(["agent", "stdio"])
                ...
                .kill_on_drop(true);
            weepcode_test_support::env::test_env_cmd_tokio(
                &mut cmd,
                &server.url(),
                home.path(),
            );
            cmd.env("WEEPCODE_AGENT", "weepcode-build");
            let mut child = cmd.spawn().expect("spawn weepcode");
            ...
            let incoming = weepcode_acp_lib::LineBufferedRead::spawn_local(incoming);
            ...
            assert!(
                sys_prompt.contains("WeepCode") || sys_prompt.contains("weepcode"),
                "WEEPCODE_AGENT=weepcode-build should override cursor model's agent_type\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
