...
use std::sync::Arc;
/// Credentials for authenticating with weepcode backend services.
///
...
/// User token (WeepCode users) sends `Bearer` + `X-WeepCode-Token-Auth: weepcode-cli`.
/// Deployment key takes precedence when both are present.
#[derive(Clone)]
pub struct WeepcodeAuthCredentials {
...
impl std::fmt::Debug for WeepcodeAuthCredentials {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("WeepcodeAuthCredentials")
            .field(
                "user_token",
...
impl WeepcodeAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your WEEPCODE_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `weepcode login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn resolve(&self) -> WeepcodeAuthCredentials {
    ...
    /// the bearer.
    pub async fn resolve_async(&self) -> WeepcodeAuthCredentials {
    ...
    pub fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        let builder = if let Some(ref key) = self.deployment_key {
            ...
            builder
                ...
                .header(
                    obfstr::obfstr!("X-WeepCode-Token-Auth"),
                    obfstr::obfstr!("weepcode-cli"),
                )
        } else {
            builder
        };
...
impl weepcode_auth::HttpAuth for WeepcodeAuthCredentials {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        WeepcodeAuthCredentials::apply(self, builder, base_url)
    }
}
#[cfg(test)]
mod tests {
    ...
    fn resolve_returns_token_when_not_expired() {
        ...
        let creds = WeepcodeAuthCredentials::new(None).with_auth_manager(mgr);
    ...
    fn resolve_returns_token_during_early_invalidation_window() {
        ...
        let creds = WeepcodeAuthCredentials::new(None).with_auth_manager(mgr.clone());
    ...
    fn resolve_returns_static_token_when_no_auth_manager() {
        let creds = WeepcodeAuthCredentials::new(Some("static-token".into()));
