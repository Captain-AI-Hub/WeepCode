...
use weepcode_hooks::discovery::HookSource;
...
pub fn discover_hook_source_paths(
    ...
    // sources. Native .weepcode/hooks/ directories are still scanned (they hold
    ...
    // user_weepcode_home() is None when no home resolves, so inspect lists the same
    // sources a live session loads, instead of a cwd-relative .weepcode.
    ...
    if let Some(ref weepcode) = weepcode {
        global.push(weepcode.join("hooks"));
    }

    let custom_paths: Vec<PathBuf> = weepcode
        .as_ref()
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".weepcode").join("hooks"));
        if !skip_cursor {
...
pub fn discover_hooks(
    ...
    weepcode_hooks::discovery::HookRegistry,
    Vec<weepcode_hooks::error::HookError>,
) {
