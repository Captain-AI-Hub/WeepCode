pub mod config;
pub mod hooks;
pub mod weepcode_auth_credentials;

// The foundation utilities live in `weepcode-shell-base` (upstream of this
// crate so they build in parallel). Re-exported at the original paths so
// existing `crate::util::…` / `weepcode_shell::util::…` users compile
// unchanged.
pub use weepcode_shell_base::util::*;
