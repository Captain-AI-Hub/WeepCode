...
/// Effective config: merged disk layers (system-managed → managed → user →
/// requirements). Kept under this historical name for its many call sites; the
/// campaign overlay it used to apply is gone, so this is a straight layer merge.
pub fn load_effective_config() -> std::io::Result<TomlValue> {
    weepcode_config::load_effective_config_disk_only()
}
/// Alias of [`load_effective_config`] for call sites documenting that they must
/// never consult remote state (one-shot CLI entrypoints).
pub fn load_effective_config_disk_only() -> std::io::Result<TomlValue> {
    weepcode_config::load_effective_config_disk_only()
}
...
pub fn load_relay_sync_enabled_sync() -> bool {
    toml_bool_sync(Some("WEEPCODE_RELAY_SYNC_ENABLED"), "relay", "enabled")
}
...
/// 1. Nearest project `.weepcode/config.toml` with `[permission]` section (from cwd upward)
/// 2. Global `~/.weepcode/config.toml` `[permission]` section
///
...
"#;
            ...
            && let Some(TomlValue::Table(models)) = table.get("models")
        {
            let default = models
                ...
                .and_then(|v| v.as_str())
                .map(|s| s.to_string());
            assert_eq!(default.as_deref(), Some("weepcode-code-fast-1"));
        } else {
