...
/// canonical theme name (`weepcodenight`, `tokyonight`, `auto`, etc.).
...
/// Persist `[models].default` (+ optional reasoning effort). `None` clears the
/// field.
///
/// This is the only sanctioned writer of `models.default`. Caller must validate
/// `value` against the model catalog first. Empty string clears the field
/// (falls back to remote/built-in default). Length over
/// [`MAX_DEFAULT_MODEL_LEN`] returns `Err`.
pub async fn persist_models_default(
    value: Option<String>,
    reasoning_effort: Option<weepcode_sampling_types::ReasoningEffort>,
) -> anyhow::Result<()> {
    let s = value.unwrap_or_default();
    if s.len() > MAX_DEFAULT_MODEL_LEN {
        anyhow::bail!(
...
/// Persist `[models].default` via [`persist_models_default`].
///
