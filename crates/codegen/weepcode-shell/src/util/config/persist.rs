...
/// Process-wide write lock for `~/.weepcode/config.toml`.
///
...
mod tests {
    ...
    fn merge_section_updates_modeled_fields_preserving_unmodeled() {
        ...
        ui.insert(
            ...
            TomlValue::String("weepcodeday".into()),
        );
        table.insert("ui".into(), TomlValue::Table(ui));
...
"#;
        ...
        assert_eq!(cfg.ui.auto_light_theme.as_deref(), Some("weepcodeday"));
...
"#;
        ...
        cfg.models.default = Some("weepcode-4".to_string());
    ...
    fn models_config_serializes_only_some_fields() {
        let m = crate::agent::config::ModelsConfig {
            default: Some("weepcode-3".to_string()),
            ..Default::default()
        };
    ...
    fn merge_section_models_only_updates_set_fields_preserves_others() {
        ...
        let cfg = crate::agent::config::ModelsConfig {
            default: Some("weepcode-new".to_string()),
            ..Default::default()
        };
    ...
    fn persist_preferred_model_flow_roundtrips_via_load_and_new_from_toml_cfg() {
        let original = "[models]\ndefault = \"weepcode-old\"\nweb_search = \"some-search\"\n";
        let root: TomlValue = toml::from_str(original).unwrap();
        let mut cfg = load_config_from_toml(&root);
        cfg.models.default = Some("weepcode-persisted".to_string());
    ...
    mod resolve_auto_compact {
        ...
        const TEST_MODEL: &str = "weepcode-4.5";
        const OTHER_MODEL: &str = "weepcode-4.3";

        /// Serialize tests that mutate `WEEPCODE_AUTO_COMPACT_THRESHOLD_PERCENT`.
    ...
    fn settings_helpers_target_correct_ui_fields() {
        ...
        let cfg = apply(|cfg| cfg.ui.auto_light_theme = Some("weepcodeday".to_string()));
        assert_eq!(cfg.ui.auto_light_theme, Some("weepcodeday".to_string()));
