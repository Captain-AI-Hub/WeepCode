...
use weepcode_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use weepcode_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `weepcode-config-types` (config
...
pub use weepcode_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use weepcode_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.weepcode/config.toml` at each level.
/// Project-scoped `.weepcode/config.toml` entries override global `~/.weepcode/config.toml`
...
/// `.weepcode/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.weepcode/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.weepcode/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.weepcode/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .weepcode/config.toml"
                );
...
pub fn mcp_preferences_path() -> PathBuf {
    weepcode_config::weepcode_home().join("mcp_preferences.json")
}
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    weepcode_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.weepcode/config.toml`.
...
/// Upsert an MCP server entry in `~/.weepcode/config.toml`.
///
...
/// config file, e.g. a project-scoped `.weepcode/config.toml`.
...
/// Delete an MCP server entry from `~/.weepcode/config.toml`.
///
...
/// config file, e.g. a project-scoped `.weepcode/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = weepcode_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.weepcode/config.toml` files.
...
// `.mcp.json` discovery moved to `weepcode-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use weepcode_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.weepcode/config.toml` files from `cwd` up to the
...
fn config_path() -> PathBuf {
    crate::util::weepcode_home::weepcode_home().join("config.toml")
}

/// Path to the user-level config file (`~/.weepcode/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.weepcode/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".weepcode").join("config.toml")
}
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.weepcode/skills", "~/.weepcode/skills/special/SKILL.md"]
ignore = ["~/.weepcode/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.weepcode/skills", "~/.weepcode/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.weepcode/skills/noisy/SKILL.md"]);
    }
...
"#;
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${WEEPCODE_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
    ...
    fn mcp_json_all_toml_names_includes_disabled() {
        ...
        std::fs::create_dir_all(&weepcode_dir).unwrap();
        std::fs::write(
            weepcode_dir.join("config.toml"),
            r#"
