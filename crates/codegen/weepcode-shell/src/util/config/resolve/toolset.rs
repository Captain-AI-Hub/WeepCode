...
/// [`weepcode_config::env_bool`] parser (`WEEPCODE_TOOLS_FIND_BFS` /
/// `WEEPCODE_TOOLS_GREP_UGREP`, plus the `WEEPCODE_FIND_BFS` / `WEEPCODE_GREP_UGREP` aliases).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.weepcode/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`weepcode_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub fn resolve_search_tools_enabled(
    ...
    let disable = weepcode_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = weepcode_config::env_bool(primary).or_else(|| weepcode_config::env_bool(alias));
    ...
    (
        resolve("WEEPCODE_TOOLS_FIND_BFS", "WEEPCODE_FIND_BFS", "find_bfs"),
        resolve(
            "WEEPCODE_TOOLS_GREP_UGREP",
            "WEEPCODE_GREP_UGREP",
            "grep_ugrep",
        ),
    )
...
/// the shared [`weepcode_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "WEEPCODE_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`WEEPCODE_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            weepcode_tools::implementations::weepcode_build::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}
...
///
/// Precedence: requirements > env (`WEEPCODE_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        weepcode_tools::implementations::weepcode_build::ask_user_question::response_timeout_env_secs(),
...
) -> weepcode_tools::implementations::weepcode_build::ask_user_question::AskUserQuestionParams {
...
mod ask_user_question_timeout_tests {
    ...
    use weepcode_tools::implementations::weepcode_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
...
mod tests {
    ...
    // Assumes WEEPCODE_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
