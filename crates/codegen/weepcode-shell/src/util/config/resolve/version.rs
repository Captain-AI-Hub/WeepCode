...
///
/// Reads `stable_version` from `~/.weepcode/version.json` (written by the
/// installer) and compares the compiled-in version against it:
...
pub fn channel_name_from_cache() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        let version_path = crate::util::weepcode_home::weepcode_home().join("version.json");
        let content = std::fs::read_to_string(&version_path).ok()?;
        let parsed: serde_json::Value = serde_json::from_str(&content).ok()?;
        let stable = parsed.get("stable_version")?.as_str()?;
        let current = semver::Version::parse(weepcode_version::VERSION).ok()?;
