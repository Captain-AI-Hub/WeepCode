...
fn make_auth(expires_at: Option<DateTime<Utc>>, create_time: DateTime<Utc>) -> WeepcodeAuth {
    WeepcodeAuth {
...
fn auth_scope_uses_oauth2_when_present() {
    let cfg = WeepcodeComConfig {
        oauth2: Some(crate::auth::OAuth2ProviderConfig {
            issuer: crate::auth::config::WEEPCODE_OAUTH2_ISSUER.to_owned(),
    ...
    assert_eq!(
        ...
        format!(
            "{}::client-123",
            crate::auth::config::WEEPCODE_OAUTH2_ISSUER
        ),
    );
}

#[test]
fn auth_scope_falls_back_when_no_oauth_provider() {
    // No OIDC and no OAuth2 provider configured (pure BYOK deployment): the
    // scope is a stable placeholder that matches no on-disk session entry.
    let cfg = WeepcodeComConfig::default();
    assert!(cfg.oidc.is_none() && cfg.oauth2.is_none());
    assert_eq!(cfg.auth_scope(), "weepcode::oauth2-unconfigured");
}
...
fn legacy_scope_fallback_reads_old_auth_json() {
    ...
    // a machine that was authenticated with an older weepcode version).
...
fn new_scope_takes_precedence_over_legacy() {
    ...
    let legacy_auth = WeepcodeAuth {
        key: "legacy-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    let new_auth = WeepcodeAuth {
        key: "new-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
...
async fn team_login_then_personal_evicts_team_token() {
    ...
    let personal_auth = WeepcodeAuth {
...
fn is_data_collection_disabled_matrix() {
    let cases: &[(&[&str], bool)] = &[
        ...
        (&["BLOCKED_REASON_NO_LOGS_MODERATED"], true),
        // No blocked reasons
        (&[], false),
        ...
        (&["BLOCKED_REASON_BILLING", "BLOCKED_REASON_NO_LOGS"], true),
    ];
    for (reasons, expected) in cases {
        ...
        assert_eq!(
            ...
            *expected,
            "reasons={reasons:?} expected={expected}",
        );
    }
}
...
fn manager_collection_predicates_fail_directions() {
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), WeepcodeComConfig::default()));
    ...
    mgr.hot_swap(WeepcodeAuth::test_default());
    assert!(!mgr.is_data_collection_disabled());
    assert!(mgr.allows_data_collection());

    // ZDR team user: both predicates suppress collection.
...
fn try_use_disk_token_accepts_different_key_on_server_rejected() {
    ...
    let mem_auth = WeepcodeAuth {
        key: "old-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    mgr.hot_swap(mem_auth);

    let disk_auth = WeepcodeAuth {
        key: "new-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
...
async fn auth_returns_expired_api_key_consistently_with_current() {
    ...
    // funnel the user back through `weepcode login`.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
async fn start_proactive_refresh_is_idempotent() {
    ...
    let stale_api_key = WeepcodeAuth {
        key: "stale-api-key".into(),
        auth_mode: AuthMode::ApiKey,
        create_time: Utc::now() - Duration::days(60),
        expires_at: None,
...
async fn permanent_failure_reads_absent_after_clear_so_auth_reports_not_logged_in() {
    ...
    // User runs `weepcode logout` which calls clear().
    mgr.clear().unwrap();

    // The diagnostic the user now sees on the next request should be
    // "Not logged in. Run `weepcode login`.", not the stale invalid_grant.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
/// `has_weepcode_code_access`. The `&mut WeepcodeAuth` signature already
...
/// contributor adding a token-like field to both `WeepcodeAuth` and
/// `UserInfo` is forced to look here.
#[test]
fn apply_user_info_enrichment_preserves_token_fields() {
    let mut disk = WeepcodeAuth {
        key: "ROT_KEY".into(),
        ...
        has_weepcode_code_access: Some(true),
    ...
    assert_eq!(
        disk.has_weepcode_code_access,
        snapshot.has_weepcode_code_access
    );
...
async fn current_api_key_async_drives_refresh_chain() {
    ...
    use weepcode_tools::types::ApiKeyProvider;

    let _xai = EnvGuard::unset("WEEPCODE_API_KEY");
    let _legacy = EnvGuard::unset("WEEPCODE_CODE_WEEPCODE_API_KEY");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), WeepcodeComConfig::default()));
    mgr.hot_swap(WeepcodeAuth {
...
/// Without this, a JWT that is time-valid but rejected by the server is
...
async fn refresh_chain_server_rejected_bypasses_valid_token_double_check() {
    ...
    // Seed a valid (non-expired) token — simulates a JWT that the server
    // rejected but is otherwise fine.
    let valid_but_rejected = WeepcodeAuth {
        key: "server-rejected-jwt".into(),
        auth_mode: AuthMode::Oidc,
        refresh_token: Some("rt-original".into()),
        expires_at: Some(Utc::now() + Duration::hours(1)),
        ..WeepcodeAuth::test_default()
    ...
    assert_eq!(mgr.current().unwrap().key, "server-rejected-jwt");
...
async fn enrich_auth_inline_populates_zdr_flags() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let mgr = AuthManager::new(dir.path(), WeepcodeComConfig::default()).with_proxy_base_url(&base);

    let mut auth = WeepcodeAuth {
        key: "tok".into(),
        ..WeepcodeAuth::test_default()
    };
...
fn oidc_session_for_team(principal_id: &str) -> WeepcodeAuth {
    WeepcodeAuth {
        key: team_jwt(principal_id),
        auth_mode: AuthMode::Oidc,
        refresh_token: Some("rt".into()),
        expires_at: Some(Utc::now() + Duration::hours(1)),
        oidc_issuer: Some(crate::auth::config::WEEPCODE_OAUTH2_ISSUER.to_string()),
        oidc_client_id: Some("client".into()),
        ..WeepcodeAuth::test_default()
    }
}
...
async fn auth_rejects_token_refreshed_into_wrong_team() {
    ...
    impl TokenRefresher for WrongTeamRefresher {
        async fn refresh(&self, _reason: RefreshReason) -> crate::auth::refresh::RefreshOutcome {
            crate::auth::refresh::RefreshOutcome::Success(Box::new(WeepcodeAuth {
                key: self.jwt.clone(),
                auth_mode: AuthMode::Oidc,
                refresh_token: Some("rt-new".into()),
                expires_at: Some(Utc::now() + Duration::hours(1)),
...
async fn cached_api_key_session_rejected_when_api_key_auth_disabled() {
    let api_key_session = || WeepcodeAuth {
        key: "weepcode-cached-key".into(),
        auth_mode: AuthMode::ApiKey,
        expires_at: Some(Utc::now() + Duration::hours(1)),
        ..WeepcodeAuth::test_default()
    };
    ...
    let mgr2 = Arc::new(AuthManager::new(dir2.path(), WeepcodeComConfig::default()));
    mgr2.hot_swap(api_key_session());
    assert_eq!(
        mgr2.current().map(|a| a.key),
        Some("weepcode-cached-key".to_string()),
...
/// No OAuth session → env or auth.json `weepcode::api_key` for voice/tools.
#[tokio::test]
#[serial_test::serial]
async fn shared_api_key_provider_static_fallthrough() {
    ...
    {
        let _legacy = EnvGuard::unset("WEEPCODE_CODE_WEEPCODE_API_KEY");
        let _key = EnvGuard::set("WEEPCODE_API_KEY", "env-only-key");
        assert_eq!(
    ...
    {
        let _key = EnvGuard::set("WEEPCODE_API_KEY", "env-should-lose");
        mgr.hot_swap(WeepcodeAuth {
...
async fn shared_api_key_provider_kill_switch_blocks_static() {
    ...
    let _key = EnvGuard::set("WEEPCODE_API_KEY", "blocked");
    let dir = tempfile::tempdir().unwrap();
...
async fn shared_api_key_provider_oidc_preferred_blocks_static() {
    ...
    let _key = EnvGuard::set("WEEPCODE_API_KEY", "should-not-use");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(
        dir.path(),
        WeepcodeComConfig {
...
async fn shared_api_key_provider_api_key_preferred_skips_session() {
    ...
    let _legacy = EnvGuard::unset("WEEPCODE_CODE_WEEPCODE_API_KEY");
    let _key = EnvGuard::set("WEEPCODE_API_KEY", "static-preferred");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(
        dir.path(),
        WeepcodeComConfig {
...
async fn shared_api_key_provider_sync_falls_through_when_session_expired() {
    ...
    let _legacy = EnvGuard::unset("WEEPCODE_CODE_WEEPCODE_API_KEY");
    let _key = EnvGuard::set("WEEPCODE_API_KEY", "static-after-expiry");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), WeepcodeComConfig::default()));
    mgr.hot_swap(WeepcodeAuth {
        key: "expired-oidc".into(),
...
async fn shared_api_key_provider_sync_buffered_session_beats_static() {
    ...
    let _legacy = EnvGuard::unset("WEEPCODE_CODE_WEEPCODE_API_KEY");
    let _key = EnvGuard::set("WEEPCODE_API_KEY", "leftover-static");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), WeepcodeComConfig::default()));
...
async fn process_key_precedence() {
    ...
    {
        let _key = EnvGuard::set("WEEPCODE_API_KEY", "env");
        assert_eq!(
            provider.current_api_key_async().await.as_deref(),
            Some("env")
        );
...
fn manual_auth_reason_maps_terminal_and_skips_non_forcing() {
    ...
    use weepcode_telemetry::events::ManualAuthReason as R;
...
async fn manual_auth_capture_attributes_and_recorder_debounces() {
    ...
    use weepcode_telemetry::events::{AuthTokenKind, ManualAuthSurface};
...
async fn manual_auth_emits_only_for_user_facing_source() {
    ...
    fn mgr_with(dir: &std::path::Path, key: &str, mode: AuthMode) -> Arc<AuthManager> {
        let mgr = Arc::new(AuthManager::new(dir, WeepcodeComConfig::default()));
