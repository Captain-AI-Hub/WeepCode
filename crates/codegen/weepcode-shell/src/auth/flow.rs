...
use crate::auth::{AuthManager, WeepcodeAuth, WeepcodeComConfig, parse_output};
use crate::util::weepcode_home;
...
fn is_cached_credential_compatible(
    auth: &WeepcodeAuth,
    weepcode_com_config: &WeepcodeComConfig,
) -> bool {
    let expected_issuer = weepcode_com_config
        .oidc
        .as_ref()
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(weepcode_com_config) {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("WEEPCODE_LOGIN_DEVICE_FLOW")
...
/// OAuth2 provider config supports it. Enterprise OIDC (`oidc=Some`) always uses
/// the loopback flow, mirroring `run_auth_flow_inner`'s precedence.
async fn cli_should_use_device(
    config: &WeepcodeComConfig,
    login_override: LoginTransportOverride,
) -> bool {
    !crate::auth::oidc::is_configured(config) && should_use_device_flow(login_override).await
}

/// Whether interactive WeepCode OAuth2 login uses the RFC 8628 device flow (vs loopback).
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `WEEPCODE_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `weepcode_build_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("WEEPCODE_LOGIN_DEVICE_FLOW");
...
async fn run_external_auth_provider(
...
) -> anyhow::Result<(WeepcodeAuth, bool)> {
    ...
    if is_refresh {
        cmd.env("WEEPCODE_AUTH_EXPIRED", "1");
    }

    weepcode_tools::util::detach_command(&mut cmd);
    cmd.envs(weepcode_tools::util::pager_env());
    ...
    let principal_policy =
        crate::auth::oidc::login_principal_policy(auth_manager.weepcode_com_config());
...
async fn run_auth_flow_inner(
    ...
    tracing::info!(
        has_oidc = weepcode_com_config.oidc.is_some(),
        has_oauth2 = weepcode_com_config.oauth2.is_some(),
        has_external_auth = weepcode_com_config.auth_provider_command.is_some(),
        reauth,
    ...
    if !force_interactive && let Some(auth) = auth_manager.current() {
        if is_cached_credential_compatible(&auth, weepcode_com_config) {
    ...
    if !force_interactive && !reauth && auth_manager.is_expired() {
        ...
        if disk_auth.as_ref().is_some_and(|d| {
            !crate::auth::is_expired(d) && is_cached_credential_compatible(d, weepcode_com_config)
        }) {
            weepcode_telemetry::unified_log::info(
    ...
    if let Some(ref cmd) = weepcode_com_config.auth_provider_command {
    ...
    // WeepCode OAuth2 also defaults to loopback; the device flow (robust on
    // remote/SSH where the loopback redirect can't reach the CLI) is opt-in via
    // --device-auth / WEEPCODE_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
    if crate::auth::oidc::is_configured(weepcode_com_config) {
        return crate::auth::oidc::run_login_flow(weepcode_com_config, auth_manager, channels)
            .await;
    }

    if let Some(ref oauth2_cfg) = weepcode_com_config.oauth2 {
    ...
    tracing::error!(
        "auth: no OAuth2 configuration available (neither enterprise OIDC nor WeepCode OAuth2 configured)"
    );
    anyhow::bail!(
        "No OAuth2 configuration available. Run `weepcode login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
pub async fn try_ensure_fresh_auth(
    ...
    let weepcode_home = weepcode_home::weepcode_home();
    let auth_manager = std::sync::Arc::new(AuthManager::new(
        &weepcode_home,
        weepcode_com_config.clone(),
    ));

    // auth() handles cached-valid (fast path), OIDC refresh, external
    // binary -- all through refresh_chain (single mutation point).
    auth_manager.configure_refresher(weepcode_com_config.auth_provider_command.clone(), None);
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
/// Like `try_ensure_fresh_auth` but also mints on cold start (external
/// provider, never a browser; may take up to ~300s). For detached modes only.
pub(crate) async fn try_ensure_session_noninteractive(
    weepcode_com_config: &WeepcodeComConfig,
) -> Option<WeepcodeAuth> {
    if let Some(auth) = try_ensure_fresh_auth(weepcode_com_config).await {
        return Some(auth);
    }
    let weepcode_home = weepcode_home::weepcode_home();
    ...
    mint_session_noninteractive(&auth_manager, weepcode_com_config).await
}
...
fn expired_refreshable_session(auth_manager: &AuthManager) -> Option<WeepcodeAuth> {
    auth_manager
        .current_or_expired()
        .filter(|a| a.is_weepcode_auth() && a.refresh_token.is_some())
}

/// Cold-start mint via the external auth provider command; `None` when none
...
async fn mint_session_noninteractive(
    ...
    if weepcode_com_config.blocks_automatic_oidc() {
        tracing::debug!(
    ...
    if let Some(cmd) = weepcode_com_config.auth_provider_command.as_deref() {
...
fn report_signed_in(auth: &WeepcodeAuth) {
    eprint!("\r\x1b[K");
    match auth.email {
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    message_prefix: Option<&str>,
) -> anyhow::Result<Option<WeepcodeAuth>> {
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(weepcode_com_config).await)
    } else {
        ensure_authenticated(weepcode_com_config, false, message_prefix)
            .await
            .map(Some)
    }
}

/// Unified `weepcode login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `WEEPCODE_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
/// loopback default. Both transports run through `run_auth_flow_inner` so the
/// external auth provider is tried first.
pub async fn run_cli_login(
    ...
    // oauth2=None) always uses the loopback flow; only an OAuth2 provider config
    // supports the device flow. Without this guard, `weepcode login` on an
    // enterprise-OIDC deployment would wrongly enter the device branch (which
    // requires `oauth2`) and error.
    let authenticated = if cli_should_use_device(&config.weepcode_com_config, login_override).await
    {
        if config.weepcode_com_config.oauth2.is_none() {
            // No OIDC and no oauth2 here, so `--oauth` can't help.
            anyhow::bail!(
                "Sign-in is not available for this deployment. Set WEEPCODE_API_KEY instead."
            );
        }
        let weepcode_home = weepcode_home::weepcode_home();
        let auth_manager = Arc::new(AuthManager::new(
            &weepcode_home,
            config.weepcode_com_config.clone(),
        ));
        // Route through the shared inner flow (not `run_device_code_login`
        // directly) so the external auth provider runs before the interactive
        // device login. `force_interactive` skips the up-front clear, so
        // abandoning the device prompt doesn't log the user out; on
        // `NotEnabled` it falls back to loopback.
        ...
        if device_auth && crate::auth::oidc::is_configured(&config.weepcode_com_config) {
            eprintln!(
    ...
    // config was actually applied; `weepcode setup` reports the no-config case.
...
pub struct LogoutResult {
    ...
    /// `true` if `WEEPCODE_API_KEY` / `WEEPCODE_CODE_WEEPCODE_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        weepcode_telemetry::external::set_identity(
            weepcode_telemetry::external::IdentityAttrs::default(),
        );
        weepcode_telemetry::external::flush();
    ...
    Ok(LogoutResult {
        was_logged_in,
        email,
        api_key_still_set: crate::agent::auth_method::has_weepcode_api_key_env(),
    })
}

/// `weepcode logout` CLI handler. Calls [`perform_logout`] and formats
/// the result to stderr.
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let weepcode_home = weepcode_home::weepcode_home();
    let auth_manager = AuthManager::new(&weepcode_home, config.weepcode_com_config.clone());
    ...
    if !result.was_logged_in {
        ...
        if result.api_key_still_set {
            eprintln!("You are authenticated via WEEPCODE_API_KEY (environment variable).");
        }
        return Ok(());
    }
    ...
    if result.api_key_still_set {
        eprintln!("WEEPCODE_API_KEY is still set and will be used for authentication.");
    }
    Ok(())
}
...
mod tests {
    ...
    use crate::auth::config::WEEPCODE_OAUTH2_ISSUER;
    use crate::env::EnvVarGuard;
    use chrono::Utc;

    /// Run `f` with `WEEPCODE_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("WEEPCODE_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("WEEPCODE_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("WEEPCODE_LOGIN_DEVICE_FLOW"),
        };
        f()
    }

    // A grok.com first-party (x.ai-issuer) OIDC session — `is_weepcode_auth()` true.
    fn oidc_session(key: &str, refresh: Option<&str>) -> WeepcodeAuth {
        WeepcodeAuth {
            key: key.into(),
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(WEEPCODE_OAUTH2_ISSUER.to_string()),
    ...
    fn expired_refreshable_session_gate() {
        ...
        let mgr = AuthManager::new(dir.path(), WeepcodeComConfig::default());
    ...
    async fn mint_session_noninteractive_uses_external_provider() {
        let dir = tempfile::tempdir().unwrap();
        let cfg = WeepcodeComConfig {
            auth_provider_command: Some("printf '%s' weepcode-ext-token".to_string()),
        ...
        assert_eq!(auth.map(|a| a.key), Some("weepcode-ext-token".to_string()));
    }
    ...
    async fn external_reauth_without_prev_auth_enriches_inline() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), WeepcodeComConfig::default())
    ...
    async fn device_flow_still_runs_external_provider() {
        ...
        let cfg = WeepcodeComConfig {
            ...
            oauth2: Some(test_oauth2_provider("http://127.0.0.1:1")),
            ..WeepcodeComConfig::default()
        };
        assert!(
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("WEEPCODE_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("WEEPCODE_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `weepcode login` must use loopback, not device —
        ...
        // An OAuth2 provider config (oidc=None, oauth2=Some) does use device.
        let oauth2_cfg = WeepcodeComConfig {
            oauth2: Some(test_oauth2_provider("https://oauth2.example")),
            ..WeepcodeComConfig::default()
        };
        assert!(oauth2_cfg.oauth2.is_some() && oauth2_cfg.oidc.is_none());
        assert!(cli_should_use_device(&oauth2_cfg, LoginTransportOverride::ForceDevice).await);
    }
    ...
    fn oidc_auth(issuer: &str) -> WeepcodeAuth {
        WeepcodeAuth {
    ...
    /// An `OAuth2ProviderConfig` for tests that need an explicit OAuth2 provider
    /// (the default config no longer fabricates one).
    fn test_oauth2_provider(issuer: &str) -> crate::auth::OAuth2ProviderConfig {
    ...
    /// Config whose OAuth2 provider issues from `WEEPCODE_OAUTH2_ISSUER`, so
    /// cached-credential issuer matching has an expected issuer to compare against.
    fn cfg_with_first_party_oauth2() -> WeepcodeComConfig {
        WeepcodeComConfig {
            oauth2: Some(test_oauth2_provider(WEEPCODE_OAUTH2_ISSUER)),
            ..WeepcodeComConfig::default()
        }
    }
    ...
    fn weblogin_cred_is_never_compatible() {
        let cfg = cfg_with_first_party_oauth2();
    ...
    fn oidc_cred_with_matching_issuer_is_compatible() {
        ...
        assert!(is_cached_credential_compatible(
            &oidc_auth(WEEPCODE_OAUTH2_ISSUER),
            &cfg,
        ));
    }
    ...
    fn pinned_cfg(team: &str) -> WeepcodeComConfig {
        WeepcodeComConfig {
    ...
    async fn run_auth_flow_uses_valid_disk_token_when_expired() {
        ...
        let valid_disk = WeepcodeAuth {
            key: "fresh-token-from-disk".into(),
            auth_mode: AuthMode::Oidc,
            expires_at: Some(Utc::now() + chrono::Duration::hours(1)),
            refresh_token: Some("new-rt".into()),
            oidc_issuer: Some(WEEPCODE_OAUTH2_ISSUER.into()),
    ...
    async fn run_auth_flow_defers_to_consumer_refresh_on_transient_failure() {
        ...
        let expired_with_rt = WeepcodeAuth {
            key: "expired-access-token".into(),
            auth_mode: AuthMode::Oidc,
            expires_at: Some(Utc::now() - chrono::Duration::hours(1)),
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        let expired_no_rt = WeepcodeAuth {
        ...
        // WEEPCODE_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    fn extract_url_from_external_provider_stderr() {
        ...
        assert_eq!(
            extract(
                "Visit the following link to sign into WeepCode: https://auth.example.com/login?code=abc"
            ),
    ...
    /// CLI `weepcode login` passes `on_stderr=None`; stderr must be inherited so
