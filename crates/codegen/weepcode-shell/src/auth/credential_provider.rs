...
use crate::util::weepcode_auth_credentials::WeepcodeAuthCredentials;
...
fn api_key_id_for(auth: Option<&crate::auth::WeepcodeAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| weepcode_telemetry::config::deployment_id_from_key(&a.key))
}
...
pub struct ShellAuthCredentialProvider {
    ...
    static_credentials: WeepcodeAuthCredentials,
}
impl ShellAuthCredentialProvider {
    pub(crate) fn new(
        ...
        let mut static_credentials = WeepcodeAuthCredentials::new(None);
...
/// Resolves the embedding credentials for `embed_base_url`, attaching the WeepCode
/// session credential only to WeepCode-operated endpoints over `https`.
pub(crate) fn embedding_session_credentials(
    embed_base_url: &str,
    auth_manager: Option<&Arc<AuthManager>>,
    api_key_provider: Option<weepcode_tools::types::SharedApiKeyProvider>,
) -> weepcode_memory::EndpointScopedCredentials {
    ...
    weepcode_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
        crate::util::is_weepcode_api_bearer_url,
        auth_credentials,
        api_key_provider,
    )
}
...
///
/// - `"weepcode-shell"`   — classic WeepCode CLI / TUI (weepcode-shell)
/// - `"weepcode-pager"`   — new WeepCode Pager / TUI (weepcode-pager)
/// - `"weepcode-desktop"` — WeepCode Desktop app
/// - `"weepcode-extension"` — VS Code / browser extension
///
/// The factory forwards both:
/// - `x-weepcode-client-version`   (e.g. "0.1.210-alpha.5 (279ffacddb)")
/// - `x-weepcode-client-identifier`
///
/// to the underlying `weepcode-file-utils::StorageClient` via
...
///   - `user_token` (normal OIDC users via com scope) → sends Bearer + `X-WeepCode-Token-Auth`
...
pub fn build_storage_client_for_proxy(
...
) -> weepcode_file_utils::storage_client::StorageClient {
    ...
    if let Some(am) = auth_manager {
        ...
        let bridge: Arc<dyn weepcode_file_utils::storage_client::Auth401AttributionCallback> =
            Arc::new(StorageClientAttributionBridge::new(am, session_id));
        weepcode_file_utils::storage_client::StorageClient::with_provider(
            proxy_base_url,
            http_client,
            provider,
        )
        .with_client_identity(weepcode_version::VERSION, client_identifier)
        .with_client_mode(crate::http::process_client_mode())
        .with_attribution(bridge)
    } else {
        let mut creds = WeepcodeAuthCredentials::new(user_token);
...
/// Bridge that lets `StorageClient` (which lives in weepcode-file-utils)
...
impl weepcode_file_utils::storage_client::Auth401AttributionCallback
...
impl HttpAuth for OtelAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        ...
        let mut creds = WeepcodeAuthCredentials::new(None);
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        weepcode_telemetry::external::set_identity(
            weepcode_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `weepcode-pager` and `weepcode-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> weepcode_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let weepcode_com_config = crate::auth::WeepcodeComConfig::default();
    let exporter = weepcode_telemetry::otel_layer::OtelExporterConfig {
    ...
    let token_header_value = weepcode_com_config.token_header.clone();
    let weepcode_home = crate::util::weepcode_home::weepcode_home();
    let bootstrap = Arc::new(AuthManager::new(&weepcode_home, weepcode_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    weepcode_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    use crate::auth::WeepcodeComConfig;
    use crate::auth::manager::AuthManager;
    use chrono::{Duration as ChronoDuration, Utc};
    use std::sync::Mutex;
    use weepcode_auth::AuthCredentialProvider;
    /// Serializes tests that pin `WEEPCODE_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `WEEPCODE_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("WEEPCODE_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("WEEPCODE_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("WEEPCODE_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("WEEPCODE_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    fn make_auth(key: &str, expires_in: ChronoDuration) -> WeepcodeAuth {
        WeepcodeAuth {
            key: key.to_string(),
    ...
    fn make_manager(dir: &tempfile::TempDir, initial: Option<WeepcodeAuth>) -> Arc<AuthManager> {
    ...
    async fn refresh_after_unauthorized_drives_recovery_state_machine() {
        ...
        let mgr = Arc::new(AuthManager::new(
            dir.path(),
            crate::auth::WeepcodeComConfig::default(),
        ));
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let mgr = make_manager(
            &dir,
            Some(make_auth(
                "weepcode-session-token",
                ChronoDuration::hours(1),
            )),
        );
        let api_key_provider: weepcode_tools::types::SharedApiKeyProvider =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use weepcode_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
        let dep = ShellAuthCredentialProvider::new(
            make_manager(&dir, None),
            Some("weepcode-token-EX".into()),
            None,
        )
        .snapshot();
        assert_eq!(
            dep.deployment_id.as_deref(),
            Some(deployment_id_from_key("weepcode-token-EX").as_str())
        );
        assert!(dep.api_key_id.is_none());
        let api_auth = WeepcodeAuth {
    ...
    async fn otel_live_refresh_after_unauthorized_drives_recovery() {
        ...
        live_mgr.hot_swap(WeepcodeAuth {
            key: "stale".into(),
            auth_mode: crate::auth::AuthMode::Oidc,
            create_time: chrono::Utc::now() - ChronoDuration::hours(2),
            user_id: "u".into(),
    ...
    /// user token, matching the precedence in `WeepcodeAuthCredentials::apply`.
