...
use super::is_weepcode_oauth2_issuer;

pub(crate) const TOKEN_TTL: Duration = Duration::days(30);
const DEFAULT_EARLY_INVALIDATION_SECS: u64 = 300; // 5 minutes

/// Legacy auth.json scope key. Fallback for auth files written by the
/// pre-OIDC relay login.
pub(super) const LEGACY_SCOPE: &str = "https://accounts.x.ai/sign-in";

/// auth.json scope key for plain API key auth (desktop login, `weepcode login --api-key`).
pub const API_KEY_SCOPE: &str = "weepcode::api_key";
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from weepcode-desktop login or `weepcode login --api-key`)
    ApiKey,
}
...
pub struct WeepcodeAuth {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub has_weepcode_code_access: Option<bool>,
    ...
    /// credential first-party (`is_weepcode_auth`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl std::fmt::Debug for WeepcodeAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("WeepcodeAuth")
...
impl WeepcodeAuth {
    ...
    /// `true` when the token comes from a first-party WeepCode account —
    /// either an OIDC login against https://auth.x.ai, or an external auth
    /// provider that declared an WeepCode issuer for its token.
    ///
    ...
    pub fn is_weepcode_auth(&self) -> bool {
        match self.auth_mode {
            AuthMode::Oidc | AuthMode::External => self
                .oidc_issuer
                .as_deref()
                .is_some_and(is_weepcode_oauth2_issuer),
    ...
    pub fn is_managed_mcp_eligible(&self) -> bool {
        self.is_weepcode_auth() || self.auth_mode == AuthMode::WebLogin
    }
    ...
    /// (`is_weepcode_auth`). Plain API keys never do.
    pub fn is_session_auth(&self) -> bool {
        match self.auth_mode {
            AuthMode::WebLogin | AuthMode::Oidc => true,
            AuthMode::External => self.is_weepcode_auth(),
    ...
    /// `true` when the team has Zero Data Retention (ZDR) enabled.
    /// Use this for trace-upload and research-data gates. Product
    /// analytics (`telemetry_enabled`) and user-facing sync features
    /// should use `is_zdr_team()` directly.
    ...
    pub(crate) fn carry_user_profile_from(&mut self, prev: &WeepcodeAuth) {
...
#[cfg(test)]
impl WeepcodeAuth {
    /// Returns a `WeepcodeAuth` with sensible defaults for tests. Override fields
    /// with struct update syntax:
    /// ```ignore
    /// WeepcodeAuth { key: "my-key".into(), ..WeepcodeAuth::test_default() }
    /// ```
...
pub(crate) type AuthStore = BTreeMap<String, WeepcodeAuth>;
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `weepcode login --legacy`
...
pub fn lookup_auth(map: &AuthStore, scope: &str) -> Option<WeepcodeAuth> {
...
/// Early-invalidation buffer. Override with `WEEPCODE_AUTH_EARLY_INVALIDATION_SECS`
/// for testing (e.g. `=5` to shrink the buffer to 5 seconds).
pub(super) fn early_invalidation() -> Duration {
    std::env::var("WEEPCODE_AUTH_EARLY_INVALIDATION_SECS")
        .ok()
...
pub(crate) fn is_expired(auth: &WeepcodeAuth) -> bool {
...
pub(crate) fn is_expired_with_buffer(auth: &WeepcodeAuth, buffer: Duration) -> bool {
...
mod tests {
    ...
    fn make_auth(mode: AuthMode) -> WeepcodeAuth {
        WeepcodeAuth {
            key: "k".into(),
            auth_mode: mode,
            create_time: Utc::now(),
    ...
    #[test]
    fn is_weepcode_auth_matrix() {
        use crate::auth::WEEPCODE_OAUTH2_ISSUER;
        let with_issuer = |mode: AuthMode, issuer: Option<&str>| WeepcodeAuth {
            oidc_issuer: issuer.map(str::to_owned),
            ..make_auth(mode)
        };

        // Only Oidc/External qualify, and only with an x.ai issuer.
        assert!(with_issuer(AuthMode::Oidc, Some(WEEPCODE_OAUTH2_ISSUER)).is_weepcode_auth());
        assert!(with_issuer(AuthMode::External, Some(WEEPCODE_OAUTH2_ISSUER)).is_weepcode_auth());
        assert!(!with_issuer(AuthMode::Oidc, None).is_weepcode_auth());
        assert!(!with_issuer(AuthMode::External, None).is_weepcode_auth());
        assert!(!with_issuer(AuthMode::Oidc, Some("https://idp.acme.example")).is_weepcode_auth());
        assert!(
            !with_issuer(AuthMode::External, Some("https://idp.acme.example")).is_weepcode_auth()
        );

        // ApiKey / WebLogin stay false even with an x.ai issuer set.
        assert!(!with_issuer(AuthMode::ApiKey, Some(WEEPCODE_OAUTH2_ISSUER)).is_weepcode_auth());
        assert!(!with_issuer(AuthMode::WebLogin, Some(WEEPCODE_OAUTH2_ISSUER)).is_weepcode_auth());
    }
    ...
    fn is_session_auth_requires_first_party_for_external() {
        ...
        assert!(with_issuer(AuthMode::External, Some(WEEPCODE_OAUTH2_ISSUER)).is_session_auth());
        ...
        assert!(!with_issuer(AuthMode::ApiKey, Some(WEEPCODE_OAUTH2_ISSUER)).is_session_auth());
    }
