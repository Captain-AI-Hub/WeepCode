...
use crate::auth::model::WeepcodeAuth;
use crate::auth::token_type::TokenType;
use weepcode_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
impl RejectedAuth {
    pub(crate) fn capture(auth: Option<&WeepcodeAuth>) -> Self {
        Self {
...
impl ManualAuthTracker {
    ...
    pub(crate) fn record(
        ...
        weepcode_telemetry::session_ctx::log_event(event);
    }
...
pub struct UnauthorizedRecovery {
    ...
    step: RecoveryStep,
    /// Error from `RefreshFromAuthority`, propagated as the recovery
    /// failure when the authority cannot recover.
...
impl UnauthorizedRecovery {
    ...
    pub async fn next(&mut self) -> Result<WeepcodeAuth, AuthError> {
    ...
    async fn resolve_next(&mut self) -> Result<WeepcodeAuth, AuthError> {
        // Team-pin gate: 401 recovery must not resurrect a wrong-team session
        // (disk adoption / refresh) for the relay to reconnect
    ...
    async fn next_step_loop(&mut self) -> Result<WeepcodeAuth, AuthError> {
        loop {
            match self.step {
                ...
                RecoveryStep::Done => {
                    // The authority refresh already failed — surface its error
                    // rather than masking it as generic exhaustion.
                    if let Some(e) = self.authority_error.take() {
                        return Err(e);
                    }
    ...
    async fn try_reload_from_disk(&self) -> Option<WeepcodeAuth> {
        let _lock = self
            .auth_manager
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            weepcode_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
    ...
    fn fresh_mint_guard(&self) -> Option<WeepcodeAuth> {
    ...
    async fn try_refresh_from_authority(&self) -> Result<WeepcodeAuth, AuthError> {
    ...
    fn is_different_token(&self, candidate: &WeepcodeAuth) -> bool {
...
mod tests {
    ...
    use crate::auth::model::{AuthMode, WeepcodeAuth};
    ...
    fn rejected_cred() -> Option<WeepcodeAuth> {
        Some(WeepcodeAuth {
    ...
    impl TokenRefresher for OkRefresher {
        async fn refresh(&self, _reason: crate::auth::manager::RefreshReason) -> RefreshOutcome {
            ...
            RefreshOutcome::Success(Box::new(WeepcodeAuth {
    ...
    fn mgr() -> (tempfile::TempDir, Arc<AuthManager>) {
        ...
        let m = Arc::new(AuthManager::new(dir.path(), WeepcodeComConfig::default()));
        (dir, m)
    }
    ...
    async fn recover_with_ok_refresher(
        m: &Arc<AuthManager>,
    ) -> (Result<WeepcodeAuth, AuthError>, u32) {
        let calls = Arc::new(AtomicU32::new(0));
        m.set_refresher(Arc::new(OkRefresher {
            calls: calls.clone(),
        }));
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.weepcode_com_config().auth_scope();
    ...
    async fn reload_from_disk_rejects_expired_different_token() {
        ...
        let scope = m.weepcode_com_config().auth_scope();
        let expired_different = WeepcodeAuth {
