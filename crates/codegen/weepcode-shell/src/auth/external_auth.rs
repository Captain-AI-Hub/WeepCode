...
pub(crate) struct ExternalAuthOutput {
    ...
    /// Token issuer. An WeepCode issuer marks the credential as first-party;
    /// see [`WeepcodeAuth::is_weepcode_auth`].
    #[serde(default)]
    pub issuer: Option<String>,
}

/// Parse process output (stdout) into a `WeepcodeAuth`. Accepts bare token or JSON.
pub(crate) fn parse_output(output: &std::process::Output) -> anyhow::Result<WeepcodeAuth> {
...
pub(crate) fn run_external_auth_sync(command: &str, is_refresh: bool) -> Option<WeepcodeAuth> {
...
pub(crate) fn refresh_with_command(
    command: &str,
    prev_auth: &WeepcodeAuth,
) -> Option<WeepcodeAuth> {
...
mod tests {
    ...
    #[test]
    fn parse_output_issuer_claim_enables_weepcode_auth() {
    ...
    #[test]
    fn sync_sets_weepcode_auth_expired_env_on_refresh() {
        let auth = run_external_auth_sync("echo $WEEPCODE_AUTH_EXPIRED", true).unwrap();
        assert_eq!(auth.key, "1");
    }

    #[test]
    fn refresh_carries_zdr_flags_forward() {
        let prev = WeepcodeAuth {
