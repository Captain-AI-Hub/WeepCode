...
/// Default scopes for the WeepCode OAuth2 provider. Includes `weepcode-cli:access`
...
pub enum PreferredAuthMethod {
    /// `WEEPCODE_API_KEY` / auth.json `weepcode::api_key` / per-model BYOK (`weepcode.api_key`).
    ApiKey,
...
#[serde(default)]
pub struct WeepcodeComConfig {
    pub weepcode_ws_origin: String,
    pub weepcode_ws_url: String,
    ...
    /// Login button label (env: `WEEPCODE_AUTH_PROVIDER_LABEL`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// refresh works. Env: `WEEPCODE_AUTH_TOKEN_TTL`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub auth_token_ttl: Option<u64>,
    /// Admin kill switch: when `Some(true)`, the `weepcode.api_key` auth method is
    /// neither advertised nor accepted, so `WEEPCODE_API_KEY`/per-model credentials
    /// can't bypass the deployment's IdP login. Env: `WEEPCODE_DISABLE_API_KEY_AUTH`.
...
/// Customer OIDC Identity Provider configuration (`[weepcode_com_config.oidc]`).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// OAuth2 provider configuration (`WEEPCODE_OAUTH2_ISSUER` / `WEEPCODE_OAUTH2_CLIENT_ID`).
///
...
const DEFAULT_OAUTH2_REFERRER: &str = "weepcode-build";
/// Historical WeepCode OAuth2 issuer. Retained only as the comparison literal
/// behind [`is_weepcode_oauth2_issuer`] (first-party session classification);
/// no default config points at it anymore.
pub const WEEPCODE_OAUTH2_ISSUER: &str = "https://auth.x.ai";
/// Returns `true` if `issuer` is the historical first-party WeepCode OAuth2
/// issuer. Client-side classification hint only (see
/// [`crate::auth::WeepcodeAuth::is_weepcode_auth`]).
pub fn is_weepcode_oauth2_issuer(issuer: &str) -> bool {
    issuer == WEEPCODE_OAUTH2_ISSUER
}
/// auth.json scope key used by the pre-OIDC `weepcode login --legacy` flow.
...
/// auth.json scope key used when neither enterprise OIDC nor an OAuth2 provider
/// is configured (pure BYOK/api-key deployments). No session-token entry can
/// exist under it; it only gives [`AuthManager`](crate::auth::AuthManager) a
/// stable, collision-free storage key.
const UNCONFIGURED_OAUTH2_AUTH_SCOPE: &str = "weepcode::oauth2-unconfigured";
impl WeepcodeComConfig {
    /// Whether `weepcode.api_key` auth is disabled. Pinning a team
    ...
    /// `WEEPCODE_DISABLE_API_KEY_AUTH` env lockdown is sticky: because the env value
    ...
    /// browser login, external auth provider) must not run — the pin is
    /// fail-closed.
    pub fn blocks_automatic_oidc(&self) -> bool {
        matches!(self.preferred_method, Some(PreferredAuthMethod::ApiKey))
    }
    /// The auth.json scope key for this config. With no OIDC/OAuth2 provider
    /// configured (pure BYOK/api-key deployments), falls back to a stable
    /// placeholder scope under which no session token can exist.
    pub fn auth_scope(&self) -> String {
        if let Some(ref oidc) = self.oidc {
            ...
            UNCONFIGURED_OAUTH2_AUTH_SCOPE.to_owned()
        }
    }
}
impl OAuth2ProviderConfig {
    ...
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("WEEPCODE_OAUTH2_ISSUER").ok()?;
        let client_id = std::env::var("WEEPCODE_OAUTH2_CLIENT_ID").ok()?;
        let principal_type = std::env::var("WEEPCODE_OAUTH2_PRINCIPAL_TYPE").ok();
        let principal_id = std::env::var("WEEPCODE_OAUTH2_PRINCIPAL_ID").ok();
        ...
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("WEEPCODE_OAUTH2_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or(default_scopes),
            principal_type,
            principal_id,
            referrer: Some(
                std::env::var("WEEPCODE_OAUTH2_REFERRER")
...
impl Default for WeepcodeComConfig {
    fn default() -> Self {
        let oidc = OidcAuthConfig::from_env();
        // No built-in OAuth2 provider: only an env-configured
        // (`WEEPCODE_OAUTH2_ISSUER`/`WEEPCODE_OAUTH2_CLIENT_ID`) or TOML-configured
        // provider applies. Enterprise OIDC wins over OAuth2 when both exist.
        ...
        Self {
            weepcode_ws_origin: std::env::var("WEEPCODE_WS_ORIGIN")
                .unwrap_or_else(|_| PROD_WS_ORIGIN.to_owned()),
            weepcode_ws_url: std::env::var("WEEPCODE_WS_URL")
                .unwrap_or_else(|_| PROD_RELAY_WS_URL.to_owned()),
            token_header: "weepcode-cli".to_owned(),
            oidc,
            oauth2,
            auth_provider_command: std::env::var("WEEPCODE_AUTH_PROVIDER_COMMAND").ok(),
            auth_provider_label: std::env::var("WEEPCODE_AUTH_PROVIDER_LABEL").ok(),
            auth_token_ttl: std::env::var("WEEPCODE_AUTH_TOKEN_TTL")
                .ok()
                .and_then(|v| v.parse().ok()),
            disable_api_key_auth: std::env::var("WEEPCODE_DISABLE_API_KEY_AUTH")
                .ok()
...
/// Parse a boolean env-var value for weepcode's on/off flags. A bare presence
/// enables the flag, but the common falsy spellings (`0`, `false`, `off`,
/// `no`, empty) count as disabled — so e.g. `WEEPCODE_DISABLE_API_KEY_AUTH=false`
...
/// True when the admin has set `WEEPCODE_DISABLE_API_KEY_AUTH` to a truthy value in
...
fn env_lockdown_forced() -> bool {
    std::env::var("WEEPCODE_DISABLE_API_KEY_AUTH")
        .ok()
        .is_some_and(|v| env_flag_enabled(&v))
}
impl OidcAuthConfig {
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("WEEPCODE_OIDC_ISSUER").ok()?;
        let client_id = std::env::var("WEEPCODE_OIDC_CLIENT_ID").ok()?;
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("WEEPCODE_OIDC_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or_else(|_| default_oidc_scopes()),
            audience: std::env::var("WEEPCODE_OIDC_AUDIENCE").ok(),
        })
    }
}
#[cfg(test)]
mod tests {
    ...
    fn team_auth_scope_is_base_scope() {
        let cfg = OAuth2ProviderConfig {
            ...
            referrer: Some("weepcode-build".into()),
        };
    ...
    fn preferred_method_deserializes_from_toml() {
        let cfg: WeepcodeComConfig = toml::from_str(
            r#"
        ...
        let cfg: WeepcodeComConfig = toml::from_str("").expect("parse empty");
