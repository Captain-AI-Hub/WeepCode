...
use crate::auth::config::WeepcodeComConfig;
use crate::auth::error::AuthError;
use crate::auth::token_type::TokenType;
use weepcode_telemetry::events::ManualAuthSurface;

#[cfg(test)]
use super::model::UserInfo;
use super::model::{
    AuthMode, WeepcodeAuth, early_invalidation, is_expired, is_expired_with_buffer, lookup_auth,
    token_suffix,
};
...
pub struct AuthManager {
    ...
    inner: Arc<RwLock<Option<WeepcodeAuth>>>,
    path: PathBuf,
    scope: String,
    ...
    power_listener: parking_lot::Mutex<Option<weepcode_system_power::SystemPowerListener>>,
...
enum LockOutcome {
    Held(AuthFileLock),
    Adopted(Box<WeepcodeAuth>),
}

// ── Construction + builders ──────────────────────────────────────────

impl AuthManager {
    pub fn new(weepcode_home: &Path, weepcode_com_config: WeepcodeComConfig) -> Self {
        let scope = weepcode_com_config.auth_scope();
        let proxy_base_url =
        ...
        weepcode_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "weepcode_home": weepcode_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "WEEPCODE_HOME": std::env::var("WEEPCODE_HOME").unwrap_or_else(|_| "(unset)".into()),
                "WEEPCODE_AUTH_PATH": std::env::var("WEEPCODE_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "WEEPCODE_AUTH": std::env::var("WEEPCODE_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // WEEPCODE_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("WEEPCODE_AUTH") {
            if let Ok(auth) = serde_json::from_str::<WeepcodeAuth>(&inline_json) {
                return Self::assemble(
                    Some(auth),
                    weepcode_home.join("auth.json"),
                    scope,
                    weepcode_com_config,
                    proxy_base_url,
                    None,
            ...
            tracing::warn!("WEEPCODE_AUTH set but failed to parse as JSON, falling back to file");
        }

        // WEEPCODE_AUTH_PATH: custom file path (overrides default $WEEPCODE_HOME/auth.json).
        let path = std::env::var("WEEPCODE_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| weepcode_home.join("auth.json"));
    ...
    /// else allowed.
    pub(crate) fn cached_token_policy_error(&self, auth: &WeepcodeAuth) -> Option<AuthError> {
        ...
        let policy = crate::auth::oidc::login_principal_policy(&self.weepcode_com_config)?;
    ...
    fn vet_cached(&self, auth: WeepcodeAuth) -> Option<WeepcodeAuth> {
    ...
    pub(crate) fn current(&self) -> Option<WeepcodeAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    #[inline]
    pub(crate) fn with_inner_write<R>(&self, f: impl FnOnce(&mut Option<WeepcodeAuth>) -> R) -> R {
    ...
    #[inline]
    pub(crate) fn with_inner_read<R>(&self, f: impl FnOnce(Option<&WeepcodeAuth>) -> R) -> R {
    ...
    pub(crate) fn current_or_expired(&self) -> Option<WeepcodeAuth> {
    ...
    pub(crate) fn current_wire_valid(&self) -> Option<WeepcodeAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    pub(crate) fn expired_auth(&self) -> Option<WeepcodeAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    fn is_token_expired(&self, auth: &WeepcodeAuth) -> bool {
    ...
    fn is_token_hard_expired(&self, auth: &WeepcodeAuth) -> bool {
        self.token_expired_with_buffer(auth, Duration::zero())
    }

    fn token_expired_with_buffer(&self, auth: &WeepcodeAuth, buffer: Duration) -> bool {
        if auth.expires_at.is_some() {
            return is_expired_with_buffer(auth, buffer);
        }
        if auth.auth_mode == AuthMode::External
            && let Some(ttl) = self.weepcode_com_config.auth_token_ttl
        {
    ...
    ///
    /// Returns the input `WeepcodeAuth` BEFORE enrichment lands; callers
    /// needing the post-enrichment view re-read `current()`.
    pub(crate) async fn update(
        self: &Arc<Self>,
        auth: WeepcodeAuth,
    ) -> std::io::Result<WeepcodeAuth> {
        ...
        match &write_result {
            Ok(()) => weepcode_telemetry::unified_log::info(
            ...
            Err(e) => weepcode_telemetry::unified_log::error(
    ...
    fn spawn_user_info_enrichment(self: &Arc<Self>, auth: WeepcodeAuth) {
        enrichment::spawn(Arc::clone(self), auth);
    }

    /// Blocking `/user` enrichment for login flows that exit before the background task lands.
    pub(crate) async fn enrich_auth_inline(&self, auth: &mut WeepcodeAuth) {
        enrichment::enrich_inline(self, auth).await;
    }

    pub(crate) fn weepcode_com_config(&self) -> &WeepcodeComConfig {
        &self.weepcode_com_config
    }
    ...
    pub(crate) fn run_external_refresh_command(&self, command: &str) -> Option<WeepcodeAuth> {
    ...
    pub(crate) fn hot_swap(&self, new_auth: WeepcodeAuth) {
    ...
    fn try_adopt_disk_token(&self, reason: RefreshReason, msg: &str) -> Option<WeepcodeAuth> {
    ...
    fn inner_auth_or_external_default(&self) -> WeepcodeAuth {
        self.inner.read().clone().unwrap_or_else(|| WeepcodeAuth {
    ...
    #[cfg(test)]
    fn persist_and_swap(&self, auth: WeepcodeAuth) -> Option<WeepcodeAuth> {
    ...
    pub(crate) fn read_disk_auth(&self) -> Option<WeepcodeAuth> {
    ...
    fn read_disk_auth_silent(&self) -> Option<WeepcodeAuth> {
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<WeepcodeAuth>, DiskAuthState) {
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in WEEPCODE_AUTH mode).
            DiskAuthState::Ok => {
                weepcode_telemetry::unified_log::info(
    ...
    pub async fn auth(self: &Arc<Self>) -> Result<WeepcodeAuth, AuthError> {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<WeepcodeAuth, AuthError> {
        // Snapshot inner ONCE for dispatch atomicity (closes a TOCTOU
        // where a concurrent `clear()` raced `token_type()` + `inner.read()`).
        let snapshot: Option<WeepcodeAuth> = self.with_inner_read(|inner| inner.cloned());
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`weepcode login` from
    ...
    pub(crate) async fn refresh_chain(
        ...
        //    the IdP to obtain one with fresh claims.
    ...
    fn check_refresh_deferral(&self, reason: RefreshReason) -> Result<(), AuthError> {
        ...
        // Dark wake (see `weepcode_system_power::PowerState` for the canonical
    ...
    pub(crate) fn is_different_token(&self, candidate: &WeepcodeAuth) -> bool {
        let current_key = self.inner.read().as_ref().map(|a| a.key.clone());
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<weepcode_telemetry::events::ManualAuth> {
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // `[weepcode_com] auth_token_ttl` to drive a real schedule.
...
impl weepcode_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
fn prefers_static_api_key(am: &AuthManager) -> bool {
    matches!(
        am.weepcode_com_config.preferred_method,
...
fn resolve_static_api_key(am: &AuthManager) -> Option<String> {
    if am.weepcode_com_config.api_key_auth_disabled() {
        return None;
    }
    if matches!(
        am.weepcode_com_config.preferred_method,
    ...
    non_empty_key(crate::agent::auth_method::read_weepcode_api_key_env().ok())
...
impl AuthManager {
    /// `weepcode::api_key` from this manager's auth file, memoized on
...
) -> weepcode_tools::types::SharedApiKeyProvider {
