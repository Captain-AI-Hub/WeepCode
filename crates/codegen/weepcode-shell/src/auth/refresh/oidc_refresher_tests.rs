...
fn write_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &WeepcodeAuth) {
...
async fn oidc_refresher_e2e_full_refresh_cycle() {
    ...
    let mgr = Arc::new(
        AuthManager::new(dir.path(), WeepcodeComConfig::default()).with_proxy_base_url(&base_url),
    );
...
async fn oidc_refresh_picks_up_valid_disk_token() {
    ...
    let fresh_on_disk = WeepcodeAuth {
...
fn expired_oidc_for(base_url: &str) -> WeepcodeAuth {
    WeepcodeAuth {
        key: "old-expired-token".into(),
        create_time: Utc::now() - Duration::hours(2),
        user_id: "user-42".into(),
...
impl AuthSnapshot for EmptySnapshot {
    fn current(&self) -> Option<WeepcodeAuth> {
        None
    }
    fn expired_auth(&self) -> Option<WeepcodeAuth> {
        None
    }
    fn read_disk_auth(&self) -> Option<WeepcodeAuth> {
        None
    }
