...
use super::super::config::{OidcAuthConfig, WeepcodeComConfig};
use super::super::{AuthManager, WeepcodeAuth};
use super::protocol::{
    OidcError, build_authorize_url, build_weepcode_auth, discover, enforce_login_principal,
...
///
/// No CORS layer: the auth code arrives via a top-level browser redirect
/// (or a manual paste), never via a cross-origin `fetch`.
...
fn callback_response(result: &CallbackResult) -> (StatusCode, Html<String>) {
    let (title, message) = match result {
        Ok(_) => (
            "Signed in",
            "You can close this window and return to WeepCode Build.",
        ),
...
pub async fn run_login_flow_with_config(
    ...
    // The OS picks a random available loopback port for the callback server.
    let listener = TcpListener::bind(("127.0.0.1", 0))
        .await
        .map_err(|e| anyhow::Error::new(OidcError::BindLoopback(e.to_string())))?;
    let port = listener.local_addr()?.port();
    let redirect_uri = format!("http://127.0.0.1:{}/callback", port);
    let oauth2 = auth_manager.weepcode_com_config().oauth2.as_ref();
    ...
    if has_client_ui {
        ...
        eprintln!();
        let provider_label = if oidc.issuer == super::super::config::WEEPCODE_OAUTH2_ISSUER {
            "WeepCode".to_owned()
        } else {
            oidc.issuer.clone()
        };
    ...
    let principal_policy = login_principal_policy(auth_manager.weepcode_com_config());
    ...
    let mut auth = build_weepcode_auth(tokens, user_info, &oidc.issuer, &oidc.client_id);
...
mod tests {
    ...
    async fn full_login_flow_via_race() {
        ...
        let auth_manager = Arc::new(
            AuthManager::new(temp_dir.path(), WeepcodeComConfig::default())
        ...
        .await
        .unwrap();
        let auth = build_weepcode_auth(tokens, user_info, &oidc_cfg.issuer, &oidc_cfg.client_id);
