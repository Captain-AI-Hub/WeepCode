...
use super::super::config::{
    ForceLoginTeam, OAuth2ProviderConfig, OidcAuthConfig, WeepcodeComConfig,
};
use super::super::{AuthMode, WeepcodeAuth};
use base64::Engine;
...
pub fn is_configured(config: &WeepcodeComConfig) -> bool {
    config.oidc.is_some()
}
...
pub(crate) fn login_principal_policy(cfg: &WeepcodeComConfig) -> Option<ForceLoginTeam> {
...
pub(super) fn build_weepcode_auth(
...
mod tests {
    ...
    fn authorize_url_includes_required_oidc_params() {
        let config = OidcAuthConfig {
            ...
            audience: Some("api://weepcode".into()),
        };
    ...
    fn authorize_url_includes_team_principal_params() {
        let config = OidcAuthConfig {
            ...
            client_id: TEST_CLIENT_ID.into(),
            scopes: vec!["offline_access".into(), "weepcode-cli:access".into()],
            audience: None,
        };
        ...
        assert!(url.contains("referrer=weepcode-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
            "expected exactly one referrer param, got: {url}"
    ...
    fn authorize_url_uses_oauth2_referrer_override_once() {
        ...
        let oauth2 = OAuth2ProviderConfig {
            ...
            principal_type: None,
            principal_id: None,
            referrer: Some("weepcode-desktop".into()),
        };
        let discovery = Discovery {
            authorization_endpoint: "https://auth.x.ai/authorize".into(),
            token_endpoint: "https://auth.x.ai/token".into(),
        ...
        assert!(url.contains("referrer=weepcode-desktop"));
        assert!(!url.contains("referrer=weepcode-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
    ...
    fn peek_access_token_principal_matrix() {
        ...
        let team_jwt = make_jwt(serde_json::json!(
            ...
            "offline_access weepcode-cli:access api:access", "principal_type" : "Team",
