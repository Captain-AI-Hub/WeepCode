...
use crate::auth::model::{UserInfo, WeepcodeAuth, lookup_auth};
...
pub(super) fn spawn(manager: Arc<AuthManager>, auth: WeepcodeAuth) {
...
async fn fetch_user_info(manager: &AuthManager, key: &str, log_label: &str) -> Option<UserInfo> {
    ...
    let token_header = &manager.weepcode_com_config.token_header;
    ...
    let response = http_client
        ...
        .header("X-WeepCode-Token-Auth", token_header.as_str())
...
pub(super) async fn enrich_inline(manager: &AuthManager, auth: &mut WeepcodeAuth) {
...
async fn run_user_info_enrichment(manager: &AuthManager, auth: WeepcodeAuth) {
    let started = std::time::Instant::now();
...
pub(super) fn apply_user_info_enrichment(disk: &mut WeepcodeAuth, user_info: UserInfo) {
