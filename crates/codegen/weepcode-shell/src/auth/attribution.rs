...
//! `Authorization` header for storage / registry /
//! idle-resume) with the live
//! [`AuthManager::current_api_key`] value. The two sinks are:
//!
//! 1. [`weepcode_telemetry::unified_log::warn`] for the local
//!    `~/.weepcode/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!             | "SessionRegistryClient.<op>" | "IdleResumeModelRefresh",
...
//!
//! [`weepcode_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`weepcode_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`weepcode_sampler::SamplerConfig::attribution_callback`] at every
//! sampler-construction site. Non-sampler sites (storage / registry /
//! idle-resume) call [`record_consumer_401`]
//! directly with their `(consumer_kind, op)` pair.

use std::sync::Arc;

use serde_json::Value as JsonValue;
use weepcode_sampler::{Auth401AttributionCallback, SamplingConsumer};
use weepcode_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`weepcode_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `weepcode_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn weepcode_tools::Auth401AttributionCallback>` for the
...
impl Auth401AttributionCallback for ShellAttribution {
    fn record_401(&self, consumer: SamplingConsumer, sent_bearer_prefix: Option<&str>) {
        ...
        // to `weepcode_sampler::SENT_BEARER_PREFIX_LEN` characters
...
/// in `weepcode-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `weepcode-shell` go through this helper -- the
/// per-client `record_401_attribution` wrappers in
/// `agent/session_registry_client.rs` and `upload/storage_client.rs` each
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.weepcode/logs/unified.jsonl) + scrubbed
...
/// and the mint/expiry fields from the resulting `WeepcodeAuth`.
///
...
fn compute_attribution_payload(
    ...
    // Single read-lock acquisition: pull the live `WeepcodeAuth` (or
    ...
    // `weepcode_com_config.auth_token_ttl` when `expires_at` is `None`
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.weepcode/auth.json` leaks in.
    ...
    fn fresh_auth(key: &str) -> WeepcodeAuth {
        WeepcodeAuth {
            key: key.to_string(),
            create_time: Utc::now(),
    ...
    /// `ShellAttribution` implements `weepcode_tools::Auth401AttributionCallback`
