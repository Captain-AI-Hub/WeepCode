...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.weepcode/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** weepcode process
    /// sharing the same `~/.weepcode` (the writer may also be this process;
...
/// Watches `~/.weepcode/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.weepcode/config.toml`, `.mcp.json`,
...
/// processes (e.g. `weepcode login` in another terminal).
///
/// Adds two **non-recursive** watches per `cwd` argument:
/// `<cwd>/` (catches `.mcp.json` and `.claude.json` at the project root) and
/// `<cwd>/.weepcode/` (catches `<cwd>/.weepcode/config.toml`). Recursing on `<cwd>`
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.weepcode/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let weepcode_home_buf = weepcode_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(weepcode_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(weepcode_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(weepcode_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(weepcode_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %weepcode_home.display(),
                    error = %e,
                    "failed to watch weepcode home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.weepcode/config.toml` so the loop above
        // watches `<cwd>/.weepcode/`), the call below installs a
        ...
        tracing::info!(
            weepcode_home = %weepcode_home.display(),
    ...
    /// Register `<cwd>/` and `<cwd>/.weepcode/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.weepcode/config.toml` trigger a
    ...
    /// `<cwd>/.weepcode/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.weepcode/` does not yet
/// exist at session-open time, the `.weepcode/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.weepcode/` followed by a
/// write to `<cwd>/.weepcode/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let weepcode_dir = cwd.join(".weepcode");
    if let Err(e) = debouncer
        .watcher()
        .watch(&weepcode_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .weepcode directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let weepcode_dir = cwd.join(".weepcode");
    if let Err(e) = debouncer.watcher().unwatch(&weepcode_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .weepcode directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.weepcode/` hasn't
...
/// Watches skill directories (`~/.weepcode/skills/`, `<repo>/.weepcode/skills/`, etc.)
...
/// recursively: `weepcode_home` (`~/.weepcode`, or `$WEEPCODE_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.weepcode` alone has `worktrees/`,
...
fn is_global_config_dir(dir: &Path, weepcode_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, weepcode_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".weepcode", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, weepcode_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(weepcode_home) {
        return true;
    }
...
/// `~/.weepcode` these exist at startup; later creation is caught on restart).
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](weepcode_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let weepcode_home = weepcode_tools::util::weepcode_home::weepcode_home();
        ...
        let dirs_to_watch = weepcode_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &weepcode_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &weepcode_home) {
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only weepcode_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_weepcode_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let weepcode_home = home.join(".weepcode");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &weepcode_home, Some(home));

        // weepcode_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&weepcode_home));
        ...
        assert!(!g(&home.join("repo").join(".weepcode")));
    }

    /// Regression for the ~/.weepcode inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.weepcode/worktrees/`) must not
    ...
    /// A write to `<weepcode_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another weepcode process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.weepcode/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let weepcode_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_weepcode = cwd.path().join(".weepcode");
        fs::create_dir_all(&project_weepcode).unwrap();
        // Seed the file before the watcher starts so we observe the
        // modification rather than the creation event.
        fs::write(project_weepcode.join("config.toml"), "").unwrap();
        ...
        fs::write(
            project_weepcode.join("config.toml"),
            "[mcp_servers.x]\ncommand = \"/bin/true\"",
        )
        .unwrap();
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.weepcode/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.weepcode/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_weepcode = new_cwd.path().join(".weepcode");
        fs::create_dir_all(&project_weepcode).unwrap();
        fs::write(project_weepcode.join("config.toml"), "").unwrap();
