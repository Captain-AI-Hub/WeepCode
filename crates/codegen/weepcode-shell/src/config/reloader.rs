...
use crate::auth::{WeepcodeAuth, read_auth_json};
...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.weepcode/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.weepcode/config.toml`,
    ...
    /// (`<cwd>/.weepcode/config.toml`, `<cwd>/.mcp.json`, or
    ...
    /// weepcode process sharing the home dir). The agent should consult the cache
    /// thrash unrelated sessions.
    ProjectMcpServersChanged {
        /// The project root whose `.weepcode/`, `.mcp.json`, or
    ...
    ModelsChanged,
    /// `~/.weepcode/models_cache.json` was rewritten on disk (possibly by another
...
impl ConfigReloader {
    ...
    pub async fn run(
        ...
        loop {
            ...
            if has_auth {
                ...
                match result {
                    Ok(Err(e)) => {
                        ...
                        let path = self.weepcode_home.join("auth.json");
            ...
            // same tick (e.g. `~/.weepcode/config.toml` AND
    ...
    fn reload_auth(&mut self) -> anyhow::Result<()> {
        let auth_path = self.weepcode_home.join("auth.json");
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.weepcode/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.weepcode/config.toml`,
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.weepcode/config.toml` | `<cwd>`           |
...
/// for `.weepcode/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
/// (`~/.weepcode/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
/// `ConfigUpdate::Skills`) and `weepcode inspect` (via the `crate::config`
...
pub(crate) fn parse_skills_config(
    config: &toml::Value,
) -> weepcode_agent::prompt::skills::SkillsConfig {
    config
        .get("skills")
...
fn parse_compat_config(config: &toml::Value) -> weepcode_tools::types::compat::CompatConfigToml {
    config
        .get("compat")
        .and_then(|v| v.clone().try_into().ok())
        .unwrap_or_default()
...
mod tests {
    ...
    fn make_auth(key: &str) -> WeepcodeAuth {
        WeepcodeAuth {
            key: key.to_string(),
    ...
    async fn reloader_detects_new_auth_key() {
        ...
        assert!(
            matches!(update, ConfigUpdate::Auth(a) if a.key == "new-key"), // a is Box<WeepcodeAuth>, Deref coercion
    ...
    /// The hash must reflect ancestor `.weepcode/config.toml` and `.mcp.json`
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".weepcode").join("config.toml"), "x = 1").unwrap();
        let h3 = hash_project_mcp_config(&child).expect("readable");
        assert_ne!(
            h2, h3,
            "ancestor .weepcode/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.weepcode/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            skills.paths,
            vec!["/home/user/.weepcode/skills".to_string()]
        );
...
theme = "dark"
yolo = true
fork_secondary_model = "weepcode-4.5"
"#,
        )
        .unwrap();
        let (theme, yolo, fork) = extract_ui_fields(&config);
        ...
        assert!(yolo);
        assert_eq!(fork.as_deref(), Some("weepcode-4.5"));
    }
...
"#,
    ...
    fn models_changed_detects_default_change() {
        let a: toml::Value =
            toml::from_str("[models]\ndefault = \"weepcode-code-fast-1\"").unwrap();
        let b: toml::Value =
            toml::from_str("[models]\ndefault = \"weepcode-code-slow-1\"").unwrap();
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.weepcode/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.weepcode/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.weepcode/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.weepcode/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(
