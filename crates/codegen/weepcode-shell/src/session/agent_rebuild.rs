...
//! [`weepcode_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`weepcode_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`weepcode_agent::Agent`] owns an [`weepcode_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`weepcode_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use weepcode_agent::config::AgentDefinition;
use weepcode_agent::error::AgentBuildError;
use weepcode_agent::prompt::context::PromptAudience;
use weepcode_agent::prompt::skills::SkillsConfig;
use weepcode_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
use weepcode_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use weepcode_tools::implementations::lsp::LspBackend;
use weepcode_tools::implementations::web_search::WebSearchConfig;
use weepcode_tools::implementations::weepcode_build::ask_user_question::types::UserQuestionRequest;
...
use weepcode_tools::types::SharedApiKeyProvider;
use weepcode_tools::types::compat::CompatConfig;
use weepcode_tools::types::memory_backend::MemoryBackend;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub attribution_callback: Option<weepcode_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<weepcode_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        if let Some(event_tx) = subagent_event_tx.clone() {
            use weepcode_tools::implementations::weepcode_build::task::backend::{
        ...
        agent
            .tool_bridge()
            .update_resource(weepcode_tools::types::resources::RespectGitignore(
                *respect_gitignore,
            ))
            .await;
        agent
            .tool_bridge()
            .update_resource(weepcode_tools::types::resources::PathNotFoundHints(
                *path_not_found_hints,
            ))
            .await;
        ...
        {
            use weepcode_tools::implementations::weepcode_build::ask_user_question::UserQuestionSender;
            agent
                .tool_bridge()
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            weepcode_tools::computer::local::LocalTerminalBackend::new_local(
                weepcode_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        system_prompt_label: weepcode_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(weepcode_tools::types::tool::ToolKind::Task)
            .expect("WeepcodeBuild Task tool should be present");
        toolset
            .tool_definitions()
            .into_iter()
            .find(|definition| definition.function.name == task_name)
            .and_then(|definition| definition.function.description)
            .expect("WeepcodeBuild Task description should be present")
    }
    ...
    async fn rebuild_projects_fresh_public_model_keys_into_task_description() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let first = spec
                    .build_agent(AgentDefinition::default_weepcode_build())
                    .await
