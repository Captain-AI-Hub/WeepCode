...
/// Method name for WeepCode extension session/update notifications.
pub(crate) const WEEPCODE_SESSION_UPDATE_METHOD: &str = "_x.ai/session/update";

/// A unified session update that can be either an ACP notification or an WeepCode extension notification.
...
pub enum SessionUpdate {
    ...
    /// WeepCode extension session notification (e.g., diff_review)
        ...
        S: serde::Serializer,
    {
        ...
        match self {
            ...
            SessionUpdate::Xai(notification) => {
                map.serialize_entry("method", WEEPCODE_SESSION_UPDATE_METHOD)?;
                map.serialize_entry("params", notification)?;
            }
        }
        map.end()
...
pub(crate) struct SessionUpdateEnvelope {
    ...
    /// Either "session/update" for ACP or "_x.ai/session/update" for WeepCode extensions.
    pub method: String,
...
impl SessionUpdateEnvelope {
    ...
    pub(crate) fn from_update(update: &SessionUpdate) -> Result<Self, serde_json::Error> {
        ...
        match update {
            ...
            SessionUpdate::Xai(notification) => Ok(Self {
                timestamp,
                method: WEEPCODE_SESSION_UPDATE_METHOD.to_string(),
                params: serde_json::to_value(notification)?,
            }),
        }
    }
    ...
    pub(crate) fn into_update(self) -> Result<SessionUpdate, serde_json::Error> {
        if self.method == WEEPCODE_SESSION_UPDATE_METHOD {
    ...
    pub(crate) fn from_str(line: &str) -> Result<SessionUpdate, serde_json::Error> {
        ...
        if let Ok(envelope) = serde_json::from_str::<BorrowedEnvelope<'_>>(line) {
            ...
            return if envelope.method == Some(WEEPCODE_SESSION_UPDATE_METHOD) {
...
pub trait StorageAdapter: Send + Sync {
    ...
    /// Append a session update (ACP update or WeepCode extension update) and increment counter
...
pub(crate) fn filter_rewind_lines<'a>(lines: Vec<&'a str>) -> Vec<&'a str> {
    ...
    for line in &lines {
        let (raw_params, is_xai) = if let Ok(env) = serde_json::from_str::<RawLinePeek<'_>>(line) {
            ...
            let xai = env.method == Some(WEEPCODE_SESSION_UPDATE_METHOD);
            (raw, xai)
        } else {
            (*line, false)
        };
...
/// Returns only `SessionUpdate::Acp` updates (WeepCode-extension updates like
...
/// Like [`load_updates_for_replay`], but resolves the session under a specific weepcode home.
...
pub enum PromptExtractEvent {
    ...
    /// A `RewindMarker` WeepCode update: truncate accumulated prompts to this index.
    ///
...
/// - WeepCode `"rewind_marker"`      → `update.target_prompt_index`
...
/// Peek inside ACP or WeepCode `params` to read the `update.sessionUpdate` tag and
...
mod tests {
    ...
    /// Wrap a WeepCode notification as the envelope stored in updates.jsonl.
    fn weepcode_envelope(session_update_json: &str) -> String {
        format!(
    ...
    #[test]
    fn weepcode_rewind_marker_yields_rewind_to() {
        let line = weepcode_envelope(
    ...
    #[test]
    fn weepcode_rewind_to_zero_yields_rewind_to_zero() {
        let line = weepcode_envelope(
    ...
    #[test]
    fn weepcode_diff_review_yields_not_user() {
        let line = weepcode_envelope(r#"{"sessionUpdate":"diff_review","content":[]}"#);
        assert_eq!(
            parse_prompt_extract_event(&line),
            PromptExtractEvent::NotUserMessage
        );
    ...
    /// A valid JSON object that has no recognisable ACP/WeepCode shape — NotUserMessage.
    #[test]
    ...
    fn prepare_replay_cursor_refused_when_tail_has_event_id_less_line() {
        ...
        // WeepCode-style line persisted by an older binary: no _meta at all.
    ...
    #[test]
    fn from_str_unknown_weepcode_variant_deserializes_via_envelope() {
        ...
        let line = weepcode_envelope(r#"{"sessionUpdate":"git_branch_update","branch":"main"}"#);
    ...
    #[test]
    fn from_str_known_weepcode_variant_still_works() {
        let line = weepcode_envelope(r#"{"sessionUpdate":"memory_flush_started"}"#);
        let update = SessionUpdateEnvelope::from_str(&line).unwrap();
        match update {
            SessionUpdate::Xai(notif) => {
                assert_eq!(
