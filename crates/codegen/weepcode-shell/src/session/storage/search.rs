...
//! other concurrently running weepcode processes (which may wipe or downgrade
...
use super::{
    ...
    WEEPCODE_SESSION_UPDATE_METHOD, collect_prompts_from_events,
};
...
/// User-configurable overrides via `~/.weepcode/settings.json` are deferred
/// to a follow-up.
...
///
/// TODO: When multiple weepcode processes run concurrently, they each have
...
fn search_db_path(root_dir: &Path) -> PathBuf {
    ...
    weepcode_sqlite_journal::JournalMode::for_db_path(&path).effective_db_path(&path)
}
...
fn collect_all_indexable_content_single_pass(updates_path: &Path) -> io::Result<(String, u64)> {
    ...
    for line_result in reader.lines() {
        ...
        // Control events (rewind markers) come from WeepCode extensions
...
mod tests {
    ...
    fn weepcode_update(session_update_json: &str) -> String {
        format!(
