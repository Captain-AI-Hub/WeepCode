...
#[cfg(test)]
pub(crate) fn noop_observability_bridge() -> weepcode_computer_hub_sdk::ObservabilityBridge {
    weepcode_computer_hub_sdk::ObservabilityBridge::new(
        None,
        weepcode_tool_protocol::SessionId::new("test").expect("valid"),
    )
}
#[cfg(test)]
pub(crate) async fn test_agent_default() -> weepcode_agent::Agent {
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> weepcode_agent::Agent {
    use weepcode_tools::implementations::weepcode_build::update_goal::UpdateGoalTool;
    use weepcode_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
/// WeepCode-build agent with the real `TodoWriteTool` (id `todo_write`, kind
/// `Plan`) registered, so `tool_for_kind(ToolKind::Plan)` resolves through the
/// live toolset instead of the literal fallback.
#[cfg(test)]
pub(crate) async fn test_weepcode_build_agent_with_todo() -> weepcode_agent::Agent {
    use weepcode_tools::implementations::weepcode_build::todo::TodoWriteTool;
    use weepcode_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> weepcode_agent::Agent {
    use weepcode_tools::implementations::weepcode_build::enter_plan_mode::EnterPlanModeTool;
    use weepcode_tools::implementations::weepcode_build::exit_plan_mode::ExitPlanModeTool;
    use weepcode_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<weepcode_tools::registry::types::ToolConfig>,
) -> weepcode_agent::Agent {
    test_agent_from_config(
        weepcode_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        weepcode_agent::AgentDefinition::default_weepcode_build(),
        std::sync::Arc::new(weepcode_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
async fn test_agent_from_config(
    ...
    definition: weepcode_agent::AgentDefinition,
    backend: std::sync::Arc<dyn weepcode_tools::computer::types::TerminalBackend>,
) -> weepcode_agent::Agent {
    use weepcode_tools::computer::local::LocalFs;
    use weepcode_tools::computer::types::AsyncFileSystem;
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("weepcode-test"),
    ...
    weepcode_agent::Agent::new(
        definition,
        weepcode_agent::PromptContext::default(),
        String::new(),
        tool_bridge,
        weepcode_agent::ReminderPolicy::default(),
        weepcode_agent::CompactionPolicy::default(),
        vec![],
        false,
    )
}
...
pub(crate) async fn create_test_actor_ex(
    ...
    gateway_tx: tokio::sync::mpsc::UnboundedSender<weepcode_acp_lib::AcpClientMessage>,
    persistence_tx: tokio::sync::mpsc::UnboundedSender<PersistenceMsg>,
) -> (
    SessionActor,
    tokio::sync::mpsc::UnboundedReceiver<SessionEvent>,
) {
    let cwd = weepcode_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(weepcode_workspace::file_system::MockFs::new(
        cwd.to_path_buf(),
    ));
    ...
    let actor = SessionActor {
        ...
        permissions: weepcode_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
