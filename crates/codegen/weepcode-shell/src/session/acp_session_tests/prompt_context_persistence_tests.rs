...
fn test_json_round_trip() {
    let ctx = weepcode_agent::PromptContext {
        ..Default::default()
    };

    let json = serde_json::to_string_pretty(&ctx).unwrap();
    let loaded: weepcode_agent::PromptContext = serde_json::from_str(&json).unwrap();
...
/// depend on `weepcode_home()` and `SessionInfo` path encoding).
#[test]
fn test_json_round_trip_via_filesystem() {
    ...
    let ctx = weepcode_agent::PromptContext::default();
    ...
    let loaded: weepcode_agent::PromptContext = serde_json::from_str(&read_json).unwrap();
...
fn test_system_prompt_is_plain_text_not_json() {
    let prompt = "You are a WeepCode Build subagent.";
...
fn test_canonical_artifacts_coexist() {
    ...
    let read_ctx: weepcode_agent::PromptContext = serde_json::from_str(
...
fn test_system_prompt_matches_chat_history_system_message() {
    ...
    let system_prompt = "You are a WeepCode Build subagent.\n\n<tool_calling>\n...";
...
fn test_corrupt_json_returns_error() {
    ...
    let result: Result<weepcode_agent::PromptContext, _> = serde_json::from_str(&json);
...
fn fake_prompt_path() -> std::path::PathBuf {
    std::path::PathBuf::from("/tmp/weepcode-test-home/sessions/cwd/sid/prompts/prompt_0.txt")
}
...
fn build_truncated_preserves_small_query_truncates_context() {
    ...
    assert!(
        ...
        "weepcode ordering: query block first"
    );
...
//
// `weepcode_home()` is a process-wide `OnceLock`, so the real async method is
