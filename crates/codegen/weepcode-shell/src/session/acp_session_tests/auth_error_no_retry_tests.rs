...
fn auth_manager_with_refresher(
    ...
    let am = Arc::new(AuthManager::new(dir.path(), WeepcodeComConfig::default()));
    am.hot_swap(WeepcodeAuth {
...
/// attribution.
fn auth_error() -> weepcode_sampler::SamplingErrorInfo {
    weepcode_sampler::SamplingErrorInfo {
        kind: weepcode_sampler::SamplingErrorKind::Auth,
...
async fn make_actor_with_auth_and_credentials(
    auth_manager: Option<Arc<AuthManager>>,
    auth_type: weepcode_chat_state::AuthType,
    api_key: String,
) -> (Arc<SessionActor>, mpsc::UnboundedReceiver<PersistenceMsg>) {
    let method_id = match auth_type {
        weepcode_chat_state::AuthType::SessionToken => "cached_token",
        weepcode_chat_state::AuthType::ApiKey => "weepcode.api_key",
    };
...
/// `ApiKey` (session-token cache miss + `WEEPCODE_API_KEY`).
async fn make_actor_with_method_and_credentials(
    ...
    actor
        .chat_state_handle
        .update_credentials(weepcode_chat_state::Credentials {
...
/// `WEEPCODE_API_KEY`) must NOT attempt an OIDC session-token refresh. The
...
fn model_not_found_error() -> weepcode_sampler::SamplingErrorInfo {
    weepcode_sampler::SamplingErrorInfo {
            kind: weepcode_sampler::SamplingErrorKind::Api,
            message: "API error (status 404 Not Found): The model weepcode-build does not exist or your team does not have access".into(),
...
async fn legacy_auth_hint_on_404_model_not_found() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                msg.contains("weepcode logout"),
                "hint must mention `weepcode logout`, got: {msg}"
            );
            assert!(
                msg.contains("weepcode login"),
                "hint must mention `weepcode login`, got: {msg}"
            );
            assert!(
...
/// runs the full recovery chain, masking the hint.
///
...
fn unauthorized_401_error() -> weepcode_sampler::SamplingErrorInfo {
    weepcode_sampler::SamplingErrorInfo {
            kind: weepcode_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://cli-chat-proxy.grok.com/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_weepcode_token_auth=weepcode-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            model_metadata: None,
...
// transiently collapsed to `ApiKey` (session-token cache miss + `WEEPCODE_API_KEY`)
...
fn session_token_auth_gate_truth_table() {
    ...
    // Session method + Unknown BYOK: refresh only against a first-party WeepCode
...
/// Negative: a genuine `weepcode.api_key` method keeps its configured key on the
...
/// End-to-end for the frozen-gate bug: a session born on `weepcode.api_key` (gate
...
async fn set_session_model_invalidates_byok_memo_for_same_model_id() {
    ...
    local
        .run_until(async {
            ...
            let cfg = weepcode_sampler::SamplerConfig {
