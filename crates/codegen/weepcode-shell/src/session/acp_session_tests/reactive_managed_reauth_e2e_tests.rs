...
//! handshake against rmcp 2.1 in `weepcode-mcp/tests/repro_sse_flood.rs`): a
...
use weepcode_mcp::servers::{ClientStateKind, HttpConfig, McpClient};

const MANAGED: &str = "weepcode_com_testconnector";
...
async fn actor_with_proxy(
    proxy_base: &str,
    gw_tx: tokio::sync::mpsc::UnboundedSender<weepcode_acp_lib::AcpClientMessage>,
...
async fn entry_b_routes_auth_rejection_but_not_policy_denial() {
    use weepcode_mcp::servers::{is_auth_rejection_message, parse_mcp_tool_name};

    // Managed-prefix gate: only `weepcode_com_*` tools enter entry-B.
