...
use weepcode_tools::reminders::task_completion::consumed_completion_ids;
use weepcode_tools::types::output::{BashOutput, TextOutput, ToolOutput};
...
async fn already_reported(actor: &SessionActor, task_id: &str) -> bool {
    use weepcode_tools::reminders::task_completion::ReportedTaskCompletions;
...
async fn between_turn_drain_suppresses_auto_wake_delivered_subagents() {
    ...
    use weepcode_tools::reminders::task_completion::AutoWakeDeliveredIds;
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let (gateway_tx, _) =
...
struct OneTaskTerminal {
    tasks: Vec<weepcode_tools::computer::types::TaskSnapshot>,
}
#[async_trait::async_trait]
impl weepcode_tools::computer::types::TerminalBackend for OneTaskTerminal {
    async fn run(
        &self,
        _: weepcode_tools::computer::types::TerminalRunRequest,
    ) -> Result<
        weepcode_tools::computer::types::TerminalRunResult,
        weepcode_tools::computer::types::ComputerError,
    > {
        unimplemented!()
    }
    ...
    async fn get_task(&self, _: &str) -> Option<weepcode_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn kill_task(&self, _: &str) -> weepcode_tools::computer::types::KillOutcome {
        weepcode_tools::computer::types::KillOutcome::NotFound
    }
    async fn wait_for_completion(
        &self,
        _: &str,
        _: Option<std::time::Duration>,
    ) -> Option<weepcode_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn list_tasks(&self) -> Vec<weepcode_tools::computer::types::TaskSnapshot> {
        self.tasks.clone()
    }
}
fn completed_bash_task(id: &str) -> weepcode_tools::computer::types::TaskSnapshot {
    weepcode_tools::computer::types::TaskSnapshot {
        task_id: id.into(),
