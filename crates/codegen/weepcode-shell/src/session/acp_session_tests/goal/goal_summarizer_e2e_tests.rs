...
//!
//! Tests mutate `WEEPCODE_GOAL_CLASSIFIER` so they carry `serial`.
...
use weepcode_tools::implementations::weepcode_build::update_goal::UpdateGoalInput;
...
async fn answer_summarizer(
    ...
    req: Box<weepcode_tools::implementations::weepcode_build::task::types::SubagentRequest>,
) {
    match behaviour {
...
fn configure_goal_actor(
    ...
    actor.tool_context.cwd =
        weepcode_paths::AbsPathBuf::new(tmp.path().to_path_buf()).expect("abs cwd");
