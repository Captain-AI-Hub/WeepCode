...
async fn persist_ack_waits_for_disk_flush_before_success() {
    ...
    local
        .run_until(async {
            ...
            let sampling_client = crate::sampling::Client::new(weepcode_sampler::SamplerConfig {
...
async fn cancel_running_task_teardown_clears_running_and_pending_work() {
    ...
    local
        .run_until(async {
            ...
            let cwd = AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
            let fs = Arc::new(
                weepcode_workspace::file_system::MockFs::new(cwd.to_path_buf()),
            );
            let terminal = Arc::new(DummyTerminal {});
            let (hunk_tx, _hunk_rx) = tokio::sync::mpsc::unbounded_channel();
            let hunk_tracker_handle = weepcode_hunk_tracker::HunkTrackerActor::spawn(
            ...
            agent
                .tool_bridge()
                .update_resource(
                    weepcode_tools::implementations::weepcode_build::task::types::CurrentPromptIdResource(
                        "running".to_string(),
                    ),
                )
                .await;
            let actor = SessionActor {
                ...
                origin_client: None,
                signals_handle: crate::session::signals::spawn_signals_actor(),
                upload_queue: Arc::new(OnceLock::new()),                agent: std::cell::RefCell::new(agent),
                last_reported_branch: std::sync::Arc::new(parking_lot::Mutex::new(None)),
                git_head_enabled: false,
                models_manager: Default::default(),
                display_cwd: std::sync::OnceLock::new(),
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                ConversationItem::assistant_tool_calls(vec![weepcode_sampling_types::ToolCall {
                    id: "call-1".into(),
    ...
    local
        .run_until(async {
            ...
            let cfg = weepcode_sampler::SamplerConfig {
                ...
                temperature: None,
                top_p: None,
                api_backend: weepcode_sampler::ApiBackend::Responses,
                auth_scheme: Default::default(),
                extra_headers: Default::default(),
                context_window: 100_000,
                client_version: None,
            ...
            let (sampler_event_tx, _sampler_event_rx) = tokio::sync::mpsc::unbounded_channel::<
                weepcode_sampler::SamplingEvent,
            >();
            let sampler_handle = weepcode_sampler::SamplerActor::spawn(
                cfg,
                weepcode_sampler::RetryPolicy::default(),
                sampler_event_tx,
            );
            let (gateway_tx, _gateway_rx) = tokio::sync::mpsc::unbounded_channel::<
                weepcode_acp_lib::AcpClientMessage,
            ...
            let request = ConversationRequest {
                items: vec![
                    ConversationItem::User(weepcode_sampling_types::UserItem { content :
                    vec![weepcode_sampling_types::ContentPart::Text { text : "hi".into(),
...
async fn skill_reminder_deferred_while_turn_running_flushed_when_idle() {
    use weepcode_tools::types::skill_discovery_tracker::{SkillUpdateEffects, SkillUpdateKind};
        ...
        actor
            ...
            .filter(|item| {
                matches!(
                    ...
                    matches!(p, weepcode_sampling_types::ContentPart::Text { text } if
