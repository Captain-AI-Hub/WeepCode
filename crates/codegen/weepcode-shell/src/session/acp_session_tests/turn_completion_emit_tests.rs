...
//! notification the real `send_weepcode_notification` persists — not a
...
fn is_turn_completed(m: &PersistenceMsg) -> bool {
    matches!(
        ...
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Xai(n))
            if matches!(n.update, WeepcodeSessionUpdate::TurnCompleted { .. })
    )
}
            ...
            *actor
            ...
            while let Ok(msg) = gateway_rx.try_recv() {
                if let weepcode_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                    && args.request.method.as_ref() == "weepcode/session_notification"
                    && let Ok(v) =
