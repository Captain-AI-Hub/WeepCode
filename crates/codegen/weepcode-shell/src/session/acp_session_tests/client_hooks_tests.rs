    ...
    local
        .run_until(async {
            ...
            *actor.client_hooks.borrow_mut() = client_hooks;
            ...
            assert_eq!(args.request.method.as_ref(), "weepcode/hooks/event");
            ...
            *actor.client_hooks.borrow_mut() = client_hooks;
            ...
            tokio::task::spawn_local(async move {
                while let Some(msg) = gateway_rx.recv().await {
                    match msg {
                        weepcode_acp_lib::AcpClientMessage::ExtMethod(args) => {
                        ...
                        weepcode_acp_lib::AcpClientMessage::SessionNotification(args) => {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
                weepcode_tools::registry::types::ToolConfig::for_tool::<
                    weepcode_tools::implementations::use_tool::UseTool,
                >(),
            ])
            .await;
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        weepcode_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
                    callback_ids: vec!["cb_0".to_string()],
                    timeout: None,
                }],
...
async fn post_tool_use_and_failure_never_double_fire() {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_weepcode_build_agent_with_todo().await;
            ...
            >| {
                ...
                while let Ok(msg) = rx.try_recv() {
                    if let weepcode_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                        && args.request.method.as_ref() == "weepcode/hooks/event"
                    {
                        let params: serde_json::Value =
                            serde_json::from_str(args.request.params.get()).unwrap();
