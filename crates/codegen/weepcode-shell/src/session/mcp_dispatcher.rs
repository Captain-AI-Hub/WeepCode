...
//!
//! Receives [`weepcode_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`weepcode_mcp::liveness`]),
//! - the [`weepcode_mcp::servers::WeepcodeClientHandler`] (server-pushed
...
pub const SERVER_STATUS_METHOD: &str = "weepcode/mcp/server_status";
...
pub struct McpServerStatusPayload {
    ...
    /// MCP server name (`weepcode_com_linear`, `github`, ...).
    pub name: String,
    /// `managed` (sourced from cli-chat-proxy / `weepcode_com_` prefix)
    /// or `local` (user `.weepcode/config.toml`).
...
pub enum McpServerStatus {
    /// Client is in [`weepcode_mcp::servers::ClientStateKind::Ready`]
...
/// [`weepcode_mcp::servers::McpClientEventKind`]). Used as the
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && weepcode_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
///
/// `gateway` is a [`weepcode_acp_lib::AcpAgentGatewaySender`] (forwarded
...
mod tests {
    ...
    fn managed_handshake_auth_rejection_maps_to_needs_auth() {
        let key = (
            "weepcode_com_notion".to_string(),
            McpClientEventKind::HandshakeFailed,
        );
        let ev = McpClientEvent::HandshakeFailed {
            server: "weepcode_com_notion".to_string(),
    ...
    fn managed_handshake_non_auth_stays_unavailable() {
        for reason in ["403 Forbidden", "cli-chat-proxy returned 502"] {
            ...
            let ev = McpClientEvent::HandshakeFailed {
                server: "weepcode_com_slack".to_string(),
    ...
    /// Contract: managed server names (starting with `weepcode_com_`)
    /// are classified as `Managed`; everything else as `Local`.
    #[test]
    fn classify_source_uses_managed_prefix() {
        assert_eq!(
            classify_source("weepcode_com_linear"),
            McpServerSource::Managed
        );
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        ...
        use weepcode_mcp::servers::{McpClient, McpState};
    ...
    fn recoverable_http_servers_excludes_managed_stdio_and_disabled() {
        let configs = vec![
            ...
            http_cfg("weepcode_com_slack"), // managed
    ...
    fn discard_gateway() -> weepcode_acp_lib::AcpAgentGatewaySender {
        let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
        weepcode_acp_lib::AcpAgentGatewaySender::new(tx)
    }
    ...
    async fn run_dispatcher_stale_transport_closed_is_fully_inert() {
        ...
        let gateway = weepcode_acp_lib::AcpAgentGatewaySender::new(gw_tx);
