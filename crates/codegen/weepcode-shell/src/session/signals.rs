//! Session signals tracking.
//!
//! This module tracks session-level counters (turns, tool calls, compactions,
//! errors) consumed by telemetry summaries, subagent progress queries, and
...
pub use weepcode_telemetry::enums::PrCreationSource;
...
pub struct SessionSignals {
    ...
    // === Bash tool patterns (weepcode_build) ===
...
impl SessionSignalsHandle {
    ...
    pub fn snapshot_gcs_queue(&self, stats: &weepcode_file_utils::queue::UploadQueueStats) {
        ...
        let _ = self.tx.send(SignalEvent::RecordGcsQueueSnapshot {
            ...
            orphans_cleaned: weepcode_file_utils::queue::last_orphans_cleaned(),
        });
    }
...
pub(crate) fn merge_tightest_trigger(current: Option<String>, new: &[String]) -> Option<String> {
    weepcode_sampling_types::doom_loop::DoomLoopSignal::tightest(
        current
            .iter()
            .map(String::as_str)
...
mod tests {
    ...
    async fn test_model_tracking() {
        ...
        // Set primary model
        handle.set_primary_model("weepcode-3");

        let snapshot = handle.snapshot().await.unwrap();
        assert_eq!(snapshot.primary_model_id, Some("weepcode-3".to_string()));
        assert_eq!(snapshot.models_used, vec!["weepcode-3".to_string()]);

        // Record additional model usage
        handle.record_model_usage("weepcode-4");
        handle.record_model_usage("weepcode-3"); // Duplicate

        let snapshot = handle.snapshot().await.unwrap();
        assert_eq!(snapshot.models_used.len(), 2);
        assert!(snapshot.models_used.contains(&"weepcode-3".to_string()));
        assert!(snapshot.models_used.contains(&"weepcode-4".to_string()));
    ...
    async fn test_seed_counts_restores_all_counters() {
        ...
        handle.seed_counts(
            ...
            vec!["weepcode-3".to_string(), "weepcode-4".to_string()],
        );
        ...
        handle.record_model_usage("weepcode-3"); // existing model
        handle.record_model_usage("weepcode-4.5"); // new model
        ...
        assert_eq!(snapshot.models_used.len(), 3); // weepcode-3 not duplicated, weepcode-5 added
        assert!(snapshot.models_used.contains(&"weepcode-4.5".to_string()));
    ...
    async fn test_restore_signals_full_round_trip() {
        ...
        handle1.record_model_usage("weepcode-3");
        ...
        handle1.record_assistant_message();
        handle1.record_model_usage("weepcode-4");
        ...
        assert!(restored.models_used.contains(&"weepcode-3".to_string()));
        assert!(restored.models_used.contains(&"weepcode-4".to_string()));
        ...
        // ITL stats must survive the restore (regression test for weepcode-critique bug)
        assert_eq!(
        ...
        // This is the exact scenario the weepcode-critique bug describes: the
        ...
        handle2.record_model_usage("weepcode-3"); // existing model (should dedup)
        ...
        assert_eq!(after_turn.models_used.len(), 2); // weepcode-3 not duplicated
