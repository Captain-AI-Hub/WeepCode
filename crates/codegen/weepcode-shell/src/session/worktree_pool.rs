...
//!   under `~/.weepcode/worktree_pool/<instance_id>/` with a `.pid` liveness
...
use weepcode_fast_worktree::{WorktreeBuilder, WorktreeSync};

const WORKTREE_POOL_LOG: &str = "weepcode_worktree_pool";
use crate::util::config::PoolConfig;
use crate::util::weepcode_home::weepcode_home;

use weepcode_tty_utils::git_command;
...
/// `~/.weepcode/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.weepcode/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.weepcode/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.weepcode/worktree_pool/`, reads each `.pid` file, and checks
...
/// `CleanAll` mode. Placed under `~/.weepcode/worktree_pool/<instance_id>/<uuid>/`.
async fn create_pooled_worktree(
    ...
    let result = tokio::task::spawn_blocking(move || {
        WorktreeBuilder::new(&source, &dest_clone)
            .creation_mode(weepcode_fast_worktree::CreationMode::GitCheckout)
            .parallelism(parallelism)
            .worktree_kind(weepcode_fast_worktree::WorktreeKind::Pool)
            .create()
    })
    .await?;
...
fn schedule_cleanup(path: PathBuf) {
    ...
    tokio::task::spawn(async move {
        ...
        if path.exists() {
            ...
            let result = tokio::task::spawn_blocking(move || {
                weepcode_fast_worktree::remove_worktree(&path_clone)
            })
            .await;
...
async fn remove_worktree_registration(path: &Path) {
    ...
    let result =
        tokio::task::spawn_blocking(move || weepcode_fast_worktree::remove_worktree(&path_owned))
            .await;
