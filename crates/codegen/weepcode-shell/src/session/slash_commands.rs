...
use weepcode_tools::implementations::skills::skill::format_skill_name;
...
impl CommandAvailability {
    ...
    /// Test helper: every gate satisfied so gated builtins stay covered.
    #[cfg(test)]
...
pub(super) fn available_commands(
    ...
    for workflow in workflows {
        *name_counts.entry(&workflow.name).or_default() += 1;
    }
    ...
    all_commands(skills, availability)
        ...
        .chain(workflows.iter().filter_map(|workflow| {
            if !availability.workflows
                || name_counts
                    .get(workflow.name.as_str())
                    .copied()
                    .unwrap_or(0)
                    > 1
                || builtin_names.contains(workflow.name.as_str())
            {
                return None;
            }
            let meta = serde_json::json!({
...
impl BuiltinAction {
    ...
    pub(crate) fn args_provided(&self) -> bool {
        match self {
            ...
            BuiltinAction::DeepResearch { query } => !query.is_empty(),
            BuiltinAction::WorkflowManage { run_id, op } => !run_id.is_empty() || !op.is_empty(),
...
///   matching today's WeepCode flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use weepcode_tools::implementations::skills::skill::{
...
pub(super) fn resolve(
    ...
    if availability.workflows
        && let Some(workflow) = workflows
            .iter()
            .find(|workflow| workflow.name == command_name)
    {
...
/// `weepcode-tools` so it stays identical to the pager's `LoopCommand` and the
...
mod tests {
    ...
    fn pre_session_builtin_commands_advertises_goal_when_flag_enabled() {
        ...
        for forbidden in ["flush", "dream", "memory", "plugins"] {
            assert!(
                !names.iter().any(|n| n == forbidden),
