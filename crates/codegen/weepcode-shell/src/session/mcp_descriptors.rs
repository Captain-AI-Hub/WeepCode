...
/// sanitizer shared with `weepcode-mcp` so the advertised folder matches disk.
...
fn is_gateway_owned_descriptor(path: &Path) -> bool {
    ...
    value
        .get("x-weepcode-managed-gateway")
...
mod tests {
    ...
    fn sanitize_replaces_unsafe_chars_and_never_empty() {
        assert_eq!(
            sanitize_descriptor_segment("weepcode_com_linear"),
            "weepcode_com_linear"
        );
    ...
    fn server_dir_is_joined_under_root() {
        let root = Path::new("/home/u/.weepcode/projects/enc/mcps");
    ...
    async fn gateway_descriptor_collision_prunes_only_gateway_owned_files() {
        ...
        std::fs::write(
            ...
            serde_json::to_vec_pretty(&serde_json::json!({
                ...
                "inputSchema": {"type": "object"},
                "x-weepcode-managed-gateway": true,
            }))
            .unwrap(),
        )
        .unwrap();
