...
impl SessionActor {
            ...
            Err(e) => {
                ...
                if terminal {
                    ...
                    crate::session::telemetry::emit_mcp_connection_span(
                        ...
                        Some(elapsed_ms),
                        None,
                        Some(weepcode_telemetry::events::McpErrorType::Auth.as_str()),
                    );
    ...
    /// or removed it from `~/.weepcode/config.toml` while we were
    /// sleeping.
    ///
    ...
    /// which parses `~/.weepcode/config.toml` + the project
    /// `.weepcode/config.toml`) on every call. The auto-restart task
    /// calls this at most:
    ...
    /// `WeepcodeClientHandler`, and the liveness watcher is armed —
    ...
    ///
    /// The `WeepcodeClientHandler` constructed inside `try_handshake`
    ...
    pub(crate) async fn respawn_stdio(&self, server: &str) -> Result<(), String> {
        ...
        if let Some(tx) = event_tx {
            ...
            let _ = tx.send(weepcode_mcp::servers::McpClientEvent::ToolsChanged {
                server: server.to_string(),
            });
        }
        let arc_client = std::sync::Arc::new(new_client);
        let _ = arc_client
            .arm_liveness_watcher(weepcode_mcp::liveness::DEFAULT_POLL_INTERVAL)
            .await;
        {
            let mut mcp_state = self.mcp_state.lock().await;
            mcp_state
    ...
    pub(super) async fn ensure_mcp_tools_initialized(&self) {
        ...
        if mcp_server_configs.is_empty() && !has_acp {
            ...
            if mcp_state.generation() == generation {
                ...
                self.events
                    .emit(weepcode_file_utils::events::Event::McpInitCancelled {
        ...
        }
        {
            ...
            let managed_count = mcp_server_configs
                ...
                .count() as u32;
            self.events
                .emit(weepcode_file_utils::events::Event::McpManagedConfigResult {
        ...
        let mcp_clients: Vec<_> = mcp_results
            ...
            .filter_map(|result| match result {
                ...
                Err(e) => {
                    ...
                    self.events
                        .emit(weepcode_file_utils::events::Event::McpServerFailed {
                            server_name: sname,
        ...
        tokio::task::spawn_local(async move {
            ...
            for client in mcp_clients.iter() {
                ...
                futs.push(async move {
                    ...
                    ew.emit(weepcode_file_utils::events::Event::McpServerStarting {
            ...
            > = std::collections::HashMap::new();
            {
                ...
                if mcp_state.generation() != generation {
                    ...
                    event_writer.emit(weepcode_file_utils::events::Event::McpInitCancelled {
                        reason: MCP_INIT_CANCELLED_CONFIG_CHANGED.to_string(),
                    });
                    return;
                }
                    ...
                    match result {
                        Ok((server_name, registrations, elapsed, timeout_sec)) => {
                            ...
                            for reg in registrations {
                                ...
                                if reg.model_visible {
                                        ...
                                        .await
                                    {
                                        ...
                                        event_writer
                                            .emit(weepcode_file_utils::events::Event::McpToolRegistrationFailed {
                                                server_name: server_name.clone(),
                            ...
                            let transport_enum = match server_transport_map
                                ...
                                .unwrap_or("unknown")
                            {
                                "stdio" => weepcode_telemetry::events::McpTransport::Stdio,
                                "sse" => weepcode_telemetry::events::McpTransport::Sse,
                                _ => weepcode_telemetry::events::McpTransport::Http,
                            };
                            weepcode_telemetry::session_ctx::log_event(
                                weepcode_telemetry::events::McpServerConnected {
                                    server_name: server_name.clone(),
                                    tool_count,
                            ...
                            let transport_str = server_transport_map
                                ...
                                .unwrap_or("unknown");
                            event_writer.emit(
                                weepcode_file_utils::events::Event::McpServerConnected {
                                    server_name: server_name.clone(),
                        ...
                        Err((server_name, ref e, needs_auth, elapsed, timeout_sec)) => {
                            ...
                            let error_type_label = match error_cat {
                                weepcode_file_utils::events::McpErrorCategory::AuthRequired => {
                                    weepcode_telemetry::events::McpErrorType::Auth
                                }
                                weepcode_file_utils::events::McpErrorCategory::Timeout => {
                                    weepcode_telemetry::events::McpErrorType::Timeout
                                }
                                _ => weepcode_telemetry::events::McpErrorType::HandshakeFailed,
                            };
                            weepcode_telemetry::session_ctx::log_event(
                            ...
                            event_writer.emit(
                                weepcode_file_utils::events::Event::McpServerFailed {
                                    server_name: server_name.clone(),
                            ...
                            let detail = (!needs_auth).then(|| {
                                weepcode_tools::util::truncate_str_with_marker(&e.to_string(), 200)
                                    .into_owned()
                            });
                ...
                weepcode_telemetry::session_ctx::log_event(
                    weepcode_telemetry::events::McpInitCompleted {
                ...
                event_writer.emit(weepcode_file_utils::events::Event::McpInitCompleted {
    ...
    pub(crate) fn connected_server_summaries(
        &self,
    ) -> Vec<weepcode_tools::types::tool_index::ServerSummary> {
        use weepcode_tools::types::tool_index::ToolSearchIndex;
    ...
    pub(super) async fn mcp_announcement_snapshot(&self) -> Option<McpAnnouncementSnapshot> {
        let server_summaries = self.connected_server_summaries();
        let mut text =
            weepcode_tools::implementations::search_tool::build_server_reminder(&server_summaries)?;
        if let Some(hint) = self.rendered_mcp_hint().await {
            text.push_str(&hint);
        }
