...
impl SessionActor {
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch) = tokio::join!(
            weepcode_workspace::session::git::get_worktree_info(cwd),
            weepcode_workspace::session::git::get_branch(cwd),
        );
        ...
        if let Ok(raw) = serde_json::value::to_raw_value(&params) {
            let notification = acp::ExtNotification::new("weepcode/git_head_changed", raw.into());
            self.notifications
                .gateway
    ...
    /// persists + forwards via `send_weepcode_notification`. Retiring
    ...
    pub(super) async fn emit_turn_completed(
        ...
        self.send_weepcode_notification_with_extra_meta(
    ...
    pub(super) fn classify_install_error(
        err: &weepcode_agent::plugins::install_registry::InstallError,
    ) -> String {
