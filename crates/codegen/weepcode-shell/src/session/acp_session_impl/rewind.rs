...
impl SessionActor {
    ...
    pub(super) async fn get_rewind_points(&self) -> RewindPointsResponse {
        ...
        let file_meta_map: std::collections::HashMap<
            usize,
            &weepcode_workspace::session::file_state::RewindPointMeta,
    ...
    pub(super) async fn handle_rewind(
        ...
        let mut files_to_revert: std::collections::HashMap<
            weepcode_workspace::session::file_state::FlexiblePath,
            Option<String>,
        ...
        if wants_conversation_rewind {
            ...
            self.persist_weepcode_update_only(WeepcodeSessionUpdate::RewindMarker {
                target_prompt_index: target_index,
    ...
    /// session: run `weepcode_chat_state::compaction_utils::repair_history` inside
    ...
    pub(super) async fn handle_repair_history(
        ...
        if turn_flag.load(std::sync::atomic::Ordering::SeqCst) {
            anyhow::bail!(weepcode_chat_state::commands::RepairHistoryBlocked);
        }
