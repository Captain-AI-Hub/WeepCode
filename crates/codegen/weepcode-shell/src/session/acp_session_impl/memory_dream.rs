...
pub(super) struct MemoryFlushSnapshot {
    counts: weepcode_chat_state::ConversationCounts,
...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &weepcode_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                weepcode_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                weepcode_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        weepcode_telemetry::session_ctx::log_event(
            weepcode_telemetry::memory_telemetry::MemorySessionSummary {
    ...
    async fn run_dream_inner(
        ...
        self.send_weepcode_notification(WeepcodeSessionUpdate::MemoryDreamCompleted {
    ...
    async fn run_dream_model_call(&self, user_message: &str) -> Result<String, acp::Error> {
        ...
        let request = ConversationRequest {
            ...
            x_weepcode_req_id: Some(format!("weepcode-dream-{}", uuid::Uuid::new_v4())),
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: weepcode_telemetry::memory_log::TARGET, "MEMORY_FLUSH: starting");
        let flush_start = std::time::Instant::now();

        self.send_weepcode_notification(WeepcodeSessionUpdate::MemoryFlushStarted)
            .await;
        ...
        let result = async {
            ...
            weepcode_telemetry::session_ctx::log_event(
                weepcode_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
            ...
            let request = ConversationRequest {
                ...
                model: Some(model),
                x_weepcode_conv_id: Some(session_id.clone()),
                x_weepcode_req_id: Some(format!("weepcode-flush-{}", uuid::Uuid::new_v4())),
                x_weepcode_session_id: Some(session_id.clone()),
                x_weepcode_agent_id: Some(weepcode_telemetry::id::agent_id()),
                ..Default::default()
            };
        ...
        tracing::info!(target: weepcode_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        weepcode_telemetry::session_ctx::log_event(
            weepcode_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => weepcode_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => weepcode_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => weepcode_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => weepcode_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        weepcode_telemetry::session_ctx::log_event(weepcode_telemetry::events::MemoryFlushed {
        ...
        self.send_weepcode_notification(WeepcodeSessionUpdate::MemoryFlushCompleted {
            result: outcome,
            path: flush_path,
        })
        .await;
    ...
    /// LLM call using the `weepcode-build` model.
    ///
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let request = ConversationRequest {
            items,
            tools: vec![],
            model: Some("weepcode-build".to_owned()),
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    weepcode_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                weepcode_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = weepcode_sampler::stream_responses(
                    raw,
                    meta,
                    request_id,
                    idle_timeout,
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = weepcode_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                weepcode_sampler::collect_response(events).await
            }
        };
