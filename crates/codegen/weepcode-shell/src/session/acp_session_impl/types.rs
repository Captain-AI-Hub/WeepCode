...
pub(crate) enum SamplerFailureRecovery {
    ...
    CompactAndResubmit,
    /// Auth 401 recovery succeeded (OIDC refresh or external provider).
...
pub(crate) enum SamplerTurnOutcome {
    ...
    Response(
        ...
        Box<weepcode_sampler::InferenceLatencyStats>,
    ),
    CompactAndResubmit,
...
pub(crate) enum DrainSource {
    Pending(weepcode_tools::implementations::weepcode_build::update_goal::UpdateGoalInput),
    Channel(weepcode_tools::implementations::weepcode_build::update_goal::UpdateGoalEnvelope),
}
