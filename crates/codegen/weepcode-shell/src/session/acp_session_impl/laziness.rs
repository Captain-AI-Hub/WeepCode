...
impl SessionActor {
    ...
    /// sees "WeepCode is thinking", streaming token chunks, or any other
    ...
    pub(crate) async fn maybe_fire_laziness_check(self: Arc<Self>) {
        ...
        let items = vec![
            ConversationItem::System(weepcode_sampling_types::SystemItem {
                content: std::sync::Arc::<str>::from(LAZINESS_CLASSIFIER_PROMPT),
            }),
            ConversationItem::User(weepcode_sampling_types::UserItem {
                content: vec![weepcode_sampling_types::ContentPart::Text {
        ...
        let request = ConversationRequest {
            ...
            // Don't pass `reasoning_effort` — `weepcode-4.5` (and
            // other tool-flavoured WeepCode variants) reject the field at
            ...
            x_weepcode_conv_id: Some(session_id_str.clone()),
            x_weepcode_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
            x_weepcode_session_id: Some(session_id_str),
