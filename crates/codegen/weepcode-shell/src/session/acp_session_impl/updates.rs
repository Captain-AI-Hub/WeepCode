...
//! its buffered/transient/direct variants, WeepCode-notification handling, and
...
impl SessionActor {
    ...
    pub(super) async fn record_subagent_usage(
        &self,
        by_model: &[(String, weepcode_chat_state::UsageTotals)],
    ...
    /// Producer for the **high-frequency streaming path** with an WeepCode
    /// extension payload. Routes through `event_tx` -> `ReplayBuffer` ->
    /// `emit_buffered` so chunks get merged + debounced + emitted.
    ///
    /// For one-shot WeepCode events (RetryState, ImageCompressed, HookExecution,
    /// AutoCompactCompleted, etc.), use `send_weepcode_notification` instead.
    ///
    /// The frequency-based split (`send_buffered_weepcode_update` vs `send_weepcode_notification`)
    /// mirrors the ACP-side split between `send_update` (high-frequency,
    /// buffered) and `emit_notification_direct` (low-frequency, direct).
    pub(super) async fn send_buffered_weepcode_update(&self, update: WeepcodeSessionUpdate) {
        self.close_rewind_window().await;
    ...
    /// - **WeepCode** (`ToolCallDeltaChunk`) -> inlines a gateway
    ...
    pub(super) async fn emit_buffered(&self, notification: SessionNotification) {
        match notification {
            ...
            SessionNotification::Xai(n) => {
                self.log_outbound_weepcode_buffered(&n);
                if self
                    .notifications
                    .gateway_enabled
    ...
    /// Tracing log for buffered WeepCode notifications emerging from
    /// emit_buffered. Mirrors `log_outbound_notification` for ACP.
    /// Visible with `RUST_LOG=acp_event=info`.
    fn log_outbound_weepcode_buffered(&self, notification: &XaiSessionNotification) {
        if !matches!(
            notification.update,
            WeepcodeSessionUpdate::ToolCallDeltaChunk { .. }
        ) {
            return;
        }
        tracing::info!(
            target : "acp_event", event = "weepcode_buffered_notification_sent", session_id =
            % self.session_info.id, "Sending buffered WeepCode session notification"
        );
    }
    ...
    /// Handle WeepCode session notifications - store them in persistence
    ...
    pub(super) async fn handle_weepcode_session_notification(
        &self,
        ...
        if !matches!(
            ...
            tracing::debug!("storing WeepCode session notification");
        }
        {
        ...
        match &notification.update {
            WeepcodeSessionUpdate::SubagentSpawned {
                ...
                let envelope = self.fire_hook(
                    weepcode_hooks::event::HookEventName::SubagentStart,
                    None,
                    weepcode_hooks::event::HookPayload::SubagentStart {
                ...
                if let Some(registry) = hook_registry_snapshot {
                    ...
                    let _ = weepcode_hooks::dispatcher::dispatch_non_blocking(
                        &registry,
                        weepcode_hooks::event::HookEventName::SubagentStart,
                        &envelope,
                        &ctx,
            ...
            WeepcodeSessionUpdate::SubagentFinished {
                ...
                let envelope = self.fire_hook(
                    weepcode_hooks::event::HookEventName::SubagentEnd,
                    None,
                    weepcode_hooks::event::HookPayload::SubagentStop {
                        subagent_id: subagent_id.clone(),
    ...
    /// Persist an WeepCode extension notification to `updates.jsonl` **without** sending it
    ...
    pub(super) fn persist_weepcode_update_only(&self, update: WeepcodeSessionUpdate) {
        let notification = XaiSessionNotification {
            session_id: self.session_info.id.clone(),
            update,
        ...
        if self
            ...
            .is_err()
        {
            tracing::warn!("Failed to send WeepCode update to persistence channel");
        }
    }
    ...
    pub(super) async fn dispatch_notification_hook(
        ...
        let envelope = self.fire_hook(
            weepcode_hooks::event::HookEventName::Notification,
            None,
            weepcode_hooks::event::HookPayload::Notification {
    ...
    /// Send an WeepCode extension notification to the client
    #[tracing::instrument(skip_all)]
    pub(super) async fn send_weepcode_notification(&self, update: WeepcodeSessionUpdate) {
        self.send_weepcode_notification_with_extra_meta(update, None)
            .await;
    }
    /// [`Self::send_weepcode_notification`] with caller-supplied `_meta` keys merged
    /// into the standard eventId/timestamp meta. Caller keys win on collision.
    #[tracing::instrument(skip_all)]
    pub(super) async fn send_weepcode_notification_with_extra_meta(
        &self,
...
#[cfg(test)]
mod weepcode_event_id_stamping_tests {
    use super::support::create_test_actor;
    use super::*;
    fn persisted_weepcode_event_id(
        prx: &mut tokio::sync::mpsc::UnboundedReceiver<PersistenceMsg>,
    ) -> String {
        loop {
            match prx.try_recv().expect("an WeepCode line must be persisted") {
                PersistenceMsg::Update(crate::session::storage::SessionUpdate::Xai(notif)) => {
                    return notif
                        ...
                        .and_then(|m| m.get("eventId"))
                        .and_then(|v| v.as_str())
                        .expect("persisted WeepCode lines must carry an eventId")
                        .to_string();
                }
                _ => continue,
            }
    ...
    /// `send_weepcode_notification` (own emission) and
    /// `handle_weepcode_session_notification` (inbound/forwarded, meta-less) —
    ...
    #[tokio::test]
    async fn actor_persisted_weepcode_lines_carry_event_id() {
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) =
                    tokio::sync::mpsc::unbounded_channel::<weepcode_acp_lib::AcpClientMessage>();
                let (persistence_tx, mut prx) =
                    tokio::sync::mpsc::unbounded_channel::<PersistenceMsg>();
                let actor = create_test_actor(0, 256_000, 85, gateway_tx, persistence_tx).await;
                actor
                    .send_weepcode_notification(WeepcodeSessionUpdate::HookAnnotation {
                        message: "own emission".into(),
                    })
                    .await;
                let own_id = persisted_weepcode_event_id(&mut prx);
                assert!(own_id.starts_with("test-actor-"));
                actor
                    .handle_weepcode_session_notification(XaiSessionNotification {
                        session_id: acp::SessionId::new("test-actor"),
                        update: WeepcodeSessionUpdate::HookAnnotation {
                            message: "inbound".into(),
                        },
                        meta: None,
                    })
                    .await;
                let inbound_id = persisted_weepcode_event_id(&mut prx);
                assert!(inbound_id.starts_with("test-actor-"));
                assert_ne!(own_id, inbound_id);
                actor.persist_weepcode_update_only(WeepcodeSessionUpdate::HookAnnotation {
                    message: "persist-only".into(),
                });
                let persist_only_id = persisted_weepcode_event_id(&mut prx);
