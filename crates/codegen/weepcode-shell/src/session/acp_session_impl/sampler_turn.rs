...
/// `SamplingError::is_auth_error` in weepcode-sampling-types: 403 is
...
pub(super) fn is_auth_tool_error(err: &weepcode_tool_runtime::ToolError) -> bool {
...
impl SessionTokenAuthGate {
    ...
    fn new(
        ...
        Self {
            ...
            model_byok,
            endpoint_is_first_party: crate::util::is_weepcode_api_url(base_url),
        }
    }
    fn active(self) -> bool {
...
pub(super) async fn call_with_auth_retry<F, Fut>(
    ...
    tool_name: &str,
    mut call: F,
) -> Result<weepcode_tools::types::output::ToolRunResult, weepcode_tool_runtime::ToolError>
where
    F: FnMut() -> Fut,
    Fut: std::future::Future<
            Output = Result<
                weepcode_tools::types::output::ToolRunResult,
...
impl SessionActor {
    ...
    /// against first-party WeepCode hosts).
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        impl weepcode_sampler::HeaderInjector for TraceContextInjector {
            fn inject(&self, headers: &mut reqwest::header::HeaderMap) {
                if let Some(tp) = weepcode_file_utils::trace_context::current_traceparent()
        ...
        impl weepcode_sampler::BearerResolver for AuthManagerBearerResolver {
        ...
        let cfg = self
            ...
            .get_sampling_config()
            .await
            .unwrap_or_else(|| weepcode_sampling_types::SamplingConfig {
                base_url: String::new(),
                model: String::new(),
        ...
        SamplingConfig {
            ...
            bearer_resolver: if use_bearer_resolver {
                self.auth_manager
                    .as_ref()
                    .map(|am| -> weepcode_sampler::SharedBearerResolver {
    ...
    pub(crate) async fn wire_permission_auto_llm_classifier(self: &Arc<Self>) {
        ...
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<(
            Vec<weepcode_workspace::permission::ClassifierMessage>,
            ...
            while let Some((messages, respond_to)) = rx.recv().await {
                let result = async {
                    ...
                    let items = messages
                        .into_iter()
                        .map(|m| match m.role {
                            weepcode_workspace::permission::ClassifierMessageRole::System => {
                                ConversationItem::system(m.text)
                            }
                            weepcode_workspace::permission::ClassifierMessageRole::User => {
                    ...
                    let request = ConversationRequest {
                        ...
                        json_schema: Some(
                            weepcode_workspace::permission::classifier_output_json_schema(),
                        ),
                        reasoning_effort: classifier_reasoning_effort,
                        x_weepcode_conv_id: Some(session_id.clone()),
                        x_weepcode_req_id: Some(format!(
                            "weepcode-perm-auto-{}",
                            uuid::Uuid::new_v4()
                        )),
                        x_weepcode_session_id: Some(session_id),
        ...
        let clf =
            weepcode_workspace::permission::LlmPermissionClassifier::with_channel(tx, prompt_type);
        debug_assert!(
            clf.has_side_query(),
    ...
    /// catalog routing (Tier-1 catalog creds / Tier-2 WeepCode-proxy via session token
    /// / `WEEPCODE_API_KEY` / deployment key), gathering the session-local auth context
    ...
    pub(super) async fn resolve_aux_sampler_config(
        &self,
        slug: &str,
    ) -> Option<weepcode_sampler::SamplerConfig> {
        let creds = self.chat_state_handle.get_credentials().await;
        ...
        let disable_api_key_auth = self
            .auth_manager
            .as_ref()
            .map(|am| am.weepcode_com_config().api_key_auth_disabled())
            .unwrap_or(false);
    ...
    async fn resolve_auto_classifier_sampler(
        &self,
        slug: &str,
    ) -> Option<(weepcode_sampler::SamplingClient, String)> {
        ...
        let client = weepcode_sampler::SamplingClient::new(cfg)
            .map_err(|e| {
                tracing::warn!(
                    error = % e,
    ...
    pub(super) async fn prepare_chat_completion(
        &self,
        force_http1: bool,
    ) -> Result<weepcode_sampler::SamplingClient, acp::Error> {
        self.refresh_token_if_expired().await;
        let mut full_config = self.reconstruct_full_config().await;
        full_config.force_http1 = force_http1;
        let sampling_client =
            weepcode_sampler::SamplingClient::new(full_config).map_err(|e| self.to_acp_error(e))?;
        Ok(sampling_client)
    }
    ...
    /// `weepcode-sampler` instead of constructing a new
    /// `OaiCompatClient`.
    ///
    ...
    pub(crate) async fn handle_sampling_failure(
        self: &Arc<Self>,
        error: weepcode_sampler::SamplingErrorInfo,
    ) -> Result<SamplerFailureRecovery, acp::Error> {
        use weepcode_sampler::SamplingErrorKind;
        ...
        let client_version = weepcode_version::VERSION;
        if auth_mode == crate::auth::AuthMode::WebLogin {
            let msg = format!(
                "{detailed_message}\n\n\
                 ...
                 To fix: run `weepcode logout` then `weepcode login` to re-authenticate with OAuth2.\n\n\
        ...
        let error_type = if weepcode_sampling_types::is_context_length_error(&error.message) {
            "context_length"
        } else {
            error.kind.as_str()
        };
    ...
    ///    `send_weepcode_notification(RetryState::Failed)`.
    pub(crate) async fn run_turn_via_sampler(
        ...
        let request_id = weepcode_sampler::RequestId::random();
            ...
            .await
        {
            ...
            Err(rich_err) => {
                self.turn_stream_drained.lock().take();
                let info = weepcode_sampler::SamplingErrorInfo::from(&rich_err);
