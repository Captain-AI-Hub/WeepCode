...
fn drop_cli_catchall_allows(
    rules: Vec<weepcode_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<weepcode_workspace::permission::types::PermissionRule>,
    Vec<weepcode_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if weepcode_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
mod cli_catchall_drop_tests {
    ...
    use weepcode_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use weepcode_workspace::permission::rules::parse_permission_rule;
    use weepcode_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    credentials: weepcode_chat_state::Credentials,
    auth_method_id: crate::agent::auth_method::SharedAuthMethodId,
    auth_manager: Option<Arc<AuthManager>>,
    attribution_callback: Option<weepcode_sampler::SharedAttributionCallback>,
    ...
    compaction_mode: weepcode_chat_state::CompactionMode,
    ...
    web_search_sampling_config: Option<weepcode_sampler::SamplerConfig>,
    ...
    app_builder_deployer_config: weepcode_tools::implementations::weepcode_build::deploy_app::AppBuilderDeployerConfig,
    ...
    prompt_audience: weepcode_agent::prompt::context::PromptAudience,
    ...
    plugin_registry: Option<std::sync::Arc<weepcode_agent::plugins::PluginRegistry>>,
    plugin_registry_handle: Option<weepcode_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::models::ModelsManager,
    inherited_permission_handle: Option<weepcode_workspace::permission::PermissionHandle>,
    api_key_provider: Option<weepcode_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<weepcode_hooks::discovery::HookRegistry>>,
    workspace_ops: weepcode_workspace::WorkspaceOps,
    cli_permission_rules: Vec<weepcode_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    parent_terminal_backend: Option<
        std::sync::Arc<dyn weepcode_tools::computer::types::TerminalBackend>,
    >,
    parent_scheduler_handle: Option<
        weepcode_tools::implementations::weepcode_build::scheduler::types::SchedulerHandle,
    >,
    ...
    weepcode_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(weepcode_agent::AgentBuildError::InvalidConfig(
        ...
        inherited_permission_handle
    {
        ...
        let mut permission_config =
            weepcode_workspace::permission::resolution::resolve_permission_config_with_fallback(
                tool_context.cwd.as_path(),
            )
            .await;
        let yolo_pin = weepcode_workspace::permission::resolution::yolo_disabled_by_policy();
            ...
            && !dropped_catchalls.is_empty()
        {
            ...
            if startup_hints.non_interactive {
                eprintln!("weepcode: --allow catch-all ignored: {reason}");
            }
        }
            ...
            match &mut permission_config {
                ...
                None => {
                    permission_config = Some(
                        weepcode_workspace::permission::types::PermissionConfig::new(
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(weepcode_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let hub_permission = if weepcode_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                .and_then(|server| {
                    weepcode_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
                ...
                .map(|t| {
                    std::sync::Arc::new(t)
                        as std::sync::Arc<
                            dyn weepcode_workspace::permission::PermissionHookTransport,
                        >
                });
        ...
        let (permissions, permission_events_rx) =
            weepcode_workspace::permission::spawn_permission_manager_with_hub(
    ...
    let web_search_config = if disable_web_search {
        weepcode_tools::implementations::WebSearchConfig::Disabled
    } else if let Some(cfg) = web_search_sampling_config {
        if let Some(api_key) = cfg.api_key {
            weepcode_tools::implementations::WebSearchConfig::Enabled {
                api_key,
    ...
    let context_window_override = std::env::var("WEEPCODE_DEBUG_CONTEXT_WINDOW")
        .ok()
    ...
    if let Some(cw) = context_window_override {
        tracing::warn!(
            ...
            "WEEPCODE_DEBUG_CONTEXT_WINDOW override active"
        );
    }
    let chat_state_sampling_config = weepcode_sampling_types::SamplingConfig {
    ...
    let actor_pruning_config = weepcode_chat_state::PruningConfig {
    ...
    let chat_state_handle = weepcode_chat_state::ChatStateActor::spawn_with_pruning(
        conversation.clone(),
    ...
    let auto_wake_delivered =
        weepcode_tools::reminders::task_completion::AutoWakeDeliveredIds::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        weepcode_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        weepcode_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    let terminal_backend: std::sync::Arc<dyn weepcode_tools::computer::types::TerminalBackend> =
        if let Some(parent_tb) = parent_terminal_backend.filter(|_| startup_hints.is_subagent) {
            ...
            )) as std::sync::Arc<dyn weepcode_tools::computer::types::TerminalBackend>
        } else {
            let backend: std::sync::Arc<dyn weepcode_tools::computer::types::TerminalBackend> =
                std::sync::Arc::new(LocalTerminalBackend::new_local(resolve_search_shadows()));
            backend
        };
    let fs_backend: std::sync::Arc<dyn weepcode_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(weepcode_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(weepcode_tools::computer::local::LocalFs)
        };
    ...
    let harness_metrics = if telemetry_enabled || weepcode_telemetry::external::is_active() {
    ...
    let compaction_policy = weepcode_agent::CompactionPolicy {
    ...
    let (user_question_tx, user_question_rx) = tokio::sync::mpsc::unbounded_channel::<
        weepcode_tools::implementations::weepcode_build::ask_user_question::types::UserQuestionRequest,
    >();
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn weepcode_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        if let Err(e) = storage.ensure_initialized() {
            tracing::warn!(
                target : weepcode_telemetry::memory_log::TARGET, error = % e,
        ...
        }
        {
            ...
            tokio::task::spawn_blocking(move || match gc_storage.gc(gc_max_age) {
                Ok(removed) if removed > 0 => {
                    tracing::info!(
                        target : weepcode_telemetry::memory_log::TARGET, removed,
        ...
        weepcode_telemetry::session_ctx::log_event(
            weepcode_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        weepcode_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    let resolved_task_output =
        weepcode_tools::reminders::task_completion::resolve_task_output_tool_name(
            agent.tool_bridge(),
        )
        .await;
    let resolved_read =
        weepcode_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge())
            .await;
    ...
    tool_context.task_output_tool_name = resolved_task_output.unwrap_or_else(|| {
        weepcode_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL.to_string()
    });
    let scheduler_handle_for_handle = {
        let toolset = agent.tool_bridge().toolset();
        let res = toolset.resources.lock().await;
        res.get::<weepcode_tools::implementations::weepcode_build::scheduler::types::SchedulerHandle>()
            .cloned()
    };
    ...
    let signals_handle = crate::session::signals::spawn_signals_actor();
    ...
    let resolved_workspace_root = weepcode_workspace::session::git::find_git_root_from_path(
    ...
    let sampler_retry_policy = weepcode_sampler::RetryPolicy {
        max_retries: max_retries.unwrap_or(5),
        rate_limit_retry_threshold: 2,
    };
    let (sampler_event_tx, sampler_event_rx) =
        tokio::sync::mpsc::unbounded_channel::<weepcode_sampler::SamplingEvent>();
    let sampler_handle = weepcode_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<weepcode_hooks::error::HookError> = Vec::new();
    let built_hook_registry: Option<Arc<weepcode_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = weepcode_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let obs_bridge = {
        ...
        weepcode_computer_hub_sdk::ObservabilityBridge::new(None, sid)
    };
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        max_turns,
        max_retries: weepcode_sampler::resolve_max_retries(max_retries),
        ...
        vcs_kind: {
            ...
            match weepcode_workspace::session::git::discover_git_root(root) {
                weepcode_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    weepcode_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => weepcode_workspace::session::git::VcsKind::None,
            }
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(weepcode_tools::types::tool_index::ToolIndex(
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(weepcode_tools::types::resources::PlanFilePath(plan_path))
            .await;
    }
    ...
    if let Some(storage) = session.memory.storage() {
        ...
        tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                tracing::info!(
                    target : weepcode_telemetry::memory_log::TARGET, files = files.len(),
                ...
                weepcode_telemetry::session_ctx::log_event(
                    weepcode_telemetry::memory_telemetry::MemoryReindex {
    ...
    }
    {
        ...
        use weepcode_tools::implementations::weepcode_build::ask_user_question::{
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use weepcode_tools::implementations::weepcode_build::ask_user_question::AskUserQuestionMode;
    ...
    let telemetry_ctx = weepcode_telemetry::session_ctx::TelemetryCtx::new(
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            weepcode_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    tokio::task::spawn_local(async move {
        weepcode_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
> {
    let (init_tx, init_rx) = tokio::sync::oneshot::channel::<
        Result<SessionInitResult, weepcode_agent::AgentBuildError>,
    >();
            ...
            local.block_on(&rt, async move {
                let _trace_span = parent_traceparent.as_ref().map(|tp| {
                    let meta = serde_json::json!({ "traceparent" : tp })
                        ...
                        .unwrap_or_default();
                    let span =
                        weepcode_file_utils::trace_context::span_from_meta_traceparent(&meta);
                    span.entered()
                });
