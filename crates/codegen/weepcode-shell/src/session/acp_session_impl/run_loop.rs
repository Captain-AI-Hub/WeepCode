...
pub(super) async fn run_session(
    ...
    mut chat_state_event_rx: mpsc::UnboundedReceiver<weepcode_chat_state::ChatStateEvent>,
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<weepcode_mcp::servers::McpClientEvent>();
        {
    ...
    loop {
        tokio::select! {
            ...
            weepcode_telemetry::memory_log::TARGET,
            ...
            } else { tracing::debug!(target : weepcode_telemetry::memory_log::TARGET,
            ...
            weepcode_telemetry::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
            ...
            Some(weepcode_chat_state::ChatStateEvent::ConversationReset { new_len }) => {
            ...
            Some(weepcode_chat_state::ChatStateEvent::ImageBudget { body_bytes, trigger_bytes,
            reclaim_target_bytes, inline_images, needs_image_compaction, evicted,
            body_bytes_after, }) => {
            weepcode_telemetry::unified_log::info("shell.image_budget", Some(session
            ...
            })),); } Some(weepcode_chat_state::ChatStateEvent::PromptIndexChanged { .. }) |
            Some(weepcode_chat_state::ChatStateEvent::TokensUpdated { .. }) => {} None => {} }
            ...
            Some((prompt_id, result)) = maybe_completion else {
            ...
            .fire_hook(weepcode_hooks::event::HookEventName::SessionEnd, None,
            weepcode_hooks::event::HookPayload::SessionEnd { reason : "channel_closed"
            ...
            weepcode_hooks::dispatcher::dispatch_non_blocking(& registry,
            weepcode_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(weepcode_hooks::event::HookEventName::Stop,
            None, weepcode_hooks::event::HookPayload::Stop { reason : "channel_closed"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            weepcode_hooks::dispatcher::dispatch_non_blocking(& registry,
            weepcode_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
            ...
            weepcode_telemetry::memory_log::TARGET, result = ? result, tool_searches =
            ...
            session.send_weepcode_notification(WeepcodeSessionUpdate::MemorySessionSaved { path :
            ...
            weepcode_telemetry::session_ctx::log_event(weepcode_telemetry::events::SessionEnded
            ...
            compaction_count : signals.compaction_count as u64, model_id, },); } } session.drain_uploads_and_shutdown_signals(). await; if ! session
            ...
            weepcode_file_utils::trace_context::link_current_span_to_meta(& meta); } let
            ...
            .update_credentials(weepcode_chat_state::Credentials { api_key : r.api_key,
            ...
            .send(weepcode_hooks_plugins_types::HooksListResponse { hooks, project_trusted,
            ...
            .send_weepcode_notification(WeepcodeSessionUpdate::PluginUpdatesInstalled { updates },)
            ...
            .push_capped(weepcode_tools::implementations::weepcode_build::task::types::MonitorEventNotification
            ...
            weepcode_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
            ...
            .handle_weepcode_session_notification(notification). await; }
            ...
            .send(weepcode_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
            ...
            .emit(weepcode_file_utils::events::Event::McpServerToggled { server_name :
            ...
            .forward_fire_and_forget(acp::ExtNotification::new("weepcode/mcp/tools_changed",
            ...
            session.fire_hook(weepcode_hooks::event::HookEventName::SessionStart, None,
            weepcode_hooks::event::HookPayload::SessionStart { source, model_id : None,
            agent_type : None, },); if let Some(registry) = session.hook_registry
            .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
            weepcode_hooks::dispatcher::dispatch_non_blocking(& registry,
            weepcode_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
            await; session.send_hook_execution("session_start", None, None, & results).
            await; } } SessionCommand::GetActiveAgent {
            ...
            .fire_hook(weepcode_hooks::event::HookEventName::SessionEnd, None,
            weepcode_hooks::event::HookPayload::SessionEnd { reason : "shutdown"
            .to_string(), turn_count : None, tool_call_count : None, },); if let
            Some(registry) = session.hook_registry.borrow().clone() { let ctx = session
            .hook_run_ctx(); let results =
            weepcode_hooks::dispatcher::dispatch_non_blocking(& registry,
            ...
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(weepcode_hooks::event::HookEventName::Stop,
            None, weepcode_hooks::event::HookPayload::Stop { reason : "shutdown"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            weepcode_hooks::dispatcher::dispatch_non_blocking(& registry,
            weepcode_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
            ...
            session.maybe_run_dream(). await; let telem = session.memory
            .telemetry_snapshot(); session.emit_memory_session_summary(& telem,
            total_chunks_at_end, session_end_result); session.drain_uploads_and_shutdown_signals(). await; if ! session.startup_hints
