...
) -> weepcode_tool_protocol::turn_hook::TurnHookOutcome {
...
) -> weepcode_tool_protocol::session_event::ToolCallOutcome {
    use crate::session::events::ToolOutcome;
    use weepcode_tool_protocol::session_event::ToolCallOutcome;
    match outcome {
...
pub(super) fn notification_hook_for_update(
    ...
    match update {
        WeepcodeSessionUpdate::DiffReview { .. } => Some((
            "permission_prompt".into(),
            Some("Diff review requested".into()),
            None,
            Some("info".into()),
        )),
        WeepcodeSessionUpdate::AutoRecoveryExhausted { error, .. } => Some((
            "agent_error".into(),
            Some(error.clone()),
            None,
            Some("error".into()),
        )),
        WeepcodeSessionUpdate::RetryState(RetryState::Exhausted { reason, .. }) => Some((
            "agent_error".into(),
            Some(reason.clone()),
            None,
            Some("error".into()),
        )),
        WeepcodeSessionUpdate::RetryState(RetryState::Failed { message, .. }) => Some((
            "agent_error".into(),
            Some(message.clone()),
            None,
            Some("error".into()),
...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> weepcode_hooks::runner::RunContext<'_> {
        weepcode_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_annotation(&self, message: &str) {
        self.send_weepcode_notification(WeepcodeSessionUpdate::HookAnnotation {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[weepcode_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, weepcode_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
        use crate::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
        use weepcode_hooks::result::HookRunResult;
        ...
        self.send_weepcode_notification(WeepcodeSessionUpdate::HookExecution {
    ...
    pub(super) async fn dispatch_hook(
        &self,
        event: weepcode_hooks::event::HookEventName,
        payload: weepcode_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            weepcode_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx)
                .await;
    ...
    pub(super) async fn emit_hook_executed_telemetry(
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                weepcode_hooks::result::HookRunResult::Success {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    weepcode_telemetry::events::HookOutcome::Success,
                ),
                weepcode_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    weepcode_telemetry::events::HookOutcome::Error,
                ),
                weepcode_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            weepcode_telemetry::session_ctx::log_event(weepcode_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn hook_execution_does_not_fire_notification_hook() {
        let update = WeepcodeSessionUpdate::HookExecution {
    ...
    fn hook_annotation_does_not_fire_notification_hook() {
        let update = WeepcodeSessionUpdate::HookAnnotation {
    ...
    fn retry_in_progress_does_not_fire_notification_hook() {
        let update = WeepcodeSessionUpdate::RetryState(RetryState::Retrying {
            attempt: 1,
            max_retries: 3,
            reason: "timeout".into(),
        });
    ...
    fn diff_review_fires_permission_prompt() {
        let update = WeepcodeSessionUpdate::DiffReview { content: vec![] };
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = WeepcodeSessionUpdate::TaskCompleted {
            task_snapshot: weepcode_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
