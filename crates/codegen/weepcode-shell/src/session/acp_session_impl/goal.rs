...
impl RoleCapability {
    ...
    fn is_satisfied(
        self,
        summary: &weepcode_tools::implementations::weepcode_build::task::types::SubagentTypeSummary,
    ) -> bool {
        match self {
...
impl SessionActor {
    ...
    pub(crate) async fn resolve_goal_role_override(
        ...
        //    silently run weepcode-build flavor. Non-strict names (weepcode-build
        //    family) run the default flavor and pass; unresolvable names fall
        //    through to the describe `Unknown` arm below.
        if weepcode_agent::config::is_strict_harness_agent_type(&pair.agent_type)
    ...
    /// agrees with `from_summary` — `search_replace` on the default weepcode-build
    /// host, not the literal `write`). The `{TOOLSET_TOOLS}` block is empty on
    /// this path (no per-role toolset enumeration); on the default weepcode-build
    ...
    pub(super) async fn prune_prior_goal_continuation_directives(&self) {
        use weepcode_sampling_types::conversation::SyntheticReason;
