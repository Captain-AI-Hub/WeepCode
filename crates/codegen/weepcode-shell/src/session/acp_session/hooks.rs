...
//!
//! Hooks registered at `session/new` (`_meta["weepcode/hooks"]`) come in two flavors,
//! both matched by the agent ([`weepcode_hooks::matcher::HookMatcher`], shared with
//! file hooks):
...
use weepcode_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use weepcode_telemetry::events::ClientHookGateOutcome;
...
const HOOK_EVENT_METHOD: &str = "weepcode/hooks/event";
const HOOK_RUN_METHOD: &str = "weepcode/hooks/run";
...
fn classify(outcome: ReverseOutcome) -> (ClientHookResponse, ClientHookGateOutcome) {
    match outcome {
                ...
                Ok(resp) => {
                    ...
                    let label = match resp.decision {
                        ...
                        ClientHookDecision::Other => {
                            tracing::warn!(
                                "weepcode/hooks/run returned an unknown decision value; failing open"
                            );
        ...
        ReverseOutcome::Transport(err) => {
            tracing::warn!(%err, "weepcode/hooks/run transport error (no client wired?); failing open");
            (
                ClientHookResponse::default(),
        ...
        ReverseOutcome::Timeout => {
            tracing::warn!("weepcode/hooks/run timed out; failing open");
            (
                ClientHookResponse::default(),
...
impl SessionActor {
    ...
    pub(super) async fn deny_tool(
        ...
        weepcode_telemetry::session_ctx::log_event(weepcode_telemetry::events::HookBlocked {
    ...
    pub(super) async fn run_pre_tool_use_client_hook(
        ...
        let tool_name = weepcode_hooks::dispatcher::extract_tool_name(envelope)
            ...
            .map(|(callback_id, timeout)| {
                ...
                async move {
                    ...
                    weepcode_telemetry::session_ctx::log_event(
                        weepcode_telemetry::events::ClientHookGate {
...
mod tests {
    ...
    async fn hook_event_active_inert_vs_active() {
        use weepcode_hooks::event::HookEventName;
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    weepcode_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
    ...
    fn matching_callback_ids_filters_by_matcher() {
        use weepcode_hooks::matcher::HookMatcher;
