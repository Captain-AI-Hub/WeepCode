...
use weepcode_file_utils::queue::UploadQueue;
...
pub struct SessionHandle {
    ...
    pub chat_state_handle: weepcode_chat_state::ChatStateHandle,
    ...
    pub permission_handle: weepcode_workspace::permission::PermissionHandle,
    ...
    pub terminal_backend:
        Option<std::sync::Arc<dyn weepcode_tools::computer::types::TerminalBackend>>,
...
impl SessionHandle {
    ...
    pub(crate) async fn get_model_metadata(&self) -> weepcode_chat_state::ModelMetadata {
            ...
            .is_ok()
        {
            ...
            weepcode_chat_state::ModelMetadata::default()
        }
    }
    ...
    /// Routes through the session actor to the ToolBridge's TerminalBackend.
    pub async fn list_tasks(&self) -> Option<Vec<weepcode_tools::types::TaskSnapshot>> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub async fn get_hooks_list(&self) -> Option<weepcode_hooks_plugins_types::HooksListResponse> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub(crate) async fn snapshot_tool_definitions(&self) -> Vec<weepcode_sampling_types::ToolSpec> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
