...
use weepcode_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
use weepcode_mcp::servers as inner;
...
pub fn build_config_resolved_event(
    ...
    cwd: &Path,
) -> weepcode_file_utils::events::Event {
    let disabled: Vec<String> = crate::util::config::disabled_mcp_server_names(cwd)
        .into_iter()
        .collect();
    let servers = configs
        .iter()
        .map(|c| weepcode_file_utils::events::McpConfigServer {
        ...
        .collect();
    weepcode_file_utils::events::Event::McpConfigResolved { servers, disabled }
}
