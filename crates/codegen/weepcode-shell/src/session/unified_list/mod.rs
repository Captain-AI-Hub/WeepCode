...
/// Env lane (desktop `WEEPCODE_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`WEEPCODE_CHAT_MODE`); hard-off in release builds.
...
pub fn force_kind_chat(req: &mut ListReq) {
    ...
    let mut filters = match meta.remove("weepcode/facetFilters") {
        Some(serde_json::Value::Object(map)) => map,
        _ => serde_json::Map::new(),
    };
    filters.insert(
    ...
    meta.insert(
        "weepcode/facetFilters".to_owned(),
...
pub struct ExtListResponseMeta {
    #[serde(rename = "weepcode/facets")]
    pub facets: FacetSummary,
    #[serde(rename = "weepcode/partial")]
...
mod tests {
    ...
    fn local(session_id: &str, updated_at: &str) -> MergedSession {
        MergedSession {
            ...
            source: "local".into(),
            model_id: Some("weepcode-build".into()),
            num_messages: 7,
            ...
            source_workspace_dir: Some("/Users/me/weepcode-src".into()),
    ...
    fn ext_superset_preserves_every_legacy_field_and_adds_title_and_meta() {
        ...
        assert_eq!(value["_meta"]["weepcode/session"]["kind"], "build");
        assert_eq!(value["gitRootDir"], "/Users/me/xai");
        assert_eq!(value["gitRemotes"][0], "git@github.com:example/repo.git");
        assert_eq!(value["sourceWorkspaceDir"], "/Users/me/weepcode-src");
    ...
    fn parsed_meta_reads_facet_filters_query_and_limit() {
        let meta = serde_json::json!(
            { "weepcode/facetFilters" : { "kind" : ["build"], "starred" : true },
            "weepcode/query" : "antelope", "weepcode/limit" : 5, }
        );
    ...
    fn forced_kind_replaces_client_build_filter() {
        let mut req = ListReq {
            meta: Some(serde_json::json!({ "weepcode/facetFilters" : { "kind" : ["build"] }, })),
            ..ListReq::default()
        };
        force_kind_chat(&mut req);
    ...
    fn forced_kind_preserves_other_facets() {
        let mut req = ListReq {
            meta: Some(serde_json::json!(
                { "weepcode/facetFilters" : { "kind" : ["build"], "starred" : [true],
                "workspace" : ["w1"] }, "weepcode/query" : "antelope", "weepcode/limit" : 5,
                }
            )),
            ..ListReq::default()
        };
    ...
    fn weepcode_auth_manager(dir: &std::path::Path) -> std::sync::Arc<crate::auth::AuthManager> {
        let am = std::sync::Arc::new(crate::auth::AuthManager::new(
            dir,
            crate::auth::WeepcodeComConfig::default(),
        ));
        am.hot_swap(crate::auth::WeepcodeAuth {
            auth_mode: crate::auth::AuthMode::Oidc,
            oidc_issuer: Some(crate::auth::WEEPCODE_OAUTH2_ISSUER.to_owned()),
    ...
    async fn forced_kind_serves_conversations_only() {
        ...
        .await;
        let _env = weepcode_test_support::EnvGuard::set(
            "WEEPCODE_CONVERSATIONS_BASE_URL",
            format!("http://{addr}"),
        );
        let home = tempfile::tempdir().expect("tempdir");
        let client = ConversationsClient::new(weepcode_auth_manager(home.path()));
        let mut req = ListReq {
            meta: Some(serde_json::json!({ "weepcode/facetFilters" : { "kind" : ["build"] }, })),
            ..ListReq::default()
        };
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off =
                weepcode_test_support::EnvGuard::unset("WEEPCODE_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on =
                weepcode_test_support::EnvGuard::set("WEEPCODE_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off =
                weepcode_test_support::EnvGuard::set("WEEPCODE_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::WEEPCODE_CHAT_MODE_ENV;
        let _chat_off = weepcode_test_support::EnvGuard::unset(WEEPCODE_CHAT_MODE_ENV);
        let _desktop_off =
            weepcode_test_support::EnvGuard::unset("WEEPCODE_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _chat = weepcode_test_support::EnvGuard::set(WEEPCODE_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        let raw = serde_json::json!(
            { "_meta" : { "weepcode/facetFilters" : { "kind" : ["build"], "starred" : [true]
            } }, }
        )
        .to_string();
        {
            let _off = weepcode_test_support::EnvGuard::unset(WEEPCODE_CHAT_MODE_ENV);
        ...
        {
            let _on = weepcode_test_support::EnvGuard::set(WEEPCODE_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
            let expected = "build";
            assert_eq!(
    ...
    fn ext_list_response_serializes_partial_reasons() {
        ...
        .expect("serialize");
        assert_eq!(
            healthy["_meta"]["weepcode/partial"],
            serde_json::json!({ "conversations" :
            false })
        );
    }
