...
use weepcode_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use weepcode_agent::prompt::skills::SkillsConfig;
use weepcode_sampler::SamplerConfig as SamplingConfig;
use weepcode_sampling_types::truncate_bytes;
...
use weepcode_tools::types::compat::CompatConfig;
use weepcode_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use weepcode_workspace::file_system::CodebaseIndexManager;
use weepcode_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use weepcode_workspace::session::file_state::{FileStateHandle, FileStateTracker};
const SESSION_LOG: &str = "weepcode_session";
#[path = "compaction.rs"]
mod compaction;
...
impl weepcode_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        weepcode_tools::types::resources::ManagedGatewayToolCallResponse,
        weepcode_tool_runtime::ToolError,
    > {
        let auth_key = self
            .auth_manager
            .get_valid_token()
            ...
            .ok_or_else(|| {
                weepcode_tool_runtime::ToolError::unauthorized("no auth token available")
            })?;
    ...
    match error {
        crate::session::managed_mcp::ManagedMcpFetchError::Status { status, message } => {
            ...
            let mut err = if status == reqwest::StatusCode::UNAUTHORIZED {
                weepcode_tool_runtime::ToolError::unauthorized(detail)
            } else if status == reqwest::StatusCode::FORBIDDEN {
                weepcode_tool_runtime::ToolError::permission_denied(detail)
            } else {
                let tool_id = weepcode_tool_protocol::ToolId::new(caller).unwrap_or_else(|_| {
                    weepcode_tool_protocol::ToolId::new("use_tool").expect("valid")
                });
                weepcode_tool_runtime::ToolError::execution(tool_id, detail)
            };
        ...
        crate::session::managed_mcp::ManagedMcpFetchError::Transport(e) => {
            weepcode_tool_runtime::ToolError::network_error(format!(
...
mod managed_gateway_error_tests {
    ...
    fn unauthorized_status_maps_to_unauthorized_and_carries_status() {
        ...
        assert_eq!(err.kind, weepcode_tool_runtime::ToolErrorKind::Unauthorized);
    ...
    fn forbidden_status_maps_to_permission_denied_and_carries_status() {
        ...
        assert_eq!(
            err.kind,
            weepcode_tool_runtime::ToolErrorKind::PermissionDenied
        );
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    fn general_status_maps_to_execution_with_caller_tool_id() {
        ...
        assert_eq!(err.kind, weepcode_tool_runtime::ToolErrorKind::Execution);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    async fn transport_error_maps_to_network_error_without_url() {
        ...
        assert_eq!(err.kind, weepcode_tool_runtime::ToolErrorKind::NetworkError);
...
pub(crate) struct SessionActor {
    ...
    /// `weepcode-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<weepcode_sampler::SharedAttributionCallback>,
    ...
    pub(crate) chat_state_handle: weepcode_chat_state::ChatStateHandle,
    ...
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<weepcode_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<weepcode_sampling_types::CompactionAtTokens>>,
    ...
    /// sampler itself sends the matching `x-weepcode-doom-loop-check` header.
    pub(crate) doom_loop_recovery: Option<weepcode_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    /// Handle for recording session signals (turn/tool/compaction counters).
    pub(crate) signals_handle: SessionSignalsHandle,
    pub(crate) upload_queue:
        std::sync::Arc<std::sync::OnceLock<weepcode_file_utils::queue::UploadQueue>>,
    ...
    pub(crate) agent: std::cell::RefCell<weepcode_agent::Agent>,
    ...
    /// Kill-switch (`WEEPCODE_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<weepcode_tools::implementations::weepcode_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, weepcode_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) extension_registry: weepcode_agent_lifecycle::LocalExtensionRegistry,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<weepcode_hooks::discovery::HookRegistry>>>,
    /// Client hooks from `session/new` `_meta["weepcode/hooks"]`; gated in
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<weepcode_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<weepcode_agent::plugins::SharedPluginRegistryHandle>,
    ...
    pub(crate) observability_bridge: weepcode_computer_hub_sdk::ObservabilityBridge,
    ...
    /// Handle to the per-session `weepcode-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: weepcode_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`weepcode_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`weepcode_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: weepcode_workspace::WorkspaceOps,
...
impl SessionActor {
    ...
    /// Session-end teardown: drain the trace-upload queue (bounded wait) and
    /// shut down the signals actor.
    pub(crate) async fn drain_uploads_and_shutdown_signals(&self) {
        const UPLOAD_DRAIN_TIMEOUT: Duration = Duration::from_secs(30);
        if let Some(queue) = self.upload_queue.get() {
            let remaining = queue.drain(UPLOAD_DRAIN_TIMEOUT).await;
            if remaining > 0 {
                ...
                tracing::warn!(
                    ...
                    remaining,
                    pending_bytes,
                    "Upload queue drain incomplete, items abandoned"
                );
            }
        }
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == weepcode_tools::implementations::weepcode_build::SCHEDULER_CREATE_TOOL_NAME
            }),
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<weepcode_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering, `weepcode prompt --json`
/// inspection, and post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &weepcode_agent::PromptContext) {
...
) -> Option<weepcode_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use weepcode_tools::types::output::{MCPOutput, ToolOutput};
    use weepcode_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl weepcode_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
    ...
    impl weepcode_tool_runtime::Tool for FixtureMcpTool {
        type Args = serde_json::Value;
        type Output = ToolOutput;
        fn id(&self) -> weepcode_tool_protocol::ToolId {
            weepcode_tool_protocol::ToolId::new("server__tool").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::weepcode_tool_runtime::ListToolsContext,
        ) -> weepcode_tool_types::ToolDescription {
            weepcode_tool_types::ToolDescription::new("server__tool", "fixture")
        }
        async fn run(
            &self,
            _ctx: weepcode_tool_runtime::ToolCallContext,
            _args: serde_json::Value,
        ) -> Result<ToolOutput, weepcode_tool_runtime::ToolError> {
...
mod tool_meta_stamp_tests {
    ...
    use super::*;
    use tokio::sync::mpsc;
    use weepcode_tools::registry::types::ToolConfig;
    use weepcode_tools::tool_taxonomy::TOOL_META_KEY;
    use weepcode_workspace::permission::PermissionCommand;
            ...
            .run_until(async {
                ...
                fixture.actor.agent = std::cell::RefCell::new(
                    test_agent_with_tools(vec![ToolConfig::from_id(
                        "WeepcodeBuild:read_file".to_string(),
                    )])
                    .await,
                );
                ...
                assert_eq!(t["namespace"], "weepcode_build");
...
mod managed_gateway_tool_tests {
    ...
    use weepcode_tools::types::output::{MCPOutput, ToolOutput};
    use weepcode_tools::types::tool::{ToolKind, ToolNamespace};
    use weepcode_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<weepcode_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
