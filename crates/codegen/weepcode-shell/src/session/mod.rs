...
pub use self::persistence::{
    ...
    session_exists_by_id, session_exists_for_cwd,
};
pub use self::result::{Empty, ExtMethodResult};
pub use weepcode_fsnotify::{
...
pub(crate) fn is_cursor_user_template(
    _template: &weepcode_agent::prompt::user_message::UserMessageTemplate,
) -> bool {
    false
}
/// `false` twin of [`is_cursor_system_template`]; see [`is_cursor_user_template`].
pub(crate) fn is_cursor_system_template(
    _template: &weepcode_agent::prompt::context::TemplateOverride,
) -> bool {
    false
}
...
/// Client-requested fs notification mode (was weepcode_fsnotify::FsNotifyMode).
...
pub use weepcode_shared::session::info;
pub mod managed_mcp;
...
pub mod persistence;
pub use weepcode_shared::placeholder_images;
pub mod plan_mode;
