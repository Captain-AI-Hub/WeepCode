...
//! shell-specific types (`weepcode_tools::MemoryBackend`, memory context).
//!
//! The core [`CompactionStateContext`] struct and its builder live in
//! `weepcode_chat_state::compaction_utils`. This module adds system-reminder
//! rendering that requires dependencies not available in `weepcode-chat-state`.
//!
//! The three **common** active-agent sections (background tasks, TODO list,
//! running subagents) are formatted by
//! [`weepcode_compaction::reminder`] so weepcode-chat and weepcode-build stay in lockstep.
...
use weepcode_compaction::reminder::{
...
pub fn to_system_reminder_sync(
    ...
    skills: &[weepcode_tools::implementations::skills::types::SkillInfo],
...
pub async fn to_system_reminder(
    ...
    discovered_agents_md: &[PathBuf],
    skills: &[weepcode_tools::implementations::skills::types::SkillInfo],
    memory_backend: Option<&dyn weepcode_tools::types::memory_backend::MemoryBackend>,
    subagent_tool_names: Option<&SubagentToolNames>,
    mcp_tool_names: Option<&McpToolNames>,
) -> Option<String> {
...
fn to_system_reminder_inner(
    ...
    discovered_agents_md: &[PathBuf],
    skills: &[weepcode_tools::implementations::skills::types::SkillInfo],
    memory_results: &[weepcode_tools::types::memory_backend::MemorySearchResult],
    subagent_tool_names: Option<&SubagentToolNames>,
    mcp_tool_names: Option<&McpToolNames>,
) -> Option<String> {
    ...
    if let Some(listing) =
        weepcode_tools::types::skill_discovery_tracker::format_compaction_skill_listing(skills)
    {
    ...
    if !ctx.connected_mcp_servers.is_empty() {
        use weepcode_tools::implementations::search_tool::format_compaction_server_line;
