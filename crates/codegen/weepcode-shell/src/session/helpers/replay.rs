...
use crate::extensions::notification::{
    CompactionCheckpointFile, CompactionCheckpointInfo, SessionUpdate as WeepcodeSessionUpdate,
};
...
pub fn find_latest_compaction_checkpoint(
    ...
    for line in raw_contents.lines() {
        ...
        >(raw_params.get())
            && let WeepcodeSessionUpdate::CompactionCheckpoint(info) = notification.update
        {
            latest = Some(*info);
        }
    }
        ...
        match update {
            SessionUpdate::Xai(notification) => {
                match &notification.update {
                    WeepcodeSessionUpdate::CompactionCheckpoint(info) => {
                        return self.handle_checkpoint(info, session_dir);
                    }
                    WeepcodeSessionUpdate::RewindMarker {
                        target_prompt_index,
                        ..
                    } => {
                    ...
                    // Other WeepCode notifications are informational — skip them.
                    _ => {}
                }
            }
...
mod tests {
    ...
    fn make_checkpoint(
        ...
        SessionUpdate::Xai(Box::new(XaiNotification {
            session_id: acp::SessionId::new("test"),
            update: WeepcodeSessionUpdate::CompactionCheckpoint(Box::new(
