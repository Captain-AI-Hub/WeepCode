//! weepcode-build's L5 wiring onto the shared full-replace engine
//! (`weepcode_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](weepcode_compaction::sample_full_replace_summary);
//! this module adapts weepcode-build's transport and telemetry to its two seams:
//!
...
//! [`FullReplaceError`](weepcode_compaction::FullReplaceError).
...
use weepcode_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use weepcode_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
...
/// [`CompactionSampler`] for weepcode-build's full-replace pass.
///
...
/// else the structured weepcode-build prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the weepcode-build prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured weepcode-build), ignoring the shared
...
/// Map weepcode-build's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces weepcode-build's per-attempt telemetry:
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    weepcode_telemetry::session_ctx::log_event(CompactionRetryDegraded {
