...
//! `weepcode-file-utils`. The orphan-rule items (`From<&permission::Decision>`
//! and the doom-loop categorizer) stay here since they need shell-local
//! types.

pub(crate) use weepcode_file_utils::events::tracker::EventTracker;
pub(crate) use weepcode_file_utils::events::types::{
...
) -> Option<weepcode_sampling_types::PriorTurnInterrupt> {
    use weepcode_sampling_types::PriorTurnInterrupt;
    match category {
...
/// committing it would silently run weepcode-build flavor. Distinct from
...
/// (`weepcode-file-utils`). The two enums are intentional mirrors and live in
