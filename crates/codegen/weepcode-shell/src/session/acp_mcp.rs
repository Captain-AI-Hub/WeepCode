...
//!
//! The official `weepcode-agent-sdk` lets a host define in-process tools (`@tool` /
//! `create_sdk_mcp_server`). When `transport="acp"`, the SDK registers them in
//! `session/new` `_meta["weepcode/mcp/servers"] = [{ "name", "serverId" }]` and the agent
...
use weepcode_mcp::acp_transport::AcpReverseInvoker;
use weepcode_mcp::servers::AcpServerEntry;
use weepcode_mcp::wire;

/// Parse `_meta["weepcode/mcp/servers"]` into [`AcpServerEntry`] registrations. Each entry
