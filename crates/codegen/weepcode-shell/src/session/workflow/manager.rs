...
use weepcode_workflow::{Journal, WorkflowOutcome, WorkflowRunParams};
...
pub(crate) const WORKFLOW_DEFAULT_AGENT_BUDGET: u64 = weepcode_workflow::DEFAULT_AGENT_BUDGET;
...
impl WorkflowManager {
    ...
    pub(crate) fn launch(
        ...
        let (run_id, journal, state) = match &spec.resume_run_id {
            Some(run_id) => {
                ...
                if existing.status
                    ...
                    && existing.agents_used >= weepcode_workflow::MAX_AGENT_BUDGET
                {
                    return Err(LaunchError::NotResumable(
        ...
        let exec = tokio::task::spawn_blocking(move || {
            weepcode_workflow::run_workflow(WorkflowRunParams {
                script,
                args,
                journal,
                host_tx,
    ...
    pub(crate) fn test_bundle_with_session_dir(
        ...
        let notify = super::notify::WorkflowNotifySender::new(
            ...
            weepcode_acp_lib::AcpAgentGatewaySender::new(gateway_tx),
            persist_tx,
            store.clone(),
        );
    ...
    fn cancel_children_for_run(&self, run_id: &str) -> bool {
        ...
        self.subagent_event_tx
            .send(
                weepcode_tools::implementations::weepcode_build::task::types::SubagentEvent::Cancel(
                    weepcode_tools::implementations::weepcode_build::task::types::SubagentCancelRequest {
                        parent_session_id: Some(self.session_id.clone()),
                        target: weepcode_tools::implementations::weepcode_build::task::types::SubagentCancelTarget::WorkflowRunId(
                            run_id.to_owned(),
                        ),
                        respond_to,
                    },
...
mod tests {
    ...
    type CancelLog = Arc<
        parking_lot::Mutex<
            Vec<weepcode_tools::implementations::weepcode_build::task::types::SubagentCancelTarget>,
        >,
    >;
    ...
    async fn pause_marks_user_paused_and_resume_replays() {
        ...
        use weepcode_tools::implementations::weepcode_build::task::types::SubagentEvent;
        ...
        use weepcode_tools::implementations::weepcode_build::task::types::SubagentResult;
    ...
    async fn failed_run_resumes_and_reexecutes_failed_host_call_live() {
        ...
        assert!(
            std::fs::read_to_string(&journal_path)
                .unwrap()
                .contains("__weepcode_workflow_host_error"),
    ...
    async fn workflow_spawns_await_to_completion() {
        ...
        assert_eq!(
            ...
            weepcode_tools::implementations::weepcode_build::task::types::ModelOverrideProvenance::Tool,
