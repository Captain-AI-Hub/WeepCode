use crate::extensions::notification::{
    SessionNotification as WeepcodeSessionNotification, SessionUpdate as WeepcodeSessionUpdate,
...
impl WorkflowNotifySender {
    ...
    fn dispatch(&self, update: WeepcodeSessionUpdate, persist: bool) {
        let mut meta = None;
        crate::util::event_id::ensure_event_id_meta(&self.session_id.0, &mut meta);
        let notification = WeepcodeSessionNotification {
...
mod tests {
    ...
    fn phase_states_derive_from_current() {
        ...
        match build_workflow_updated(&state, 0, 0) {
            WeepcodeSessionUpdate::WorkflowUpdated { phases, .. } => {
    ...
    fn completed_run_marks_current_phase_done() {
        ...
        t.apply_outcome(
            "wf_2",
            &weepcode_workflow::WorkflowOutcome::Completed {
                result: serde_json::json!("done"),
            },
        );
        let state = t.get("wf_2").unwrap();
        match build_workflow_updated(&state, 0, 0) {
            WeepcodeSessionUpdate::WorkflowUpdated { phases, status, .. } => {
