...
pub enum SessionCommand {
    ...
    SetSessionModel {
        sampling_config: weepcode_sampler::SamplerConfig,
        use_concise: bool,
    ...
    /// rather than the agent-level default (e.g. `weepcode-4.5`).
    ///
    /// Keeps the existing base_url, api_key, and other config — only changes
    /// the `model` field sent in the `x-weepcode-model-override` header and merges
    ...
    GetModelMetadata {
        responds_to: oneshot::Sender<weepcode_chat_state::ModelMetadata>,
    },
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<weepcode_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.weepcode/hooks`,
    ...
    RepairHistory {
        ...
        respond_to: oneshot::Sender<
            anyhow::Result<weepcode_chat_state::compaction_utils::HistoryRepairReport>,
        >,
    },
    GetRewindPoints {
    ...
    /// WeepCode extension session notification - client-side events to store in persistence
    ...
    RecordSubagentUsage {
        by_model: Vec<(String, weepcode_chat_state::UsageTotals)>,
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<weepcode_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        task_id: String,
        respond_to: oneshot::Sender<Result<weepcode_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<weepcode_tools::types::TaskSnapshot>>>,
    },
    ...
    GetHooksList {
        respond_to: oneshot::Sender<weepcode_hooks_plugins_types::HooksListResponse>,
    },
    /// Execute a hooks management action from the pager modal.
    HooksAction {
        action: weepcode_hooks_plugins_types::HooksAction,
        respond_to: oneshot::Sender<weepcode_hooks_plugins_types::ActionOutcome>,
    },
    ...
    PluginsList {
        respond_to:
            oneshot::Sender<Option<std::sync::Arc<weepcode_agent::plugins::PluginRegistry>>>,
    },
    ...
    /// from the notification bridge, which does not go through `send_weepcode_notification`).
    ...
    /// tool-free model call (default `weepcode-build-0.1` when available via
    ...
    /// `weepcode-build` model, low temperature, and capped output tokens.
    RewriteMemoryNote {
        raw_text: String,
    ...
    TakeHarnessTraceTurns {
        respond_to:
            oneshot::Sender<Vec<Vec<weepcode_sampling_types::conversation::ConversationItem>>>,
    },
