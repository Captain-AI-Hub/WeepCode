...
pub enum ClientMode {
    /// Headless mode (weepcode agent, weepcode agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (weepcode agent stdio, weepcode -p) - uses local IPC.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `weepcode update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `weepcode update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `weepcode update` just installed; the leader uses
...
mod tests {
    ...
    fn workspace_status_payload_roundtrip() {
        let payload = ControlPayload::WorkspaceStatus {
            ...
            uptime_ms: 4200,
            active_tool_calls: 2,
            sessions: vec!["weepcode-a".into(), "weepcode-b".into()],
            pid: 4242,
        };
