//! Leader-follower IPC architecture for weepcode-shell.
//!
...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.weepcode/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//!
//! ```ignore
//! use weepcode_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
mod transport;
use crate::env::WeepcodeBuildEnvironment;
...
const CLIENT_LEADER_VERSION: &str = weepcode_version::VERSION;
...
/// Maximum reconnection attempts for bounded mode (headless/`weepcode -p`).
...
pub struct LeaderDescriptor {
    ...
    pub environment: Option<WeepcodeBuildEnvironment>,
...
pub enum LeaderTarget {
    Environment(WeepcodeBuildEnvironment),
    WsUrl(String),
    Pid(u32),
}
fn known_environment_for_ws_url(ws_url: &str) -> Option<WeepcodeBuildEnvironment> {
    let environments: &[WeepcodeBuildEnvironment] = &[WeepcodeBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
fn known_environment_for_suffix(ws_url_suffix: &str) -> Option<WeepcodeBuildEnvironment> {
    let environments: &[WeepcodeBuildEnvironment] = &[WeepcodeBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
pub async fn discover_leaders() -> Vec<LeaderDescriptor> {
    discover_leaders_in(&crate::util::weepcode_home::weepcode_home()).await
}
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!(
                            ...
                            "reason" : reason, "killer_ver" : weepcode_version::VERSION,
                            }
                        )),
                    );
                }
...
impl From<&crate::auth::WeepcodeComConfig> for LeaderEnvUrls {
    fn from(c: &crate::auth::WeepcodeComConfig) -> Self {
        Self {
            weepcode_ws_url: c.weepcode_ws_url.clone(),
            weepcode_ws_origin: c.weepcode_ws_origin.clone(),
        }
    }
}
...
pub enum ReconnectPolicy {
    ...
    /// Suitable for headless/`weepcode -p` where hanging forever is unacceptable.
...
///     "weepcode-tui", ClientMode::Stdio, env_urls, caps, status_tx,
/// );
///
...
///
/// The `env_urls.weepcode_ws_url` determines which leader instance to connect to.
/// Different WS URLs get different leader processes (via hashed socket paths).
///
/// # Arguments
///
/// * `client_type` - Identifier for the client type (e.g., "weepcode-tui", "vscode")
...
pub async fn connect_or_spawn(
    ...
    let mut lock = LeaderLock::new(&env_urls.weepcode_ws_url);
...
///
/// For a **managed install** — the running binary lives under `weepcode_home`
/// (e.g. `~/.weepcode/...`) — prefer the managed `~/.weepcode/bin/weepcode` symlink. After an
/// install atomically swaps that symlink, `current_exe()` still resolves
/// (via `/proc/self/exe` on Linux) to the *old* versioned target,
...
/// not under `weepcode_home`), keep `current_exe()` so the spawned leader matches the
/// calling binary.
///
/// Falls back to `~/.weepcode/bin/weepcode` only when `current_exe()` is unavailable.
fn resolve_exe_for_spawn() -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_with_home(&crate::util::weepcode_home::weepcode_home())
}
fn resolve_binary_with_home(weepcode_home: &Path) -> Result<std::path::PathBuf, ConnectionError> {
    resolve_binary_impl(weepcode_home, std::env::current_exe().ok())
}
/// Binary file name for the managed weepcode install (`weepcode` / `weepcode.exe`).
fn managed_weepcode_bin_name() -> &'static str {
    if cfg!(windows) {
        "weepcode.exe"
    } else {
        "weepcode"
...
fn resolve_binary_impl(
    ...
    let managed_bin = weepcode_home.join("bin").join(managed_weepcode_bin_name());
    if let Some(ref exe) = current_exe
        && path_is_under(exe, weepcode_home)
        && managed_bin.exists()
    {
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    cmd.arg("--weepcode-ws-url").arg(&env_urls.weepcode_ws_url);
    cmd.arg("--weepcode-ws-origin")
        .arg(&env_urls.weepcode_ws_origin);
    ...
    for key in [
        "WEEPCODE_DEBUG_LOG",
        "WEEPCODE_HOOKS_LOG",
        "WEEPCODE_LOG_SAMPLING",
        "WEEPCODE_INSTRUMENTATION",
    ] {
    ...
    let log_path = crate::util::weepcode_home::weepcode_home().join("leader.log");
    ...
    let leader_log = std::env::var("WEEPCODE_LEADER_LOG")
        .or_else(|_| std::env::var("RUST_LOG"))
        .unwrap_or_else(|_| "weepcode_shell=info,weepcode_acp_lib=warn,weepcode_mcp=warn".into());
...
mod tests {
    ...
    async fn reconnect_against_hung_leader_exhausts_attempts_without_respawn() {
        ...
        let env_urls = LeaderEnvUrls {
            weepcode_ws_url: "wss://test.invalid".into(),
            weepcode_ws_origin: "https://test.invalid".into(),
        };
    ...
    fn resolve_binary_prefers_current_exe() {
        ...
        std::fs::write(bin_dir.join("weepcode"), "fake-binary").unwrap();
    ...
    fn resolve_binary_prefers_current_exe_over_symlink() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let target_v2 = bin_dir.join("weepcode-v2");
        std::fs::write(&target_v2, "new-binary").unwrap();
        std::os::unix::fs::symlink(&target_v2, bin_dir.join("weepcode")).unwrap();
        let result = resolve_binary_with_home(temp.path()).unwrap();
        let current = std::env::current_exe().unwrap();
        assert_eq!(result, current);
    }
    ...
    fn resolve_binary_prefers_managed_symlink_for_managed_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let new_target = bin_dir.join("weepcode-v2");
        std::fs::write(&new_target, "new-binary").unwrap();
        let managed = bin_dir.join("weepcode");
        std::os::unix::fs::symlink(&new_target, &managed).unwrap();
        let stale_target = bin_dir.join("weepcode-v1");
    ...
    fn resolve_binary_prefers_current_exe_for_out_of_tree_install() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        std::fs::write(bin_dir.join(managed_weepcode_bin_name()), "managed").unwrap();
    ...
    fn resolve_binary_falls_back_to_managed_when_no_current_exe() {
        ...
        let bin_dir = temp.path().join("bin");
        std::fs::create_dir_all(&bin_dir).unwrap();
        let managed = bin_dir.join(managed_weepcode_bin_name());
