...
/// two-function shape in `weepcode-tools` (F29).
...
/// `weepcode-tools` — existing items keep their prior content when
...
/// binary. Wraps a `weepcode_sampler::SamplingClient` and pulls the
/// text content out of the response.
pub struct SamplerClassifierClient {
    inner: weepcode_sampler::SamplingClient,
}

impl SamplerClassifierClient {
    pub fn new(inner: weepcode_sampler::SamplingClient) -> Self {
        Self { inner }
    }
}
...
pub fn build_classifier_request(
    ...
    ConversationRequest {
        ...
        x_weepcode_conv_id: session_id_str.clone(),
        x_weepcode_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
        x_weepcode_session_id: session_id_str,
        x_weepcode_agent_id: Some(weepcode_telemetry::id::agent_id()),
...
pub struct RunArgs {
    ...
    /// WeepCode-home directory containing the `auth.json` to consult as
    /// the third API-key fallback. Defaults to
    /// `weepcode_shell::util::weepcode_home::weepcode_home()` when `None`.
    /// Exposed as a CLI flag for testability.
    pub weepcode_home: Option<PathBuf>,
}
...
/// Resolve the API key from `--api-key` → `$WEEPCODE_API_KEY` →
...
/// provider command (if `WEEPCODE_AUTH_PROVIDER_COMMAND` is set) is
...
/// the user can act on.
pub async fn resolve_api_key(explicit: Option<&str>, weepcode_home: &Path) -> Result<String> {
    if let Some(k) = explicit.map(str::trim).filter(|s| !s.is_empty()) {
        return Ok(k.to_owned());
    }
    let env_key = std::env::var("WEEPCODE_API_KEY").ok();
    if let Some(k) = env_key.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
        return Ok(k.to_owned());
    }
    if let Some(key) = non_interactive_auth_key(weepcode_home).await? {
        return Ok(key);
    }
    Err(anyhow!(
        "no API key: pass --api-key, set WEEPCODE_API_KEY, or run `weepcode login` to populate \
         <weepcode-home>/auth.json. An expired OIDC token is auto-refreshed when a refresh_token \
         is present; if not, re-login is required."
    ))
}

/// Build an `AuthManager` for `weepcode_home`, install the
...
/// explicit `weepcode_home` so `--weepcode-home` overrides work in test
/// fixtures (the upstream helper hardcodes the global `weepcode_home()`
/// path).
///
/// Returns:
...
async fn non_interactive_auth_key(weepcode_home: &Path) -> Result<Option<String>> {
    use crate::auth::{AuthError, AuthManager, WeepcodeComConfig};
    ...
    let config = WeepcodeComConfig::default();
    let auth_provider_command = config.auth_provider_command.clone();
    let manager = std::sync::Arc::new(AuthManager::new(weepcode_home, config));
    ...
    match manager.auth().await {
        ...
        Err(e) => Err(anyhow!(
            "auth.json refresh failed: {e}. Run `weepcode login` to re-authenticate, \
             or pass --api-key / set $WEEPCODE_API_KEY to bypass auth.json."
        )),
    }
}
...
async fn build_sampler_client(
    ...
    model: String,
    api_key: Option<&str>,
    weepcode_home: Option<&Path>,
) -> Result<weepcode_sampler::SamplingClient> {
    let default_home;
    let weepcode_home_path: &Path = match weepcode_home {
        Some(p) => p,
        None => {
            default_home = crate::util::weepcode_home::weepcode_home();
            default_home.as_path()
        }
    };
    let resolved = resolve_api_key(api_key, weepcode_home_path).await?;
    let config = weepcode_sampler::SamplerConfig {
        api_key: Some(resolved),
        base_url,
        model,
        max_completion_tokens: Some(LAZINESS_MAX_OUTPUT_TOKENS),
        ..weepcode_sampler::SamplerConfig::default()
    };
    weepcode_sampler::SamplingClient::new(config).map_err(|e| anyhow!("build SamplingClient: {e}"))
}
...
mod tests {
    use super::*;
    use weepcode_sampling_types::{AssistantItem, ToolCall, ToolResultItem};
    ...
    async fn classifier_request_matches_production_shape() {
        ...
        assert_eq!(req.x_weepcode_conv_id.as_deref(), Some("sess-x"));
        assert_eq!(req.x_weepcode_session_id.as_deref(), Some("sess-x"));
        let req_id = req.x_weepcode_req_id.as_deref().expect("req id");
        ...
        assert_eq!(
            req.x_weepcode_agent_id.as_deref(),
            Some(weepcode_telemetry::id::agent_id().as_str())
        );
    }
    ...
    /// OIDC scope (the same scope `weepcode login` writes today and
    /// `AuthManager` reads). `auth_mode: api_key` skips the refresh
    /// path entirely — useful for "plain key, no refresh wanted"
    /// fixtures.
    fn write_auth_json(weepcode_home: &Path, key: &str) {
        let scope = crate::auth::WeepcodeComConfig::default().auth_scope();
        ...
        std::fs::write(weepcode_home.join("auth.json"), body.to_string()).expect("write auth.json");
    }
    ...
    fn write_empty_auth_json(weepcode_home: &Path) {
        std::fs::write(weepcode_home.join("auth.json"), "{}").expect("write auth.json");
    }
    ...
    fn write_fresh_oidc_auth_json(weepcode_home: &Path, key: &str) {
        let scope = crate::auth::WeepcodeComConfig::default().auth_scope();
        let body = serde_json::json!({
            scope: {
                "key": key,
    ...
    fn write_expired_oidc_auth_json_no_refresh(weepcode_home: &Path, key: &str) {
        let scope = crate::auth::WeepcodeComConfig::default().auth_scope();
        let body = serde_json::json!({
            scope: {
                "key": key,
    ...
    async fn resolve_api_key_precedence() {
        ...
        assert!(
            resolve_api_key(None, weepcode_home).await.is_err(),
            "no flag, no env, no auth.json → error",
        );
        assert!(
            resolve_api_key(Some(""), weepcode_home).await.is_err(),
            "empty flag → error",
        );
        assert!(
            resolve_api_key(Some("   "), weepcode_home).await.is_err(),
        ...
        assert_eq!(
            resolve_api_key(Some("from-flag"), weepcode_home)
                .await
                .expect("ok"),
            "from-flag",
        );

        unsafe { std::env::set_var("WEEPCODE_API_KEY", "from-env") };
        assert_eq!(
            resolve_api_key(None, weepcode_home).await.expect("ok"),
            "from-env"
        );
        assert_eq!(
            resolve_api_key(Some("from-flag"), weepcode_home)
        ...
        assert_eq!(
            resolve_api_key(Some(""), weepcode_home).await.expect("ok"),
            "from-env",
            "empty flag falls through to env",
        );
        assert_eq!(
            resolve_api_key(Some("   "), weepcode_home)
                .await
                .expect("ok"),
            "from-env",
        ...
        unsafe { std::env::set_var("WEEPCODE_API_KEY", "   ") };
        assert!(
            resolve_api_key(None, weepcode_home).await.is_err(),
    ...
    async fn resolve_api_key_falls_back_to_auth_json() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let weepcode_home = tmp.path();
        write_auth_json(weepcode_home, "from-auth-json");
        // `with_no_env` returns sync; we re-implement the clearing
        // inline so we can `.await` the resolver.
        let key =
            with_env_isolated(async { resolve_api_key(None, weepcode_home).await.expect("ok") })
                .await;
    ...
    async fn resolve_api_key_skips_auth_json_when_env_provided() {
        ...
        let key = with_env_isolated(async {
            unsafe { std::env::set_var("WEEPCODE_API_KEY", "from-env") };
            let resolved = resolve_api_key(None, weepcode_home).await.expect("ok");
    ...
    async fn resolve_api_key_errors_when_all_three_sources_missing() {
        ...
        let msg = with_env_isolated(async {
            let err = resolve_api_key(None, weepcode_home)
                .await
        ...
        assert!(
            msg.contains("--api-key")
                && msg.contains("WEEPCODE_API_KEY")
                && msg.contains("weepcode login")
    ...
    async fn resolve_api_key_skips_empty_auth_json_entry() {
        ...
        // the unified error.
        write_auth_json(weepcode_home, "");
        let err1 = with_env_isolated(async { resolve_api_key(None, weepcode_home).await }).await;
        assert!(err1.is_err(), "empty auth.json entry is treated as absent");

        write_auth_json(weepcode_home, "   ");
        let err2 = with_env_isolated(async { resolve_api_key(None, weepcode_home).await }).await;
        assert!(
            err2.is_err(),
    ...
    async fn resolve_api_key_returns_refreshable_oidc_token() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let weepcode_home = tmp.path();
        write_fresh_oidc_auth_json(weepcode_home, "fresh-oidc-key");
        let key =
            with_env_isolated(async { resolve_api_key(None, weepcode_home).await.expect("ok") })
                .await;
    ...
    async fn resolve_api_key_returns_error_when_oidc_token_expired_and_no_refresh_token() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let weepcode_home = tmp.path();
        write_expired_oidc_auth_json_no_refresh(weepcode_home, "stale-oidc-key");
        let err = with_env_isolated(async { resolve_api_key(None, weepcode_home).await }).await;
        assert!(
            err.is_err(),
    ...
    /// * `WEEPCODE_API_KEY` — would return early from `resolve_api_key`.
    /// * `WEEPCODE_AUTH` — inline-JSON credentials override that bypasses
    ///   the on-disk read entirely (`AuthManager::new`).
    /// * `WEEPCODE_AUTH_PATH` — overrides the auth.json path; if set to
    ///   the operator's real `~/.weepcode/auth.json`, the test would read
    ///   live OIDC credentials instead of the scratch fixture.
    /// * `WEEPCODE_AUTH_PROVIDER_COMMAND` — selects an external
    ...
    const ISOLATED_ENV_KEYS: &[&str] = &[
        ...
        "WEEPCODE_AUTH",
        "WEEPCODE_AUTH_PATH",
        "WEEPCODE_AUTH_PROVIDER_COMMAND",
    ];
    ...
    async fn run_rejects_missing_trace_path() {
        let prev = std::env::var("WEEPCODE_API_KEY").ok();
        unsafe { std::env::remove_var("WEEPCODE_API_KEY") };
        let tmp = tempfile::tempdir().expect("tempdir");
        let args = RunArgs {
            ...
            min_confidence: None,
            include_reasoning: None,
            weepcode_home: Some(tmp.path().to_path_buf()),
        };
        ...
        match prev {
            Some(v) => unsafe { std::env::set_var("WEEPCODE_API_KEY", v) },
            None => unsafe { std::env::remove_var("WEEPCODE_API_KEY") },
        }
    }
