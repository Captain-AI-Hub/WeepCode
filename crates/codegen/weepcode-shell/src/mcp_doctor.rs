//! `weepcode mcp doctor` -- runtime health check for MCP servers.
...
fn discover_servers(cwd: &Path) -> (Vec<ConfigSourceStatus>, Vec<DiscoveredServer>) {
    ...
    let user_config = weepcode_home.join("config.toml");
    if user_config.is_file() {
        sources.push(ConfigSourceStatus {
            path: "~/.weepcode/config.toml".to_string(),
...
/// Discover managed `weepcode_com_*` servers if the user has WeepCode auth on disk.
async fn try_discover_managed_servers() -> (ConfigSourceStatus, Vec<DiscoveredServer>) {
    ...
    if !snapshot.is_managed_mcp_eligible() {
        return managed_skipped(format!("{:?} auth (not WeepCode OIDC)", snapshot.auth_mode));
    }

    let token = match auth_manager.get_valid_token().await {
        Ok(key) => key,
        Err(_) => return managed_skipped("auth expired — run `weepcode login`"),
    };
...
fn resolve_command(command: &str) -> Option<String> {
    ...
    weepcode_tools::util::detach_std_command(&mut cmd);
    cmd.output()
        .ok()
...
async fn check_tools_list(service: &mcp_servers::McpService) -> Check {
    use weepcode_mcp::rmcp::model::PaginatedRequestParams;
    match service
...
pub async fn run_doctor(cwd: &Path, name_filter: Option<&str>) -> DoctorReport {
    ...
    // Folder-trust gate: `weepcode mcp doctor` actually STARTS each server
    ...
    //
    // `remote = None` is intentional: standalone `weepcode mcp doctor` has no loaded
...
pub fn print_report(report: &DoctorReport) {
    ...
    if report.servers.is_empty() {
        ...
        println!("  Run `weepcode mcp add --help` to get started.");
        println!();
        return;
    }
    ...
    println!(
        ...
        if report.failing_count > 0 {
            " Run `weepcode mcp doctor --json` for full diagnostics."
        } else {
            ""
        }
    );
