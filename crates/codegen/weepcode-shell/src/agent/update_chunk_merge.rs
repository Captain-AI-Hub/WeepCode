    ...
    fn merge(
        ...
        match (prev, new) {
            ...
            (Some(SessionNotification::Xai(prev)), SessionNotification::Xai(new)) => {
                let (force, first, second) = merge_weepcode_chunks(*prev, *new);
                (
                    force,
...
/// Merge two consecutive WeepCode notifications.
fn merge_weepcode_chunks(
...
mod tests {
    ...
    #[test]
    fn weepcode_same_id_deltas_merge_args_and_preserve_name() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
    ...
    #[test]
    fn weepcode_different_tool_call_id_forces_flush() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
    ...
    #[test]
    fn weepcode_tool_call_delta_is_bufferable() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
    ...
    #[test]
    fn weepcode_after_acp_forces_flush() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        let acp_chunk = msg_chunk("s", 1, "thinking...");
        let weepcode_chunk = delta_chunk("s", Some("call_1"), 0, Some("bash"), Some("args"));

        assert!(buf.consume_chunk(acp_chunk).is_none());
        let (flushed, rest) = buf
            .consume_chunk(weepcode_chunk)
