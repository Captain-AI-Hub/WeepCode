...
/// ~54 min, matching weepcode-web's refetch cadence.
...
pub const WEEPCODE_CHAT_MODE_ENV: &str = "WEEPCODE_CHAT_MODE";
...
mod tests {
    ...
    fn name_falls_back_to_id_when_title_blank() {
        let mut m = available("weepcode-4.5", "");
        m.title = "   ".to_owned();
        let resp = ListModesResponse {
            modes: vec![m],
            default_mode_id: String::new(),
        ...
        let state = modes_to_model_state(&resp);
        assert_eq!(state.available_models[0].name, "weepcode-4.5");
    }
}
