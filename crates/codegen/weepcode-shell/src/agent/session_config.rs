...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct WeepcodeSessionDetail {
...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        ...
        assert_eq!(selected_models[0].id, "weepcode-build");
    ...
    fn model_label_falls_back_to_id_when_name_empty() {
        let models = [model("weepcode-build", "")];
        let current = acp::ModelId::from("weepcode-build");
        let opts = build_session_config_options(&models, &current, &[], None);
        assert_eq!(opts[0].label, "weepcode-build");
    }
    ...
    #[test]
    fn weepcode_session_detail_serializes_camel_case() {
        let detail = WeepcodeSessionDetail::build(
            "sess-1".to_string(),
        ...
        assert_eq!(v["currentModelId"], "weepcode-build");
