...
//! inside a cloned repository (`.mcp.json`, project `.weepcode/config.toml`,
//! `~/.claude.json` `projects.<cwd>`, project `.weepcode/lsp.json`). Those configs
...
//! interactive prompt, and the durable [`weepcode_workspace::trust::TrustStore`]
//! reads/writes — lives in `weepcode-workspace` (client-side); this module keeps
...
//! on [`weepcode_workspace::folder_trust::decide`]; the consume-side nuance is
...
use weepcode_workspace::trust::{TrustStore, is_unsafe_trust_root, workspace_key};

// Decision-side (scan/decide/prompt/store) relocated to `weepcode-workspace`
// (client crate). `grant_folder_trust` is the ONLY moved item referenced from
// OUTSIDE this module (shell call sites + the pager's
// `weepcode_shell::agent::folder_trust::grant_folder_trust`), so only it is
...
pub use weepcode_workspace::folder_trust::grant_folder_trust;
use weepcode_workspace::folder_trust::{
...
// NOTE: this folder-trust store (`~/.weepcode/trusted_folders.toml`) is SEPARATE
// from the pre-existing per-plugin trust store
// (`weepcode_agent::plugins::TrustStore` at `~/.weepcode/trusted-plugins`, plus the
...
/// [`weepcode_workspace::folder_trust::revoke_folder_trust_store`].
///
...
pub fn revoke_folder_trust(cwd: &Path) -> bool {
    ...
    let was_trusted = weepcode_workspace::folder_trust::revoke_folder_trust_store(cwd);
...
/// ([`weepcode_workspace::folder_trust::repo_config_kinds`] /
...
/// folder is gated (so an `.weepcode/lsp.json`-only repo still has a non-empty reason
...
pub(crate) fn detected_config_kinds(cwd: &Path) -> Vec<String> {
    weepcode_workspace::folder_trust::repo_config_kinds(cwd)
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
pub(crate) fn agent_inline_hooks_allowed(
    scope: weepcode_agent::config::AgentScope,
    trusted: impl FnOnce() -> bool,
) -> bool {
    scope != weepcode_agent::config::AgentScope::Project || trusted()
}
...
/// whose cwd differs from the launch dir, `weepcode mcp doctor`) passes `false`, so
...
///   `store_trusted`: a `weepcode --trust` grant issued AFTER this workspace was
...
/// (session merge, the session-less agent pool, `weepcode mcp doctor`). It MUST
...
///
/// Sources: project `.weepcode/config.toml [mcp_servers]` (NOT the user-tier global
...
/// `~/.weepcode/config.toml` is dropped when untrusted. This is intended — untrusted
...
pub fn project_scoped_mcp_names(cwd: &Path) -> HashSet<String> {
    ...
    // `.weepcode/config.toml [mcp_servers]` entries tagged project (the loader's key
...
pub fn filter_untrusted_project_lsp(
    ...
    sourced: std::collections::BTreeMap<
        String,
        (
            weepcode_tools::implementations::lsp::config::LspServerConfig,
            weepcode_tools::types::config_source::ConfigSource,
        ),
    >,
) -> std::collections::BTreeMap<String, weepcode_tools::implementations::lsp::config::LspServerConfig>
{
    weepcode_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
        sourced,
...
mod tests {
    ...
    use weepcode_workspace::folder_trust::repo_configs_present;
    ...
    fn simulate_release_build() -> EnvGuard {
        EnvGuard::set(weepcode_version::TEST_VERSION_ENV, "0.0.0-sim")
    }
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; WEEPCODE_HOME-isolated so the seed can't touch the real user file and
        // `#[serial]` because WEEPCODE_HOME is process-global.
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. WEEPCODE_HOME-isolated so the grant writes to a temp
        // store; `#[serial]` because WEEPCODE_HOME is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. WEEPCODE_HOME-isolated so the grant writes to a temp store;
        // `#[serial]` because WEEPCODE_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
    ...
    fn revoke_on_unrecordable_home_root_records_no_deny() {
        ...
        // home; WEEPCODE_HOME-isolated store; WEEPCODE_FOLDER_TRUST unset so the
        ...
        std::fs::create_dir_all(home.path().join(".weepcode").join("hooks")).unwrap();
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // actually reads `.envrc`. WEEPCODE_HOME-isolated so the trust store is empty;
        // WEEPCODE_FOLDER_TRUST unset so the default-on feature flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
        let _flag = EnvGuard::unset("WEEPCODE_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        let env = weepcode_workspace::envrc::load_envrc_or_empty(tmp.path());
        assert_eq!(
            env.get("GATED_ENVRC"),
            Some(&"1".to_string()),
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // folder merges it. WEEPCODE_HOME-isolated so the trust store is empty;
        // WEEPCODE_FOLDER_TRUST unset so the default-on feature flag applies.
        use weepcode_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
        let _flag = EnvGuard::unset("WEEPCODE_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). WEEPCODE_HOME-isolated so the trust store is empty.
        use weepcode_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
        let _flag = EnvGuard::unset("WEEPCODE_FOLDER_TRUST");
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        // predicate used at mvp_agent/subagent. WEEPCODE_HOME-isolated (empty store).
        use weepcode_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
        let _flag = EnvGuard::unset("WEEPCODE_FOLDER_TRUST");
        let tmp = repo_tmp();
        let agents = tmp.path().join(".weepcode").join("agents");
        ...
        .unwrap();
        let def = weepcode_agent::discovery::by_name_in_cwd("explore", tmp.path())
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // code-exec config (here `.weepcode/hooks`), and no store grant must be
        // DENIED — even though no verdict was recorded first (the gate re-resolves
        // fail-closed rather than defaulting open). WEEPCODE_HOME-isolated (empty
        // store); WEEPCODE_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
        let _flag = EnvGuard::unset("WEEPCODE_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".weepcode").join("hooks")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + WEEPCODE_HOME-isolated so the
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // configs present. WEEPCODE_HOME-isolated so the seeded store is the temp one;
        // WEEPCODE_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
        let _flag = EnvGuard::unset("WEEPCODE_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        // WEEPCODE_TEST_VERSION unset so `is_local_build()` is genuinely true.
        let _unset_ver = EnvGuard::unset(weepcode_version::TEST_VERSION_ENV);
        if option_env!("WEEPCODE_VERSION").is_some() {
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.weepcode/plugins/<x>/`, no
        ...
        // discover_plugins/build_for_cwd/reload. WEEPCODE_HOME-isolated (empty store);
        // WEEPCODE_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
        let _flag = EnvGuard::unset("WEEPCODE_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".weepcode").join("plugins").join("evil")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        // WEEPCODE_HOME-isolated (empty store); WEEPCODE_FOLDER_TRUST unset so the kill-switch
        // is the only signal.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
        ...
        let cold = repo_tmp();
        std::fs::create_dir_all(cold.path().join(".weepcode").join("hooks")).unwrap();
        assert!(
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // un-gating here.
        //
        // WEEPCODE_HOME-isolated so both the folder-trust store and the plugin trust
        // store start empty (deterministic untrusted); WEEPCODE_FOLDER_TRUST unset so
        // the default-on flag applies; `#[serial]` because both are process-global.
        use weepcode_agent::plugins::discovery::DiscoveryConfig;
        use weepcode_agent::plugins::{PluginRegistry, SharedPluginRegistryHandle};
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
        let _flag = EnvGuard::unset("WEEPCODE_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // WEEPCODE_HOME-isolated so the grant writes to a temp store; WEEPCODE_FOLDER_TRUST unset
        // so the default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
        let _flag = EnvGuard::unset("WEEPCODE_FOLDER_TRUST");
        let tmp = repo_tmp();
        let hooks_dir = tmp.path().join(".weepcode").join("hooks");
        ...
        let has_project_hook = |reg: &weepcode_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
        ...
        let git_root = weepcode_workspace::session::git::find_git_root_from_path(tmp.path()).ok();
        let (reg, _errs) = crate::util::hooks::discover_hooks(
            git_root.as_deref(),
            &weepcode_tools::types::compat::CompatConfig::default(),
            untrusted,
        );
        assert!(
    ...
    fn filter_untrusted_project_lsp_drops_only_project() {
        ...
        use weepcode_tools::implementations::lsp::config::LspServerConfig;
        use weepcode_tools::types::config_source::ConfigSource;
            ...
            m.insert(
                ...
                (
                    ...
                    ConfigSource::Project {
                        path: PathBuf::from("/repo/.weepcode/lsp.json"),
                    },
                ),
            );
            m.insert(
                ...
                (
                    LspServerConfig::default(),
                    ConfigSource::User {
                        path: PathBuf::from("/home/.weepcode/lsp.json"),
                    },
                ),
            );
            m
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        use weepcode_tools::implementations::lsp::config::load_servers_with_plugins_sourced;
        use weepcode_tools::types::config_source::ConfigSource;

        // A `<cwd>/.weepcode/lsp.json` server must be tagged `Project` so the gate
        // can distinguish it from user/plugin servers. Asserts on the specific
        // key, so any real `~/.weepcode/lsp.json` on the test host is irrelevant.
        let tmp = repo_tmp();
        let weepcode = tmp.path().join(".weepcode");
        std::fs::create_dir_all(&weepcode).unwrap();
        std::fs::write(
            weepcode.join("lsp.json"),
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // from `<cwd>/.weepcode/lsp.json` is dropped once the workspace is untrusted.
        let tmp = repo_tmp();
        let weepcode = tmp.path().join(".weepcode");
        std::fs::create_dir_all(&weepcode).unwrap();
        std::fs::write(
    ...
    /// (`.mcp.json`) and `projtoml` (`.weepcode/config.toml [mcp_servers]`).
    ...
    /// `.weepcode/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in
    ...
    fn cached_untrusted_is_upgraded_by_a_later_grant() {
        ...
        record(&key, false);
        assert!(!project_scope_allowed(tmp.path()));
        // Simulate a `weepcode --trust` grant landing in the store after the
    ...
    fn explicit_child_untrust_survives_reload_despite_ancestor_trust() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join(weepcode_workspace::trust::TRUST_FILE_NAME);
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        unsafe { std::env::set_var("WEEPCODE_FOLDER_TRUST", "1") };
        // Simulate a release-stamped build: an unstamped local/dev build (as in CI,
        // no WEEPCODE_VERSION) auto-trusts, so the gate would never engage without this.
        unsafe { std::env::set_var(weepcode_version::TEST_VERSION_ENV, "0.0.0-sim") };
        let tmp = repo_tmp();
        ...
        unsafe { std::env::remove_var(weepcode_version::TEST_VERSION_ENV) };
        unsafe { std::env::remove_var("WEEPCODE_FOLDER_TRUST") };
    }
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // WEEPCODE_HOME so the store is empty/seeded in temp. `#[serial]` because both
        // vars are process-global.
        let _feature = EnvGuard::set("WEEPCODE_FOLDER_TRUST", "1");
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
        ...
        let untrusted = repo_tmp();
        std::fs::create_dir_all(untrusted.path().join(".weepcode").join("hooks")).unwrap();
        ...
        let trusted = repo_tmp();
        std::fs::create_dir_all(trusted.path().join(".weepcode").join("hooks")).unwrap();
        let mut store = TrustStore::load();
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // `is_local_build_honors_test_version_override`), with WEEPCODE_TEST_VERSION
        // unset so `is_local_build()` is genuinely true. WEEPCODE_HOME-isolated so the
        // real store is never touched.
        let _sim = EnvGuard::unset(weepcode_version::TEST_VERSION_ENV);
        if option_env!("WEEPCODE_VERSION").is_some() {
            return; // a release-stamped test binary is not a local build
        }
        let home = tempfile::tempdir().unwrap();
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // => the GUI prompt is warranted. WEEPCODE_HOME-isolated so the store starts
        // empty; `#[serial]` because WEEPCODE_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
        let tmp = repo_tmp();
        ...
        // WEEPCODE_FOLDER_TRUST unset: env outranks the remote flag, so an ambient
        // opt-out would otherwise false-fail the Prompt assertion.
        let _sim = EnvGuard::set(weepcode_version::TEST_VERSION_ENV, "0.0.0-sim");
        let _flag = EnvGuard::unset("WEEPCODE_FOLDER_TRUST");
        let remote = RemoteSettings {
    ...
    fn prompt_warranted_false_when_feature_disabled() {
        ...
        // local-build path is not what's under test; WEEPCODE_HOME-isolated and
        // WEEPCODE_FOLDER_TRUST unset so the kill-switch is the only signal.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
        let _flag = EnvGuard::unset("WEEPCODE_FOLDER_TRUST");
        let _sim = simulate_release_build();
    ...
    fn detected_config_kinds_summarizes_present_markers() {
        ...
        std::fs::write(tmp.path().join(".mcp.json"), "{}").unwrap();
        std::fs::create_dir_all(tmp.path().join(".weepcode").join("hooks")).unwrap();
        std::fs::write(tmp.path().join(".weepcode").join("lsp.json"), "{}").unwrap();
    ...
    fn detected_config_kinds_reports_lsp_only_repo() {
        ...
        // `.weepcode/lsp.json` must still produce a non-empty reason list.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".weepcode")).unwrap();
        std::fs::write(tmp.path().join(".weepcode").join("lsp.json"), "{}").unwrap();
        let kinds = detected_config_kinds(tmp.path());
