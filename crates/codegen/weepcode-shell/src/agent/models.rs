...
use crate::auth::{AuthManager, WeepcodeAuth, WeepcodeComConfig};
use crate::remote::{FetchModelsResult, fetch_models_blocking};
use crate::sampling::SamplerConfig as SamplingConfig;
use globset::{Glob, GlobSet, GlobSetBuilder};
use weepcode_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
impl ModelFetchAuth {
    ...
    ///
    /// A `deployment_key` outranks an ambient `WEEPCODE_API_KEY` so a stray env key
    ...
    pub(crate) fn resolve(endpoints: &config::EndpointsConfig, has_cached_session: bool) -> Self {
        if endpoints.has_custom_endpoint() {
            ...
            Self::Deployment
        } else if crate::agent::auth_method::has_weepcode_api_key_env() {
            Self::ApiKey
        } else {
            Self::Session
        }
...
struct Inner {
    ...
    gateway: RwLock<Option<weepcode_acp_lib::AcpAgentGatewaySender>>,
...
impl ModelsManager {
    ...
    pub(crate) fn set_gateway(&self, gateway: weepcode_acp_lib::AcpAgentGatewaySender) {
    ...
    pub fn model_compactions_remaining(
        &self,
        model_id: &str,
    ) -> Option<weepcode_sampling_types::CompactionsRemaining> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    pub fn model_compaction_at_tokens(
        &self,
        model_id: &str,
    ) -> Option<weepcode_sampling_types::CompactionAtTokens> {
        self.inner
            .models
            .read()
            .get(model_id)
    ...
    ///
    /// `model_id` may be a routing slug (`config.model`, e.g. `weepcode-4.5`)
    /// OR a catalog key; the catalog map is keyed by the config key, which can
    /// differ from the slug for custom/enterprise ids (e.g. key `enterprise-weepcode-build`
    /// → slug `weepcode-4.5`). Resolve to the catalog key first so a slug
    ...
    /// Hot-reload the catalog from `~/.weepcode/models_cache.json` after an
    ...
    /// When another weepcode process sharing `~/.weepcode` (a `--no-leader` run, a
    /// newer client, weepcode-desktop) fetches a fresher `/v1/models` catalog and
    ...
    /// `ModelsCacheManager` path is fixed to `weepcode_home()`, a process-wide
    /// `OnceLock`).
...
struct ModelsCache {
    ...
    weepcode_version: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl ModelsCacheManager {
    fn new() -> Self {
        Self {
            path: crate::util::weepcode_home::weepcode_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    fn load_fresh(
        ...
        if cache.weepcode_version.as_deref() != Some(weepcode_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            weepcode_version: Some(weepcode_version::VERSION.to_string()),
    ...
    match fetch_models_blocking(endpoints, auth, fetch_auth) {
        Ok(FetchModelsResult { models, etag }) if !models.is_empty() => {
            let api_base_url_override = match fetch_auth {
                ModelFetchAuth::ApiKey => Some(endpoints.weepcode_api_base_url.clone()),
                _ => None,
            };
...
fn resolve_prefetch_env_with_auth(auth: Option<WeepcodeAuth>) -> Option<PrefetchEnv> {
...
/// explicit off switch must hold even when a stray login, `WEEPCODE_API_KEY`, or
...
fn resolve_prefetch_env(weepcode_com_config: Option<WeepcodeComConfig>) -> Option<PrefetchEnv> {
    let weepcode_home = crate::util::weepcode_home::weepcode_home();
    let auth_manager = AuthManager::new(&weepcode_home, weepcode_com_config.unwrap_or_default());
...
pub fn start_early_prefetch_with_auth(auth: Option<WeepcodeAuth>) -> Option<EarlyPrefetchHandle> {
...
pub fn start_early_prefetch(
    weepcode_com_config: Option<WeepcodeComConfig>,
) -> Option<EarlyPrefetchHandle> {
    let env = resolve_prefetch_env(weepcode_com_config)?;
    Some(spawn_prefetch_thread(env))
}
...
///
/// Sessions persist the routing slug (`[model.X].model`, e.g. `weepcode-4.5`);
/// the catalog and `/model` picker use config keys (e.g. `enterprise-weepcode-build`).
...
pub fn resolve_model_catalog(
    ...
    // must not stamp `none` onto weepcode-4.5, which only offers low/medium/high).
...
mod tests {
    ...
    fn test_manager() -> ModelsManager {
        ...
        let tmp = std::env::temp_dir().join("weepcode-test-models-manager");
        let auth_manager = Arc::new(AuthManager::new(&tmp, WeepcodeComConfig::default()));
        ModelsManager::new(
            None,
            IndexMap::new(),
            acp::ModelId::new("default"),
    ...
    async fn set_current_model_id_change_fires_watch_to_all_subscribers() {
        ...
        mgr.set_current_model_id(acp::ModelId::new("weepcode-4"));
    ...
    async fn model_switch_generation_snapshot_reflects_current_state() {
        ...
        mgr.set_current_model_id(acp::ModelId::new("weepcode-3"));
    ...
    fn current_reasoning_effort_seeded_from_config() {
        let tmp = std::env::temp_dir().join("weepcode-test-models-manager-seed");
        let auth_manager = Arc::new(AuthManager::new(&tmp, WeepcodeComConfig::default()));
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        use indexmap::IndexMap;
        use weepcode_sampling_types::ReasoningEffortOption;
        ...
        let mut no_none = ModelEntry {
            info: config::ModelInfo::fallback("weepcode-4.5"),
            api_key: None,
            env_key: None,
            api_base_url: None,
        };
        ...
        prefetched.insert("weepcode-4.5".to_string(), no_none);
        ...
        let catalog = resolve_model_catalog(&cfg, Some(prefetched));
        assert_eq!(
            catalog["weepcode-4.5"].info.reasoning_effort,
            Some(ReasoningEffort::High),
    ...
    fn config_menu_only_model_derives_support_and_default() {
        ...
        let tmp = std::env::temp_dir().join("weepcode-test-models-manager-menu-only");
        let auth_manager = Arc::new(AuthManager::new(&tmp, WeepcodeComConfig::default()));
        let mgr = ModelsManager::new(
            None,
            catalog,
    ...
    fn first_apply_refresh_reselects_default_model() {
        let mgr = test_manager();
        let mut cfg = config::Config::default();
        cfg.models.default = Some("weepcode-3".to_string());

        assert!(!mgr.has_fetched_real_catalog());

        let prefetched = make_prefetched(&["weepcode-3", "weepcode-4"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);

        assert!(mgr.has_fetched_real_catalog());
        assert_eq!(mgr.current_model_id().0.as_ref(), "weepcode-3");
    }
    ...
    fn subsequent_refresh_reselects_when_model_removed() {
        ...
        // Second refresh with weepcode-4 removed.
        let prefetched = make_prefetched(&["weepcode-3", "weepcode-4.5"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
    ...
    fn apply_config_honors_new_preferred_model() {
        ...
        *mgr.inner.cfg.write() = stale_cfg;
        ...
        new_cfg.models.default = Some("weepcode-3".to_string());
    ...
    fn apply_config_falls_back_when_preferred_not_in_catalog() {
        ...
        let mut new_cfg = config::Config::default();
        new_cfg.models.default = Some("weepcode-nonexistent".to_string());
        mgr.apply_config(new_cfg);
    ...
    fn auth_refresh_then_config_reload_preserves_user_model() {
        ...
        *mgr.inner.etag.write() = None;
        ...
        let prefetched = make_prefetched(&["weepcode-3", "weepcode-4"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
        assert_eq!(mgr.current_model_id().0.as_ref(), "weepcode-4");

        // Config reload with persisted preference.
        let mut new_cfg = config::Config::default();
        new_cfg.models.default = Some("weepcode-4".to_string());
        mgr.apply_config(new_cfg);
        assert_eq!(mgr.current_model_id().0.as_ref(), "weepcode-4");
    }
    ...
    fn reload_from_disk_cache_applies_external_catalog() {
        ...
        cache.persist(
            &make_prefetched(&["weepcode-4.5", "weepcode-4.3"]),
            Some("etag-ext"),
            auth_method,
            &mgr.cache_origin(),
        );
        ...
        assert!(mgr.has_fetched_real_catalog());
        assert!(mgr.models().contains_key("weepcode-4.5"));
        assert!(mgr.models().contains_key("weepcode-4.3"));
    ...
    fn reload_from_disk_cache_ignores_stale_cache() {
        ...
        let stale = ModelsCache {
            ...
            models: make_prefetched(&["weepcode-stale"]),
        };
        cache.atomic_write(&stale);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("weepcode-stale"));
    ...
    fn reload_from_disk_cache_ignores_auth_method_mismatch() {
        ...
        cache.persist(
            &make_prefetched(&["weepcode-other-auth"]),
            Some("etag-x"),
            other,
            &mgr.cache_origin(),
        );
        ...
        assert!(!mgr.models().contains_key("weepcode-other-auth"));
    }
    ...
    fn reload_from_disk_cache_ignores_origin_mismatch() {
        ...
        let auth_method = mgr.inner.fetch_auth.read().cache_auth_method();
        cache.persist(
            &make_prefetched(&["weepcode-other-origin"]),
            Some("etag-y"),
            auth_method,
        ...
        assert!(!mgr.models().contains_key("weepcode-other-origin"));
        assert!(mgr.inner.etag.read().is_none());
    }
    ...
    fn reload_from_disk_cache_ignores_legacy_cache_without_origin() {
        ...
        let legacy = ModelsCache {
            ...
            models: make_prefetched(&["weepcode-legacy"]),
        };
        cache.atomic_write(&legacy);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("weepcode-legacy"));
    }
    ...
    fn clear_resets_has_fetched_real_catalog() {
        ...
        let prefetched = make_prefetched(&["weepcode-4.5", "weepcode-4.3"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
        let first_available = mgr.available().keys().next().unwrap().clone();
        assert_eq!(
            mgr.current_model_id().0.as_ref(),
    ...
    fn resolve_custom_endpoint_always_wins() {
        let _key = EnvGuard::set("WEEPCODE_API_KEY", "test-key");
    ...
    fn resolve_falls_back_to_session_when_nothing_set() {
        let _unset = EnvGuard::unset("WEEPCODE_API_KEY");
        let _unset_legacy = EnvGuard::unset("WEEPCODE_CODE_WEEPCODE_API_KEY");
        let endpoints = config::EndpointsConfig::default();
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Session,
    ...
    /// `deployment_key` outranks a stray `WEEPCODE_API_KEY`, but session wins over both.
    #[test]
    #[serial]
    fn resolve_deployment_key_outranks_ambient_api_key() {
        let _key = EnvGuard::set("WEEPCODE_API_KEY", "stray-env-key");
        let endpoints = config::EndpointsConfig {
            deployment_key: Some("deploy-key".to_owned()),
            ..config::EndpointsConfig::default()
        };
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, false),
            ModelFetchAuth::Deployment,
            "managed deployment_key should outrank an ambient WEEPCODE_API_KEY",
        );
        assert_eq!(
            ModelFetchAuth::resolve(&endpoints, true),
            ModelFetchAuth::Session,
    ...
    /// once — session auth, ambient `WEEPCODE_API_KEY`, `deployment_key`, AND a
    ...
    fn prefetch_env_none_when_remote_fetch_disabled_despite_credentials() {
        ...
        assert!(
            resolve_prefetch_env_from_parts(
                Some(WeepcodeAuth::test_default()),
                endpoints.clone(),
                false,
            )
            .is_none(),
    ...
    fn build_prefetched_map_distinct_ids_same_slug() {
        let entries = vec![
            make_entry_config_with_id(Some("auto"), "weepcode-build", Some("Auto")),
        ...
        assert!(map.contains_key("weepcode-build"));
        assert!(map.contains_key("weepcode-composer-2.5-fast"));
        assert_eq!(
            map["auto"].info.model, "weepcode-build",
            "auto entry should still route to weepcode-build"
        );
        assert_eq!(map["weepcode-build"].info.model, "weepcode-build");
    }
    ...
    fn build_prefetched_map_duplicate_id_overwrites() {
        let entries = vec![
            make_entry_config_with_id(Some("weepcode-build"), "weepcode-build", Some("First")),
            make_entry_config_with_id(Some("weepcode-build"), "weepcode-build", Some("Second")),
        ];
        let map = build_prefetched_map(entries, None);

        assert_eq!(map.len(), 1, "duplicate id: second overwrites first");
        assert_eq!(map["weepcode-build"].info.name.as_deref(), Some("Second"));
    }
    ...
    fn resolve_default_model_prefers_id_over_model_slug() {
        ...
        catalog.insert(
            "auto-weepcode-build".to_string(),
            make_model_entry("weepcode-build"),
        );
        catalog.insert(
        ...
        let mut cfg = config::Config::default();
        cfg.models.default = Some("weepcode-build".to_string());

        let (key, _, _) = resolve_default_model(&cfg, &catalog, true);
        assert_eq!(key, "weepcode-build", "must match id, not first slug hit");
    }
    ...
    fn resolve_catalog_key_maps_routing_slug_to_config_key() {
        ...
        models.insert(
            "enterprise-weepcode-build".to_string(),
            make_model_entry("weepcode-4.5"),
        );
        models.insert("weepcode-4.3".to_string(), make_model_entry("weepcode-4.3"));

        let persisted = acp::ModelId::new("weepcode-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "enterprise-weepcode-build");
    }

    #[test]
    fn resolve_catalog_key_prefers_exact_key_match() {
        let mut models = IndexMap::new();
        models.insert("weepcode-4.5".to_string(), make_model_entry("weepcode-4.5"));

        let persisted = acp::ModelId::new("weepcode-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("exact key must resolve");
        assert_eq!(key.0.as_ref(), "weepcode-4.5");
    }
    ...
    fn resolve_catalog_key_last_slug_match_wins() {
        let mut models = IndexMap::new();
        models.insert(
            "default-weepcode-build".to_string(),
            make_model_entry("weepcode-4.5"),
        );
        models.insert(
            "user-weepcode-build".to_string(),
            make_model_entry("weepcode-4.5"),
        );

        let persisted = acp::ModelId::new("weepcode-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "user-weepcode-build");
    }
    ...
    fn selectable_prefers_available_identity_over_non_selectable_exact_key() {
        ...
        let available = test_available_keys(&["enterprise-weepcode-build", "weepcode-4.3"]);

        let persisted = acp::ModelId::new("weepcode-build");
        assert_eq!(
    ...
    fn selectable_prefers_exact_key_over_later_slug_match() {
        ...
        models.insert("other".to_string(), make_model_entry("weepcode-build"));

        let available = test_available_keys(&["weepcode-build", "other"]);

        let persisted = acp::ModelId::new("weepcode-build");
        let key = selectable_catalog_key_for_persisted(&models, &available, &persisted)
            .expect("exact selectable key must win");
        assert_eq!(key.0.as_ref(), "weepcode-build");
    }
