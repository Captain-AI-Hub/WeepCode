...
/// Env var that, when set, advertises `weepcode.api_key` as a viable auth method.
///
/// Kept as a constant so test code and the production check stay in sync.
pub const WEEPCODE_API_KEY_ENV_VAR: &str = "WEEPCODE_API_KEY";

/// Legacy env var name. Checked as a fallback when `WEEPCODE_API_KEY` is not set,
/// so existing deployments that use the old name keep working.
pub const LEGACY_WEEPCODE_API_KEY_ENV_VAR: &str = "WEEPCODE_CODE_WEEPCODE_API_KEY";

/// Read the API key from the environment.
///
/// Checks `WEEPCODE_API_KEY` first, then falls back to the legacy
/// `WEEPCODE_CODE_WEEPCODE_API_KEY` for backward compatibility.
pub fn read_weepcode_api_key_env() -> Result<String, std::env::VarError> {
    std::env::var(WEEPCODE_API_KEY_ENV_VAR)
        .or_else(|_| std::env::var(LEGACY_WEEPCODE_API_KEY_ENV_VAR))
}

/// Returns `true` if either `WEEPCODE_API_KEY` or `WEEPCODE_CODE_WEEPCODE_API_KEY` is set.
pub fn has_weepcode_api_key_env() -> bool {
    read_weepcode_api_key_env().is_ok()
}

/// Whether `weepcode.api_key` should be advertised (and pushed FIRST) when building
/// the `auth_methods` list at `initialize()` time.
///
/// Regression: `weepcode.api_key` must stay first when only per-model credentials
/// exist (no global `WEEPCODE_API_KEY`). Deferring it made BYOK users hit the login
...
///
/// `disable_api_key_auth` (`[weepcode_com_config] disable_api_key_auth` /
/// `WEEPCODE_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
/// method is never advertised, regardless of available credentials, so
/// `WEEPCODE_API_KEY` can't bypass a deployment's forced IdP login.
pub fn should_advertise_weepcode_api_key<'a, I>(disable_api_key_auth: bool, models: I) -> bool
where
    I: IntoIterator<Item = &'a ModelEntry>,
{
    ...
    has_weepcode_api_key_env() || models.into_iter().any(ModelEntry::has_own_credentials)
}
...
pub struct AuthMethodsBuildInputs<'a> {
    /// True if `weepcode.api_key` should be advertised AT ALL. Caller computes via
    /// [`should_advertise_weepcode_api_key`]. When `preferred_method` is `Oidc`,
    ...
    /// WeepCode browser login (it appends `provider.setup` instead).
    ...
    /// True if `weepcode_com_config.auth_provider_command` is configured (sets
...
pub struct BuiltAuthMethods {
    ...
    /// `cached_token` wins over `weepcode.api_key` when both are present. When
...
/// `has_external_api_key` is true, the **first** entry MUST be `weepcode.api_key`.
...
/// 1. `weepcode.api_key`     (if `has_external_api_key`)
/// 2. `cached_token`    (if `has_cached_token`)
/// 3. `provider.setup`  (always)
///
/// WeepCode: the WeepCode browser-OAuth methods (`grok.com` / `oidc`) are no longer
/// advertised on the unpinned path. A user with no credentials is offered
/// in-TUI API-provider setup instead of an WeepCode login wall. Enterprise OIDC
...
/// - `weepcode.api_key`  else if `has_external_api_key`
/// - `None`         otherwise
///
/// Pinned (`preferred_method`):
/// - `ApiKey`: only `weepcode.api_key` if available; else empty list + `None` (fail).
/// - `Oidc`: `cached_token` (if any) + interactive login; never `weepcode.api_key`.
...
fn build_pinned_api_key(has_external_api_key: bool) -> BuiltAuthMethods {
    ...
    BuiltAuthMethods {
        methods: vec![weepcode_api_key_auth_method()],
        default_auth_method_id: Some(acp::AuthMethodId::new(WEEPCODE_API_KEY_METHOD_ID)),
    }
}
...
fn build_unpinned(has_external_api_key: bool, has_cached_token: bool) -> BuiltAuthMethods {
    ...
    if has_external_api_key {
        methods.push(weepcode_api_key_auth_method());
        default_auth_method_id = Some(acp::AuthMethodId::new(WEEPCODE_API_KEY_METHOD_ID));
    }

    if has_cached_token {
        methods.push(cached_token_auth_method());
        // cached_token wins over weepcode.api_key for default_auth_method_id so
        ...
        if overrode_api_key {
            weepcode_telemetry::unified_log::info(
                "auth method priority: cached_token overrides weepcode.api_key for default_auth_method_id",
                None,
...
/// advertises WeepCode browser login; it appends `provider.setup` instead.
fn push_interactive_login(
    ...
    if has_enterprise_oidc {
        ...
        // the same `cfg.weepcode_com_config.oidc` Option, so the inconsistent
        // `(true, None)` combination is a programmer error -- panic loudly
        // (matches the original `cfg.weepcode_com_config.oidc.as_ref().unwrap()`
        ...
        methods.push(weepcode_com_auth_method(
            login_label,
            has_auth_provider_command,
        ));
    }
...
impl AuthMethodKind {
    pub fn from_id(id: &acp::AuthMethodId) -> Self {
        match id.0.as_ref() {
            WEEPCODE_API_KEY_METHOD_ID => Self::XaiApiKey,
            CACHED_TOKEN_AUTH_METHOD_ID => Self::CachedToken,
            WEEPCODE_COM_METHOD_ID => Self::WeepcodeCom,
    ...
    pub fn is_session_based(self) -> bool {
        matches!(self, Self::CachedToken | Self::WeepcodeCom | Self::Oidc)
    }
    ...
    pub fn needs_interactive_login(self) -> bool {
        matches!(self, Self::WeepcodeCom | Self::Oidc | Self::ProviderSetup)
    }
...
/// `WEEPCODE_API_KEY` is set, which demoted live OIDC sessions to non-refreshable
...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `weepcode login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `weepcode login`, set WEEPCODE_API_KEY, or add api_key to ~/.weepcode/config.toml.";

/// Next ACP method id when `cached_token` cannot proceed (missing / expired /
/// legacy WebLogin), or `None` when fallthrough is forbidden.
///
/// Unpinned: prefer non-interactive `weepcode.api_key` when advertiseable, else
/// interactive `provider.setup` (WeepCode: in-TUI provider form, not WeepCode
/// browser OAuth).
///
...
pub const PREFERRED_API_KEY_UNAVAILABLE: &str = "preferred_method=api_key but no API key is configured (set WEEPCODE_API_KEY or model api_key/env_key in config.toml).";

/// Error when `preferred_method=oidc` but the session path cannot proceed.
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `weepcode login` to authenticate.";

pub const WEEPCODE_API_KEY_METHOD_ID: &str = "weepcode.api_key";
pub fn weepcode_api_key_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(
            acp::AuthMethodId::new(WEEPCODE_API_KEY_METHOD_ID),
            "weepcode.api_key".to_string(),
        )
        .description(Some(format!(
            "{WEEPCODE_API_KEY_ENV_VAR} or api_key/env_key in config.toml"
        ))),
    )
}
...
pub fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.weepcode/auth.json".to_string())),
    )
}

pub const WEEPCODE_COM_METHOD_ID: &str = "grok.com";

/// WeepCode OAuth2/OIDC auth. Method id `"grok.com"` kept for ACP wire-compat.
pub fn weepcode_com_auth_method(
    label: Option<&str>,
    has_auth_provider_command: bool,
) -> acp::AuthMethod {
    let name = label.unwrap_or("WeepCode");
    ...
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(
            acp::AuthMethodId::new(WEEPCODE_COM_METHOD_ID),
            name.to_string(),
        )
...
mod tests {
    ...
    /// `cached_token` to non-interactive `weepcode.api_key` (not browser OAuth).
    ...
    /// `weepcode.api_key` first, while `default_auth_method_id` prefers session;
    /// after session fails, this helper must still pick `weepcode.api_key`.
    #[test]
    ...
    /// (WeepCode: in-TUI provider form, not WeepCode browser OAuth).
    #[test]
    ...
    fn auth_method_kind_classifier_matrix() {
        ...
        let api_id = acp::AuthMethodId::new(WEEPCODE_API_KEY_METHOD_ID);
    ...
    // Reordering so `weepcode.api_key` is after login methods must fail the tests below.

    /// BYOK with only per-model `env_key` must list `weepcode.api_key` first.
    #[test]
    fn enterprise_byok_first_method_is_weepcode_api_key() {
        ...
        assert_eq!(
            ...
            "BYOK enterprise-style: auth_methods.first() MUST be weepcode.api_key \
        ...
        assert!(
            ...
            "first method MUST NOT need interactive login when weepcode.api_key is available",
        );
    }

    /// BYOK + cached session token: weepcode.api_key stays first in the methods
    /// list (skips login screen), but `default_auth_method_id` is
    /// `cached_token` (keeps OIDC refresh alive).
    #[test]
    fn byok_with_cached_token_keeps_weepcode_api_key_first() {
        let inputs = AuthMethodsBuildInputs {
        ...
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::XaiApiKey),
            "weepcode.api_key MUST precede cached_token in advertised order",
        );
    ...
    /// Unpinned path no longer advertises WeepCode browser login: with enterprise
    ...
    fn unpinned_enterprise_oidc_not_advertised_provider_setup_appended() {
        ...
        assert!(
            !built
                .methods
                .iter()
                .any(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::WeepcodeCom),
    ...
    /// path that still advertises WeepCode browser login). Pinning this here so the
    ...
    fn auth_provider_command_sets_external_provider_meta() {
        ...
        let weepcode = built
            .methods
            .iter()
            .find(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::WeepcodeCom)
            .expect("grok.com must be advertised on the pinned OIDC path");
        assert_eq!(weepcode.name(), "Acme Corp");
        let meta = weepcode.meta().expect("meta should be set");
        assert_eq!(
    ...
    /// `~/.weepcode/config.toml` skeleton from the bug report, walks it through
    /// the same predicate (`should_advertise_weepcode_api_key`) and the same
    /// list-builder (`build_auth_methods`) that `MvpAgent::initialize()` uses
    /// in production, and asserts that `auth_methods.first()` is `weepcode.api_key`
    /// (which causes the pager to skip the login screen).
    ///
    /// This is the test that *would have caught* that regression -- if you mentally
    /// re-introduce that bug (push weepcode.api_key LAST when has_external_api_key
    ...
    fn enterprise_byok_config_does_not_require_login() {
        ...
        let _global = EnvGuard::unset(WEEPCODE_API_KEY_ENV_VAR);
        ...
        // provider.setup method. Confirms the predicate isn't trivially true.
        {
            ...
            let has_external_api_key = should_advertise_weepcode_api_key(false, models.values());
            ...
            assert_ne!(
                first_kind(&built.methods),
                Some(AuthMethodKind::XaiApiKey),
                "without env_key resolved, weepcode.api_key must NOT be advertised first",
            );
        }

        // With the env var present (the actual enterprise scenario), the predicate
        // returns true and the builder MUST put `weepcode.api_key` first so the
        // pager's `startup_auth_metadata()` returns `needs_login = false`.
        {
            ...
            assert_eq!(
                first_kind(&built.methods),
                Some(AuthMethodKind::XaiApiKey),
                "BYOK: weepcode.api_key must be auth_methods.first(); deferred-to-last \
    ...
    /// `WEEPCODE_API_KEY` alone (no per-model creds) also triggers
    /// advertising `weepcode.api_key` as the first method. Historical "external
    /// key" path; covered here so the predicate keeps treating env-var-only
    /// users the same as per-model users.
    #[test]
    #[serial]
    fn global_external_api_key_advertises_weepcode_api_key_first() {
        let _set = EnvGuard::set(WEEPCODE_API_KEY_ENV_VAR, "weepcode-external-key");
    ...
    /// AND per-model env_key), so the builder never advertises `weepcode.api_key`
    /// and the pager sends the user to the deployment's login method instead.
    #[test]
    #[serial]
    fn disable_api_key_auth_suppresses_weepcode_api_key_method() {
        let _set = EnvGuard::set(WEEPCODE_API_KEY_ENV_VAR, "weepcode-external-key");
        let cfg = Config::default();
        let models = resolve_model_list(&cfg, None);

        // Flag off: today's behavior (advertised first).
        assert!(should_advertise_weepcode_api_key(false, models.values()));

        // Flag on: never advertised, regardless of credentials.
        let has_external_api_key = should_advertise_weepcode_api_key(true, models.values());
        assert!(!has_external_api_key);
        let built = build_auth_methods(AuthMethodsBuildInputs {
            has_external_api_key,
            ..default_inputs()
        ...
        assert!(
            ...
            "weepcode.api_key must not be advertised when disable_api_key_auth is set",
        );
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::ProviderSetup),
    ...
    /// Legacy `WEEPCODE_CODE_WEEPCODE_API_KEY` env var is accepted as a fallback
    /// when `WEEPCODE_API_KEY` is not set, ensuring existing deployments keep working.
    #[test]
    #[serial]
    fn legacy_env_var_fallback_advertises_weepcode_api_key() {
        let _unset_new = EnvGuard::unset(WEEPCODE_API_KEY_ENV_VAR);
        let _set_legacy = EnvGuard::set(LEGACY_WEEPCODE_API_KEY_ENV_VAR, "weepcode-legacy-key");
        assert!(has_weepcode_api_key_env());
        assert_eq!(read_weepcode_api_key_env().unwrap(), "weepcode-legacy-key");
    ...
    /// When both `WEEPCODE_API_KEY` and `WEEPCODE_CODE_WEEPCODE_API_KEY` are set,
    /// the new name takes precedence.
    #[test]
    #[serial]
    fn new_env_var_takes_precedence_over_legacy() {
        let _new = EnvGuard::set(WEEPCODE_API_KEY_ENV_VAR, "new-key");
        let _legacy = EnvGuard::set(LEGACY_WEEPCODE_API_KEY_ENV_VAR, "old-key");
        assert_eq!(read_weepcode_api_key_env().unwrap(), "new-key");
    }

    // -- weepcode login --legacy regression coverage ------------------------
    //
    // `weepcode login --legacy` produces a WeepcodeAuth with `auth_mode: WebLogin`,
    // `oidc_issuer: None`, and no `expires_at` (30-day hardcoded TTL).
    // When this token is present via the `WEEPCODE_AUTH` env var (or via legacy
    ...
    // to-end. A regression in WEEPCODE_AUTH JSON parsing or in auth method
    // ordering would send legacy-token users to the login screen.

    /// END-TO-END REGRESSION TEST: a legacy auth token (WebLogin, no
    /// expires_at) present in the `WEEPCODE_AUTH` env var, with no other auth
    ...
    #[test]
    #[serial]
    fn weepcode_login_legacy_token_does_not_require_login() {
        use crate::auth::{AuthManager, AuthMode, WeepcodeAuth, WeepcodeComConfig};

        // Ensure clean slate for "no other auth available".
        let _g1 = EnvGuard::unset("WEEPCODE_AUTH_PATH");
        let _g2 = EnvGuard::unset(WEEPCODE_API_KEY_ENV_VAR);

        // Construct a legacy-style token exactly as `weepcode login --legacy`
        // produces: WebLogin mode, no OIDC fields, no refresh_token, no
        // expires_at (is_expired falls back to 30-day age check).
        let legacy_token = WeepcodeAuth {
            ...
            refresh_token: None,
            expires_at: None,
            ..WeepcodeAuth::test_default()
        };

        // Provide it via WEEPCODE_AUTH env var (highest priority code path in
        ...
        let _g = EnvGuard::set("WEEPCODE_AUTH", &legacy_json);

        // AuthManager picks it up from the env var directly (no file needed).
        let dir = tempfile::tempdir().unwrap();
        let cfg = WeepcodeComConfig::default();
        let mgr = AuthManager::new(dir.path(), cfg);
        let current = mgr.current();
        assert!(
            current.is_some(),
            "legacy token in WEEPCODE_AUTH env MUST be loaded directly -- if this fails, \
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g1 = EnvGuard::unset("WEEPCODE_AUTH");
        let _g2 = EnvGuard::unset("WEEPCODE_AUTH_PATH");
    ...
    fn pin_api_key_with_key_only_advertises_api_key() {
        ...
        assert_eq!(method_ids(&built), vec![WEEPCODE_API_KEY_METHOD_ID]);
        assert_eq!(default_id(&built), Some(WEEPCODE_API_KEY_METHOD_ID));
    }
    ...
    fn pin_oidc_with_session_hides_api_key() {
        ...
        assert_eq!(
            method_ids(&built),
            vec![CACHED_TOKEN_AUTH_METHOD_ID, WEEPCODE_COM_METHOD_ID]
        );
    ...
    fn pin_oidc_without_session_is_interactive_only() {
        ...
        assert_eq!(method_ids(&built), vec![WEEPCODE_COM_METHOD_ID]);
        assert!(built.default_auth_method_id.is_none());
    }
}
