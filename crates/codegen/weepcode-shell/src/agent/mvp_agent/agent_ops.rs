...
impl MvpAgent {
    ...
    pub(super) fn build_summary_client(
        ...
        let config = match crate::agent::config::resolve_aux_model_sampling_config(
            ...
            None => {
                // The aux slug (default: the bundled WeepCode model) did not
                // resolve to usable credentials — e.g. a BYOK-only setup
                // where the bundled WeepCode entry has no key. Falling back to
    ...
    pub(super) fn current_or_buffered_auth(&self) -> Option<crate::auth::WeepcodeAuth> {
        self.auth_manager
            .current()
            .or_else(|| {
    ...
    /// Requires feature flag AND WeepCode authentication (OIDC or legacy WebLogin).
    ...
    /// must not block the ACP response (weepcode-desktop sends `initialize` immediately).
    ...
    /// plus user/marketplace walks stalled weepcode-desktop's first `initialize`),
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            let Some(auth) = self
                ...
                .current()
                .filter(|a| {
                    a.is_weepcode_auth() || a.auth_mode == crate::auth::AuthMode::ApiKey
                }) else {
                return;
            };
            let (user_id, team_id) = if auth.is_weepcode_auth() {
                (Some(auth.user_id), auth.team_id)
            } else {
                (None, auth.team_id)
            };
            weepcode_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
                ...
                weepcode_version::VERSION.to_owned(),
    ...
    /// Return the current WeepcodeAuth credentials, if authenticated and not expired.
    pub(crate) fn current_auth(&self) -> Option<crate::auth::WeepcodeAuth> {
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &weepcode_agent::plugins::SharedPluginRegistryHandle {
    ...
    /// discovery on weepcode-desktop Windows).
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<weepcode_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    weepcode_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    weepcode_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match weepcode_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => weepcode_workspace::WorkspaceOps::local(handle),
            Err(e) => {
    ...
    ///     expired or missing.
    ///
    /// Returns `ApiKey` only when the auth method is BYOK (`weepcode.api_key`) or
    ...
    pub(crate) fn auth_type(&self) -> weepcode_chat_state::AuthType {
    ...
    /// When `cached_token` cannot proceed, prefer non-interactive `weepcode.api_key`
    /// iff `should_advertise_weepcode_api_key`; otherwise `provider.setup` (WeepCode:
    ...
    pub(super) fn cached_token_fallthrough_method_id(
        ...
        let preferred = self.cfg.borrow().weepcode_com_config.preferred_method;
        let id = auth_method::method_id_after_cached_token_unavailable(
            auth_method::should_advertise_weepcode_api_key(
                self.cfg.borrow().weepcode_com_config.api_key_auth_disabled(),
    ...
    /// legacy; the unpinned fallthrough targets `weepcode.api_key` or
    ...
    pub(super) async fn authenticate_after_cached_token_unavailable(
        ...
        let meta = if method_id.0.as_ref() == auth_method::WEEPCODE_COM_METHOD_ID {
    ...
    pub(super) async fn refresh_remote_settings(&self, auth: &crate::auth::WeepcodeAuth) {
        if !crate::util::config::resolve_remote_fetch_enabled() {
            tracing::debug!("post-auth settings refresh skipped: remote_fetch disabled");
            return;
        }
        let is_xai = auth.is_weepcode_auth();
        ...
        let (
            ...
            let weepcode_user_id = is_xai.then(|| user_id.clone());
            let weepcode_team_id = is_xai.then(|| team_id.clone()).flatten();
        ...
        weepcode_telemetry::client::init(
            telemetry_config,
            telemetry_mode,
            weepcode_user_id,
            weepcode_team_id,
    ...
    pub(super) async fn send_model_auto_switched(
        ...
        if let Ok(params) = serde_json::value::to_raw_value(&notification) {
            let _ = self
                .gateway
                .ext_notification(
                    acp::ExtNotification::new("weepcode/session_notification", params.into()),
                )
                .await;
        }
    }
    ...
    pub(crate) fn prepare_sampling_config_for_model(
        ...
        if matches!(preferred, Some(crate ::auth::PreferredAuthMethod::Oidc))
            ...
            && credentials.auth_type == weepcode_chat_state::AuthType::ApiKey
        {
            credentials.api_key = None;
            credentials.auth_type = weepcode_chat_state::AuthType::SessionToken;
        }
        ...
        if !has_session_key && credentials.auth_type == weepcode_chat_state::AuthType::ApiKey
    ...
    ///
    /// Both BYOK and session (OAuth) users go direct to `weepcode_api_base_url`.
    ...
    pub(super) fn prepare_image_gen_config(
        &self,
    ) -> weepcode_tools::implementations::weepcode_build::image_gen::ImageGenConfig {
        use weepcode_tools::implementations::weepcode_build::image_gen::ImageGenConfig;
        ...
        let cfg = self.cfg.borrow();
        let base_url = cfg.endpoints.weepcode_api_base_url.clone();
        let version = cfg
            .client_version
            .clone()
            .unwrap_or_else(|| weepcode_version::VERSION.to_string());
        let alpha_test_key = cfg.endpoints.alpha_test_key.clone();
        let mut headers = indexmap::IndexMap::new();
        headers.insert("user-agent".to_string(), format!("weepcode-build/{version}"));
    ...
    pub(super) fn prepare_app_builder_deployer_config(
        &self,
    ) -> weepcode_tools::implementations::weepcode_build::deploy_app::AppBuilderDeployerConfig {
        use weepcode_tools::implementations::weepcode_build::deploy_app::AppBuilderDeployerConfig;
        AppBuilderDeployerConfig::Disabled
    }
    /// Build video generation config. Video tools call the WeepCode API directly.
    pub(super) fn prepare_video_gen_config(
        &self,
    ) -> weepcode_tools::implementations::weepcode_build::video_gen::VideoGenConfig {
        use weepcode_tools::implementations::weepcode_build::video_gen::VideoGenConfig;
    ...
    ///
    /// Enabled gate: `disable_web_search` kill-switch > `WEEPCODE_WEB_FETCH` env >
    /// remote settings `web_fetch_enabled` > default (false).
    ///
    /// Params resolution (TOML > env > remote settings > default):
    /// - `proxy_endpoint`: `[toolset.web_fetch] proxy_endpoint` > `WEEPCODE_WEB_FETCH_PROXY` > remote settings > None
    /// - `allowed_domains`: `[toolset.web_fetch] allowed_domains` > remote settings > built-in defaults
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> weepcode_tools::implementations::weepcode_build::web_fetch::WebFetchConfig {
        use weepcode_tools::implementations::weepcode_build::web_fetch::WebFetchConfig;
        let cfg = self.cfg.borrow();
    ...
    pub fn with_models(
        ...
        if !cfg.weepcode_com_config.api_key_auth_disabled() {
        ...
        let has_weepcode_auth = auth_manager
            .current_or_expired()
            .is_some_and(|a| a.is_weepcode_auth());
        let relay_sync_enabled = tui_mode && relay_config_enabled && has_weepcode_auth;
        ...
        if relay_sync_enabled {
            tracing::info!("[weepcode] Relay sync: ENABLED");
        } else if tui_mode && relay_config_enabled && !has_weepcode_auth {
            tracing::info!(
                "[weepcode] Relay sync: DISABLED (no auth - run 'weepcode login' first)"
            );
        ...
        let instance = Self {
            ...
            plugin_registry_handle: weepcode_agent::plugins::SharedPluginRegistryHandle::new(
                None,
            ...
            monitor_event_buffer: weepcode_tools::implementations::weepcode_build::task::types::MonitorEventBuffer::default(),
        ...
        instance
            .auth_manager
            .configure_refresher(
                instance.cfg.borrow().weepcode_com_config.auth_provider_command.clone(),
    ...
    pub async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
    ) -> Result<weepcode_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> weepcode_tools::implementations::weepcode_build::task::types::SubagentCancelOutcome {
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<weepcode_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    /// 2. Config file/env enables it ([relay] enabled or WEEPCODE_RELAY_SYNC_ENABLED)
    ...
    pub async fn list_hooks(
        &self,
        session_id: &acp::SessionId,
    ) -> Option<weepcode_hooks_plugins_types::HooksListResponse> {
    ...
    pub async fn execute_hooks_action(
        &self,
        session_id: &acp::SessionId,
        action: weepcode_hooks_plugins_types::HooksAction,
    ) -> Option<weepcode_hooks_plugins_types::ActionOutcome> {
        if matches!(action, weepcode_hooks_plugins_types::HooksAction::Untrust)
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&weepcode_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
    ...
    pub async fn execute_plugins_action(
        &self,
        session_id: &acp::SessionId,
        action: weepcode_hooks_plugins_types::PluginsAction,
    ) -> Option<weepcode_hooks_plugins_types::ActionOutcome> {
        let is_reload = matches!(action, weepcode_hooks_plugins_types::PluginsAction::Reload);
        let handle = self.get_session_handle(session_id)?;
        let outcome = handle.execute_plugins_action(action).await;
        let succeeded = matches!(
            outcome.as_ref().map(| o | & o.status),
            Some(weepcode_hooks_plugins_types::OutcomeStatus::Success)
        );
    ...
    pub fn plugin_registry_snapshot(
        &self,
    ) -> Option<std::sync::Arc<weepcode_agent::plugins::PluginRegistry>> {
    ...
    pub(super) fn trace_upload_config_snapshot(
        ...
        let auth_token = if cfg.endpoints.deployment_key.is_none() {
            self.auth_manager
                .current_or_expired()
                .filter(|auth| auth.is_weepcode_auth())
                .map(|auth| auth.key)
        } else {
            None
        };
    ...
    pub(super) fn session_config_meta(
        ...
        let detail = session_config::WeepcodeSessionDetail::build(
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<weepcode_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        base: u64,
        turns: Vec<Vec<weepcode_sampling_types::conversation::ConversationItem>>,
    ) -> Vec<(PromptTraceContext, PromptMetadata, weepcode_chat_state::TurnCapture)> {
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let capture = weepcode_chat_state::TurnCapture {
                messages: items,
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            ...
            None => {
                ...
                weepcode_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
        ...
        let queue = session_handle
            .upload_queue
            .get_or_init(|| {
                let weepcode_home = crate::util::weepcode_home::weepcode_home();
    ...
    /// 5. `WEEPCODE_AGENT` env var.
    /// 6. Built-in default agent.
    ///
    /// `WEEPCODE_AGENT` and an explicit `[agent] name` bypass step 1.
    /// Strict-harness classification is structural — see
    /// [`weepcode_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub fn resolve_agent_definition(
        ...
        acp_agent_profile: Option<weepcode_agent::AgentDefinition>,
        model_agent_type: Option<&str>,
    ) -> weepcode_agent::AgentDefinition {
        use weepcode_agent::AgentDefinition;
        let weepcode_agent_env_set = std::env::var("WEEPCODE_AGENT")
            .ok()
            .is_some_and(|s| !s.trim().is_empty());
        let config_agent_explicitly_set = agent_config.name.is_some();
        let model_requires_strict_harness = model_agent_type
            .is_some_and(weepcode_agent::config::is_strict_harness_agent_type);
        if !weepcode_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && let Some(def) = weepcode_agent::discovery::by_name_in_cwd(required, cwd)
        {
            tracing::info!(
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = weepcode_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = % name,
        ...
        let agent_name = std::env::var("WEEPCODE_AGENT").ok();
        let resolved = match agent_name.as_deref() {
            Some("browser-use") | Some("browser_use") => AgentDefinition::browser_use(),
            Some("weepcode-build-concise") | Some("weepcode_build_concise") => {
                AgentDefinition::weepcode_build_concise()
            }
            Some(path) if std::path::Path::new(path).is_absolute() => {
                match AgentDefinition::from_file(path) {
                    ...
                    Err(e) => {
                        ...
                        AgentDefinition::weepcode_build_plan()
                    }
                }
            }
            Some(name) => {
                weepcode_agent::discovery::by_name_in_cwd(name, cwd)
                    .unwrap_or_else(AgentDefinition::weepcode_build_plan)
            }
            None => AgentDefinition::weepcode_build_plan(),
        };
        if !weepcode_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && resolved.name != required
        {
            ...
            if let Some(def) = weepcode_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let fs_notify_config = init
            ...
            .meta
            .as_ref()
            .and_then(|m| m.get("weepcode/fs_notify"))
            .and_then(|v| {
                use crate::session::{ClientFsConfig, ClientFsMode};
                use weepcode_fsnotify::FsConfig;
        ...
        let fs: Arc<dyn weepcode_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let hunk_plan = plan_hunk_tracking(
            init
                ...
                .meta
                .as_ref()
                .and_then(|m| m.get("weepcode/hunkTracker"))
        ...
        let incremental_bash_output = init
            ...
            .meta
            .as_ref()
            .and_then(|m| m.get("weepcode/incrementalBashOutput"))
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        let no_color = init
            .client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("weepcode/bashOutputNoColor"))
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        ...
        let (hunk_tracker_handle, hunk_event_rx) = match hunk_plan.actor_mode {
            ...
            None => (weepcode_hunk_tracker::HunkTrackerHandle::noop(), None),
        };
        let has_weepcode_auth = self.auth_manager.current().is_some_and(|a| a.is_weepcode_auth());
        let loc_tracking_enabled = hunk_tracking_enabled && has_weepcode_auth
            && (self
                .cfg
                .borrow()
                .remote_settings
                ...
                .unwrap_or(false)
                || std::env::var("WEEPCODE_LOC_TRACKING")
        ...
        let loc_aggregate_rx = match hunk_event_rx {
            Some((hunk_event_rx, loc_cancel)) if loc_tracking_enabled => {
                ...
                let loc_writer = weepcode_hunk_tracker::JsonlHunkRecordWriter::new(loc_path);
                let loc_ctx = weepcode_hunk_tracker::LocSinkContext {
                    session_id: session_info.id.0.to_string(),
                    agent_id: agent_id(),
                ...
                tokio::spawn(
                    weepcode_hunk_tracker::run_loc_sink(
                        hunk_event_rx,
                        loc_writer,
                        loc_ctx,
                        loc_cancel,
        ...
        let mut session_env = weepcode_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
        let envrc = match preloaded_envrc {
            Some(env) => env,
            None => {
                weepcode_workspace::envrc::load_envrc_or_empty_when_trusted(
                    cwd.as_path(),
        ...
        let pinned_model: Option<(acp::ModelId, ModelEntry)> = match &agent_definition
            .model
        {
            weepcode_agent::config::ModelOverride::Override(id) => {
            ...
            weepcode_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = weepcode_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = weepcode_tools::util::weepcode_home::weepcode_home()
                    .join("lsp.json");
                let project_path = tool_ctx.cwd.as_path().join(".weepcode").join("lsp.json");
                tracing::warn!(
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let credentials = weepcode_chat_state::Credentials {
            ...
            let attribution_callback: Option<
                weepcode_sampler::SharedAttributionCallback,
            > = Some(
            ...
            let agent_hook_registry_override = agent_definition
                ...
                .and_then(|hooks_config| {
                    ...
                    let (specs, errors) = weepcode_hooks::config::parse_hooks_from_value_with_dir(
                        &hooks_val,
            ...
            let git_head_changed = init
                ...
                .meta
                .as_ref()
                .and_then(|m| m.get("weepcode/gitHeadChanged"))
                .and_then(|v| v.as_bool());
            ...
            spawn_session_on_thread(
                    ...
                    weepcode_agent::prompt::context::PromptAudience::Primary,
                    None,
                    None,
                    disable_web_search,
        ...
        if let Some(mut loc_rx) = loc_aggregate_rx {
            ...
            tokio::spawn(async move {
                while let Some(agg) = loc_rx.recv().await {
                    match agg {
                        weepcode_hunk_tracker::LocAggregate::LinesChanged {
                            author_type,
                            lines_added,
                            lines_removed,
                            file_path,
                            ...
                            let is_agent = author_type
                                == weepcode_hunk_tracker::AuthorType::Agent;
                            signals
                                .record_loc_change(
                                    is_agent,
                                    lines_added,
                        ...
                        weepcode_hunk_tracker::LocAggregate::LinesReverted {
