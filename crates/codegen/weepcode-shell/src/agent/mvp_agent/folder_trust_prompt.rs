...
//! (`x.ai/folder_trust/request`) that asks a GUI client (weepcode-desktop) to decide
...
//! verdict is [`weepcode_workspace::folder_trust::TrustOutcome::Prompt`]. No
...
//! `.weepcode/lsp.json` servers start on the NEXT session open (the durable grant
...
impl MvpAgent {
    ...
    pub(crate) fn parse_interactive_trust_capability(init: &acp::InitializeRequest) -> bool {
        init.client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("weepcode/folderTrust"))
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = weepcode_workspace::trust::workspace_key(cwd);
        ...
        let targets: Vec<ReloadTarget> = self
            ...
            .values()
            .filter(|h| {
                weepcode_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key
            })
            .map(|h| ReloadTarget {
        ...
        tokio::task::spawn_local(async move {
            ...
            let ext_request = acp::ExtRequest::new("weepcode/folder_trust/request", raw_params.into());
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a weepcode_agent::plugins::SharedPluginRegistryHandle,
    ...
    proxy_url: &'a str,
    compat: &'a weepcode_tools::types::CompatConfig,
...
async fn reload_project_servers_after_grant(ctx: ReloadAfterGrant<'_>) {
    ...
    for target in ctx.targets {
        ...
        // The session's OWN project hooks (`.weepcode/hooks`, `.cursor/hooks.json`),
...
mod tests {
    ...
    fn parse_interactive_trust_capability_present_and_true() {
        ...
        meta.insert(
            "weepcode/folderTrust".to_string(),
