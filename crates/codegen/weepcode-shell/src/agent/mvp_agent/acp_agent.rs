...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        weepcode_telemetry::unified_log::info("agent initialized", None, None);
        ...
        {
            let root = crate::util::weepcode_home::weepcode_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    weepcode_workspace::permission::cleanup_stale_permission_state(
        ...
        weepcode_workspace::trust::migrate_legacy_hook_trust();
        if let Some(auth) = self.auth_manager.current() {
            ...
            weepcode_telemetry::unified_log::info(
        ...
        if client_type == ClientType::Generic {
            match client_identifier.as_deref() {
                Some("weepcode-web") => client_type = ClientType::WeepcodeWeb,
                Some("nebula") => client_type = ClientType::Nebula,
                Some("weepcode-code-extension") => client_type = ClientType::Extension,
                Some("weepcode-desktop") => client_type = ClientType::Desktop,
                _ => {}
            }
        }
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        if !self.cfg.borrow().weepcode_com_config.api_key_auth_disabled()
            && auth_method::read_weepcode_api_key_env().is_err()
            && let Some(api_key) = crate::auth::read_api_key(
                &crate::util::weepcode_home::weepcode_home(),
            )
        {
            unsafe { std::env::set_var("WEEPCODE_API_KEY", &api_key) };
            tracing::info!("auth: loaded API key from auth.json (weepcode::api_key scope)");
            weepcode_telemetry::unified_log::info(
                "auth: loaded API key from auth.json (weepcode::api_key scope)",
                None,
                None,
            );
        }
            ...
            .api_key_auth_disabled();
        {
            ...
            let gc = &cfg.weepcode_com_config;
        ...
        let has_external_api_key = auth_method::should_advertise_weepcode_api_key(
            disable_api_key_auth,
        ...
        if !init_has_current && init_is_expired {
            ...
            if refreshed {
                ...
                weepcode_telemetry::unified_log::warn(
                    "auth: token expired, silent refresh failed - re-authentication required",
                    None,
                    None,
                );
        ...
        let (
            ...
            let cfg = self.cfg.borrow();
            let issuer = cfg.weepcode_com_config.oidc.as_ref().map(|o| o.issuer.clone());
            (
                cfg.weepcode_com_config.auth_provider_label.clone(),
                cfg.weepcode_com_config.auth_provider_command.is_some(),
                cfg.weepcode_com_config.oidc.is_some(),
                issuer,
            )
        };
        let preferred_method = self.cfg.borrow().weepcode_com_config.preferred_method;
        ...
        weepcode_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!(
                    { "weepcode_home" : crate ::util::weepcode_home::weepcode_home().display()
        ...
        debug_assert!(
            ...
            "BYOK invariant violated: weepcode.api_key MUST be auth_methods.first() \
            ...
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                .agent_capabilities(
                    acp::AgentCapabilities::new()
                        ...
                        .meta(
                            serde_json::json!(
                                { "weepcode/fs_notify" : true, "weepcode/hooks" : { "blockingEvents"
                                : [weepcode_hooks::event::HookEventName::PreToolUse],
                                "decisions" : ["deny"], }, }
                            )
                                .as_object()
                                .cloned(),
                ...
                .meta({
                    let metadata = parse_json_object_env("WEEPCODE_AGENT_METADATA");
                    serde_json::json!(
                        { "grokShell" : true, "defaultAuthMethodId" :
                        default_auth_method_id_wire, (weepcode_mcp::wire::MCP_SDK) :
                        ...
                        weepcode_version::VERSION, "agentId" : agent_id(),
    ...
    async fn authenticate(
        ...
        if let Some(preferred) = self.cfg.borrow().weepcode_com_config.preferred_method {
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::WEEPCODE_API_KEY_METHOD_ID => {
                if self.cfg.borrow().weepcode_com_config.api_key_auth_disabled() {
                ...
                if sampling_config.api_key.is_none() {
                    if let Ok(api_key) = auth_method::read_weepcode_api_key_env() {
                        ...
                        .any(|m| m.has_own_credentials())
                    {
                        ...
                        return Err(
                            acp::Error::auth_required()
                                .data(
                                    "Set WEEPCODE_API_KEY or add api_key/env_key to config.toml.",
                                ),
                        );
                    }
                }
                ...
                log_event(weepcode_telemetry::events::Login {
            ...
            auth_method::PROVIDER_SETUP_METHOD_ID => {
                ...
                // authenticates via `weepcode.api_key` once a `[model.*]` entry with
                ...
                Err(acp::Error::auth_required().data(
                    "No API provider configured. Run `weepcode` interactively and choose \
                     \"Configure API Provider\" on the welcome screen, or add a \
                     [model.<name>] table with base_url/api_key to ~/.weepcode/config.toml.",
                ))
            }
            auth_method::WEEPCODE_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                let weepcode_ctx = self.auth_manager.weepcode_com_config();
                let auth_meta = AuthRequestMeta::from_json(arguments.meta.as_ref());
                tracing::info!(
                ...
                let use_oidc = self.cfg.borrow().resolve_weepcode_oauth(cli_oauth);
                tracing::debug!(
                ...
                let auth_result = if !auth_meta.headless {
                    ...
                    tokio::select! {
                        ...
                        weepcode_ctx, crate ::auth::AuthChannels { url_tx : Some(url_tx),
                    ...
                    tokio::select! {
                        biased; _ = cancel.cancelled() => { cancelled = true;
                        Err(anyhow::anyhow!("Authentication cancelled")) } r = crate
                        ::auth::run_auth_flow(& self.auth_manager, weepcode_ctx, auth_meta
    ...
    async fn new_session(
        ...
        weepcode_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::SessionStarted {
        ...
        if product_analytics || weepcode_telemetry::external::is_active() {
            ...
            let perm = if session_yolo_mode {
                weepcode_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                weepcode_telemetry::enums::PermissionMode::Auto
            } else {
                weepcode_telemetry::enums::PermissionMode::Ask
            };
            tokio::spawn(async move {
                let git = weepcode_telemetry::context::collect_git_context(&cwd_str);
                let ev = weepcode_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                weepcode_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) = match weepcode_workspace::session::git::discover_git_root(
            cwd.as_path(),
        ) {
        ...
        if let Some(obj) = meta.as_object_mut() {
            obj.insert("weepcode/sessionConfig".to_string(), session_config_value);
            obj.insert("weepcode/sessionDetail".to_string(), session_detail_value);
        }
        Ok(
    ...
    async fn load_session(
        ...
        let persist_data = arguments
            .meta
            .as_ref()
            .and_then(|m| m.get("weepcode/persist"))
            .cloned();
        ...
        let git_root = weepcode_workspace::session::git::find_git_root_from_path(
                cwd.as_path(),
            )
            .ok();
            ...
            Some(s) => {
                match s.state {
                    crate::session::plan_mode::PlanModeState::Inactive => {
                        weepcode_telemetry::events::PlanModeState::Inactive
                    }
                    crate::session::plan_mode::PlanModeState::Pending => {
                        weepcode_telemetry::events::PlanModeState::Pending
                    }
                    crate::session::plan_mode::PlanModeState::Active
                    | crate::session::plan_mode::PlanModeState::ExitPending => {
                        weepcode_telemetry::events::PlanModeState::Active
                    }
                }
            }
            None => weepcode_telemetry::events::PlanModeState::Inactive,
        };
        ...
        let restore_code_requested = request_meta
            .as_ref()
            .and_then(|m| m.get("weepcode/restore_code"))
            .and_then(|v| v.as_bool())
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            weepcode_workspace::session::git::warn_registry_disabled_restore(
                session_id.0.as_ref(),
            );
        }
        let restore_checkout_allowed = weepcode_workspace::session::git::restore_code_checkout_allowed(
            cwd.as_path(),
        ...
        {
            tracing::warn!(
                target : weepcode_workspace::session::git::RESTORE_CODE_LOG, session_id =
                % session_id.0, supplied_cwd = % cwd.as_str(), persisted_cwd = % summary
                .info.cwd, target_sha = % target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a weepcode worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
            weepcode_telemetry::unified_log::warn(
        ...
        if restore_code_requested && restore_checkout_allowed
            && let Some(ref target_sha) = summary.head_commit
        {
            use weepcode_workspace::session::git::RestoreKind;
            let outcome = weepcode_workspace::session::git::checkout_session_commit(
                    cwd.as_path(),
                    target_sha,
                    true,
                    session_id.0.as_ref(),
        ...
        let load_envrc = {
            let skip_envrc = request_meta
                .as_ref()
                .and_then(|m| m.get("weepcode/skip_envrc"))
                .and_then(|v| v.as_bool())
                .unwrap_or(false);
            if skip_envrc {
                false
        ...
        let preloaded_envrc = weepcode_workspace::envrc::load_envrc_or_empty_when_trusted(
            cwd.as_path(),
        ...
        let prompt_display_cwd = request_meta
            .as_ref()
            .and_then(|m| m.get("weepcode/display_cwd"))
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
        ...
        let is_weepcode_build = persisted_model.0.starts_with("weepcode-build");
        let same_family_fallback = if is_weepcode_build {
            available.keys().find(|id| id.0.starts_with("weepcode-build")).cloned()
        } else {
            available.keys().find(|id| !id.0.starts_with("weepcode-build")).cloned()
        };
        ...
        if let Some(persist) = persist_data {
            response_meta_map.insert("weepcode/persist".to_string(), persist);
        }
        ...
        if summary.head_commit.is_some() && let Some(ref cwd) = session_cwd
            && summary
                ...
                .as_deref()
                .is_none_or(|root| {
                    weepcode_workspace::session::git::find_git_root_from_path(
                ...
                })
        {
            ...
            let current_head = weepcode_workspace::session::git::git_cli(
                    cwd_path,
                    &["rev-parse", "HEAD"],
                )
                .await
                .ok();
            if let Some(divergence) = weepcode_workspace::session::git::detect_head_divergence(
            ...
            .and_then(|h| h.current_prompt_id.lock().ok().and_then(|g| g.clone()))
        {
            response_meta_map
                .insert(
                    "weepcode/runningPromptId".to_string(),
        ...
        response_meta_map.insert("weepcode/sessionConfig".to_string(), session_config_value);
        response_meta_map.insert("weepcode/sessionDetail".to_string(), session_detail_value);
        ...
        if self.product_analytics_enabled() {
            log_event(weepcode_telemetry::events::SessionLoad {
                session_id: session_id.0.to_string(),
    ...
    async fn prompt(
        ...
        if let Some(meta) = arguments.meta.as_ref() {
            weepcode_file_utils::trace_context::link_current_span_to_meta(
        ...
        if let Some(ctx) = trace_context.clone() {
            ...
            let mut prompt_metadata = PromptMetadata {
                ...
                shell_version: Some(weepcode_version::VERSION.to_string()),
                workspace_type: None,
            ...
            self.gateway
                .forward_fire_and_forget(
                    acp::ExtNotification::new(
                        "weepcode/session/prompt_complete",
                        params.into(),
                    ),
                );
        }
        ...
        match stop_result {
            Ok(turn_ok) => {
                ...
                let turn_messages: Option<weepcode_chat_state::TurnCapture> = {
                    let (tx, rx) = oneshot::channel();
                    if handle
                        .cmd_tx
                ...
                if let Some(ctx) = trace_context {
                        ...
                        && let Some(client) = self.session_registry_client()
                    {
                        ...
                        tokio::spawn(async move {
                            let git_out = |args: &[&str]| -> Option<String> {
                                weepcode_tty_utils::git_command()
                                    .current_dir(&cwd_str)
                                    .args(args)
                                    .output()
                                    .ok()
                    ...
                    async fn advance_last_turn(
                        ...
                        let repo_head_at_end = weepcode_tty_utils::git_command()
                            .current_dir(&cwd)
                            .args(["rev-parse", "HEAD"])
                            .output()
                            .ok()
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = weepcode_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = weepcode_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
    ...
    async fn ext_method(
        ...
        if let Some(meta) = &request_meta {
            weepcode_file_utils::trace_context::link_current_span_to_meta(meta);
        }
        ...
        let result = match method.as_ref() {
            "weepcode/getApiKey" | "weepcode/setApiKey" => {
                crate::extensions::auth::handle(self, &args).await
            }
            "weepcode/session/info" | "weepcode/session/close" | "weepcode/session/list"
            | "weepcode/sessions/list" => {
                crate::agent::handlers::session::handle(self, &args).await
            }
            "weepcode/workspaces/list" => {
                crate::agent::handlers::workspaces::handle(self, &args).await
            }
            "weepcode/session/updates" => {
                crate::extensions::session_updates::handle(&args, &self.gateway).await
            }
            "weepcode/session/load_history" => {
                crate::extensions::chat_conversation_history::handle(self, &args).await
            }
            "weepcode/session/search" => {
                crate::extensions::session_search::handle(&args).await
            }
            "weepcode/session/resolve_local_for_worktree_resume"
            | "weepcode/session/rehydrate" => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::worktree::handle(self, &ops, &args).await
            }
            "weepcode/session/rename" | "weepcode/session/delete"
            | "weepcode/session/update_mcp_servers" | "weepcode/session/fork"
            | "weepcode/internal/reload_all_mcp_servers"
            | "weepcode/internal/reload_project_mcp_servers" | "weepcode/internal/reload_skills"
            | "weepcode/internal/reload_models" | "weepcode/internal/reload_models_cache"
            | "weepcode/internal/auth_cleared" | "weepcode/plugins/reload"
            | "weepcode/commands/list" => {
                crate::extensions::session_admin::handle(self, &args).await
            }
            "weepcode/session/repair" => crate::extensions::repair::handle(self, &args).await,
            s if s.starts_with("weepcode/provider/") => {
                crate::extensions::provider_setup::handle(self, &args).await
            }
            "weepcode/memory/flush" | "weepcode/memory/rewrite" => {
                crate::extensions::memory::handle(self, &args).await
            }
            "weepcode/skills/refresh-baseline" => {
                self.refresh_skill_baseline_for_all_sessions();
                crate::extensions::to_ext_response(
                    Ok(serde_json::json!({ "ok" : true })),
                )
            }
            "weepcode/interject" => crate::extensions::interject::handle(self, &args).await,
            "weepcode/recap" => crate::extensions::recap::handle(self, &args).await,
            "weepcode/cloud/terminate" => {
                crate::extensions::auth_gate::require_weepcode_auth(
                    &self.auth_manager,
                    "Authentication required",
                    "Run `weepcode login` to authenticate.",
                )?;
            ...
            "weepcode/rollout/survey" => {
                crate::extensions::rollout::handle(self, &args).await
            }
            "weepcode/prompt_history" => {
                crate::extensions::prompt_history::handle(self, &args).await
            }
            "weepcode/suggest" => crate::extensions::suggest::handle(self, &args).await,
            "weepcode/suggestPrompt" => crate::extensions::suggest::handle(self, &args).await,
            s if s.starts_with("weepcode/auth/") => {
                crate::extensions::auth::handle(self, &args).await
            }
            s if s.starts_with("weepcode/session_summaries/") => {
                crate::agent::handlers::session::handle(self, &args).await
            }
            s if s.starts_with("weepcode/git/worktree/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::worktree::handle(self, &ops, &args).await
            }
            s if s.starts_with("weepcode/git/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::git::handle(self, &ops, &args).await
            }
            s if s.starts_with("weepcode/compact_conversation") => {
                crate::extensions::memory::handle(self, &args).await
            }
            s if s.starts_with("weepcode/plugins/") => {
                crate::extensions::plugins::handle(self, &args).await
            }
            s if s.starts_with("weepcode/marketplace/") => {
                crate::extensions::marketplace::handle(self, &args).await
            }
            s if s.starts_with("weepcode/hooks/") => {
                crate::extensions::hooks::handle(self, &args).await
            }
            s if s.starts_with("weepcode/hunk-tracker/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::hunk_tracker::handle(self, &ops, &args).await
            }
            s if s.starts_with("weepcode/pr/") => {
            ...
            s if s.starts_with("weepcode/debug/") => {
                crate::extensions::debug::handle(self, &args).await
            }
            s if s.starts_with("weepcode/rewind") => {
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "weepcode/yolo_mode_changed"
        ...
        if args.method.as_ref() == "weepcode/permissions/reset" {
        ...
        if args.method.as_ref() == "weepcode/internal/evict_sessions" {
            self.handle_evict_sessions(&args.params).await;
        }
        if args.method.as_ref() == "weepcode/toggle_plan_mode"
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        {
        ...
        if matches!(
            args.method.as_ref(), "weepcode/queue/remove" | "weepcode/queue/reorder" |
            "weepcode/queue/clear" | "weepcode/queue/edit" | "weepcode/queue/interject"
        )
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        ...
        if args.method.as_ref() == "weepcode/terminal/pty/input"
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        {
        ...
        if args.method.as_ref() == "_x.ai/session/update" {
            ...
            >(args.params.get()) {
                tracing::info!(
                    "Storing WeepCode session notification: session_id={}", notification
                    .session_id.0
                );
                    ...
                    .get(&notification.session_id)
                {
                    ...
                    tracing::warn!(
                        "Received WeepCode session notification for unknown session: {}",
                        notification.session_id.0
                    );
                }
            } else {
                tracing::warn!("Failed to parse WeepCode session notification params");
            }
        }
        if args.method.as_ref() == "weepcode/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                weepcode_telemetry::session_ctx::log_event(weepcode_telemetry::events::NonGitDecisionEvent {
        ...
        if args.method.as_ref() == "weepcode/telemetry/multi_agent_followup" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                weepcode_telemetry::session_ctx::log_event(weepcode_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| weepcode_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "weepcode/telemetry/multi_agent_apply" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                weepcode_telemetry::session_ctx::log_event(weepcode_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "weepcode/telemetry/multi_agent_discard" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                weepcode_telemetry::session_ctx::log_event(weepcode_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| weepcode_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == weepcode_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                weepcode_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            weepcode_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
