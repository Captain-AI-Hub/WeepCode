...
use weepcode_telemetry::id::{agent_id, agent_instance_id};
use weepcode_telemetry::session_ctx::log_event;
use weepcode_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use weepcode_workspace::permission::{ClientType, PermissionEvent};
...
use weepcode_workspace::session::git::GitDiscoveryResult;
use weepcode_hunk_tracker::HunkTrackerActor;
...
pub(crate) fn reject_direct_hub_cloud_meta(
    ...
    if session_meta.and_then(|m| m.get("weepcode/cloud_server_id")).is_some() {
...
pub(crate) const SESSION_PLUGIN_DIRS_CAPABILITY_KEY: &str = "weepcode/pluginDirs";
...
fn mark_as_replay(
    ...
    if let Some(persist) = persist_data {
        obj.insert("weepcode/persist".to_string(), persist.clone());
    }
}
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a weepcode_sampling_types::TokenUsage>,
...
pub struct MvpAgent {
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.weepcode/`).
    ///
    ...
    session_index_claims: RefCell<
        HashMap<acp::SessionId, std::sync::Arc<weepcode_codebase_graph::IndexManagerHandle>>,
    >,
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        weepcode_tools::implementations::weepcode_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: weepcode_tools::implementations::weepcode_build::task::types::MonitorEventBuffer,
    ...
    subagent_roles: std::collections::HashMap<
        String,
        weepcode_subagent_resolution::config::SubagentRole,
    >,
    ...
    pub(crate) plugin_registry_handle: weepcode_agent::plugins::SharedPluginRegistryHandle,
    ...
    /// cwd→git root plus user/marketplace dirs and stalled weepcode-desktop's first
    ...
    workspace_ops: RefCell<Option<weepcode_workspace::WorkspaceOps>>,
...
pub(crate) fn inherited_harness_template(
    current: &weepcode_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<weepcode_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !weepcode_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = weepcode_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// unchanged — compatible stock switches (e.g. `weepcode-build` →
/// `weepcode-build-plan`) intentionally preserve the session's original ACP
/// `agentProfile`.
...
/// default wire format and toolset, so switching e.g. `weepcode-build` →
/// `weepcode-build-plan` doesn't require rebuilding the harness and would
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use weepcode_agent::config::is_strict_harness_agent_type;
    match (
...
use weepcode_chat_state::conversation_util::replace_or_insert_system_head;
...
///
/// Headers injected:
///  - `x-weepcode-client-version` -- required by the proxy's version-gate check.
///    Uses `client_version` when provided, otherwise falls back to cli-chat-proxy
///    compile-time `CARGO_PKG_VERSION`.
///  - `X-WeepCode-Token-Auth` / `x-authenticateresponse` -- required by the
...
fn inject_proxy_headers(
    ...
    headers
        .entry("x-weepcode-client-version".to_string())
        .or_insert_with(|| {
            client_version
                .map(String::from)
...
fn resolve_hunk_tracking_mode(
    mode_str: Option<&str>,
) -> Option<weepcode_hunk_tracker::TrackingMode> {
    ...
    Some(
        serde_json::from_value(serde_json::Value::String(mode.to_ascii_lowercase()))
            .unwrap_or(weepcode_hunk_tracker::TrackingMode::AllDirty),
    )
}
...
struct HunkTrackingPlan {
    ...
    actor_mode: Option<weepcode_hunk_tracker::TrackingMode>,
}
...
impl MvpAgent {
    ...
    /// - WeepCode updates (`"_x.ai/session/update"`) → `ExtNotification`.
    ///
    ...
    fn forward_raw_replay_line(
        ...
        completions: &mut Vec<
            tokio::sync::oneshot::Receiver<weepcode_acp_lib::AcpResult<()>>,
        >,
        mark_replay: bool,
        ...
        if is_xai {
            if target_client_id.is_none() && !mark_replay {
                ...
                >(raw_params.get()) else {
                    tracing::debug!(
                        "replay: skipping WeepCode update with unparseable params"
                    );
                    return;
                };
                if let Some(obj) = params.as_object_mut() {
                    ...
                    if let Some(m) = meta.as_object_mut() {
                        ...
                        if let Some(pd) = persist_data {
                            m.insert("weepcode/persist".to_string(), pd.clone());
                        }
                        if let Some(tid) = target_client_id {
                            m.insert("weepcode/leaderClientId".to_string(), tid.clone());
                        }
                    }
                }
            ...
            if let Some(tid) = target_client_id {
                stamp_meta_value(&mut notification.meta, "weepcode/leaderClientId", tid);
            }
    ...
    pub(super) fn replay_session_updates_from_offset_enqueue(
        ...
        target_client_id: Option<&serde_json::Value>,
        mark_replay: bool,
    ) -> Vec<tokio::sync::oneshot::Receiver<weepcode_acp_lib::AcpResult<()>>> {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = weepcode_tools::types::TaskSnapshot {
    ...
    pub(super) async fn maybe_fetch_post_auth_settings(&self) {
        ...
        let is_weepcode_auth = auth.is_weepcode_auth();
        ...
        tracing::info!("post-auth remote_settings fetch succeeded");
        {
                ...
                && cfg.mode != crate::agent::config::AgentMode::Generic
            {
                ...
                if cfg.storage_mode == StorageMode::Writeback && !is_weepcode_auth {
        ...
        if let Ok(params) = serde_json::value::to_raw_value(&payload) {
            self.gateway
                .forward_fire_and_forget(
                    acp::ExtNotification::new("weepcode/settings/update", params.into()),
                );
        }
    }
    ...
    ///    against `~/.weepcode/bundled/` and the manifest.
