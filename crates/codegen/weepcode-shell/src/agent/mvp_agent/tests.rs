...
mod hunk_tracking_mode {
    ...
    use weepcode_hunk_tracker::TrackingMode;
    #[test]
...
fn harness_pair(id: &str) -> Vec<weepcode_sampling_types::conversation::ConversationItem> {
    use weepcode_sampling_types::ToolCall;
    use weepcode_sampling_types::conversation::ConversationItem;
    vec![
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_defaults_to_weepcode_build() {
    let prev = std::env::var("WEEPCODE_AGENT").ok();
    unsafe {
        std::env::remove_var("WEEPCODE_AGENT");
    }
    ...
    if let Some(v) = prev {
        unsafe { std::env::set_var("WEEPCODE_AGENT", v) }
    }
}
/// When model_agent_type = Some("codex"), the codex agent is selected even
/// though the default chain would return weepcode-build.
#[test]
#[serial_test::serial]
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = weepcode_agent::AgentDefinition::from_json(&serde_json::json!(
...
/// `weepcode-build-plan`, models in the catalog that still declare
/// `agent_type = "weepcode-build"` explicitly must NOT preempt an ACP
/// profile. Any value in the `weepcode-build*` family is the stock harness
/// with no strict requirement.
#[test]
#[serial_test::serial]
fn resolve_agent_definition_acp_profile_wins_for_explicit_weepcode_build_family() {
    let prev = std::env::var("WEEPCODE_AGENT").ok();
    unsafe {
        std::env::remove_var("WEEPCODE_AGENT");
    }
    ...
    .expect("agent definition must parse");
    for family_variant in ["weepcode-build", "weepcode-build-plan", "weepcode-build-concise"] {
        let def = MvpAgent::resolve_agent_definition(
            tmp.path(),
            None,
            &config::AgentSelectionConfig::default(),
        ...
        assert_eq!(
            def.name, "custom-devbox-profile",
            "ACP profile must win for weepcode-build family variant `{family_variant}`"
        );
    }
    if let Some(v) = prev {
        unsafe { std::env::set_var("WEEPCODE_AGENT", v) }
...
fn inherited_harness_template_skips_nonstrict_model() {
    use weepcode_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let agents_dir = tmp.path().join(".weepcode").join("agents");
    ...
    assert_eq!(
        def.model,
        weepcode_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("WEEPCODE_AGENT", v) },
        None => unsafe { std::env::remove_var("WEEPCODE_AGENT") },
    }
}
#[test]
...
/// flow: a zero-turn switch from `weepcode-build` (a client profile name) to
/// `weepcode-build-plan` (the default model agent_type) must be
...
fn harnesses_are_compatible_for_stock_family_pairs() {
    assert!(harnesses_are_compatible("weepcode-build", "weepcode-build-plan"));
    assert!(harnesses_are_compatible("weepcode-build-plan", "weepcode-build"));
    assert!(harnesses_are_compatible("weepcode-build", "weepcode-build"));
    assert!(harnesses_are_compatible(
        "weepcode-build-concise",
        "weepcode-build-plan"
    ));
    assert!(harnesses_are_compatible(
        "remote-sidebar",
...
fn harnesses_are_compatible_rejects_strict_mismatches() {
    ...
    assert!(!harnesses_are_compatible("weepcode-build-plan", "codex"));
}
#[test]
fn explicit_agent_type_wins_over_session_default() {
    assert_eq!(
        resolve_required_agent_type(Some("cursor"), "weepcode-build-plan"),
        "cursor"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_weepcode_build_plan() {
    assert_eq!(
        resolve_required_agent_type(None, "weepcode-build-plan"),
        "weepcode-build-plan"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_weepcode_build() {
    assert_eq!(
        resolve_required_agent_type(None, "weepcode-build"),
        "weepcode-build"
    );
}
#[test]
fn null_agent_type_returns_to_session_default_after_cursor_switch() {
    let session_default = "weepcode-build-plan";
    let required_after_null = resolve_required_agent_type(None, session_default);
    assert_eq!(required_after_null, "weepcode-build-plan");
...
fn agent_name_unchanged_without_harness_rebuild() {
    let unchanged = agent_name_after_model_switch(false, "weepcode-build-plan", "remote-sidebar");
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use weepcode_tools::computer::local::{LocalFs, LocalTerminalBackend};
    use weepcode_tools::notification::ToolNotificationHandle;
    use weepcode_tools::registry::types::SessionContext;
    let tmp = tempfile::tempdir().unwrap();
    ...
    let builder = weepcode_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: weepcode_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: weepcode_tools::implementations::weepcode_build::image_gen::ImageGenConfig::default(),
        video_gen_config: weepcode_tools::implementations::weepcode_build::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: weepcode_tools::implementations::weepcode_build::deploy_app::AppBuilderDeployerConfig::default(),
        api_key_provider: None,
        auth_provider: None,
        attribution_callback: None,
        system_reminder_tag: weepcode_tools::reminders::DEFAULT_REMINDER_TAG,
    };
    let toolset = builder
...
fn make_test_handle(
    ...
    let hunk_tracker_handle = weepcode_hunk_tracker::HunkTrackerActor::spawn(
        "test".to_string(),
        std::path::PathBuf::from("/tmp"),
        hunk_event_tx,
        weepcode_hunk_tracker::TrackingMode::AllDirty,
        hunk_cancel,
    );
    crate::session::SessionHandle {
        cmd_tx,
        ...
        max_turns: None,
        hunk_tracker_handle,
        chat_state_handle: weepcode_chat_state::ChatStateHandle::noop(),
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            weepcode_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(weepcode_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
        ...
        permission_handle: weepcode_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        agent_name: "weepcode-build".to_string(),
        managed_mcp_proxy_base_url: String::new(),
        session_default_agent_profile: None,
        allowed_subagent_types: None,
        hook_registry: None,
        workspace_ops: weepcode_workspace::WorkspaceOps::for_test(),
...
async fn lookup_session_model_returns_per_session_model() {
    ...
    let sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("weepcode-3-fast", false, None)),
...
async fn lookup_session_model_fallback_no_session() {
    let default_model = acp::ModelId::new("weepcode-3");
...
async fn set_session_model_does_not_cross_contaminate() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("weepcode-3", false, None)),
        (sid_b.clone(), make_test_handle("weepcode-3", false, None)),
    ]
    .into();
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use weepcode_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn session_config_options_resolves_routing_slug_to_catalog_model() {
    use crate::agent::config::{EndpointsConfig, ModelEntry};
    use weepcode_sampling_types::ReasoningEffort;
    let agent = build_minimal_agent_for_tests();
...
async fn yolo_toggle_scoped_by_client_identifier() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("weepcode-3", false, Some("weepcode-tui")),
        ),
        (
            sid_vscode.clone(),
            make_test_handle("weepcode-3", false, Some("weepcode-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("weepcode-tui"), true);
...
async fn yolo_toggle_can_disable_session_started_with_yolo_enabled() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("weepcode-3", true, Some("weepcode-tui")),
        ),
        (
            sid_other.clone(),
            make_test_handle("weepcode-3", true, Some("weepcode-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("weepcode-tui"), false);
...
fn parse_code_nav_capability_present_and_true() {
    ...
    meta.insert(
        "weepcode/codeNavigation".to_string(),
...
async fn test_per_session_code_nav_isolation() {
    let web_handle = {
        let mut h = make_test_handle("model", false, Some("weepcode-web"));
        h.code_nav_enabled = true;
        h
    };
    let tui_handle = {
        let mut h = make_test_handle("model", false, Some("weepcode-tui"));
    ...
    let check = |handle: &crate::session::SessionHandle| {
        ...
        if !matches!(ct, ClientType::WeepcodeWeb) {
...
async fn ext_method_routes_auth_cleared_and_refreshes_resident_sessions() {
    ...
    local
        .run_until(async {
            let agent = build_agent_with_auth(crate::auth::WeepcodeAuth {
                key: "eligible".into(),
                auth_mode: crate::auth::AuthMode::WebLogin,
                ..crate::auth::WeepcodeAuth::test_default()
            });
            use acp::Agent as _;
            ...
            agent
                .ext_method(acp::ExtRequest::new(
                    "weepcode/internal/auth_cleared",
...
fn build_minimal_agent_for_tests() -> MvpAgent {
    ...
    use crate::auth::{AuthManager, WeepcodeComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(temp_dir.path(), WeepcodeComConfig::default()));
...
fn build_agent_with_auth(auth: crate::auth::WeepcodeAuth) -> MvpAgent {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, WeepcodeComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
    let auth_manager =
...
async fn ensure_plugin_registry_lazily_populates_snapshot() {
    ...
    let weepcode_home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("WEEPCODE_HOME", weepcode_home.path());
    ...
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(auth_home.path(), WeepcodeComConfig::default()));
    let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
    let gateway = GatewaySender::new(tx);
...
async fn resident_activity_reports_needs_input_when_pending() {
    ...
    let handle = make_test_handle("weepcode-3", false, None);
...
fn drain_roster_changed(
    rx: &mut tokio::sync::mpsc::UnboundedReceiver<weepcode_acp_lib::AcpClientMessage>,
) -> Option<crate::agent::roster::RosterChanged> {
    let mut found = None;
    while let Ok(msg) = rx.try_recv() {
        if let weepcode_acp_lib::AcpClientMessage::ExtNotification(args) = msg {
            if found.is_none()
...
async fn push_roster_activity_delta_broadcasts_overridden_activity() {
    ...
    agent
        .sessions
        .borrow_mut()
        .insert(sid.clone(), make_test_handle("weepcode-3", false, None));
...
async fn test_web_session_with_capability_is_eligible() {
    ...
    let mut handle = make_test_handle("model", false, Some("weepcode-web"));
...
async fn test_tui_session_is_rejected() {
    ...
    let mut handle = make_test_handle("model", false, Some("weepcode-tui"));
    handle.code_nav_enabled = true;
    let sessions = [(sid.clone(), handle)].into();
    assert_eq!(
...
async fn test_leader_mode_two_sessions_stay_isolated() {
    ...
    let mut web_handle = make_test_handle("model", false, Some("weepcode-web"));
    web_handle.code_nav_enabled = true;
    let mut tui_handle = make_test_handle("model", false, Some("weepcode-tui"));
...
async fn test_unknown_session_id_returns_session_required() {
    ...
    let mut known_handle = make_test_handle("model", false, Some("weepcode-web"));
...
mod eligibility_gates {
    ...
    fn check_gates(
        ...
        if !matches!(client_type, ClientType::WeepcodeWeb) {
            return Err(CodeNavEligibility::ClientNotWeb);
        }
        if !code_nav_enabled {
            return Err(CodeNavEligibility::CapabilityNotAdvertised);
    ...
    fn tui_client_rejected() {
        assert_eq!(
            check_gates(ClientType::WeepcodeTUI, true, true),
            Err(CodeNavEligibility::ClientNotWeb)
        );
    }
    #[test]
    fn web_client_no_capability_rejected() {
        assert_eq!(
            check_gates(ClientType::WeepcodeWeb, false, true),
            Err(CodeNavEligibility::CapabilityNotAdvertised)
        );
    }
    #[test]
...
async fn auth_type_session_based_no_current_returns_session_token() {
    for method_id in [
        ...
        crate::agent::auth_method::WEEPCODE_COM_METHOD_ID,
...
/// BYOK guard. Users with `weepcode.api_key` must continue to report `ApiKey`
...
#[tokio::test(flavor = "current_thread")]
async fn auth_type_weepcode_api_key_no_current_returns_api_key() {
    let agent = build_minimal_agent_for_tests();
    agent.set_auth_method(acp::AuthMethodId::new(
        crate::agent::auth_method::WEEPCODE_API_KEY_METHOD_ID,
    ));
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(
        agent.auth_type(),
        weepcode_chat_state::AuthType::ApiKey,
        "weepcode.api_key auth must report ApiKey -- BYOK has no session-token \
...
async fn auth_type_session_based_with_current_returns_session_token() {
    use crate::auth::WeepcodeAuth;
    let agent = build_minimal_agent_for_tests();
    agent.set_auth_method(acp::AuthMethodId::new(
        crate::agent::auth_method::OIDC_METHOD_ID,
    ));
    agent.auth_manager.hot_swap(WeepcodeAuth::test_default());
    assert!(agent.auth_manager.current().is_some());
    assert_eq!(agent.auth_type(), weepcode_chat_state::AuthType::SessionToken,);
}
...
async fn auth_type_no_method_id_no_current_returns_api_key() {
    ...
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(agent.auth_type(), weepcode_chat_state::AuthType::ApiKey,);
}
...
/// Minimal agent whose `weepcode_com_config` engages the api-key kill switch
...
fn build_agent_with_api_key_auth_disabled() -> MvpAgent {
    ...
    let gateway = GatewaySender::new(tx);
    let mut cfg = AgentConfig::default();
    cfg.weepcode_com_config.disable_api_key_auth = Some(true);
    MvpAgent::new(gateway, &cfg, auth_manager, None).expect("valid test config")
}
/// Deployment-key / managed-config user: `WEEPCODE_API_KEY` resolves and the kill
/// switch is off, so a dead `cached_token` MUST fall through to `weepcode.api_key`
...
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    use crate::agent::auth_method::{WEEPCODE_API_KEY_ENV_VAR, WEEPCODE_API_KEY_METHOD_ID};
    use weepcode_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("WEEPCODE_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(WEEPCODE_API_KEY_ENV_VAR, "test-deployment-key");
    let agent = build_minimal_agent_for_tests();
    assert_eq!(
        agent
            ...
            .as_ref()
            .map(|id| id.0.as_ref()),
        Some(WEEPCODE_API_KEY_METHOD_ID),
        "deployment-key user (WEEPCODE_API_KEY set, no kill switch) must fall \
         through to weepcode.api_key on a dead cached_token -- not interactive login",
    );
}
/// Forced-IdP deployment: even with `WEEPCODE_API_KEY` present, the admin kill
/// switch keeps the fallthrough on interactive `provider.setup` (api-key auth
/// is neither advertised nor an eligible fallthrough).
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_respects_kill_switch() {
    use crate::agent::auth_method::{PROVIDER_SETUP_METHOD_ID, WEEPCODE_API_KEY_ENV_VAR};
    use weepcode_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("WEEPCODE_DISABLE_API_KEY_AUTH");
    let _key = EnvGuard::set(WEEPCODE_API_KEY_ENV_VAR, "test-deployment-key");
    ...
    assert_eq!(
        ...
        "disable_api_key_auth must keep the cached_token fallthrough on \
         interactive provider.setup so WEEPCODE_API_KEY can't bypass forced login",
    );
}
/// No advertiseable credentials at all (no env key, no kill switch): the user
/// genuinely needs to configure a provider, so the fallthrough is interactive
/// `provider.setup` (WeepCode: in-TUI form, not WeepCode browser OAuth).
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_falls_to_provider_setup_without_credentials() {
    use crate::agent::auth_method::{
        LEGACY_WEEPCODE_API_KEY_ENV_VAR, PROVIDER_SETUP_METHOD_ID, WEEPCODE_API_KEY_ENV_VAR,
    };
    use weepcode_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("WEEPCODE_DISABLE_API_KEY_AUTH");
    let _new = EnvGuard::unset(WEEPCODE_API_KEY_ENV_VAR);
    ...
    assert_eq!(
        agent
            ...
            .map(|id| id.0.as_ref()),
        Some(PROVIDER_SETUP_METHOD_ID),
        "no API-key creds and no kill switch -> interactive provider.setup",
    );
}
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use weepcode_tools::implementations::weepcode_build::video_gen::{
    ...
    fn zdr_s3() -> ZdrVideoOutputS3Config {
        ZdrVideoOutputS3Config {
            ...
            key_prefix: "weepcode-videos/".into(),
            expires_secs: 900,
...
async fn data_collection_enabled_for_normal_user() {
    let agent = build_agent_with_auth(crate::auth::WeepcodeAuth::test_default());
    assert!(
...
/// trace-upload config switch; it must also honor ZDR, checked at
/// invocation time.
#[tokio::test]
async fn diagnostic_upload_skipped_for_zdr_team_user() {
    ...
    assert_eq!(
        ...
        0,
        "no diagnostics request may leave the machine for a ZDR team"
    );
}
#[tokio::test]
...
fn parse_session_kind_matrix() {
    ...
    let cases: &[(&str, serde_json::Value, SessionKind)] = &[
        (
            "chat",
            json!({ "weepcode/session" : { "kind" : "chat" } }),
            SessionKind::Chat,
        ),
        (
            "build",
            json!({ "weepcode/session" : { "kind" : "build" } }),
            SessionKind::Build,
        ),
        (
            "chat_malformed_sibling",
            json!({ "weepcode/session" : { "kind" : "chat", "facets" : "not-a-map" } }),
            SessionKind::Chat,
        ),
        (
            "unknown_kind",
            json!({ "weepcode/session" : { "kind" : "frob" } }),
            SessionKind::Build,
        ),
...
fn chat_initial_model_matrix() {
    let cases: &[(&str, bool, Option<&str>, Option<&str>)] = &[
        ("chat_with_model", true, Some("weepcode-4.5"), Some("weepcode-4.5")),
        ("chat_without_model", true, None, None),
        ("build_with_model", false, Some("weepcode-4.5"), None),
...
fn chat_new_session_model_state_matrix() {
    ...
    let cases: &[(&str, acp::SessionModelState, Option<&str>, &str)] = &[
        (
            ...
            state_with("auto", &["auto", "weepcode-4"]),
            Some("weepcode-4"),
            "weepcode-4",
        ),
        (
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = weepcode_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = weepcode_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(weepcode_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<weepcode_workspace::permission::PermissionEvent>();
    agent
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::WEEPCODE_DISABLE_CUSTOM_BRIDGE_ENV);
    run_local_for_bridge_test(|| async {
        let agent = build_minimal_agent_for_tests();
        let params = serde_json::json!({ "sessionId" : "sess-local" });
        let err = agent
            .ext_method(acp::ExtRequest::new(
                "weepcode/rewind/points",
                std::sync::Arc::from(serde_json::value::to_raw_value(&params).unwrap()),
            ))
            .await
...
/// `test_cancel_ends_in_flight_turn_and_frees_slot` (weepcode-agent-sdk).
#[test]
...
fn make_live_session_handle(
    ...
    let mut handle = make_test_handle("test-model", false, Some("weepcode-tui"));
...
async fn drive_disconnect_many(agent: &MvpAgent, sids: &[&acp::SessionId]) {
    ...
    agent
        .ext_notification(acp::ExtNotification::new(
            "weepcode/internal/evict_sessions",
            raw.into(),
        ))
        .await
...
fn build_agent_with_gateway_rx() -> (
    MvpAgent,
    tokio::sync::mpsc::UnboundedReceiver<weepcode_acp_lib::AcpClientMessage>,
) {
    use crate::agent::config::Config as AgentConfig;
    use crate::auth::{AuthManager, WeepcodeComConfig};
    let temp_dir = tempfile::tempdir().unwrap();
...
async fn answer_folder_trust_request(
    gw_rx: &mut tokio::sync::mpsc::UnboundedReceiver<weepcode_acp_lib::AcpClientMessage>,
    outcome: &str,
) -> serde_json::Value {
    ...
    let weepcode_acp_lib::AcpClientMessage::ExtMethod(args) = msg else {
        panic!("expected an ext_method reverse-request, got a different message");
    };
    assert_eq!(args.request.method.as_ref(), "weepcode/folder_trust/request");
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use weepcode_test_support::EnvGuard;
    use weepcode_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
    let _sim = EnvGuard::set(weepcode_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("WEEPCODE_FOLDER_TRUST");
...
fn interactive_trust_prompt_client_error_fails_closed() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        let msg = tokio::time::timeout(std::time::Duration::from_secs(2), gw_rx.recv())
            ...
            .expect("trust request must be sent")
            .expect("gateway channel open");
        assert!(matches!(msg, weepcode_acp_lib::AcpClientMessage::ExtMethod(_)));
        drop(msg);
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(300), cmd_rx.recv())
                .await
...
fn interactive_trust_prompt_dedups_same_workspace() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(first, Ok(Some(weepcode_acp_lib::AcpClientMessage::ExtMethod(_)))),
...
fn interactive_trust_prompt_reprompts_after_untrust() {
    use weepcode_test_support::EnvGuard;
    use weepcode_hooks_plugins_types::HooksAction;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("WEEPCODE_HOME", home.path());
    let _sim = EnvGuard::set(weepcode_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("WEEPCODE_FOLDER_TRUST");
    ...
    run_local_for_bridge_test(|| async {
        ...
        assert!(
            matches!(
                ...
                Ok(Some(weepcode_acp_lib::AcpClientMessage::ExtMethod(_)))
            ),
...
mod direct_hub_cloud_removed {
    ...
    fn cloud_server_id_meta_is_hard_error() {
        let meta = serde_json::json!({ "weepcode/cloud_server_id" : "srv-123" });
    ...
    fn cloud_server_id_null_still_present_is_hard_error() {
        let meta = serde_json::json!({ "weepcode/cloud_server_id" : null });
        let err = reject_direct_hub_cloud_meta(meta.as_object()).expect_err("must reject");
        assert_direct_hub_error(err);
    }
    #[test]
    fn cloud_server_id_with_gateway_meta_still_hard_error() {
        let meta = serde_json::json!(
            { "weepcode/cloud_server_id" : "srv-legacy", "envId" : "env-1",
            "weepcode/cloud_existing_workspace" : { "server_id" : "ws-1", "cwd" :
            "/workspace" } }
        );
    ...
    async fn emit_settings_update_carries_permission_mode_from_cfg() {
        ...
        local
            .run_until(async {
                ...
                let weepcode_acp_lib::AcpClientMessage::ExtNotification(args) = msg else {
                    panic!("expected ExtNotification, got {msg:?}");
                };
                assert_eq!(args.request.method.as_ref(), "weepcode/settings/update");
