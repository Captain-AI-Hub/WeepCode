...
impl RelayConfig {
    ...
    /// (`is_weepcode_auth`: x.ai-issuer OIDC or external credential) with a
    ...
    pub(crate) fn for_session(
        session: &WeepcodeAuth,
        ctx: &WeepcodeComConfig,
        alpha_test_key: Option<String>,
        auth_manager: Option<Arc<AuthManager>>,
    ) -> Option<Self> {
        if !session.is_weepcode_auth() || session.key.is_empty() {
            return None;
        }
        let _ = alpha_test_key;
        Some(Self {
            ws_url: ctx.weepcode_ws_url.clone(),
            ws_origin: ctx.weepcode_ws_origin.clone(),
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `weepcode login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
    ...
    loop {
        ...
        match connect_to_relay(&config, proxy_url.as_deref(), &cancel).await {
            Ok(ws) => {
                ...
                tprintln!("Disconnected from WeepCode WebSocket server");
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(weepcode_version::VERSION),
    );
    req.headers_mut().insert(
    ...
    let read_from_ws = async move {
        loop {
            tokio::select! {
                ...
                weepcode_telemetry::unified_log::warn("relay: read liveness timeout, reconnecting",
...
mod tests {
    ...
    /// Helper to create a test WeepcodeAuth with the given key.
    fn test_auth(key: &str) -> WeepcodeAuth {
        WeepcodeAuth {
            key: key.to_string(),
    ...
    #[test]
    fn for_session_builds_only_for_weepcode_issuer() {
        use crate::auth::WEEPCODE_OAUTH2_ISSUER;
        let cfg = WeepcodeComConfig::default();
        let builds = |a: &WeepcodeAuth| RelayConfig::for_session(a, &cfg, None, None).is_some();
        let xai = WeepcodeAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(WEEPCODE_OAUTH2_ISSUER.to_string()),
            ..test_auth("weepcode-bearer")
        };
        assert!(xai.is_weepcode_auth(), "precondition: is_weepcode_auth");
        assert!(builds(&xai));
        let external_xai = WeepcodeAuth {
            auth_mode: AuthMode::External,
            oidc_issuer: Some(WEEPCODE_OAUTH2_ISSUER.to_string()),
            ..test_auth("ext-bearer")
        };
        assert!(
            external_xai.is_weepcode_auth(),
            "precondition: is_weepcode_auth"
        );
        assert!(builds(&external_xai));
        assert!(!builds(&WeepcodeAuth {
            key: String::new(),
            ..xai.clone()
        }));
        assert!(!builds(&WeepcodeAuth {
    ...
    /// Helper: write a WeepcodeAuth to disk under the given scope.
    fn write_test_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &WeepcodeAuth) {
    ...
    async fn auth_recovery_refreshes_and_heals_missing_auth_json() {
        ...
        let cfg = crate::auth::WeepcodeComConfig::default();
        let scope = cfg.auth_scope();
        let am = Arc::new(
            AuthManager::new(dir.path(), cfg.clone()).with_proxy_base_url("http://127.0.0.1:1"),
        );
        let expired_session = WeepcodeAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(WEEPCODE_OAUTH2_ISSUER.to_string()),
    ...
    async fn attempt_auth_recovery_same_key_backs_off_without_cancel() {
        ...
        let fresh_session = WeepcodeAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(WEEPCODE_OAUTH2_ISSUER.to_string()),
