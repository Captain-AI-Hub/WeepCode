...
use crate::auth::{AuthManager, AuthMode, WeepcodeAuth, run_auth_flow};
...
fn spawn_agent_local(
    ...
    tokio::task::spawn_local(
        GatewayReceiver::new(gw_rx, conn)
            .with_on_meta(weepcode_file_utils::trace_context::span_from_meta_traceparent)
            .run(),
    );
    handle_io
}
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = weepcode_agent::prompt::workspace_user::optional_workspace_user_dir();
    ...
    tokio::spawn(async move {
        while skills_rx.recv().await.is_some() {
            ...
            let line = internal_reload_request_line(
                "skills-reload",
                "weepcode/internal/reload_skills",
                serde_json::json!({}),
            );
...
/// ([`weepcode_fsnotify::shared`]) run their event loops on a runtime that outlives
...
fn register_fs_watch_runtime() {
    weepcode_fsnotify::set_runtime_handle(tokio::runtime::Handle::current());
}
...
pub async fn run_stdio_agent(
    ...
    weepcode_telemetry::unified_log::set_version(weepcode_version::VERSION);
    ...
    weepcode_file_utils::queue::cleanup_orphaned_uploads(
        &weepcode_home::weepcode_home(),
        weepcode_file_utils::queue::DEFAULT_MAX_AGE,
    );

    // Log the client that launched us (set by weepcode-desktop when spawning `weepcode agent stdio`).
    // This appears early in unified.jsonl and is extremely useful for auth diagnostics.
    if let Ok(version) = std::env::var("WEEPCODE_CLIENT_VERSION") {
        crate::unified_log::info(
            "WEEPCODE_CLIENT_VERSION",
            None,
    ...
    // blocking stdin reads (see `weepcode_acp_lib::spawn_stdin_line_reader`): on
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `weepcode agent stdio` is a local/interactive entrypoint (spawned by
            // weepcode-desktop), so it needs the gate like the leader and pager paths;
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    weepcode_telemetry::unified_log::set_version(weepcode_version::VERSION);
    // `weepcode agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `weepcode agent stdio` stays interactive.
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a grok.com session. \
        Run `weepcode login` to sign in, or use `weepcode agent stdio` for API-key access.";
    ...
    let ctx = &agent_config.weepcode_com_config;
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!("Session expired. Please run 'weepcode login' to re-authenticate.")
            }
            None => anyhow::bail!("No cached credentials found. Run `weepcode login`."),
        }
        ...
        // already mints external-provider creds, so it would run the provider twice.
        let auth_manager = Arc::new(AuthManager::new(
            &weepcode_home::weepcode_home(),
            ctx.clone(),
        ));
        if crate::agent::auth_method::has_weepcode_api_key_env()
    ...
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow && !no_browser {
            ...
            eprintln!();
            let url_for_open = weepcode_code_url.clone();
...
fn should_seed_shared_session(existing: Option<&WeepcodeAuth>, session: &WeepcodeAuth) -> bool {
    match existing {
        None => true,
        Some(existing) => {
...
///
/// * `relay_on_demand == false` (default — explicit `weepcode agent leader`
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: weepcode_version::VERSION.to_string(),
    })
    ...
    let auth: Option<WeepcodeAuth> = crate::auth::try_ensure_session_noninteractive(ctx).await;

    let auth_for_prefetch: Option<WeepcodeAuth> = auth.clone();
    let endpoints_for_prefetch = agent_config.endpoints.clone();
    ...
    local_set
        .run_until(async move {
            ...
            // `weepcode agent leader` (devbox / systemd) has no local IPC clients
            ...
            let auth_scope = agent_config.weepcode_com_config.auth_scope();
            // Gated on user_weepcode_home() so a cwd-relative .weepcode/auth.json is never
            // read as the user auth store when no home resolves.
            let initial_auth_key_hash = weepcode_config::user_weepcode_home()
                    ...
                    match update {
                        ...
                        ConfigUpdate::ProjectMcpServersChanged { cwd } => {
                            ...
                            let line = internal_reload_request_line(
                                ...
                                "weepcode/internal/reload_project_mcp_servers",
                        ...
                        ConfigUpdate::ModelsChanged => {
                            ...
                            let line = internal_reload_request_line(
                                ...
                                "weepcode/internal/reload_models",
                                serde_json::json!({}),
                            );
                            let mut tx = acp_tx_for_config.lock().await;
                            if let Err(e) = tx.write_all(line.as_bytes()).await {
                        ...
                        ConfigUpdate::ModelsCacheChanged => {
                            // External write to ~/.weepcode/models_cache.json
                            // (another weepcode process fetched a fresher /v1/models
                            ...
                            let line = internal_reload_request_line(
                                ...
                                "weepcode/internal/reload_models_cache",
                                serde_json::json!({}),
                            );
                            let mut tx = acp_tx_for_config.lock().await;
                            if let Err(e) = tx.write_all(line.as_bytes()).await {
                        ...
                        ConfigUpdate::Ui {
                            ...
                            let notification = serde_json::json!({
                                "jsonrpc": "2.0",
                                "method": "weepcode/config_changed",
                                "params": {
                                    "section": "ui",
                                    "changes": {
                                        "theme": theme,
...
mod tests {
    ...
    fn oidc_session(key: &str, create_time: chrono::DateTime<chrono::Utc>) -> WeepcodeAuth {
        WeepcodeAuth {
            key: key.into(),
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(crate::auth::WEEPCODE_OAUTH2_ISSUER.to_string()),
    ...
    fn test_relay_config(addr: std::net::SocketAddr) -> crate::agent::relay::RelayConfig {
        ...
        let cfg = crate::auth::WeepcodeComConfig {
            weepcode_ws_url: format!("ws://{addr}"),
            weepcode_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
    ...
    /// `weepcode agent leader` (devbox/systemd — no local IPC clients,
        ...
        local
            .run_until(async {
                let slot = spawn_leader_relay(
                    config,
                    false, // eager: explicit `weepcode agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
