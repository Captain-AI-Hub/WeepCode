...
pub(super) fn parse_queue_edit_command(
    ...
    match method {
        ...
        "weepcode/queue/clear" => Some(SessionCommand::ClearQueue { owner }),
...
mod tests {
    ...
    fn parse_queue_edit_command_maps_each_method() {
        ...
        match parse_queue_edit_command("weepcode/queue/remove", &p, Some("weepcode-tui".into())) {
            Some(SessionCommand::RemoveQueuedPrompt {
                id,
                expected_version,
                owner,
                ...
                assert_eq!(owner.as_deref(), Some("weepcode-tui"));
            }
        ...
        match parse_queue_edit_command("weepcode/queue/remove", &p, None) {
            Some(SessionCommand::RemoveQueuedPrompt {
                expected_version, ..
        ...
        match parse_queue_edit_command("weepcode/queue/reorder", &p, None) {
            Some(SessionCommand::ReorderQueue { ordered_ids }) => {
        ...
        match parse_queue_edit_command("weepcode/queue/edit", &p, Some("weepcode-vscode".into())) {
            Some(SessionCommand::EditQueuedPrompt {
                id,
                new_text,
                editor,
                ...
                assert_eq!(editor.as_deref(), Some("weepcode-vscode"));
            }
        ...
        match parse_queue_edit_command("weepcode/queue/interject", &p, Some("weepcode-tui".into()))
        {
            Some(SessionCommand::InterjectQueuedPrompt {
                id,
                expected_version,
        ...
        match parse_queue_edit_command("weepcode/queue/interject", &p, None) {
        ...
        assert!(
            parse_queue_edit_command("weepcode/queue/interject", &serde_json::json!({}), None)
                .is_none()
        );

        // unknown method → None.
        assert!(
            parse_queue_edit_command("weepcode/queue/bogus", &serde_json::json!({}), None)
                .is_none()
        );
        // remove without id → None (can't target an entry).
        assert!(
            parse_queue_edit_command("weepcode/queue/remove", &serde_json::json!({}), None)
                .is_none()
        );
    }
}
