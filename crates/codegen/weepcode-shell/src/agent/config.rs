...
use crate::auth::{AuthManager, OidcAuthConfig, WeepcodeComConfig};
...
use std::sync::Arc;
use weepcode_agent::prompt::skills::SkillsConfig;
use weepcode_sampler::{AuthScheme, SamplerConfig};
use weepcode_sampling_types::{
...
use weepcode_tools::types::compat::{
...
pub const DEFAULT_AGENT_TYPE: &str = "weepcode-build-plan";
...
/// Default base URL for the public WeepCode API.
pub const WEEPCODE_API_BASE_URL_DEFAULT: &str = "https://api.x.ai/v1";
...
pub struct EndpointsConfig {
    ...
    /// Base URL for the public WeepCode API.
    pub weepcode_api_base_url: String,
    ...
    /// Env: `WEEPCODE_MODELS_BASE_URL`. Enables custom endpoint mode.
    /// List URL defaults to `{models_base_url}/models`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub models_base_url: Option<String>,
    /// Env: `WEEPCODE_MODELS_LIST_URL`. Overrides the default `{base}/models` list URL.
    #[serde(alias = "models_endpoint", skip_serializing_if = "Option::is_none")]
    pub models_list_url: Option<String>,
    /// Env: `WEEPCODE_TRACE_UPLOAD_URL`. Where trace uploads go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_url: Option<String>,
    /// Env: `WEEPCODE_TRACE_UPLOAD_BUCKET`. Direct bucket (`gs://` or `s3://`), bypasses proxy.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_bucket: Option<String>,
    /// Env: `WEEPCODE_TRACE_UPLOAD_REGION`. AWS region (S3 only).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_region: Option<String>,
    /// Env: `WEEPCODE_TRACE_UPLOAD_CREDENTIALS_FILE`. Path to GCS SA key or AWS credentials file.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Env: `WEEPCODE_TRACE_UPLOAD_ENDPOINT_URL`. Custom S3-compatible endpoint.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_endpoint_url: Option<String>,
    /// Env: `WEEPCODE_DEPLOYMENT_KEY`. Management API key for enterprise deployments.
    /// Sent on telemetry and service requests for deployment-level attribution.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub deployment_key: Option<String>,
    /// Env: `WEEPCODE_MANAGED_CONFIG_URL`. Override the managed config endpoint.
    ...
    /// favor of `WEEPCODE_INTERNAL_OTLP_TRACES_ENDPOINT`, and ignored by the internal
    /// pipeline when `WEEPCODE_EXTERNAL_OTEL` is set (the standard `OTEL_*` vars then
    ...
    /// Env: `WEEPCODE_INTERNAL_OTLP_TRACES_ENDPOINT`. Full INTERNAL traces endpoint,
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub weepcode_internal_otlp_traces_endpoint: Option<String>,
    /// Env: `WEEPCODE_INTERNAL_OTLP_HEADERS`. `k=v,k2=v2` extra headers for the
    /// internal export (debug). Wins over the legacy `OTEL_EXPORTER_OTLP_HEADERS`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub weepcode_internal_otlp_headers: Option<String>,
    /// External-OTEL master switch, captured at construction via
    /// [`external_otel_master_switch_resolved`] — the same layered resolution
    /// (requirement pin > `WEEPCODE_EXTERNAL_OTEL` env > `[telemetry].otel_enabled`
...
pub(crate) fn default_asset_server_url() -> String {
    std::env::var("WEEPCODE_ASSET_SERVER_URL")
...
/// format, shared with `WEEPCODE_INTERNAL_OTLP_HEADERS`): split on `,`,
...
impl EndpointsConfig {
    ...
    /// the public default. NEVER falls back to `weepcode_api_base_url` — that is the
    ...
    /// cli-chat-proxy, never `weepcode_api_base_url`.
    pub fn resolve_trace_upload_url(&self) -> String {
        blank_as_unset(&self.trace_upload_url).unwrap_or_else(|| self.proxy_url())
    }
    /// Managed deployment-config URL (`weepcode setup`): explicit `managed_config_url`,
    /// else `proxy_url` + `/deployment/config`. Never `weepcode_api_base_url`, so the
    ...
    /// 1. `weepcode_internal_otlp_traces_endpoint` (verbatim)
    ...
    /// 3. `proxy_url` + `/traces`.
    /// Uses the proxy default (not the `weepcode_api_base_url` fallback) so
    /// telemetry reports to WeepCode even when inference is overridden. When the
    ...
    pub fn resolve_otlp_traces_endpoint(&self) -> String {
        if let Some(full) = blank_as_unset(&self.weepcode_internal_otlp_traces_endpoint) {
            ...
            && let Some(legacy) = self.legacy_internal_otlp_traces_endpoint()
        {
            tracing::warn!(
                "Repointing the internal trace pipeline via OTEL_EXPORTER_OTLP_ENDPOINT / \
                 ...
                 WEEPCODE_INTERNAL_OTLP_TRACES_ENDPOINT instead — the standard OTEL_* vars will \
    ...
    /// Extra headers for the INTERNAL export: `weepcode_internal_otlp_headers`
    ...
    pub fn resolve_otlp_headers(&self) -> Vec<(String, String)> {
        if let Some(headers) = blank_as_unset(&self.weepcode_internal_otlp_headers) {
    ...
    /// `weepcode_internal_otlp_*` override shadowed that half.
    ///
    ...
    pub fn internal_otlp_consumed_standard_vars(&self) -> bool {
        ...
        let endpoint_consumed = blank_as_unset(&self.weepcode_internal_otlp_traces_endpoint)
            .is_none()
            && self.legacy_internal_otlp_traces_endpoint().is_some();
        let headers_consumed = blank_as_unset(&self.weepcode_internal_otlp_headers).is_none()
    ...
    /// still honored by the internal pipeline even with `WEEPCODE_EXTERNAL_OTEL`
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            cli_chat_proxy_base_url: std::env::var("WEEPCODE_CLI_CHAT_PROXY_BASE_URL").ok(),
            weepcode_api_base_url: std::env::var("WEEPCODE_WEEPCODE_API_BASE_URL")
                .unwrap_or_else(|_| WEEPCODE_API_BASE_URL_DEFAULT.to_owned()),
            alpha_test_key: None,
            models_base_url: env_string("WEEPCODE_MODELS_BASE_URL"),
            models_list_url: env_string("WEEPCODE_MODELS_LIST_URL"),
            trace_upload_url: env_string("WEEPCODE_TRACE_UPLOAD_URL"),
            trace_upload_bucket: env_string("WEEPCODE_TRACE_UPLOAD_BUCKET"),
            trace_upload_region: env_string("WEEPCODE_TRACE_UPLOAD_REGION"),
            trace_upload_credentials_file: env_string("WEEPCODE_TRACE_UPLOAD_CREDENTIALS_FILE"),
            trace_upload_credentials: None,
            trace_upload_endpoint_url: env_string("WEEPCODE_TRACE_UPLOAD_ENDPOINT_URL"),
            deployment_key: env_string("WEEPCODE_DEPLOYMENT_KEY"),
            managed_config_url: env_string("WEEPCODE_MANAGED_CONFIG_URL"),
            ...
            weepcode_internal_otlp_traces_endpoint: env_string(
                "WEEPCODE_INTERNAL_OTLP_TRACES_ENDPOINT",
            ),
            weepcode_internal_otlp_headers: env_string("WEEPCODE_INTERNAL_OTLP_HEADERS"),
...
pub use weepcode_config_types::{BoolFlag, ConfigSource, LazinessDetectorPerModelConfig, Resolved};
...
pub use weepcode_config::env_bool;
...
fn resolve_compaction_mode_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> weepcode_chat_state::CompactionMode {
    use weepcode_chat_state::CompactionMode;
...
fn resolve_compaction_detail_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> weepcode_chat_state::CompactionDetail {
    use weepcode_chat_state::CompactionDetail;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(weepcode_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: weepcode_tools::types::compat::CompatCell,
...
pub(crate) use weepcode_telemetry::config::env_telemetry_mode;
pub use weepcode_telemetry::config::{TelemetryConfig, TelemetryMode};
...
impl PluginsConfig {
    ...
    /// Native `.weepcode/config.toml` entries already present take precedence:
    ...
    pub fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                weepcode_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
    ...
    pub fn to_discovery_config(&self) -> weepcode_agent::plugins::discovery::DiscoveryConfig {
        weepcode_agent::plugins::discovery::DiscoveryConfig {
...
pub struct ModelsConfig {
    ...
    /// Unset = remote pin, then the client hint / built-in `weepcode-build-0.1`
...
/// (`weepcode workspace` / leader `with_default_hub_url`). Does **not** enable
...
pub struct HubConfig {
    ...
    /// `weepcode workspace start` when the CLI does not pass `--hub-url`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl SandboxSettingsConfig {
    ...
    pub fn resolve_profile(
        ...
        resolve_string_flag(cli_arg, "WEEPCODE_SANDBOX", self.profile.as_deref(), None)
    ...
    pub fn resolve_auto_allow_bash(&self, requirement: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("WEEPCODE_SANDBOX_AUTO_ALLOW_BASH")
...
impl SuggestionsConfig {
    pub fn resolve_enabled(
        ...
        BoolFlag::env("WEEPCODE_SUGGESTIONS")
            .config(self.enabled)
    ...
    pub fn resolve_ai_enabled(
        ...
        BoolFlag::env("WEEPCODE_SUGGESTIONS_AI")
    ...
    pub fn resolve_ai_model(&self) -> String {
        resolve_string_flag(
            None,
            "WEEPCODE_SUGGESTIONS_AI_MODEL",
            self.ai_model.as_deref(),
            None,
        )
        .map(|r| r.value)
        .unwrap_or_else(|| "weepcode-build".to_owned())
    }
}
...
///
/// These supplement the built-in scan locations (`.weepcode/skills/`,
/// `.agents/skills/`, `~/.weepcode/skills/`). They're written by `/import-claude`
...
pub struct Config {
    ...
    /// Warnings from `[model.*]` parsing; surfaced by `weepcode inspect`.
    #[serde(skip)]
    pub model_override_warnings: Vec<super::config_model_override_parse::ModelOverrideWarning>,
    pub weepcode_com_config: WeepcodeComConfig,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// Typed as `WeepcodeComConfig` (same schema) so sub-field typos are caught.
    #[serde(default, skip_serializing)]
    pub auth: Option<WeepcodeComConfig>,
    /// `[desktop]` section — owned by weepcode-desktop (Electron app), opaque to the CLI agent.
    #[serde(default, skip_serializing)]
    ...
    /// `[marketplace]` — also read by `weepcode_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    /// Defaults to reading from WEEPCODE_STORAGE_MODE env var.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<weepcode_agent::config::AgentDefinition>,
    #[serde(skip)]
    pub cli_agent_overrides: CliAgentOverrides,
    /// Whether subagent (task tool) support is enabled. Enabled by default;
    /// disabled only via `WEEPCODE_SUBAGENTS=0` or `[subagents] enabled = false`.
    /// Not remotely gated.
    #[serde(skip)]
    ...
    /// and `.weepcode/roles/*.toml` file discovery.
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, weepcode_subagent_resolution::config::SubagentRole>,
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, weepcode_subagent_resolution::config::SubagentPersona>,
    ...
    pub zdr_video_output_s3:
        Option<weepcode_tools::implementations::weepcode_build::video_gen::ZdrVideoOutputS3Config>,
    ...
    /// Image describe model (`weepcode-build` default via `ModelOverrideConfig::resolve`).
    #[serde(skip)]
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<weepcode_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<weepcode_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub fn apply_to_definition(&self, def: &mut weepcode_agent::config::AgentDefinition) {
    ...
    pub fn apply_to_subagent_definition(&self, def: &mut weepcode_agent::config::AgentDefinition) {
...
pub use weepcode_agent::config::AgentDefinition;
pub use weepcode_agent::config::Effort;
pub use weepcode_agent::config::PermissionMode;
pub use weepcode_shared::ui_config::{ContextualHints, UiConfig};
...
/// ```toml
/// [agent]
/// # Use a named agent (looked up via discovery: .weepcode/agents/, ~/.weepcode/agents/, built-ins)
...
/// 4. `WEEPCODE_AGENT` env var
/// 5. Default `weepcode-build` agent
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct AgentSelectionConfig {
    /// Name of a built-in or discovered agent definition.
    /// Looked up via `weepcode_agent::discovery::by_name_in_cwd()`.
    /// Examples: "weepcode-build", "browser-use", or a custom agent name.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// Path to an agent definition file (.md with YAML frontmatter).
    /// When set, the agent is loaded from this file.
    /// Supports environment variable expansion (e.g., `$HOME/.weepcode/agents/my-agent.md`).
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            weepcode_com_config: WeepcodeComConfig::default(),
            shortcuts: None,
            hints: None,
            ui: UiConfig::default(),
            ...
            client_version: Some(weepcode_version::VERSION.to_string()),
...
impl Config {
    ...
    pub fn create_auth_manager(&self) -> AuthManager {
        AuthManager::new(
            &crate::util::weepcode_home::weepcode_home(),
            self.weepcode_com_config.clone(),
        )
    ...
    pub fn new_from_toml_cfg(raw_config: &toml::Value) -> Result<Self, String> {
        ...
        if config.weepcode_com_config.oidc.is_none() {
            config.weepcode_com_config.oidc = OidcAuthConfig::from_env();
        }
        if config.weepcode_com_config.oidc.is_none() && config.weepcode_com_config.oauth2.is_none()
        {
            config.weepcode_com_config.oauth2 = crate::auth::OAuth2ProviderConfig::from_env();
        }
    ...
    pub fn resolve_subagents(
        ...
        let remote = self
            .remote_settings
            .as_ref()
            .and_then(|settings| settings.subagents_max_depth);
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        let env = std::env::var(crate::config::SubagentsConfig::ENV_MAX_DEPTH).ok();
        let toml_max_depth = ctx
            .raw_config
            .get("subagents")
            .and_then(|section| section.get("max_depth"))
            .and_then(|value| value.as_integer());
        let remote_max_depth = ctx
            .remote_settings
            .and_then(|settings| settings.subagents_max_depth);
        self.subagents_max_depth = crate::config::SubagentsConfig::resolve_max_depth(
            env.as_deref(),
            toml_max_depth,
            remote_max_depth,
        );
        ...
        self.auto_wake_enabled = BoolFlag::env("WEEPCODE_AUTO_WAKE")
    ...
    /// If the TOML contains `[auth]`, copy its contents under `[weepcode_com_config]`.
    /// `[weepcode_com_config]` takes precedence if both are present (explicit wins).
    ///
    /// This lets customers write the shorter `[auth.oidc]` instead of `[weepcode_com_config.oidc]`.
    fn expand_auth_alias(raw_config: &toml::Value) -> toml::Value {
        ...
        {
            if let Some(gcc) = table.get_mut("weepcode_com_config") {
                ...
                table.insert("weepcode_com_config".to_owned(), auth);
            }
        }
        config
    }
    fn apply_env_overrides(&mut self) {
        ...
        if let Some(mode) = env_telemetry_mode("WEEPCODE_TELEMETRY_ENABLED") {
    ...
    /// config.toml key, or `WEEPCODE_TWO_PASS_COMPACTION` env.
    ...
    pub(crate) fn resolve_trace_upload(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEEPCODE_TELEMETRY_TRACE_UPLOAD")
    ...
    pub(crate) fn trace_upload_decision_debug(&self) -> serde_json::Value {
        ...
        serde_json::json!(
            ...
            "in_env_trace_upload" : std::env::var("WEEPCODE_TELEMETRY_TRACE_UPLOAD").ok(),
            "in_env_telemetry_enabled" : std::env::var("WEEPCODE_TELEMETRY_ENABLED").ok(),
    ...
    pub(crate) fn resolve_two_pass_compaction(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEEPCODE_TWO_PASS_COMPACTION")
            .config(self.features.two_pass_compaction)
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    /// Server-side doom-loop check policy (the `x-weepcode-doom-loop-check`
    ...
    /// `WEEPCODE_DOOM_LOOP_RECOVERY` > TOML `enabled` > remote `enabled` >
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<weepcode_sampling_types::DoomLoopRecoveryPolicy> {
        use weepcode_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
            .and_then(|s| s.doom_loop_recovery.as_ref());
        let enabled = BoolFlag::env("WEEPCODE_DOOM_LOOP_RECOVERY")
    ...
    /// Gate first-run auto-registration of the official WeepCode marketplace source.
    /// Precedence: env `WEEPCODE_OFFICIAL_MARKETPLACE_AUTO_REGISTER` > remote settings >
    ...
    pub(crate) fn resolve_official_marketplace_auto_register(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEEPCODE_OFFICIAL_MARKETPLACE_AUTO_REGISTER")
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    pub(crate) fn resolve_lsp_tools(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEEPCODE_LSP_TOOLS")
    ...
    pub(crate) fn resolve_web_fetch(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEEPCODE_WEB_FETCH")
    ...
    pub(crate) fn resolve_ask_user_question(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEEPCODE_ASK_USER_QUESTION")
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use weepcode_tools::implementations::weepcode_build::IMAGE_EDIT_TOOL_NAME;
        ...
        BoolFlag::env("WEEPCODE_IMAGE_EDIT").default(true).resolve()
    }
    ...
    /// Precedence: env `WEEPCODE_IMAGE_GEN_MODEL_OVERRIDE` > `[features]
    /// image_gen_model_override` config > remote settings `image_gen_model_override`.
    /// `None` → default model (`weepcode-imagine-image-quality`).
    pub(crate) fn resolve_image_gen_model_override(&self) -> Option<String> {
        resolve_string_flag(
            None,
            "WEEPCODE_IMAGE_GEN_MODEL_OVERRIDE",
    ...
    pub(crate) fn resolve_goal_classifier_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("WEEPCODE_GOAL_CLASSIFIER")
    ...
    pub(crate) fn resolve_goal_planner_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("WEEPCODE_GOAL_PLANNER")
    ...
    pub(crate) fn resolve_goal_summary_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("WEEPCODE_GOAL_SUMMARY")
    ...
    pub(crate) fn resolve_goal_strategist_every(&self, classifier_max_runs: u32) -> Resolved<u32> {
        Self::resolve_goal_u32(
            "WEEPCODE_GOAL_STRATEGIST_EVERY",
    ...
    pub(crate) fn resolve_goal_use_current_model_only(&self) -> Resolved<bool> {
        BoolFlag::env("WEEPCODE_GOAL_USE_CURRENT_MODEL_ONLY")
    ...
    pub(crate) fn resolve_write_file(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEEPCODE_WRITE_FILE")
    ...
    pub(crate) fn resolve_backend_tools(&self) -> Resolved<bool> {
        BoolFlag::env("WEEPCODE_BACKEND_SEARCH")
            .config(self.features.backend_tools)
            .default(true)
            .resolve()
    }
    /// Resolve the mode (env `WEEPCODE_COMPACTION_MODE` > config > remote settings >
    /// default, unrecognized falling through) and, for `Segments`, attach the
    /// separately-resolved detail level.
    pub(crate) fn resolve_compaction_mode(&self) -> weepcode_chat_state::CompactionMode {
        resolve_compaction_mode_from(
            env_string("WEEPCODE_COMPACTION_MODE").as_deref(),
    ...
    /// Resolve verbatim-input flag: env `WEEPCODE_COMPACTION_VERBATIM_INPUT` > config > remote settings > default `true`.
    pub(crate) fn resolve_compaction_verbatim_input(&self) -> bool {
        BoolFlag::env("WEEPCODE_COMPACTION_VERBATIM_INPUT")
    ...
    /// Precedence: env `WEEPCODE_COMPACTION_DETAIL`, then config
    ...
    fn resolve_compaction_detail(&self) -> weepcode_chat_state::CompactionDetail {
        resolve_compaction_detail_from(
            env_string("WEEPCODE_COMPACTION_DETAIL").as_deref(),
    ...
    pub fn resolve_cancel_rewind(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("WEEPCODE_CANCEL_REWIND")
            .config(self.features.cancel_rewind)
            .feature_flag(ff)
            .default(true)
            .resolve()
    }
    /// Resolve whether to use weepcode's default OAuth2 (WeepCode auth.x.ai).
    ///
    /// Enterprise OIDC (`oidc` in config.toml) always wins — this only gates
    /// the default WeepCode OAuth2 fallback when no enterprise OIDC is configured.
    ///
    /// Priority: `--oauth` > WEEPCODE_OAUTH_ENABLED env > default (true = OAuth).
    pub fn resolve_weepcode_oauth(&self, cli_oidc: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("WEEPCODE_OAUTH_ENABLED")
            .cli(cli_oidc)
            .default(true)
            .resolve()
    }
    ...
    /// narrow non-recursive watches for `<cwd>/` and `<cwd>/.weepcode/`.
    ///
    /// Thin delegate to the canonical
...
///
/// `requirement > cli > env (WEEPCODE_MCP_LIVENESS_WATCHERS) > config >
...
pub fn resolve_mcp_liveness_watchers(
    ...
    BoolFlag::env("WEEPCODE_MCP_LIVENESS_WATCHERS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// `BoolFlag` precedence:
///
/// `requirement > cli > env (WEEPCODE_MCP_AUTO_RESTART) > config >
/// managed > feature_flag > default (true)`.
///
...
/// precedence is single-sourced.
///
/// Recovery is on by default; opt out via `WEEPCODE_MCP_AUTO_RESTART=false`,
...
pub fn resolve_mcp_auto_restart(
    ...
    BoolFlag::env("WEEPCODE_MCP_AUTO_RESTART")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
///
/// `requirement > cli > env (WEEPCODE_MCP_PUSH_SERVER_STATUS) > config >
/// managed > feature_flag > default (true)`.
///
...
pub fn resolve_mcp_push_server_status(
    ...
    BoolFlag::env("WEEPCODE_MCP_PUSH_SERVER_STATUS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// [`resolve_mcp_liveness_watchers`]:
///
/// `requirement > cli > env (WEEPCODE_MCP_RECURSIVE_CONFIG_WATCH) >
...
/// operators flip this flag (e.g. via `WEEPCODE_MCP_RECURSIVE_CONFIG_
...
pub fn resolve_mcp_recursive_config_watch(
    ...
    BoolFlag::env("WEEPCODE_MCP_RECURSIVE_CONFIG_WATCH")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
impl SyncBoolFlag {
    ...
    /// Either-direction env resolver (typically `WEEPCODE_*`). Returns
...
pub fn is_telemetry_disabled_sync() -> bool {
    !SyncBoolFlag::new(telemetry_enabled_from_toml)
        ...
        .enable_env(weepcode_telemetry_env_enabled)
        .resolve()
}
...
pub fn is_error_reporting_disabled_sync() -> bool {
    !SyncBoolFlag::new(error_reporting_enabled_from_toml)
        ...
        .enable_env(|| env_bool("WEEPCODE_ERROR_REPORTING"))
...
/// `WEEPCODE_TELEMETRY_ENABLED` resolved through `TelemetryMode::parse` so the
/// extended string forms (e.g. `"session_metrics"`) are accepted.
fn weepcode_telemetry_env_enabled() -> Option<bool> {
    env_telemetry_mode("WEEPCODE_TELEMETRY_ENABLED").map(|m| !m.is_disabled())
}
/// Load `~/.weepcode/requirements.toml` standalone so the admin pin can beat
/// env vars. The merged config layer can't express that — last-merge-wins
/// loses provenance.
pub(crate) fn read_requirements_toml() -> Option<toml::Value> {
    let path = crate::util::weepcode_home::weepcode_home().join("requirements.toml");
    let content = std::fs::read_to_string(&path).ok()?;
    toml::from_str(&content).ok()
}
/// Resolve the external-OTEL master switch exactly the way the external
/// stream's activation does: **requirement pin > `WEEPCODE_EXTERNAL_OTEL` env >
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        weepcode_config::load_merged_requirements().as_ref(),
        env_bool("WEEPCODE_EXTERNAL_OTEL"),
...
/// by `weepcode setup`) sit under the env vars, requirements pins are applied on
...
pub fn resolve_external_otel_config(
    client: weepcode_telemetry::external::config::ExternalClientInfo,
) -> Option<weepcode_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<weepcode_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| weepcode_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            weepcode_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = weepcode_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub fn apply_external_otel_remote_policy(settings: Option<&crate::util::config::RemoteSettings>) {
    ...
    let policy = weepcode_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        weepcode_telemetry::external::apply_remote_policy(policy);
    }
}
...
fn managed_settings_env_flag(key: &str) -> Option<bool> {
    let path = weepcode_config::claude_managed_settings_path()?;
    let content = std::fs::read_to_string(&path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    weepcode_workspace::permission::resolution::json_env_flag(json.get("env"), key)
}
    ...
    entries
        ...
        .map(|m| {
            ...
            let config = ModelEntryConfig {
                ...
                api_base_url: Some(endpoints.weepcode_api_base_url.clone()),
                name: m.name,
...
pub struct ModelEntryConfig {
    ...
    /// If not set, falls back to env_key, then WEEPCODE_API_KEY.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// If not set, falls back to WEEPCODE_API_KEY.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Can also be set via the `WEEPCODE_MAX_RETRIES` environment variable.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_retries: Option<u32>,
    ...
    /// Without this set, WeepCode API models send args as one delta
...
pub struct ModelInfo {
    ...
    /// Always has a value; defaults to `"weepcode-build-plan"` when the server
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<weepcode_workspace::permission::ClassifierPromptType>,
...
pub struct Features {
    ...
    /// Show a blocking warning when WeepCode starts outside a Git repository.
    /// Default: false. Used as the local fallback when the `non_git_warning` remote settings
    /// flag in `weepcode_build_settings` is absent. When the remote flag is present it takes
    ...
    /// (`image_gen_model_override`) / env / default (`weepcode-imagine-image-quality`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// env (`WEEPCODE_COMPACTION_MODE`). Parsed via `CompactionMode::parse`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_mode: Option<String>,
    /// `none` | `minimal` | `balanced` | `verbose` (default). `None` = defer to
    /// env (`WEEPCODE_COMPACTION_DETAIL`). The `segments` verbatim detail level.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// `WEEPCODE_MCP_AUTO_RESTART` to opt out).
    ///
...
pub struct ResolvedCredentials {
    pub api_key: Option<String>,
    pub base_url: String,
    pub auth_type: weepcode_chat_state::AuthType,
    pub auth_scheme: AuthScheme,
}
...
/// Priority: model api_key/env_key > session token > WEEPCODE_API_KEY.
///
...
pub fn resolve_credentials(model: &ModelEntry, session_key: Option<&str>) -> ResolvedCredentials {
    ...
    let (api_key, base_url, auth_type) = if let Some(key) = model.own_credential() {
        (
            Some(key),
            info.base_url.clone(),
            weepcode_chat_state::AuthType::ApiKey,
        )
    } else if let Some(key) = session_key {
        (
            Some(key.to_owned()),
            info.base_url.clone(),
            weepcode_chat_state::AuthType::SessionToken,
        )
    } else if let Ok(key) = crate::agent::auth_method::read_weepcode_api_key_env() {
        let url = model
            .api_base_url
            .clone()
            .unwrap_or_else(|| info.base_url.clone());
        (Some(key), url, weepcode_chat_state::AuthType::ApiKey)
    } else {
...
/// `disable_api_key_auth` at the credential seam: swap a first-party WeepCode API
/// key for the IdP session (absent => request fails => forces login). BYOK
/// (non-WeepCode `base_url`) is untouched; no-op when the switch is off.
pub fn enforce_disable_api_key_auth(
    ...
    if disable_api_key_auth
        && creds.auth_type == weepcode_chat_state::AuthType::ApiKey
        && crate::util::is_weepcode_api_url(&creds.base_url)
    {
        creds.auth_type = weepcode_chat_state::AuthType::SessionToken;
        creds.api_key = session_key.map(str::to_owned);
        weepcode_telemetry::unified_log::debug(
...
pub use weepcode_telemetry::config::deployment_id_from_key;
...
pub fn try_resolve_model_credentials(
    ...
    enforce_disable_api_key_auth(
        &mut credentials,
        cfg.weepcode_com_config.api_key_auth_disabled(),
        session_key,
    );
    Some(credentials)
}
...
pub fn resolve_aux_model_sampling_config(
    ...
    let weepcode_bearer = session_key
        .map(|s| s.to_owned())
        .or_else(|| crate::agent::auth_method::read_weepcode_api_key_env().ok())
        .or_else(|| endpoints.deployment_key.clone());
    if let Some(bearer) = weepcode_bearer {
...
/// on BYOK / non-proxy routes for internal slugs like `weepcode-build`).
...
pub fn resolve_chat_state_auth_type(
    model_id: &str,
    session_key: Option<&str>,
    fallback: weepcode_chat_state::AuthType,
) -> weepcode_chat_state::AuthType {
...
///
/// * cli-chat-proxy bases get `X-WeepCode-Token-Auth` and
...
pub fn inject_url_derived_headers(
    ...
    if is_first_party_proxy_url(base_url) {
        headers
            .entry("X-WeepCode-Token-Auth".to_string())
            .or_insert_with(|| "weepcode-cli".to_string());
        headers
...
///
/// `is_cli_chat_proxy_url` also accepts loopback hosts (WeepCode local dev and
/// their mock-server tests), but WeepCode must never stamp WeepCode-only auth
...
mod tests {
    ...
    use weepcode_test_support::EnvGuard;
    #[test]
    fn main_cli_tools_override_preserves_profile_injection_policy() {
        ...
        let mut cases = vec![(AgentDefinition::default_weepcode_build(), true)];
    ...
    fn auto_mode_config_parses_from_toml_and_json_equivalently() {
        use weepcode_workspace::permission::ClassifierPromptType;
        let toml_src = r#"
enabled = true
prompt_type = "no_user_tool_prefix"
classifier_model = "weepcode-4.5"
...
"#;
        ...
        let json = serde_json::json!(
            ...
            : "weepcode-4.5", "reasoning_effort" : "low" }
        );
        ...
        for cfg in [&from_toml, &from_json] {
            ...
            assert_eq!(cfg.classifier_model.as_deref(), Some("weepcode-4.5"));
    ...
    fn inject_url_derived_headers_adds_proxy_headers_for_cli_chat_proxy_url() {
        ...
        assert_eq!(
            headers.get("X-WeepCode-Token-Auth").map(String::as_str),
            Some("weepcode-cli")
        );
        assert_eq!(
    ...
    fn inject_url_derived_headers_skips_proxy_headers_for_external_url() {
        ...
        assert!(headers.get("X-WeepCode-Token-Auth").is_none());
        assert!(headers.get("x-authenticateresponse").is_none());
    }
    /// WeepCode: a third-party endpoint on loopback (mock, litellm, ollama)
    /// must never receive WeepCode-only auth headers even though
    /// `is_cli_chat_proxy_url` trusts loopback for WeepCode local dev.
    #[test]
    fn inject_url_derived_headers_skips_proxy_headers_for_loopback_url() {
        for url in [
            ...
            assert!(
                headers.get("X-WeepCode-Token-Auth").is_none(),
                "{url}: X-WeepCode-Token-Auth must not leak to loopback third-party endpoints"
            );
            assert!(
    ...
    fn inject_url_derived_headers_does_not_overwrite_existing_entries() {
        let mut headers = IndexMap::new();
        headers.insert(
            "X-WeepCode-Token-Auth".to_string(),
    ...
    fn finalize_image_describe_sampler_none_uses_active_session_model_not_forced_helper() {
        ...
        assert_ne!(cfg.model, "weepcode-build");
    }
    #[test]
    fn finalize_image_describe_sampler_some_stamps_session_fields() {
        ...
        let aux = SamplerConfig {
            model: "weepcode-build".into(),
            ..Default::default()
        };
        let (model, cfg) =
            finalize_image_describe_sampler_config(Some(aux), &active, Some("cli".into()), Some(7));
        assert_eq!(model, "weepcode-build");
        assert_eq!(cfg.model, "weepcode-build");
        assert_eq!(cfg.client_identifier.as_deref(), Some("cli"));
        assert_eq!(cfg.max_retries, Some(7));
    }
    #[test]
    fn resolve_aux_model_honors_weepcode_build_override() {
        let endpoints = EndpointsConfig::default();
        let mut catalog = IndexMap::new();
        catalog.insert(
            "weepcode-build".to_string(),
            test_model_entry(
    ...
    fn parses_model_api_key() {
        ...
        assert_eq!(model.info.model, "weepcode-4.5");
    ...
    fn sampling_config_uses_fallback_when_no_model_api_key() {
        ...
        let sampling_config = sampling_config_for_model(
            ...
            ResolvedCredentials {
                ...
                auth_type: weepcode_chat_state::AuthType::ApiKey,
    ...
    fn default_models_dual_endpoint_routing() {
        ...
        for (model_id, entry) in default_model_entries(&endpoints) {
            ...
            assert_eq!(
                api_key_creds.base_url, endpoints.weepcode_api_base_url,
    ...
    fn env_keys_resolve_first_set_wins() {
        let keys = EnvKeys::new([
            "WEEPCODE_TEST_ENV_KEY_PRIMARY",
            "WEEPCODE_TEST_ENV_KEY_FALLBACK",
        ]);
        assert_eq!(keys.resolve_value_with(|_| None), None, "none set");
        assert_eq!(
            keys.resolve_value_with(
                |n| (n == "WEEPCODE_TEST_ENV_KEY_FALLBACK").then(|| "from-fallback".into())
            ),
            Some("from-fallback".into())
        );
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "WEEPCODE_TEST_ENV_KEY_PRIMARY" => Some("from-primary".into()),
                "WEEPCODE_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
        ...
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "WEEPCODE_TEST_ENV_KEY_PRIMARY" => Some(String::new()),
                "WEEPCODE_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
                _ => None,
            }),
            Some("from-fallback".into())
    ...
    fn env_keys_resolve_skips_whitespace_only_value() {
        let keys = EnvKeys::new(["WEEPCODE_TEST_WS_PRIMARY", "WEEPCODE_TEST_WS_FALLBACK"]);
        assert_eq!(
            keys.resolve_value_with(|n| match n {
                "WEEPCODE_TEST_WS_PRIMARY" => Some("   ".into()),
                "WEEPCODE_TEST_WS_FALLBACK" => Some("real".into()),
                _ => None,
            }),
            Some("real".into())
        );
        assert_eq!(
            EnvKeys::single("WEEPCODE_TEST_WS_ONLY").resolve_value_with(|_| Some("   ".into())),
            None
        );
        assert_eq!(
            EnvKeys::single("WEEPCODE_TEST_WS_PAD").resolve_value_with(|_| Some("  tok  ".into())),
            Some("  tok  ".into())
        );
    }
    #[test]
    ...
    fn first_own_credential_empty_api_key_falls_through_to_env_key() {
        use weepcode_test_support::EnvGuard;
        let var = "WEEPCODE_TEST_FIRST_OWN_CRED_ENV";
    ...
    fn resolve_credentials_multi_env_key_uses_lc_alias() {
        use weepcode_chat_state::AuthType;
        let primary = "WEEPCODE_TEST_MULTI_ENV_PRIMARY";
        let alias = "WEEPCODE_TEST_MULTI_ENV_LC_ALIAS";
        unsafe {
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_session() {
        use weepcode_chat_state::AuthType;
        use weepcode_test_support::EnvGuard;
        let primary = "WEEPCODE_TEST_EMPTY_ENV_PRIMARY";
        let alias = "WEEPCODE_TEST_EMPTY_ENV_LC_ALIAS";
    ...
    fn resolve_credentials_empty_env_key_falls_through_to_global_key() {
        use crate::agent::auth_method::{
            LEGACY_WEEPCODE_API_KEY_ENV_VAR, WEEPCODE_API_KEY_ENV_VAR,
        };
        use weepcode_chat_state::AuthType;
        use weepcode_test_support::EnvGuard;
        let sentinel = "weepcode-global-sentinel-key";
        let primary = "WEEPCODE_TEST_EMPTY_ENV_GLOBAL_PRIMARY";
        let alias = "WEEPCODE_TEST_EMPTY_ENV_GLOBAL_ALIAS";
        let _primary = EnvGuard::set(primary, "");
        let _alias = EnvGuard::set(alias, "");
        let _global = EnvGuard::set(WEEPCODE_API_KEY_ENV_VAR, sentinel);
        let _legacy = EnvGuard::unset(LEGACY_WEEPCODE_API_KEY_ENV_VAR);
        let mut model = test_model_entry("m", "https://inference.example/v1", None, None, None);
        model.env_key = Some(EnvKeys::new([primary, alias]));
        assert!(!model.has_own_credentials());
        let creds = resolve_credentials(&model, None);
    ...
    fn resolve_credentials_no_session_key_returns_api_key() {
        let model = test_model_entry("m", "https://example.com/v1", None, None, None);
        let creds = resolve_credentials(&model, None);
        assert_eq!(creds.auth_type, weepcode_chat_state::AuthType::ApiKey);
    }
    fn api_key_creds(base_url: &str) -> ResolvedCredentials {
        ResolvedCredentials {
            api_key: Some("weepcode-secret".to_string()),
            base_url: base_url.to_string(),
            auth_type: weepcode_chat_state::AuthType::ApiKey,
            auth_scheme: Default::default(),
        }
    ...
    fn enforce_disable_api_key_auth_blocks_first_party_only() {
        ...
        assert_eq!(creds.auth_type, AuthType::ApiKey);
        assert_eq!(creds.api_key.as_deref(), Some("weepcode-secret"));
        let mut creds = api_key_creds("https://api.x.ai/v1");
    ...
    fn try_resolve_model_credentials_swaps_first_party_own_key_under_kill_switch() {
        ...
        assert_eq!(creds.api_key.as_deref(), Some("weepcode-model-key"));
        enforce_disable_api_key_auth(&mut creds, true, Some("session-jwt"));
        assert_eq!(
            creds.auth_type,
    ...
    fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
        ...
        assert_eq!(config.api_backend, ApiBackend::Messages);
        let client = weepcode_sampler::SamplingClient::new(config).expect("client should build");
    ...
    fn user_override_parses_compaction_at_tokens_from_toml() {
        use weepcode_sampling_types::CompactionAtTokens;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn user_override_parses_compactions_remaining_from_toml() {
        use weepcode_sampling_types::CompactionsRemaining;
        let dm = crate::models::default_model();
        let raw_config: toml::Value = toml::from_str(&format!(
            r#"
            [model."{dm}"]
    ...
    fn compaction_mode_precedence_env_over_config_over_remote_over_default() {
        ...
        assert_eq!(
            ...
            CompactionMode::Segments(weepcode_chat_state::CompactionDetail::default())
        );
        assert_eq!(
    ...
    /// weepcode-shell/src/agent/models.rs:857 for any BYOK Claude config.
    #[test]
    ...
    #[test]
    fn model_agent_type_defaults_to_weepcode_build() {
        let raw_config: toml::Value = toml::from_str(
            r#"
            [model.my-model]
            model = "my-model"
    ...
    fn invalid_glob_is_rejected_by_validation() {
        ...
        assert!(ModelGlobSet::compile(Some(&vec!["weepcode[".to_string()])).is_err());
        let raw: toml::Value = toml::from_str(
            r#"
            [models]
            allowed_models = ["weepcode["]
            "#,
        )
        .unwrap();
    ...
    #[test]
    fn auth_alias_maps_to_weepcode_com_config() {
        let raw: toml::Value = toml::from_str(
            r#"
            [auth.oidc]
        ...
        let oidc = cfg.weepcode_com_config.oidc.expect("oidc should be set");
        assert_eq!(oidc.issuer, "https://example.okta.com");
        assert_eq!(oidc.client_id, "test-id");
    }
    #[test]
    fn weepcode_com_config_still_works() {
        let raw: toml::Value = toml::from_str(
            r#"
            [weepcode_com_config.oidc]
            issuer = "https://example.okta.com"
            client_id = "test-id"
            "#,
        )
    ...
    fn disable_api_key_auth_parses_from_auth_alias() {
        ...
        assert_eq!(absent.weepcode_com_config.disable_api_key_auth, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(cfg.weepcode_com_config.disable_api_key_auth, Some(true));
    }
    ...
    fn force_login_team_uuid_parses_string_and_array() {
        ...
        let absent = Config::new_from_toml_cfg(&toml::from_str("").unwrap()).unwrap();
        assert_eq!(absent.weepcode_com_config.force_login_team_uuid, None);
        let raw: toml::Value = toml::from_str(
            r#"
            [auth]
        ...
        let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
        assert_eq!(
            cfg.weepcode_com_config.force_login_team_uuid,
    ...
    fn force_login_team_uuid_implies_api_key_auth_disabled() {
        use crate::auth::{ForceLoginTeam, WeepcodeComConfig};
        let base = WeepcodeComConfig {
            disable_api_key_auth: None,
            force_login_team_uuid: None,
            ..WeepcodeComConfig::default()
        };
    ...
    fn e2e_config_toml_model_overrides_default() {
        ...
        unsafe { std::env::set_var("WEEPCODE_API_KEY", "weepcode-key") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.base_url, "https://inference.example.com/v1");
        unsafe { std::env::remove_var("WEEPCODE_API_KEY") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.base_url, "https://inference.example.com/v1");
    }
    #[test]
    ...
    fn config_models_default_is_not_overwritten_by_default_models_json() {
        ...
        let resolved = resolve_string_flag(
            None,
            "WEEPCODE_DEFAULT_MODEL_TEST_NONEXISTENT",
            config_default,
    ...
    fn config_models_default_custom_model_is_in_resolved_model_list() {
        ...
        assert!(
            models.contains_key("acme-weepcode"),
            "user-defined model must be in the resolved model list"
        );
        let model = models.get("acme-weepcode").unwrap();
        assert_eq!(model.info.model, "weepcode-4.5");
        assert_eq!(model.info.base_url, "https://inference.example.com/v1");
    }
    #[test]
    ...
    fn e2e_default_model_with_external_api_key_routes_to_api_xai() {
        ...
        let model = models
            .get(crate::models::default_model())
            .expect("default model should exist");
        unsafe { std::env::set_var("WEEPCODE_API_KEY", "weepcode-external-key") };
        let sampling = resolve_sampling(model, None);
        assert_eq!(sampling.api_key.as_deref(), Some("weepcode-external-key"));
        assert_eq!(
    ...
    fn e2e_credential_priority_model_key_beats_session_beats_env() {
        ...
        unsafe { std::env::set_var("WEEPCODE_API_KEY", "env-key") };
    ...
    fn e2e_duplicate_model_field_both_entries_survive() {
        ...
        let user = models.get("acme-weepcode").unwrap();
    ...
    #[test]
    fn e2e_enterprise_custom_endpoint_skips_weepcode_defaults() {
        ...
        assert!(
            ...
            "WeepCode default must not leak into enterprise model list"
        );
    ...
    fn e2e_acp_model_info_no_dedup_on_model_field() {
        ...
        assert!(
            acp_models.contains_key(&acp::ModelId::new("default-weepcode")),
            "default entry should be addressable by map key"
        );
        assert!(
            acp_models.contains_key(&acp::ModelId::new("acme-weepcode")),
    ...
    fn e2e_enterprise_endpoints_plus_partial_model_override() {
        ...
        let (_, models) = resolve_models_from_toml(
            ...
            weepcode_api_base_url = "https://enterprise-api.acme.com/v1"
    ...
    fn e2e_enterprise_endpoints_only_no_model_override() {
        ...
        assert_eq!(
            model.api_base_url.as_deref(),
            Some("https://enterprise-api.acme.com/v1"),
            "default model should use enterprise weepcode_api_base_url"
        );
    }
    ...
    fn unset_endpoint_env_vars() {
        for k in [
            "WEEPCODE_CLI_CHAT_PROXY_BASE_URL",
            "WEEPCODE_WEEPCODE_API_BASE_URL",
            "WEEPCODE_TRACE_UPLOAD_URL",
            "WEEPCODE_MANAGED_CONFIG_URL",
            "WEEPCODE_MODELS_BASE_URL",
            "WEEPCODE_MODELS_LIST_URL",
            "OTEL_EXPORTER_OTLP_ENDPOINT",
            "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT",
            "OTEL_EXPORTER_OTLP_HEADERS",
            "WEEPCODE_INTERNAL_OTLP_TRACES_ENDPOINT",
            "WEEPCODE_INTERNAL_OTLP_HEADERS",
            "WEEPCODE_EXTERNAL_OTEL",
        ] {
            unsafe { std::env::remove_var(k) };
        }
    }
    /// INVARIANT: auxiliary-service resolvers resolve to the cli-chat-proxy, never
    /// `weepcode_api_base_url` — overriding ONLY inference keeps every aux endpoint on
    ...
    fn aux_endpoints_resolve_to_proxy_never_inference() {
        ...
        let cfg = EndpointsConfig {
            weepcode_api_base_url: inference.to_string(),
        ...
        assert_eq!(cfg.weepcode_api_base_url, inference);
    ...
    /// REGRESSION: the managed-config URL never follows `weepcode_api_base_url`
    ...
    fn loader_managed_config_url_never_follows_inference_endpoint() {
        ...
        let cfg = Config::new_from_toml_cfg(
            &toml::from_str(
                r#"[endpoints]
                weepcode_api_base_url = "https://inference.acme-corp.example/xai/v1""#,
            )
            .unwrap(),
        )
    ...
    fn resolve_session_recap_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("WEEPCODE_SESSION_RECAP") };
        let cfg = Config::default();
    ...
    fn resolve_session_recap_env_off_overrides_default() {
        unsafe { std::env::set_var("WEEPCODE_SESSION_RECAP", "0") };
        let cfg = Config::default();
        let r = cfg.resolve_session_recap();
        assert!(!r.value);
    ...
    fn resolve_two_pass_compaction_precedence() {
        unsafe { std::env::remove_var("WEEPCODE_TWO_PASS_COMPACTION") };
        ...
        assert!(r.value);
        assert_eq!(r.source, ConfigSource::Config);
        unsafe { std::env::set_var("WEEPCODE_TWO_PASS_COMPACTION", "0") };
        let r = config_over_remote.resolve_two_pass_compaction();
    ...
    fn resolve_doom_loop_recovery_precedence() {
        ...
        unsafe { std::env::remove_var("WEEPCODE_DOOM_LOOP_RECOVERY") };
        let default_cfg = Config::default();
        assert!(
        ...
        unsafe { std::env::set_var("WEEPCODE_DOOM_LOOP_RECOVERY", "0") };
        assert!(
    ...
    fn resolve_trace_upload_disabled_when_telemetry_off_despite_remote_flag() {
        unsafe { std::env::remove_var("WEEPCODE_TELEMETRY_ENABLED") };
        unsafe { std::env::remove_var("WEEPCODE_TELEMETRY_TRACE_UPLOAD") };
        let mut cfg = Config::default();
    ...
    fn resolve_goal_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("WEEPCODE_GOAL") };
        let cfg = Config::default();
    ...
    fn resolve_goal_env_overrides_config() {
        unsafe { std::env::set_var("WEEPCODE_GOAL", "1") };
        let mut cfg = Config::default();
    ...
    fn resolve_ask_user_question_defaults_to_true_when_unset() {
        unsafe { std::env::remove_var("WEEPCODE_ASK_USER_QUESTION") };
        let cfg = Config::default();
    ...
    fn resolve_ask_user_question_env_overrides_remote_settings() {
        unsafe { std::env::set_var("WEEPCODE_ASK_USER_QUESTION", "1") };
        let cfg = Config {
            remote_settings: Some(crate::util::config::RemoteSettings {
    ...
    fn resolve_image_gen_model_override_remote_settings_or_config() {
        unsafe { std::env::remove_var("WEEPCODE_IMAGE_GEN_MODEL_OVERRIDE") };
        ...
        assert_eq!(
            with(None, Some("weepcode-imagine-image")).resolve_image_gen_model_override(),
            Some("weepcode-imagine-image".to_owned())
        );
        assert_eq!(
            with(
                Some("weepcode-imagine-image-pro"),
                Some("weepcode-imagine-image")
            )
            .resolve_image_gen_model_override(),
            Some("weepcode-imagine-image-pro".to_owned())
        );
    }
    #[test]
    #[serial]
    fn imagine_tools_disabled_gates_image_edit() {
        unsafe { std::env::remove_var("WEEPCODE_IMAGE_EDIT") };
        ...
        unsafe { std::env::set_var("WEEPCODE_IMAGE_EDIT", "1") };
    ...
    fn clear_goal_envs() {
        unsafe {
            std::env::remove_var("WEEPCODE_GOAL");
            std::env::remove_var("WEEPCODE_GOAL_CLASSIFIER");
            std::env::remove_var("WEEPCODE_GOAL_PLANNER");
            std::env::remove_var("WEEPCODE_GOAL_SUMMARY");
            std::env::remove_var("WEEPCODE_GOAL_VERIFIER_N");
            std::env::remove_var("WEEPCODE_GOAL_CLASSIFIER_MAX");
            std::env::remove_var("WEEPCODE_GOAL_STRATEGIST_EVERY");
            std::env::remove_var("WEEPCODE_GOAL_REVERIFY_AFTER");
        }
    }
    ...
    fn resolve_goal_classifier_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("WEEPCODE_GOAL_CLASSIFIER", "0") };
        let r = cfg_with_goal_and_remote(true, remote_classifier(true))
            .resolve_goal_classifier_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("WEEPCODE_GOAL_CLASSIFIER", "1") };
    ...
    fn resolve_goal_planner_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("WEEPCODE_GOAL_PLANNER", "0") };
        let r =
            cfg_with_goal_and_remote(true, remote_planner(true)).resolve_goal_planner_enabled(true);
        assert!(!r.value);
        assert_eq!(r.source, ConfigSource::Env);
        unsafe { std::env::set_var("WEEPCODE_GOAL_PLANNER", "1") };
    ...
    fn resolve_goal_summary_env_overrides_default_and_remote() {
        clear_goal_envs();
        unsafe { std::env::set_var("WEEPCODE_GOAL_SUMMARY", "0") };
        let r =
...
"#,
    ...
    const GOAL_USE_CURRENT_ENV: &str = "WEEPCODE_GOAL_USE_CURRENT_MODEL_ONLY";
...
[goal]
enabled = true
planner_model = { model = "weepcode-build", agent_type = "weepcode-build-plan" }

[goal.strategist_model]
model = "weepcode-composer-2.5-fast"
agent_type = "cursor"

[[goal.skeptic_models]]
model = "weepcode-build"
agent_type = "weepcode-build-plan"
...
"#;
        ...
        assert_eq!(cfg.goal.skeptic_models[0].model, "weepcode-build");
        assert_eq!(
...
planner_model = { agent_type = "weepcode-build-plan" }
"#;
        let raw: toml::Value = toml::from_str(toml_str).unwrap();
        let cfg = Config::new_from_toml_cfg(&raw)
...
classifier_max_runs = 6
planner_model = { model = "weepcode-build", agent_type = "weepcode-build-plan" }
strategist_model = { model = "weepcode-composer-2.5-fast", agent_type = "cursor" }
...
"#,
        ...
        let weepcode_build = crate::util::config::GoalRoleModel {
            model: "weepcode-build".into(),
            agent_type: "weepcode-build-plan".into(),
        };
        let composer = crate::util::config::GoalRoleModel {
            model: "weepcode-composer-2.5-fast".into(),
        ...
        assert_eq!(
            planner.value,
            GoalRoleModelChoice::Explicit(weepcode_build.clone())
        );
        ...
        assert_eq!(
            ...
            vec![
                GoalRoleModelChoice::Explicit(weepcode_build),
    ...
    fn config_warns_on_section_typo() {
        let raw: toml::Value = toml::from_str(
            r#"
            [endpoint]
            deployment_key = "weepcode-token-test"
        "#,
        )
        .unwrap();
    ...
    fn internal_otlp_test_config() -> EndpointsConfig {
        EndpointsConfig {
            ...
            weepcode_internal_otlp_traces_endpoint: None,
            weepcode_internal_otlp_headers: None,
            external_otel_master_switch: false,
            ..Default::default()
        }
    }
    /// `weepcode_internal_otlp_traces_endpoint` wins over the legacy `OTEL_*`
    /// fields regardless of the master switch.
    #[test]
    fn internal_otlp_endpoint_weepcode_internal_wins_regardless_of_switch() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                weepcode_internal_otlp_traces_endpoint: Some(
            ...
            assert_eq!(
                ...
                "switch={switch}: WEEPCODE_INTERNAL_OTLP_TRACES_ENDPOINT must win verbatim (trailing / trimmed)"
            );
        }
    }
    ...
    fn internal_otlp_consumed_standard_vars_cases() {
        ...
        for case in cases {
            let cfg = EndpointsConfig {
                ...
                weepcode_internal_otlp_traces_endpoint: case
                    .internal_ep
                    .then(|| "https://internal.example/traces".to_string()),
                weepcode_internal_otlp_headers: case.internal_headers.then(|| "ik=iv".to_string()),
                ..internal_otlp_test_config()
            };
            assert_eq!(
    ...
    /// Headers precedence: `weepcode_internal_otlp_headers` wins; legacy
    /// `otel_exporter_otlp_headers` only when the master switch is unset.
    #[test]
    fn internal_otlp_headers_precedence() {
        for switch in [false, true] {
            let cfg = EndpointsConfig {
                weepcode_internal_otlp_headers: Some("x-debug=1".to_string()),
    ...
    fn ext_client() -> weepcode_telemetry::external::config::ExternalClientInfo {
        weepcode_telemetry::external::config::ExternalClientInfo::default()
    }
    #[test]
    fn external_otel_default_off_and_double_opt_in() {
        ...
        assert!(
            resolve_external_otel_config_with(
                None,
                None,
                ext_env(&[("WEEPCODE_EXTERNAL_OTEL", "1")]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    fn external_otel_file_table_layered_under_env() {
        ...
        assert!(
            resolve_external_otel_config_with(
                Some(&effective),
                None,
                ext_env(&[("WEEPCODE_EXTERNAL_OTEL", "0")]),
                ext_client(),
                false,
            )
            .is_none()
    ...
    /// (managed config / requirements — no `WEEPCODE_EXTERNAL_OTEL` env var) must
    ...
    fn clear_runtime_env_vars() {
        unsafe {
            std::env::remove_var("WEEPCODE_SUBAGENTS");
            std::env::remove_var("WEEPCODE_RESPECT_GITIGNORE");
            std::env::remove_var("WEEPCODE_WEB_SEARCH_MODEL");
            std::env::remove_var("WEEPCODE_SESSION_SUMMARY_MODEL");
            std::env::remove_var("WEEPCODE_CURSOR_SKILLS_ENABLED");
            std::env::remove_var("WEEPCODE_CURSOR_RULES_ENABLED");
            std::env::remove_var("WEEPCODE_CURSOR_AGENTS_ENABLED");
            std::env::remove_var("WEEPCODE_CLAUDE_SKILLS_ENABLED");
            std::env::remove_var("WEEPCODE_CLAUDE_RULES_ENABLED");
            std::env::remove_var("WEEPCODE_CLAUDE_AGENTS_ENABLED");
        }
    }
    fn clear_managed_mcp_env_vars() {
        unsafe {
            std::env::remove_var("WEEPCODE_MANAGED_MCPS_ENABLED");
            std::env::remove_var("WEEPCODE_MANAGED_MCP_GATEWAY_TOOLS_ENABLED");
        }
    }
...
"#,
    ...
    fn resolve_raw_compat_sessions_load_failure_allows_env_override() {
        let _env = isolate_compat_env();
        let _codex = EnvGuard::set("WEEPCODE_CODEX_SESSIONS_ENABLED", "true");
        let resolved = resolve_compat_sessions_from_raw(Err(()), None);
        assert!(!resolved.cursor.sessions);
        assert!(!resolved.claude.sessions);
        assert!(resolved.codex.sessions);
    ...
    fn resolve_compat_env_sessions_disable_independently() {
        ...
        for (vendor, env_var) in [
            (CompatVendor::Cursor, "WEEPCODE_CURSOR_SESSIONS_ENABLED"),
            (CompatVendor::Claude, "WEEPCODE_CLAUDE_SESSIONS_ENABLED"),
            (CompatVendor::Codex, "WEEPCODE_CODEX_SESSIONS_ENABLED"),
        ] {
    ...
    fn resolve_compat_precedence_and_reserved_codex_hook() {
        ...
        let _session = EnvGuard::set("WEEPCODE_CURSOR_SESSIONS_ENABLED", "true");
        let _hook = EnvGuard::set("WEEPCODE_CODEX_HOOKS_ENABLED", "true");
        let resolved = resolve_compat_config(&config, Some(&remote));
        assert!(resolved.cursor.sessions);
        assert!(resolved.codex.hooks);
    }
    ...
    fn resolve_runtime_fields_compat_asymmetric_sources() {
        let _env = isolate_compat_env();
        let _cursor = EnvGuard::set("WEEPCODE_CURSOR_SESSIONS_ENABLED", "false");
        let raw: toml::Value =
    ...
    fn resolve_runtime_fields_gitignore_from_env() {
        clear_runtime_env_vars();
        unsafe { std::env::set_var("WEEPCODE_RESPECT_GITIGNORE", "0") };
        let raw = empty_config();
        let mut cfg = Config::new_from_toml_cfg(&raw).unwrap();
        cfg.resolve_runtime_fields(&RuntimeResolutionContext {
            raw_config: &raw,
...
"#,
    ...
    fn is_telemetry_explicitly_disabled_sync_env_signals() {
        unsafe { std::env::set_var("WEEPCODE_TELEMETRY_ENABLED", "0") };
        unsafe { std::env::remove_var("DISABLE_TELEMETRY") };
        assert!(is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::set_var("WEEPCODE_TELEMETRY_ENABLED", "1") };
        assert!(!is_telemetry_explicitly_disabled_sync());
        unsafe { std::env::remove_var("WEEPCODE_TELEMETRY_ENABLED") };
    ...
    fn version_overrides_apply_into_typed_config() {
        let mut value: toml::Value = toml::from_str(
            r#"
[models]
default = "weepcode-build"

[[version_overrides]]
minimum_version = "1.8.0"
[version_overrides.models]
default = "weepcode-4.5"
"#,
        )
        .unwrap();
        let v = semver::Version::parse("1.8.0").unwrap();
        weepcode_config::apply_version_overrides(&mut value, &v).unwrap();
        let cfg = Config::new_from_toml_cfg(&value).unwrap();
        assert_eq!(cfg.models.default.as_deref(), Some("weepcode-4.5"));
    }
    /// Reproduce the enterprise managed config bug: [model.weepcode-build] sets
    /// context_window=500k for model="weepcode-4.5", but
    /// [models].default="weepcode-4.5" resolves to the bare
    /// prefetched entry (256k) because Layer 3 only overrides key
    /// "weepcode-build", not key "weepcode-4.5".
    ///
    ...
    fn slug_propagation_enterprise_managed_config_key_mismatch() {
        ...
        prefetched.insert("weepcode-4.5".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let by_key = resolved
            .get("weepcode-build")
            .expect("weepcode-build key must exist");
        assert_eq!(by_key.info.context_window.get(), 500_000);
        assert_eq!(by_key.info.model, "weepcode-4.5");
        let by_latest = resolved
            .get("weepcode-4.5")
            .expect("weepcode-4.5 key must exist");
        assert_eq!(
            by_latest.info.context_window.get(),
            500_000,
            "BUG: prefetched 'weepcode-4.5' should inherit 500k from \
             sibling 'weepcode-build' (same model slug), not stay at {default_cw}"
        );
    }
    ...
    fn slug_propagation_inherits_api_backend_but_not_agent_type() {
        ...
        prefetched.insert("weepcode-4.5".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let latest = resolved.get("weepcode-4.5").unwrap();
        assert_eq!(
    ...
    fn resolve_model_list_config_reasoning_efforts_beats_remote() {
        ...
        .unwrap();
        let cfg = Config::new_from_toml_cfg(&raw_config).expect("config should parse");
        let mut entry = prefetch_model_entry("weepcode-x", 200_000, ApiBackend::default());
        entry.info.reasoning_efforts = vec![ReasoningEffortOption {
            id: "high".to_string(),
            value: ReasoningEffort::High,
            label: "High".to_string(),
        ...
        let mut prefetched = IndexMap::new();
        prefetched.insert("weepcode-x".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
    ...
    fn resolve_model_list_inherits_context_window_from_default_when_prefetched_has_fallback() {
        let cfg = Config::default();
        let default_cw = DEFAULT_CONTEXT_WINDOW;
        let entry = prefetch_model_entry("weepcode-build", default_cw, ApiBackend::default());
        let mut prefetched = IndexMap::new();
        prefetched.insert("weepcode-build".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let entry = resolved.get("weepcode-build").expect("model must exist");
        assert_ne!(
    ...
    fn resolve_model_list_does_not_override_explicitly_set_context_window() {
        ...
        let entry = prefetch_model_entry("weepcode-build", explicit_cw, ApiBackend::default());
        let mut prefetched = IndexMap::new();
        prefetched.insert("weepcode-build".to_owned(), entry);
        let resolved = resolve_model_list(&cfg, Some(prefetched));
        let entry = resolved.get("weepcode-build").expect("model must exist");
    ...
    fn resolve_model_list_inherits_agent_type_and_api_backend() {
        ...
        if let Some(default) = defaults.get("weepcode-build") {
    ...
    fn resolve_model_list_prunes_bundled_entries_not_in_prefetch() {
        ...
        if let Some(e) = defs.shift_remove("weepcode-build") {
            p.insert("weepcode-build".to_string(), e);
        }
        let resolved = resolve_model_list(&cfg, Some(p));
        assert!(resolved.contains_key("weepcode-build"));
        let no_p = resolve_model_list(&cfg, None);
        assert!(no_p.contains_key("weepcode-build"));
    }
    #[test]
    ...
    fn resolve_model_list_prefetch_replaces_bundled_entirely() {
        let cfg = Config::default();
        let mut p = IndexMap::new();
        let e = prefetch_model_entry("weepcode-4.5", 500_000, ApiBackend::Responses);
        p.insert("weepcode-4.5".to_string(), e);
        let resolved = resolve_model_list(&cfg, Some(p));
        assert!(resolved.contains_key("weepcode-4.5"));
        assert!(!resolved.contains_key("weepcode-build"));
    }
    #[test]
    ...
    /// Regression: enterprise managed config aliases weepcode-build to their own
    /// endpoint with env_key. The bundled weepcode-build has supported_in_api=false.
    ...
    fn byok_config_overlay_visible_to_api_key_users() {
        ...
        let entry = resolved
            .get("weepcode-build")
            .expect("weepcode-build must exist");
        assert!(
    ...
    fn mcp_liveness_watchers_default_is_true() {
        unsafe { std::env::remove_var("WEEPCODE_MCP_LIVENESS_WATCHERS") };
    ...
    fn mcp_liveness_watchers_requirement_wins_over_everything() {
        unsafe { std::env::set_var("WEEPCODE_MCP_LIVENESS_WATCHERS", "true") };
    ...
    fn mcp_liveness_watchers_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("WEEPCODE_MCP_LIVENESS_WATCHERS", "false") };
    ...
    fn mcp_auto_restart_default_is_true() {
        unsafe { std::env::remove_var("WEEPCODE_MCP_AUTO_RESTART") };
    ...
    fn mcp_auto_restart_requirement_wins_over_everything() {
        unsafe { std::env::set_var("WEEPCODE_MCP_AUTO_RESTART", "false") };
    ...
    fn mcp_auto_restart_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("WEEPCODE_MCP_AUTO_RESTART", "true") };
    ...
    fn mcp_push_server_status_default_is_true() {
        unsafe { std::env::remove_var("WEEPCODE_MCP_PUSH_SERVER_STATUS") };
    ...
    fn mcp_push_server_status_requirement_wins_over_everything() {
        unsafe { std::env::set_var("WEEPCODE_MCP_PUSH_SERVER_STATUS", "true") };
    ...
    fn mcp_push_server_status_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("WEEPCODE_MCP_PUSH_SERVER_STATUS", "false") };
    ...
    fn mcp_recursive_config_watch_default_is_true() {
        unsafe { std::env::remove_var("WEEPCODE_MCP_RECURSIVE_CONFIG_WATCH") };
    ...
    fn mcp_recursive_config_watch_requirement_wins_over_everything() {
        unsafe { std::env::set_var("WEEPCODE_MCP_RECURSIVE_CONFIG_WATCH", "true") };
    ...
    fn mcp_recursive_config_watch_env_wins_over_config_and_below() {
        unsafe { std::env::set_var("WEEPCODE_MCP_RECURSIVE_CONFIG_WATCH", "false") };
