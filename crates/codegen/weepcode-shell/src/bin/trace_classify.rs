...
//!       [--api-key <key> | $WEEPCODE_API_KEY | <weepcode-home>/auth.json] \
//!       [--min-confidence 0.7] \
...
use clap::Parser;
use weepcode_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
...
struct Cli {
    ...
    /// has access to.
    #[arg(long, default_value = "weepcode-4.5")]
    model: String,
    ...
    /// API key. Overrides `$WEEPCODE_API_KEY` when set; falls back to
    /// `$WEEPCODE_API_KEY`, then `<weepcode-home>/auth.json` (`weepcode::api_key`
    ...
    /// shell uses (`$WEEPCODE_HOME` or `~/.weepcode`). Exposed primarily for
    /// tests / sandboxed invocations.
    #[arg(long)]
    weepcode_home: Option<PathBuf>,
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let args = RunArgs {
        ...
        weepcode_home: cli.weepcode_home,
    };
...
mod tests {
    ...
    fn cli_parses_minimal_args() {
        ...
        assert!(cli.weepcode_home.is_none());
    }
    ...
    #[test]
    fn cli_weepcode_home_override_parses() {
        let cli = Cli::try_parse_from([
            "trace_classify",
            "--trace",
            "foo.json",
            "--weepcode-home",
            "/tmp/scratch-weepcode",
        ])
        .expect("parse");
        assert_eq!(
            cli.weepcode_home,
            Some(PathBuf::from("/tmp/scratch-weepcode"))
        );
    }
    ...
    fn cli_defaults_match_documented_values() {
        ...
        assert_eq!(by_id("model"), vec!["weepcode-4.5"]);
