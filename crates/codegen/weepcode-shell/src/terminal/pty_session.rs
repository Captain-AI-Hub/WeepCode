...
const NOTIFICATION_METHOD: &str = "weepcode/terminal/pty/notification";
...
/// `WEEPCODE_SHELL`) since `$SHELL` is absent.
fn resolve_pty_shell(shell: Option<&str>) -> (String, Vec<String>) {
    ...
    #[cfg(not(unix))]
    {
        use weepcode_config::shell::{WindowsShell, detect_windows_shell};
...
mod tests {
    ...
    use weepcode_acp_lib::acp_gateway;
