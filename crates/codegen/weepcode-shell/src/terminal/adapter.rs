//! AcpTerminalAdapter: implements `weepcode-tools::TerminalBackend` using ACP gateway calls.
//!
//! This adapter enables bash tool execution over ACP (remote execution).
//! It translates weepcode-tools' `TerminalBackend` trait into ACP protocol calls:
...
fn wrap_command(command: &str) -> Result<String, ComputerError> {
    // On Windows the ACP client (weepcode-desktop) spawns with `shell: true`
...
/// Wraps weepcode-shell's ACP gateway to satisfy weepcode-tools' TerminalBackend.
