...
/// Resolved absolute path to bash. On Unix uses the `weepcode_config` shell
/// resolution cascade (`$WEEPCODE_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        weepcode_config::shell::unix_shell_path(weepcode_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `weepcode-tools`.
pub use weepcode_tools::util::pager_env;
...
/// weepcode-pager which renders its own UI and doesn't need raw ANSI codes).
///
