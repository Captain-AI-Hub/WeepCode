...
use weepcode_agent::plugins::discovery::PluginScope;
use weepcode_agent::plugins::git_install::{self, UpdateStatus};
use weepcode_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use weepcode_plugin_marketplace::git::{self, SourceCacheLease};
use weepcode_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.weepcode/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &weepcode_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `weepcode plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `weepcode plugin install ./{name}`, or add a \
                     source with `weepcode plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `weepcode plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `weepcode plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
/// both tighten-only: no remote settings layer may relax a local security
/// policy, and an unreadable config falls back to the env knob.
pub fn marketplace_require_sha() -> bool {
    weepcode_config::load_effective_config_disk_only()
        .map(|c| weepcode_plugin_marketplace::load_require_sha(&c))
        .unwrap_or_else(|_| weepcode_plugin_marketplace::env_require_sha())
}
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &weepcode_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.weepcode/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user weepcode via user_weepcode_home() (None when no home resolves) and
    // home separately, so removal still runs from $WEEPCODE_HOME when no home dir
    // exists, and never touches a cwd-relative .weepcode.
    let home = dirs::home_dir();
    let weepcode = weepcode_config::user_weepcode_home();

    let mut settings_candidates: Vec<std::path::PathBuf> = Vec::new();
    if let Some(ref weepcode) = weepcode {
        settings_candidates.push(weepcode.join("settings.local.json"));
        settings_candidates.push(weepcode.join("settings.json"));
    }
    ...
    if let Some(ref weepcode) = weepcode {
        known_candidates.push(weepcode.join("plugins").join("known_marketplaces.json"));
    }
    if let Some(ref home) = home {
...
mod tests {
    ...
    fn registered_source_label_uses_addressable_qualifier() {
        assert_eq!(
            registered_source_label(&git_source(
                "WeepCode Official",
                "https://github.com/weepcode-org/plugin-marketplace.git"
            )),
            "WeepCode Official (weepcode-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn candidate_label_includes_pin_hint() {
        assert_eq!(
            ...
            "WeepCode Official (pin: sentry@weepcode-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn unknown_qualifier_error_lists_registered_marketplaces() {
        let err = MarketplaceInstallError::UnknownQualifier {
            ...
            registered: vec![
                "WeepCode Official (weepcode-org/plugin-marketplace)".into(),
        ...
        assert!(
            msg.contains("  - WeepCode Official (weepcode-org/plugin-marketplace)"),
            "{msg}"
        );
    ...
    fn ambiguous_qualifier_error_lists_source_names() {
        let err = MarketplaceInstallError::AmbiguousQualifier {
            qualifier: "weepcode-org/plugin-marketplace".into(),
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("weepcode plugin install ./sentry"), "{msg}");
        assert!(msg.contains("weepcode plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        let err = MarketplaceInstallError::NameAmbiguous {
            name: "sentry".into(),
            candidates: vec![
                "WeepCode Official (pin: sentry@weepcode-org/plugin-marketplace)".into(),
            ],
        };
        let msg = err.to_string();
        assert!(
        ...
        assert!(
            msg.contains("  - WeepCode Official (pin: sentry@weepcode-org/plugin-marketplace)"),
            "{msg}"
        );
        assert!(
            msg.contains("weepcode plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    const OFFICIAL_URL: &str = "https://github.com/weepcode-org/plugin-marketplace.git";

    #[test]
    fn plan_install_qualifier_unknown_lists_registered_labels() {
        let sources = [
            git_source("WeepCode Official", OFFICIAL_URL),
        ...
        match err {
            MarketplaceInstallError::UnknownQualifier {
                ...
                assert_eq!(
                    registered,
                    vec![
                        "WeepCode Official (weepcode-org/plugin-marketplace)".to_string(),
    ...
    fn plan_install_qualifier_ambiguous_lists_source_names() {
        let sources = [
            ...
            git_source(
                "Mirror B",
                "git@github.com:weepcode-org/plugin-marketplace.git",
            ),
        ];
        let err = plan_install(
            &sources,
            "sentry",
            Some("weepcode-org/plugin-marketplace"),
            |_| Ok(Vec::new()),
        )
        .expect_err("two sources share the owner/repo");
        match err {
            MarketplaceInstallError::AmbiguousQualifier { qualifier, sources } => {
                assert_eq!(qualifier, "weepcode-org/plugin-marketplace");
                assert_eq!(
                    sources,
    ...
    fn plan_install_qualifier_not_found_when_scan_lacks_name() {
        let sources = [git_source("WeepCode Official", OFFICIAL_URL)];
        let err = plan_install(
            &sources,
            "sentry",
            Some("weepcode-org/plugin-marketplace"),
        ...
        match err {
            MarketplaceInstallError::QualifiedNameNotFound {
                ...
                assert_eq!(source_display, "WeepCode Official");
            }
    ...
    fn plan_install_bare_name_official_match_proceeds_despite_skip() {
        ...
        let plan = plan_install(&sources, "sentry", None, |source| {
            if source.name == "WeepCode Official" {
                Ok(vec![mp_entry("sentry")])
            } else {
                Err("sync failed".to_string())
            }
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> weepcode_workspace::permission::resolution::MarketplaceAllowlist {
        weepcode_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = weepcode_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
    ...
    fn resolve_qualified_source_name_with_matches_git_owner_repo() {
        ...
        let name = resolve_qualified_source_name_with(&sources, "weepcode-org/plugin-marketplace")
            .expect("qualifier should match the official source");
        assert_eq!(name, "WeepCode Official");
    }
