...
use crate::auth::{WeepcodeAuth, WeepcodeComConfig};
...
const WEEPCODE_CODE_BACKEND_URL: &str = "https://code.grok.com";
const DEFAULT_TIMEOUT: Duration = Duration::from_secs(30);
const WEEPCODE_CODE_WEB_URL: &str = "https://grok.com";
/// Build a share URL from a permission ID
pub fn share_url(permission_id: &str) -> String {
    let web_url = std::env::var("WEEPCODE_CODE_WEB_URL")
        .unwrap_or_else(|_| WEEPCODE_CODE_WEB_URL.to_string());
...
fn add_cli_chat_proxy_headers_blocking(
    ...
    let mut builder = builder
        ...
        .header(
            "X-WeepCode-Token-Auth",
            WeepcodeComConfig::default().token_header,
        )
        .header("x-userid", &auth.user_id)
        .header("x-weepcode-client-version", weepcode_version::VERSION);
...
async fn add_bundle_fetch_headers(
    ...
    let mut credentials = crate::util::weepcode_auth_credentials::WeepcodeAuthCredentials::new(
    ...
    weepcode_file_utils::trace_context::inject_trace_context_into_request(builder)
}
...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn weepcode_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("WEEPCODE_CODE_BACKEND_URL")
                .unwrap_or_else(|_| WEEPCODE_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    /// instead of requiring the caller to pass `&WeepcodeAuth`.
    pub fn with_auth_manager(mut self, manager: std::sync::Arc<crate::auth::AuthManager>) -> Self {
        let credentials: std::sync::Arc<dyn weepcode_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    async fn resolve_auth(&self) -> Result<WeepcodeAuth, BackendError> {
        let manager = self
            .auth_manager
            .as_ref()
    ...
    /// Must include X-WeepCode-Token-Auth so nginx auth subrequest routes to authenticate_weepcode_cli_token.
    /// See: crates/codegen/weepcode-shell/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        if let Ok(v) = HeaderValue::from_str(&crate::http::process_client_identifier()) {
            headers.insert("x-weepcode-client-identifier", v);
        }
        headers.insert(
            crate::http::CLIENT_MODE_HEADER,
        ...
        headers.insert(
            ...
            HeaderValue::from_static(weepcode_version::VERSION),
        );
        Ok(headers)
    }
    async fn send_with_auth(
        ...
        let builder = weepcode_file_utils::trace_context::inject_trace_context_into_request(
...
/// Fetch `weepcode_build_login_device_flow` from cli-chat-proxy `GET /v1/login-config`.
///
/// Unauthenticated (pre-login); `x-weepcode-agent-id` is the per-install bucketing key.
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(weepcode_telemetry::id::agent_id)
        .await
        .ok()?;
    ...
    let response = client
        ...
        .header("x-weepcode-agent-id", agent_id)
        .header("x-weepcode-client-version", weepcode_version::VERSION)
        .header(
            "x-weepcode-client-identifier",
            crate::http::process_client_identifier(),
...
impl ListModelsEndpoint {
    fn from_endpoints(
        ...
        if endpoints.has_custom_endpoint() {
            ...
            Self {
                url: format!("{}/models", endpoints.weepcode_api_base_url),
                auth: EndpointAuth::ApiKey,
            }
        } else {
            Self {
...
pub(crate) fn fetch_models_blocking(
    ...
    match source.auth {
        EndpointAuth::ApiKey => {
            let api_key = crate::agent::auth_method::read_weepcode_api_key_env()
                .or_else(|_| {
                ...
                .map_err(|_| {
                    BackendError::Auth(
                        "No API key for custom models endpoint. Set WEEPCODE_API_KEY.".into(),
                    )
                })?;
        ...
        EndpointAuth::Session => {
            ...
            request = request
                .header("Authorization", format!("Bearer {}", &auth.key))
                .header("X-WeepCode-Token-Auth", "weepcode-cli")
                .header("x-userid", &auth.user_id)
                .header("x-weepcode-client-version", weepcode_version::VERSION)
                .header(
                    crate::http::CLIENT_MODE_HEADER,
...
pub fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| weepcode_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(weepcode_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
mod tests {
                ...
                get(
                    |State(state): State<LoginConfigServerState>, headers: HeaderMap| async move {
                        state.seen.lock().unwrap().push(LoginConfigHeaders {
                            ...
                            agent_id: header_str(&headers, "x-weepcode-agent-id"),
                            client_identifier: header_str(&headers, "x-weepcode-client-identifier"),
                            client_version: header_str(&headers, "x-weepcode-client-version"),
                        });
    ...
    async fn fetch_login_device_flow_sends_only_unauthenticated_headers() {
        ...
        assert!(
            ...
            "must send x-weepcode-agent-id (the bucketing key)"
        );
        assert!(
            h.client_identifier.is_some(),
            "must send x-weepcode-client-identifier"
        );
        assert!(
            h.client_version.is_some(),
            "must send x-weepcode-client-version"
        );
    ...
    fn test_auth_manager() -> Arc<crate::auth::AuthManager> {
        ...
        let mgr =
            crate::auth::AuthManager::new(dir.path(), crate::auth::WeepcodeComConfig::default());
    ...
    async fn fetch_subagent_bundle_success() {
        ...
        assert_eq!(headers.token_auth.as_deref(), Some("weepcode-cli"));
    ...
    fn parse_openai_format_uses_id_field() {
        let value = serde_json::json!(
            { "id" : "weepcode-3", "object" : "model", "owned_by" : "xai", "context_window" :
            131072 }
        );
        let result = parse_remote_model_value(&value, "https://api.x.ai/v1").unwrap();
        assert_eq!(result.model, "weepcode-3");
        assert_eq!(result.base_url, "https://api.x.ai/v1");
        assert_eq!(result.name.as_deref(), Some("weepcode-3"));
    }
    #[test]
    ...
    fn parse_reads_reasoning_effort_fields() {
        ...
        let value = serde_json::json!(
            { "model" : "weepcode-4.5", "context_window" : 1_000_000,
        ...
        let value = serde_json::json!(
            { "model" : "weepcode-4.5", "contextWindow" : 1_000_000,
    ...
    fn parse_reads_reasoning_efforts_list() {
        use weepcode_sampling_types::ReasoningEffort;
        let value = serde_json::json!(
            { "model" : "weepcode-4.5", "context_window" : 1_000_000, "reasoning_efforts" :
    ...
    fn parse_remote_model_value_no_laziness_detector_block_yields_default() {
        let value = serde_json::json!(
            { "model" : "weepcode-4", "context_window" : 256_000, }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert_eq!(
    ...
    fn parse_remote_model_value_parses_camelcase_key() {
        let value = serde_json::json!(
            { "model" : "weepcode-4", "context_window" : 256_000, "lazinessDetector" : {
    ...
    fn parse_remote_model_value_parses_snake_case_laziness_detector() {
        let value = serde_json::json!(
            { "model" : "weepcode-4", "context_window" : 256_000, "laziness_detector" : {
    ...
    fn parse_remote_model_value_parses_meta_laziness_detector() {
        let value = serde_json::json!(
            { "model" : "weepcode-4", "context_window" : 256_000, "_meta" : {
    ...
    fn parse_reads_show_model_fingerprint_field() {
        let value = serde_json::json!(
            { "model" : "weepcode-build", "context_window" : 256_000,
            "show_model_fingerprint" : true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "weepcode-build", "contextWindow" : 256_000, "showModelFingerprint" :
            true }
        );
        let result = parse_remote_model_value(&value, "https://default.url").unwrap();
        assert!(result.show_model_fingerprint);
        let value = serde_json::json!(
            { "model" : "weepcode-build", "context_window" : 256_000, "_meta" : {
    ...
    /// ApiKey → `weepcode_api_base_url` (ApiKey, public default when unset); a custom
    ...
    /// REGRESSION: `weepcode setup` must send the deployment key to
    ...
    fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
        ...
        unsafe { std::env::set_var("WEEPCODE_DEPLOYMENT_KEY", "weepcode-token-ENTERPRISE") };
        ...
        unsafe { std::env::remove_var("WEEPCODE_DEPLOYMENT_KEY") };
    }
    #[derive(Clone)]
