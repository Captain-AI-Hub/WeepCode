...
//! weepcode-web's chat picker shows, distinct from the CLI `/v1/models` build
...
impl ChatModelsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("WEEPCODE_MODES_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("WEEPCODE_CONVERSATIONS_BASE_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .or_else(|| {
    ...
    /// Gated only on a valid grok.com bearer — deliberately NOT `is_weepcode_auth()`
