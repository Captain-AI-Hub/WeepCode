...
const WEEPCODE_WEB_URL: &str = "https://grok.com";
...
/// Body for `PUT /rest/app-chat/conversations/{id}` (weepcode-web `chatUpdateConversation`).
...
impl ConversationsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("WEEPCODE_CONVERSATIONS_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("WEEPCODE_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| WEEPCODE_WEB_URL.to_string());
        Self {
    ...
    async fn require_weepcode_auth(&self) -> Result<WeepcodeAuth, ConvError> {
    ...
    fn apply_auth_headers(
        ...
        let mut builder = builder
            ...
            .header(
                ...
                self.auth.weepcode_com_config().token_header.clone(),
            )
    ...
    pub async fn list_conversations(
        ...
        let auth = self.require_weepcode_auth().await?;
