//! Shim — see `weepcode_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `weepcode_shell::instrumentation_timer!`). Keeping the macro here
...
pub use weepcode_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `weepcode_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `weepcode_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `weepcode_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `weepcode_telemetry::instrumentation`.
#[macro_export]
