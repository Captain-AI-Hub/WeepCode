...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "weepcode/suggest" => handle_suggest(agent, args).await,
        "weepcode/suggestPrompt" => handle_suggest_prompt(agent, args).await,
...
struct SuggestPromptRequest {
    ...
    /// override, or `weepcode-build-0.1` when its catalog offers it). One tier
    /// of the shell-side resolution in
    /// `prompt_suggest::effective_suggest_model`: env > config.toml > remote
    /// > this hint > `weepcode-build-0.1` default, catalog-guarded (a
...
/// forever. Reasoning models (e.g. `weepcode-build`) can take ~30s on a cold
