...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        m if m.starts_with("weepcode/compact_conversation") => handle_compact(agent, args).await,
        "weepcode/memory/flush" => handle_flush(agent, args).await,
        "weepcode/memory/rewrite" => handle_rewrite(agent, args).await,
