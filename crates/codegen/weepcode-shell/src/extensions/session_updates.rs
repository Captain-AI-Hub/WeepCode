...
//! WeepCode extensions) and extract the notification payload from `params`.
    ...
    for (chunk_idx, chunk) in lines.chunks(chunk_size).enumerate() {
        ...
        if let Ok(raw) = serde_json::value::to_raw_value(&params) {
            gateway.forward_fire_and_forget(acp::ExtNotification::new(
                "weepcode/session/updates/chunk",
...
mod tests {
    ...
    fn dummy_gateway() -> weepcode_acp_lib::AcpAgentGatewaySender {
    ...
    fn make_request(
        ...
        acp::ExtRequest::new("weepcode/session/updates", std::sync::Arc::from(raw))
    }
    ...
    async fn handle_falls_back_to_id_lookup_for_divergent_cwd() {
        ...
        // Clean up the dir written under the real weepcode home.
        let _ = std::fs::remove_dir_all(&child_dir);
    }

    fn capturing_gateway() -> (
        weepcode_acp_lib::AcpAgentGatewaySender,
        tokio::sync::mpsc::UnboundedReceiver<weepcode_acp_lib::AcpClientMessage>,
    ) {
        let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
        (weepcode_acp_lib::AcpAgentGatewaySender::new(tx), rx)
    }
    ...
    fn weepcode_rewind(target: usize) -> String {
        format!(
