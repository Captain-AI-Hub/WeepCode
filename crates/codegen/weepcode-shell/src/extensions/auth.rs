...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "weepcode/auth/getBearerToken" => handle_get_bearer_token(agent).await,
        "weepcode/getApiKey" => handle_get_api_key(),
        "weepcode/setApiKey" => handle_set_api_key(args),
        "weepcode/auth/submit_code" => handle_submit_code(agent, args),
        "weepcode/auth/get_url" => handle_get_url(agent).await,
        "weepcode/auth/cancel" => handle_cancel(agent, args),
        "weepcode/auth/logout" => handle_logout(agent, args).await,
        "weepcode/auth/info" => handle_info(agent),
...
fn handle_get_api_key() -> ExtResult {
    let key = crate::agent::auth_method::read_weepcode_api_key_env().ok();
...
fn handle_set_api_key(args: &acp::ExtRequest) -> ExtResult {
    ...
    if let Some(k) = key {
        if k.is_empty() {
            crate::auth::clear_api_key(&weepcode_home)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
            unsafe { std::env::remove_var("WEEPCODE_API_KEY") };
        } else {
            crate::auth::store_api_key(&weepcode_home, k)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
            unsafe { std::env::set_var("WEEPCODE_API_KEY", k) };
        }
    } else {
        crate::auth::clear_api_key(&weepcode_home)
            .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
...
fn handle_info(agent: &MvpAgent) -> ExtResult {
    ...
    struct AuthInfoResponse {
        ...
        /// `weepcode-asset://` URL resolved by the Electron protocol handler,
    ...
    // Return a weepcode-asset:// URL that the Electron renderer resolves at
    ...
    let profile_image_url = match raw_asset_id.as_deref().filter(|k| !k.is_empty()) {
        ...
        Some(key) => Some(format!("weepcode-asset:///{key}")),
        None => None,
    };
