...
use weepcode_telemetry::id::agent_id;

#[tracing::instrument(skip_all, fields(method = %args.method))]
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "weepcode/session/rename" => handle_session_rename(agent, args).await,
        "weepcode/session/delete" => handle_session_delete(agent, args).await,
        "weepcode/session/update_mcp_servers" => handle_update_mcp_servers(agent, args).await,
        "weepcode/session/fork" => handle_session_fork(agent, args).await,
        "weepcode/internal/reload_all_mcp_servers" => handle_reload_all_mcp_servers(agent).await,
        "weepcode/internal/reload_project_mcp_servers" => {
            handle_reload_project_mcp_servers(agent, args).await
        }
        "weepcode/internal/reload_skills" => handle_reload_skills(agent),
        "weepcode/internal/reload_models" => handle_reload_models(agent),
        "weepcode/internal/reload_models_cache" => handle_reload_models_cache(agent),
        "weepcode/internal/auth_cleared" => handle_auth_cleared(agent),
        "weepcode/plugins/reload" => handle_plugins_reload(agent).await,
        "weepcode/commands/list" => handle_commands_list(agent, args).await,
    ...
    client
        ...
        .map_err(|e| match e {
            ConvError::NoOauth => acp::Error::invalid_request()
                .data("chat session rename requires WeepCode OAuth credentials"),
...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Mirrored by the `weepcode sessions delete <id>` CLI path.
...
async fn soft_delete_chat_conversation(agent: &MvpAgent, conversation_id: &str) -> ExtResult {
    ...
    client
        ...
        .map_err(|e| match e {
            ConvError::NoOauth => acp::Error::invalid_request()
                .data("chat session delete requires WeepCode OAuth credentials"),
            other => acp::Error::internal_error()
...
/// hot-reload watcher when `<cwd>/.weepcode/config.toml`,
...
/// Hot-reload the model catalog from `~/.weepcode/models_cache.json` after an
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // that `x.ai/commands/list` (the pull used by weepcode-desktop after session
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .weepcode/plugins under it, plus
