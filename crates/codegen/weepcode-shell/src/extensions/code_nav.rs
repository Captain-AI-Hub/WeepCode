...
//! using the weepcode-codebase-graph index.
//!
//! ## Extension Methods
//!
//! | Method | Description |
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        ...
        "weepcode/code/find-definitions" => {
...
fn to_code_nav_ext_response(resp: weepcode_workspace::workspace_ops::CodeNavResponse) -> ExtResult {
    let symbol = resp
        .locations
        .first()
...
fn eligibility_error(reason: CodeNavEligibility) -> acp::Error {
    let msg = match reason {
        CodeNavEligibility::ClientNotWeb => {
            "code navigation is currently only enabled for weepcode-web clients"
        }
        CodeNavEligibility::CapabilityNotAdvertised => {
