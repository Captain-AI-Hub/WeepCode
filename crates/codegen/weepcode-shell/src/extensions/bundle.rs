...
mod tests {
    ...
    fn test_auth() -> crate::auth::WeepcodeAuth {
        crate::auth::WeepcodeAuth {
            key: "token".to_string(),
    ...
    fn status_only_reports_bundled_cache_not_higher_priority_sources() {
        ...
        std::fs::create_dir_all(project_root.join(".weepcode/personas")).unwrap();
        std::fs::create_dir_all(project_root.join(".weepcode/roles")).unwrap();
        std::fs::write(
            project_root.join(".weepcode/personas/researcher.toml"),
            "instructions = \"project persona\"\n",
        )
        .unwrap();
        std::fs::write(
            project_root.join(".weepcode/roles/reviewer.toml"),
