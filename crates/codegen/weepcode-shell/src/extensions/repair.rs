...
//!
//! Repairs via [`weepcode_chat_state::compaction_utils::repair_history`]. Resident
...
use weepcode_chat_state::compaction_utils::HistoryRepairReport;
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "weepcode/session/repair" => handle_session_repair(agent, args).await,
...
/// back atomically. `weepcode_root` is injectable for tests.
async fn repair_on_disk(
    weepcode_root: &std::path::Path,
    session_id: &str,
    dry_run: bool,
) -> ExtResult {
    let summary = crate::session::persistence::find_summary_by_session_id_in_root(
        session_id,
        &weepcode_root.join("sessions"),
    )
    .ok_or_else(|| {
    ...
    let storage = JsonlStorageAdapter::with_root(weepcode_root.to_path_buf());
    ...
    let report = weepcode_chat_state::compaction_utils::repair_history(&mut chat_history);
