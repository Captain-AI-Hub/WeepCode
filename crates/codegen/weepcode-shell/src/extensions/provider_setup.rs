...
//! persists the profile as a `[model.<slug>]` table in `~/.weepcode/config.toml`,
//! points `[models].default` at it, and hot-reloads the model catalog so a
//! follow-up `authenticate("weepcode.api_key")` succeeds on BYOK credentials alone
//! — no WeepCode OAuth flow is ever involved.
...
fn handle_save_provider(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let slug = upsert_provider_profile(&profile).map_err(|e| {
        acp::Error::internal_error().data(format!("failed to persist provider: {e:#}"))
    })?;

    // Rebuild the live catalog from the just-written config so the new entry
    // is selectable and its BYOK credentials unlock `authenticate(weepcode.api_key)`.
...
mod tests {
    ...
    fn unknown_format_is_rejected() {
        assert!(ProviderApiFormat::from_wire_str("weepcode").is_none());
    }
}
