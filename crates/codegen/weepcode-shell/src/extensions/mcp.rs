...
/// `weepcode_mcp::wire`), these methods are private to the agent↔client channel and
...
pub mod mcp_methods {
    ...
    pub const PREFIX: &str = "weepcode/mcp/";

    pub const LIST: &str = "weepcode/mcp/list";
    pub const READ_RESOURCE: &str = "weepcode/mcp/read_resource";
    pub const AUTH_STATUS: &str = "weepcode/mcp/auth_status";
    pub const AUTH_TRIGGER: &str = "weepcode/mcp/auth_trigger";
    pub const SETUP: &str = "weepcode/mcp/setup";
    pub const TOGGLE: &str = "weepcode/mcp/toggle";
    pub const TOGGLE_TOOL: &str = "weepcode/mcp/toggle_tool";
    pub const UPSERT: &str = "weepcode/mcp/upsert";
    pub const DELETE: &str = "weepcode/mcp/delete";

    pub const SERVERS_UPDATED: &str = "weepcode/mcp/servers_updated";
    pub const TOOLS_CHANGED: &str = "weepcode/mcp/tools_changed";
    pub const INIT_PROGRESS: &str = "weepcode/mcp/init_progress";
}
...
pub async fn notify_servers_updated(
    gateway: &weepcode_acp_lib::AcpAgentGatewaySender,
...
pub async fn build_mcp_status(
    ...
    tool_bridge: &Arc<weepcode_tools::bridge::ToolBridge>,
    event_writer: Option<&weepcode_file_utils::events::EventWriter>,
) -> McpStatusSnapshot {
    ...
    for client in &clients {
        ...
        if let Some(ew) = event_writer {
            ew.emit(weepcode_file_utils::events::Event::McpHealthCheck {
...
pub async fn init_agent_mcp_pool(mcp_state: &Arc<TokioMutex<McpState>>, cwd: &std::path::Path) {
    ...
    let noop = weepcode_file_utils::events::EventWriter::noop();
...
//
// Implements the `McpResourceProvider` trait from weepcode-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `weepcode-mcp` directly.
...
impl weepcode_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<weepcode_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(weepcode_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<weepcode_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(weepcode_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(weepcode_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(weepcode_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
...
mod tests {
    ...
    fn test_mcp_list_response_serialization() {
        let resp = McpListResponse {
            servers: vec![
                McpServerEntry {
                    name: "weepcode_com_linear".to_string(),
                    display_name: None,
                    source: McpServerSource::Managed,
                    config: McpServerConfig::Http {
                        ...
                        scope_name: Some("WeepCode CLI".to_string()),
                    },
                    source_label: None,
                    setup: None,
                    setup_values: None,
        ...
        assert_eq!(json["servers"][0]["scopeName"], "WeepCode CLI");
    ...
    fn disabled_managed_http_rows_keep_non_gateway_placeholder_config() {
        let entry = disabled_server_placeholder_entry("weepcode_com_slack");
    ...
    fn disabled_placeholders_match_handle_list_catalog_relationships() {
        ...
        append_disabled_like_handle_list(
            &mut servers,
            &["weepcode_com_slack", "weepcode_mcp_linear"],
            true,
        );
        assert!(
            servers.is_empty(),
        ...
        let mut servers = build_mcp_catalog_with_gateway_tools(&[], &[], None, &Default::default());
        append_disabled_like_handle_list(&mut servers, &["weepcode_com_slack"], false);
        assert_eq!(servers.len(), 1);
        assert_eq!(servers[0].name, "weepcode_com_slack");
    ...
    fn test_disabled_session_state_serialization() {
        let entry = McpServerEntry {
            name: "weepcode_com_slack".to_string(),
            display_name: None,
            source: McpServerSource::Managed,
            source_label: None,
            setup: None,
