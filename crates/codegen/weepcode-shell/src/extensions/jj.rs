//! Jujutsu extension handlers — delegates to [`weepcode_workspace::session::jj`].

use agent_client_protocol as acp;

use super::{Empty, ExtResult, to_ext_response, to_ext_response_partial};
use weepcode_workspace::session::git::{CommitData, StageData};
use weepcode_workspace::session::jj;
...
pub async fn try_handle(
    ...
    match method {
        "weepcode/git/status" => Some(to_ext_response(jj::status(git_root).await)),
        "weepcode/git/info" => Some(to_ext_response(jj::info(git_root).await)),
        // git HEAD points at `@-` in a colocated repo; route to jj so we report
        // the working-copy commit (`@`), consistent with `status`/`info`.
        "weepcode/git/current_commit" => Some(to_ext_response(jj::current_commit(git_root).await)),
        "weepcode/git/branches" => Some(to_ext_response(jj::list_bookmarks(git_root).await)),

        // jj has no staging area — stage/unstage are no-ops
        "weepcode/git/stage" => Some(to_ext_response(Ok(StageData { paths: Vec::new() }))),
        "weepcode/git/stage/content" | "weepcode/git/unstage" => {
            Some(to_ext_response(Ok(Empty {})))
        }

        "weepcode/git/discard" => {
        ...
        "weepcode/git/checkout" => Some(Err(acp::Error::invalid_params()
            .data("checkout is not supported in jj repos; use `jj new` or `jj edit`"))),
        "weepcode/git/stash" => Some(Err(acp::Error::invalid_params()
