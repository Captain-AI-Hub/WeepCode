...
use weepcode_hooks::event::{HookEventEnvelope, HookEventName};
use weepcode_hooks::matcher::HookMatcher;
use weepcode_hooks_plugins_types::{HookEvent, HookHandlerType, HookInfo};
...
pub fn hook_spec_to_info(spec: &weepcode_hooks::config::HookSpec) -> HookInfo {
    ...
    HookInfo {
        ...
        disabled: weepcode_hooks::trust::is_hook_disabled(&spec.name),
    }
}
...
/// Parse client hooks from `session/new` `_meta["weepcode/hooks"]`, shaped
...
pub(crate) fn parse_client_hooks(meta: Option<&acp::Meta>) -> ClientHooks {
    ...
    let Some(map) = meta
        .and_then(|m| m.get("weepcode/hooks"))
    ...
    for (event_name, value) in map {
        ...
        let Some(array) = value.as_array() else {
            tracing::warn!(event = %event_name, "weepcode/hooks event value is not an array; skipping");
            continue;
        };
...
pub(crate) fn reconnect_client_hooks(meta: Option<&acp::Meta>) -> Option<ClientHooks> {
    meta.and_then(|m| m.get("weepcode/hooks"))
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "weepcode/hooks/action" => {
            let req: weepcode_hooks_plugins_types::HooksActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    ...
    fn parse_client_hooks_canonicalizes_subagent_alias() {
        let meta = serde_json::json!({
            "weepcode/hooks": { "SubagentEnd": [{ "hookCallbackIds": ["cb"] }] }
        });
        let hooks = parse_client_hooks(meta.as_object());
    ...
    fn reconnect_client_hooks_only_when_key_present() {
        ...
        let cleared =
            reconnect_client_hooks(serde_json::json!({ "weepcode/hooks": {} }).as_object());
        assert!(cleared.is_some_and(|h| h.is_empty()));

        let set = reconnect_client_hooks(
            serde_json::json!({
                "weepcode/hooks": { "PreToolUse": [{ "hookCallbackIds": ["cb"] }] }
            })
            .as_object(),
        );
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use weepcode_hooks::event::{HookEventEnvelope, HookPayload};
