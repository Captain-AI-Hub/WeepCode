...
pub fn loaded_plugin_to_info(plugin: &weepcode_agent::plugins::LoadedPlugin) -> PluginInfo {
    use weepcode_agent::plugins::discovery::PluginScope as AgentScope;
...
fn origin_to_dto(origin: &weepcode_agent::plugins::PluginOrigin) -> PluginOrigin {
    use weepcode_agent::plugins::PluginOrigin as AgentOrigin;
    match origin {
        AgentOrigin::CliOverride => PluginOrigin::CliOverride,
        AgentOrigin::ProjectWeepcode => PluginOrigin::ProjectWeepcode,
        AgentOrigin::ProjectClaude => PluginOrigin::ProjectClaude,
        AgentOrigin::UserWeepcode => PluginOrigin::UserWeepcode,
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "weepcode/plugins/action" => {
            let req: weepcode_hooks_plugins_types::PluginsActionRequest =
                super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    use super::*;
    use weepcode_agent::plugins::PluginOrigin as AgentOrigin;
    use weepcode_agent::plugins::discovery::{PluginId, PluginScope as AgentScope};

    fn make_loaded_plugin(origin: AgentOrigin) -> weepcode_agent::plugins::LoadedPlugin {
        let root = std::path::PathBuf::from("/tmp/test-plugin");
        weepcode_agent::plugins::LoadedPlugin {
    ...
    fn info_carries_origin_and_marketplace_display_name() {
        let plugin = make_loaded_plugin(AgentOrigin::MarketplaceInstall {
            source_name: Some("WeepCode Official".to_string()),
