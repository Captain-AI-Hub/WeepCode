...
use weepcode_tools::types::{KillOutcome, TaskSnapshot};
...
///
/// `pub` (with both serde directions) so ACP clients (weepcode-pager) build
...
pub async fn handle_subagent(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "weepcode/subagent/list_running" => {
