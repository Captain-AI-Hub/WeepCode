//! Data APIs for `weepcode models`. Clients own display.
...
/// Status for the `weepcode models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
#[derive(Debug, PartialEq, Eq)]
pub enum AuthStatus {
    ApiKey,
    /// Auth host from `weepcode_ws_origin` (scheme stripped).
    LoggedIn(String),
...
impl AuthStatus {
    ...
    /// [`crate::agent::auth_method::should_advertise_weepcode_api_key`] so
    ...
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if agent_config.create_auth_manager().current().is_some() {
            let origin = &agent_config.weepcode_com_config.weepcode_ws_origin;
            let host = origin
                .strip_prefix("https://")
        ...
        if crate::agent::auth_method::should_advertise_weepcode_api_key(
            agent_config.weepcode_com_config.api_key_auth_disabled(),
            models.values(),
...
mod tests {
    use super::*;
    use crate::agent::auth_method::{LEGACY_WEEPCODE_API_KEY_ENV_VAR, WEEPCODE_API_KEY_ENV_VAR};
    use crate::agent::config::Config;
    use crate::auth::{AuthMode, WeepcodeAuth};
    use serial_test::serial;
    use weepcode_test_support::EnvGuard;

    /// Isolate process-global auth sources that `AuthStatus::resolve` consults.
    ///
    /// Uses `WEEPCODE_AUTH_PATH` (not `WEEPCODE_HOME`) so a OnceLock-cached real home
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            EnvGuard::unset(WEEPCODE_API_KEY_ENV_VAR),
            EnvGuard::unset(LEGACY_WEEPCODE_API_KEY_ENV_VAR),
            EnvGuard::unset("WEEPCODE_AUTH"),
            EnvGuard::set("WEEPCODE_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("WEEPCODE_DEPLOYMENT_KEY"),
            EnvGuard::unset("WEEPCODE_WS_ORIGIN"),
            EnvGuard::unset("WEEPCODE_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_api_key_env() {
        ...
        let _key = EnvGuard::set(WEEPCODE_API_KEY_ENV_VAR, "weepcode-test-key");
    ...
    fn resolve_legacy_api_key_env() {
        let (_dir, _g) = isolate_auth_sources();
        let _key = EnvGuard::set(LEGACY_WEEPCODE_API_KEY_ENV_VAR, "legacy-key");
        assert_eq!(AuthStatus::resolve(&Config::default()), AuthStatus::ApiKey);
    }
    ...
    fn resolve_oauth_session() {
        let (_dir, _g) = isolate_auth_sources();
        let token = WeepcodeAuth {
            key: "session-token".into(),
            auth_mode: AuthMode::WebLogin,
            ..WeepcodeAuth::test_default()
        };
        let json = serde_json::to_string(&token).unwrap();
        let _auth = EnvGuard::set("WEEPCODE_AUTH", &json);
