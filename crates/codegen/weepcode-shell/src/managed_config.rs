...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    weepcode_config::MANAGED_CONFIG_FILENAME,
    weepcode_config::REQUIREMENTS_FILENAME,
    weepcode_config::signed_policy::SIGNATURE_SIDECAR_FILE,
    weepcode_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE,
];
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            weepcode_config::MANAGED_CONFIG_CACHE_FILE,
    ...
    let atomic_write_tmp_prefixes = [
        format!("{}.", weepcode_config::MANAGED_CONFIG_CACHE_FILE),
        format!(
            "{}.",
            weepcode_config::signed_policy::SIGNATURE_SIDECAR_FILE
        ),
...
/// would just 401).
fn eligible_team_principal(auth: WeepcodeAuth) -> Option<WeepcodeAuth> {
...
fn read_active_team_auth() -> Option<WeepcodeAuth> {
    let home = crate::util::weepcode_home::weepcode_home();
...
/// files (TUI tick vs `weepcode login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `weepcode setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `WEEPCODE_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("WEEPCODE_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `weepcode-http` classifier.
...
/// Override with `WEEPCODE_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("WEEPCODE_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// snapshots, and OTel: the **server** WeepcodeBuildDeployment UUID (the id
...
/// Resolve deployment key from `WEEPCODE_DEPLOYMENT_KEY` env var, then config files.
...
/// Callers doing auto-fetch should check this; explicit user actions (weepcode setup) skip it.
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("WEEPCODE_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.weepcode/`, trying the
...
async fn sync_bounded(
    budget: SyncBudget,
    team_override: Option<WeepcodeAuth>,
...
/// read-only `weepcode setup --json` both build on this.
...
fn apply_fetched(
    ...
    let verified = if weepcode_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(weepcode_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        weepcode_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        ...
        {
            clear_squatting_dir(
                &home.join(weepcode_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
            weepcode_config::signed_policy::write_managed_identity_sidecar(&home, &claim_sidecar)?;
        }
    }
    ...
    clear_squatting_dir(&home.join(weepcode_config::MANAGED_CONFIG_CACHE_FILE));
...
fn served_principal_of(payload: &weepcode_config::signed_policy::SignedPayload) -> Option<&str> {
    payload
        .deployment_id
        .as_deref()
...
fn verified_claim_sidecar(
...
) -> Option<weepcode_config::signed_policy::SignatureEnvelope> {
    use weepcode_config::signed_policy::now_unix;
    ...
    let claim = match weepcode_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
fn claim_binds_to(
    claim: &weepcode_config::signed_policy::ManagedIdentityClaim,
    served_principal: Option<&str>,
) -> bool {
...
/// Post-login hook for `weepcode login` and the ACP/TUI authenticate flow: clear any
...
pub async fn post_login_sync(authenticated: Option<WeepcodeAuth>) -> ManagedConfigSync {
    clear_orphan();
    if !is_fetch_enabled() {
...
/// Whether a credential exists that `weepcode setup` could install config for.
...
pub async fn ensure_managed_policy_present(
    ...
    let team = tokio::time::timeout(SESSION_START_AUTH_DEADLINE, auth_manager.auth())
        ...
        .ok()
        .and_then(Result::ok)
        .filter(WeepcodeAuth::is_team_principal);
    if !has_principal() {
        return;
    }
...
/// are the non-forgeable layers. Recovery: reconnect / `weepcode setup`; ceasing to serve
...
fn bump_managed_rollback_floor() {
    ...
    if !weepcode_config::signed_policy::verification_active() {
        return;
    }
    let home = crate::util::weepcode_home::weepcode_home();
    match try_lock_managed_config(&home) {
        Some(_lock) => {
            weepcode_config::bump_rollback_floor(&home);
        }
...
/// Outcome of the `weepcode setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.weepcode`.
    Installed,
...
/// Result of `weepcode setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `weepcode setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `weepcode setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `weepcode setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `weepcode setup` would refuse
    // is reported as an error, not printed as installable config.
    if source.is_some()
        && weepcode_config::signed_policy::verification_active()
...
/// Run the `weepcode setup` sync for the current principal. The caller must check
