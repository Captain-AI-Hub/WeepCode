...
pub use self::types::*;
pub use weepcode_sampler::ApiBackend;
pub use weepcode_sampler::SamplingClient as Client;

// Re-export async-openai Responses API types under `rs` namespace
pub use async_openai::types::responses as rs;

// ---------------------------------------------------------------------------
// weepcode-sampler re-exports
// ---------------------------------------------------------------------------
//
// The actual streaming / retry / HTTP-client logic lives in the
// `weepcode-sampler` crate. We re-export the public surface here so
...
// `weepcode_sampler::*`. The shell-side `sampling::client::Config`
