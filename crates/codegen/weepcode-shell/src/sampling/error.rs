...
//!
//! The canonical error types now live in `weepcode_sampling_types::error`.
//! This module re-exports them and adds `map_sampling_err_to_acp` which
//! depends on `agent_client_protocol::Error` (a weepcode-shell dependency).

// Re-export everything from the standalone crate.
pub use weepcode_sampling_types::error::*;
...
/// User-facing text is produced by [`format_rate_limited_user_message`].
/// Pager/headless use that helper. Desktop may still special-case
/// `stopReason: rate_limit` with its own UI and ignore the body — that is a
/// client choice, not a shell requirement to suppress detail.
pub const RATE_LIMITED_ERROR_CODE: i32 = -32003;

/// OAuth / session rate-limit copy.
pub const RATE_LIMITED_USER_MESSAGE_OAUTH: &str =
    "You\u{2019}ve hit the rate limit. Please try again later.";

/// API key / team rate-limit copy.
pub const RATE_LIMITED_USER_MESSAGE_API_KEY: &str =
    "You\u{2019}ve hit the API rate limit. Check your provider limits or try again later.";
...
/// for the selected method). Do **not** decide from `has_weepcode_api_key_env()` alone:
...
/// Legacy free-usage exhaustion code CCP returned on HTTP 429.
...
/// Whether flattened server detail is legacy free-usage-quota exhaustion, not
/// transient throttling. Sniffs the well-known code embedded by `parse_error_bytes`.
...
///
/// 1. Legacy free-usage well-known code → generic auth-aware fallback.
/// 2. Any other non-empty body → shown as-is (capacity, team RPS limits, etc.).
/// 3. Empty body → [`rate_limited_user_message`].
///
...
/// This stays in weepcode-shell because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                weepcode_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: weepcode_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != weepcode_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == weepcode_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn attach_prompt_usage_preserves_error_kind_and_round_trips() {
        let mut ledger = weepcode_chat_state::UsageLedger::default();
        ledger.record_main_loop_call(
            "m",
            &weepcode_sampling_types::TokenUsage {
                prompt_tokens: 3,
                completion_tokens: 1,
                total_tokens: 4,
                reasoning_tokens: 0,
    ...
    fn format_rate_limited_surfaces_nonempty_server_detail() {
        ...
        // Provider-specific rate-limit copy is surfaced as-is.
        let team = "resource-exhausted: Too many requests for team abc.";
    ...
    #[test]
    fn rate_limit_fallbacks_are_provider_neutral() {
        assert!(!RATE_LIMITED_USER_MESSAGE_OAUTH.contains("Upgrade"));
        assert!(!RATE_LIMITED_USER_MESSAGE_API_KEY.contains("grok.com"));
    ...
    #[test]
    fn format_rate_limited_free_usage_uses_generic_fallback() {
