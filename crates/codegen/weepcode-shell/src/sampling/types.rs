// Re-export all types from the standalone weepcode-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use weepcode_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `weepcode-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `weepcode-sampler` crate can reference them without a circular
// dep on `weepcode-shell`.

// Tests for the types now live in weepcode-sampling-types crate.

use weepcode_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `weepcode-sampling-types` or
/// `weepcode-tools`) so neither of those crates needs to depend on
