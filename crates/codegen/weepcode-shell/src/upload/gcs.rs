...
//! `StorageClient` constructed inside `weepcode_file_utils::gcs::*` helpers.
//!
...
//! to an `weepcode_file_utils::gcs::*` helper.
...
/// shell-side adapter that lets `weepcode_file_utils::gcs::*` helpers wire
...
/// `WEEPCODE_TELEMETRY_GCS_BUCKET`; `None` disables trace uploads until a bucket
/// is configured.
pub(crate) const SESSION_TRACES_BUCKET: Option<&str> =
    option_env!("WEEPCODE_SESSION_TRACES_BUCKET_DEFAULT");
...
pub(crate) async fn upload_to_auth_diagnostics(
    ...
    match weepcode_file_utils::gcs::upload_bytes(
