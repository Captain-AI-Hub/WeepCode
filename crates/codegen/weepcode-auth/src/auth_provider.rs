...
pub trait AuthCredentialProvider: HttpAuth + Send + Sync + 'static {
    ...
    /// (`weepcode-desktop`, `weepcode login`). The `token` field MUST mirror
    ...
    /// Whether `X-WeepCode-Token-Auth` should be sent with the bearer token.
    /// `false` for deployment keys (bare Bearer), `true` for user/OAuth tokens.
    /// See `WeepcodeAuthCredentials::apply()` for the wire format contract.
