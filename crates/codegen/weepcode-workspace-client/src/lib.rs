...
//! `weepcode-workspace`. Wire types live in
//! `weepcode_workspace_types::rpc`; this crate adds the connected-state
...
use weepcode_computer_hub_sdk::harness::ToolHarness;
use weepcode_tool_runtime::{ToolCallContext, ToolStreamItem, TypedToolOutput};
use weepcode_workspace_types::rpc::agents_md::{AgentConfigFile, DiscoverAgentsMdReq};
use weepcode_workspace_types::rpc::code_nav::{
...
use weepcode_workspace_types::rpc::fs::{
...
use weepcode_workspace_types::rpc::git::{
...
use weepcode_workspace_types::rpc::hunks::{
...
use weepcode_workspace_types::rpc::search::{
    ContentSearchData, ContentSearchRequest, FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq,
    FuzzyStatusReq,
};
use weepcode_workspace_types::rpc::session::{
    BeginPromptReq, EndPromptReq, FileRewindResponse, RewindToReq,
};
use weepcode_workspace_types::rpc::skills::{DiscoverPluginsReq, DiscoverSkillsReq, SkillInfo};
...
use weepcode_workspace_types::rpc::worktree::{
...
use weepcode_workspace_types::rpc::{RpcEnvelope, RpcError, WORKSPACE_RPC_TOOL_ID, WorkspaceRpc};
...
pub async fn consume_stream_terminal(
    stream: &mut weepcode_tool_runtime::ToolStream<TypedToolOutput>,
) -> Result<TypedToolOutput, weepcode_tool_runtime::ToolError> {
    loop {
...
/// Check whether a [`ToolError`](weepcode_tool_runtime::ToolError) indicates
...
pub fn is_transport_fatal(err: &weepcode_tool_runtime::ToolError) -> bool {
    match err.kind {
        weepcode_tool_runtime::ToolErrorKind::NetworkError => true,
        weepcode_tool_runtime::ToolErrorKind::Custom => err
            .details
            .as_ref()
...
impl WorkspaceClient {
    ...
    pub async fn rpc_raw(
        ...
        let tool_id = weepcode_tool_protocol::ToolId::new(WORKSPACE_RPC_TOOL_ID)
...
mod tests {
    ...
    use weepcode_computer_hub_sdk::harness::LocalRegistry;
    use weepcode_tool_protocol::{SessionId, ToolId};
    use weepcode_tool_runtime::{Tool, ToolError};
    use weepcode_tool_types::ToolDescription;
    use weepcode_workspace_types::rpc::skills::SkillScope;
    ...
    impl weepcode_tool_runtime::ToolOutput for RawOut {}
    #[derive(Debug)]
    ...
    impl Tool for FakeWorkspaceRpc {
        ...
        fn description(&self, _ctx: &::weepcode_tool_runtime::ListToolsContext) -> ToolDescription {
        ...
        async fn run(&self, _ctx: ToolCallContext, args: Self::Args) -> Result<RawOut, ToolError> {
            ...
            match args.method.as_str() {
                ...
                "workspace.discover_skills" => ok(serde_json::json!(
                    ...
                    "path" : "/workspace/.weepcode/skills/my-skill/SKILL.md", "scope"
                    : "local", }]
                )),
    ...
    async fn consume_stream_terminal_returns_ok() {
        ...
        let mut stream = weepcode_tool_runtime::terminal_only(Ok(typed));
        assert_eq!(
    ...
    async fn consume_stream_terminal_returns_err() {
        let mut stream: weepcode_tool_runtime::ToolStream<TypedToolOutput> =
            weepcode_tool_runtime::terminal_only(Err(ToolError::network_error("oops")));
    ...
    async fn consume_stream_terminal_exhausted_stream_is_network_error() {
        ...
        let mut stream = weepcode_tool_runtime::terminal_only::<TypedToolOutput>(Ok(typed));
