...
///
/// - `FullReplace` (default): weepcode-build's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from weepcode-shell's
...
/// time (token threshold only, matching weepcode-build) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as weepcode-build full-replace auto-compact). Worthless early passes are
    ...
    /// `<weepcode_user_queries>` preamble prepended to the history compaction
...
pub const DEFAULT_COMPACTION_MODEL_NAME: &str = "weepcode-4.20";
