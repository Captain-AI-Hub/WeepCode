...
pub enum CompactionTarget {
    ...
    /// with a single summary — weepcode-build's full-replace strategy. No tail is
...
/// stream processor. Implemented by WeepCode chat's
/// `StreamProcessor` (`Item = Arc<WeepcodeTurn>`).
///
...
pub trait CompactionStreamProc: Send + Sync {
    ...
    /// item type (`Arc<WeepcodeTurn>`) already satisfies it.
