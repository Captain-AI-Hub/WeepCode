...
// The `Shared` summarizer reuses weepcode-build's full-replace summarization core
...
/// `FullReplace` strategy (default): weepcode-build's full-replace — summarize the
...
/// `<weepcode_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching weepcode-build. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize the whole conversation through weepcode-build's shared core.
    ...
    //     Shared with WeepCode chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 3a. For `History` target, split prior `<weepcode_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<weepcode_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<weepcode_user_queries>` preamble below — stays intra's.
    let summary_text = match policy.summarizer {
        ...
        // New (default): weepcode-build's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<weepcode_user_queries>` preamble so
...
/// — weepcode-build's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and weepcode-build stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // weepcode-build appends the summarization prompt as the final user message;
    ...
    .await
    {
        // weepcode-build returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on WeepCode chat's
...
/// the literal error messages produced by the WeepCode chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the WeepCode chat sampler when the
    ...
    /// — guards the forwarding that WeepCode chat (`Arc<WeepcodeTurn>`) relies on.
    #[test]
