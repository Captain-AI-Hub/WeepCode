...
//! stay in each product host (for example `weepcode-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `weepcode-sampling-types`. It is decoupled from both WeepCode chat and
//! weepcode-build hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — weepcode-build's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — WeepCode chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — WeepCode chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by WeepCode chat and
//! weepcode-build; hosts still own snapshotting and host-only sections).
...
// weepcode-build's full-replace subsystem now lives under `code_compaction`;
