//! Observability seam for the full-replace (weepcode-build) pass.
//!
//! The shared orchestrator reports per-attempt and terminal outcomes through
//! this trait so each harness can emit its own telemetry (weepcode-build:
...
//!
//! Emission points are part of the behavior contract: the weepcode-build observer
