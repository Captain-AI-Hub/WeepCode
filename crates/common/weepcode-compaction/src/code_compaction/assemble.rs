//! Compacted-history assembly (weepcode-build's rebuild structure, generic).
//!
//! Moved from `weepcode-chat-state::compaction_utils::build_compacted_history` and
...
//! ```
//!
//! weepcode-build is the canonical harness. The summary carrier text is built by
...
pub fn assemble_compacted_history<T: CompactionItemFactory>(
    ...
    // weepcode-build keeps the legacy `<user_query>`-wrapped continuation text and
...
mod tests {
    ...
    #[test]
    fn weepcode_build_order_recent_before_summary() {
