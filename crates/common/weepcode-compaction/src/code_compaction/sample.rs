...
//!
//! The canonical `sample → classify → retry` loop, used by **both** weepcode-build's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and WeepCode chat's intra `Shared` summarizer
...
//! the winning summary (weepcode-build cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the weepcode-build host uses to step down its input size).
