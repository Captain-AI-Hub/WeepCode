...
//!
//! Behavior is byte-for-byte identical for WeepCode chat (`T = Arc<WeepcodeTurn>`).
...
///   summaries so their `<weepcode_user_queries>` sections can be split out.
...
/// Extracts **all** `<weepcode_user_queries>...</weepcode_user_queries>` blocks
...
pub fn split_prior_compaction_text(text: &str) -> (Option<String>, String) {
    let start_tag = "<weepcode_user_queries>";
    let end_tag = "</weepcode_user_queries>";
...
/// Extract a `<weepcode_user_queries>` XML block from `User` items in `turns`.
///
/// Walks `turns`, finds `User` items, and formats each as a `<weepcode_query>`
/// element with text content (from [`CompactionItem::text`]) and any
/// `<weepcode_file id="..." name="..." />` lines for the item's attachment
...
pub fn extract_user_queries_from_turns<T: CompactionItem>(
    ...
    let mut result = String::from("<weepcode_user_queries>\n");
    ...
    for turn in turns {
        ...
        result.push_str("<weepcode_query>");
    ...
    result.push_str("</weepcode_user_queries>");
    Some(result)
}

/// Walk `turns`, find any prior compaction summary items, extract their
/// `<weepcode_user_queries>` blocks via [`split_prior_compaction_text`], and
/// concatenate them.
///
...
pub struct SeparatedHistoryTurns<T> {
    /// `turns` with the `<weepcode_user_queries>` block stripped from every
    ...
    /// Concatenation of every `<weepcode_user_queries>` block found (in
    ...
    /// regardless of whether it contained a `<weepcode_user_queries>` block.
...
/// `<weepcode_user_queries>` block (preserved verbatim for the next summary)
...
///   loop, so the LLM never sees `<weepcode_user_queries>` from earlier rounds.
...
///   would see the prior `<weepcode_user_queries>` and tend to copy it into the
...
pub fn separate_prior_user_queries<T: CompactionItemBuilder>(
    ...
    for turn in turns {
        if turn.is_compaction_summary() {
            ...
            // `<weepcode_user_queries>` block (and therefore stripped to an empty
...
mod tests {
    ...
    fn extract_user_queries_wraps_single_user_turn() {
        ...
        assert!(out.starts_with("<weepcode_user_queries>"));
        assert!(out.ends_with("</weepcode_user_queries>"));
        assert!(out.contains("<weepcode_query>hello world</weepcode_query>"));
    }
    ...
    fn extract_prior_user_queries_concatenates_blocks() {
        let inner = "<weepcode_user_queries>\n<weepcode_query>first</weepcode_query>\n</weepcode_user_queries>";
        let inner2 = "<weepcode_user_queries>\n<weepcode_query>second</weepcode_query>\n</weepcode_user_queries>";
    ...
    fn separate_strips_user_queries_from_summary_item() {
        let prior = "<weepcode_user_queries>\n<weepcode_query>Q1</weepcode_query>\n</weepcode_user_queries>\n\n<chunk_summary index=\"0\">S1</chunk_summary>";
        ...
        assert!(prior.contains("<weepcode_user_queries>"));
        ...
        match &sep.turns_for_llm[0] {
            MockItem::Developer { text, summary } => {
                ...
                assert!(!text.contains("<weepcode_user_queries>"));
    ...
    fn separate_drops_summary_item_with_no_rest() {
        let only_queries =
            "<weepcode_user_queries>\n<weepcode_query>Q</weepcode_query>\n</weepcode_user_queries>";
