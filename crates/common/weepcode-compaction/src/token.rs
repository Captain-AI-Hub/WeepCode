...
//!
//! - WeepCode chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `WeepcodeTurn::get_num_tokens`.
//! - weepcode-build estimates with `bytes / 4`.
//!
...
//! (WeepCode chat: `ItemTokenCounter<Arc<WeepcodeTurn>>`), so exactly one mechanism
//! is in play.
