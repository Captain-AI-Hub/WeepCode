...
//! harness implements [`CompactionItem`] for its `WeepcodeTurn`;
//! weepcode-build implements it for `weepcode_sampling_types::ConversationItem`.
//!
...
//! `<weepcode_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `WeepcodeRole` (WeepCode chat) and the
/// `ConversationItem` variants (weepcode-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (WeepCode chat) — maps to System on harnesses without a
...
/// extraction (`<weepcode_file id=".." name=".." />` lines in the
/// `<weepcode_user_queries>` preamble).
...
///
/// Implementors:
/// - WeepCode chat: `WeepcodeTurn`
/// - weepcode-build: `ConversationItem`
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (WeepCode chat's
    /// `WeepcodeTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (WeepCode chat: a
    ...
    /// strips their `<weepcode_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<weepcode_file>` lines in the
    /// `<weepcode_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (WeepCode chat: a `Developer` turn with category
    ...
    /// requests/results (WeepCode chat: keep only `Channel` contents of an
...
/// constructs the typed harness items that make up weepcode-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by WeepCode chat's
/// `WeepcodeTurn`; adding these constructors to it as required methods would break
/// that impl. They are also weepcode-build-specific (WeepCode chat's tail-keep path
...
///
/// The weepcode-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (WeepCode chat stores turns as `Arc<WeepcodeTurn>`).
