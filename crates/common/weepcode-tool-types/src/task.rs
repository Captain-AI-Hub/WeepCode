...
/// A built-in subagent type shared by the CLI (`weepcode-agent`) and other
...
mod tests {
    ...
    fn task_tool_input_model_parses_explicit() {
        let input: TaskToolInput =
            serde_json::from_str(r#"{"description": "d", "prompt": "p", "model": "weepcode-3"}"#)
                .unwrap();
        assert_eq!(input.model.as_deref(), Some("weepcode-3"));
    }
    ...
    fn sanitize_optional_arg_moves_when_no_trim() {
        assert_eq!(
            sanitize_optional_arg(Some("weepcode-3".into())).as_deref(),
            Some("weepcode-3")
        );
        assert_eq!(
            sanitize_optional_arg(Some("  weepcode-3  ".into())).as_deref(),
    ...
    // match what the weepcode-shell MiniJinja templates render for the default
    // weepcode-build toolset (monitor + task + bash + read present, POSIX). The
