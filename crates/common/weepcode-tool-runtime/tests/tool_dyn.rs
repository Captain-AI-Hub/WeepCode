...
//! `WeepcodeToolDyn` / `WeepcodeToolFamily` tests but against the runtime's
//! typed `Tool` trait.
...
use weepcode_tool_protocol::{ToolCapabilities, ToolId};
...
async fn tool_dyn_delegates_id_description_capabilities() {
    ...
    assert_eq!(
        tool.description(&weepcode_tool_runtime::ListToolsContext::default())
            .name,
        "blocking_echo"
    );
