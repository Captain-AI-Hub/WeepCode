...
/// `filename`, `path`, `metadata`) that the WeepCode SLOP converter uses to
...
pub enum ContentBlock {
    ...
    ///
    /// Optional metadata fields are used by the WeepCode SLOP converter to
    ...
    /// - `path`: file path on the WeepCode Computer filesystem
