...
use weepcode_tool_protocol::{McpBlock, SessionId, ToolId, ToolOutputWire};
...
/// [`weepcode_computer_hub_sdk::ToolServerBuilder`] to register them
/// with the hub.
///
...
impl McpBridge {
    ...
    /// Handlers to register with a [`weepcode_computer_hub_sdk::ToolServerBuilder`].
    ///
...
#[async_trait]
impl weepcode_computer_hub_sdk::ToolServerHandler for McpToolHandler {
...
mod tests {
    ...
    async fn bridge_handles_transport_error() {
        ...
        let item = stream.next().await.unwrap();
        match item {
            weepcode_tool_runtime::ToolStreamItem::Terminal(Err(ref e))
                if e.kind == weepcode_tool_runtime::ToolErrorKind::Execution =>
            {
                assert!(
