...
/// v1.18: Added `shell_version`: the weepcode-shell agent binary version, distinct
///        from `client_version` (the UI client's version). They coincide for the
///        TUI but differ for embedding clients like weepcode-desktop.
...
/// OS-level sandbox state for a trace turn (local `weepcode-sandbox`, not cloud sandbox).
...
pub struct PromptMetadata {
    ...
    /// The agent type / harness name for this session (e.g. "weepcode-build", "codex").
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub agent_type: Option<String>,
    /// Version of the weepcode-shell agent binary that handled this turn
    /// (`weepcode_version::VERSION`). Self-reported by the agent, so it reflects
    ...
    /// like weepcode-desktop the bundled shell differs from the app version.
    #[serde(default, skip_serializing_if = "Option::is_none")]
